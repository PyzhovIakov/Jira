...
Функция GetLeadStructure(ServiceID, SourceID)
	...
	|	CRM_ИсточникиПолученияЛидовДополнительныеРеквизиты.Свойство КАК Свойство,
	|	CRM_ИсточникиПолученияЛидовДополнительныеРеквизиты.Обязательно КАК Обязательно
	|ИЗ
...
Функция LoadLead(ServiceID, SourceID, LeadStructure)
	...
	Если Заявка.ИДСобытия <> "" Тогда
		...
		Если НЕ НайденнаяЗаявка.Пустая() Тогда
			...
			Структура.Вставить("text", НСтр("ru='Заявка уже зарегистрированна в системе под номером';en='Request is already registered in system by number'") 
				+ " " 
				+ НайденнаяЗаявка.Номер);
	...
	Если ЗначениеЗаполнено(Заявка.UTMМетка) Тогда
		...
		Если ЗначениеЗаполнено(КаналПервичногоИнтереса) И ЗначениеЗаполнено(КампанияИД) Тогда
			ДанныеКампании = Новый Структура("Идентификатор, Наименование", КампанияИД, СтрМетка);
			СтруктураВозвр = CRM_СистемаСквознойАналитики.ПолучитьРекламнуюКампаниюПоИД(ДанныеКампании, 
		...
		Если ЗначениеЗаполнено(Заявка.КаналПервичногоИнтереса)
			 И Заявка.КаналПервичногоИнтереса.ТипЗначения.СодержитТип(Тип("СправочникСсылка.МаркетинговыеМероприятия")) Тогда
			Заявка.ИсточникПервичногоИнтереса = Справочники.МаркетинговыеМероприятия.НайтиПоНаименованию(СтрМетка);
		КонецЕсли;	
	...
	Структура.Вставить("status", "success");
	Структура.Вставить("text", НСтр("ru='Создана новая заявка в системе под номером';en='New request created in system with number'") 
		+ " " 
		+ Заявка.Номер);
...
Функция НайтиЗаявкуПоЗаполненнымРеквизитам(Телефон, ЭлектроннаяПочта, ЭтоЗвонок)
	...
	Иначе 
		...
		Запрос2.УстановитьПараметр("ТекДата", ТекущаяДатаСеанса());
