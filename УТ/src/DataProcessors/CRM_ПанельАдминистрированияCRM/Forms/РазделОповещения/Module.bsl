...
&НаКлиенте
Процедура CRM_ПоказыватьНапоминанияВЦентреОповещенийПриИзменении(Элемент)
	Подключаемый_ПриИзмененииРеквизита(Элемент);
КонецПроцедуры
...
Процедура CRM_ИспользоватьОповещенияЭлектроннаяПочтаПриИзменении(Элемент)
	Подключаемый_ПриИзмененииРеквизита(Элемент);
	Если НаборКонстант.CRM_ИспользоватьОповещенияЭлектроннаяПочта
		И ЗначениеЗаполнено(НаборКонстант.CRM_УчетнаяЗаписьОповещений) Тогда
		CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.Оповещения.Настройка.ЭлектроннаяПочта");
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура CRM_УчетнаяЗаписьОповещенийПриИзменении(Элемент)
	Если Не ПроверитьУчетнуюЗаписьОповещений() Тогда
		Возврат;
	КонецЕсли;
	Подключаемый_ПриИзмененииРеквизита(Элемент);
	Если НаборКонстант.CRM_ИспользоватьОповещенияЭлектроннаяПочта
...
Процедура CRM_ИспользоватьОповещенияСМСПриИзменении(Элемент)
	Подключаемый_ПриИзмененииРеквизита(Элемент);
	Если НаборКонстант.CRM_ИспользоватьОповещенияСМС И ЗначениеЗаполнено(НаборКонстант.CRM_НомерТелефонаОповещений) Тогда
		CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.Оповещения.Настройка.SMS");
	КонецЕсли;
КонецПроцедуры
...
Процедура CRM_НомерТелефонаОповещенийНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	ЗаполнитьСписокНомеровОповещений(СтандартнаяОбработка);
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьСписокНомеровОповещений(СтандартнаяОбработка = Неопределено)
...
&НаКлиенте
Процедура CRM_СрокАктуальностиНапоминанийПоУмолчаниюПриИзменении(Элемент)
	Подключаемый_ПриИзмененииРеквизита(Элемент);
КонецПроцедуры

&НаКлиенте
Процедура CRM_ИспользоватьОповещенияМессенджерПриИзменении(Элемент)
	Подключаемый_ПриИзмененииРеквизита(Элемент);
	Если НаборКонстант.CRM_ИспользоватьОповещенияМессенджер 
		И ЗначениеЗаполнено(НаборКонстант.CRM_УчетнаяЗаписьМессенджераОповещений) Тогда
		CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.Оповещения.Настройка.Мессенджер");
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура CRM_УчетнаяЗаписьМессенджераОповещенийПриИзменении(Элемент)
	Подключаемый_ПриИзмененииРеквизита(Элемент);
	Если НаборКонстант.CRM_ИспользоватьОповещенияМессенджер 
		И ЗначениеЗаполнено(НаборКонстант.CRM_УчетнаяЗаписьМессенджераОповещений) Тогда
		CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.Оповещения.Настройка.Мессенджер");
...
Процедура УстановитьДоступность(РеквизитПутьКДанным = "")
	...
	Если РеквизитПутьКДанным = "НаборКонстант.CRM_ИспользоватьНапоминания" Или РеквизитПутьКДанным = "" Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Элементы, "ГруппаИспользоватьНапоминания",
			"Видимость", НаборКонстант.CRM_ИспользоватьНапоминания);
	КонецЕсли;
	
	Если РеквизитПутьКДанным = "НаборКонстант.CRM_ИспользоватьОповещенияЭлектроннаяПочта"
		Или РеквизитПутьКДанным = "" Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Элементы, "ГруппаИспользоватьОповещенияEmail",
			"Видимость", НаборКонстант.CRM_ИспользоватьОповещенияЭлектроннаяПочта);
	КонецЕсли;
	
	Если РеквизитПутьКДанным = "НаборКонстант.CRM_ИспользоватьОповещенияСМС" Или РеквизитПутьКДанным = "" Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Элементы, "ГруппаИспользоватьОповещенияSMS",
			"Видимость", НаборКонстант.CRM_ИспользоватьОповещенияСМС);
	КонецЕсли;
	
	Если РеквизитПутьКДанным = "НаборКонстант.CRM_ИспользоватьОповещенияМессенджер" Или РеквизитПутьКДанным = "" Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Элементы, "CRM_УчетнаяЗаписьМессенджераОповещений",
			"Видимость", НаборКонстант.CRM_ИспользоватьОповещенияМессенджер);
	КонецЕсли;
...
Функция ПроверитьУчетнуюЗаписьОповещений()
	Если ЗначениеЗаполнено(НаборКонстант.CRM_СервисРассылокОповещенийПоУмолчанию)
		...
		И НЕ CRM_ИнтеграцияССервисамиРассылок.ПроверитьАдресУчетнойЗаписи(НаборКонстант.CRM_СервисРассылокОповещенийПоУмолчанию,
			 НаборКонстант.CRM_УчетнаяЗаписьОповещений.АдресЭлектроннойПочты) Тогда
		Возврат Ложь;
	КонецЕсли;
	Возврат Истина;
КонецФункции
