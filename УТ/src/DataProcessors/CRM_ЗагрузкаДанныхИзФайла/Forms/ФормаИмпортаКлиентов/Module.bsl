...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ CRM_ЛицензированиеЭкспортныеМетоды.ПодсистемаCRMИспользуется() Тогда
		CRM_ЛицензированиеЭкспортныеМетоды.ВывестиУведомлениеОНедоступности(НСтр("ru = 'форму загрузки'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	
	Если Параметры.Свойство("РежимЗагрузкиВДокумент") Тогда
		РежимЗагрузкиВДокумент = Параметры.РежимЗагрузкиВДокумент;
		
		СкрытьНеИспользуемыеВРежимеРеквизиты = Не РежимЗагрузкиВДокумент;
		
		Элементы.Декорация2.Видимость = СкрытьНеИспользуемыеВРежимеРеквизиты;
		Элементы.Декорация3.Видимость = СкрытьНеИспользуемыеВРежимеРеквизиты;
		
		Элементы.Группа1.Видимость   = СкрытьНеИспользуемыеВРежимеРеквизиты;
		Элементы.ГруппаНиз.Видимость = СкрытьНеИспользуемыеВРежимеРеквизиты;
...
Процедура ПередЗагрузкойДанныхИзНастроекНаСервере(Настройки)
	...
	Если ТаблицаСоответствия <> Неопределено Тогда
		...
		Для каждого Строка Из ТаблицаСоответствия Цикл
			Строки = СопоставлениеРеквизитов.НайтиСтроки(Новый Структура("ПредставлениеКолонкиФайла",
				 Строка.ПредставлениеКолонкиФайла));
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если РежимЗагрузкиВДокумент Тогда
		...
		Если СопоставлениеРеквизитов.НайтиСтроки(Новый Структура("Сопоставлен", Истина)).Количество() = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не сопоставлены поля справочников с колонками файла!';
				|en='Catalogs fields are not matching to file columns!'"));
			Отказ = Истина;
		КонецЕсли;
		
		Если СопоставлениеРеквизитов.НайтиСтроки(Новый Структура("Сопоставлен, ИмяРеквизита", Истина,
			 "Клиент")).Количество() = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("en = 'There is no matching with the client!';
				| ru = 'Нет сопоставления с клиентом!'"));
			Отказ = Истина;
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ЮрФизЛицо) Тогда
		...
		Для каждого Строка Из СтрокиОбязательных Цикл
			Если СопоставлениеРеквизитов.НайтиСтроки(Новый Структура("Сопоставлен, Объект", Истина,
				 Строка.Объект)).Количество() > 0 Тогда
				Если СопоставлениеРеквизитов.НайтиСтроки(Новый Структура("Сопоставлен, Объект, ИмяРеквизита",
					 Истина, Строка.Объект,
					 Строка.ИмяРеквизита)).Количество() = 0 Тогда
					Если РеквизитыЗаполняемыеПоУмолчанию.НайтиПоЗначению(Строка.Объект + "." 
						+ Строка.ИмяРеквизита) = Неопределено Тогда
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрШаблон(НСтр("ru='Не сопоставлено обязательное для заполнения поле ""%1""!';
							|en='Required field ""%1"" is not compared!'"),
...
Процедура ВыбратьФайл(Команда)
	...
	Если СопоставлениеРеквизитов.Количество() > 0 Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения,
			...
			|en='All ma before comparison will be lost. Continue?'"),
...
Процедура Загрузить(Команда)
	...
	Если ПроверитьЗаполнение() Тогда
		Если РежимЗагрузкиВДокумент Тогда
			МассивДанных = ПолучитьЗаписиДляПередачиВДокумент();
			Закрыть(МассивДанных);
			Возврат;
		КонецЕсли;
		
		Если СтрокВЗагружаемомФайле > 50 Тогда
			CRM_ОбщегоНазначенияКлиент.ПоказатьНашеПредупреждение(НСтр("ru='Можно продолжать использовать систему,
				| по окончании загрузки появится оповещение."
...
"Operation time may be long due to file size.'"), 5, НСтр("ru='Импорт клиентов из файла начался'"));
		...
		ИначеЕсли ДлительнаяОперация.Статус = "Выполняется" Тогда
			...
			ОповещениеОЗавершении = Новый ОписаниеОповещения("ВывестиСообщениеПоОкончанииФоновогоЗадания",
		...
		ИначеЕсли ДлительнаяОперация.Статус = "Выполняется" Тогда
			ТекстСостояния = НСтр("en = 'Import Customers from File'; ru = 'Импорт клиентов из файла'");
			ТекстПояснения = НСтр("ru = 'Выполняется загрузка клиентов...'; en = 'Сlients are being loaded...'");
			Состояние(ТекстСостояния, , ТекстПояснения, БиблиотекаКартинок.ДлительнаяОперация48);
		КонецЕсли;
	КонецЕсли;
...
Процедура СопоставлениеРеквизитовПредставлениеРеквизитаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Для Каждого Строка Из СопоставлениеРеквизитов Цикл
		Если Строка(Строка.ТипРеквизита) <> "Строка" Или РежимЗагрузкиВДокумент Тогда
			МассивРеквизитов.Добавить(Строка.ПредставлениеРеквизита);
		КонецЕсли;
	КонецЦикла;
	
	Если ЗагрузкаВДокумент = "Телемаркетинг" Тогда
		МассивРеквизитов.Добавить("Электронная почта");
	ИначеЕсли ЗагрузкаВДокумент = "РассылкаЭлектронныхПисем" Тогда
		МассивРеквизитов.Добавить("Телефон");
	ИначеЕсли ЗагрузкаВДокумент = "СписокРассылок" Тогда
		МассивРеквизитов.Добавить("Контакт (Наименование)");
		МассивРеквизитов.Добавить("Телефон");
	КонецЕсли;
	
	ПараметрыОткрытия.Вставить("ИсключитьРеквизиты", МассивРеквизитов);
	ПараметрыОткрытия.Вставить("РежимЗагрузкиВДокумент", РежимЗагрузкиВДокумент);
	
	ОткрытьФорму("Обработка.CRM_ЗагрузкаДанныхИзФайла.Форма.ФормаВыбораРеквизита", ПараметрыОткрытия,
		 Элемент, , , , ,
...
Процедура СценарийПродажНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("Справочник.CRM_СостоянияИнтересов.Форма.ФормаВыбора", ПараметрыФормыВыбора, Элемент,
...
Процедура ВидимостьЭлементов()
	Элементы.СценарийПродаж.Видимость = (СопоставлениеРеквизитов.НайтиСтроки(Новый Структура("Объект,
		| ИмяРеквизита", "Документ.CRM_Интерес",
		 "Тема")).Количество() > 0);
	//Элементы.ИскатьПоИННКПП.Видимость = (СопоставлениеРеквизитов.НайтиСтроки(Новый Структура("Объект,
		//| ИмяРеквизита", "Справочник.Партнеры",
		// "CRM_ИНН")).Количество() > 0);
	Элементы.ИскатьПоТелефону.Видимость = (СопоставлениеРеквизитов.НайтиСтроки(Новый Структура("ТипРеквизита",
		 ТипКИТелефон)).Количество() > 0); 
	Элементы.ИскатьПоEmail.Видимость = (СопоставлениеРеквизитов.НайтиСтроки(Новый Структура("ТипРеквизита",
		 ТипКИEMail)).Количество() > 0);
...
Процедура ПослеЗагрузкиФайлаСДаннымиВТаблицуЗначенийСервер(АдресТаблицыСДанными)
	...
	Если ЗначениеЗаполнено(ЮрФизЛицо) И Не РежимЗагрузкиВДокумент Тогда
		ЮрФизЛицоПриИзмененииНаСервере();
	КонецЕсли;
...
Процедура ВыборФайлаПродолжение(ПодключеноРасширениеРаботыСФайлами, ДополнительныеПараметры) Экспорт
	...
	ПараметрыЗагрузки.Диалог.Фильтр = НСтр("ru='Все поддерживаемые форматы файлов(*.xls;
		|*.xlsx;*.ods;*.mxl;*.csv;*.txt)|*.xls;*.xlsx;*.ods;*.mxl;*.csv;*.txt|Книга Excel 97 (*.xls)|*.xls|Книга Excel 2007 (*.xlsx)|*.xlsx|Электронная таблица OpenDocument (*.ods)|*.ods|Текстовый документ c разделителями (*.csv;*.txt)|*.csv;*.txt|Табличный документ (*.mxl)|*.mxl'");
...
Функция ЗагрузитьНаСервере()
	...
	ПараметрыМетода.Вставить("СопоставлениеРеквизитов",
		 РеквизитФормыВЗначение("СопоставлениеРеквизитов",
		 Тип("ТаблицаЗначений")));
...
&НаСервере
Функция ПолучитьЗаписиДляПередачиВДокумент()
	
	ТаблицаДанных = ПолучитьИзВременногоХранилища(АдресТаблицыСДанными);
	Сопоставления = РеквизитФормыВЗначение("СопоставлениеРеквизитов", Тип("ТаблицаЗначений"));
	
	Обработки.CRM_ЗагрузкаДанныхИзФайла.ЗаполнитьЗначенияРеквизитовВТаблице(Сопоставления, ТаблицаДанных);
	
	Для Каждого ТекущаяСтрокаСопоставления Из Сопоставления Цикл
		НайденнаяКолонка = ТаблицаДанных.Колонки.Найти(ТекущаяСтрокаСопоставления.ИмяКолонкиФайла);
		
		Если НайденнаяКолонка <> Неопределено Тогда
			НайденнаяКолонка.Имя = ТекущаяСтрокаСопоставления.ИмяРеквизита + "Колонка";
		КонецЕсли;
	КонецЦикла;
	
	МассивПередачиДанных = Новый Массив;
	
	КолонкаНаименованияСуществует	= (ТаблицаДанных.Колонки.Найти("КлиентКолонка") <> Неопределено);
	КолонкаКонтактаСуществует		= (ТаблицаДанных.Колонки.Найти("КонтактКолонка") <> Неопределено);
	КолонкаТелефонаСуществует		= (ТаблицаДанных.Колонки.Найти("ТелефонКолонка") <> Неопределено);
	КолонкаПочтыСуществует			= (ТаблицаДанных.Колонки.Найти("ПочтаКолонка") <> Неопределено);
	
	Для Каждого ТекущаяСтрокаДанных Из ТаблицаДанных Цикл
		НаименованиеКлиента			= ?(КолонкаНаименованияСуществует,	СокрЛП(ТекущаяСтрокаДанных.КлиентКолонка),					"");
		НаименованиеКонтактногоЛица	= ?(КолонкаКонтактаСуществует,		СокрЛП(ТекущаяСтрокаДанных.КонтактКолонка),					"");
		АдресаЭлектроннойПочты		= ?(КолонкаПочтыСуществует,			СокрЛП(ТекущаяСтрокаДанных.ПочтаКолонка), 					"");
		ТелефонныеНомераСтрокой		= ?(КолонкаТелефонаСуществует,		СокрЛП(ТекущаяСтрокаДанных.ТелефонКолонка),					"");
		МассивТелефонныхНомеров		= ?(КолонкаТелефонаСуществует,		СтрРазделить(ТелефонныеНомераСтрокой,
			 РазделительКИ, Ложь),
				Новый Массив);
		
		НайденныйКлиент		= Справочники.Партнеры.НайтиПоНаименованию(НаименованиеКлиента, Истина);
		НайденныйКонтакт	= Справочники.КонтактныеЛицаПартнеров.ПустаяСсылка();
		
		ПартнерСтрокой = НаименованиеКлиента;
		КонтактСтрокой = НаименованиеКонтактногоЛица;
		
		Если ЗначениеЗаполнено(НайденныйКлиент) И КолонкаКонтактаСуществует Тогда
			НайденныйКонтакт = Справочники.КонтактныеЛицаПартнеров.НайтиПоНаименованию(СокрЛП(НаименованиеКонтактногоЛица),
				 Истина, ,
				 НайденныйКлиент.Владелец);
		КонецЕсли;
		
		ВыбранныйТелефонныйНомер = ?(МассивТелефонныхНомеров.Количество() > 0, СокрЛП(МассивТелефонныхНомеров[0]), "");
		
		СтруктураДанных = Новый Структура();
		СтруктураДанных.Вставить("Партнер",			НайденныйКлиент);
		СтруктураДанных.Вставить("Контакт",			НайденныйКонтакт);
		СтруктураДанных.Вставить("ПартнерСтрокой",	ПартнерСтрокой);
		СтруктураДанных.Вставить("КонтактСтрокой",	КонтактСтрокой);
		СтруктураДанных.Вставить("Телефон",			ВыбранныйТелефонныйНомер);
		СтруктураДанных.Вставить("ТелефонСтрокой",	ТелефонныеНомераСтрокой);
		СтруктураДанных.Вставить("Почта",			АдресаЭлектроннойПочты);
...
КонецФункции // ПолучитьЗаписиДляПередачиВДокумент()
