...
Процедура ВыполнитьКонвертациюНаСервере()
	...
	Пока РезультатЗапроса.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(РезультатЗапроса.Автор) Тогда
			ВзаимодействиеОбъект.Организация = CRM_ОбщегоНазначенияПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(РезультатЗапроса.Автор,
				 "ОсновнаяОрганизация");
		КонецЕсли;
		...
		Если ЭтоЗвонок Тогда
			...
			ВзаимодействиеОбъект.Содержание = НСтр("ru='Для связи:';en='Contact:'") + РезультатЗапроса.АбонентКакСвязаться + Символы.ПС 
				+ Символы.ПС 
				+ ВзаимодействиеОбъект.Содержание;
		...
		Иначе
			...
			Пока ВыборкаУчастники.Следующий() Цикл
				...
				ИначеЕсли ТипЗнч(Контакт) = Тип("СправочникСсылка.Пользователи")
		...
		ВзаимодействиеОбъект.Содержание = НСтр("ru='Перенесено из документа: ';en='Transferred from document: '") + РезультатЗапроса.Представление + Символы.ПС 
			+ Символы.ПС 
			+ ВзаимодействиеОбъект.Содержание;
		...
		Исключение
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Не удалось записать документ ';en='Failed to write document '") 
				+ ВзаимодействиеОбъект);
...
Процедура ВыполнитьКонвертацию(Команда)
	...
	ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Выполняется конвертация взаимодействий...';
		|en='Interaction conversion in progress...'"));
