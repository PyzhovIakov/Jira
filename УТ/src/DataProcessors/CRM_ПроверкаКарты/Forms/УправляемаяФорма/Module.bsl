...
Процедура ВыборФайла(Элемент, ИмяСвойства, ПроверятьСуществование = Ложь, Знач РасширениеПоУмолчанию = "txt",
	...
	ДополнительныеПараметры = Новый Структура("Элемент, ИмяСвойства, ПроверятьСуществование,
		| РасширениеПоУмолчанию, АрхивироватьФайлДанных, ВыборФайлаПравил",
...
Функция ПолучитьЗначениеПоПозиции(стр, Позиция)
	...
	Для Ном = 1 По Позиция Цикл
		Поз_1 = СтрНайти(стр_1, ",");
		Если Поз_1 = 0 Тогда
			Возврат 0;
		КонецЕсли;
		стр_1 = Сред(стр_1, Поз_1 + 1, стрДлина);
	КонецЦикла;
	Поз_2 = СтрНайти(стр_1, ",");
	Если Поз_2 = 0 Тогда
		Возврат стр_1;
	КонецЕсли;
...
Функция ПолучитьИсходящиеЭлементы(ТекущийЭлемент, СхемаМаршрута)
	...
	МассивЛиний			= ТаблицаЭлементов.НайтиСтроки(Новый Структура("Тип, НачалоЭлемент",
		 Тип("ЭлементГрафическойСхемыСоединительнаяЛиния"),
		 ТекущийЭлемент));
...
Функция ПолучитьИсходящиеЗаполнитьДерево(ТекущийЭлемент,
	...
	Если НЕ (ТипЗнч(ТекущийЭлемент) = Тип("ЭлементГрафическойСхемыСлияние"))
		 И (ДеревоТочекЗнач.Строки.Найти(ТекущийЭлемент.Имя, "ИмяЭлемента", Истина) = Неопределено) Тогда
	//Если НЕ (ТипЗнч(ТекущийЭлемент) = Тип("ЭлементГрафическойСхемыСлияние")) Тогда
		...
		ИЛИ ТипЗнч(ТекущийЭлемент) = Тип("ЭлементГрафическойСхемыВложенныйБизнесПроцесс") Тогда
			...
			ТекущийЭлемент = ПолучитьИсходящиеЗаполнитьДерево(ИсходящиеЭлементы[0], СхемаМаршрута,
				 ТекущаяСтрокаДерева, ДеревоТочекЗнач, Разделение, ИндексВетки, ВариантНаименование,
				 ВариантЗначение);
			...
			Для Каждого ЭлементМассива Из ИсходящиеЭлементыРазделения Цикл
				...
				Если НЕ КонечныйЭлементВетки = Неопределено Тогда
					...
					Если КонечныйЭлементВетки.Имя = ("Слияние" + СтрЗаменить(ТекущийЭлемент.Имя, "Разделение", "")) Тогда
						...
						МассивСтрок = СтрокаВетокРазделения.Строки.НайтиСтроки(Новый Структура("Разделение,Слияние",
							 ТекущийЭлемент.Имя, ""),
							 Истина);
...
Функция СформироватьТаблицуЭлементовГрафическойСхемы(СхемаМаршрута)
	...
	Для Каждого ЭлементСхемы Из СхемаМаршрута.ЭлементыГрафическойСхемы Цикл
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "ЦветФона") Тогда
			НоваяСтрокаЭлемента.ЦветФона = ЭлементСхемы.ЦветФона;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "ЦветРамки") Тогда
			НоваяСтрокаЭлемента.ЦветРамки = ЭлементСхемы.ЦветРамки;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "ГоризонтальноеПоложение") Тогда
			НоваяСтрокаЭлемента.ГоризонтальноеПоложение	= ЭлементСхемы.ГоризонтальноеПоложение;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "ВертикальноеПоложение") Тогда
			НоваяСтрокаЭлемента.ВертикальноеПоложение = ЭлементСхемы.ВертикальноеПоложение;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "ПоложениеКартинки") Тогда
			НоваяСтрокаЭлемента.ПоложениеКартинки = ЭлементСхемы.ПоложениеКартинки;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "РазмерКартинки") Тогда
			НоваяСтрокаЭлемента.РазмерКартинки = ЭлементСхемы.РазмерКартинки;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "Картинка") Тогда
			НоваяСтрокаЭлемента.Картинка = ЭлементСхемы.Картинка;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "Лево") Тогда
			НоваяСтрокаЭлемента.Лево = ЭлементСхемы.Лево;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "Верх") Тогда
			НоваяСтрокаЭлемента.Верх = ЭлементСхемы.Верх;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "Ширина") Тогда
			НоваяСтрокаЭлемента.Ширина = ЭлементСхемы.Ширина;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "Высота") Тогда
			НоваяСтрокаЭлемента.Высота = ЭлементСхемы.Высота;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "ПрозрачныйФон") Тогда
			НоваяСтрокаЭлемента.ПрозрачныйФон = ЭлементСхемы.ПрозрачныйФон;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "Гиперссылка") Тогда
			НоваяСтрокаЭлемента.Гиперссылка = ЭлементСхемы.Гиперссылка;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "Рамка") Тогда
			НоваяСтрокаЭлемента.Рамка = ЭлементСхемы.Рамка;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "Значение") Тогда
			НоваяСтрокаЭлемента.Значение = ЭлементСхемы.Значение;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "ЦветЛинии") Тогда
			НоваяСтрокаЭлемента.ЦветЛинии = ЭлементСхемы.ЦветЛинии;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "Линия") Тогда
			НоваяСтрокаЭлемента.Линия = ЭлементСхемы.Линия;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "ПоложениеТекста") Тогда
			НоваяСтрокаЭлемента.ПоложениеТекста = ЭлементСхемы.ПоложениеТекста;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "КонецЭлемент") Тогда
			НоваяСтрокаЭлемента.КонецЭлемент = ЭлементСхемы.КонецЭлемент;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "КонецСторона") Тогда
			НоваяСтрокаЭлемента.КонецСторона = ЭлементСхемы.КонецСторона;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "КонецВариант") Тогда
			НоваяСтрокаЭлемента.КонецВариант = ЭлементСхемы.КонецВариант;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "КонецЛево") Тогда
			НоваяСтрокаЭлемента.КонецЛево = ЭлементСхемы.КонецЛево;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "КонецВерх") Тогда
			НоваяСтрокаЭлемента.КонецВерх = ЭлементСхемы.КонецВерх;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "НачалоЭлемент") Тогда
			НоваяСтрокаЭлемента.НачалоЭлемент = ЭлементСхемы.НачалоЭлемент;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "НачалоСторона") Тогда
			НоваяСтрокаЭлемента.НачалоСторона = ЭлементСхемы.НачалоСторона;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "НачалоВариант") Тогда
			НоваяСтрокаЭлемента.НачалоВариант = ЭлементСхемы.НачалоВариант;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "НачалоЛево") Тогда
			НоваяСтрокаЭлемента.НачалоЛево = ЭлементСхемы.НачалоЛево;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "НачалоВерх") Тогда
			НоваяСтрокаЭлемента.НачалоВерх = ЭлементСхемы.НачалоВерх;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "Пояснение") Тогда
			НоваяСтрокаЭлемента.Пояснение = ЭлементСхемы.Пояснение;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементСхемы, "Групповая") Тогда
...
&НаСервере
// Процедура читает графическую схему дерева точек.
//
// Параметры:
//	СхемаМаршрута	- Файл	- Схема маршрута.
//
Процедура ПрочитатьГрафическуюСхемуСформироватьДеревоТочек(СхемаМаршрута, ПутьКВременномуФайлу)
...
&НаСервере
Процедура ПрочитатьДеревоПроверить(Знач ТекущаяСтрокаДерева,
								 ДеревоТочекЗнач,
								 Знач Разделение = "",
								 Знач ИндексВетки = 0,
	...
	Для Каждого СтрокаДерева Из ТекущаяСтрокаДерева.Строки Цикл
		...
		ИЛИ СтрокаДерева.ВидЭлемента = "ВыборВарианта" Тогда
			...
			ИЛИ ПредыдущийВидЭлемента = "Обработка" Тогда
				...
				НоваяСтрокаОшибки.ОписаниеОшибки	= "Условие/Выбор варианта идет после недопустимой точки (старт,
					| разделение, слияние, обработка,
					| условие/выбор варианта)";
	...
	Для Каждого СтрокаЭлемента Из ТаблицаЭлементов Цикл
		...
		ИЛИ СтрокаЭлемента.Тип = Тип("ЭлементГрафическойСхемыВыборВарианта") Тогда
			...
			Если СтрокаЭлемента.Тип = Тип("ЭлементГрафическойСхемыСтарт") Тогда
				МассивИсходящихЛиний	= ТаблицаЭлементов.НайтиСтроки(Новый Структура("Тип, НачалоЭлемент",
					 Тип("ЭлементГрафическойСхемыСоединительнаяЛиния"),
			...
			ИначеЕсли СтрокаЭлемента.Тип = Тип("ЭлементГрафическойСхемыЗавершение") Тогда
				МассивВходящихЛиний		= ТаблицаЭлементов.НайтиСтроки(Новый Структура("Тип, КонецЭлемент",
					 Тип("ЭлементГрафическойСхемыСоединительнаяЛиния"),
					 СтрокаЭлемента.Элемент));
...
Процедура ПодключаемыйСтекОшибокПриАктивизацииСтроки()
	...
	Если ДеревоЗаполнено Тогда
		...
		ОбщегоНазначенияКлиентСервер.ПолучитьИдентификаторСтрокиДереваПоЗначениюПоля("ИмяЭлемента",
			 ИдентификаторСтроки, КоллекцияЭлементов, ТекущиеДанные.ИмяЭлемента,
			 Ложь);
...
Процедура СхемаМаршрутаПриАктивизации(Элемент)
	...
	Если ДеревоЗаполнено Тогда
		...
		ОбщегоНазначенияКлиентСервер.ПолучитьИдентификаторСтрокиДереваПоЗначениюПоля("ИмяЭлемента",
			 ИдентификаторСтроки, КоллекцияЭлементов, ЭлементСхемы.Имя,
			 Ложь);
