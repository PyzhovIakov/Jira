...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Мероприятие) Тогда
		ОткрытьФорму("Обработка.CRM_ЖурналПосещений.Форма.ФормаНастройки", , , ЭтотОбъект, , ,
			 Новый ОписаниеОповещения("НастроитьПараметрыЗакрытие", ЭтотОбъект,
			 Новый Структура));
	КонецЕсли;	
КонецПроцедуры
...
Процедура ПроверитьЖурналПосещенийСервер()
	...
	Для каждого Строка Из ТабЖурнала.Строки Цикл
		...
		Для Каждого СтрокаДата Из Строка.Строки Цикл
			...
			Если СтрНайти(СтрокаДата.КлиентНаименование, "Посетитель №") = 0 Тогда
