...
Процедура СписокПосетителейПриАктивизацииСтроки(Элемент)
	...
	Если Стр <> Неопределено Тогда
		...
		Заявка = Стр.Ссылка;
		Если ЗначениеЗаполнено(Заявка) Тогда
			Элементы.ДекорацияГиперссылкаЗаявка.Заголовок = "Заявка от " 
				+ Формат(CRM_ОбщегоНазначенияСервер.ПолучитьЗначениеРеквизита(Заявка, "Дата"), "ДЛФ=DD"); 
		Иначе
...
Процедура ПолучитьРеквизитыКлиента(ПотенциальныйКлиент)
	...
	Иначе
		Телефон = "";
	КонецЕсли;
	Стр = ПотенциальныйКлиент.КонтактнаяИнформация.Найти(Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты,
		 "Тип");
	Если Стр <> Неопределено Тогда
...
Процедура УстановитьОтборы()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(СписокПосетителей,
		 "Мероприятие", Объект.Мероприятие,
		 Истина);
	//Элементы.ЗаполнитьИзРегистрации.Видимость = ОтображатьЗарегистрированных;
	//Элементы.СписокЗарегистрированных.Видимость = ОтображатьЗарегистрированных;
...
Процедура СозданиеЗаявкиОкончание(ДанныеВыбора, ДопПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ДанныеВыбора) Тогда
		Элементы.ДекорацияГиперссылкаЗаявка.Заголовок = "Заявка от " 
			+ Формат(CRM_ОбщегоНазначенияСервер.ПолучитьЗначениеРеквизита(ДанныеВыбора, "Дата"), "ДЛФ=DD");
	Иначе
		Элементы.ДекорацияГиперссылкаЗаявка.Заголовок = "Создать заявку";
	КонецЕсли;
КонецПроцедуры
