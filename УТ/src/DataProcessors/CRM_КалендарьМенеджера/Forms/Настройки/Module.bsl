...
Процедура КомандаПоУмолчанию(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения,
		 НСтр("ru='Будут загружены настройки по умолчанию. Продолжить?';
		|en='The default settings will be unloaded. Continue?'"), РежимДиалогаВопрос.ДаНет, ,
...
Процедура ЗаполнитьРеквизитыПоНастройкам(СтруктураНастройки)
	Если ТипЗнч(СтруктураНастройки) = Тип("Структура") Тогда
		// Настройки отображения
		Если СтруктураНастройки.Свойство("НастройкиОтображения")
			 И ТипЗнч(СтруктураНастройки.НастройкиОтображения) = Тип("Структура") Тогда
			   ...
			   И СтруктураНастройкиПоУмолчанию.НастройкиОтображения.Свойство("СписокПредставлениеСобытияВКалендаре") Тогда
				...
				Для Каждого СтрСписка Из СтруктураНастройкиПоУмолчанию.НастройкиОтображения.СписокПредставлениеСобытияВКалендаре Цикл
					...
					Если НайденнаяСтр = Неопределено Тогда
						НовыйСтрСписка = СписокПредставлениеСобытияВКалендаре.Добавить(СтрСписка.Значение,
							 СтрСписка.Представление, СтрСписка.Пометка,
							 СтрСписка.Картинка);
					Иначе
		...
		// Общие настройки
		Если СтруктураНастройки.Свойство("ОбщиеНастройки")
			 И ТипЗнч(СтруктураНастройки.ОбщиеНастройки) = Тип("Структура") Тогда
	...
	НастройкиРабочееВремяПользователя =
		CRM_ОбщегоНазначенияСервер.ПолучитьСтруктуруРабочихВременПользователя(Пользователь);
...
Функция ПроверитьСохранить()
	...
	Если ОграничениеВремениС > ОграничениеВремениПо Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Время начала ограничения больше времени окончания ограничения!';
			|en='Starting time of restriction are more than ending time of restriction!'"));
		Результат = Ложь;
	КонецЕсли;
	
	Если РабочийДеньС > РабочийДеньПо Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Время начала рабочего времени больше времени окончания рабочего времени!';
			|en='Starting time of working hours are more than ending time of working hours!'"));
		Результат = Ложь;
	КонецЕсли;
...
Процедура ЗаполнитьСписокВыбораВремени(Элемент)
	...
	Для нИндекс = 0 По 23 Цикл
		Элемент.СписокВыбора.Добавить(Дата("00010101") + нИндекс * 60 * 60, Формат(Дата("00010101") 
			+ нИндекс * 60 * 60, "ДФ=HH:mm; ДП=00:00"));
	КонецЦикла;
КонецПроцедуры
