...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ CRM_ЛицензированиеЭкспортныеМетоды.ПодсистемаCRMИспользуется() Тогда
		CRM_ЛицензированиеЭкспортныеМетоды.ВывестиУведомлениеОНедоступности(НСтр("ru = 'Календарь'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если Параметры.Свойство("РежимВыбора") Тогда
		...
		Если РежимВыбора Тогда
			Параметры.Свойство("РежимВыбора_ОбъектАдресат",			РежимВыбора_ОбъектАдресат);
			Параметры.Свойство("РежимВыбора_ДатаИВремяНачала",		РежимВыбора_ДатаИВремяНачала);
			Параметры.Свойство("РежимВыбора_ДатаИВремяЗавершения",	РежимВыбора_ДатаИВремяЗавершения);
			Параметры.Свойство("РежимВыбора_ВесьДень",				РежимВыбора_ВесьДень);
		КонецЕсли;
	Иначе
		РежимВыбора = Ложь;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ОтборЗначениеПодразделение) Тогда
		ОтборЗначениеПользователь = ТекущийПользователь;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ОтборЗначениеПользователь) Тогда
		СписокПользователей.ЗагрузитьЗначения(ОбновитьСписокПользователей(ОтборЗначениеПользователь));
	КонецЕсли;
	...
	КонецЕсли;
	CRM_СобытияФорм.ПриСозданииНаСервере(ЭтотОбъект, Отказ, СтандартнаяОбработка, СтруктураПараметров);
	// -Рабочий стол
	...
	Элементы.ОтборЗначениеПодразделение.Видимость = ИспользоватьПодразделения;
	Элементы.ОтборЗначениеПроект.Видимость = ПолучитьФункциональнуюОпцию("CRM_РаботаВЛокальномРежиме");
	Элементы.ГруппаПоиск.Видимость = ПолучитьФункциональнуюОпцию("ИспользоватьПолнотекстовыйПоиск");
	
	Если Параметры.Свойство("CRM_КонтекстВызова") Тогда
		Если Параметры.CRM_КонтекстВызова = "Органайзер" Тогда
			CRM_ЦентрМониторингаВызовСервера.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.Взаимодействия.КалендарьСинхронизацияСGoogle.ОткрытиеВОдинКлик.ОткрытиеИзРазделаОрганайзер");
		КонецЕсли;
	Иначе
		CRM_ЦентрМониторингаВызовСервера.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.Взаимодействия.КалендарьСинхронизацияСGoogle.ОткрытиеВОдинКлик.ОткрытиеБезКонтекста");
	КонецЕсли;
	
	ИндексППДАктуален = ИндексППДАктуален();
...
Процедура ПриОткрытии(Отказ)
	...
	Если ДеревоРесурсовЭлементы.Количество() > 0 Тогда
		...
		ЭлементПользователи = Неопределено;
		Для Каждого ТекЭлемент Из ДеревоРесурсовЭлементы Цикл
			Если ТекЭлемент.Имя = "Пользователь" Тогда
				ЭлементПользователи = ТекЭлемент;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		Если ЭлементПользователи = Неопределено Тогда
			Элементы.ДеревоРесурсов.ТекущаяСтрока = ДеревоРесурсовЭлементы[0].ПолучитьИдентификатор();
		Иначе
			Элементы.ДеревоРесурсов.ТекущаяСтрока = ЭлементПользователи.ПолучитьИдентификатор();
		КонецЕсли;
	КонецЕсли;
	...
	Элементы.Календарь.ВыделенныеДаты.Добавить(ОбщегоНазначенияКлиент.ДатаСеанса());
	...
	Если НастройкиОбщие.Свойство("Автообновление") И НастройкиОбщие.Автообновление = Истина
		...
		ПериодАвтообновления	= Макс(НастройкиОбщие.ПериодАвтообновления * КоличествоСекундВМинуте, КоличествоСекундВМинуте);
		
		ПодключитьОбработчикОжидания("Подключаемый_Автообновление", ПериодАвтообновления);
	КонецЕсли;
	
	ВидимостьЛегенды = Объект.КэшНастроекОтображения.НастройкиОтображения.ОтображатьЦветомЗагруженностьКалендаря;
	
	Элементы.Легенда.Видимость			= ВидимостьЛегенды;
	Элементы.ГруппаЛегенда.Видимость	= ВидимостьЛегенды;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	бОбрабатывать = (ИмяСобытия = "ОбновленыДанныеСобытия" Или ИмяСобытия = "НастройкиКалендаряИзменены");
	...
	Если ИмяСобытия = "Запись_Взаимодействие" Тогда
		ИндексППДАктуален();
	КонецЕсли;
...
Процедура КалендарьПриВыводеПериода(Элемент, ОформлениеПериода)
	...
	ОтображатьЦветомЗагруженностьКалендаря =
		Объект.КэшНастроекОтображения.НастройкиОтображения.ОтображатьЦветомЗагруженностьКалендаря;
	...
	Для Каждого ПериодКалендаря Из ОформлениеПериода.Даты Цикл
		...
		Цвет = РаскраситьДень(ПериодКалендаря.Дата, Баллов, ОтображатьЦветомЗагруженностьКалендаря);
		...
		Если Цвет <> Неопределено Тогда
			...
			Если Цвет = WebЦвета.Белый И НачалоДня(ПериодКалендаря.Дата) < НачалоДня(ОбщегоНазначенияКлиент.ДатаСеанса()) Тогда
	...
	КоличествоЧасовВДне		= 24;
	
	ДатаДоОбновления = ДатаОтображения;
	ДеньВМесяце = CRM_УправлениеПроектамиКлиентСервер.РазностьДатВДнях(ДатаДоОбновления, НачалоМесяца(ДатаДоОбновления));
	ДатаОтображения = ОформлениеПериода.НачалоПериода + ДеньВМесяце * КоличествоСекундВЧасе * КоличествоЧасовВДне;
...
&НаКлиенте
Процедура КалендарьСписокТекстПоискаАвтоПодбор(Элемент, Текст, ДанныеВыбора,
...
КонецПроцедуры // КалендарьПроверкаПеретаскивания()
...
Процедура КалендарьПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, ВыбраннаяДата)
	...
	Если ПараметрыПеретаскивания.Значение.Количество() > 0 Тогда
		...
		Для Каждого ТекущаяСтрокаПеретаскиваемогоЭлемента Из ПараметрыПеретаскивания.Значение Цикл
			Если ТипЗнч(ТекущаяСтрокаПеретаскиваемогоЭлемента) = Тип("ЭлементПланировщика") Тогда
				ОбъектСобытия = ТекущаяСтрокаПеретаскиваемогоЭлемента.Значение.Источник;
				
				Если ТипЗнч(ОбъектСобытия) = Тип("ДокументСсылка.CRM_Взаимодействие")
					 Или ТипЗнч(ОбъектСобытия) = Тип("ДокументСсылка.CRM_ЭтапКалендарногоПлана")
					 Или ТипЗнч(ОбъектСобытия) = Тип("СправочникСсылка.МаркетинговыеМероприятия")
					 Или ТипЗнч(ОбъектСобытия) = Тип("ДокументСсылка.CRM_Телемаркетинг") Тогда
					ОбрабатываемыеЭлементы.Добавить(ТекущаяСтрокаПеретаскиваемогоЭлемента.Значение);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		
		Если СуществуетПереодическоеСобытие(ОбрабатываемыеЭлементы) Тогда
			ДоступныеДействия = Новый Массив;
			ДоступныеДействия.Добавить("ТолькоТекущее");
			ДоступныеДействия.Добавить("ДоКонцаРяда");
			
			ДополнительныеПараметры = Новый Структура(
				"ОбрабатываемыеЭлементы,ВыбраннаяДата",
				ОбрабатываемыеЭлементы, ВыбраннаяДата
			);
			
			ОписаниеОповещения = Новый ОписаниеОповещения(
				"КалендарьПеретаскиваниеЗавершение",
				ЭтотОбъект,
...
КонецПроцедуры // КалендарьПеретаскивание()

&НаКлиенте
Процедура ВариантПериодаПриИзменении(Элемент)
	
	Элементы.ГруппаРесурсы.Видимость	= Ложь;
	Элементы.ГруппаОтборы.Видимость		= ?(ВариантПериода <> "ПоРесурсам", Элементы.ГруппаОтборы.Видимость, Ложь);
	...
	Если Не Элемент = Неопределено Тогда
		Если ВариантПериода = "День" Тогда
			CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.Взаимодействия.КалендарьСинхронизацияСGoogle.Удобство.КоличествоНажатийКнопкаДень");
		ИначеЕсли ВариантПериода = "Неделя" Тогда
			CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.Взаимодействия.КалендарьСинхронизацияСGoogle.Удобство.КоличествоНажатийКнопкаНеделя");
		ИначеЕсли ВариантПериода = "Месяц" Тогда
			CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.Взаимодействия.КалендарьСинхронизацияСGoogle.Удобство.КоличествоНажатийКнопкаМесяц");
		ИначеЕсли ВариантПериода = "ПоРесурсам" Тогда
			CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.Взаимодействия.КалендарьСинхронизацияСGoogle.Удобство.КоличествоНажатийКнопкаРесурсы");
		КонецЕсли;
	КонецЕсли;
...
Процедура НеОтображатьЗавершенныеПриИзменении(Элемент)
	CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.Взаимодействия.КалендарьСинхронизацияСGoogle.Удобство.КоличествоНажатийОтборНеОтображатьЗавершенные");
	ВариантПериодаПриИзменении(Неопределено);
КонецПроцедуры
...
Процедура ПланировщикПередСозданием(Элемент, Начало, Конец, Значения, Текст, СтандартнаяОбработка)
	...
	Если РежимВыбора Тогда
		...
		РезультатРаботыСКалендарем.Вставить("Начало",	ОкруглитьЗначениеДаты(Начало));
		РезультатРаботыСКалендарем.Вставить("Конец",	ОкруглитьЗначениеДаты(Конец));
		
		ЗакрыватьПриВыборе = Ложь;
		ОповеститьОВыборе(РезультатРаботыСКалендарем);
		ПодключитьОбработчикОжидания("Подключаемый_ЗакрытьФормуПриВыборе", 0.1, Истина);
...
&НаКлиенте
Процедура Подключаемый_ЗакрытьФормуПриВыборе()
	Закрыть();
КонецПроцедуры
...
Процедура ПланировщикПриОкончанииРедактирования(Элемент, НовыйЭлемент, ОтменаРедактирования)
	...
	ОтложитьОбновлениеКеша = Ложь;
	
	Если РежимВыбора Тогда
		Если НовыйЭлемент Тогда
			ОтменаРедактирования = Истина;
		Иначе
			РежимВыбора_ДатаИВремяНачала		= ОкруглитьЗначениеДаты(Элемент.ВыделенныеЭлементы[0].Начало);
			РежимВыбора_ДатаИВремяЗавершения	= ОкруглитьЗначениеДаты(Элемент.ВыделенныеЭлементы[0].Конец);
		КонецЕсли;
		
		РезультатРаботыСКалендарем = Новый Структура;
		РезультатРаботыСКалендарем.Вставить("ЭтоРезультатРаботыСКалендарем");
		РезультатРаботыСКалендарем.Вставить("Начало",	РежимВыбора_ДатаИВремяНачала);
		РезультатРаботыСКалендарем.Вставить("Конец",	РежимВыбора_ДатаИВремяЗавершения);
	...
	Если НовыйЭлемент Тогда
		УстанавливаемыйОтветственный = ?(
			ВариантПериода <> "ПоРесурсам",
			ОтборЗначениеПользователь,
			Элемент.ВыделенныеЭлементы[0].ЗначенияИзмерений.Получить("Пользователь"));
		...
		ЗначенияЗаполнения.Вставить("Ответственный",			УстанавливаемыйОтветственный);
		...
		Если ПараметрыМероприятия.Свойство("ЦветФона") Тогда
			...
			ИначеЕсли ТипЗнч(ПараметрыМероприятия.ЦветФона) = Тип("СправочникСсылка.CRM_Категории") Тогда
				Элемент.ВыделенныеЭлементы[0].ЦветФона = Новый Цвет(ПараметрыМероприятия.ЦветФона.ЦветКрасный,
					 ПараметрыМероприятия.ЦветФона.ЦветЗеленый,
					 ПараметрыМероприятия.ЦветФона.ЦветСиний);
			КонецЕсли;
		КонецЕсли;
	Иначе
		ОбрабатываемыеЭлементы = Новый Массив;
		...
		Для Каждого ВыделенныйЭлемент Из Элемент.ВыделенныеЭлементы Цикл
			Если НЕ (ВыделенныйЭлемент.Значение = Неопределено)
				...
				СтруктураПараметров.Вставить("ТипЭтапа",
							CRM_ОбщегоНазначенияСервер.ПолучитьЗначениеРеквизита(ВыделенныйЭлемент.Значение.Источник,
					 "ТипЭтапа"));
				...
				ФормаПереноса			= ПолучитьФорму("Документ.CRM_ЭтапКалендарногоПлана.Форма.ФормаПереносаСроков",
				...
				ОткрытьФорму("Документ.CRM_ЭтапКалендарногоПлана.Форма.ФормаПереносаСроков",
					 СтруктураПараметров, ЭтотОбъект, , ВариантОткрытияОкна.ОтдельноеОкно , , ОповещениеПереносКТ,
					 РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
			ИначеЕсли НЕ (ВыделенныйЭлемент.Значение = Неопределено) Тогда
				Если ВыделенныйЭлемент.Конец = Дата(1970, 1, 1, 0, 0, 0) Тогда
					ВыделенныйЭлемент.Начало	= ВыделенныйЭлемент.Значение.ПериодНачало;
					ВыделенныйЭлемент.Конец		= ВыделенныйЭлемент.Значение.ПериодОкончание;
				КонецЕсли;
				
				Если ВариантПериода = "Месяц" Тогда
					ДатаНачало			= День(ВыделенныйЭлемент.Значение.ПериодНачало);
					ДатаПослеОбработки	= День(ВыделенныйЭлемент.Начало);
					
					Если Не ДатаНачало = ДатаПослеОбработки Тогда
						НачалоДоОбработки	= ВыделенныйЭлемент.Значение.ПериодНачало;
						КонецДоОбработки	= ?(
							ВыделенныйЭлемент.Значение.ПериодОкончание = КонецДня(ВыделенныйЭлемент.Значение.ПериодОкончание),
							ВыделенныйЭлемент.Значение.ПериодОкончание - 1,
							ВыделенныйЭлемент.Значение.ПериодОкончание
						);
						
						ВремяНачало	= НачалоДоОбработки - НачалоДня(НачалоДоОбработки);
						ВремяКонец	= КонецДня(КонецДоОбработки) - КонецДоОбработки;
						
						ДатаКонцаПослеОбработки		= ВыделенныйЭлемент.Конец - ВремяКонец;
						ДатаНачалаПослеОбработки	= НачалоДня(ВыделенныйЭлемент.Начало) + ВремяНачало;
						
						ВыделенныйЭлемент.Начало	= ДатаНачалаПослеОбработки;
						ВыделенныйЭлемент.Конец		= ДатаКонцаПослеОбработки;
					Иначе 
						ВыделенныйЭлемент.Начало	= ВыделенныйЭлемент.Значение.ПериодНачало;
						ВыделенныйЭлемент.Конец		= ВыделенныйЭлемент.Значение.ПериодОкончание;
					КонецЕсли; 
				Иначе
					ВыделенныйЭлемент.Начало	= ОкруглитьЗначениеДаты(ВыделенныйЭлемент.Начало);
					ВыделенныйЭлемент.Конец		= ОкруглитьЗначениеДаты(ВыделенныйЭлемент.Конец);
				КонецЕсли;
			...
			Если НайденныеВзаимодействия.Количество() > 0 Тогда
				Для Каждого тОбъект Из НайденныеВзаимодействия Цикл 
					ВыделенныйЭлемент.Текст = ПолучитьПредставлениеСобытия(тОбъект.Тема, тОбъект.Партнер,
						 тОбъект.Пользователь, ВыделенныйЭлемент.Начало,
		...
		Если СуществуетПереодическоеСобытие(ОбрабатываемыеЭлементы) Тогда
			...
			Если ВариантПериода = "Неделя" Или ВариантПериода = "День" Тогда
				// Если это неделя или день, то проверить, осуществляется ли перетаскивание в рамках дня или дней.
				Если ПеретаскиваниеОсуществляетсяВРамкахДня(ОбрабатываемыеЭлементы) Тогда
					ДоступныеДействия.Добавить("ВесьРяд");
				КонецЕсли;
			КонецЕсли;
			
			ДополнительныеПараметры = Новый Структура(
				"ОбрабатываемыеЭлементы,ВыбраннаяДата",
				ОбрабатываемыеЭлементы, НачалоДня(Элемент.ВыделенныеЭлементы[0].Начало)
			);
			...
			ОтложитьОбновлениеКеша = Истина;
		Иначе
			СохранениеВыполнено = CRM_МероприятияСервер.СохранитьИзмененияВБазу(ОбрабатываемыеЭлементы);
	...
	Если Не ОтложитьОбновлениеКеша Тогда
		ПодключитьОбработчикОжидания("Подключаемый_ПринудительноОбновитьКэшОформления", 0.1, Истина);
	КонецЕсли;
...
Процедура ПланировщикПередУдалением(Элемент, Отказ)
	...
	Если СуществуетПереодическоеСобытие(ОбрабатываемыеЭлементы) Тогда
		...
		ДополнительныеПараметры = Новый Структура(
			"ОбрабатываемыеЭлементы", ОбрабатываемыеЭлементы);
		
		ОписаниеОповещения = Новый ОписаниеОповещения(
			"КалендарьУдалениеСобытийЗавершение",
			ЭтотОбъект, ДополнительныеПараметры);
...
КонецПроцедуры // ПланировщикОбработкаФормированияКоманд()
...
&НаКлиенте
Процедура ВесьДеньПриОкончанииРедактирования(Элемент, НовыйЭлемент, ОтменаРедактирования)
	
	ОтменаРедактирования = Истина;
	
	Если РежимВыбора И РежимВыбора_ВесьДень И Элемент.ВыделенныеЭлементы[0].Значение.Источник = РежимВыбора_ОбъектАдресат Тогда
		ДлительностьМероприятия = КонецДня(РежимВыбора_ДатаИВремяЗавершения) - НачалоДня(РежимВыбора_ДатаИВремяНачала);
		НачалоМероприятия = НачалоДня(Элемент.ВыделенныеЭлементы[0].Начало);
		
		РезультатРаботыСКалендарем = Новый Структура;
		РезультатРаботыСКалендарем.Вставить("ЭтоРезультатРаботыСКалендарем");
		РезультатРаботыСКалендарем.Вставить("Начало",	НачалоДня(НачалоМероприятия));
		РезультатРаботыСКалендарем.Вставить("Конец",	КонецДня(НачалоМероприятия + ДлительностьМероприятия));
		
		ОповеститьОВыборе(РезультатРаботыСКалендарем);
	КонецЕсли;
	
КонецПроцедуры // ВесьДеньПриОкончанииРедактирования()
...
Процедура Создать(Команда)
	...
	CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.Взаимодействия.КалендарьСинхронизацияСGoogle.Удобство.КоличествоНажатийКнопкаПлюс");
	...
	Если ВариантПериода = "ПоРесурсам" Тогда
		...
		ВыбранныйОтветственный	= Неопределено;
		
		Для Каждого ТекущаяВыделеннаяСтрока Из Элементы.ДеревоРесурсов.ВыделенныеСтроки Цикл
			СтрокаИзДереваЗначений = ДеревоРесурсов.НайтиПоИдентификатору(ТекущаяВыделеннаяСтрока);
			
			Если СтрокаИзДереваЗначений = Неопределено
					Или СтрокаИзДереваЗначений.ЭтоГруппа
					Или СтрокаИзДереваЗначений.ЭтоСистемнаяГруппа
					Или СтрокаИзДереваЗначений.ЭтоЗаголовок Тогда
				Продолжить;
			КонецЕсли;
			
			ПолеДублирования = "";
			
			Если ТипЗнч(СтрокаИзДереваЗначений.Значение) = Тип("СправочникСсылка.Пользователи") Тогда
				Если ЗначениеЗаполнено(ВыбранныйОтветственный) Тогда
					ПолеДублирования = "Пользователь";
				Иначе
					ВыбранныйОтветственный = СтрокаИзДереваЗначений.Значение;
				КонецЕсли;
			ИначеЕсли ТипЗнч(СтрокаИзДереваЗначений.Значение) = Тип("СправочникСсылка.CRM_Помещения") Тогда
				Если ЗначениеЗаполнено(ВыбранноеПомещение) Тогда
					ПолеДублирования = "Помещение";
				Иначе
					ВыбранноеПомещение = СтрокаИзДереваЗначений.Значение;
				КонецЕсли;
			КонецЕсли;
			
			Если Не ПустаяСтрока(ПолеДублирования) Тогда
				ОбщегоНазначенияКлиент.СообщитьПользователю(
					СтрШаблон("%1 дублируется. Необходимо выбрать только одно значение из категории %1", ПолеДублирования), , ,
					"ДеревоРесурсов");
				Возврат;
			КонецЕсли;
		КонецЦикла;
		
		ЗначенияЗаполнения.Вставить("Помещение",		ВыбранноеПомещение);
		ЗначенияЗаполнения.Вставить("Ответственный",	ВыбранныйОтветственный);
	КонецЕсли;
	
	ПараметрыФормы = Новый Структура("ЗначенияЗаполнения", ЗначенияЗаполнения);
	ОткрытьФорму("Документ.CRM_Взаимодействие.ФормаОбъекта", ПараметрыФормы, Элементы.Планировщик);
...
Процедура Сегодня(Команда)
	CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.Взаимодействия.КалендарьСинхронизацияСGoogle.Удобство.КоличествоНажатийКнопкаСегодня");
	Элементы.Календарь.ВыделенныеДаты.Очистить();
	ДатаОтображения = ОбщегоНазначенияКлиент.ДатаСеанса();
	КалендарьПриИзменении(Элементы.Календарь);
КонецПроцедуры

&НаКлиенте
Процедура Вперед(Команда)
	CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.Взаимодействия.КалендарьСинхронизацияСGoogle.Удобство.КоличествоНажатийКнопкаВперед");
	Элементы.Календарь.ВыделенныеДаты.Очистить();
...
Процедура Назад(Команда)
	CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.Взаимодействия.КалендарьСинхронизацияСGoogle.Удобство.КоличествоНажатийКнопкаНазад");
	Элементы.Календарь.ВыделенныеДаты.Очистить();
	Если ВариантПериода = "Список" ИЛИ ВариантПериода = "ПоРесурсам" ИЛИ ВариантПериода = "День" Тогда
		ДатаОтображения = ДатаОтображения - 86400;
	ИначеЕсли ВариантПериода = "Неделя" Тогда
...
Процедура Настройки(Команда)
	...
	ФормаНастройки = ПолучитьФорму(МетаданныеПуть + ".Форма.Настройки", Параметрыформы, ЭтотОбъект, ,
...
Процедура ОткрытиеФормыНастройкиЗавершение(Результат, ДополнительныеПараметры) Экспорт 
	...
	Если ДополнительныеПараметры.Свойство("ФормаНастройки") Тогда
		...
		Если ФормаНастройки.НастройкиБылиИзменены Тогда
			...
			CRM_ОбщегоНазначенияКлиентСервер.ОбновлениеТаблицыФормы(Элементы.ДеревоРесурсов);
		КонецЕсли;
	КонецЕсли;
...
Процедура ДействиеСинхронизации(Команда)
	...
	Если СостояниеСинхронизации.Действие = "Настройка" Тогда
		ОткрытьФорму("Справочник.CRM_СервисыКалендарей.Форма.НастроитьСинхронизацию");
		CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.Взаимодействия.КалендарьСинхронизацияСGoogle.Удобство.КоличествоНажатийКнопкаСинхронизацияИзКалендаря");
	Иначе
		CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.Взаимодействия.КалендарьСинхронизацияСGoogle.Удобство.КоличествоНажатийКнопкаРучногоЗапускаСинхронизацииИзКалендаря");
...
&НаКлиенте
Процедура СвернутьРазвернутьОтборы(Команда)
	
	CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.Взаимодействия.КалендарьСинхронизацияСGoogle.Удобство.КоличествоНажатийГруппаОтборы");
...
&НаКлиенте
Процедура СвернутьРазвернутьОбъектыКалендаря(Команда)
	
	CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.Взаимодействия.КалендарьСинхронизацияСGoogle.Удобство.КоличествоНажатийГруппаОтборыКалендаря");
	
	Элементы.ГруппаПоказывать.Видимость = Не Элементы.ГруппаПоказывать.Видимость;
	
КонецПроцедуры

&НаКлиенте
Процедура СвернутьРазвернутьЛегенда(Команда)
	
	CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.Взаимодействия.КалендарьСинхронизацияСGoogle.Удобство.КоличествоНажатийГруппаЛегенда");
	
	Элементы.ГруппаЛегенда.Видимость = Не Элементы.ГруппаЛегенда.Видимость;
	
КонецПроцедуры

&НаКлиенте
Процедура СвернутьРазвернутьРесурсы(Команда)
	
	Элементы.ГруппаРесурсы.Видимость = Не Элементы.ГруппаРесурсы.Видимость;
	
КонецПроцедуры // СвернутьРазвернутьРесурсы()
...
&НаКлиенте
Процедура СоздатьВыборИзМеню(Команда)
	
	Список = Новый СписокЗначений;
	Список.Добавить("Взаимодействие", НСтр("ru = 'Взаимодействие'"));
	Список.Добавить("ЛичнаяЗадача", НСтр("ru = 'Личная задача'"));
	
	ОписаниеОповещения = Новый ОписаниеОповещения("СоздатьПослеВыбораИзМеню", ЭтотОбъект);
...
Функция СуществуетПереодическоеСобытие(ОбрабатываемыеЭлементы)
	
	Для Каждого ТекущееСобытие Из ОбрабатываемыеЭлементы Цикл
		Если CRM_МероприятияСервер.СобытиеЯвляетсяСобытиемРяда(ТекущееСобытие.Источник) Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
КонецФункции // СуществуетПереодическоеСобытие()

&НаСервереБезКонтекста
Функция ПеретаскиваниеОсуществляетсяВРамкахДня(ОбрабатываемыеЭлементы)
	
	Для Каждого ТекущееСобытие Из ОбрабатываемыеЭлементы Цикл
		РевизитНачало = ТекущееСобытие.РеквизитНачало;
		
		Если НачалоДня(ТекущееСобытие.ПериодНачало) <> НачалоДня(ТекущееСобытие.Источник[РевизитНачало]) Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
...
КонецФункции // ПеретаскиваниеОсуществляетсяВРамкахДня()

&НаКлиенте
Процедура КалендарьПеретаскиваниеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат <> Неопределено И Результат.ПринятьИзменения Тогда
		РезультатПереноса = CRM_МероприятияСервер.ПереместитьМероприятияРяда(
			ДополнительныеПараметры.ОбрабатываемыеЭлементы,
			ДополнительныеПараметры.ВыбраннаяДата,
			Результат.СпособИзмененияРяда);
		
		Если РезультатПереноса Тогда
			Элементы.Календарь.Обновить();
			ОбновитьНадписи();
			ПодключитьОбработчикОжидания("Подключаемый_ПринудительноОбновитьКэшОформления", 0.1, Истина);
			ОповеститьОбИзменении(Тип("СправочникСсылка.CRM_РядыСобытий"));
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("en = 'The series events could not be rescheduled.'; ru = 'Не удалось перенести события ряда.'"));
		КонецЕсли;
...
КонецПроцедуры // КалендарьПеретаскиваниеЗавершение()

&НаКлиенте
Процедура КалендарьУдалениеСобытийЗавершение(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат <> Неопределено И Результат.ПринятьИзменения Тогда
		РезультатПереноса = CRM_МероприятияСервер.УдалитьМероприятияРяда(
			ДополнительныеПараметры.ОбрабатываемыеЭлементы,
			Результат.СпособИзмененияРяда
		);
...
КонецПроцедуры // КалендарьУдалениеСобытийЗавершение()
...
Процедура ОбновитьОтборТиповОбъектов()
	...
	Для Каждого ОбъектСтрока Из РегистрируемыеОбъекты Цикл
		...
		Если НоваяСтрока.Представление = "Задача" Тогда
			НоваяСтрока.Представление = "Личная задача";
		КонецЕсли;
...
&НаКлиенте
Функция РаскраситьДень(День, Баллов, УчитыватьЗагруженность)
	...
	Если УчитыватьЗагруженность Тогда
		...
		КонецЕсли;
	Иначе
		Цвет = ?(Не ЭтоРабочийДень, WebЦвета.СеребристоСерый, WebЦвета.Белый);
	КонецЕсли;
...
Процедура РаскраситьКалендарь(ТекДатаКалендаря, ОбновитьКэш = Ложь)
	...
	Если Не ЗначениеЗаполнено(ОтборЗначениеПользователь) И ЗначениеЗаполнено(ОтборЗначениеПодразделение) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "CRM_СобытияКалендаря.Пользователь = &Пользователь",
			 "CRM_СобытияКалендаря.Подразделение = &Подразделение");
	КонецЕсли;
...
Процедура ОбновитьНадписи()
	Элементы.НадписьДатаРасписания.Заголовок = НСтр("ru='Расписание составлено до ';en='Schedule is set to '") + Формат(ДатаОтображения 
		+ 14 * 86400,
		 "ДЛФ=DD");
	...
	ИначеЕсли ВариантПериода = "Неделя" Тогда
		ТекстПериода = Формат(НачалоНедели(ДатаОтображения), "ДФ=""дд ММММ""") + " - " 
			+ Формат(КонецНедели(ДатаОтображения),
			 "ДФ=""дд ММММ""");
	ИначеЕсли ВариантПериода = "Месяц" Тогда
		ТекстПериода = Формат(НачалоМесяца(ДатаОтображения), "ДФ=""дд ММММ""") + " - " 
			+ Формат(КонецМесяца(ДатаОтображения),
			 "ДФ=""дд ММММ""");
	ИначеЕсли ВариантПериода = "Список" Тогда
		ТекстПериода = Формат(ДатаОтображения, "ДФ=""дд ММММ""") + " - " + Формат(ДатаОтображения 
			+ 14 * 86400,
			 "ДФ=""дд ММММ""");
	КонецЕсли;
	
	Заголовок = НСтр("ru='Календарь менеджера (';en='Manager`s Calendar ('") + ТекстПериода + "): " 
		+ ОтборЗначениеПользователь;
	
	Если НЕ ЗначениеЗаполнено(СрокИсполнения) Тогда
		Элементы.НадписьСрокИсполнения.Заголовок = "";
	Иначе
		Элементы.НадписьСрокИсполнения.Заголовок = НСтр("ru='  Взаимодействие небходимо выполнить к ';en=' Interaction should performed to '") 
			+ Формат(СрокИсполнения,
			 "ДЛФ=DD");
...
Функция ВренутьПериодыПоДате(ДатаДня)
	...
	РасписаниеИнтервалШкалыВремени			= ?(Объект.КэшНастроекОтображения.НастройкиОтображения.РасписаниеИнтервалШкалыВремени < 30,
		 30,
		 Объект.КэшНастроекОтображения.НастройкиОтображения.РасписаниеИнтервалШкалыВремени);
...
Функция ПолучитьПредставлениеСобытия(Тема = "", Партнер, Пользователь, ПериодНачало, ПериодОкончание, ВесьДень = Ложь)
	...
	СписокПредставлениеСобытияВКалендаре =
		Объект.КэшНастроекОтображения.НастройкиОтображения.СписокПредставлениеСобытияВКалендаре;
	
	Если ВесьДень Тогда
		ПредставлениеПериод = "";
	ИначеЕсли ПериодНачало = ПериодОкончание Тогда
		ПредставлениеПериод = Формат(ПериодОкончание, "ДФ=ЧЧ:мм");
	ИначеЕсли НачалоДня(ПериодНачало) <> НачалоДня(ПериодОкончание) Тогда
		ПредставлениеПериод = Формат(ПериодНачало, "ДФ='дд МММ ЧЧ:мм'") 
			+ " - " + Формат(ПериодОкончание, "ДФ='дд МММ ЧЧ:мм'");
	Иначе
...
Процедура ОбновитьДанныеПланировщикаСервер()
	...
	ПоказыватьНедействительныхПользователей	=
		Объект.КэшНастроекОтображения.НастройкиОтображения.ПоказыватьНедействительныхПользователей;
	КалендарьКоличествоДнейВНеделе			= Объект.КэшНастроекОтображения.НастройкиОтображения.КалендарьКоличествоДнейВНеделе;
	СписокПредставлениеСобытияВКалендаре	=
		Объект.КэшНастроекОтображения.НастройкиОтображения.СписокПредставлениеСобытияВКалендаре;
	РасписаниеИнтервалШкалыВремени			= ?(Объект.КэшНастроекОтображения.НастройкиОтображения.РасписаниеИнтервалШкалыВремени < 30,
	...
	Если ВариантПериода = "ПоРесурсам" Тогда
		Планировщик.ШкалаВремени.Положение = ?(РасположениеШкалыВремени = 0, ПоложениеШкалыВремени.Лево,
	...
	НачалоДняТекущейДаты = НачалоДня(ТекущаяДатаСеанса());
	...
	Для Каждого СтрокаПериодаДат Из ТаблицаПериодов Цикл
		...
		Если НЕ ВариантПериода = "Месяц" Тогда
			Для Каждого ТекПериодОтображения Из СтрокаПериодаДат.ТаблицаПериодов Цикл
				ИнтервалПериода = Планировщик.ТекущиеПериодыОтображения.Добавить(ТекПериодОтображения.НачалоПериода,
					 ТекПериодОтображения.ОкончаниеПериода);
				...
				Если ТекПериодОтображения.Обед Тогда
					Если Планировщик.ШкалаВремени.Элементы.Количество() = 2 Тогда
						...
						Для Ном = 1 По КоличествоЦиклов Цикл
							МеткаОбеда = Планировщик.ШкалаВремени.Элементы[1].Метки.Добавить(ТекПериодОтображения.НачалоПериода 
								+ КоличествоСекнд);
							МеткаОбеда.Текст = "";
				...
				Если ТекПериодОтображения.НеРабочееВремя Тогда
					НовыйИнтервалФора = Планировщик.ИнтервалыФона.Добавить(ТекПериодОтображения.НачалоПериода,
						 ТекПериодОтображения.ОкончаниеПериода);
					НовыйИнтервалФора.Цвет = ?(
						НачалоДня(ТекПериодОтображения.НачалоПериода) = НачалоДня(НачалоДняТекущейДаты),
						Новый Цвет(204, 236, 252),
						Новый Цвет(241, 241, 241));
				Иначе
					Если НачалоДня(ТекПериодОтображения.НачалоПериода) = НачалоДня(НачалоДняТекущейДаты) Тогда
						НовыйИнтервалФора = Планировщик.ИнтервалыФона.Добавить(ТекПериодОтображения.НачалоПериода,
							 ТекПериодОтображения.ОкончаниеПериода);
						НовыйИнтервалФора.Цвет = Новый Цвет(235, 247, 254);
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
		Иначе
	...
	ЦветФонаДляЗадачКСроку = CRM_СинхронизацияКалендарей.ПолучитьЦветКалендаря(, 0);
	...
	Для Каждого Мероприятие Из Мероприятия Цикл
		...
		Если ТипЗнч(Мероприятие.Объект) = Тип("ДокументСсылка.CRM_Взаимодействие") Тогда
			...
			ДокументИнтерес = ?(ЗначениеЗаполнено(Мероприятие.Объект.ДокументОснование),
				 Мероприятие.Объект.ДокументОснование,
				 Неопределено);
		...
		ИначеЕсли ТипЗнч(Мероприятие.Объект) = Тип("ЗадачаСсылка.ЗадачаИсполнителя") Тогда
			Если Мероприятие.Объект.CRM_ВариантСрока = "КСроку" Тогда
				РеквизитНачало = "ДатаНачала";
			Иначе
				РеквизитНачало = "CRM_ПлановаяДатаПринятияКИсполнению";
			КонецЕсли;
		...
		Если ВариантПериода = "ПоРесурсам" Тогда
			...
			Если ТипЗнч(Мероприятие.Объект) = Тип("ЗадачаСсылка.ЗадачаИсполнителя") И Мероприятие.НаВесьДень Тогда
			...
			ПериодОкончание = Мероприятие.ПериодОкончание;
			Если ТипЗнч(Мероприятие.Объект) = Тип("ЗадачаСсылка.ЗадачаИсполнителя") 
				И Мероприятие.НаВесьДень Тогда
				ПериодОкончание = Мероприятие.ПериодНачало;
			КонецЕсли;
			
			ЭлементПланировщика.Значение	= СтруктураЗначений;
			ЭлементПланировщика.Текст		= ПолучитьПредставлениеСобытия(
				Мероприятие.Тема,
				Мероприятие.Партнер,
				Мероприятие.Пользователь,
				Мероприятие.ПериодНачало,
				ПериодОкончание,
				ТипЗнч(Мероприятие.Объект) = Тип("СправочникСсылка.МаркетинговыеМероприятия"));
			
			Если НЕ ПустаяСтрока(Мероприятие.ИндексЦвета) Тогда
				Если ТипЗнч(Мероприятие.Объект) = Тип("ЗадачаСсылка.ЗадачаИсполнителя") Тогда
					Ответственный = Мероприятие.Объект.Исполнитель;
				Иначе
					Ответственный = Мероприятие.Объект.Ответственный;
				КонецЕсли;
				ЭлементПланировщика.ЦветФона = CRM_СинхронизацияКалендарей.ПолучитьЦветКалендаря(Ответственный,
					 Мероприятие.ИндексЦвета);
			...
			ИначеЕсли ТипЗнч(Мероприятие.Объект) = Тип("ЗадачаСсылка.ЗадачаИсполнителя") Тогда
				ЭлементПланировщика.Картинка = БиблиотекаКартинок.CRM_ЛичнаяЗадача;
				Если Мероприятие.НаВесьДень Тогда
					ЭлементПланировщика.ЦветФона = ЦветФонаДляЗадачКСроку;
				КонецЕсли;
			Иначе
				...
				Иначе
					ЭлементПланировщика.Картинка = БиблиотекаКартинок.CRM_Взаимодействие;
				КонецЕсли;
			КонецЕсли;
		...
		Иначе
			...
			Если НЕ ВариантПериода = "Месяц" И (Мероприятие.НаВесьДень 
			 ИЛИ НачалоДня(Мероприятие.ПериодОкончание) > НачалоДня(Мероприятие.ПериодНачало)) Тогда
			// -Проекты
			// ИЛИ (Мероприятие.ПериодНачало <= ПериодДанных.ДатаНачала
				// И Мероприятие.ПериодОкончание>=ПериодДанных.ДатаОкончания) Тогда
				...
				СтруктураЭлемента.Текст = ПолучитьПредставлениеСобытия(Мероприятие.Тема, Мероприятие.Партнер,
					 Мероприятие.Пользователь, Мероприятие.ПериодНачало, Мероприятие.ПериодОкончание,
					 Истина);
				СтруктураЭлемента.ОсновнаяКатегория	= Мероприятие.ОсновнаяКатегория;
				СтруктураЭлемента.НачалоПериода		= НачалоДня(?(Мероприятие.ПериодНачало >= ПериодДанных.ДатаНачала,
					 Мероприятие.ПериодНачало,
					 ПериодДанных.ДатаНачала));
				СтруктураЭлемента.ОкончаниеПериода	= КонецДня(?(Мероприятие.ПериодОкончание <= ПериодДанных.ДатаОкончания,
					 Мероприятие.ПериодОкончание,
				...
				СтрокаПериодаНаВесьДень.НачалоПериода		= НачалоДня(?(Мероприятие.ПериодНачало >= ПериодДанных.ДатаНачала,
					 Мероприятие.ПериодНачало,
					 ПериодДанных.ДатаНачала));
				СтрокаПериодаНаВесьДень.ОкончаниеПериода	= КонецДня(?(Мероприятие.ПериодОкончание <= ПериодДанных.ДатаОкончания,
					 Мероприятие.ПериодОкончание,
					 ПериодДанных.ДатаОкончания));
				...
				Если ТипЗнч(Мероприятие.Объект) = Тип("ЗадачаСсылка.ЗадачаИсполнителя") 
					И Мероприятие.НаВесьДень Тогда
					Полчаса = 30 * 60;
					ЭлементПланировщика = Планировщик.Элементы.Добавить(Мероприятие.ПериодНачало, Мероприятие.ПериодНачало + Полчаса);
					ЭлементПланировщика.ЦветФона = ЦветФонаДляЗадачКСроку;
					ЭлементПланировщика.Значение = СтруктураЗначений;
					ЭлементПланировщика.Текст = ПолучитьПредставлениеСобытия(
						Мероприятие.Тема,
			...
			Иначе
				Если (Мероприятие.ПериодОкончание - Мероприятие.ПериодНачало) < 1800 
					Или (ТипЗнч(Мероприятие.Объект) = Тип("ЗадачаСсылка.ЗадачаИсполнителя") И Мероприятие.НаВесьДень)Тогда
					ЭлементПланировщика = Планировщик.Элементы.Добавить(Мероприятие.ПериодНачало, Мероприятие.ПериодНачало + 1800);
				Иначе	
					ЭлементПланировщика = Планировщик.Элементы.Добавить(Мероприятие.ПериодНачало, Мероприятие.ПериодОкончание);
				КонецЕсли;	
				...
				Если ТипЗнч(Мероприятие.Объект) = Тип("ДокументСсылка.CRM_ЭтапКалендарногоПлана") Тогда
					Если Мероприятие.Объект.ТипЭтапа = Перечисления.CRM_ТипыЭтапов.Этап Тогда
						РазностьДат 				 = Цел((НачалоДня(Мероприятие.Объект.ПлановаяДатаОкончания) - НачалоДня(Мероприятие.Объект.ПлановаяДатаНачала)) / 86400) 
							+ 1;
						ПериодСтрокой				 = НРег(ЧислоПрописью(РазностьДат, , НСтр("ru='день, дня, дней, м,,,,,
							| 0';en='day, day, days, m,,,,,
							| 0'")));
				...
				Если Мероприятие.Объект = РежимВыбора_ОбъектАдресат Тогда
					ЭлементПланировщика.ЦветФона = ЦветаСтиля.CRM_СерыйДляВторостепенныхФункций;
				ИначеЕсли Не ПустаяСтрока(Мероприятие.ИндексЦвета) Тогда
					Если ТипЗнч(Мероприятие.Объект) = Тип("ЗадачаСсылка.ЗадачаИсполнителя") Тогда
						Ответственный = Мероприятие.Объект.Исполнитель;
					Иначе
						Ответственный = Мероприятие.Объект.Ответственный;
	...
	ЗаголовокГруппыНаВсеьДень = СтрЗаменить(ЗаголовокГруппыНаВсеьДень, "%1",
		 Формат(ТаблицаПериодовНаВесьДень.Количество(),
		 "ЧН=Нет"));
	...
	Пока Обрабатываем Цикл
		...
		Для Каждого СтрокаПериода Из ТаблицаПериодовНаВесьДень Цикл
			Если СтрокаПериода.Обработано Тогда
				Продолжить;
			КонецЕсли;
	...
	Элементы.ГруппаНаВесьДень.Заголовок = НСтр("ru='Событий на весь день: ';en='Events for whole day: '") 
		+ Формат(ТаблицаПериодовНаВесьДень.Количество(),
		 "ЧН=Нет");
		...
		Для Каждого СтруктураЭлемента Из СтрокаИтога.МассивСтруктур Цикл
			...
			Для Ном = 1 По КоличествоДней Цикл
				...
				Иначе
					...
					Если НЕ ВывелиТекст Тогда
						...
						Если ТипЗнч(ОбъектИсточник) = Тип("ДокументСсылка.CRM_ЭтапКалендарногоПлана") Тогда
							Если ОбъектИсточник.ТипЭтапа	 = Перечисления.CRM_ТипыЭтапов.Этап Тогда
								РазностьДат 				 = Цел((НачалоДня(ОбъектИсточник.ПлановаяДатаОкончания) - НачалоДня(ОбъектИсточник.ПлановаяДатаНачала)) / 86400) 
									+ 1;
								ПериодСтрокой				 = НРег(ЧислоПрописью(РазностьДат, , НСтр("ru='день, дня, дней, м,,,,,
									| 0';en='day, day, days, m,,,,,
									| 0'")));
					...
					Если СтруктураЭлемента.Значение.Источник = РежимВыбора_ОбъектАдресат Тогда
						ЭлементПланировщика.ЦветФона = ЦветаСтиля.CRM_СерыйДляВторостепенныхФункций;
					ИначеЕсли ЗначениеЗаполнено(СтруктураЭлемента.ОсновнаяКатегория) Тогда
					...
					Иначе
						НовыйИнтервалФона = ВесьДень.ИнтервалыФона.Добавить(ДатаПериода + НачМин, ДатаПериода + КонМин);
						НовыйИнтервалФона.Цвет = ЭлементПланировщика.ЦветРамки;
					КонецЕсли;
	...
	Элементы.ВесьДень.Высота = Макс(2, ВесьДень.Элементы.Количество() + 1);
...
Процедура УстановитьОтображениеПланировщикаВесьДень()
	...
	Если ВариантПериода = "День" Тогда
		...
		// ВесьДень.ОтступСКонцаПереносаШкалыВремени = ?(НастройкиОтображения.ОкончаниеРабочегоДня = 0,
			// 0,
			// 24 - НастройкиОтображения.ОкончаниеРабочегоДня);
		...
		КонецЕсли;
		ВесьДень.ШкалаВремени.Положение = ПоложениеШкалыВремени.Лево;
		ВесьДень.ШкалаВремени.Элементы[0].Формат = "ДФ='мм" + Символы.НПП + Символы.НПП + Символы.НПП 
...
Процедура УстановитьОтображениеПланировщика()
	...
	Если ВариантПериода = "День" Тогда
		...
		Планировщик.ОтступСКонцаПереносаШкалыВремени = ?(НастройкиОтображения.ОкончаниеРабочегоДня = 0, 0,
			 24 - НастройкиОтображения.ОкончаниеРабочегоДня);
		Планировщик.ОтображатьПеренесенныеЗаголовки = Ложь;
		// Планировщик.ОтображатьПеренесенныеЗаголовки = Истина;
		Планировщик.ОтображатьПеренесенныеЗаголовкиШкалыВремени = Ложь;
...
Функция ПолучитьПериодДанных(ВариантПериода, ДатаОтображения)
	...
	Если ВариантПериода = "День" ИЛИ ВариантПериода = "ПоРесурсам" Тогда
		...
		Если Тип(Элементы.Календарь.ВыделенныеДаты) <> Тип("Массив")
			 ИЛИ Элементы.Календарь.ВыделенныеДаты.Количество() = 0 Тогда
...
Процедура УстановитьПредставлениеПериода(Форма)
	...
	ИначеЕсли Форма.ВариантПериода = "Месяц" Тогда
		...
		Форма.ПредставлениеПериода = ПредставлениеПериода(НачалоМесяца(Форма.ДатаОтображения),
			 КонецМесяца(Форма.ДатаОтображения));
...
Процедура СохранитьНастройкиЗначенийОтборов()
	...
	Для Каждого ИмяОтбора Из МассивДоступныхОтборов Цикл
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭтотОбъект, "ОтборЗначение" + ИмяОтбора) Тогда
			СтруктураДляСохранения.Отборы.Вставить("ОтборЗначение" + ИмяОтбора, ЭтотОбъект["ОтборЗначение" + ИмяОтбора]);
		КонецЕсли;
		
	КонецЦикла;
	
	CRM_ХранилищеНастроек.Сохранить("Обработка_CRM_Календарь_Настройки", "НастройкиЗначенийОтборов",
...
Процедура СохранитьНастройкиФормы()
	...
	CRM_ХранилищеНастроек.Сохранить("Обработка_CRM_Календарь_Настройки", "ПоказатьСкрытьОтборы",
		 Элементы.КнопкаПоказатьСкрытьОтборы.Пометка);
	
	// // Флаг КалендарьСписокФлагПоискПоПодстроке
	//CRM_ХранилищеНастроек.Сохранить("Обработка_CRM_Календарь_Настройки",
		// "КалендарьСписокФлагПоискПоПодстроке",
		// КалендарьСписокФлагПоискПоПодстроке);
	
	CRM_ХранилищеНастроек.Сохранить("Обработка_CRM_Календарь_Настройки", "СтруктураНастроек",
		 Объект.КэшНастроекОтображения);
...
Процедура Подключаемый_ОбновлениеИндексаПДД()
	...
	Состояние(НСтр("ru='Обновление индекса полнотекстового поиска завершено...';
		|en='Update of full text search index is completed...'")); 
	КалендарьОбновитьКлиент();
КонецПроцедуры
...
Функция СоздатьНовоеМероприятие(ЗначенияЗаполнения)
	...
	ТаблицаНастроекОповещений.Колонки.Добавить("Периодичность",
		 Новый ОписаниеТипов("ПеречислениеСсылка.CRM_ПериодичностьОповещений"));
...
Процедура ОбработатьЗначениеПланировщикаНаВесьДень()
	...
	Если ТипЗнч(ЗначениеЭлемента.Источник) = Тип("Массив") Тогда
		...
		Параметрыформы = Новый Структура("СписокСобытий,Дата", СписокСобытий,
			 НачалоДня(Элементы.ВесьДень.ВыделенныеЭлементы[0].Начало));
		ФормаНастройки = ПолучитьФорму(МетаданныеПуть + ".Форма.ФормаСпискаНаВесьДень", Параметрыформы,
			 ЭтотОбъект, , ВариантОткрытияОкна.ОтдельноеОкно);
...
Процедура ПродолжитьСинхронизироватьНаКлиенте()
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru='Учетная запись не настроена.'");
...
Процедура ОтключитьСинхронизациюНаСервере()
	...
	CRM_СинхронизацияКалендарей.УдалитьУзелПланаОбмена(Календарь);
...
Функция ПолучитьРегистрируемыеОбъекты()
	...
	Для Каждого Тип Из ОписаниеТиповОбъекта.Типы() Цикл
		...
		Попытка
			МдОбъекта = Метаданные.НайтиПоТипу(Тип);
		...
		Если Тип = Тип("СправочникСсылка.МаркетинговыеМероприятия") Тогда
			Если НЕ (Константы.CRM_ИспользоватьМаркетинговыеМероприятия.Получить() = Истина) Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;	
		Если Тип = Тип("ДокументСсылка.CRM_Телемаркетинг") Тогда
			Если НЕ (Константы.CRM_ИспользоватьТелемаркетинг.Получить() = Истина) Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;	
...
Процедура ОтборТипыОбъектовПометкаПриИзменении(Элемент)
	ПризнакСписокОбновлятьПоПолнотекстовомуПоиску = Истина;
	ЭлементДанных = ОтборТипыОбъектов.НайтиПоИдентификатору(Элементы.ОтборТипыОбъектов.ТекущаяСтрока);
	Если ЭлементДанных.ПолноеИмя = "Документ.CRM_Взаимодействие" Тогда
		CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.Взаимодействия.КалендарьСинхронизацияСGoogle.Удобство.КоличествоНажатийГруппаОтборыВзаимодействие");
	ИначеЕсли ЭлементДанных.ПолноеИмя = "Документ.CRM_ЭтапКалендарногоПлана" Тогда
		CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.Взаимодействия.КалендарьСинхронизацияСGoogle.Удобство.КоличествоНажатийГруппаОтборыКалендаряЗадачаИКТ");
	ИначеЕсли ЭлементДанных.ПолноеИмя = "Справочник.МаркетинговыеМероприятия" Тогда
		CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.Взаимодействия.КалендарьСинхронизацияСGoogle.Удобство.КоличествоНажатийГруппаОтборыКалендаряМероприятие");
	ИначеЕсли ЭлементДанных.ПолноеИмя = "Документ.CRM_Телемаркетинг" Тогда
		CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.Взаимодействия.КалендарьСинхронизацияСGoogle.Удобство.КоличествоНажатийГруппаОтборыКалендаряТелемаркетинг");
	КонецЕсли;
	КалендарьОбновитьКлиент();
КонецПроцедуры
...
Процедура ЗагрузитьНастройкиЗначенийОтборов()
	...
	Если ТипЗнч(СохраненныеОтборы) = Тип("Структура") Тогда
		Если СохраненныеОтборы.Свойство("ОтборСкрытьВыполненные")
			 И ТипЗнч(СохраненныеОтборы.ОтборСкрытьВыполненные) = Тип("Булево") Тогда
		...
		Если СохраненныеОтборы.Свойство("ОтборТипыОбъектов")
			 И ТипЗнч(СохраненныеОтборы.ОтборТипыОбъектов) = Тип("Соответствие") Тогда
		...
		Если СохраненныеОтборы.Свойство("Отборы") И ТипЗнч(СохраненныеОтборы.Отборы) = Тип("Соответствие") Тогда
			Для Каждого КлючЗначение Из СохраненныеОтборы.Отборы Цикл
				Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭтотОбъект, КлючЗначение.Ключ) Тогда
...
Процедура ЗаполнитьСписокИдентификаторовВыделенныхСтрокВДеревеРесурсов()
	...
	Если ТекущиеДанные <> Неопределено
		 И МассивИдентификаторовСтрок.Найти(ТекущиеДанные.ПолучитьИдентификатор()) = Неопределено Тогда
...
Функция СравнитьСпискиЗначений(Список1, Список2)
	Если ТипЗнч(Список1) <> ТипЗнч(Список2) Или ТипЗнч(Список1) <> Тип("СписокЗначений")
		 Или ТипЗнч(Список2) <> Тип("СписокЗначений") Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Для нИндекс = 0 По Список1.Количество() - 1 Цикл
		Если Список1[нИндекс].Значение <> Список2[нИндекс].Значение
			 Или Список1[нИндекс].Представление <> Список2[нИндекс].Представление Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
...
Процедура ДеревоРесурсовПриАктивизацииСтроки(Элемент)
	...
	Иначе
		...
		Если (бДоступностьПереместитьВверх Или бДоступностьПереместитьВниз) И ТекущиеДанные.ЭтоЗаголовок Тогда
			ДеревоРесурсовЭлементы = ДеревоРесурсов.ПолучитьЭлементы();
			Попытка
				Индекс = ДеревоРесурсовЭлементы.Индекс(ТекущиеДанные);
			
			Исключение	Индекс = Неопределено;
			КонецПопытки;
			Если Индекс <> Неопределено Тогда
				Если Индекс > 0 
					И ДеревоРесурсовЭлементы[Индекс - 1].ЭтоСистемнаяГруппа Тогда
		...
		бДоступностьИзменитьГруппу = (ТекущиеДанные.ЭтоГруппа И Не ТекущиеДанные.ЭтоЗаголовок
			 И Не ТекущиеДанные.ЭтоСистемнаяГруппа);
	КонецЕсли;
...
Процедура ДобавитьУдалитьЭлементВДеревеРесурсов(ДобавитьГруппу = Ложь,
	...
	Если ТекущиеДанные.ЭтоСистемнаяГруппа Тогда
		Возврат;
	КонецЕсли;
	
	ТекущийРодитель = ТекущиеДанные.ПолучитьРодителя();
	
	Если Удалить И (ТекущийРодитель = Неопределено Или ТекущиеДанные.ЭтоЗаголовок) Тогда
		Возврат;
	КонецЕсли;
	...
	Если ДобавитьГруппу Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("ДобавитьУдалитьЭлементВДеревеРесурсовЗавершение",
			 ЭтотОбъект, Новый Структура("ЭлементыДляДобавления, ТекущееИмяРесурса, ТекущиеДанные",
			 ЭлементыДляДобавления, ТекущееИмяРесурса,
			 ТекущиеДанные));
		ПоказатьВводСтроки(ОписаниеОповещения, НСтр("ru='Название группы';en='Group name'"), , , Ложь);
		Возврат;
	Иначе
		ДополнительныеПараметры = Новый Структура("ТекущиеДанные, ТекущееИмяРесурса,
			| ЭлементыДляДобавления, ОткрыватьДиалогВыбораЗначения, ДобавитьГруппу, ОбновлятьКалендарь",
		...
		Если ОткрыватьДиалогВыбораЗначения Тогда
			...
			Если ОткрытьВРежимеПодбора Тогда
				...
				ФормаВыбора.Открыть();
			Иначе
				ОписаниеОповещения = Новый ОписаниеОповещения("ДобавитьУдалитьЭлементВДеревеРесурсовПродолжение",
					 ЭтотОбъект,
					 ДополнительныеПараметры);
				ФормаВыбора.ОписаниеОповещенияОЗакрытии = ОписаниеОповещения;
				ФормаВыбора.РежимОткрытияОкна = РежимОткрытияОкнаФормы.БлокироватьОкноВладельца;
				ФормаВыбора.Открыть();
				Если ВыбранныйРесурсСсылка = Неопределено Тогда
					Возврат;
				КонецЕсли;
			КонецЕсли;
		Иначе
	...
	Иначе
		Если ЗначениеЗаполнено(ВыбранныйРесурсСсылка) Тогда
			Если Не ДополнительныеПараметры.ОткрыватьДиалогВыбораЗначения Тогда
				...
				КонецЦикла;
			Иначе
				Если ДополнительныеПараметры.ТекущиеДанные.ЭтоГруппа
					 Или (ДополнительныеПараметры.ТекущиеДанные.ЭтоЗаголовок
					 И Не ДополнительныеПараметры.ТекущиеДанные.ЭтоСистемнаяГруппа) Тогда
			...
			Если ЗначениеЗаполнено(НоваяСтрока.Значение)
				 И ТипЗнч(НоваяСтрока.Значение) = Тип("СправочникСсылка.Пользователи") Тогда
				НоваяСтрока.Недействителен = CRM_ОбщегоНазначенияСервер.ПолучитьЗначениеРеквизита(НоваяСтрока.Значение,
					 "Недействителен");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ДобавитьУдалитьЭлементВДеревеРесурсовЗавершение(СтрокаОписание, ДополнительныеПараметры) Экспорт
	...
	Если Не ЗначениеЗаполнено(СтрокаОписание) Тогда
		Возврат;
	КонецЕсли;
...
Процедура КомандаДеревоРесурсовИзменитьГруппу(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("КомандаДеревоРесурсовИзменитьГруппуЗавершение",
		 ЭтотОбъект,
		 ТекущиеДанные);
...
Процедура ЗагрузитьНастройкиДеревоРесурсов()
	...
	Если ТипЗнч(СтруктураДеревоРесурсов) = Тип("Структура") Тогда
		Для Каждого СтрокаТипРесурса Из Дерево.Строки Цикл
			Если СтрокаТипРесурса.ЭтоСистемнаяГруппа Тогда
				Продолжить;
			КонецЕсли;
			
			ИмяРесурса = СтрокаТипРесурса.Имя;
			Если СтруктураДеревоРесурсов.Свойство(ИмяРесурса)
				 И ТипЗнч(СтруктураДеревоРесурсов[ИмяРесурса]) = Тип("ДеревоЗначений") Тогда
	...
	Для Каждого Идентификатор Из СписокИдентификаторовВыделенныхСтрокВДеревеРесурсов Цикл
		...
		Если НайденнаяСтрока.ЭтоСистемнаяГруппа Тогда
			Если НайденнаяСтрока.Имя = "ОбщиеПомещения" Тогда
				...
				|	И Помещения.Общее;
				|");
				// Запрос выполняется только один раз за цикл
...
Функция НайтиОбъектыПолнотекстовыйПоискРежимСписок()
	...
	РазмерПорции = 200;
	СтрокаДляПоиска = ?(КалендарьСписокФлагПоискПоПодстроке, "" + КалендарьСписокТекстПоиска + "*",
	...
	// КонецЦикла;
	// CRM_ХранилищеНастроек.Сохранить("Обработка_CRM_Календарь_Настройки", "ИсторияПоиска",
		// КалендарьСписокТекстПоискаСписокВыбора.ВыгрузитьЗначения());
...
Функция ПолучитьСтруктуруДанныхЗапроса()
	...
	|			ИЛИ СобытияКалендаряПользователя.Важность = ЗНАЧЕНИЕ(Перечисление.ВариантыВажностиЗадачи.Низкая)
	|			ТОГДА 0
	|		КОГДА СобытияКалендаряПользователя.Важность = ЗНАЧЕНИЕ(Перечисление.ВариантыВажностиВзаимодействия.Высокая)
	|			ИЛИ СобытияКалендаряПользователя.Важность = ЗНАЧЕНИЕ(Перечисление.ВариантыВажностиЗадачи.Высокая)
	|			ТОГДА 2
	|		ИНАЧЕ 1
	|	КОНЕЦ КАК Важность,
	...
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ
	|	//%УСЛОВИЕ_РЕДАКТИРОВАНИЕ_ОБЪЕКТА%
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	тмпКалендарь.Объект КАК Объект,
	|	тмпКалендарь.Пользователь КАК Пользователь,
	|	тмпКалендарь.ПериодНачало КАК ПериодНачало,
	...
	|	тмпКалендарь.Тема КАК Тема,
	|	тмпКалендарь.НаВесьДень КАК НаВесьДень,
	|	тмпКалендарь.Помещение КАК Помещение,
	|	тмпКалендарь.Проект КАК Проект,
	|	тмпКалендарь.Подразделение КАК Подразделение,
	|	тмпКалендарь.Важность КАК Важность,
	|	тмпКалендарь.Завершено КАК Завершено,
	|	тмпКалендарь.Партнер КАК Партнер,
	|	ВЫБОР
	|		КОГДА тмпКалендарь.Объект ССЫЛКА Документ.CRM_ЭтапКалендарногоПлана
	...
	|		ИНАЧЕ """"
	|	КОНЕЦ КАК Партнеры,
	|	тмпКалендарь.ВидВзаимодействия КАК ВидВзаимодействия,
	|	тмпКалендарь.ВидДела КАК ВидДела,
	|	тмпКалендарь.ОсновнаяКатегория КАК ОсновнаяКатегория,
	|	тмпКалендарь.ИндексЦвета КАК ИндексЦвета,
	|	ВЫБОР
	|		КОГДА тмпКалендарь.Завершено
	|			ТОГДА тмпКалендарь.ПериодОкончание
	|		ИНАЧЕ тмпКалендарь.ПериодНачало
	|	КОНЕЦ КАК Дата,
	|	тмпКалендарь.Потенциал КАК Потенциал,
	|	тмпКалендарь.СтатусКонтрольнойТочки КАК СтатусКонтрольнойТочки" + ?(ВариантПериода = "Список", ",
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(тмпКалендарь.Объект) = ТИП(Документ.CRM_ЭтапКалендарногоПлана) 
	|			ТОГДА 
	|				ВЫБОР
	|					КОГДА ВЫРАЗИТЬ(тмпКалендарь.Объект КАК Документ.CRM_ЭтапКалендарногоПлана).ТипЭтапа = Значение(Перечисление.CRM_ТипыЭтапов.Этап)
	|						ТОГДА 2
	|					ИНАЧЕ 1
	|				КОНЕЦ
	|		ИНАЧЕ 13
	...
	Если бИспользуетсяПолнотекстовыйПоискСписка Тогда
		...
		СтруктураРезультат.Отборы["Объект"].Вставить("Значение",		?(ТекстОшибки = Неопределено,
			 СписокНайденныхОбъектов,
	...
	СтруктураРезультат.ТекстЗапроса = СтрЗаменить(СтруктураРезультат.ТекстЗапроса,
		 "//%УСЛОВИЕ_ТИПЫ_ОБЪЕКТОВ%",
		 СтрУсловие);
	...
	Если бДляРежимаОтображенияИспользуютсяВыбранныеРесурсы Тогда
		...
		СтруктураРезультат.Отборы["ГруппаОтбораПоВыбраннымРесурсам"].Вставить("ВидСравнения",
	...
	Если НЕ ВариантПериода = "ПоРесурсам" И
		...
		// Если недоступна роль CRM_ПросмотрЗадачДругихПользователейВКалендаре, тогда доступны только объекты
	...
	Если НЕ бДляРежимаОтображенияИспользуютсяВыбранныеРесурсы Тогда
		...
		СтруктураРезультат.Отборы["Пользователь"].Вставить("Значение",		СписокПользователей);
		СтруктураРезультат.Отборы["Пользователь"].Вставить("Использование",	(СписокПользователей.Количество() > 0));
	...
	// Если задан редактируемый объект и временя совпадают, значит необходимо
	// получить по нему данные из запроса, чтобы отобразить в календаре.
	Если ЗначениеЗаполнено(РежимВыбора_ОбъектАдресат) Тогда
		ПериодДанных	= ПолучитьПериодДанных(ВариантПериода, ДатаОтображения);
		ДатаНачало		= ПериодДанных.ДатаНачала;
		ДатаКонец		= ПериодДанных.ДатаОкончания;
		
		Если (РежимВыбора_ДатаИВремяНачала >= ДатаНачало И РежимВыбора_ДатаИВремяНачала <= ДатаКонец)
				Или (РежимВыбора_ДатаИВремяЗавершения >= ДатаНачало И РежимВыбора_ДатаИВремяЗавершения <= ДатаКонец) Тогда
			СтруктураРезультат.ТекстЗапроса = СтрЗаменить(
				СтруктураРезультат.ТекстЗапроса,
				"//%УСЛОВИЕ_РЕДАКТИРОВАНИЕ_ОБЪЕКТА%",
				"ИЛИ СобытияКалендаряПользователя.Объект = &РедактируемыйОбъект");
			
			СтруктураРезультат.Параметры = Новый Структура();
			СтруктураРезультат.Параметры.Вставить("РедактируемыйОбъект", РежимВыбора_ОбъектАдресат);
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьДанныеСобытийЗаПериодПоДням(ПериодНачало = Неопределено, ПериодОкончание = Неопределено)
	...
	Если ТипЗнч(СтруктураДанныхЗапроса.Отборы) = Тип("Соответствие") Тогда
		СоздатьОтборПоОписанию(Настройки.Отбор.Элементы, СтруктураДанныхЗапроса.Отборы,
	...
	//	УстановитьЗначениеВОтборе(Настройки.Отбор, "СобытиеНаВесьДень",
		// ВидСравненияКомпоновкиДанных.Равно, Ложь, Истина,
		// КомпоновщикНастроек.Настройки.ДоступныеПоляВыбора);
	// КонецЕсли;
	
	КомпоновщикМакета = Новый КомпоновщикМакетаКомпоновкиДанных;
	МакетКомпоновки = КомпоновщикМакета.Выполнить(СКДДанныеКалендаря, Настройки, , ,
...
Процедура СформироватьКалендарьНаСервере()
	...
	Для Каждого СтрокаРезультата Из РезультатТаблица Цикл
		...
		Если СтрокаРезультата.Объект = РежимВыбора_ОбъектАдресат Тогда
			НоваяСтрока.Дата			= НачалоДня(РежимВыбора_ДатаИВремяНачала);
			НоваяСтрока.ПериодНачало	= РежимВыбора_ДатаИВремяНачала;
			НоваяСтрока.ПериодОкончание	= РежимВыбора_ДатаИВремяЗавершения;
			НоваяСтрока.НаВесьДень		= РежимВыбора_ВесьДень;
		КонецЕсли;
		
		НоваяСтрока.Время = ВремяМероприятия(
			НоваяСтрока.ПериодНачало,
			НоваяСтрока.ПериодОкончание,
			НоваяСтрока.НаВесьДень);
		
		Если НоваяСтрока.Время = "Весь день" И НЕ СтрокаРезультата.НаВесьДень Тогда
			НоваяСтрока.НаВесьДень = Истина;
		КонецЕсли;
		
		Если НачалоДня(НоваяСтрока.Дата) <> ВыводимыйДень Тогда
			ВыводимыйДень = НачалоДня(НоваяСтрока.Дата);
			НоваяСтрока.День = ДатаМероприятия(НоваяСтрока.Дата);
		Иначе
		...
		Если Не ЗначениеЗаполнено(СтрокаРезультата.Партнер)
				И ТипЗнч(СтрокаРезультата.Объект) = Тип("ДокументСсылка.CRM_Взаимодействие")
				И ЗначениеЗаполнено(СтрокаРезультата.Объект.ПотенциальныйКлиент) Тогда
		...
		ИначеЕсли ТипЗнч(СтрокаРезультата.Объект) = Тип("ЗадачаСсылка.ЗадачаИсполнителя")
			И ЗначениеЗаполнено(СтрокаРезультата.Партнер) Тогда
			НоваяСтрока.Партнеры = СтрокаРезультата.Партнер;
		КонецЕсли;
	...
	Если РежимВыбора И Не ЗначениеЗаполнено(РежимВыбора_ОбъектАдресат) Тогда
		НоваяСтрока = Мероприятия.Добавить();
		НоваяСтрока.Дата			= НачалоДня(РежимВыбора_ДатаИВремяНачала);
		НоваяСтрока.ПериодНачало	= РежимВыбора_ДатаИВремяНачала;
		НоваяСтрока.ПериодОкончание	= РежимВыбора_ДатаИВремяЗавершения;
		НоваяСтрока.НаВесьДень		= РежимВыбора_ВесьДень;
		НоваяСтрока.ОбъектСсылка	= РежимВыбора_ОбъектАдресат;
		НоваяСтрока.Объект			= РежимВыбора_ОбъектАдресат;
	КонецЕсли;
...
Процедура КалендарьОбновитьКлиент(Параметры = Неопределено)
	...
	Если ВариантПериода = "Список" Или ВариантПериода = "ПоРесурсам" Тогда
		Если ТипЗнч(Параметры) <> Тип("Структура")
			 Или Не Параметры.Свойство("НеЗаполнятьСписокИдентификаторовВыделенныхСтрокВДеревеРесурсов") Тогда
			ЗаполнитьСписокИдентификаторовВыделенныхСтрокВДеревеРесурсов();
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьОписаниеОтбораДляЭлемента(Элемент, КонтекстЭтаФорма)
	...
	Если ИмяПоляОтбора = "Пользователь" Тогда
		...
		СтруктураРезультат.ВидСравненияПоУмолчанию = ВидСравненияКомпоновкиДанных.ВСписке;
		СтруктураРезультат.ЗначениеПоУмолчанию = КонтекстЭтаФорма.СписокПользователей;
...
Функция ПолучитьПредставленияОтборов()
	...
	СтруктураРезультат.Вставить("СтатусКонтрольнойТочки",
					НСтр("ru='Статус контрольной точки';en='Status of check point'"));
...
&НаСервереБезКонтекста
Функция ОбновитьСписокПользователей(ОтборЗначениеПользователь)
	Если ЗначениеЗаполнено(ОтборЗначениеПользователь) Тогда
		Возврат РегистрыСведений.CRM_ОтсутствиеСотрудников.ЗамещаемыеПользователи(ОтборЗначениеПользователь);
	Иначе
...
Процедура ПроверитьЗаполнениеОтбораПоПользователю(бБылиИзмененияВОтборах = Неопределено)
	...
	Для Каждого ПолеОтбора Из МассивДоступныхОтборов Цикл
		...
		Если СтрНайти(",Пользователь,Подразделение,Партнер,Проект,Помещение,", "," + ПолеОтбора + ",") = 0 Тогда
			Продолжить;
		КонецЕсли;
	...
	Если бВключитьОтборПоТекущемуПользователю Тогда
		Если Не ЗначениеЗаполнено(ОтборЗначениеПользователь) Тогда
			...
			Для Каждого ПолеОтбора Из МассивДоступныхОтборов Цикл
				Если СтрНайти(",Пользователь,Партнер,Проект,Помещение,", "," + ПолеОтбора + ",") = 0 Тогда
					Продолжить;
				КонецЕсли;
...
Процедура ОтборЗначениеПриИзменении(Элемент)
	...
	Если Элемент = Элементы.ОтборЗначениеПользователь Тогда
		СписокПользователей.ЗагрузитьЗначения(ОбновитьСписокПользователей(ОтборЗначениеПользователь));
		ОтборЗначениеПодразделение = ПредопределенноеЗначение("Справочник.СтруктураПредприятия.ПустаяСсылка");
	КонецЕсли;
	...
	Если ОписаниеОтбора.ИмяИсточникаДанныхЗначение = "ОтборЗначениеПользователь" Тогда
		CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.Взаимодействия.КалендарьСинхронизацияСGoogle.Удобство.КоличествоНажатийОтборПользователь");
	ИначеЕсли ОписаниеОтбора.ИмяИсточникаДанныхЗначение = "ОтборЗначениеПодразделение" Тогда
		CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.Взаимодействия.КалендарьСинхронизацияСGoogle.Удобство.КоличествоНажатийОтборПодразделение");
	ИначеЕсли ОписаниеОтбора.ИмяИсточникаДанныхЗначение = "ОтборЗначениеПомещение" Тогда
		CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.Взаимодействия.КалендарьСинхронизацияСGoogle.Удобство.КоличествоНажатийОтборПомещение");
	ИначеЕсли ОписаниеОтбора.ИмяИсточникаДанныхЗначение = "ОтборЗначениеКлиент" Тогда
		CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.Взаимодействия.КалендарьСинхронизацияСGoogle.Удобство.КоличествоНажатийОтборКлиент");
	ИначеЕсли ОписаниеОтбора.ИмяИсточникаДанныхЗначение = "ОтборЗначениеПроект" Тогда
		CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.Взаимодействия.КалендарьСинхронизацияСGoogle.Удобство.КоличествоНажатийОтборПроект");
	КонецЕсли;
...
Процедура ОтборЗначениеОчистка(Элемент, СтандартнаяОбработка)
	...
	Если СтрНайти(",Пользователь,Подразделение,Партнер,Проект,Помещение,", "," + ОписаниеОтбора.ПолеОтбора + ",") > 0 Тогда
...
Процедура ПриПолученииРезультатаРаспознавания(РезультатРаспознавания, ПродолжитьРаспознавание, Контекст) Экспорт
	...
	ТекущийЭлемент = Элементы.КалендарьСписокТекстПоиска;
...
Функция РаспознаваниеРечи_МассивПолей()
	...
	CRM_РаботаСРечьюБМОСервер.ДополнитьМассивПолейРаспознавания(МассивПолей, "КалендарьСписокТекстПоиска");
...
// Обрабатывает результат выбора из меню создания нового взаимодействия/задачи
//
// Параметры:
//  ВыбранныйЭлемент  - выбранное значение из меню или Неопределено
//  Параметры  - дополнительные параметры
//
&НаКлиенте
Процедура СоздатьПослеВыбораИзМеню(ВыбранныйЭлемент, Параметры) Экспорт
	
	Если ВыбранныйЭлемент = Неопределено Тогда
		Возврат;
	ИначеЕсли ВыбранныйЭлемент.Значение = "Взаимодействие" Тогда
		КомандаФормы = Команды.Найти("Создать");
		Создать(КомандаФормы);
	ИначеЕсли ВыбранныйЭлемент.Значение = "ЛичнаяЗадача" Тогда
		КомандаФормы = Команды.Найти("СоздатьЛичнуюЗадачу");
		СоздатьЛичнуюЗадачу(КомандаФормы);
	КонецЕсли;
