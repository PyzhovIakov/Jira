...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Свойство("МассивОбъектов", МассивОбъектов) Тогда
		ВызватьИсключение НСтр("ru='Обработка не предназначена для непосредственного использования.';
			|en='Handling are not intended for immediate usage.'");
	КонецЕсли;
...
Процедура Сформировать(Команда)
	...
	Если Не ЗначениеЗаполнено(Наименование) Тогда
		ПоказатьПредупреждение(,
			 НСтр("ru='Не заполнена тема поручения.';
			|en='The topic of the assignment is not completed.'"));
		Возврат;
	...
	Иначе	
		...
		Если ПустыеСтроки.Количество() = ТаблицаОбъектов.Количество() Тогда
			ПоказатьПредупреждение(,
				 НСтр("ru='В списке не указано ни одного исполнителя. ';
				|en='There is any executor in list. '"));
			Возврат;
		
		КонецЕсли;	
			
		Если ПустыеСтроки.Количество() > 0 Тогда
			ПоказатьВопрос(Опопвещение,
				 НСтр("ru='Список содержит строки с незаполненным исполнителем. Такие строки будут пропущены. "
...
Функция СформироватьСервер()
	...
	Попытка
		Пока Выборка.Следующий() Цикл
			...
			ПараметрыФормирования.Вставить("Дата", ТекущаяДатаСеанса());
			ПараметрыФормирования.Вставить("ДатаИсполнения",
				 CRM_ОбщегоНазначенияКлиентСервер.СформироватьДатуИзДатыИВремени(ДатаИсполнения,
				 ВремяИсполнения));
			ПараметрыФормирования.Вставить("ДатаКонтроля",
				 CRM_ОбщегоНазначенияКлиентСервер.СформироватьДатуИзДатыИВремени(ДатаКонтроля,
				 ВремяКонтроля));
			...
			ПараметрыФормирования.Вставить("Исполнитель", ?(ЗначениеЗаполнено(Выборка.Исполнитель),
				 Выборка.Исполнитель,
				 Исполнитель));
