...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УстановитьПараметрыСписковПоНастройкам(ЭтотОбъект);
...
Процедура НастроитьОтборПоКлассификаторам(Команда)
	...
	ПараметрыФормы.Вставить("ЛогическоеВыражение"	  , ЭтотОбъект["ЛогическоеВыражениеОтбораКлассификаторов" 
		+ СтруктураРаздела.ИмяСписка]);
...
Процедура НастроитьОтборПоКлассификаторамЗавершение(СтруктураЗначенийКлассификаторов, ДополнительныеПараметры) Экспорт
	...
	ЭтотОбъект["ЛогическоеВыражениеОтбораКлассификаторов" 
		+ ДополнительныеПараметры.ИмяСписка] = СтруктураЗначенийКлассификаторов.ЛогическоеВыражение;
	УстановитьОтборПоКлассификаторам(СтруктураЗначенийКлассификаторов.ЗначенияКлассификаторов,
		 СтруктураЗначенийКлассификаторов.ЗначенияРеквизитов);
...
Процедура СохранитьНастройку(Команда)
	...
	ПараметрыНастройки.Вставить("ЛогическоеВыражениеОтбораКлассификаторов",
		 ЭтотОбъект["ЛогическоеВыражениеОтбораКлассификаторов" 
		+ СтруктураРаздела.ИмяСписка]);
	...
	СтруктураБыстрыхОтборов.Вставить("ОтборДатаВключенияКлассификатораВидСравнения",
		 ОтборДатаВключенияКлассификатораВидСравнения);
...
Процедура Подключаемый_ВариантыСписка(Команда)
	...
	ОткрытьФорму("Справочник.CRM_ВариантыСписковУправленияКлассификацией.ФормаСписка", , ЭтотОбъект,
...
Процедура ЗаполнитьДанныеТекущегоРаздела(Форма, ТекущийРаздел)
	...
	СтруктураРаздела.ОтборСписокЗначенийКлассификаторов =
		ОбщегоНазначенияКлиентСервер.СкопироватьСписокЗначений(Форма.ОтборСписокЗначенийКлассификаторов);
...
Процедура ПеренестиДанныеТекущегоРаздела(Форма, ТекущийРаздел)
	...
	Форма.ОтборСписокЗначенийКлассификаторов =
		ОбщегоНазначенияКлиентСервер.СкопироватьСписокЗначений(СтруктураРаздела.ОтборСписокЗначенийКлассификаторов);
	
	СписокПринадлежность = Новый СписокЗначений;
	СписокПринадлежность.Добавить(СтруктураРаздела.КлассификаторПринадлежность);
	
	ПараметрыВыбора = Новый Массив;
	ПараметрыВыбора.Добавить(Новый ПараметрВыбора("Отбор.СпособФормирования",
		 ПредопределенноеЗначение("Перечисление.СпособыФормированияСегментов.ФормироватьВручную")));
...
Процедура СформироватьВыбранныеПоля()
	...
	Запрос.Текст = 
		...
		|	CRM_ЗначенияКлассификаторов.Код КАК Код,
	...
	Пока Выборка.Следующий() Цикл
		...
		Элемент = Элементы.Добавить(ИмяТаблицы + "_" + Формат(Сч, "ЧГ="), Тип("ПолеФормы"),
			 Элементы["ПоляЗначенийКлассифкатора" 
			+ ИмяТаблицы]);
		...
		НоваяСтрока.Аналитика =
			CRM_КлассификаторыЭкспортныеМетоды.ПолучитьЗначениеИспользуемойАналитикиКлассификатора(Классификатор);
...
Процедура ОбработатьДанныеСтрокиКлиент(ДанныеСтроки, ИмяСписка, ИмяПоля, СтандартнаяОбработка)
	...
	СтрокаПоля = НайтиСтрокуСоответствия(ЭтотОбъект, ИмяСписка, ИмяПоля);
	...
	Если ЗначениеЗаполнено(СтрокаПоля.ДополнительныйРеквизит) Тогда
		...
		ПоказатьВводЗначения(Оповещение, СтрокаКлассификатора.ЗначениеРеквизита,
			 Элементы[ИмяПоля].Заголовок,
...
Процедура ИзменитьКлассификатор(ОбъектСсылка, СтрокаКлассификатора, Включить, Рекурсивно = Ложь)
	...
	Попытка
		...
		Если Рекурсивно
			 И СтрокаКлассификатора.ВидОтображения = Перечисления.CRM_ВидыОтображенияКлассификаторов.ОдиночноеЗначение Тогда
...
&НаКлиенте
Процедура УстановитьОтборПоКлассификаторам(Знач ЗначенияКлассификаторов = Неопределено,
	 Знач ЗначенияРеквизитов = Неопределено)
	...
	ГруппаИЛИ = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(Список.Отбор.Элементы,
		 "ГруппаИЛИЗначенийИРеквизитовКлассификатора",
	...
	Если ЗначенияКлассификаторов.Количество() > 0 Тогда
		...
		Для Сч = 0 По КоличествоЭлементовОтбора - 1 Цикл
			ГруппаИ = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(ГруппаИЛИ.Элементы, "ГруппаИЗначенийИРеквизитовКлассификатора" + Формат(Сч,
				 "ЧГ="),
...
Процедура УстановитьПараметрыСписковПоНастройкам(Форма)
	...
	#Если Клиент Тогда
		КонецПериода  = КонецДня(ОбщегоНазначенияКлиент.ДатаСеанса());
	#КонецЕсли
	
	НачалоПериода = CRM_КлассификаторыКлиентСервер.РассчитатьНачалоПериода(КонецПериода,
		 Форма.Настройки.ИнтервалПродаж,
		 Форма.Настройки.ПериодичностьПродаж);
	
	Для каждого КлючЗначение Из Форма.ДанныеРазделов Цикл
		СтруктураРаздела = КлючЗначение.Значение;
		
		ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(Форма[СтруктураРаздела.ИмяСписка],
			 "НачалоПериода",
			 НачалоПериода);
		ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(Форма[СтруктураРаздела.ИмяСписка],
			 "КонецПериода" ,
...
Процедура ОбновитьОтображениеУстановленныхОтборов(ИмяСписка)
	...
	Если ДекорацияЛогическоеВыражениеВидимость Тогда
		ЭлементДекорацияЛогическоеВыражение.Заголовок = ?(ЛогическоеВыражениеОтбораКлассификаторов = "ИЛИ",
			 НСтр("ru='Один из: ';en='One of: '"),
			 НСтр("ru='Все: '"));
	КонецЕсли;
...
Процедура СохранитьНастройки(СохраняемыеНастройки)
	...
	CRM_ХранилищеНастроек.Сохранить(ИмяФормы, "Настройки", СохраняемыеНастройки);	
...
Процедура ВариантСпискаПриИзменении(ИмяКоманды)
	...
	СтруктураРаздела = СтруктураТекущегоРаздела(ЭтотОбъект);	
	CRM_КлассификаторыЭкспортныеМетоды.ИницализироватьНаборДанныхСписка(ЭтотОбъект, ИмяКоманды,
		 ПолучитьПараметрыСписка(СтруктураРаздела.ИмяСписка));
...
Процедура СформироватьПредставлениеКорзины(Форма, СтруктураРаздела = Неопределено)
	...
	Иначе
		ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='выбрано (%1)';en='selected (%1)'"),
			 Формат(ЭлементовКорзины,
			 "ЧГ="));	
	КонецЕсли;
...
Процедура ИнициализироватьНастройки(СтруктураРаздела, Настройка)
	...
	ВариантСпискаПриИзменении(?(ЗначениеЗаполнено(Настройка.ВариантСписка), Настройка.ВариантСписка, "ОсновнойВариант" 
		+ ИмяСписка));
	...
	ЭтотОбъект["ЛогическоеВыражениеОтбораКлассификаторов" 
		+ ИмяСписка] = Настройка.ЛогическоеВыражениеОтбораКлассификаторов;
