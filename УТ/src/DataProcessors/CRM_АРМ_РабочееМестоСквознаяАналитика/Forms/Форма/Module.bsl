...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ CRM_ЛицензированиеЭкспортныеМетоды.ПодсистемаCRMИспользуется() Тогда
		CRM_ЛицензированиеЭкспортныеМетоды.ВывестиУведомлениеОНедоступности(НСтр("ru = 'форму Сквозной аналитики'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	
	CRM_ЛицензированиеЭкспортныеМетоды.ПолучитьЗащищеннуюОбработку().
		Обработка_CRM_АРМ_РабочееМестоСквознаяАналитика_Форма_ПриСозданииНаСервере(
	...
	Для Каждого ТекЭлемент Из Элементы.ДС_Свод.ПодчиненныеЭлементы Цикл
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ТекЭлемент, "Заголовок")
			И СтрНайти(ТекЭлемент.Заголовок, "%ВалютаУчета%") > 0 Тогда
	...
	НастройкиОтчета.Вставить("Контекстный", (Параметры.Свойство("Контекст") И ЗначениеЗаполнено(Параметры.Контекст))
		 ИЛИ (Параметры.Свойство("ОтборРасшифровки")
		 И Параметры.ОтборРасшифровки.Количество() > 0));
	...
	НастройкиОтчета.Вставить("ФормироватьНепосредственно", НастройкиОтчета.Внешний Или НастройкиОтчета.Безопасный
		 ИЛИ (Параметры.Свойство("ФормироватьНепосредственно")
		 И Параметры.ФормироватьНепосредственно));
	...
	ДС_Свод.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Вставить("Период", Период);
...
Процедура ПриОткрытии(Отказ)
	...
	УстановитьВидимостьЭлементовИКнопок(Ложь, Ложь, Ложь, Истина);
...
Процедура ПредставлениеПериодаНажатиеЗавершение(НовыйПериод, ДополнительныеДанные) Экспорт
	...
	ДС_Свод.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Вставить("Период", НовыйПериод);
...
Процедура ВидДиаграммыНачалоВыбораЗадержка()
	...
	Меню.Добавить(ТипДиаграммы.Гистограмма, НСтр("ru='Обычные гистограммы';en='Usual histograms'"), ,
		 КартинкаДиаграммы(ТипДиаграммы.Гистограмма));
	Меню.Добавить(ТипДиаграммы.ГистограммаГоризонтальная,
		 НСтр("ru='Горизонт. гистограммы';en='Horizon. histogram'"), ,
		 КартинкаДиаграммы(ТипДиаграммы.ГистограммаГоризонтальная));
	Меню.Добавить(ТипДиаграммы.Круговая, НСтр("ru='Круговые диаграммы';en='Pie chart'"), ,
		 КартинкаДиаграммы(ТипДиаграммы.Круговая));
	Меню.Добавить(ТипДиаграммы.Изометрическая,
		 НСтр("ru='Изометрические диаграммы';en='Isometric diagrams'"), ,
		 КартинкаДиаграммы(ТипДиаграммы.Изометрическая));
...
Процедура ВидДиаграммыВыборГруппы(Значение, ДополнительныеДанные) Экспорт
	...
	Если Значение.Значение = ТипДиаграммы.График Тогда
		...
		Меню.Добавить(ТипДиаграммы.ГрафикСОбластямиИНакоплением,
			 НСтр("ru='С областями и накоплением';en='With areas and accumulation'"));
	...
	ИначеЕсли Значение.Значение = ТипДиаграммы.Гистограмма Тогда
		...
		Меню.Добавить(ТипДиаграммы.ГистограммаСНакоплениемОбъемная,
			 НСтр("ru='Объемная с накоплением';en='Volume with accumulation'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаНормированнаяОбъемная,
			 НСтр("ru='Объемная нормированная';en='Volume normalized'"));
	ИначеЕсли Значение.Значение = ТипДиаграммы.ГистограммаГоризонтальная Тогда
		...
		Меню.Добавить(ТипДиаграммы.ГистограммаСНакоплениемГоризонтальнаяОбъемная,
			 НСтр("ru='Объемная с накоплением';en='Volume with accumulation'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаНормированнаяГоризонтальнаяОбъемная,
			 НСтр("ru='Объемная нормированная';en='Volume normalized'"));
...
Процедура ПриИзмененииВидаДиаграммы()
	...
	Если НЕ ВидДиаграммы = Неопределено Тогда
		...
		Если Параметр = Неопределено ИЛИ НЕ Параметр.Использование Тогда
			Параметр =
				КомпоновщикНастроек.Настройки.ПараметрыВывода.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("ТипДиаграммы"));
		КонецЕсли; 
...
Функция СписокБыстрыхФильтров()
	...
	Если НастройкиОтчета.Свойство("ДополнительныеФильтры")
		 И ТипЗнч(НастройкиОтчета.ДополнительныеФильтры) = Тип("СписокЗначений") Тогда
		Для каждого ОписаниеПоля Из НастройкиОтчета.ДополнительныеФильтры Цикл
			...
			КонецЕсли; 
			ДоступноеПоле =
				КомпоновщикНастроекТаблица.Настройки.ДоступныеПоляВыбора.НайтиПоле(Новый ПолеКомпоновкиДанных(ОписаниеПоля.Значение));
...
Процедура ДобавитьФильтрЗавершение(Элемент, ДополнительныеДанные) Экспорт
	...
	Если Элемент.Значение = Неопределено Тогда
		...
		СтруктураПараметров.Вставить("АдресНастроек",
			 ПоместитьВоВременноеХранилище(КомпоновщикНастроекТаблица.Настройки,
			 УникальныйИдентификатор));
		ОткрытьФорму("Обработка.CRM_АРМ_РабочееМестоСквознаяАналитика.Форма.CRM_ФормаСпискаПолей",
...
&НаСервере
Процедура УстановитьВидимостьЭлементовИКнопок(Кампании, Цели, Источники, Свод)
...
Процедура Подключаемый_ПараметрПриИзменении(Элемент)
	...
	Иначе
		...
		Иначе	
			Параметр =
				КомпоновщикНастроекТаблица.Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных(Стр.Поле));
...
Процедура Подключаемый_ФильтрПриИзменении(Элемент)
	...
	Если Стр.НестандартныйОбработчик Тогда
		ВыполнитьНестандартнуюОбработку(Стр.ПолучитьИдентификатор(), Элемент.Имя);
	ИначеЕсли СтрНайти(Стр.Поле, "ДобавленноеПоле_") > 0 Тогда
		Значение = ЗначениеПоляФильтра(Стр);
	...
	Если НЕ ПараметрыВыбора = Неопределено Тогда
		...
		Если ТипЗнч(МассивПараметров) = Тип("Массив") Тогда
			...
			Для каждого Параметр Из МассивПараметров Цикл
				Если СтрНайти(Параметр.Имя, "Отбор.") = 0 Тогда
					Продолжить;
				КонецЕсли; 
...
Процедура Подключаемый_ИерархияФильтраНажатие(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если ЭлементПоле.ВыборГруппИЭлементов = ГруппыИЭлементы.Элементы Тогда
		...
		Элемент.Подсказка = НСтр("ru='Используется выбор групп. Нажмите для выбора элементов.';
			|en='Use select of groups. Press to select items.'");
	...
	Иначе
		...
		Элемент.Подсказка = НСтр("ru='Используется выбор элементов. Нажмите для выбора групп.';
			|en='Use elements selection. Click to select groups.'");
...
Процедура УстановитьВидимостьИДоступность(Событие = "")
	...
	Если ПолнаяИнициализация Тогда
		...
		Элементы.ГруппаСравнение.Видимость = НастройкиОтчета.ИспользоватьСравнение
			 И НастройкиОтчета.РежимПериода = "ЗаПериод";
	...
	Если ИзмененыПоказатели ИЛИ ИзмененыПериодыСравнения Тогда
		...
		Элементы.ГруппаВариантыСравнения.Видимость = ((КоличествоПоказателей = 1 ИЛИ НЕ Истина)
			 И ТипЗнч(ПолеСравнения) = Тип("ПеречислениеСсылка.Периодичность")
			 И НастройкиОтчета.ИспользоватьДополнительныеРежимыСравнения);
	КонецЕсли;
	...
	Если ИзмененРежимРедактирования Тогда
		...
		КонецЦикла; 
		Элементы.РежимРедактированияСтрок.Картинка = ?(РежимРедактированияСтрок,
			 БиблиотекаКартинок.CRM_РежимРедактированияСпискаЭлементовАктивно,
			 БиблиотекаКартинок.CRM_РежимРедактированияСпискаЭлементов);
		Элементы.РежимРедактированияКолонок.Картинка = ?(РежимРедактированияКолонок,
			 БиблиотекаКартинок.CRM_РежимРедактированияСпискаЭлементовАктивно,
			 БиблиотекаКартинок.CRM_РежимРедактированияСпискаЭлементов);
	КонецЕсли;
...
Процедура ОбновитьЗаголовокЭлемента(Стр)
	...
	Если Стр.Тип <> ТипГруппировка() И Стр.Тип <> ТипРеквизит() И Стр.Тип <> ТипПоказатель()
		 И Стр.Тип <> ТипГруппаПоказателей() Тогда
		Возврат;
	КонецЕсли;
	Если Стр.Тип = ТипРеквизит() Тогда
		ШиринаЗаголовка = 23 - ?(РежимРедактированияСтрок, 7, 0);
	ИначеЕсли Стр.Тип = ТипГруппировка() Тогда 
		ШиринаЗаголовка = 28 - ?(РежимРедактированияСтрок, 7,
			 0) - ?(Стр.ДополнительныеПараметры.Свойство("ТипГруппировки"), 4,
			 0);
...
Функция ТаблицаВариантов(ТолькоВидимые = Истина)
	...
	ИначеЕсли ТолькоВидимые Тогда
		...
		Запрос.Параметры.Вставить("ОтключенныеПредопределенныеВарианты",
			 CRM_ОтчетыПовтИсп.ПредопределенныеВариантыНедоступныеПоФО());
...
Функция ЭлементСтуктурыДиаграмма(ЭлементыСтруктуры, Создать = Ложь)
	...
	КонецЦикла;
	Если Создать Тогда
		ЭлементСтруктуры = ?(ЭлементыСтруктуры.Количество() > 0, ЭлементыСтруктуры.Вставить(0,
			 Тип("ДиаграммаКомпоновкиДанных")),
			 ЭлементыСтруктуры.Добавить(Тип("ДиаграммаКомпоновкиДанных")));
...
Процедура СформироватьОбновитьДС()
	...
	ДС_Свод.Параметры.УстановитьЗначениеПараметра("Период1", Период.ДатаНачала);
	ДС_Свод.Параметры.УстановитьЗначениеПараметра("Период2", Период.ДатаОкончания);
	ДС_Свод.Параметры.УстановитьЗначениеПараметра("ФильтрРекламныеКампании", ФильтрРекламныеКампании);
	ДС_Свод.Параметры.УстановитьЗначениеПараметра("РекламныеКампании", ОтборРекламныеКампании);
...
Процедура СкопироватьСтруктуруРекурсивно(Источник, Приемник, СхемаКомпоновкиДанных, СкрытьСравнение = Ложь)
	...
	Для каждого ЭлементИсточник Из Источник Цикл
		Если ТипЗнч(ЭлементИсточник) = Тип("ГруппировкаКомпоновкиДанных")
			 ИЛИ ТипЗнч(ЭлементИсточник) = Тип("ГруппировкаТаблицыКомпоновкиДанных") Тогда
			...
			Для каждого ПорядокИсточник Из ЭлементИсточник.Порядок.Элементы Цикл
				...
				КонецЕсли; 
			КонецЦикла; 
			СкопироватьСтруктуруРекурсивно(ЭлементИсточник.Структура, ЭлементПриемник.Структура,
...
Процедура ОбновитьНастройкиДиаграммы()
	...
	КонецЕсли;
	Если НЕ НастройкиОтчета.НестандартнаяДиаграмма И НЕ НастройкиОтчета.ВыводитьТолькоТочки
		 И ЗначениеЗаполнено(ДиаграммаТочки)
		 И ЗначениеЗаполнено(ДиаграммаСерии) Тогда
...
Процедура УдалитьЭлементыРекурсивно(Группа, МассивЭлементов, МассивРеквизитов)
	...
	Для каждого Элемент Из Группа.ПодчиненныеЭлементы Цикл
		...
		ИначеЕсли ТипЗнч(Элемент) = Тип("ПолеФормы") Тогда
			...
			Позиция = СтрНайти(Элемент.ПутьКДанным, ".");
...
Процедура ВнестиИзмененияВРеквизитыНаФорме(ПолнаяЗамена = Истина)
	...
	Если ПолнаяЗамена Тогда
		...
		УдалитьЭлементыРекурсивно(Элементы.ГруппаДополнительныеНастройки, МассивУдаляемыхЭлементов,
	...
	Для каждого Идентификатор Из Идентификаторы Цикл
		...
		ИначеЕсли Стр.Тип = ТипПараметр() Тогда
			Если Стр.ВидЭлемента = "Поле" Тогда
				Параметр =
					КомпоновщикНастроекТаблица.Настройки.ПараметрыДанных.ДоступныеПараметры.НайтиПараметр(Новый  ПараметрКомпоновкиДанных(Стр.Поле));
...
Процедура ЗаполнитьВариантыГруппировкиКолонок()
	...
	Элементы.ПолеГруппировкиКолонок.СписокВыбора.Добавить("<Прочие>", НСтр("ru='Прочее';en='Other'"), ,
...
Функция ЗаголовокГруппыПолей(СтрРодитель)
	...
	Для каждого СтрРеквизита Из СтрРодитель.ПолучитьЭлементы() Цикл
		ЗаголовокГруппировки = ЗаголовокГруппировки + ?(ПустаяСтрока(ЗаголовокГруппировки), "", ", ") 
			+ СтрРеквизита.Заголовок;
	КонецЦикла;
...
Процедура ЗаполнитьОписанияГруппировок()
	...
	Если ДополнительныеСвойства.Свойство("Сравнение")
		 ...
		 И НЕ ЗначениеЗаполнено(ПолеСравнения) Тогда
		...
		КонецЕсли; 
	КонецЕсли; 
	Если ДополнительныеСвойства.Свойство("ГруппировкаКолонок")
		 И ЗначениеЗаполнено(ДополнительныеСвойства.ГруппировкаКолонок)
		 И НЕ ЗначениеЗаполнено(ПолеГруппировкиКолонок) Тогда
...
Процедура ДобавитьОписанияГруппировокРекурсивно(ЭлементыСтруктуры, ЭтоСтрока = Истина)
	...
	Для каждого Элемент Из ЭлементыСтруктуры Цикл
		...
		ИначеЕсли ЭтоСтрока И (ТипЗнч(Элемент) = Тип("ГруппировкаКомпоновкиДанных")
			 ИЛИ ТипЗнч(Элемент) = Тип("ГруппировкаТаблицыКомпоновкиДанных")) Тогда
			...
			КонецЕсли; 
			Стр = ДобавитьОписаниеНастройки(ПоляНастроек.ПолучитьЭлементы(), ТипГруппировка(), ИмяРеквизита,
				 ЗаголовокГруппировки,
				 Элемент);
			Если Стр = Неопределено Тогда
				Продолжить;
		...
		ИначеЕсли НЕ ЭтоСтрока И ТипЗнч(Элемент) = Тип("ГруппировкаТаблицыКомпоновкиДанных") Тогда
			...
			Если НЕ ДополнительныеСвойства.Свойство("ГруппировкаКолонок") И НЕ ДополнительныеСвойства.Свойство("Сравнение") Тогда
				...
				Если ИмяПоля = "ДинамическийПериод" Тогда
					...
					Иначе
						ЗначениеСравнения =
							КомпоновщикНастроекТаблица.Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("Периодичность"));
						ПолеСравнения = ЗначениеСравнения.Значение;
					КонецЕсли; 
				Иначе
					Если ЗначениеЗаполнено(ПолеГруппировкиКолонок) И НЕ ЗначениеЗаполнено(ПолеСравнения)
						 И НастройкиОтчета.ИспользоватьСравнение Тогда
						Если Элементы.ПолеСравнения.СписокВыбора.НайтиПоЗначению(ПолеГруппировкиКолонок) = Неопределено Тогда
							ДоступноеПолеГруппировки =
								КомпоновщикНастроекТаблица.Настройки.ДоступныеПоляВыбора.НайтиПоле(ПолеГруппировкиКолонок);
							Элементы.ПолеСравнения.СписокВыбора.Вставить(Элементы.ПолеСравнения.СписокВыбора.Количество() - 1,
								 ПолеГруппировкиКолонок,
								 ДоступноеПолеГруппировки.Заголовок); 
						КонецЕсли;
						ПолеСравнения = ПолеГруппировкиКолонок;
					КонецЕсли; 
					Если Элементы.ПолеГруппировкиКолонок.СписокВыбора.НайтиПоЗначению(ИмяПоля) = Неопределено
						 И НЕ ДоступноеПоле = Неопределено Тогда
						Элементы.ПолеГруппировкиКолонок.СписокВыбора.Вставить(Элементы.ПолеГруппировкиКолонок.СписокВыбора.Количество() - 1,
							 ИмяПоля,
							 ДоступноеПоле.Заголовок); 
					КонецЕсли;
					ПолеГруппировкиКолонок = ИмяПоля;
				КонецЕсли; 
			ИначеЕсли ДополнительныеСвойства.Свойство("ГруппировкаКолонок")
				 И НРег(ИмяПоля) = НРег(ДополнительныеСвойства.ГруппировкаКолонок) Тогда
			...
			ИначеЕсли ДополнительныеСвойства.Свойство("Сравнение") И НРег(ИмяПоля) = НРег(ДополнительныеСвойства.Сравнение) Тогда
				...
				Иначе
					...
					Если Элементы.ПолеСравнения.СписокВыбора.НайтиПоЗначению(ИмяПоля) = Неопределено
						 И НЕ ДоступноеПоле = Неопределено Тогда
						Элементы.ПолеСравнения.СписокВыбора.Вставить(Элементы.ПолеСравнения.СписокВыбора.Количество() - 1,
							 ИмяПоля,
							 ДоступноеПоле.Заголовок); 
				...
				КонецЕсли; 
			ИначеЕсли ДополнительныеСвойства.Свойство("Сравнение") И ИмяПоля = "ДинамическийПериод"
				...
				Иначе
					...
					Если Элементы.ПолеСравнения.СписокВыбора.НайтиПоЗначению(ДополнительныеСвойства.Сравнение) = Неопределено Тогда
						Элементы.ПолеСравнения.СписокВыбора.Вставить(Элементы.ПолеСравнения.СписокВыбора.Количество() - 1,
							 ДополнительныеСвойства.Сравнение); 
					КонецЕсли;
					ПолеСравнения = ДополнительныеСвойства.Сравнение;
				КонецЕсли; 
			КонецЕсли; 
	...
	Для каждого Реквизит Из Стр.Реквизиты Цикл
		...
		Иначе
			...
			Если НЕ ПустаяСтрока(ИмяГруппыВставки) Тогда
				Группа = Элементы.Вставить(ИмяРеквизита + "_Группа", Тип("ГруппаФормы"),
					 Элементы.ГруппаСтрокиЭлементы, Элементы[ИмяГруппыВставки]);
			Иначе
				Группа = Элементы.Добавить(ИмяРеквизита + "_Группа", Тип("ГруппаФормы"), Элементы.ГруппаСтрокиЭлементы);
			КонецЕсли; 
		КонецЕсли; 
...
Процедура ДобавитьОтступ(ИмяРеквизита, Группа)
	...
	Если Группа.ПодчиненныеЭлементы.Количество() > 0 Тогда
		КартинкаОтступ = Элементы.Вставить(ИмяРеквизита + "_Отступ", Тип("ДекорацияФормы"), Группа,
			 Группа.ПодчиненныеЭлементы[0]);
	Иначе
...
Процедура ДобавитьПереключательИерархииГруппировки(Стр, ИмяРеквизита, Группа)
	...
	КонецЕсли; 
	ДоступноеПоле =
		КомпоновщикНастроекТаблица.Настройки.ДоступныеПоляВыбора.НайтиПоле(Новый ПолеКомпоновкиДанных(Стр.Поле));
	Если ДоступноеПоле = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Функция ТребуетсяТаблица()
	...
	Возврат Не ПустаяСтрока(ПолеГруппировкиКолонок) ИЛИ (НастройкиОтчета.ИспользоватьСравнение
		 И ЗначениеЗаполнено(ПолеСравнения));
...
&НаСервере
Процедура ДополнитьСписокДобавленныхПоказателей(Группа, СписокПолей)
...
Процедура ОбновитьГруппировкиРазворачивания()
	...
	Для каждого СтрокаГруппировки Из Строки Цикл
		...
		Если СтрокаГруппировки.ВидЭлемента <> "Флажок" ИЛИ ИмяРеквизита = Неопределено
			 ИЛИ ЭтотОбъект[ИмяРеквизита] <> Истина Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ДобавитьОписаниеФильтра(Путь, НестандартныйОбработчик = Ложь, УстановитьФильтр = Истина)
	...
	Если СтрНайти(Путь, "ДобавленноеПоле_") > 0 Тогда
...
Процедура ОпределитьЭлементыИРеквизитыФильтра(Стр, ЭлементОтбора = Неопределено, ДоступноеПоле, РучноеДобавление)
	...
	Если (ТипЗначения.СодержитТип(Тип("Дата")) ИЛИ ТипЗначения.СодержитТип(Тип("Число")))
	...
	ИначеЕсли ТипЗначения.СодержитТип(Тип("Булево")) Тогда
		...
		Значения.Добавить(Перечисления.CRM_БулевоТриСостояния.Ложь, Формат(Ложь,
			 ДоступноеПоле.ФорматРедактирования),
			 Значение = Истина);
		Значения.Добавить(Перечисления.CRM_БулевоТриСостояния.Истина, Формат(Истина,
			 ДоступноеПоле.ФорматРедактирования),
			 Значение = Ложь);
...
Функция ЗначениеПоляФильтра(Стр)
	...
	ИначеЕсли Стр.ВидЭлемента = "Пределы" 
		...
		Если ЗначениеОт = 0 И ЗначениеДо = 0 Тогда
			Возврат Неопределено;
		Иначе
			Возврат Новый Структура("ЗначениеОт, ЗначениеДо", ?(ЗначениеОт = 0, Неопределено, ЗначениеОт),
				 ?(ЗначениеДо = 0, Неопределено,
				 ЗначениеДо));
		КонецЕсли;
	ИначеЕсли Стр.ВидЭлемента = "Флажок" Тогда
		Значения = Новый СписокЗначений;
...
&НаСервере
Процедура УстановитьЗначениеФильтра(Поле, Значение, ВидСравнения = Неопределено,
	 ОбновлятьВПанели = Истина,
	 ЗаменитьЗначение = Ложь)
	...
	ИначеЕсли ТипЗнч(Значение) = Тип("Структура") Тогда
		...
		Для каждого ЭлементОтбор Из НайденныеЭлементы Цикл
			Если НЕ ЗначениеОт = Неопределено
				...
				Продолжить;
			КонецЕсли; 
			Если НЕ ЗначениеДо = Неопределено
				...
				Продолжить;
			КонецЕсли;
			Если НЕ ЗначениеЗаполнено(ЗначениеОт)
				 И (ЭлементОтбор.ВидСравнения = ВидСравненияКомпоновкиДанных.Больше
				 ИЛИ ЭлементОтбор.ВидСравнения = ВидСравненияКомпоновкиДанных.БольшеИлиРавно) Тогда
				// Не изменяем
				Продолжить;
			КонецЕсли; 
			Если НЕ ЗначениеЗаполнено(ЗначениеДо)
				 И (ЭлементОтбор.ВидСравнения = ВидСравненияКомпоновкиДанных.Меньше
				 ИЛИ ЭлементОтбор.ВидСравнения = ВидСравненияКомпоновкиДанных.МеньшеИлиРавно) Тогда
				// Не изменяем
				Продолжить;
	...
	Иначе
		...
		Если КоличествоЭлементов = 0 ИЛИ ЗаменитьЗначение Тогда
			...
			НайденныйЭлемент =
				КомпоновщикНастроекТаблица.Настройки.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		...
		Иначе
			...
			ИначеЕсли НЕ ВидСравнения = НайденныйЭлемент.ВидСравнения Тогда
				...
				Если (НайденныйЭлемент.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно
					 И (ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке
					 ИЛИ ВидСравнения = ВидСравненияКомпоновкиДанных.ВСпискеПоИерархии))
					ИЛИ (НайденныйЭлемент.ВидСравнения = ВидСравненияКомпоновкиДанных.НеРавно
						 И (ВидСравнения = ВидСравненияКомпоновкиДанных.НеВСписке
						 ИЛИ ВидСравнения = ВидСравненияКомпоновкиДанных.НеВСпискеПоИерархии)) Тогда
				...
				ИначеЕсли (ВидСравнения = ВидСравненияКомпоновкиДанных.Равно
					 И (НайденныйЭлемент.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке
					 ИЛИ НайденныйЭлемент.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСпискеПоИерархии))
					ИЛИ (ВидСравнения = ВидСравненияКомпоновкиДанных.НеРавно
						 И (НайденныйЭлемент.ВидСравнения = ВидСравненияКомпоновкиДанных.НеВСписке
			...
			Иначе
				...
				Если ТипЗнч(НайденныйЭлемент.ПравоеЗначение) = Тип("СписокЗначений")
					 И НЕ ТипЗнч(Значение) = Тип("СписокЗначений") Тогда
					Если НайденныйЭлемент.ПравоеЗначение.НайтиПоЗначению(Значение) = Неопределено Тогда
						НайденныйЭлемент.ПравоеЗначение.Добавить(Значение);
					КонецЕсли;
				ИначеЕсли ТипЗнч(НайденныйЭлемент.ПравоеЗначение) = Тип("СписокЗначений")
					 И ТипЗнч(Значение) = Тип("СписокЗначений") Тогда
...
Процедура УдалитьФильтр(Идентификатор)
	...
	Для каждого ЭлементОтбор Из КомпоновщикНастроекТаблица.Настройки.Отбор.Элементы Цикл
		Если ЭлементОтбор.ПредставлениеПользовательскойНастройки = СтрНаложенногоФильтра.ИмяКнопки Тогда
			Если СтрНайти(ЭлементОтбор.ПредставлениеПользовательскойНастройки, "ДобавленноеПоле_") > 0 Тогда
		...
		Если ЭлементОтбор.ВидСравнения = ВидСравненияКомпоновкиДанных.БольшеИлиРавно
			ИЛИ ЭлементОтбор.ВидСравнения = ВидСравненияКомпоновкиДанных.МеньшеИлиРавно Тогда
			МассивКОтключению.Добавить(ЭлементОтбор);
		ИначеЕсли ТипЗнч(ЭлементОтбор.ПравоеЗначение) = Тип("СписокЗначений")
			 И ТипЗнч(СтрНаложенногоФильтра.Значение) = Тип("СписокЗначений") Тогда
			МассивКОтключению.Добавить(ЭлементОтбор);
...
Процедура ОбновитьРеквизитыФильтровВПанелиПоПредставлению(Представление, Значение)
	...
	Строки = СтрокиДерева(ПоляНастроек, СтруктураОтбора);
	Для каждого Стр Из Строки Цикл
		Если НЕ ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Стр.ДополнительныеПараметры,
			 "СвязьПоПредставлению",
			 Ложь) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ОбновитьРеквизитыФильтровВПанели(Поле, Значение, ВидСравнения = Неопределено, Удаление = Ложь)
	...
	ИначеЕсли Стр.ВидЭлемента = "Пределы" Тогда
		...
		ИначеЕсли ТипЗнч(Значение) = Тип("Структура") Тогда 
			ЗначениеОт = Значение.ЗначениеОт; 
			ЗначениеДо = Значение.ЗначениеДо;
		ИначеЕсли ВидСравнения = ВидСравненияКомпоновкиДанных.Больше
			 ИЛИ ВидСравнения = ВидСравненияКомпоновкиДанных.БольшеИлиРавно Тогда
			ЗначениеОт = Значение; 
			ЗначениеДо = Неопределено;
		ИначеЕсли ВидСравнения = ВидСравненияКомпоновкиДанных.Меньше
			 ИЛИ ВидСравнения = ВидСравненияКомпоновкиДанных.МеньшеИлиРавно Тогда
			ЗначениеОт = Неопределено; 
			ЗначениеДо = Значение;
		КонецЕсли;
		Итерация = 1;
...
Процедура ЗаполнитьТаблицуНаложенныхФильтров()
	...
	Для каждого ЭлементОтбор Из КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
		...
		Если НЕ ПустаяСтрока(ЭлементОтбор.Представление)
			 И ЭлементОтбор.РежимОтображения <> РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный Тогда
			...
			Если ТипЗнч(ЭлементОтбор) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных")
				 И ЭлементОтбор.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаНе Тогда
		...
		Если ЭлементОтбор.ВидСравнения = ВидСравненияКомпоновкиДанных.БольшеИлиРавно 
			...
			ИначеЕсли ТипЗнч(Стр.Значение) = Тип("Структура") Тогда
				Стр.Значение.ЗначениеОт = ?(Стр.Значение.ЗначениеОт = Неопределено, ЭлементОтбор.ПравоеЗначение,
					 Макс(?(ЭлементОтбор.ПравоеЗначение = Неопределено, 0, ЭлементОтбор.ПравоеЗначение),
					 Стр.Значение.ЗначениеОт));
		...
		ИначеЕсли ЭлементОтбор.ВидСравнения = ВидСравненияКомпоновкиДанных.МеньшеИлиРавно 
			ИЛИ ЭлементОтбор.ВидСравнения = ВидСравненияКомпоновкиДанных.Меньше Тогда
			Если ТипЗнч(Стр.Значение) = Тип("СтандартныйПериод") Тогда
				Стр.Значение.ДатаОкончания = ?(НЕ ЗначениеЗаполнено(ЭлементОтбор.ПравоеЗначение),
					 ЭлементОтбор.ПравоеЗначение,
					 ЭлементОтбор.ПравоеЗначение);
			ИначеЕсли ТипЗнч(Стр.Значение) = Тип("Структура") Тогда
				Стр.Значение.ЗначениеДо = ?(Стр.Значение.ЗначениеДо = Неопределено, ЭлементОтбор.ПравоеЗначение,
					 Мин(?(ЭлементОтбор.ПравоеЗначение = Неопределено, 0, ЭлементОтбор.ПравоеЗначение),
					 Стр.Значение.ЗначениеДо));
			ИначеЕсли ТипЗнч(Стр.Значение) = Тип("Число") Тогда
				Стр.Значение = Новый Структура("ЗначениеОт, ЗначениеДо");
				Стр.Значение.ЗначениеДо = ?(Стр.Значение.ЗначениеДо = Неопределено, ЭлементОтбор.ПравоеЗначение,
					 Мин(?(ЭлементОтбор.ПравоеЗначение = Неопределено, 0, ЭлементОтбор.ПравоеЗначение),
			...
			КонецЕсли; 
		ИначеЕсли ТипЗнч(ЭлементОтбор.ПравоеЗначение) = Тип("СписокЗначений")
			 И ЭлементОтбор.ПравоеЗначение.Количество() <= 10 Тогда
	...
	Для каждого Стр Из НаложенныеФильтры Цикл
		...
		ИначеЕсли ТипЗнч(Стр.Значение) = Тип("Структура") Тогда
			Если НЕ Стр.Значение.ЗначениеОт = Неопределено И НЕ Стр.Значение.ЗначениеДо = Неопределено Тогда
				ПредставлениеЗначения = НСтр("ru='от ';en='from '") + Строка(Стр.Значение.ЗначениеОт) + НСтр("ru=' до '") 
					+ Строка(Стр.Значение.ЗначениеДо);
		...
		ИначеЕсли ТипЗнч(Стр.Значение) = Тип("Булево") ИЛИ ТипЗнч(Стр.Значение) = Тип("Дата") Тогда
			...
			Иначе
				...
				Если Параметр = Неопределено ИЛИ НЕ ЗначениеЗаполнено(Параметр.Значение) Тогда
					Параметр =
						ПолеСхемы.ПараметрыРедактирования.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("ФорматРедактирования"));
				КонецЕсли; 
			КонецЕсли;
...
Функция ЗаголовокГруппыОтбора(Группа)
	...
	Для каждого Элемент Из Группа.Элементы Цикл
		...
		Если ТипЗнч(Элемент.ЛевоеЗначение) = Тип("ПолеКомпоновкиДанных") Тогда
			...
			Результат = Результат + ?(ДоступноеПоле = Неопределено ИЛИ ПустаяСтрока(ДоступноеПоле.Заголовок),
				 Строка(Элемент.ЛевоеЗначение), ДоступноеПоле.Заголовок);
		Иначе
			Результат = Результат + Строка(Элемент.ЛевоеЗначение);
		КонецЕсли; 
		Результат = Результат + ПредставлениеВидаСравнения(Элемент.ВидСравнения);
		Если Элемент.ВидСравнения <> ВидСравненияКомпоновкиДанных.Заполнено
			 И Элемент.ВидСравнения <> ВидСравненияКомпоновкиДанных.НеЗаполнено Тогда
			Если ТипЗнч(Элемент.ПравоеЗначение) = Тип("ПолеКомпоновкиДанных") Тогда
				...
				Результат = Результат + ?(ДоступноеПоле = Неопределено
					 ИЛИ ПустаяСтрока(ДоступноеПоле.Заголовок), Строка(Элемент.ЛевоеЗначение), ДоступноеПоле.Заголовок);
			Иначе
...
Процедура ОбновитьОтображениеОтметокФильтров()
	...
	Если Элементы.ПредставлениеПериода.Видимость Тогда
		...
		ТекущаяДлинаСтрокиОтметок = Элементы.ПредставлениеПериода.Ширина 
			+ СтрДлина(Элементы.ПредставлениеПериода.Заголовок) 
			+ 1;
	Иначе
	...
	Для каждого Стр Из НаложенныеФильтры Цикл
		...
		Если Элементы.СтрокиОтметокФильтров.ПодчиненныеЭлементы.Количество() = 0
			...
			ГруппаСтрока = Элементы.Добавить("СтрокаОтметокФильтров_" + Индекс, Тип("ГруппаФормы"),
				 Элементы.СтрокиОтметокФильтров);
		...
		Иначе
			ГруппаСтрока =
				Элементы.СтрокиОтметокФильтров.ПодчиненныеЭлементы[Элементы.СтрокиОтметокФильтров.ПодчиненныеЭлементы.Количество() - 1];
		КонецЕсли;
		...
		ЭлементОтметка = Элементы.Вставить(Стр.ИмяКнопки, Тип("ДекорацияФормы"), ГруппаСтрока,
			 ГруппаСтрока.ПодчиненныеЭлементы[ГруппаСтрока.ПодчиненныеЭлементы.Количество() - 1]);
...
Процедура ВыполнитьНестандартнуюОбработку(ИдентификаторСтроки, ИмяЭлемента, Значение = Неопределено)
	...
	Для каждого Реквизит Из Стр.Реквизиты Цикл
		...
		КонецЕсли; 
	КонецЦикла; 
	ОтчетОбъект.ПриИзмененииНестандартногоРеквизита(Стр.Тип, Стр.Поле, СтруктураЗначений,
		 КомпоновщикНастроекТаблица.Настройки, ЭтотОбъект,
		 ИмяЭлемента);
...
Процедура НаложитьОтборПоДобавленномуПолю(Поле, Значение, УправляющиеСимволы = "")
	...
	Если Поле = "ДобавленноеПоле_Тег" Тогда
		...
		Если ПолеКонтрагент = Неопределено Тогда
			Возврат;
		КонецЕсли;
		НовыйВидСравнения = ?(УправляющиеСимволы = "-", ВидСравненияКомпоновкиДанных.НеВСпискеПоИерархии,
...
Функция ДобавитьОписаниеНастройки(ЭлементыДобавления, Тип, ИмяПоля, Представление = "", ЭлементСхемы = Неопределено)
	...
	Если Тип = ТипФильтр() Тогда
		ДоступноеПоле =
			КомпоновщикНастроекТаблица.Настройки.ДоступныеПоляОтбора.НайтиПоле(Новый ПолеКомпоновкиДанных(ИмяПоля));
	ИначеЕсли Тип = ТипПараметр() Тогда
		ДоступноеПоле =
			КомпоновщикНастроекТаблица.Настройки.ПараметрыДанных.ДоступныеПараметры.НайтиПараметр(Новый ПараметрКомпоновкиДанных(ИмяПоля));
	ИначеЕсли Тип = ТипГруппировка() Тогда
		ДоступноеПоле =
			КомпоновщикНастроекТаблица.Настройки.ДоступныеПоляГруппировок.НайтиПоле(Новый ПолеКомпоновкиДанных(ИмяПоля));
	Иначе
		ДоступноеПоле =
			КомпоновщикНастроекТаблица.Настройки.ДоступныеПоляВыбора.НайтиПоле(Новый ПолеКомпоновкиДанных(ИмяПоля));
	КонецЕсли; 
...
Процедура ПослеВыбораРекламнойКампании(ВыбранныйЭлемент, СписокПараметров) Экспорт
	...
	Если ВыбранныйЭлемент.Значение = "По списку" ИЛИ ВыбранныйЭлемент.Значение = "ВЫБРАТЬ кампанию"  Тогда
		...
		ОткрытьФорму("Обработка.CRM_АРМ_РабочееМестоСквознаяАналитика.Форма.ФормаВыбораРекламныхКампаний",
...
Процедура НастройкиИсточников(Команда)
	...
	ОткрытьФорму("Обработка.CRM_АРМ_РабочееМестоСквознаяАналитика.Форма.ФормаНастройкиСквознойАналитики",
		 , ЭтотОБъект, , , , ,
		 РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
&НаКлиенте
Процедура ОтчетТабличныйДокументТаблицаОбработкаРасшифровки(Элемент, Расшифровка,
...
Функция ПолучитьДанныеРасшифровки(Расшифровка)
	...
	СтруктураВозврата = Новый Структура("Тип",
		 ?(ТипЗнч(ЭлементРасшифровки) = Тип("ЭлементРасшифровкиКомпоновкиДанныхГруппировка"),
		 ТипГруппировка(),
		 ТипРеквизит()));
	...
	Если ЗначениеЗаполнено(СтруктураВозврата.Поле) Тогда
		ДоступноеПоле =
			КомпоновщикНастроекТаблица.Настройки.ДоступныеПоляВыбора.НайтиПоле(Новый ПолеКомпоновкиДанных(СтруктураВозврата.Поле));
		Если ДоступноеПоле = Неопределено Тогда
...
Процедура УстановитьЗаголовкиПолей(СхемаКомпоновкиДанных)
	...
	Для Каждого ТекПоле Из СхемаКомпоновкиДанных.ВычисляемыеПоля Цикл
		Если СтрНайти(ТекПоле.Заголовок, "%ВалютаУчета%") > 0 Тогда
...
Процедура СкрытьНастройкиОтчета(Команда = Неопределено)
	...
	Элементы.СкрытьНастройкиОтчета.Заголовок = ?(НЕ СкрыватьНастройки,
		 НСтр("ru='Скрыть настройки';en='Hide Settings'"),
		 НСтр("ru='Показать настройки'"));
...
Процедура РасширенныеНастройки(Команда)
	...
	ОткрытьФорму("Обработка.CRM_АРМ_РабочееМестоСквознаяАналитика.Форма.ФормаВарианта", ПараметрыФ,
		 ЭтотОбъект, , , , ,
		 РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура СохранитьНастройки(Команда)
	...
	СохранитьНастройкиВХранилище(ИмяФормы);
...
Функция ВосстановитьНастройкиИзХранилища(КлючОбъекта)
	...
	КлючНастроек = "РабочееМестоСквознаяАналитика";
	ЗначениеНастроек = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(КлючОбъекта, КлючНастроек, , ,
...
Процедура ПолучитьСтруктуруКэшируемыеЗначения()
	...
	КэшированныеЗначения.Вставить("ДиаграммаПроцессорВывода",
...
Функция ПолучитьТаблицуДанных(Период, ИмяГруппировки)
	...
	ЕстьРасчетыСПокупателями = (Метаданные.РегистрыНакопления.Найти("CRM_РасчетыСПокупателями") <> Неопределено);
	...
	|ИЗ
	|	РегистрНакопления.CRM_РасчетыСПокупателями
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЗаказКлиента.Объект КАК Документ,
	...
	|	ПО ВременнаяТаблицаДанные.Сделка = ВременнаяТаблицаВыручка.CRM_Интерес
	|
...
Процедура ОтчетПоКампаниям(Команда)
	УстановитьВидимостьЭлементовИКнопок(Истина, Ложь, Ложь, Ложь);
КонецПроцедуры

&НаКлиенте
Процедура ОтчетПоЦелям(Команда)
	УстановитьВидимостьЭлементовИКнопок(Ложь, Истина, Ложь, Ложь);
КонецПроцедуры

&НаКлиенте
Процедура ОтчетПоИсточникам(Команда)
	УстановитьВидимостьЭлементовИКнопок(Ложь, Ложь, Истина, Ложь);
КонецПроцедуры

&НаКлиенте
Процедура ОтчетСвод(Команда)
	УстановитьВидимостьЭлементовИКнопок(Ложь, Ложь, Ложь, Истина);
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ДС_СводПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
