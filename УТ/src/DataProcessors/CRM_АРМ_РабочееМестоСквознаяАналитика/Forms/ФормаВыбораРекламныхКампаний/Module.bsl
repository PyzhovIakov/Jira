...
Процедура Выбрать(Команда = Неопределено)
	...
	СтруктураВыбора = Новый Структура("ВключатьДанныеБезКампаний,ВыбранныеКампании",
		 ВключатьДанныеБезКампаний,
		 ВыбранныеКампании);
...
Процедура ПоказатьНайденныеКампании()
	...
	Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
		Для Каждого ТекСтрока Из ТаблицаРекламныхКампаний Цикл
			...
			Иначе
				ТекСтрока.УдовлетворяетУсловиюПоиска = (СтрНайти(ВРЕГ(ТекСтрока.РекламныеКампании), ВРЕГ(СтрокаПоиска)) > 0);
