...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьСписокПользователей(ИспользоватьГруппы);
	...
	Если ПомеченныйПользователь <> Неопределено И ЗначениеПометки <> СтрокаСпискаПользователей.Пометка Тогда
		...
		ГруппаПользователей.НаименованиеГруппыИПомеченоПользователей = 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 (%2)'"),
				 Строка(ГруппаПользователей.Группа),
				 ГруппаПользователей.ПомеченоПользователей);
	КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьСписокПользователей(ИспользоватьГруппы)
	...
	Если ИспользоватьГруппы Тогда
		Обработки.CRM_НастройкиПользователей.ЗаполнитьДеревоГрупп(ДеревоГрупп);
		СписокВсехПользователейТаблица = Обработки.CRM_НастройкиПользователей.ПользователиДляКопирования(
			ПользовательИсточник, СписокВсехПользователейТаблица);
	Иначе
		СписокПользователейТаблица = Обработки.CRM_НастройкиПользователей.ПользователиДляКопирования(
			ПользовательИсточник, СписокПользователейТаблица);
	КонецЕсли;
