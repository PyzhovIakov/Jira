...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если CRM_РаботаСРечьюБМОСервер.ПоддерживаетсяРаспознавание() Тогда 
...
Процедура ЗапретитьОблачноеРаспознаваниеПриИзмененииНаСервере()
	...
	Константы.CRM_НастройкиРаботыСРечью.СохранитьНастройку(
...
Процедура ПереключитьГруппуПодключитьОтключить()
	...
	ВнешнееРасположениеПоддерживается = CRM_РаботаСРечьюБМОСервер.ПоддерживаетсяИспользованиеВнешнегоРасположения();
...
Функция ВыполнитьАутентификациюВСервисеРаботыСРечью()
	...
	ВладелецТикета = CRM_РаботаСРечьюБМОКлиентСервер.ИдентификаторСервиса();
	...
	Если Не ПустаяСтрока(РезультатПолученияТикета.Тикет) Тогда
		...
		CRM_РаботаСРечьюБМОСервер.УстановитьПараметрыВнешнегоПодключенияИнформационнойБазы(
			CRM_РаботаСРечьюБМОСервер.НовыйПараметрыВнешнегоПодключенияРаботыСРечью(Токен)
		);
...
Функция ТекущиеПараметрыРаботыСРечью()
	...
	ТекущиеПараметры = CRM_РаботаСРечьюБМОСервер.ПолучитьПараметрыВнешнегоПодключенияИнформационнойБазы();
...
Функция ПолучитьТокенСервисаРаботыСРечью(Адрес, Тикет)
	...
	Соединение = Новый HTTPСоединение(Адрес, , , , Прокси, 20, Новый ЗащищенноеСоединениеOpenSSL());
...
Процедура ОтключитьСервисРаспознавания()
	...
	CRM_РаботаСРечьюБМОСервер.УстановитьПараметрыВнешнегоПодключенияИнформационнойБазы(Неопределено);
	CRM_РаботаСРечьюБМОСервер.УстановитьПараметрыВнешнегоПодключенияСеанса(Неопределено);
...
Функция ПолучитьБалансСервисаРаботыСРечью()
	...
	ПараметрыРаспознавания = CRM_РаботаСРечьюБМОСервер.ПолучитьПараметрыВнешнегоПодключенияИнформационнойБазы();
...
&НаСервереБезКонтекста
Функция ПредставлениеБаланса(ДанныеБаланса)
...
Процедура ЗаполнитьДоступныеМодели()
	...
	Попытка
		ОписаниеМоделейРаспознавания = CRM_РаботаСРечьюБМОСервер.ПолучитьОписаниеМоделейРаспознавания();
	Исключение
	...
	Элементы.РекомендацияВключитьСкачать.Видимость = Не ЕстьДоступнаяЛокальная
		И (ЗапретитьОблачноеРаспознавание Или Не CRM_РаботаСРечьюБМОСервер.ПоддерживаетсяИспользованиеВнешнегоРасположения());
...
Функция КартинкаЭтоЛокальная(ОписаниеМодели)
	...
	Если ОписаниеМодели.ДоступнаВЛокальномВарианте Тогда
		Возврат БиблиотекаКартинок.CRM_МодельРаспознаванияРечиДоступна;
	ИначеЕсли ОписаниеМодели.ДоступнаДляПолучения Тогда
		Возврат БиблиотекаКартинок.CRM_МодельРаспознаванияРечиСкачать;
	Иначе
		Возврат БиблиотекаКартинок.CRM_МодельРаспознаванияРечиНеДоступна;
	КонецЕсли;
...
Процедура ЗагрузитьИУстановитьМодельНаСервере(ИдентификаторМодели)
	...
	АдресВременногоХранилища = CRM_РаботаСРечьюБМОСервер.ПолучитьМодельРаспознаванияВоВременноеХранилище(
		CRM_РаботаСРечьюБМОСервер.НовыйИдентификаторМоделиРаспознаванияРечи(ИдентификаторМодели)
	);
	CRM_РаботаСРечьюБМОСервер.УстановитьМодельРаспознаванияДляИнформационнойБазы(АдресВременногоХранилища);
...
Процедура УдалитьЛокальнуюМодельНаСервере(ИдентификаторМодели)
	...
	CRM_РаботаСРечьюБМОСервер.УдалитьМодельРаспознаванияДляИнформационнойБазы(
		CRM_РаботаСРечьюБМОСервер.НовыйИдентификаторМоделиРаспознаванияРечи(ИдентификаторМодели)
	);
...
Процедура УстановитьУсловноеОформление()
	...
	Если ЗапретитьОблачноеРаспознавание Или Не CRM_РаботаСРечьюБМОСервер.ПоддерживаетсяИспользованиеВнешнегоРасположения() Тогда
...
Процедура ОбщаяГрамматика(Команда)
	...
	ОткрытьФорму("Обработка.CRM_ПараметрыРаспознаванияРечи.Форма.РедактированиеГрамматики", ПараметрыОткрытия);
...
Процедура ТестовыйПакетТекстОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "#ПодключитьБесплатныйСтартовыйПакет" Тогда
		...
		ИдентификаторСервиса = CRM_РаботаСРечьюБМОКлиентСервер.ИдентификаторСервиса();
