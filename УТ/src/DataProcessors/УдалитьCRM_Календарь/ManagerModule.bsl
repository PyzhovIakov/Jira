...
Функция ЗаполнитьДополнительныеСвойстваСобытия(ОписаниеСобытия, НастройкиОтображения) Экспорт
	...
	Если бЕстьОсновнаяКатегорияСобытия И ЗначениеЗаполнено(ОписаниеСобытия.ОсновнаяКатегорияСобытия) Тогда
		ОписаниеСобытия.ЦветФона = Новый Цвет(ОписаниеСобытия.ОсновнаяКатегорияСобытия.ЦветКрасный,
			 ОписаниеСобытия.ОсновнаяКатегорияСобытия.ЦветЗеленый,
			 ОписаниеСобытия.ОсновнаяКатегорияСобытия.ЦветСиний);
	Иначе
		Если ТипЗнч(ОписаниеСобытия.Событие) = Тип("ДокументСсылка.CRM_ЭтапКалендарногоПлана") Тогда
...
Процедура СформироватьДеревоЗадач(Таблица, Дерево, ПолеГруппировки = Неопределено, ОчищатьСтроки = Истина) Экспорт
	...
	Иначе
		...
		Для Каждого ЗначениеГруппировки Из МассивЗначенияГруппировок Цикл
			...
			Попытка
				СтрокаГруппа.ЗначениеГруппировки = ЗначениеГруппировки;
			
			Исключение КонецПопытки;
			Попытка
				СтрокаГруппа.ИмяГруппировки = ПолеГруппировки;
