...
//
Процедура ЗаполнитьТаблицуЗапросов(ТаблицаЗапросов, ДопДокументы, ДопПоляШапки,
	 УчитыватьТЧВДопДокументах = Ложь, ТолькоАктуальные = Ложь,
	 ОтборВсписке = Ложь) Экспорт
	...
	Для Каждого СтрокаКорня Из ДеревоДокументов.Строки Цикл
		...
		Для Каждого СтрокаРеквизитТЧ Из СтрокаКорня.Строки Цикл
			...
			Если Не ПустаяСтрока(СтрокаРеквизитТЧ.ТабличнаяЧасть) Тогда
				...
				ТекстУсловия = СтрЗаменить(ШаблонУсловияТЧ, "%ИмяТаблицыТЧ%", СтрокаКорня.Документ.Имя + "." 
					+ СтрокаРеквизитТЧ.ТабличнаяЧасть);
		...
		Для Каждого КлючИЗначение Из СоответствиеПолей Цикл
			Для Каждого ЭлементСтруктура Из КлючИЗначение.Значение Цикл
				...
				Если РеквизитСоставной(КлючИЗначение.Ключ) Тогда
					ПутьКТаблицеРеквизита = "ВЫРАЗИТЬ(ТаблицаШапки." + КлючИЗначение.Ключ.Имя + " КАК Документ." 
						+ ЭлементСтруктура.Документ.Имя 
						+ ")";
				Иначе
				...
				Если СоответствиеПолей.Количество() > 0 Тогда
					...
					Для Каждого КлючИЗначение Из СоответствиеПолей Цикл
						Для Каждого ТекСтруктура Из КлючИЗначение.Значение Цикл
							...
							Если РеквизитСоставной(КлючИЗначение.Ключ) Тогда
								ПутьКТаблице = "ВЫРАЗИТЬ(ТабличнаяЧасть." + КлючИЗначение.Ключ.Имя + " КАК Документ." 
									+ ТекСтруктура.Документ.Имя 
									+ ")";
							Иначе
