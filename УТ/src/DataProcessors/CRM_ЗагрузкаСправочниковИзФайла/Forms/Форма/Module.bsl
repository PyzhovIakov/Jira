...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтаФорма.Элементы.ПапкаРодитель.Видимость = (Не ЭтаФорма.Параметры.ВидСправочника = "Массив"
												 И Не ЭтаФорма.ВидСправочника = "Партнеры"
	...
	Если Параметры.Свойство("СписокИсключаемыхПолей") Тогда
		СписокИсключаемыхПолей = Параметры.СписокИсключаемыхПолей;
	КонецЕсли;
...
Процедура ПриОткрытииПродолжение(Подключено, Отказ) Экспорт
	...
	Если НЕ РасширениеПодключено И НЕ Отказ Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения,
			НСтр("ru = 'Не установлено расширение для работы с файлами в Веб-клиенте! Выполнить установку?'"),
...
Процедура ПередЗагрузкойДанныхИзНастроекНаСервере(Настройки)
	...
	Если НЕ Настройки["ПапкаРодитель"] = Неопределено
		И НЕ ТипЗнч(Настройки["ПапкаРодитель"]) = Тип("СправочникСсылка."+ВидСправочника) Тогда
...
Процедура НаименованиеПрограммыПриИзменении(Элемент)
	...
	Если НЕ Объект.ИмяФайлаОбмена = "" Тогда
		...
		Если КодПрограммы = 5 Тогда
			Если (Не Прав(Объект.ИмяФайлаОбмена, 4) = ".txt") И (Не Прав(Объект.ИмяФайлаОбмена, 4) = ".csv")
				И (Не Прав(Объект.ИмяФайлаОбмена, 4) = ".xls") Тогда
		...
		ИначеЕсли КодПрограммы = 7 Тогда	
			Если (Не Прав(Объект.ИмяФайлаОбмена, 4) = ".txt") И (Не Прав(Объект.ИмяФайлаОбмена, 4) = ".csv")
				И (Не Прав(Объект.ИмяФайлаОбмена, 4) = ".tdf") Тогда
				Объект.ИмяФайлаОбмена = "";
			КонецЕсли;
		// Файл MS Excel
...
Процедура ИмяФайлаОбменаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Объект.ПоляФайла.Количество() > 0 Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения,
			НСтр("ru = 'Все сделанные ранее сопоставления будут утеряны. Продолжить?'"),
			РежимДиалогаВопрос.ДаНет);
	Иначе
		ВыборФайла(Элемент);
	КонецЕсли;
...
&НаКлиенте
Процедура ВидСправочникаАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание,
...
Процедура CRM_ОбработкаВыбораТиповОтношений(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.Клиент И Не НовыеЗначенияТипыОтношений.Клиент Тогда
		...
		Если НовыеЗначенияТипыОтношений.ОбслуживаетсяТорговымиПредставителями Тогда
			...
			ЭлементОбслуживаетсяТорговымиПредставителями =
				CRM_ВсеТипыОтношений.НайтиПоЗначению("ОбслуживаетсяТорговымиПредставителями");
	...
	ИначеЕсли Не ДополнительныеПараметры.ОбслуживаетсяТорговымиПредставителями
		И НовыеЗначенияТипыОтношений.ОбслуживаетсяТорговымиПредставителями Тогда
...
Процедура ПоляСправочникаНаименованиеПоляОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ВыбранноеЗначение = НСтр("ru = 'Несколько полей...'") Тогда
		...
		СтандартнаяОбработка = Ложь;
		ОписаниеОповещения = Новый ОписаниеОповещения("ПоляСправочникаНаименованиеПоляОбработкаВыбораЗавершение",
			ЭтотОбъект,
			ТекущиеДанные);
		ПараметрыФормы = Новый Структура("ЗначенияЗаполнения, НаименованиеРеквизита, СписокПолейФайла",
			Новый Структура("Объект", Объект),
			ТекущиеДанные.НаименованиеРеквизита,
			СписокПолейФайла);
		ОткрытьФорму("Обработка.CRM_ЗагрузкаСправочниковИзФайла.Форма.ФормаПодбора", ПараметрыФормы, Элемент, , , ,
			ОписаниеОповещения,
...
Процедура Загрузить(Команда)
	...
	// +CRM_Модуль в типовой УТ 11/ERP реквизит ВидНоменклатуры является обязательным для заполнения
	...
	Если НЕ ВидСправочника = "Массив" И ТипДействия = "Стандартный" Тогда
		...
		Если ВидСправочника = "Партнеры" Тогда
			...
			Если CRM_ОбщегоНазначенияПовтИсп.ЭтоCRM() Тогда
				ЗаписьВыполнена =
					CRM_НачалоРаботыСПрограммойВызовСервера.ЗаписатьНастроенныйРазделРешения(ПредопределенноеЗначение("Перечисление.CRM_РазделыНастройкиРешения.ПортретКлиента"));
...
Процедура CRM_ЗаполнитьТипыОтношений()
	...
	Для Каждого ТипОтношений Из CRM_ВсеТипыОтношений Цикл
		ТипОтношенийСтр = Строка(ТипОтношений.Значение);
		Если ТипОтношений.Пометка Тогда
			CRM_ТипыОтношений = CRM_ТипыОтношений  + "; " + ТипОтношенийСтр;
		КонецЕсли;
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭтотОбъект, ТипОтношенийСтр) Тогда
			ЭтотОбъект[ТипОтношенийСтр] = ТипОтношений.Пометка;
		КонецЕсли;
...
Процедура ЗаполнитьРеквизитыСправочника()
	...
	Если ЗначениеЗаполнено(ЗагрузкаВДокумент) Тогда
		...
		Если СписокИсключаемыхПолей.НайтиПоЗначению(Нстр("ru='Телефон';en='Phone'")) = Неопределено Тогда
			НоваяСтрока = Объект.ПоляСправочника.Добавить();
		...
		Если СписокИсключаемыхПолей.НайтиПоЗначению(Нстр("ru='Email';en='Email'")) = Неопределено Тогда
			НоваяСтрока = Объект.ПоляСправочника.Добавить();
...
Процедура ЗаполнитьПоляСправочникаДополнительнымиРеквизитами()
	...
	Для Каждого ДопРеквизит Из Объект.ДополнительныеРеквизиты Цикл
		НайденыеСтроки = Объект.ПоляСправочника.НайтиСтроки(Новый Структура("НаименованиеРеквизита",
			ДопРеквизит.Наименование + Разделитель));
...
Процедура ОчиститьПоляСправочникаОтДополнительныхРеквизитов()
	...
	Пока Ном > 0 Цикл
		...
		НайденыеСтроки = Объект.ДополнительныеРеквизиты.НайтиСтроки(Новый Структура("Наименование",
			 Объект.ПоляСправочника[Ном].НаименованиеРеквизита));
		Если НайденыеСтроки.Количество() = 0 Тогда
...
Процедура ЗаполнитьДополнительныеРеквизиты()
	...
	Для Каждого СтрокаТЗ Из ТаблицаДополнительныхРеквизитов Цикл
		...
		Если    СтрокаТЗ.ТипЗначения.СодержитТип(Тип("Булево"))
			...
			Или СтрокаТЗ.ТипЗначения.СодержитТип(Тип("СправочникСсылка.ЗначенияСвойствОбъектов")) Тогда
...
Процедура ВыборФайла(Элемент, ПроверятьСуществование = Ложь, Знач РасширениеПоУмолчанию = "xml",
	...
	Иначе
		ПоказатьПредупреждение(,
			НСтр("ru = 'Для выбора файла необходимо установить расширение для работы с файлами в Веб-клиенте.'"));
	КонецЕсли;
...
&НаКлиенте
Процедура ПрочитатьПоляИзВнешнегоФайлаЗавершение(Результат, АдресExcel, ВыбранноеИмяФайла,
	...
	Если Результат Тогда
		...
		Расширение =
			ОбщегоНазначенияКлиентСервер.РасширениеБезТочки(ОбщегоНазначенияКлиентСервер.ПолучитьРасширениеИмениФайла(ВыбранноеИмяФайла));
...
Функция ЗагрузитьФайлСДаннымиВТабличныйДокументНаСервере(АдресВременногоХранилища, РасширениеВыбранногоФайла)
	...
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания =
		НСтр("ru='Загрузка справочников из файла: Выполнение серверного метода загрузка данных из файла';
		|en='Loading catalogs from file: Executing a server method of loading data from a file'");
...
Процедура ПослеЗагрузкиФайлаСДаннымиВТабличныйДокумент(ФоновоеЗадание, ДополнительныеПараметры) Экспорт
	...
	Если ФоновоеЗадание.Статус = "Выполнено" Тогда
		...
		СписокПолейФайла.Вставить(0,
			НСтр("ru='Несколько полей...';en='Multiple fields ...'"),
			НСтр("ru='Несколько полей...';en='Multiple fields ...'"));
		СопоставитьПоляФайлаПолямСправочника();
		
	Иначе
		ВывестиСообщениеОбОшибке(НСтр("ru='Не удалось произвести загрузку данных.';en='Failed to load data.'"),
...
//	СтруктураДанныхФормы.Вставить("НаименованиеПрограммы"	, НаименованиеПрограммы);
	...
	Если СтруктураДанныхФормы.ТаблицаЦен.Количество() > 0 Тогда
		ЗначенияЗаполнения = Новый Структура("ТаблицаНоменклатуры, Дата", АдресХранилищаТаблицаЦен, ТекущаяДата());
		ПараметрыФормы = Новый Структура("ЗначенияЗаполнения", ЗначенияЗаполнения);
		ОткрытьФорму("Документ.УстановкаЦенНоменклатуры.ФормаОбъекта",ПараметрыФормы,ЭтаФорма);
		АдресХранилищаТаблицаЦен = Неопределено;
	КонецЕсли;
	...
	Для Каждого ЗначениеМассива Из МассивФайла Цикл
		...
		Для Каждого ЗначениеМассиваСоответствий Из МассивСоответствий Цикл
			...
			Для Каждого ЗначениеСписка Из ЗначениеМассиваСоответствий.СписокИндексовПолей Цикл
				ЗначениеРеквизита = ЗначениеРеквизита + ?(ЗначениеМассива["_"+Строка(ЗначениеСписка)] 
					= "", "", ?(ЗначениеРеквизита = "", "", " ")+ЗначениеМассива["_"+Строка(ЗначениеСписка)]);
			КонецЦикла;
			
			СтруктураЗначений[ЗначениеМассиваСоответствий.ИмяРеквизита] = ЗначениеРеквизита;
			
			Если Не ЗначениеРеквизита = "" Тогда
				ОТК = Истина;
			КонецЕсли;
...
Функция ПроверкаСопоставленияРеквизитовПолям(ТекстПредупреждения)
	...
	Если Объект.ПоляФайла.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ru='Не сопоставлены реквизиты объекта с полями файла данных.'");
		Возврат Ложь;
	КонецЕсли;
	...
	Если Не ВидСправочника = "Массив" Тогда
		...
		Если ВидСправочника = "Партнеры" Тогда
			Если ЗначениеЗаполнено(ЗагрузкаВДокумент) Тогда
				Массив = 
					Объект.ПоляСправочника.НайтиСтроки(Новый Структура("НаименованиеРеквизита, Обязательный",
					"Наименование", Истина));
			...
			Иначе
				Массив =
					Объект.ПоляСправочника.НайтиСтроки(Новый Структура("НаименованиеРеквизита, Обязательный",
					"Наименование_Партнер", Истина));
				Если Массив.Количество() = 0 Тогда
					ТекстПредупреждения = ТекстПредупреждения +
						НСтр("ru = 'Отсутствует ключевой реквизит ""Партнер (Наименование).""'");
				ИначеЕсли Не ЗначениеЗаполнено(Массив[0].НаименованиеПоля) Тогда
					ТекстПредупреждения = ТекстПредупреждения +
						НСтр("ru = 'Реквизит ""Партнер (Наименование)"" обязателен для сопоставления'");
				КонецЕсли;
				
				Если СоздаватьКонтрагента Тогда
					Массив =
						Объект.ПоляСправочника.НайтиСтроки(Новый Структура("НаименованиеРеквизита, Обязательный",
						"Наименование_Контрагент", Истина));
					Если Массив.Количество() = 0 Тогда
						ТекстПредупреждения = ТекстПредупреждения +
							НСтр("ru = 'Отсутствует ключевой реквизит ""Контрагент (Наименование).""'");
					ИначеЕсли Не ЗначениеЗаполнено(Массив[0].НаименованиеПоля) Тогда
						ТекстПредупреждения = ТекстПредупреждения +
							НСтр("ru = 'Реквизит ""Контрагент (Наименование)"" обязателен для сопоставления'");
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		Иначе
...
Функция СформироватьМассивСоответствий()
	...
	Для Каждого СтрокаТЧ Из Объект.ПоляСправочника Цикл
		...
		Если СписокИП.Количество() > 0 Тогда
			...
			СтруктураПолейРеквизитов = Новый Структура("Обязательный, ИмяРеквизита, ПризнакКИ, СсылкаКИ,
				| СписокИндексовПолей, РеквизитСправочник, РеквизитДата, ДополнительныйРеквизит, РеквизитТип,
				| СоздаватьЭлемент");
...
Функция ЗагрузитьИзExcel()
	...
	Для а = 0 По (СписокПолейФайла.Количество() - 1) Цикл
		СписокКлючейСтрутуры = СписокКлючейСтрутуры + ?(СписокКлючейСтрутуры = "", "", ",") + "_"
			+ СокрЛП(СтрЗаменить(а, Символы.НПП, ""));
	КонецЦикла;
	...
	Для Row = 2 По RowCount Цикл
		...
		Если ЕстьЗаполненныеСтроки И ОбрабатыватьДоПервойПустойСтроки И Не бЗначенияЗаполнены Тогда
			Прервать;
		ИначеЕсли Не бЗначенияЗаполнены Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ЗагрузитьИзТекста(Разделитель, ОписаниеОповещения)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ЗагрузитьИзТекстаЗавершение",
		ЭтотОбъект, Новый Структура("ТекстФайла, Разделитель, ОписаниеОповещения",
		ТекстФайла, Разделитель, ОписаниеОповещения));
...
Процедура НайтиСоздатьЭлементыСправочника(МассивСоответствий, МассивФайла, СтруктураДанныхФормы)
	...
	Для Каждого ЗначениеМассива Из МассивФайла Цикл
		...
		Если СтруктураДанныхФормы.ВидСправочника = "Номенклатура" Тогда
			...
			Для Каждого ЗначениеСписка Из СтрокаНаименования.СписокИндексовПолей Цикл
				ЗначениеНаименования = ЗначениеНаименования + ?(ЗначениеМассива["_"
					+ Строка(ЗначениеСписка)] =
						"", "", ?(ЗначениеНаименования = "", "", " ") + ЗначениеМассива["_" + Строка(ЗначениеСписка)]);
			КонецЦикла;
		Иначе
			...
			Для Каждого ЗначениеМассиваСоответствий Из МассивСоответствий Цикл
				Если ЗначениеМассиваСоответствий.Обязательный Тогда
					...
					ИначеЕсли (Прав(СокрЛП(ЗначениеМассиваСоответствий.ИмяРеквизита),11) = "_Контрагент") Тогда
						...
						СтрокаНаименования = ЗначениеМассиваСоответствий;
						Для Каждого ЗначениеСписка Из СтрокаНаименования.СписокИндексовПолей Цикл
							ЗначениеНаименованияКонтрагента = ЗначениеНаименованияКонтрагента + ?(ЗначениеМассива["_"
								+ Строка(ЗначениеСписка)] =
									"", "", ?(ЗначениеНаименованияКонтрагента = "", "", " ") + ЗначениеМассива["_" + Строка(ЗначениеСписка)]);
						КонецЦикла;
					КонецЕсли;
				КонецЕсли;
			...
			Если НЕ  Наименование_КЛ = Неопределено Тогда
				Для Каждого ЗначениеСписка ИЗ Наименование_КЛ.СписокИндексовПолей Цикл
					ЗначениеНаименованияКЛ = ЗначениеНаименованияКЛ + ?(ЗначениеМассива["_"
						+ Строка(ЗначениеСписка)] =
							"", "", ?(ЗначениеНаименованияКЛ = "", "", " ")+ЗначениеМассива["_" + Строка(ЗначениеСписка)]);
				КонецЦикла;
			Иначе
		...
		Если СтруктураДанныхФормы.ВидСправочника = "Номенклатура" Тогда
			...
			ЭлементСправочника = ПолучитьСсылкуСправочника(СтруктураДанныхФормы.ВидСправочника, ЗначениеДляПоиска,
				СтруктураДанныхФормы.ПапкаРодитель, , ,
				НовыхЭлементов, НайденоЭлементов);
			
			ЗаполнитьРеквизитыИзМассиваСоответствий(СтруктураДанныхФормы, ЗначениеМассива,
				ЭлементСправочника, МассивСоответствий, "Номенклатура");
			...
			// +CRM_Модуль так как в типовой УТ 11/ERP реквизит ВидНоменклатуры обязательный, и при его выборе 
			...
			Иначе
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Нстр("ru = 'Не удалось загрузить номенклатуру'")
					+ " - " + ЭлементСправочника.Наименование);
			КонецЕсли;
			
		Иначе
			Если СокрЛП(ЗначениеНаименования) = ""
				И СокрЛП(ЗначениеНаименованияКонтрагента) = ""
				И СокрЛП(ЗначениеНаименованияКЛ) = "" Тогда
			...
			ЗаполнитьРеквизитыИзМассиваСоответствий(СтруктураДанныхФормы, ЗначениеМассива, Партнер,
				МассивСоответствий, "Партнеры");
			...
			Если Не ЗначениеДляПоискаКонт = "" Тогда
				Контрагент		= ПолучитьСсылкуСправочника("Контрагенты", ЗначениеДляПоискаКонт, , ,
					Партнер.Ссылка, НовыхЭлементов, НайденоЭлементов);
				ЗаполнитьРеквизитыИзМассиваСоответствий(СтруктураДанныхФормы, ЗначениеМассива, Контрагент,
					МассивСоответствий, "Контрагенты");
				Контрагент.Записать();
			КонецЕсли;
			
			Если Не ЗначениеДляПоискаКЛ = "" Тогда
				КонтактноеЛицо	= ПолучитьСсылкуСправочника("КонтактныеЛицаПартнеров", ЗначениеДляПоискаКЛ, ,
					Партнер.Ссылка, , НовыхЭлементов, НайденоЭлементов);
				ЗаполнитьРеквизитыИзМассиваСоответствий(СтруктураДанныхФормы, ЗначениеМассива, КонтактноеЛицо,
					МассивСоответствий, "КонтактныеЛицаПартнеров");
		...
		Если СтруктураДанныхФормы.ВидСправочника = "Номенклатура" Тогда
			Для Каждого СтрокаРеквизитов Из МассивСоответствий Цикл
				Если Лев(СтрокаРеквизитов.ИмяРеквизита,5) = "Цена_" Тогда
					...
					МассивСтрокТЧ = СтруктураДанныхФормы.ПоляСправочника.НайтиСтроки(Новый Структура("НаименованиеРеквизита",
						СтрокаРеквизитов.ИмяРеквизита));
					ВидЦены = МассивСтрокТЧ[0].СправочникСсылка;
					
					Значение = "";
					Для Каждого ЗначениеСписка Из СтрокаРеквизитов.СписокИндексовПолей Цикл
						Значение = Значение + ?(ЗначениеМассива["_"
							+ Строка(ЗначениеСписка)]=
								"", "", " " + ЗначениеМассива["_" + Строка(ЗначениеСписка)]);
		...
		Если СтруктураДанныхФормы.ВидСправочника = "Партнеры" Тогда
			Если Не СокрЛП(ЗначениеНаименования)= "" И Не СокрЛП(ЗначениеНаименованияКЛ)= "" Тогда
				СсылкаКЛ = Справочники.КонтактныеЛицаПартнеров.НайтиПоНаименованию(СокрЛП(ЗначениеНаименованияКЛ),
					Истина, , Партнер.Ссылка);
					...
					Иначе
						...
						Если СтрокаТЧ.Тип = Перечисления.ТипыКонтактнойИнформации.Телефон Тогда
							...
							Если СтрокаТЧ.Вид = Справочники.ВидыКонтактнойИнформации.ТелефонПартнера 
								Или СтрокаТЧ.Вид = Справочники.ВидыКонтактнойИнформации.ТелефонКонтактногоЛица
								Или СтрокаТЧ.Вид = Справочники.ВидыКонтактнойИнформации.ТелефонОрганизации Тогда
								
								ИмяРеквизита = "CRM_Телефон";
							ИначеЕсли СтрокаТЧ.Вид = Справочники.ВидыКонтактнойИнформации.МобильныйТелефонКонтактногоЛица
								Или СтрокаТЧ.Вид = Справочники.ВидыКонтактнойИнформации.CRM_МобильныйТелефонПользователя
								Или СтрокаТЧ.Вид = Справочники.ВидыКонтактнойИнформации.ТелефонМобильныйФизическиеЛица
								// +CRM_Модуль
								// Или СтрокаТЧ.Вид = Справочники.ВидыКонтактнойИнформации.МобильныйТелефонЧастногоЛицаКонтрагента
								Или (ЗначениеЗаполнено(ВидКИМобильныйТелефон) И СтрокаТЧ.Вид = ВидКИМобильныйТелефон)
								// -CRM_Модуль
								Или СтрокаТЧ.Вид = Справочники.ВидыКонтактнойИнформации.CRM_МобильныйТелефонЧастногоЛица Тогда
						...
						Если СтрокаТЧ.Тип = Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты Тогда
							Если СтрокаТЧ.Вид = Справочники.ВидыКонтактнойИнформации.EmailКонтактногоЛица
								Или СтрокаТЧ.Вид = Справочники.ВидыКонтактнойИнформации.EmailПартнера
								Или СтрокаТЧ.Вид = Справочники.ВидыКонтактнойИнформации.EmailОрганизации
								Или СтрокаТЧ.Вид = Справочники.ВидыКонтактнойИнформации.EmailПользователя
								Или СтрокаТЧ.Вид = Справочники.ВидыКонтактнойИнформации.EMailФизическиеЛица Тогда
	...
	Если СтруктураДанныхФормы.ТаблицаЦен.Количество() > 0 Тогда
		АдресХранилищаТаблицаЦен = ПоместитьВоВременноеХранилище(СтруктураДанныхФормы.ТаблицаЦен.Выгрузить());
	КонецЕсли;
...
Функция ПропарситьЗначениеДаты(Значение, КодПрограммы)
	...
	Если КодПрограммы = 4 Тогда
		Если (СтрДлина(Значение) = 10) И (СтрЧислоВхождений(Значение, ".") = 2) И (СтрНайти(Значение,".") = 3) Тогда
			Попытка
				Год		= Прав(Значение,4);
			...
			КонецПопытки;
		ИначеЕсли (СтрДлина(Значение) = 8)  И (СтрЧислоВхождений(Значение, ".") = 2) И (СтрНайти(Значение,".") = 3) Тогда
			Попытка
				Год		= Прав(Значение, 2);
				Месяц	= Сред(Значение, 4, 2);
				День	= Лев(Значение, 2);
	...
	Если КодПрограммы = 5 Или КодПрограммы = 6 Тогда
		Если (СтрДлина(Значение) = 10 И СтрЧислоВхождений(Значение, ".") = 2 И СтрНайти(Значение,".") = 3)
			Или (СтрДлина(Значение) = 9 И СтрЧислоВхождений(Значение, ".") = 2 И СтрНайти(Значение,".") = 2) Тогда
			
			Попытка
				Год		= Прав(Значение, 4);
				Месяц	= Сред(Значение, ?(СтрНайти(Значение, ".") = 2, 3, 4), 2);
				День	= Лев(Значение, ?(СтрНайти(Значение, ".") = 2, 1, 2));
				Возврат Дата(Год + Месяц + ?(СтрНайти(Значение, ".") = 2, "0" + День, День));
			Исключение
				Возврат Дата("00010101");
			КонецПопытки;
		КонецЕсли;
...
Функция НайтиПередатьВДокументВыбранныеЭлементы(МассивСоответствий, МассивФайла, СтруктураДанныхФормы)
	...
	Для Каждого ЗначениеМассива Из МассивФайла Цикл
		...
		НайденныйКлиент = НайтиПартнераИлиКЛ("Партнеры", ЗначениеДляПоиска, МассивСоответствий, ЗначениеМассива,
			СтруктураДанныхФормы, Неопределено, ЗагрузкаВДокумент);
		...
		Иначе
			НайденныйКонтакт = НайтиПартнераИлиКЛ("КонтактныеЛицаПартнеров", ЗначениеДляПоиска, МассивСоответствий, ЗначениеМассива,
				СтруктураДанныхФормы, Неопределено, ЗагрузкаВДокумент);
...
Процедура ДобавитьУдалитьРеквизитыЮрФизЛица()
	...
	Если ЭтаФорма.ЮрФизЛицо = Перечисления.КомпанияЧастноеЛицо.Компания Тогда
		...
		НайденыеСтроки = Объект.ПоляСправочника.НайтиСтроки(Новый Структура("НаименованиеРеквизита",
			"CRM_Отчество_Партнер"));
		Если НЕ НайденыеСтроки.Количество() = 0 Тогда
			Объект.ПоляСправочника.Удалить(Объект.ПоляСправочника.Индекс(НайденыеСтроки[0]));
		КонецЕсли;
		
		НайденыеСтроки = Объект.ПоляСправочника.НайтиСтроки(Новый Структура("НаименованиеРеквизита",
			"ДатаРождения_Партнер"));
		Если НЕ НайденыеСтроки.Количество() = 0 Тогда
			Объект.ПоляСправочника.Удалить(Объект.ПоляСправочника.Индекс(НайденыеСтроки[0]));
		КонецЕсли;
...
&НаСервереБезКонтекста
Функция НайтиПартнераИлиКЛ(СправочникСтр, ЗначениеНаименованияПартнераИлиКЛ, МассивСоответствий,
	 ЗначениеМассива, СтруктураДанныхФормы, ПартнерВладелец = Неопределено,
	...
	ИначеЕсли СправочникСтр = "КонтактныеЛицаПартнеров" Тогда
		Ссылка = Справочники.КонтактныеЛицаПартнеров.НайтиПоНаименованию(СокрЛП(ЗначениеНаименованияПартнераИлиКЛ),
			 Истина, ,
			 ПартнерВладелец);
	Иначе
	...
	Если Не ЗначениеЗаполнено(Ссылка) И СтруктураДанныхФормы.ИскатьКлиентовПоАдресамEMail Тогда
		МассивСсылок = НайтиПоТипуКонтактнойИнформации(СправочникСтр, МассивСоответствий, ЗначениеМассива,
			 Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты, ПартнерВладелец,
			 ТипДокумента);
	...
	Если Не ЗначениеЗаполнено(Ссылка) И СтруктураДанныхФормы.ИскатьКлиентовПоТелефонам Тогда
		МассивСсылок = НайтиПоТипуКонтактнойИнформации(СправочникСтр, МассивСоответствий, ЗначениеМассива,
			 Перечисления.ТипыКонтактнойИнформации.Телефон, ПартнерВладелец,
			 ТипДокумента);
		Если МассивСсылок.Количество() > 0 Тогда
			Ссылка = МассивСсылок[0];
		КонецЕсли;
...
&НаСервереБезКонтекста
Функция НайтиПоТипуКонтактнойИнформации(СправочникСтр, МассивСоответствий, ЗначениеМассива, ТипКИ,
		ПартнерВладелец = Неопределено,
		ТипДокумента = Неопределено)
	...
	Если ЗначениеЗаполнено(ЗначениеКИ) Тогда
		...
		|	" + ?(ЗначениеЗаполнено(ПартнерВладелец) И СправочникСтр = "КонтактныеЛицаПартнеров",
			"И КонтактнаяИнформация.Ссылка.Владелец = &ПартнерВладелец", "") + "
		|	И
		|		(
		|		
		|		КонтактнаяИнформация.Представление = &ЗначениеКИ
		|		" + ?(ТипКИ = Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты,
			"ИЛИ КонтактнаяИнформация.АдресЭП = &ЗначениеКИ", "") + "
		|		" + ?(ТипКИ = Перечисления.ТипыКонтактнойИнформации.Телефон,
			"ИЛИ КонтактнаяИнформация.НомерТелефона = &ЗначениеКИ", "") + "
		|		" + ?(ТипКИ = Перечисления.ТипыКонтактнойИнформации.Телефон,
			"ИЛИ КонтактнаяИнформация.НомерТелефонаБезКодов = &ЗначениеКИ", "") + "
		|		
		|		)
		|	
		|УПОРЯДОЧИТЬ ПО
...
&НаСервереБезКонтекста
Функция ПолучитьСсылкуСправочника(ИмяСправчника,ЗначениеДляПоиска, Родитель = Неопределено, Владелец = Неопределено,
	Партнер = Неопределено, НовыхЭлементов,НайденоЭлементов)
...
Функция ПолучитьНаборыСвойствДляПартнеров()
	...
	// +CRM_Модуль добавлен новый параметр
	Возврат УправлениеСвойствамиПереопределяемый.ПолучитьНаборыСвойствДляПартнеров(Клиент, Конкурент, Поставщик,
		ПрочиеОтношения); // , ЮрФизЛицо);
...
&НаСервереБезКонтекста
Процедура ЗаполнитьРеквизитыИзМассиваСоответствий(СтруктураДанныхФормы, ЗначениеМассива, ЭлементСправочника,
	МассивСоответствий,
	ИмяСправочника)
	...
	Для Каждого СтрокаРеквизитов ИЗ МассивСоответствий Цикл
		...
		Если Не СтрокаРеквизитов.ПризнакКИ И Не СтрокаРеквизитов.ДополнительныйРеквизит Тогда
			...
			Иначе
				...
				Если СтрокаРеквизитов.РеквизитСправочник Тогда
					Если ИмяСправочника = "Партнеры" И ИмяРеквизита = "БизнесРегион"
						И Не СтрокаРеквизитов.ДополнительныйРеквизит Тогда
					...
					Если ИмяСправочника = "КонтактныеЛицаПартнеров" И ИмяРеквизита = "CRM_Должность"
						И Не СтрокаРеквизитов.ДополнительныйРеквизит Тогда
						Если ЗначениеЗаполнено(Значение) Тогда 
		...
		ИначеЕсли СтрокаРеквизитов.ДополнительныйРеквизит Тогда
			...
			ПВХСвойство =
				ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию(ДанныеФормыСвойство.Наименование);
				...
				Если ТипИнформации = Перечисления.ТипыКонтактнойИнформации.Телефон
					...
					СтрокаТЧ.Значение = УправлениеКонтактнойИнформациейСлужебный.СтруктураВСтрокуJSON(
							УправлениеКонтактнойИнформациейСлужебный.КонтактнаяИнформацияВСтруктуруJSON(
								CRM_ОбщегоНазначенияСервер.ЗначениеПолейВXML(ЗначенияПолей, СтрокаРеквизитов.СсылкаКИ), СтрокаТЧ.Тип));
					СтрокаТЧ.Представление	= УправлениеКонтактнойИнформациейКлиентСервер.СформироватьПредставлениеТелефона(
						СтруктураНомера.КодСтраны, СтруктураНомера.КодГорода, СтруктураНомера.НомерТелефона, СтруктураНомера.Добавочный,
						СтруктураНомера.Комментарий);
				Иначе
					СтрокаТЧ.Значение =
						УправлениеКонтактнойИнформациейСлужебныйВызовСервера.КонтактнаяИнформацияПоПредставлению(Значение,
							СтрокаРеквизитов.СсылкаКИ, "");
				КонецЕсли;
