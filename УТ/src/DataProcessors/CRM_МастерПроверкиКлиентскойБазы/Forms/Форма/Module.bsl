...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ВидМастера")
		...
		Если ВидМастера = Перечисления.CRM_ВидыМастераПроверкиКлиентскойБазы.КонтрольРегистрацииКлиентов Тогда
			...
			Элементы.ДекорацияОписаниеСтраницыСтраницаКонтрольЗаполненияКИВыборПолучателейПоручений.Заголовок =
				НСтр("ru='Распределение документов по исполнителям';
				|en='Distribution of documents by executors'");
		КонецЕсли;
...
Процедура ДекорацияОтборНажатие(ИмяРеквизитаАдресОтбор, ОписаниеОповещенияОЗавершении)
	...
	ПараметрыСтруктура.Вставить("Заголовок",
		 НСтр("ru='Настройка отбора по клиентам и контактным лицам';
		|en='Settings of filter by customers and contact persons'"));
	Если ЭтоАдресВременногоХранилища(ЭтотОбъект[ИмяРеквизитаАдресОтбор]) Тогда
		ПараметрыСтруктура.Вставить("ОтборКомпоновкиДанных",
			 ПолучитьИзВременногоХранилища(ЭтотОбъект[ИмяРеквизитаАдресОтбор]));
	КонецЕсли;
	...
	ЭлементОграничениеИспользования = Новый Структура("Поле,ВидыОграничений");
	ЭлементОграничениеИспользования.Поле = Новый Структура("Поле,ПутьКДанным,Заголовок",
		 "ТипКлассификации", "ТипКлассификации",
		 "ТипКлассификации");
	ЭлементОграничениеИспользования.ВидыОграничений = Новый Массив;
	ЭлементОграничениеИспользования.ВидыОграничений.Добавить("Условие");
	ОграничениеИспользования.Добавить(ЭлементОграничениеИспользования);
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ДекорацияОтборНажатиеЗавершение", ЭтотОбъект,
		 Новый Структура("ИмяРеквизитаАдресОтбор, ОписаниеОповещения", ИмяРеквизитаАдресОтбор,
...
Процедура ДекорацияОтборНажатиеЗавершение(НастроенныйОтбор, ДополнительныеПараметры) Экспорт
	...
	Если НастроенныйОтбор <> Неопределено Тогда
		Если ЭтоАдресВременногоХранилища(ЭтотОбъект[ДополнительныеПараметры.ИмяРеквизитаАдресОтбор]) Тогда
			УдалитьИзВременногоХранилища(ЭтотОбъект[ДополнительныеПараметры.ИмяРеквизитаАдресОтбор]);
		КонецЕсли;
		
		Если НастроенныйОтбор.Элементы.Количество() = 0 Тогда
			ЭтотОбъект[ДополнительныеПараметры.ИмяРеквизитаАдресОтбор] = "";
		Иначе
			ЭтотОбъект[ДополнительныеПараметры.ИмяРеквизитаАдресОтбор] = ПоместитьВоВременноеХранилище(НастроенныйОтбор,
...
Процедура ДекорацияОтборДляДублирующихсяРеквизитовНажатие(Элемент)
	ОписаниеОповещения = Новый ОписаниеОповещения("ДекорацияОтборНажатиеПослеЗавершения", ЭтотОбъект,
		 "АдресОтборДляДублирующихсяРеквизитов");
...
Процедура КонтрольРегистрацииКлиентовФлагСобытияПриИзменении(Элемент)
	...
	КонецЕсли;
	Элементы.ДекорацияОтборКонтрольРегистрацииКлиентов.Доступность = (КонтрольРегистрацииКлиентовФлагСобытия
		 Или КонтрольРегистрацииКлиентовФлагЭлектронныеПисьма);
КонецПроцедуры
...
Процедура ПодразделениеПриИзменении(Элемент)
	...
	Если ФормулаРасчета = "" Тогда
		...
		Элементы.ДекорацияФормулаРасчетаПотенциала.Заголовок   = НСтр("ru='Для подразделения не определена формула расчета потенциала клиентов. После настройки расчета потенциала необходимо выполнить команду ""Сохранить и пересчитать потенциалы""';
			|en='For the department,
			| the formula for calculating the potential of customers is not defined. After setting the calculation of the potential,
			| you must execute the command ""Save and recalculate the potentials""'");
	...
	Иначе	
		...
		Элементы.ДекорацияФормулаРасчетаПотенциала.Заголовок   = НСтр("ru='Формула расчета потенциала:';en='The formula for calculating the potential is:'") 
			+ Символы.ПС 
			+ ФормулаРасчета;
		Элементы.ДекорацияФормулаРасчетаПотенциала.Видимость   = Истина;
...
Процедура ДекорацияНастроитьРасчетПотенциаловНажатие(Элемент)
	...
	ОповещениеРасчетПотенциалов		= Новый ОписаниеОповещения("ОбработкаРасчетПотенциалов", ЭтотОбъект,
		 ДополнительныеПараметры);
	ОткрытьФорму("РегистрСведений.CRM_ВесаПоказателейПотенциала.Форма.ФормаНастройкиПотенциала",
		 ПараметрыФормы, ЭтотОбъект, , , ,
		 ОповещениеРасчетПотенциалов);
КонецПроцедуры
...
Процедура ПодразделениеОткрытие(Элемент, СтандартнаяОбработка)
	...
	ОповещениеРасчетПотенциалов		= Новый ОписаниеОповещения("ОбработкаРасчетПотенциалов", ЭтотОбъект,
		 ДополнительныеПараметры);
	ОткрытьФорму("Справочник.СтруктураПредприятия.ФормаОбъекта", ПараметрыФормы, ЭтотОбъект, , , ,
		 ОповещениеРасчетПотенциалов);
КонецПроцедуры
...
Процедура ДеревоФормированиеСписковВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Попытка
		ЗначениеОткрыть = ТекДанные.Партнер;
	...
	Попытка
		ПоказатьЗначение(, ЗначениеОткрыть);
...
Процедура КомандаДалее(Команда)
	...
	Если Элементы.ПанельСтраницы.ТекущаяСтраница = Элементы.СтраницаКонтрольЗаполненияВыборПолучателейПоручений Тогда
		Если Не ЗначениеЗаполнено(ДатаИсполнения) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не выбрана дата исполнения!';
				|en='Date of execution are not selected!'"), ,
				 "ДатаИсполнения");
			Возврат;
		КонецЕсли;
		Если Не ЗначениеЗаполнено(ДатаКонтроля) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не выбрана дата контроля!';
				|en='Date of monitoring are not selected!'"), ,
				 "ДатаКонтроля");
			Возврат;
		КонецЕсли;
		Если ДатаКонтроля < ДатаИсполнения Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Дата контроля не может быть меньше даты исполнения!';
				|en='Control date must not be earlier than execution date!'"), ,
				 "ДатаКонтроля");
			Возврат;
		КонецЕсли;
		Если Не ЗначениеЗаполнено(Проверяющий) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не выбран проверяющий!';
				|en='The check are not selected!'"), ,
				 "Проверяющий");
			Возврат;
		КонецЕсли;
	КонецЕсли;
		...
		ИначеЕсли ТекСтраница = Элементы.СтраницаКонтрольЗаполненияКИВыборВидовКИ Тогда
			...
			Если ВыбранныеВидыКИ.Партнер.Количество() = 0 И ВыбранныеВидыКИ.КонтактноеЛицо.Количество() = 0 Тогда
				ПоказатьПредупреждение(,
					 НСтр("ru='Не выбрано ни одного вида контактной информации!';
					|en='It is not selected any type of the contact information!'"));
				Возврат;
			КонецЕсли;
	...
	ИначеЕсли ВидКонтроля = ПредопределенноеЗначение("Перечисление.CRM_ВидыМастераПроверкиКлиентскойБазы.ЗаполнениеДополнительныхРеквизитов") Тогда
		...
		ИначеЕсли ТекСтраница = Элементы.СтраницаКонтрольЗаполненияДопРеквизитовВыборДопРеквизитов Тогда
			...
			Если ВыбранныеДопРеквизиты.Партнер.Количество() = 0 И ВыбранныеДопРеквизиты.КонтактноеЛицо.Количество() = 0 Тогда
				ПоказатьПредупреждение(,
					 НСтр("ru='Не выбрано ни одного дополнительного реквизита!';
					|en='It is not selected any additional attribute!'"));
				Возврат;
			КонецЕсли;
	...
	ИначеЕсли ВидКонтроля = ПредопределенноеЗначение("Перечисление.CRM_ВидыМастераПроверкиКлиентскойБазы.КонтрольПотенциалаКлиентов") Тогда
		...
		ИначеЕсли ТекСтраница = Элементы.СтраницаКонтрольПотенциалаВыборРеквизитов Тогда
			Если Не КонтрольПотенциалаФлагБезЗаполненногоПотенциала
				 И (Не ВедетсяИсторияРеквизитовВлияющихНаПотенциалКлиента
				 Или Не КонтрольПотенциалаФлагИзменилисьРеквизитыВлияющиеНаПотенциал) Тогда
			...
			Если ПолучитьФормулуРасчетаПотенциала() = "" Тогда
				ПоказатьПредупреждение(,
					 НСтр("ru='У подразделения не заполнена формула расчета потенциала!';
					|en='Department does not have a formula for calculating the potential!'"));
				Возврат;
			КонецЕсли;
			Состояние(НСтр("ru='Формируется список клиентов...';en='The list of customers is formed...'"));
	...
	ИначеЕсли ВидКонтроля = ПредопределенноеЗначение("Перечисление.CRM_ВидыМастераПроверкиКлиентскойБазы.КонтрольРегистрацииКлиентов") Тогда
		...
		ИначеЕсли ТекСтраница = Элементы.СтраницаКонтрольРегистрацииКлиентовВыборРеквизитов Тогда
			...
			РазвернутьСтрокиДерева(Элементы.ДеревоСобытияКонтрольРегистрацииКлиентов,
				 "ДеревоСобытияКонтрольРегистрацииКлиентов",
				 Ложь);
...
Процедура КомандаТаблицаИсполнителейОчистить(Команда)
	Если ТаблицаИсполнителей.Количество() > 0 Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения,
			 НСтр("ru='Очистить список исполнителей?';en='Clear the list of executors?'"),
			 РежимДиалогаВопрос.ДаНет, ,
...
Процедура ВидимостьИДоступностьЭлементовФормы(Форма)
	...
	Форма.Элементы.КонтрольПотенциалаФлагИзменилисьРеквизитыВлияющиеНаПотенциал.Доступность =
		Форма.ВедетсяИсторияРеквизитовВлияющихНаПотенциалКлиента;
	...
	Форма.Элементы.ДекорацияОтборКонтрольРегистрацииКлиентов.Доступность = (Форма.КонтрольРегистрацииКлиентовФлагСобытия
		 Или Форма.КонтрольРегистрацииКлиентовФлагЭлектронныеПисьма);
...
Функция ПолучитьТекстЗапросаДляОтборов(ВидКонтроля, ДополнительныеПараметры = Неопределено)
	Если ВидКонтроля = ПредопределенноеЗначение("Перечисление.CRM_ВидыМастераПроверкиКлиентскойБазы.КонтрольРегистрацииКлиентов") Тогда
		...
		Если ДополнительныеПараметры.КонтрольРегистрацииКлиентовФлагСобытия Тогда
			...
			|	И ТелефонныеЗвонки.АбонентКонтакт.Ссылка ЕСТЬ NULL
			|	
			|";
...
Функция ПолучитьВыбранныхИсполнителейПоОтветственнымМенеджерамКлиентов()
	...
	Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
		Если ЗначениеЗаполнено(НайденнаяСтрока.Партнер)
			 И ТипЗнч(НайденнаяСтрока.Партнер) = Тип("СправочникСсылка.Партнеры")
			 И МассивПартнеры.Найти(НайденнаяСтрока.Партнер) = Неопределено Тогда
...
Процедура СформироватьДеревоПартнеровКИ()
	...
	ТаблицаПартнерыПоОтбору = CRM_ОбщегоНазначенияСервер.ВыполнитьЗапросВКомпоновкеДанных(ПолучитьТекстЗапросаДляОтборов(ВидКонтроля),
		 ТекОтбор);
	...
	Пока ВыборкаПартнеры.Следующий() Цикл
		Если Не ЗначениеЗаполнено(ВыборкаПартнеры.Партнер) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура СформироватьДеревоПартнеровДопРеквизиты()
	...
	Если ВыбранныеДопРеквизиты.Партнер.Количество() > 0 Тогда
		Для Каждого ДопРеквизит Из ВыбранныеДопРеквизиты.Партнер Цикл
			...
			ИмяПараметраДопРеквизитПустоеЗначение = "ДопРеквизитПустоеЗначение" 
				+ Формат(СчетчикПараметраДопРеквизит, "ЧН=0; ЧГ=");
			...
			Иначе
				...
				|ЛЕВОЕ СОЕДИНЕНИЕ
				|	Справочник.Партнеры." 
					+ ?(ТипЗнч(ДопРеквизит) = Тип("ПланВидовХарактеристикСсылка.CRM_ДополнительнаяИнформация"),
						 "CRM_ДополнительнаяИнформация", "ДополнительныеРеквизиты") 
						+ " КАК ПартнерыДополнительныеРеквизиты
				|	
				|ПО
				...
				|	Партнеры.Ссылка В (&Партнеры)
				|	И НЕ Партнеры.ПометкаУдаления
				|	И (ПартнерыДополнительныеРеквизиты.Значение = &" + ИмяПараметраДопРеквизитПустоеЗначение 
					+ " ИЛИ ПартнерыДополнительныеРеквизиты.Значение ЕСТЬ NULL)
				|	
				|";
				
				Запрос.УстановитьПараметр(ИмяПараметраДопРеквизит, ДопРеквизит);
				Запрос.УстановитьПараметр(ИмяПараметраДопРеквизитПустоеЗначение,
					 ДопРеквизит.ТипЗначения.ПривестиЗначение(Неопределено));
			КонецЕсли;
	...
	Если ВыбранныеДопРеквизиты.КонтактноеЛицо.Количество() > 0 Тогда 
		Для Каждого ДопРеквизит Из ВыбранныеДопРеквизиты.КонтактноеЛицо Цикл
			...
			Если ТипЗнч(ДопРеквизит) = Тип("Строка") Тогда
				...
				Если ВыразитьСтрокуВЗапросе Тогда
					ТекстЗапроса = ТекстЗапроса + "
					|	И ВЫРАЗИТЬ(КонтактныеЛицаПартнеров." + ДопРеквизит + " КАК СТРОКА(100)) = &" 
						+ ИмяПараметраДопРеквизитПустоеЗначение 
						+ "
					|	
					|";
				Иначе
				...
				НайденныеСтроки = ТаблицаРеквизитыКонтактногоЛицаДополнительные.НайтиСтроки(Новый Структура("Реквизит",
					 ДопРеквизит));
				Запрос.УстановитьПараметр(ИмяПараметраДопРеквизит, НайденныеСтроки[0].Представление);
				Запрос.УстановитьПараметр(ИмяПараметраИмяРеквизита, ДопРеквизит);
			...
			Иначе
				...
				|	И КонтактныеЛицаПартнеров.Ссылка В (&КонтактныеЛица)
				|	И НЕ КонтактныеЛицаПартнеров.ПометкаУдаления
				|	И (КонтактныеЛицаПартнеровДополнительныеРеквизиты.Значение = &" 
					+ ИмяПараметраДопРеквизитПустоеЗначение 
					+ " ИЛИ КонтактныеЛицаПартнеровДополнительныеРеквизиты.Значение ЕСТЬ NULL)
				|	
				|";
	...
	Таблица.Свернуть("Партнер,ДопРеквизитПартнера,КонтактноеЛицо,ДопРеквизитКонтактногоЛица,
		|ИмяРеквизитаПартнера,
		|ИмяРеквизитаКонтактногоЛица");
	...
	Пока ВыборкаПартнеры.Следующий() Цикл
		...
		Если СоответствиеСтроки[ВыборкаПартнеры.Партнер] <> Неопределено
			И СоответствиеСтроки[ВыборкаПартнеры.Партнер].Строки.Количество() = 0 Тогда
			// BSLLS:MissingCodeTryCatchEx-off
			Попытка
				Дерево.Строки.Удалить(СоответствиеСтроки[ВыборкаПартнеры.Партнер]);
...
Процедура СформироватьДеревоПартнеровКонтрольПотенциала()
	...
	|				(ВЫБРАТЬ
	|					тмпКлиентыСПотенциалами.Клиент КАК Клиент
	|				ИЗ
...
Процедура ДобавитьСтрокуПартнеров(СоответствиеРезультат, Исполнитель, СтрокаДерева)
	...
	Если ЗначениеЗаполнено(СтрокаДерева.Партнер) Тогда
		Если ВидКонтроля = Перечисления.CRM_ВидыМастераПроверкиКлиентскойБазы.ДублированиеКлиентов Тогда
			СоответствиеРезультат[Исполнитель].Добавить(Новый Структура("Партнер,ПартнерыДубли",
				 СтрокаДерева.Партнер,
				 Новый Массив()));
			Для Каждого ПодстрокаПартнер Из СтрокаДерева.Строки Цикл
				СоответствиеРезультат[Исполнитель][СоответствиеРезультат[Исполнитель].ВГраница()].ПартнерыДубли.Добавить(Новый Структура("Партнер,
					|Реквизиты", ПодстрокаПартнер.Партнер,
					 Новый Массив()));
		...
		Иначе
			Если	ТипЗнч(СтрокаДерева.Партнер) = Тип("СправочникСсылка.Партнеры")
				...
				//
				СоответствиеРезультат[Исполнитель].Добавить(Новый Структура("Партнер,Реквизиты",
					 СтрокаДерева.Партнер,
					 Новый Массив()));
				Для Каждого ПодчСтрока Из СтрокаДерева.Строки Цикл
					Если ЗначениеЗаполнено(ПодчСтрока.Партнер) Тогда
						Если ТипЗнч(ПодчСтрока.Партнер) = Тип("СправочникСсылка.Партнеры")
							 Или ТипЗнч(ПодчСтрока.Партнер) = Тип("СправочникСсылка.КонтактныеЛицаПартнеров") Тогда
...
Процедура ВывестиКоличествоВыбранныхКлиентов()
	...
	Иначе
		...
		КоличествоНаОдногоПользователя = ?(КоличествоПользователей > 0,
			 Цел(КоличествоВыбранныхПартнеров / КоличествоПользователей),
			 КоличествоВыбранныхПартнеров);
	...
	Если ПоОтветственнымМенеджерам Тогда
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			...
			Иначе
				...
				Иначе
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Для некоторых клиентов не указан основной менеджер - нужно выбрать иполнителя для клиентов,
...
Функция СформироватьПоручения(ВыбранныеИсполнители)
	...
	РаспределенныеПартнеры = РаспределитьПартнеровПоИсполнителям((ВидРаспределенияПоИсполнителям = 0),
			...
			Для Каждого СтрокаНабора Из НаборИсполнителей Цикл
				Если СтрокаНабора.ТочкаМаршрута = НаборЭтапов[0].ТочкаМаршрута Тогда		// точка действия "Выполнить"
					...
					НаборЭтапов[0].ВариантВыполненияСтрокой = CRM_БизнесПроцессыСервер.ПолучитьПредставлениеВариантаУсловия(НаборЭтапов[0].ТочкаМаршрута,
						 ...
						 НовыйБизнесПроцесс.НомерВерсииКартыМаршрута);
...
Функция НайтиДубли(СсылкаНаОбъект, СтруктураПоиска, СписокКонтактов)
		...
		Если ИмяРеквизита = "CRM_ИНН" Тогда
			Если CRM_ОбщегоНазначенияСервер.НайтиВМетаданныхПоИмени("Справочники", "Контрагенты") Тогда
				Если CRM_ОбщегоНазначенияСервер.НайтиВМетаданныхПоИмени("Справочники", "Партнеры") Тогда
					...
					// Используется при наличии справочника Контрагенты (в спарках)
					...
					// Запрос выполняется только один раз за цикл, если ИмяРеквизита = "CRM_ИНН"
	...
	Иначе	
		Для Каждого СтрокаМассива Из МассивНайденных Цикл
			Если СтрокаМассива.Ссылка = СсылкаНаОбъект Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ФормированиеПоручений()
	...
	Если ВидКонтроля = ПредопределенноеЗначение("Перечисление.CRM_ВидыМастераПроверкиКлиентскойБазы.КонтрольПотенциалаКлиентов") Тогда
		...
		МассивПользователейНеРазрешеноРедактированиеПотенциала =
			ПроверитьПравоПользователейНаРедактированиеПотенциалаКлиента(ИсполнителиКонтрольПотенциалаКлиентов);
		
		Если МассивПользователейНеРазрешеноРедактированиеПотенциала.Количество() > 0 Тогда
			ДополнительныеПараметры = Новый Структура("МассивПользователейНеРазрешеноРедактированиеПотенциала,
				| ИсполнителиКонтрольПотенциалаКлиентов",
				МассивПользователейНеРазрешеноРедактированиеПотенциала, ИсполнителиКонтрольПотенциалаКлиентов);
			ОписаниеОповещения = Новый ОписаниеОповещения("ФормированиеПорученийПродолжение", ЭтотОбъект,
				 ДополнительныеПараметры);
			ПоказатьВопрос(ОписаниеОповещения,
				 НСтр("ru='Один или более из выбранных пользователей не имеет право на изменение потенциала клиента."
...
Процедура СохранитьНастройкиМастера()
	...
	СтруктураНастройки.Вставить("КонтрольПотенциалаФлагБезЗаполненногоПотенциала",
		 КонтрольПотенциалаФлагБезЗаполненногоПотенциала);
	СтруктураНастройки.Вставить("КонтрольПотенциалаФлагИзменилисьРеквизитыВлияющиеНаПотенциал",
		 КонтрольПотенциалаФлагИзменилисьРеквизитыВлияющиеНаПотенциал);
	СтруктураНастройки.Вставить("КонтрольРегистрацииКлиентовФлагСобытия", КонтрольРегистрацииКлиентовФлагСобытия);
	СтруктураНастройки.Вставить("КонтрольРегистрацииКлиентовФлагЭлектронныеПисьма",
		 КонтрольРегистрацииКлиентовФлагЭлектронныеПисьма);
...
Процедура ОбработкаРасчетПотенциалов(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Иначе	
		Элементы.ДекорацияФормулаРасчетаПотенциала.ЦветТекста  = Новый Цвет(0, 0, 255);
		Элементы.ДекорацияФормулаРасчетаПотенциала.Заголовок   = НСтр("ru='Формула расчета потенциала:'") 
			+ Символы.ПС 
			+ ФормулаРасчета;
		Элементы.ДекорацияФормулаРасчетаПотенциала.Видимость   = Истина;
		Элементы.ДекорацияНастроитьРасчетПотенциалов.Видимость = Ложь;
...
Процедура РассчитатьПотенциалыНаСервере()
	...
	Для Каждого СтрокаДерева Из ДеревоПартнерыКонтрольПотенциала.ПолучитьЭлементы() Цикл
		...
		Попытка
			МенеджерЗаписи.Записать();
		Исключение
			СобытиеЖурнала =  НСтр("ru = 'Расчет потенциалов'", ОбщегоНазначения.КодОсновногоЯзыка());
