...
Процедура СоздаемДопРеквизитыДляВсехОбъектов()
	...
	Если ПолучитьПВХПоИмени(ИмяДляРазработчика) = Неопределено Тогда
		...
		Структура.Вставить("НаборСвойств", НаборДополнительныхРеквизитовИСведений("Справочник_Партнеры_Общие"));
	...
	Если ПолучитьПВХПоИмени(ИмяДляРазработчика) = Неопределено Тогда
		...
		Структура.Вставить("Имя", ИмяДляРазработчика);
		Структура.Вставить("Заголовок", "ID amoCRM");
		Структура.Вставить("НаборСвойств", НаборДополнительныхРеквизитовИСведений("Справочник_КонтактныеЛицаПартнеров"));
		Структура.Вставить("ТипЗначения", Новый ОписаниеТипов("Строка", , , , Новый КвалификаторыСтроки("20")));
		Структура.Вставить("Доступен", Истина);
		Структура.Вставить("Виден", Ложь);
	...
	Если ПолучитьПВХПоИмени(ИмяДляРазработчика) = Неопределено Тогда
		...
		Структура.Вставить("Имя", ИмяДляРазработчика);
		Структура.Вставить("Заголовок", "ID amoCRM");
		Структура.Вставить("НаборСвойств", НаборДополнительныхРеквизитовИСведений("Документ_CRM_Интерес"));
		Структура.Вставить("ТипЗначения", Новый ОписаниеТипов("Строка", , , , Новый КвалификаторыСтроки("20")));
		Структура.Вставить("Доступен", Истина);
		Структура.Вставить("Виден", Ложь);
	...
	Если ПолучитьПВХПоИмени(ИмяДляРазработчика) = Неопределено Тогда
		...
		Структура.Вставить("Имя", ИмяДляРазработчика);
		Структура.Вставить("Заголовок", "ID amoCRM");
		Структура.Вставить("НаборСвойств", НаборДополнительныхРеквизитовИСведений("Документ_CRM_Взаимодействие"));
		Структура.Вставить("ТипЗначения", Новый ОписаниеТипов("Строка", , , , Новый КвалификаторыСтроки("20")));
		Структура.Вставить("Доступен", Истина);
		Структура.Вставить("Виден", Ложь);
...
Процедура КомпанииПриИзменении(Элемент)
	...
	Если Объект.Компании И Объект.ПартнерыСопоставление.Количество() = 0 Тогда
		...
		ПоказатьВопрос(Оповещение,
			 НСтр("ru='Перед началом загрузки рекомендуется сопоставить реквизиты партнера с реквизитами AmoCRM ';
			|en='Before starting download, it is recommended to match partner details with AmoCRM details'"),
			 Режим,
			 0);
	КонецЕсли;
...
Процедура КонтактыПриИзменении(Элемент)
	...
	Если Объект.Компании И Объект.КонтактныеЛицаСопоставление.Количество() = 0 Тогда
		...
		ПоказатьВопрос(Оповещение,
			 НСтр("ru='Перед началом загрузки рекомендуется сопоставить реквизиты контактных лиц с реквизитами AmoCRM ';
			|en='Before you start downloading,
			| it is recommended to compare details of contact persons with details AmoCRM'"), Режим,
			 0);
	КонецЕсли;
...
Процедура ЗагрузитьПользователейИзAmoCRM(Команда)
	...
	Если Объект.ПользователиСопоставление.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Оповещение,
			 НСтр("ru='Табличная часть не пустая. Очистить?';en='Tabular section is not empty. Clear?'"), Режим,
			 0);
	Иначе	
...
Процедура ОтправитьУведомлениеНаПочтуПриИзменении(Элемент)
	...
	Если НЕ Отказ Тогда
		ПоказатьПредупреждение(,
			 "Системная учетная запись для отправки почты не настроена. Для настройки перейдите в раздел ""Настройки"" - ""Учетные записи электронной почты""");
...
Процедура Авторизация(Команда)
	...
	ОткрытьФорму("Обработка.CRM_ОбменСAmoCrm.Форма.ФормаАвторизации", ПараметрыДоступа, ЭтотОбъект, , ,
		 , ,
		 РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
