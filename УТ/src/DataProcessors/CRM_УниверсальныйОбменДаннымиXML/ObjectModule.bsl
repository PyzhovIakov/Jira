...
// Много вызовов Выполнить() для обработчиков событий правил обмена
...
Функция ПривестиНомерКДлине(Знач Стр, Длина,
	 ДобавлятьНулиЕслиДлинаНеМеньшеТекущейДлиныНомера = Истина,
	 Префикс = "") Экспорт
...
Процедура УдалениеОбъектаВыборки(Объект, Правило, Свойства = Неопределено, ВходящиеДанные = Неопределено) Экспорт
	...
	Если Не ПустаяСтрока(Правило.ПередУдалением) Тогда
		...
		Исключение
			...
			ЗаписатьИнформациюОбОшибкеОбработчикаОчисткиДанных(29, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), Правило.Имя, Объект,
...
Функция ВыгрузитьПоПравилу(Источник					= Неопределено,
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		СтрокаПользователю = ?(ТолькоПолучитьУзелСсылки,
			 НСтр("ru='Конвертация ссылки на объект: %1';en='Conversion of reference to object: %1 '"),
			 НСтр("ru='Конвертация объекта: %1'"));
	...
	Если ПКО.ЕстьОбработчикПослеВыгрузкиВФайл Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеВыгрузкиОбработчикаПКО(76, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), ПКО, Источник,
...
Процедура ПередатьОдинПараметрВПриемник(Имя, ИсходноеЗначениеПараметра, ПравилоКонвертации = "") Экспорт
	...
	Иначе
		...
		ВыгрузитьДанныеСсылочногоОбъекта(ИсходноеЗначениеПараметра, Неопределено, ПравилоКонвертации,
			 Неопределено, Неопределено, УзелПараметра,
			 Истина);
	...
	Если Не ПустаяСтрока(ИмяВременногоФайла) Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Универсальный обмен данными в формате XML';
				|en='Universal Data Exchange (XML)'", КодОсновногоЯзыка()),
...
Функция ЗаписатьВПротоколВыполнения(Код = "", СтруктураЗаписи = Неопределено, ВзвестиФлагОшибок = Истина, 
	...
	Если СтруктураЗаписи <> Неопределено Тогда
		...
		Для каждого Поле Из СтруктураЗаписи Цикл
			...
			КонецЕсли; 
			Ключ = Поле.Ключ;
			Стр  = Стр + Символы.ПС + Отступ + Символы.Таб + одДополнитьСтроку(Ключ, Выравнивание) 
				+ " =  " + Строка(Значение);
...
//
Процедура ЗаписатьИнформациюОбОшибкеОбработчикаОчисткиДанных(КодСообщения, СтрокаОшибки,
	 ИмяПравилаОчисткиДанных, Объект = "", ИмяОбработчика = "")
...
Процедура ЗаписатьИнформациюОбОшибкеОбработчикиПВД(КодСообщения, СтрокаОшибки, ИмяПравила,
	 ИмяОбработчика, Объект = Неопределено)
...
Процедура ЗагрузитьПКС(ПравилаОбмена, ТаблицаСвойств, ТаблицаПоиска = Неопределено, СтрокаПолейПоиска = "")
	...
	НоваяСтрока.УпрощеннаяВыгрузкаСвойства = НЕ НоваяСтрока.ПолучитьИзВходящихДанных
		...
		И (НоваяСтрока.ТипИсточника = "Строка" ИЛИ НоваяСтрока.ТипИсточника = "Число"
...
Процедура ВыгрузитьОбработчикиСобытий(Отказ) Экспорт
	...
	ДополнитьПравилаИнтерфейсамиОбработчиков(Конвертация, ТаблицаПравилКонвертации,
		 ТаблицаПравилВыгрузки, ТаблицаПравилОчистки);
	...
	ИмяВременногоФайлаОбработчиковСобытий =
		ПолучитьНовоеУникальноеИмяВременногоФайла(ИмяВременногоФайлаОбработчиковСобытий);
...
Процедура ВыгрузитьКонструкторВнешнейОбработки(Результат)
	...
	ИнтерфейсПроцедурыВнешнейОбработки = "Процедура " 
		+ ПолучитьИнтерфейсПроцедурыВнешнейОбработки("Конструктор") + " Экспорт";
			...
			Если АлгоритмыОбработчика.Количество() <> 0 Тогда // В этой строке есть вызовы алгоритмов.
				...
				Для Каждого Алгоритм Из АлгоритмыОбработчика Цикл
					...
					Для ИндексСтрокиАлгоритма = 1 По СтрЧислоСтрок(ОбработчикАлгоритма) Цикл
						...
						Результат.ЗаписатьСтроку(СтрокаПрефиксаДляВложенногоКода 
							+ СтрПолучитьСтроку(ОбработчикАлгоритма, ИндексСтрокиАлгоритма));
...
Процедура ПолучитьПолныйКодАлгоритмовСУчетомВложенности()
	...
	Для Каждого Алгоритм Из Алгоритмы Цикл
		...
		АлгоритмыИнтегрированные.Вставить(Алгоритм.Ключ,
			 ЗаменитьВызовыАлгоритмовКодомЭтихАлгоритмовВОбработчике(Алгоритм.Значение, Алгоритм.Ключ,
			 Новый Массив));
...
Функция ЗаменитьВызовыАлгоритмовКодомЭтихАлгоритмовВОбработчике(Обработчик, АлгоритмВладелец,
	...
	ЗаписатьЗаголовокБлокаАлгоритма(ОбработчикНовый, АлгоритмВладелец, СтрокаПрефикса,
		 НСтр("ru='{НАЧАЛО АЛГОРИТМА}';en='{ALGORITHM START}'"));
		...
		Если АлгоритмыОбработчика.Количество() <> 0 Тогда // В этой строке есть вызовы алгоритмов.
			...
			Для Каждого Алгоритм Из АлгоритмыОбработчика Цикл
				...
				Если МассивЗапрошенныхЭлементов.Найти(ВРег(Алгоритм)) <> Неопределено Тогда // Рекурсивный вызов алгоритма.
					...
					ЗаписатьЗаголовокБлокаАлгоритма(ОбработчикНовый, Алгоритм, СтрокаПрефиксаДляВложенногоКода,
						 НСтр("ru='{РЕКУРСИВНЫЙ ВЫЗОВ АЛГОРИТМА}';
				...
				Иначе
					...
					ОбработчикНовый = ОбработчикНовый 
						+ ЗаменитьВызовыАлгоритмовКодомЭтихАлгоритмовВОбработчике(Алгоритмы[Алгоритм], Алгоритм,
							 СкопироватьМассив(МассивЗапрошенныхЭлементов),
	...
	ЗаписатьЗаголовокБлокаАлгоритма(ОбработчикНовый, АлгоритмВладелец, СтрокаПрефикса,
		 НСтр("ru='{КОНЕЦ АЛГОРИТМА}';en='{ALGORITHM END}'"));
...
Функция ПолучитьПрефиксДляВложенногоАлгоритма(СтрокаОбработчика, СтрокаПрефикса)
	...
	СтрокаПрефиксаДляВложенногоКода = СтрокаПрефикса + Лев(СтрокаОбработчика,
		 НомерПозицииШаблонаВыполнить - 1) 
		+ Символы.Таб;
...
Процедура ОчиститьДанныеПоПравилу(Правило)
	...
	Если Не ПустаяСтрока(Правило.ПередОбработкой) Тогда
		...
		Исключение
			...
			ЗаписатьИнформациюОбОшибкеОбработчикаОчисткиДанных(27, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), Правило.Имя, "",
	...
	Если Не ПустаяСтрока(Правило.ПослеОбработки) Тогда
		...
		Исключение
			...
			ЗаписатьИнформациюОбОшибкеОбработчикаОчисткиДанных(28, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), Правило.Имя, "",
...
Процедура ЗаписатьОбъектВИБ(Объект, Тип)
	...
	Исключение
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "ДополнительныеСвойства")
			И Объект.ДополнительныеСвойства.Свойство("ЗарегистрироватьКОбменуДляОтката") Тогда
			
			УзелОбмена =
				ПланыОбмена.CRM_СинхронизацияСМобильнымПриложениемНовый.НайтиПоКоду(Объект.ДополнительныеСвойства.ИдентификаторУзла);
			Если НЕ УзелОбмена.Пустая() Тогда
				ПланыОбмена.ЗарегистрироватьИзменения(УзелОбмена, Объект.Ссылка);
				РегистрыСведений.CRM_ЛогиСобытийiCRM.ЗаписатьСобытие("(ЗаписатьОбъектВИБ) Откат объекта: " 
					+ Объект.Ссылка, Перечисления.CRM_ТипыСобытийЛогаiCRM.Предупреждение, УзелОбменаiCRM);
			КонецЕсли;
...
Функция ПрочитатьСвойство(Тип, ИмяПКО = "")
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		ИначеЕсли (ИмяУзла = "Свойство" ИЛИ ИмяУзла = "ЗначениеПараметра")
			 И (ФайлОбмена.ТипУзла = одТипУзлаXML_КонецЭлемента) Тогда
...
Функция НайтиИлиСоздатьОбъектПоСвойству(СтруктураСвойств, ТипОбъекта, СвойстваПоиска, СвойстваПоискаНеЗамещать,
	...
	Если РежимПоискаОсновногоОбъекта Тогда
		...
		СвойстваОбъектаМодифицированы = УстановитьРеквизитыПоискаУОбъекта(НайденныйИлиСозданныйОбъект,
			 СвойстваПоиска, СвойстваПоискаНеЗамещать);
...
Функция ПолучитьТипСвойства()
	...
	// +CRM
	МассивСоставляющих = СтрРазделить(СтроковыйТипСвойства, ".");
	Если МассивСоставляющих.Количество() > 0 Тогда
		Если МассивСоставляющих[0] = "СправочникСсылка"
			ИЛИ МассивСоставляющих[0] = "ДокументСсылка"
			ИЛИ МассивСоставляющих[0] = "ПеречислениеСсылка" Тогда
			МассивСоставляющих[0] = СтрЗаменить(МассивСоставляющих[0], "Ссылка", "");
			СоединениеСтрок = СтрСоединить(МассивСоставляющих, ".");
			
			Если Метаданные.НайтиПоПолномуИмени(СоединениеСтрок) = Неопределено Тогда
				Возврат Неопределено;
			КонецЕсли;
...
Функция НайтиЭлементЗапросом(СтруктураСвойств, СвойстваПоиска, ТипОбъекта = Неопределено, 
	...
	КоличествоСвойствДляПоиска = ?(КоличествоРеальныхСвойствДляПоиска = Неопределено,
		 СвойстваПоиска.Количество(), КоличествоРеальныхСвойствДляПоиска);
	...
	Для каждого Свойство Из СвойстваПоиска Цикл
		...
		Попытка
			...
			СтрокаНеограниченнойДлины = ОпределитьЭтотПараметрНеограниченнойДлинны(СтруктураСвойств,
				 ЗначениеПараметра, ИмяПараметра);		
		...
		Если СтрокаНеограниченнойДлины Тогда
			...
			ТекстЗапроса = ТекстЗапроса + ?(КоличествоСвойствПоКоторымУстановленПоиск > 1, " И ", "") 
				+ ИмяПараметра + " ПОДОБНО &" 
				+ ИмяПараметра;
...
Функция НайтиСсылкуНаДокумент(СвойстваПоиска, СтруктураСвойств, КоличествоРеальныхСвойствДляПоиска,
...
Функция НайтиЭлементПоСвойствамПоиска(ТипОбъекта, ИмяТипаОбъекта, СвойстваПоиска, 
	...
	КоличествоРеальныхСвойствДляПоиска = КоличествоРеальныхСвойствДляПоиска - ?(СвойствоУникальныйИдентификатор <> Неопределено,
		 1, 0);
	КоличествоРеальныхСвойствДляПоиска = КоличествоРеальныхСвойствДляПоиска - ?(СвойствоИмяПредопределенного <> Неопределено,
		 1, 0);
...
Функция ОбработатьПоискОбъектаПоСтруктуре(НомерОбъекта, ТипОбъекта, СозданныйОбъект,
	...
	Если СсылкаНаОбъект <> Неопределено Тогда
		...
		Если РежимПоискаОсновногоОбъекта Тогда
			...
			СвойстваОбъектаМодифицированы = УстановитьРеквизитыПоискаУОбъекта(СозданныйОбъект,
				 СвойстваПоиска, СвойстваПоискаНеЗамещать);
...
Процедура ПрочитатьИнформациюОСвойствахПоиска(ТипОбъекта, СвойстваПоиска, СвойстваПоискаНеЗамещать, 
	...
	ПрочитатьСвойстваПоискаИзФайла(СвойстваПоиска, СвойстваПоискаНеЗамещать, ИнформацияОТипах,
		 ПоискПоДатеНаРавенство, ПараметрыОбъекта);	
...
Функция НайтиОбъектПоСсылке(ТипОбъекта,
	...
	ИначеЕсли (СвойствоУникальныйИдентификатор <> Неопределено) Тогда
		...
		НужноПродолжитьПоискЕслиЭлементПоGUIDНеНайден =
			ОпределитьПоТипуОбъектаИспользоватьДополнительныйПоискПоПолямПоиска(СтруктураСвойств.ТипСсылкиСтрокой);
		...
		Если Не НужноПродолжитьПоискЕслиЭлементПоGUIDНеНайден Тогда
			...
			Если Не ЗначениеЗаполнено(СсылкаНаОбъект)
				...
				СсылкаНаОбъект =
					СтруктураСвойств.Менеджер.ПолучитьСсылку(Новый УникальныйИдентификатор(СвойствоУникальныйИдентификатор));
				ОбъектНайден = Ложь;
	...
	Если ПКО = Неопределено Тогда
		...
		ПКО =
			ОпределитьПоТипуОбъектаПриемникаПравилоКонвертацииКотороеСодержитАлгоритмПоиска(СтруктураСвойств.ТипСсылкиСтрокой);
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		ИначеЕсли ИмяУзла = "СубконтоДт" ИЛИ ИмяУзла = "СубконтоКт" Тогда
			...
			Пока ФайлОбмена.Прочитать() Цикл
				...
				ИначеЕсли (ИмяУзла = "СубконтоДт" ИЛИ ИмяУзла = "СубконтоКт")
					 И (ФайлОбмена.ТипУзла = одТипУзлаXML_КонецЭлемента) Тогда
...
Процедура ЗагрузитьДвижения(Объект, Имя, Очистить, ОбщаяИнформацияОТипеДокумента, НужноЗаписатьОбъект, 
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		Если ИмяУзла = "Запись" Тогда
			...
			НужноЗаписатьОбъект = Истина;
			УстановитьСвойстваЗаписи(Объект, Запись, ИнформацияОТипах, ПараметрыОбъекта, ИмяДвижений,
				 ДанныеПоискаПоТЧ, КопияТЧДляПоиска, НомерЗаписи);
			НомерЗаписи = НомерЗаписи + 1;
...
Процедура ДобавитьСложныйПараметрПриНеобходимости(ПараметрыДанных, ИмяВеткиПараметров, НомерСтроки,
...
Процедура ПровестиГенерациюКодаНомераПриНеобходимости(ГенерироватьНовыйНомерИлиКодЕслиНеУказан,
	 Объект, ИмяТипаОбъекта, НужноЗаписатьОбъект, РежимОбменДанными)
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		Если ИмяУзла = "Свойство"
			...
			Если НЕ ЭтоПараметрДляОбъекта
				...
				ИЛИ (Имя = "ЭтоГруппа")
				// +CRM
				ИЛИ CRM_РеквизитИлиСвойствоНеНайдено(Объект, Имя)
				// -CRM
		...
		ИначеЕсли (ИмяУзла = "Объект") И (ФайлОбмена.ТипУзла = одТипУзлаXML_КонецЭлемента) Тогда
			...
			ИначеЕсли ИмяТипаОбъекта <> "Перечисление" Тогда
				...
				Если ЭтоСсылочныйТипОбъекта Тогда 	
					...
					ПровестиГенерациюКодаНомераПриНеобходимости(ГенерироватьНовыйНомерИлиКодЕслиНеУказан, Объект,
						 ИмяТипаОбъекта, НужноЗаписатьОбъект,
				...
				Если НужноЗаписатьОбъект Тогда
					...
					// +CRM
					ДополнительныйОтказОтЗаписи = Ложь;
					Если ИмяТипаОбъекта = "Задача" И НЕ ЗначениеЗаполнено(Объект.Дата) Тогда
						ДополнительныйОтказОтЗаписи = Истина;
					КонецЕсли;
					
					Если Не ДополнительныйОтказОтЗаписи Тогда
...
//
// Возвращаемое значение:
//   Булево - Истина - установлена дата запрета изменения
//		и загружаемый объект имеет дату меньше установленной, иначе Ложь.
//
...
// +CRM
Функция CRM_РеквизитИлиСвойствоНеНайдено(Объект, Имя)
	
	МассивУстаревшихРеквизитов = Новый Массив;
	МассивУстаревшихРеквизитов.Добавить("CRM_Автор");
	МассивУстаревшихРеквизитов.Добавить("Автор");
	
	Возврат НЕ ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, Имя)
		И МассивУстаревшихРеквизитов.Найти(Имя) <> Неопределено;	
...
Процедура ВыгрузитьГруппуСвойств(Источник, Приемник, ВходящиеДанные, ИсходящиеДанные, ПКО, ПКГС, УзелКоллекцииСвойств, 
	...
	Если ВидПриемника = "ТабличнаяЧасть" Тогда
		...
		СоздатьОбъектыДляЗаписиДанныхВXML(СтруктураУзлаСвойств, УзелКоллекцииОбъектов, Истина,
			 ПКГС.Приемник,
			 ИмяГоловногоУзла);
...
Процедура ВыгрузитьСубконто(Источник,
	...
	Для каждого КлючИЗначение Из Значение Цикл
		...
		УзелСсылки = ВыгрузитьПоПравилу(ВидСубконто, , ИсходящиеДанные, , ИмяПКОВидСубконто, ,
...
Функция ВыгрузитьДанныеСсылочногоОбъекта(Значение, ИсходящиеДанные, ИмяПКО, ПКОСвойств,
	 ТипПриемника, УзелСвойства,
	 Знач ВыгрузитьТолькоСсылку)
	
	ЭтоПравилоСГлобальнойВыгрузкой = Ложь;
	УзелСсылки    = ВыгрузитьПоПравилу(Значение, , ИсходящиеДанные, , ИмяПКО, , ВыгрузитьТолькоСсылку,
		 ПКОСвойств, ЭтоПравилоСГлобальнойВыгрузкой);
	ТипУзлаСсылки = ТипЗнч(УзелСсылки);
...
Процедура ВыгрузитьСвойства(Источник, Приемник, ВходящиеДанные, ИсходящиеДанные, ПКО, КоллекцияПКС,
	...
	Для каждого ПКС Из КоллекцияПКС Цикл
		...
		ИначеЕсли ПКС.ЭтоГруппа Тогда
			...
			ВыгрузитьГруппуСвойств(Источник, Приемник, ВходящиеДанные, ИсходящиеДанные, ПКО, ПКС,
				 УзелКоллекцииСвойств, ВыгрузитьТолькоСсылку,
...
// 
Процедура ВыгрузкаОбъектаВыборки(Объект, Правило, Свойства = Неопределено,
	 ВходящиеДанные = Неопределено,
...
Процедура ВыгрузитьДанныеПоПравилу(Правило)
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("ru = 'Правило выгрузки данных: %1 (%2)';en='Data export rule: %1 (%2)'"),
			 СокрЛП(Правило.Имя), СокрЛП(Правило.Наименование));
...
Процедура ПолучитьСтрокуПравилВыгрузкиПоОбъектуОбмена(Данные, МетаданныеПоследнегоОбъекта, МетаданныеОбъектаВыгрузки, 
	...
	Если СтруктураДанных = Неопределено Тогда
		...
		Если МетаданныеПоследнегоОбъекта <> МетаданныеОбъектаВыгрузки Тогда
			...
			ТекущаяСтрокаПравилаВыгрузки = НайтиСтрокуДереваПравилВыгрузкиПоТипуВыгрузки(ВременныйМассивПравилКонвертации,
				 Тип("КонстантыНабор"));
...
Функция ОбработатьВыгрузкуДляПлановОбмена(СоответствиеУзловИПравилВыгрузки, СтруктураДляУдаленияРегистрацииИзменений)
	...
	Для Каждого СтрокаСоответствия Из СоответствиеУзловИПравилВыгрузки Цикл
		...
		ТекущаяУдачнаяВыгрузка = ВыполнитьВыгрузкуИзмененныхДанныхДляУзлаОбмена(УзелОбмена,
			 МассивПравилКонвертации,
...
Процедура ЗагрузитьПравилаОбмена(Источник = "", ТипИсточника = "XMLФайл") Экспорт
	...
	Если Конвертация.Свойство("ПослеЗагрузкиПравилОбмена", ТекстСобытияПослеЗагрузкиПравилОбмена)
		...
		Попытка
			...
			Если ФлагРежимОтладкиОбработчиков Тогда
				...
				ВызватьИсключение НСтр("ru='Отладка обработчика ""После загрузки правил обмена"" не поддерживается.';
					|en='Debugging of handler ""ПослеЗагрузкиПравилОбмена"" is not supported.'");
		...
		Исключение
			...
			Текст = НСтр("ru='Обработчик: ""%1"": %2';en='Handler: ""%1"": %2'");
...
Функция ПроизвестиЧтениеДанных(РезультирующаяСтрокаСИнформациейОбОшибке = "") Экспорт
					...
					Попытка
						...
						Если ФлагРежимОтладкиОбработчиков Тогда
							...
							ВызватьИсключение НСтр("ru='Отладка обработчика ""После загрузки параметров"" не поддерживается.';
								|en='Debugging of handler ""ПослеЗагрузкиПараметров"" is not supported.'");
	...
	Исключение
		...
		РезультирующаяСтрокаСИнформациейОбОшибке = ЗаписатьВПротоколВыполнения(СтрокаОшибки, Неопределено,
			 Истина, , ,
			 Истина);
...
Процедура ИнициализацияТаблицыПравилКонвертации()
	...
	ДобавитьНедостающиеКолонки(Колонки, "ПродолжитьПоискПоПолямПоискаЕслиПоИдентификаторуНеНашли",
		 одОписаниеТипа("Булево"));
...
Процедура ДополнитьМассивМенеджеровСсылочнымТипом(Менеджеры, МенеджерыДляПлановОбмена, ОбъектМД,
	 ИмяТипа, Менеджер, ПрефиксИмениТипа,
	 ВозможенПоискПоПредопределенным = Ложь)
...
Процедура ДополнитьМассивМенеджеровТипомРегистра(Менеджеры, ОбъектМД, ИмяТипа, Менеджер,
	 ПрефиксИмениТипаЗапись,
	...
	Если ИмяТипа = "РегистрСведений" Тогда
		...
		Периодический =
			(ОбъектМД.ПериодичностьРегистраСведений <> Метаданные.СвойстваОбъектов.ПериодичностьРегистраСведений.Непериодический);
		ПодчиненныйРегистратору =
			(ОбъектМД.РежимЗаписи = Метаданные.СвойстваОбъектов.РежимЗаписиРегистра.ПодчинениеРегистратору);
...
Процедура ИнициализацияМенеджеров()
	...
	Для каждого ОбъектМД Из Метаданные.Справочники Цикл
		...
		ДополнитьМассивМенеджеровСсылочнымТипом(Менеджеры, МенеджерыДляПлановОбмена, ОбъектМД,
			 "Справочник", Справочники[ОбъектМД.Имя], "СправочникСсылка",
			 Истина);
	...
	Для каждого ОбъектМД Из Метаданные.ПланыВидовХарактеристик Цикл
		...
		ДополнитьМассивМенеджеровСсылочнымТипом(Менеджеры, МенеджерыДляПлановОбмена, ОбъектМД,
			 "ПланВидовХарактеристик", ПланыВидовХарактеристик[ОбъектМД.Имя], "ПланВидовХарактеристикСсылка",
			 Истина);
	...
	Для каждого ОбъектМД Из Метаданные.ПланыСчетов Цикл
		...
		ДополнитьМассивМенеджеровСсылочнымТипом(Менеджеры, МенеджерыДляПлановОбмена, ОбъектМД,
			 "ПланСчетов", ПланыСчетов[ОбъектМД.Имя], "ПланСчетовСсылка",
			 Истина);
	...
	Для каждого ОбъектМД Из Метаданные.ПланыВидовРасчета Цикл
		...
		ДополнитьМассивМенеджеровСсылочнымТипом(Менеджеры, МенеджерыДляПлановОбмена, ОбъектМД,
			 "ПланВидовРасчета", ПланыВидовРасчета[ОбъектМД.Имя], "ПланВидовРасчетаСсылка",
			 Истина);
	...
	Для каждого ОбъектМД Из Метаданные.ПланыОбмена Цикл
		...
		ДополнитьМассивМенеджеровСсылочнымТипом(Менеджеры, МенеджерыДляПлановОбмена, ОбъектМД,
			 "ПланОбмена", ПланыОбмена[ОбъектМД.Имя],
			 "ПланОбменаСсылка");
	...
	Для каждого ОбъектМД Из Метаданные.БизнесПроцессы Цикл
		...
		ДополнитьМассивМенеджеровСсылочнымТипом(Менеджеры, МенеджерыДляПлановОбмена, ОбъектМД,
			 "БизнесПроцесс", БизнесПроцессы[ОбъектМД.Имя],
			 "БизнесПроцессСсылка");
	...
	Для каждого ОбъектМД Из Метаданные.РегистрыСведений Цикл
		...
		ДополнитьМассивМенеджеровТипомРегистра(Менеджеры, ОбъектМД, "РегистрСведений",
			 РегистрыСведений[ОбъектМД.Имя], "РегистрСведенийЗапись",
	...
	Для каждого ОбъектМД Из Метаданные.РегистрыБухгалтерии Цикл
		...
		ДополнитьМассивМенеджеровТипомРегистра(Менеджеры, ОбъектМД, "РегистрБухгалтерии",
			 РегистрыБухгалтерии[ОбъектМД.Имя], "РегистрБухгалтерииЗапись",
	...
	Для каждого ОбъектМД Из Метаданные.РегистрыНакопления Цикл
		...
		ДополнитьМассивМенеджеровТипомРегистра(Менеджеры, ОбъектМД, "РегистрНакопления",
			 РегистрыНакопления[ОбъектМД.Имя], "РегистрНакопленияЗапись",
	...
	Для каждого ОбъектМД Из Метаданные.РегистрыРасчета Цикл
		...
		ДополнитьМассивМенеджеровТипомРегистра(Менеджеры, ОбъектМД, "РегистрРасчета",
			 РегистрыРасчета[ОбъектМД.Имя], "РегистрРасчетаЗапись",
			 "РегистрРасчетаВыборка");
...
Процедура СоздатьСтруктуруКонвертации()
	...
	Конвертация  = Новый Структура("ПередВыгрузкойДанных, ПослеВыгрузкиДанных, ПередВыгрузкойОбъекта,
		| ПослеВыгрузкиОбъекта, ПередКонвертациейОбъекта, ПередЗагрузкойОбъекта, ПослеЗагрузкиОбъекта,
...
Функция ВыгрузитьДанныеCRM() Экспорт
	...
	РезультатВыгрузки = Новый Структура("ЕстьОшибки, ОписаниеОшибки", Ложь, "");
	
	РаботаВозможна = ВыполнитьДействияПередЧтениемДанных();
	
	Если Не РаботаВозможна Тогда
		РезультатВыгрузки.ЕстьОшибки = Истина;
		РезультатВыгрузки.ОписаниеОшибки = "(ВыполнитьДействияПередЧтениемДанных) возвращает Ложь";
	...
	Попытка
		СтрокаСОшибками = "";
		РезультатЧтения = ПроизвестиЧтениеДанных(СтрокаСОшибками);
		
		Если ЗначениеЗаполнено(СтрокаСОшибками) Тогда
			РезультатВыгрузки.ЕстьОшибки = Истина;
			РезультатВыгрузки.ОписаниеОшибки = "(ПроизвестиЧтениеДанных)" + СтрокаСОшибками;
		КонецЕсли;
	...
	Исключение
		...
		РезультатВыгрузки.ЕстьОшибки = Истина;
		РезультатВыгрузки.ОписаниеОшибки = "(ВыгрузитьДанныеCRM)" + ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()) + ";следошибка;" 
			+ РезультатВыгрузки.ОписаниеОшибки;
	...
	Возврат РезультатВыгрузки;
	// Возврат Результат; // legacy
КонецФункции

Функция ВыполнитьВыгрузкуИзмененныхДанныхДляУзлаОбменаCRM(УзелОбмена, МассивПравилКонвертации,
	 СтруктураДляУдаленияРегистрацииИзменений,
	 МассивОбъектовДляВыгрузки)
				...
				Если ПоследняяСтрокаПравилаВыгрузки <> Неопределено Тогда
					...
					Если Не ПустаяСтрока(ПоследняяСтрокаПравилаВыгрузки.ПослеОбработки) Тогда
						...
						Исключение
							...
							ЗаписатьИнформациюОбОшибкеОбработчикиПВД(32, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
								 ПоследняяСтрокаПравилаВыгрузки.Имя,
				...
				Если ТекущаяСтрокаПравилаВыгрузки <> Неопределено Тогда
					...
					Если Не ПустаяСтрока(ТекущаяСтрокаПравилаВыгрузки.ПередОбработкой) Тогда
						...
						Исключение
							...
							ЗаписатьИнформациюОбОшибкеОбработчикиПВД(31, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
								 ТекущаяСтрокаПравилаВыгрузки.Имя,
...
Функция ОбработатьВыгрузкуДляПлановОбменаCRM(СоответствиеУзловИПравилВыгрузки,
	 СтруктураДляУдаленияРегистрацииИзменений, МассивОбъектовДляВыгрузки)
	...
	Для Каждого СтрокаСоответствия Из СоответствиеУзловИПравилВыгрузки Цикл
		...
		ТекущаяУдачнаяВыгрузка = ВыполнитьВыгрузкуИзмененныхДанныхДляУзлаОбменаCRM(УзелОбмена,
			 МассивПравилКонвертации, ЛокальнаяСтруктураДляУдаленияРегистрацииИзменений,
			 МассивОбъектовДляВыгрузки);
	...
	Попытка
		...
		Если Не Отказ Тогда
			...
			Если Не Отказ Тогда
				...
				УдачноВыгруженоПоПланамОбмена = ОбработатьВыгрузкуДляПлановОбменаCRM(СоответствиеУзловИПравилВыгрузки,
					 СтруктураДляУдаленияРегистрацииИзменений, МассивОбъектовДляВыгрузки);
