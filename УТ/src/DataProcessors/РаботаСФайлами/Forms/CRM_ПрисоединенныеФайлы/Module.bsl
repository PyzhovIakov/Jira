...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Метаданные.ОпределяемыеТипы.ВладелецФайлов.Тип.СодержитТип(ТипВладельца) Тогда
		...
		Если Метаданные.Справочники.Найти(ПолноеИмяВладельца +
			РаботаСФайламиСлужебный.СуффиксСправочникаПрисоединенныеФайлы()) = Неопределено Тогда
	...
	ИмяСправочникаХранилищаВерсийФайлов =
		РаботаСФайламиСлужебный.ИмяСправочникаХраненияВерсийФайлов(Параметры.ВладелецФайла);
	...
	Если НЕ ЕстьПравоИспользоватьШаблоны
		 	 Или НастройкиФайлов.НеСоздаватьФайлыПоШаблону.Найти(Метаданные.НайтиПоТипу(ТипЗнч(ВладелецФайла))) <> Неопределено Тогда
	...
	Если ТипЗнч(Параметры.ВладелецФайла) = Тип("БизнесПроцессСсылка.CRM_БизнесПроцесс")
		И ЗначениеЗаполнено(Параметры.CRM_ТекущаяЗадача) Тогда
		Элементы.CRM_ТолькоПоТекущейЗадаче.Видимость = Истина;
	КонецЕсли;
	// -CRM
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЕстьПравоДобавления Тогда
		...
		ДоступнаКомандаСканировать = (Не ТолькоПросмотр)
			И РаботаСФайламиСлужебныйКлиент.ДоступноСканирование();
		// -CRM
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Запись_Файл" Тогда
		...
		Если Параметр.ЭтоНовый = Истина Тогда
			...
			// +CRM
			Если Параметры.CRM_ЗапрашиватьКонтекст Тогда
			...
			Если ЗначениеЗаполнено(Параметры.CRM_ТекущаяЗадача) Тогда
				ЗаполнитьЗадачуВПрисоединенномФайле(Источник);
			КонецЕсли;
			// -CRM
...
&НаКлиенте
Процедура ДекорацияДатаСинхронизацииОбработкаНавигационнойСсылки(Элемент,
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	// -CRM
		...
		РаботаСФайламиСлужебныйКлиент.ВыбратьРежимИРедактироватьФайл(Обработчик, ДанныеФайла,
			 Элементы.ФормаРедактировать.Доступность);
	// +CRM
	КонецЕсли;
	// -CRM
...
Процедура НастроитьСинхронизацию(Команда)
	...
	ОткрытьФорму("РегистрСведений.НастройкиСинхронизацииФайлов.Форма.ПростаяФормаЗаписиНастройки",
		 ПараметрыЗаписи,
		 ЭтотОбъект);
...
Процедура ЗагрузитьПапку(Команда)
	...
	// +CRM
	ОповещениеЗавершения = Новый ОписаниеОповещения("ДиалогОткрытияФайлаЗавершение", ЭтотОбъект);
...
Процедура УстановитьДоступностьФайловыхКоманд(Результат = Неопределено, ПараметрыВыполнения = Неопределено) Экспорт
	...
	Для каждого ИмяЭлементаФормы Из ИменаЭлементовКнопокФормы Цикл
		...
		ЭлементФормыДоступен = ИменаКоманд[ЭлементФормы.ИмяКоманды] = Истина Или ИменаКоманд[ЭлементФормы.Имя] = Истина;
		Если ЭлементФормы.Доступность <> ЭлементФормыДоступен Тогда
			ЭлементФормы.Доступность = ЭлементФормыДоступен;
...
Процедура НастроитьДинамическийСписок()
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ЗадачаФайла", 
					?(ИмяСправочникаХранилищаФайлов = "CRM_БизнесПроцессПрисоединенныеФайлы", 
						"Файлы.Задача КАК ЗадачаФайла", "НЕОПРЕДЕЛЕНО"));
	// -CRM
...
Процедура ПриОтправкеФайловПочтой(ПараметрыОтправки, Знач ФайлыДляОтправки, ВладелецФайлов, УникальныйИдентификатор)
	...
	РаботаСФайламиПереопределяемый.ПриОтправкеФайловПочтой(ПараметрыОтправки, ФайлыДляОтправки,
		 ВладелецФайлов, УникальныйИдентификатор);
КонецПроцедуры
...
&НаСервере
Процедура ЗаполнитьЗадачуВПрисоединенномФайле(Источник)
	
	Если ТипЗнч(Источник) = Тип("Массив") Тогда
		Для Каждого Файл Из Источник Цикл
			ФайлОбъект = Файл.ПолучитьОбъект();
			ФайлОбъект.Задача = Параметры.CRM_ТекущаяЗадача;
			ФайлОбъект.Записать();
		КонецЦикла;
	Иначе
		ФайлОбъект = Источник.ПолучитьОбъект();
...
&НаКлиенте
Процедура CRM_ТолькоПоТекущейЗадачеПриИзменении(Элемент)
	
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Список, "ЗадачаФайла",
		Параметры.CRM_ТекущаяЗадача, ВидСравненияКомпоновкиДанных.Равно, "ТолькоПоТекущейЗадаче", CRM_ТолькоПоТекущейЗадаче);
		
	CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики("CRM_Статистика.БизнесПроцессыИЗадачи.ЗадачаБизнесПроцесса.УстановкаФлагаОтборФайловПоТекущейЗадаче");
