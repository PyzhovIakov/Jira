...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ CRM_ЛицензированиеЭкспортныеМетоды.ПодсистемаCRMИспользуется() Тогда
		CRM_ЛицензированиеЭкспортныеМетоды.ВывестиУведомлениеОНедоступности(НСтр("ru = 'форму Планирования проектов'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	
	CRM_ЛицензированиеЭкспортныеМетоды.ПолучитьЗащищеннуюОбработку().
		Обработка_CRM_ПланированиеПроектов_ПриСозданииНаСервере(ЭтотОбъект, Отказ, СтандартнаяОбработка);
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(СписокЗаписейПланов.Отбор, "Проект",
	...
	Элементы.КнопкаВыводитьПодразделения.Видимость = ИспользоватьПодразделения;
...
Процедура ОтборЗначениеПодразделениеПриИзменении(Элемент)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(СписокПроектов.Отбор, "CRM_Подразделение",
		 ОтборЗначениеПодразделение, ВидСравненияКомпоновкиДанных.ВИерархии, ,
		 ЗначениеЗаполнено(ОтборЗначениеПодразделение));
...
Процедура ОтборЗначениеПартнерПриИзменении(Элемент)
	// Установим отбор в списке проектов.
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(СписокПроектов.Отбор, "CRM_Партнер",
		 ОтборЗначениеПартнер, , ,
		 ЗначениеЗаполнено(ОтборЗначениеПартнер));
	ОтборЗначениеПриИзменении(Элемент);
КонецПроцедуры
...
Процедура КомандаСписокЗаписейПлановРазбитьПоМесяцам(Команда)
	...
	Если Элементы.СписокЗаписейПланов.ВыделенныеСтроки.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ОписаниеОповещения = Новый ОписаниеОповещения("КомандаСписокЗаписейПлановРазбитьПоМесяцамЗавершение", ЭтотОбъект);
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Выбранные записи будут разбиты по месяцам,
		| продолжить?';en='Selected records will be br on months, to continue?'"),
...
Процедура КомандаСписокЗаписейПлановРазбитьПоМесяцамЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		CRM_ОбщегоНазначенияКлиентСервер.ОбновлениеТаблицыФормы(Элементы.СписокЗаписейПланов);
	КонецЕсли;
КонецПроцедуры
		...
		Попытка
			...
			Пока ТекМесяц <= НачалоМесяца(ПериодЗаписиОкончание) Цикл
				Если НачалоМесяца(ТекМесяц) = НачалоМесяца(ПериодЗаписиНачало) Тогда
					СтруктураОписаниеПериода = Обработки.CRM_ПланированиеПроектов.ПолучитьПродолжительностьПериода(ПериодЗаписиНачало,
						 КонецМесяца(ТекМесяц));
				...
				ИначеЕсли НачалоМесяца(ТекМесяц) = НачалоМесяца(ПериодЗаписиОкончание) Тогда
					СтруктураОписаниеПериода = Обработки.CRM_ПланированиеПроектов.ПолучитьПродолжительностьПериода(НачалоМесяца(ТекМесяц),
						 ПериодЗаписиОкончание);
					РабочихДнейПериоде = СтруктураОписаниеПериода.ВДнях;
					РабочихЧасовВПериоде = СтруктураОписаниеПериода.ВЧасах;
...
Процедура ПриИзмененииПериодаПланирования(ПриоритетДатыНачала = Истина)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ДатаНачала) Тогда
		Объект.ДатаНачала = НачалоМесяца(ОбщегоНазначенияКлиент.ДатаСеанса());
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Объект.ДатаКонца) Тогда
		Объект.ДатаКонца = КонецМесяца(ОбщегоНазначенияКлиент.ДатаСеанса());
	КонецЕсли;
	...
	Если КоличествоМесяцевПланирования > 12 Тогда
		ПоказатьПредупреждение(,
			 НСтр("ru='Нельзя указывать период планирования больше 1 года!';
			|en='It is impossible to specify the period of scheduling more than 1 year!'"),
			 5);
...
Процедура ДобавитьЗаголовокПланов(ДеревоЗначенийПланы)
	...
	Пока ТекМесяц <= НачалоМесяца(Объект.ДатаКонца) Цикл
		ПродолжительностьПериода = Обработки.CRM_ПланированиеПроектов.ПолучитьПродолжительностьПериода(ТекМесяц,
			 КонецМесяца(ТекМесяц),
	...
	СтрокаДереваПланов.ГруппировкаПредставление = НСтр("ru='НОРМА ВРЕМЕНИ';en='Norm of time'") + " (" 
		+ Строка(ПроизводственныйКалендарь) 
		+ ")";
...
Процедура СформироватьДиаграммуГанта(ТекСтрокаДерева)
	...
	Иначе
		...
		Пока Истина Цикл
			...
			Если ТекРодитель = Неопределено Или (флВыводитьПодразделения
				 И ТипЗнч(ТекРодитель.Группировка) = Тип("СправочникСсылка.СтруктураПредприятия")) Тогда
				Прервать;
			Иначе
	...
	ДиаграммаГанта.ОбластьЗаголовка.Расположение = РасположениеОбластиЗаголовкаДиаграммы.Нет;
...
Функция ПолучитьДанныеДиаграммыГантаДляСтроки(СтрокаПланов, СтруктураПоиска, МассивПериоды, МассивСуффиксыКолонок)
	...
	Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
		СтруктураПериод = Новый Структура("ДатаНачала,ДатаКонца,ДатаНачалаЗаписи,ДатаКонцаЗаписи,
			|Продолжительность,Трудоемкость,ПроцентЗагрузки,
			|УровеньЗагрузки");
...
Процедура ОформитьИнтервалДиаграммыГанта(Интервал, ДанныеСтроки)
	НайденныеСтроки = ТаблицаОформление.НайтиСтроки(Новый Структура("Идентификатор", "Гант" 
		+ ДанныеСтроки.УровеньЗагрузки));
	Если НайденныеСтроки.Количество() > 0 Тогда
...
&НаКлиенте
Процедура РекурсивноеФормированиеДГ(СтрокаПланов, ЗначенияПоиска, МассивПериоды,
	 МассивСуффиксыКолонок, бСтрокаИтогов = Ложь, бПодразделение = Ложь, ЗначениеТочкиРодителя = -1,
	 Знач СтруктураПоиска = Неопределено)
	Если СтрокаПланов = Неопределено Тогда
		...
		Для Каждого ТекСтрокаПланов Из ПланыДеревоЭлементы Цикл
			Если ПланыДеревоЭлементы.Индекс(ТекСтрокаПланов) = 0 Тогда
				Продолжить;
			КонецЕсли;
			РекурсивноеФормированиеДГ(ТекСтрокаПланов, ЗначенияПоиска, МассивПериоды, МассивСуффиксыКолонок,
				 бСтрокаИтогов,
				 бПодразделение);
		КонецЦикла;
	...
	Иначе
		...
		ДанныеСтроки = ПолучитьДанныеДиаграммыГантаДляСтроки(СтрокаПланов, СтруктураПоиска, МассивПериоды,
			 МассивСуффиксыКолонок);
		...
		Если ПодчиненныеСтроки.Количество() > 0 Тогда
			Для Каждого ПодчиненнаяСтрока Из ПодчиненныеСтроки Цикл
				Если Не бСтрокаИтогов И Не бПодразделение Тогда
					Если ТипЗнч(ПодчиненнаяСтрока.Группировка) = Тип("СправочникСсылка.Проекты")
						 И ЗначениеЗаполнено(ЗначенияПоиска.Проект)
						 И ПодчиненнаяСтрока.Группировка <> ЗначенияПоиска.Проект Тогда
						Продолжить;
					ИначеЕсли ТипЗнч(ПодчиненнаяСтрока.Группировка) = Тип("СправочникСсылка.CRM_ЭтапыПроектов")
						 И ЗначениеЗаполнено(ЗначенияПоиска.Этап)
						 И ПодчиненнаяСтрока.Группировка <> ЗначенияПоиска.Этап Тогда
						Продолжить;
					ИначеЕсли ТипЗнч(ПодчиненнаяСтрока.Группировка) = Тип("СправочникСсылка.Пользователи")
						 И ЗначениеЗаполнено(ЗначенияПоиска.Исполнитель)
						 И ПодчиненнаяСтрока.Группировка <> ЗначенияПоиска.Исполнитель Тогда
						Продолжить;
					КонецЕсли;
				КонецЕсли;
				
				РекурсивноеФормированиеДГ(ПодчиненнаяСтрока, ЗначенияПоиска, МассивПериоды,
					 МассивСуффиксыКолонок, бСтрокаИтогов, бПодразделение, НоваяТочка.Значение,
					 СтруктураПоиска);
...
Процедура ПерезаполнитьДанныеПланов()
	...
	бНеЗаполненКалендарьЗаПериод = Не Обработки.CRM_ПланированиеПроектов.ПолучитьПродолжительностьПериода(НачалоДня(Объект.ДатаНачала),
		 КонецДня(Объект.ДатаКонца)).РассчитаноСУчетомГрафика;
	...
	ТекстЗапроса =	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
					|	CRM_ПланыПроектов.Проект КАК Проект,
					|	CRM_ПланыПроектов.Этап КАК Этап,
					|	CRM_ПланыПроектов.Этап.РеквизитДопУпорядочивания КАК ЭтапПорядок,
					|	CRM_ПланыПроектов.Исполнитель КАК Исполнитель,
					|	CRM_ПланыПроектов.ДатаНачала КАК ДатаНачала,
					|	CRM_ПланыПроектов.ДатаКонца КАК ДатаКонца,
					|	CRM_ПланыПроектов.Продолжительность КАК Продолжительность,
					|	CRM_ПланыПроектов.ПроцентЗагрузки КАК ПроцентЗагрузки,
					|	CRM_ПланыПроектов.Трудоемкость КАК Трудоемкость,
					|	CRM_ПланыПроектов.Комментарий КАК Комментарий,
	...
	Пока Выборка.Следующий() Цикл
		ПродолжительностьПериода = Обработки.CRM_ПланированиеПроектов.ПолучитьПродолжительностьПериода(Выборка.ДатаНачала,
			 КонецДня(Выборка.ДатаКонца));
		...
		Пока ТекДатаПериода <= Выборка.ДатаКонца Цикл
			...
			ПродолжительностьПериода = Обработки.CRM_ПланированиеПроектов.ПолучитьПродолжительностьПериода(ДатаНачала_,
				 КонецДня(ДатаКонца_));
			...
			Если ТекДатаПериода >= Объект.ДатаНачала И ТекДатаПериода < Объект.ДатаКонца Тогда
				...
				Если НачалоМесяца(Объект.ДатаНачала) <= НачалоМесяца(Выборка.ДатаНачала)
					 И НачалоМесяца(ТекДатаПериода) = НачалоМесяца(Выборка.ДатаКонца) Тогда
	...
	Если бНеЗаполненКалендарьЗаПериод Тогда	
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='В выбранном периоде не заполнен календарь! Вывод без учета календаря!';
			|en='In the selected period the calendar is not filled in! Show without calendar!'"));
	ИначеЕсли Не бРассчитаноСУчетомГрафика И Не ПустаяСтрока(РасшифровкаРассчитанныхБезУчетаГрафика) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заполнен календарь за период! Вывод без учета календаря:';en='The calendar during the period is not filled in! An output without a calendar:'") 
			+ Символы.ПС 
			+ РасшифровкаРассчитанныхБезУчетаГрафика);
	КонецЕсли;
...
Процедура ЗаполнитьМесяцы()
	...
	НовыйМесяц = Месяцы.Добавить();
	НовыйМесяц.Номер = 1;
	НовыйМесяц.Наименование = "Январь";
	НовыйМесяц.Сокр = "янв";
	
	НовыйМесяц = Месяцы.Добавить();
	НовыйМесяц.Номер = 2;
	НовыйМесяц.Наименование = "Февраль";
	НовыйМесяц.Сокр = "фев";
	
	НовыйМесяц = Месяцы.Добавить();
	НовыйМесяц.Номер = 3;
	НовыйМесяц.Наименование = "Март";
	НовыйМесяц.Сокр = "март";
	
	НовыйМесяц = Месяцы.Добавить();
	НовыйМесяц.Номер = 4;
	НовыйМесяц.Наименование = "Апрель";
	НовыйМесяц.Сокр = "апр";
	
	НовыйМесяц = Месяцы.Добавить();
	НовыйМесяц.Номер = 5;
	НовыйМесяц.Наименование = "Май";
	НовыйМесяц.Сокр = "май";
	
	НовыйМесяц = Месяцы.Добавить();
	НовыйМесяц.Номер = 6;
	НовыйМесяц.Наименование = "Июнь";
	НовыйМесяц.Сокр = "июнь";
	
	НовыйМесяц = Месяцы.Добавить();
	НовыйМесяц.Номер = 7;
	НовыйМесяц.Наименование = "Июль";
	НовыйМесяц.Сокр = "июль";
	
	НовыйМесяц = Месяцы.Добавить();
	НовыйМесяц.Номер = 8;
	НовыйМесяц.Наименование = "Август";
	НовыйМесяц.Сокр = "авг";
	
	НовыйМесяц = Месяцы.Добавить();
	НовыйМесяц.Номер = 9;
	НовыйМесяц.Наименование = "Сентябрь";
	НовыйМесяц.Сокр = "сент";
	
	НовыйМесяц = Месяцы.Добавить();
	НовыйМесяц.Номер = 10;
	НовыйМесяц.Наименование = "Октябрь";
	НовыйМесяц.Сокр = "окт";
	
	НовыйМесяц = Месяцы.Добавить();
	НовыйМесяц.Номер = 11;
	НовыйМесяц.Наименование = "Ноябрь";
	НовыйМесяц.Сокр = "нояб";
	
	НовыйМесяц = Месяцы.Добавить();
	НовыйМесяц.Номер = 12;
	НовыйМесяц.Наименование = "Декабрь";
	НовыйМесяц.Сокр = "дек";
...
Процедура ЗаполнитьОформление()
	...
	Для НомерСтроки = 1 По Макет.ВысотаТаблицы Цикл
		...
		Попытка
			ЗначениеКолонкаИспользоватьШрифт = Булево(Макет.Область(НомерСтроки, КолонкаИспользоватьШрифт,
				 НомерСтроки,
				 КолонкаИспользоватьШрифт).Параметр);
		Исключение
			ЗначениеКолонкаИспользоватьШрифт = Ложь;
		КонецПопытки;
		Попытка
			ЗначениеИспользоватьЦветТекста = Булево(Макет.Область(НомерСтроки, КолонкаИспользоватьЦветТекста,
				 НомерСтроки,
				 КолонкаИспользоватьЦветТекста).Параметр);
		Исключение
			ЗначениеИспользоватьЦветТекста = Ложь;
		КонецПопытки;
		Попытка
			ЗначениеИспользоватьЦветФона = Булево(Макет.Область(НомерСтроки, КолонкаИспользоватьЦветФона,
				 НомерСтроки,
				 КолонкаИспользоватьЦветФона).Параметр);
		Исключение
...
Процедура ОбновитьУсловноеОформлениеФормы()
	...
	// СтрокаНормаВремени 
	НовыйЭлемент = ДобавитьНовыйЭлементОформления(СтруктураОформление.СтрокаНормаВремени,
		 "ПланыДеревоГруппировкаПредставление");
	...
	// Подразделение
	НовыйЭлемент = ДобавитьНовыйЭлементОформления(СтруктураОформление.ГруппировкаПодразделение,
		 "ПланыДеревоГруппировкаПредставление");
	...
	// Проект
	НовыйЭлемент = ДобавитьНовыйЭлементОформления(СтруктураОформление.ГруппировкаПроект,
		 "ПланыДеревоГруппировкаПредставление");
	...
	// Ресурс
	НовыйЭлемент = ДобавитьНовыйЭлементОформления(СтруктураОформление.ГруппировкаРесурс,
		 "ПланыДеревоГруппировкаПредставление");
	...
	// Этап
	НовыйЭлемент = ДобавитьНовыйЭлементОформления(СтруктураОформление.ГруппировкаЭтап,
		 "ПланыДеревоГруппировкаПредставление");
	...
	НовыйЭлемент = ДобавитьНовыйЭлементОформления(СтруктураОформление.ГруппировкаНезапланированныеПроекты,
		 "ПланыДеревоГруппировкаПредставление");
	ПолеОтбора = НовыйЭлемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ПолеОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ПланыДерево.ИдентификаторОформления");
	ПолеОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	...
	Пока ТекДатаПериода <= НачалоДня(Объект.ДатаКонца) Цикл
		...
		НовыйЭлемент = ДобавитьНовыйЭлементОформления(?(РежимПланирования = 1,
			 СтруктураОформление.ЭтапЗначениеВПериоде, СтруктураОформление.ГруппировкаРесурс),
			 КолонкаПериодПредставление, ,
			 ЦветЧерный);
...
Процедура СменитьПорядокКолонокЗаписейПланов()
	...
	Если РежимПланирования = 1 Тогда // по проектам
		Элементы.Переместить(Элементы.СписокЗаписейПлановИсполнитель, Элементы.СписокЗаписейПланов,
			 Элементы.СписокЗаписейПлановДатаНачала);
		Элементы.Переместить(Элементы.СписокЗаписейПлановПлановЭтап, Элементы.СписокЗаписейПланов,
			 Элементы.СписокЗаписейПлановИсполнитель);
		Элементы.Переместить(Элементы.СписокЗаписейПлановПлановПроект, Элементы.СписокЗаписейПланов,
			 Элементы.СписокЗаписейПлановПлановЭтап);
	...
	ИначеЕсли РежимПланирования = 2 Тогда // по ресурсам
		...
		Элементы.Переместить(Элементы.СписокЗаписейПлановПлановПроект, Элементы.СписокЗаписейПланов,
			 Элементы.СписокЗаписейПлановПлановЭтап);
		Элементы.Переместить(Элементы.СписокЗаписейПлановИсполнитель, Элементы.СписокЗаписейПланов,
			 Элементы.СписокЗаписейПлановПлановПроект);
		// установим сортировку
		ЭлементПорядка = СписокЗаписейПланов.Порядок.Элементы.Добавить(Тип("ЭлементПорядкаКомпоновкиДанных"));
...
Процедура ВывестиДеревоПланов()
	...
	Если НЕ ЗначениеЗаполнено(Объект.ДатаНачала) ИЛИ НЕ ЗначениеЗаполнено(Объект.ДатаКонца)
		 ИЛИ Объект.ДатаНачала > Объект.ДатаКонца Тогда
		Возврат;
	КонецЕсли;
	...
	Если НеобходимоОбновитьКолонкиДереваПланов Тогда
		...
		CRM_ЛицензированиеЭкспортныеМетоды.ПолучитьЗащищеннуюОбработку().
			Обработка_CRM_ПланированиеПроектов_ОбновитьКолонкиДереваПланов(ЭтотОбъект);
		
	КонецЕсли; // Обновление ревизитов дерева планов.
	...
	Если флВыводитьНераспланированныеРесурсы Тогда
		...
		Если НезапланированныеПроектыЭтапы.Количество() > 0 Тогда
			...
			ИмяВетки = ?(НЕ ЗначениеЗаполнено(Элементы.СписокПроектов.ТекущаяСтрока)
				 ИЛИ Элементы.СписокПроектов.ТекущаяСтрока.ЭтоГруппа, НСтр("ru='[НЕЗАПЛАНИРОВАННЫЕ ПРОЕКТЫ]';en='[UNPLANNED PROJECTS]'"), НСтр("ru='[НЕЗАПЛАНИРОВАННЫЕ ЭТАПЫ]'"));
...
&НаСервере
Процедура СформироватьДеревоПланов(ДеревоТЧ, ДеревоПланов, ИспользуемыйКалендарь = Неопределено,
	 СоответствиеКэшРабочихЧасовВПериоде = Неопределено,
	 ТрудоемкостьКалендарь = Неопределено)
	...
	Для Каждого СтрокаДереваТЧ Из ДеревоТЧ.Строки Цикл
		...
		Если СтрокиНижнегоУровня[0].Строки.Количество() = 0 Тогда
			...
			Для Каждого СтрокаНижнегоУровняТЧ Из СтрокиНижнегоУровня Цикл
				...
				ИначеЕсли РежимДанныхПланирования = "Дни" Тогда
					СтрокаДереваПланов[ИмяКолонкиМесяца] = Окр(СтрокаДереваПланов[ИмяКолонкиМесяца] 
						+ СтрокаНижнегоУровняТЧ.Трудоемкость / 8, 1, РежимОкругления.Окр15как20);
				...
				Если РежимПланирования = 2 Тогда // Ресурсы
					...
					Если РабочееВремяМесяцаЧасы = Неопределено Тогда
						РабочееВремяМесяцаЧасы = Обработки.CRM_ПланированиеПроектов.ПолучитьПродолжительностьПериода(НачалоМесяца(СтрокаНижнегоУровняТЧ.ДатаНачала),
							 КонецМесяца(СтрокаНижнегоУровняТЧ.ДатаНачала),
							 ИспользуемыйКалендарь).ВЧасах;
						СоответствиеКэшРабочихЧасовВПериоде.Вставить(НачалоМесяца(СтрокаНижнегоУровняТЧ.ДатаНачала),
							 РабочееВремяМесяцаЧасы);
					КонецЕсли;
					СтрокаДереваПланов[ИмяКолонкиМесяцаОформление] = Обработки.CRM_ПланированиеПроектов.ВычислитьЗагрузку(РабочееВремяМесяцаЧасы,
						 СтрокаНижнегоУровняТЧ.Трудоемкость);
				КонецЕсли;
			КонецЦикла;
			...
			Если РежимДанныхПланирования = "Проценты" Тогда
				Если ТрудоемкостьКалендарь = Неопределено Тогда
					...
					Пока ТекДата <= НачалоМесяца(Объект.ДатаКонца) Цикл
						...
						Если РабочееВремяМесяцаЧасы = Неопределено Тогда
							РабочееВремяМесяцаЧасы = Обработки.CRM_ПланированиеПроектов.ПолучитьПродолжительностьПериода(НачалоМесяца(ТекДата),
								 КонецМесяца(ТекДата),
								 ИспользуемыйКалендарь).ВЧасах;
		...
		Иначе
			СформироватьДеревоПланов(СтрокаДереваТЧ, СтрокаДереваПланов, ИспользуемыйКалендарь,
				 СоответствиеКэшРабочихЧасовВПериоде);
			...
			Пока ТекДата <= НачалоМесяца(Объект.ДатаКонца) Цикл
				...
				СтрокаДереваПланов[ИмяКолонкиМесяцаТрудоемкостьВЧасах] =
					СтрокаДереваПланов.Строки.Итог(ИмяКолонкиМесяцаТрудоемкостьВЧасах);
				...
				Если РежимДанныхПланирования = "Проценты" Тогда
					...
					Если РабочееВремяМесяцаЧасы = Неопределено Тогда
						...
						СоответствиеКэшРабочихЧасовВПериоде.Вставить(НачалоМесяца(ТекДата), РабочееВремяМесяцаЧасы);
					КонецЕсли;
					СтрокаДереваПланов[ИмяКолонкиМесяца] = ?(РабочееВремяМесяцаЧасы = 0, 0,
						 СтрокаДереваПланов[ИмяКолонкиМесяцаТрудоемкостьВЧасах] / РабочееВремяМесяцаЧасы * 100);
					
					ПроцентыСредДляИтоговПоСтрокам = 0;
					Если СтрокаДереваПланов.Строки.Количество() > 0 Тогда
						ПроцентыСредДляИтоговПоСтрокам =
							СтрокаДереваПланов.Строки.Итог(ИмяКолонкиМесяца) / СтрокаДереваПланов.Строки.Количество();
					КонецЕсли;
			...
			Если РежимДанныхПланирования = "Проценты" Тогда
				...
				СтрокаДереваПланов.Итоги = ?(ТрудоемкостьКалендарь = 0, 0,
					 СтрокаДереваТЧ.Трудоемкость / ТрудоемкостьКалендарь * 100);
...
Процедура УстановитьОтборыЗаписейПланов(ОтбиратьПоДате = Истина)
	...
	Если ОтбиратьПоДате Тогда
		...
		ГруппаОтбораПоПериоду_Или = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(СписокЗаписейПланов.Отбор.Элементы,
			 "ОтборПоПериоду_ИЛИ",
			 ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли);
		
		ГруппаОтбораПоПериоду_И1 = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(ГруппаОтбораПоПериоду_Или.Элементы,
			 "ОтборПоПериоду_И1",
			 ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ);
		// Используем "ДобавитьЭлементКомпоновки" вместо "УстановитьЭлементОтбора", потому что в группе 2 поля с одним именем.
		ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбораПоПериоду_И1, "ДатаНачала",
			 ВидСравненияКомпоновкиДанных.БольшеИлиРавно, Объект.ДатаНачала, ,
			 Истина);
		ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбораПоПериоду_И1, "ДатаНачала",
			 ВидСравненияКомпоновкиДанных.МеньшеИлиРавно, Объект.ДатаКонца, ,
			 Истина);
		
		ГруппаОтбораПоПериоду_И2 = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(ГруппаОтбораПоПериоду_Или.Элементы,
			 "ОтборПоПериоду_И2",
			 ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ);
		ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбораПоПериоду_И2, "ДатаКонца",
			 ВидСравненияКомпоновкиДанных.БольшеИлиРавно, Объект.ДатаНачала, ,
			 Истина);
		ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбораПоПериоду_И2, "ДатаКонца",
			 ВидСравненияКомпоновкиДанных.МеньшеИлиРавно, Объект.ДатаКонца, ,
		...
		ГруппаОтбораПоПериоду_И3 = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(ГруппаОтбораПоПериоду_Или.Элементы,
			 "ОтборПоПериоду_И3",
			 ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ);
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(ГруппаОтбораПоПериоду_И3, "ДатаНачала",
			 Объект.ДатаНачала, ВидСравненияКомпоновкиДанных.МеньшеИлиРавно, ,
			 Истина);
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(ГруппаОтбораПоПериоду_И3, "ДатаКонца",
			 Объект.ДатаКонца, ВидСравненияКомпоновкиДанных.БольшеИлиРавно, ,
			 Истина);
	...
	Если МассивВыбранных.Количество() > 1 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(СписокЗаписейПланов.Отбор, "Проект",
			 МассивВыбранных, ВидСравненияКомпоновкиДанных.ВСписке, ,
			 Истина);
	Иначе
		ТекущийПроект = Элементы.СписокПроектов.ТекущаяСтрока;
		Если ЗначениеЗаполнено(ТекущийПроект) Тогда
			Если ОбъектЯвляетсяГруппой(ТекущийПроект) Тогда
				ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(СписокЗаписейПланов.Отбор, "Проект",
					 ТекущийПроект, ВидСравненияКомпоновкиДанных.ВИерархии, ,
					 Истина);
			Иначе
				ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(СписокЗаписейПланов.Отбор, "Проект",
					 ТекущийПроект, , ,
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(СписокЗаписейПланов.Отбор, "Подразделение",
		 ОтборЗначениеПодразделение, , ,
		 ЗначениеЗаполнено(ОтборЗначениеПодразделение));
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(СписокЗаписейПланов.Отбор,
		 "Проект.CRM_Партнер", ОтборЗначениеПартнер, , ,
		 ЗначениеЗаполнено(ОтборЗначениеПартнер));
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(СписокЗаписейПланов.Отбор, "Исполнитель",
		 ОтборЗначениеИсполнитель, , ,
		 ЗначениеЗаполнено(ОтборЗначениеИсполнитель));
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(СписокЗаписейПланов.Отбор,
		 "Проект.Ответственный", ОтборЗначениеОтветственный, , ,
		 ЗначениеЗаполнено(ОтборЗначениеОтветственный));
	// отключим этап
...
Процедура УстановитьОтборыКонтрольныхТочек(ОтбиратьПоДате = Истина)
	...
	Если ОтбиратьПоДате Тогда
		ГруппаОтбораПоПериоду = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(СписокКонтрольныхТочек.Отбор.Элементы,
			 "ОтборПоПериоду",
			 ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ);
		// Используем "ДобавитьЭлементКомпоновки" вместо "УстановитьЭлементОтбора", потому что в группе 2 поля с одним именем.
		ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбораПоПериоду, "Дата",
			 ВидСравненияКомпоновкиДанных.БольшеИлиРавно, Объект.ДатаНачала, ,
			 Истина);
		ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбораПоПериоду, "Дата",
			 ВидСравненияКомпоновкиДанных.МеньшеИлиРавно, КонецДня(Объект.ДатаКонца), ,
			 Истина);
	КонецЕсли;
	...
	МассивВыбранных = Элементы.СписокПроектов.ВыделенныеСтроки;
	Если МассивВыбранных.Количество() > 1 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(СписокКонтрольныхТочек.Отбор, "Проект",
			 МассивВыбранных, ВидСравненияКомпоновкиДанных.ВСписке, ,
			 Истина);
	Иначе
		ТекущийПроект = Элементы.СписокПроектов.ТекущаяСтрока;
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(СписокКонтрольныхТочек.Отбор, "Подразделение",
		 ОтборЗначениеПодразделение, , ,
		 ЗначениеЗаполнено(ОтборЗначениеПодразделение));
...
Процедура ВыполнитьОтборыПриАктивацииВДеревеПланов()
	...
	Пока ТекРодитель <> Неопределено Цикл
		...
		ИначеЕсли ТекТипЗначения = Тип("СправочникСсылка.CRM_ЭтапыПроектов") Тогда
			ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(СписокЗаписейПланов.Отбор, "Этап",
				 ТекРодитель.Группировка, , ,
				 Истина);
			ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(СписокКонтрольныхТочек.Отбор, "Этап",
				 ТекРодитель.Группировка, , ,
				 Истина);
		...
		ИначеЕсли ТекТипЗначения = Тип("СправочникСсылка.СтруктураПредприятия") Тогда
			ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(СписокЗаписейПланов.Отбор, "Подразделение",
				 ТекРодитель.Группировка, , ,
				 Истина);
			ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(СписокКонтрольныхТочек.Отбор,
				 "Подразделение", ТекРодитель.Группировка, , ,
				 Истина);
...
Функция ОткрытьФормуЗаписиЭтапа(КлючЗаписи = Неопределено)
	...
	Попытка
		Форма = ПолучитьФорму("РегистрСведений.CRM_ПланыПроектов.ФормаЗаписи", ПараметрыСтруктура,
			 Элементы.СписокЗаписейПланов);
