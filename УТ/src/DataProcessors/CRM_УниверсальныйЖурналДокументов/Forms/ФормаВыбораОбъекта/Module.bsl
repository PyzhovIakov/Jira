...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого СтрокаСписка Из Параметры.СписокОбъектов Цикл
		...
		СтрокаСпискаНаиментование = ?(ТипЗнч(СтрокаСписка.Значение) = Тип("Строка"),
			 СтрокаСписка.Значение,
			 СтрокаСписка.Значение.Наименование);
			
		Если СтрНайти(СтрокаСпискаНаиментование, "Анкета") = 0 Тогда
			
			СписокОбъектов.Добавить(СтрокаСписка.Значение, СтрокаСписка.Представление, СтрокаСписка.Пометка,
				 СтрокаСписка.Картинка);
