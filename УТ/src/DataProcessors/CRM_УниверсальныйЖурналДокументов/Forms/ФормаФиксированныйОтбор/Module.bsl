...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Список.Параметры.УстановитьЗначениеПараметра("ДокументыБезИнтереса",
		 НСтр("ru='Документы без интереса';en='Documents without interest'"));
	...
	Если Параметры.Свойство("БлокироватьИзменениеОтбораПоКонтакту") 
		...
		КонецЕсли;
		Элементы.ОтборПартнер.Видимость			= Ложь;
		Если Параметры.Свойство("ОтборКонтакт")
			 И ТипЗнч(Параметры.ОтборКонтакт) = Тип("СправочникСсылка.КонтактныеЛицаПартнеров") Тогда
			ПризнакФормаОткрытаПоПартнеру = Истина;
		КонецЕсли;
		Элементы.СписокКонтактноеЛицо.Видимость = Ложь;
	КонецЕсли;
	
	Если Параметры.Свойство("ОтборПоИнтересу")
			И ТипЗнч(Параметры.ОтборПоИнтересу) = Тип("ДокументСсылка.CRM_Интерес") Тогда
		ОтборПоИнтересу = Параметры.ОтборПоИнтересу;
		
		CRM_ОбщегоНазначенияКлиентСервер.ИзменитьЭлементОтбораСписка(
			Список, "CRM_Интерес", Параметры.ОтборПоИнтересу, Истина);
	КонецЕсли;
	
	ПервыйВызов = Истина;
	
	ГруппироватьПоИнтересам  = CRM_ХранилищеНастроек.Загрузить("Обработка_CRM_УниверсальныйЖурналДокументов_Настройки",
		 "ГруппироватьПоИнтересам");
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(ОтборКонтактноеЛицо) Тогда
		CRM_ОбщегоНазначенияКлиентСервер.ИзменитьЭлементОтбораСписка(
			Список, "КонтактноеЛицо", ОтборКонтактноеЛицо, Истина, ВидСравненияКомпоновкиДанных.Равно);
	КонецЕсли;
	CRM_ТегиКлиент.БыстрыйОтборИзменение(ЭтотОбъект, ТекущийВидЖурнала, ОтборПоТипуОбъекта,
		 ОтборОтветственный, ?(Элементы.ОтборПартнер.Видимость, ОтборПартнер,
		 Неопределено));
...
Процедура ПриЗакрытииНаСервере()
	...
	CRM_ХранилищеНастроек.Сохранить("Обработка_CRM_УниверсальныйЖурналДокументов_Настройки",
		 "ГруппироватьПоИнтересам",
		 ГруппироватьПоИнтересам);
		...
		Если ЗначениеЗаполнено(ОтборПоТипуОбъекта) Тогда
			Если ДоступныеДляОтбораОбъекты.НайтиПоЗначению(ОтборПоТипуОбъекта) = Неопределено Тогда			
				...
				CRM_ОбщегоНазначенияКлиентСервер.ИзменитьЭлементОтбораСписка(Список, "ТипОбъекта",
					 ДоступныеДляОтбораОбъекты, Истина,
					 ВидСравненияКомпоновкиДанных.ВСписке);		
			Иначе
				CRM_ОбщегоНазначенияКлиентСервер.ИзменитьЭлементОтбораСписка(Список, "ТипОбъекта",
					  ОтборПоТипуОбъекта, Истина,
...
Процедура ОтборПартнерПриИзменении(Элемент)
	...
	CRM_ТегиКлиент.БыстрыйОтборИзменение(
		...
		ОтборПоТипуОбъекта,
		ОтборОтветственный,
		?(Элементы.ОтборПартнер.Видимость, ОтборПартнер, Неопределено));
	
	CRM_ОбщегоНазначенияКлиентСервер.ИзменитьЭлементОтбораСписка(
		Список, "ГлавнаяЗапись", Истина, ОтборПартнер <> Неопределено);
...
Процедура СписокПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если Копирование Тогда
		...
		Если ТипЗнч(ОбъектСсылка) = Тип("ЗадачаСсылка.ЗадачаИсполнителя") Тогда
			...
			Иначе
				ПоказатьПредупреждение(, НСтр("ru='Копирование задач, кроме личных,
					| запрещено!';en='Copying tasks other than personal is prohibited!'"));
			КонецЕсли;
		Иначе
			...
			Попытка
				Если ТипЗнч(ОбъектСсылка) = Тип("ДокументСсылка.CRM_Интерес") Тогда
					...
					ФормаИнтереса = ПолучитьФорму("Документ.CRM_Интерес.Форма.ФормаДокументаНовая", СтруктураПараметров);
					
					ИсточникиИнтереса = ПолучитьИсточникиПервичногоИнтереса(ОбъектСсылка, ТекДанные.Клиент);
	...
	Иначе
		...
		Иначе
			...
			ОткрытьФорму("Обработка.CRM_УниверсальныйЖурналДокументов.Форма.ФормаВыбораОбъекта",
...
Процедура СписокПередНачаломДобавленияЗавершение(ВыбранноеЗначение, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранноеЗначение <> Неопределено Тогда
		...
		Попытка
			...
			Иначе
				ОткрытьФорму(ДанныеДляОткрытияФормы.ПолноеИмяМетаданных + ".ФормаОбъекта",
					 СтруктураЗначенияЗаполнения, Элементы.Список);
			КонецЕсли;
		Исключение	
...
Процедура СписокПередУдалением(Элемент, Отказ)
	...
	КонецПопытки;
	Если ТекДанныеПометкаУдаления = Неопределено Тогда
		Возврат;
	КонецЕсли;
	Попытка
	...
	КонецПопытки;
	Если ТекДанныеОбъект = Неопределено Тогда
		Возврат;
	КонецЕсли;
	Если ТекДанныеПометкаУдаления Тогда
		ТекстВопроса = НСтр("ru='Снять с';en='Remove with'") + " """ + Строка(ТекДанные.Объект) + """ " 
			+ НСтр("ru='пометку на удаление?'");
	Иначе
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("СписокПередУдалениемЗавершение", ЭтотОбъект,
		 Новый Структура("ТекДанныеОбъект, ТекДанныеПометкаУдаления", ТекДанныеОбъект,
		 ТекДанныеПометкаУдаления));
...
Процедура СписокПередУдалениемЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	Если Ответ = КодВозвратаДиалога.Да Тогда
		ПометитьСнятьПометкуУдаленияНаСервере(ДополнительныеПараметры.ТекДанныеОбъект,
			 НЕ ДополнительныеПараметры.ТекДанныеПометкаУдаления);
		ОбновитьСписок();
	КонецЕсли;
КонецПроцедуры
...
Процедура КомандаПривязатьВзаимодействия(Команда)
	...
	Состояние(НСтр("ru='Количество новых привязанных документов взаимодействия';
		|en='Quantity of the new anchor documents of interaction'")
...
Процедура КомандаСоздатьНаОсновании(Команда)
	...
	КонецПопытки;
	Если ТекДанныеТипОбъекта = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	Если НайденныеСтроки.Количество() = 0 Тогда 
		ПоказатьПредупреждение(,
			 НСтр("ru='Для данного объекта не предусмотрен ввод на основании.';
			|en='For this object there is no input based on.'"), 5);
		Возврат;
	КонецЕсли;
	СписокВводимыхНаОснованииОбъектов = НайденныеСтроки[0].СписокВводимыхНаОснованииОбъектов;
	Если СписокВводимыхНаОснованииОбъектов.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	ДопПараметры = Новый Структура("СписокВводимыхНаОснованииОбъектов, ТекОбъект",
		 СписокВводимыхНаОснованииОбъектов,
		 ТекДанные.Объект);
	Если ТекДанные.ПометкаУдаления Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения, """" + Строка(ТекДанные.Объект) + """ " 
			+ НСтр("ru='помечен на удаление. Продолжить?';en='it is mark on deleting. Continue?'"),
...
Процедура КомандаСоздатьНаОснованииПродолжение(Ответ, ДополнительныеПараметры) Экспорт
	...
	КонецЕсли;
	СписокВводимыхНаОснованииОбъектов =
		УдалитьНедоступныеДокументыИзВводаНаОсновании(ДополнительныеПараметры.СписокВводимыхНаОснованииОбъектов);
	СтруктураПараметров = Новый Структура("СписокОбъектов,Заголовок",
		 СписокВводимыхНаОснованииОбъектов.Скопировать(),
		 НСтр("ru='Создать на основании';en='Create Based on'"));
	ОписаниеОповещения = Новый ОписаниеОповещения("КомандаСоздатьНаОснованииЗавершение", ЭтотОбъект,
		 ДополнительныеПараметры.ТекОбъект);
	ОткрытьФорму("Обработка.CRM_УниверсальныйЖурналДокументов.Форма.ФормаВыбораОбъекта",
		 СтруктураПараметров, ЭтотОбъект, , , , ОписаниеОповещения,
		 РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры // КомандаСоздатьНаОсновании()
...
Процедура УстановитьЗаголовокФормы(Форма)
	...
	Если Форма.ПризнакФормаОткрытаПоПартнеру И НЕ Форма.Элементы.ОтборПартнер.Видимость
		...
		Если ЗначениеЗаполнено(Форма.ОтборКонтактноеЛицо) Тогда
			ЗаголовокФормы = ЗаголовокФормы + " " + НСтр("ru='по контакту';en='by contact'") + " """ 
				+ Строка(Форма.ОтборКонтактноеЛицо) + """" + " " + НСтр("ru='клиента'") + " """ 
				+ Строка(Форма.ОтборПартнер) 
				+ """";
		Иначе
			ЗаголовокФормы = ЗаголовокФормы + " " + НСтр("ru='по клиенту';en='on customer'") + " """ 
				+ Строка(Форма.ОтборПартнер) 
				+ """";
		КонецЕсли;
	ИначеЕсли ЗначениеЗаполнено(Форма.ОтборПоИнтересу) Тогда
		ЗаголовокФормы = ЗаголовокФормы + " " + НСтр("ru='по интересу';en='on interest'") + " """ 
			+ Строка(Форма.ОтборПоИнтересу) 
			+ """";
	КонецЕсли;
	...
	Для Каждого ТекОтбор Из Форма.ОтборСозданы Цикл
		Если ТекОтбор.Пометка Тогда
			...
			Если ЗначениеЗаполнено(ОтборПоПериоду.ДатаНачала) ИЛИ ЗначениеЗаполнено(ОтборПоПериоду.ДатаОкончания) Тогда
				ЗаголовокФормы = ЗаголовокФормы + " (" + ПолучитьПредставлениеПериода(ОтборПоПериоду.ДатаНачала,
...
Функция ПолучитьИсточникиПервичногоИнтереса(ИнтересСсылка, ПартнерСсылка)
	...
	ИсточникиИнтереса = Новый Структура(
		"КаналПервичногоИнтереса, ИсточникПервичногоИнтереса", Неопределено, Неопределено);
...
Процедура ЗаполнитьТаблицуВводимыеНаОснованииОбъекты()
	Обработки.CRM_УниверсальныйЖурналДокументов.ЗаполнитьТаблицуВводимыеНаОснованииОбъекты(ТаблицаВводимыеНаОснованииОбъекты,
		 Истина);
КонецПроцедуры
...
Процедура ЗаполнитьДоступныеДляСозданияОбъекты()
	...
	Если ЗначениеЗаполнено(ОтборПоТипуОбъекта)
		 И ТипЗнч(ОтборПоТипуОбъекта) = Тип("ПланВидовХарактеристикСсылка.CRM_ОбъектыЖурналаДокументов") Тогда
...
Процедура ЗаполнитьДоступныеДляОтбораОбъекты()
	...
	Если ТекущийВидЖурналаЗаполнен(ТекущийВидЖурнала) Тогда
		ДоступныеДляОтбораОбъекты.ЗагрузитьЗначения(Обработки.CRM_УниверсальныйЖурналДокументов.ПолучитьСоставГруппыЖурнала(ТекущийВидЖурнала,
			 Истина));
	Иначе
...
Процедура ПровестиОтменитьПроведениеНаСервере(ОбъектСсылка, Провести)
	...
	Если НЕ ПроведениеДокументаРазрешено И НЕ Провести Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьФормуТекущегоОбъекта(Интерес = Ложь)
	...
	Если Элементы.Список.ТекущиеДанные <> Неопределено  Тогда
		...
		И НЕ ТипЗнч(СсылкаНаТекущийОбъект) = Тип("ЗадачаСсылка.ЗадачаИсполнителя")) И НЕ МожноОткрытьОбъект(СсылкаНаТекущийОбъект) Тогда
			...
			Возврат;
		КонецЕсли;	
		Если ТипЗнч(СсылкаНаТекущийОбъект) = Тип("Строка") Тогда
			Возврат;
		КонецЕсли;
		...
		Попытка
			Если ТипЗнч(Элементы.Список.ТекущиеДанные.ГруппировкаСтроки) = Тип("СтрокаГруппировкиДинамическогоСписка") Тогда
				СсылкаНаТекущийОбъект =
					Элементы.Список.ТекущиеДанные[Элементы.Список.ТекущиеДанные.ГруппировкаСтроки.ИмяГруппировки];
			Иначе
				СсылкаНаТекущийОбъект = Элементы.Список.ТекущиеДанные.Объект;
			КонецЕсли;
		Исключение
...
Процедура УстановитьОтборБезИнтересов()
	Если ТипИнтерес <> Неопределено Тогда
		ЭлементОтбора =
			Список.КомпоновщикНастроек.ФиксированныеНастройки.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
