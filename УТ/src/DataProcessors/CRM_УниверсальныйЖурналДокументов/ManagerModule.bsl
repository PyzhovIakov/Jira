...
Процедура ЗаполнитьТаблицуВводимыеНаОснованииОбъекты(ТаблицаВводимыеНаОснованииОбъекты, ПроверятьПрава = Ложь) Экспорт
	...
	Для Каждого МетаданныеВид Из МассивВидов Цикл
		Для Каждого МетаданныеОбъект Из Метаданные[МетаданныеВид] Цикл
			...
			Если МетаданныеОбъектПолноеИмя = "Документ.Встреча"
				 ИЛИ МетаданныеОбъектПолноеИмя = "Документ.ЗапланированноеВзаимодействие" Тогда Продолжить; КонецЕсли;
			Для Каждого ОбъектВводитсяНаОсновании Из МетаданныеОбъект.ВводитсяНаОсновании Цикл
				...
				Если СсылочныйТипОбъекта = Неопределено Тогда
					Продолжить;
				КонецЕсли;
				Если Не Метаданные.ПланыВидовХарактеристик.CRM_ОбъектыЖурналаДокументов.Тип.СодержитТип(СсылочныйТипОбъекта) Тогда
					Продолжить;
				КонецЕсли;
				
				СсылкаПВХ = СоответствиеМетаданныхСсылкамНаПВХОбъектыПоКлиенту[ОбъектВводитсяНаОсновании];
				Если СсылкаПВХ = ПланыВидовХарактеристик.CRM_ОбъектыЖурналаДокументов.ПустаяСсылка() Тогда
					Продолжить;
				КонецЕсли;
				...
				Если СсылкаПВХ = Неопределено Тогда
					...
					Если СсылкаПВХ = Неопределено Тогда
						СоответствиеМетаданныхСсылкамНаПВХОбъектыПоКлиенту.Вставить(ОбъектВводитсяНаОсновании,
							 ПланыВидовХарактеристик.CRM_ОбъектыЖурналаДокументов.ПустаяСсылка());
						Продолжить;
					Иначе
