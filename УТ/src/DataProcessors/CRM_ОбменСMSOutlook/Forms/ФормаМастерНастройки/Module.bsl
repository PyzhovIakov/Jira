...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ CRM_ЛицензированиеЭкспортныеМетоды.ПодсистемаCRMИспользуется() Тогда
		CRM_ЛицензированиеЭкспортныеМетоды.ВывестиУведомлениеОНедоступности(НСтр("ru = 'форму Обмена'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	
	CRM_ЛицензированиеЭкспортныеМетоды.ПолучитьЗащищеннуюОбработку().
		Обработка_CRM_ОбменСMSOutlook_ПриСозданииНаСервере(ЭтотОбъект, Отказ, СтандартнаяОбработка);
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если НЕ (НеЗадаватьВопросПриЗакрытииФормы = Истина) Тогда
		...
		ПоказатьВопрос(ОбратныйВызов,
			 НСтр("ru='Работа мастера не была завершена. Сохранить измененные настройки?';
			|en='The work of the wizard was not completed. Do you want to save the changed settings?'"),
...
Процедура НадписьОткрытьСписокКонтатковOutlookНажатие(Элемент)
	...
	мСобытияOutlook = CRM_ОбменСMSOutlookКлиент.ПолучитьСинхронизируемыеКонтактыOutlook(Конфигурация.СтруктураСвойств,
	...
	СписокВыбора.ПоказатьВыборЭлемента(ПустоеОповещениеВыбора,
		 НСтр("ru='Список синхронизируемых контактов Outlook';
		|en='Outlook Contacts Synchronization List'"));
КонецПроцедуры
...
Процедура НадписьОткрытьСписокСобытийOutlookНажатие(Элемент)
	СписокВыбора = Новый СписокЗначений;
	Конфигурация = ПолучитьКонфигурациюОбмена();
	мСобытияOutlook = CRM_ОбменСMSOutlookКлиент.ПолучитьСинхронизируемыеСобытияOutlook(Конфигурация.СтруктураСвойств,
		 Конфигурация.Настройка);
	СписокВыбора = Новый СписокЗначений;
	Для Каждого Событие Из мСобытияOutlook Цикл
		СписокВыбора.Добавить(Событие.Представление_Outlook);
	...
	ПустоеОповещениеВыбора = Новый ОписаниеОповещения("ПустойОбработчикВыбораИзСписка", ЭтотОбъект, Новый Структура);
	СписокВыбора.ПоказатьВыборЭлемента(ПустоеОповещениеВыбора,
		 НСтр("ru='Список синхронизируемых событий Outlook';
		|en='Outlook Event Synchronization List'"));
КонецПроцедуры
...
Процедура ДекорацияНастроитьСинхронизациюКонтактнойИнформацииНажатие(Элемент)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ДекорацияНастроитьСинхронизациюКонтактнойИнформацииНажатиеЗавершение",
		 ЭтотОбъект);
...
&НаКлиенте
Процедура ДекорацияНастроитьСинхронизациюКонтактнойИнформацииНажатиеЗавершение(СсылкаНаХранилищеЗначения,
...
Процедура КомандаДалее(Команда)
	...
	Если Элементы.ГруппаПанель.ТекущаяСтраница = Элементы.СтраницаЗавершение Тогда
		НеЗадаватьВопросПриЗакрытииФормы = Истина;
		ОбновитьТаблицуПапокOutlook(ДеревоПапокOutlook);
		Если НЕ CRM_ОбщегоНазначенияСервер.СсылкаСуществует(Объект.УчетнаяЗаписьПочтыДляСинхронизации)
			 ИЛИ НЕ Объект.СинхронизироватьПисьма Тогда
			Объект.УчетнаяЗаписьПочтыДляСинхронизации =
				ПредопределенноеЗначение("Справочник.УчетныеЗаписиЭлектроннойПочты.ПустаяСсылка");
		КонецЕсли;
	...
	Если Индекс < Предел - 1 Тогда
		...
		Пока Истина Цикл
			Индекс = Индекс + 1;
			Если Элементы.ГруппаПанель.ПодчиненныеЭлементы[Индекс].Видимость
				 И Элементы.ГруппаПанель.ПодчиненныеЭлементы[Индекс].Доступность Тогда
...
Процедура УправлениеВидимостьюДоступностьюЭлементов()
	...
	Иначе	
		Элементы.СинхронизироватьКонтактыСобытияOutlook.Видимость	=
			CRM_ОбменСMSOutlookКлиент.OutlookМеньше2013(OutlookObj.Application.Version);
	КонецЕсли;	
...
Процедура ИзментьИмяКомпьютера(ПрограммаMSOutlook, ИмяКомпьютера)
...
Функция ПолучитьПервые10ОбъектовСинхронизации(ПрограммаOutlook)
	...
	|	CRM_СоответствиеОбъектовOutlook.ПрограммаOutlook КАК ПрограммаOutlook,
	|	CRM_СоответствиеОбъектовOutlook.СобственнаяСсылка КАК СобственнаяСсылка,
	|	CRM_СоответствиеОбъектовOutlook.СсылкаВOutlook КАК СсылкаВOutlook
	|ИЗ
...
&НаКлиенте
Процедура УдалитьИзБазыУдаленныеMSOutlook(ПрограммаOutlook)
...
Функция ЗаполнитьУчетнуюЗаписьПочтыДляСинхронизации()
	...
	Если РезультатЗапроса.Пустой() Тогда // Создать новую учетную запись для данной синхронизации.
		...
		УчетнаяЗаписьОбъект.Наименование = НСтр("ru='MS Outlook (';en='MS Outlook ('") 
			+ ?(СтрДлина(НаименованиеПрограммы) > 37, Лев(НаименованиеПрограммы, 34), НаименованиеПрограммы) 
			+ "...)";
...
Функция НайтиСтроки(мТаблица, СтрПоиска = Неопределено, Ошибка = "")
	...
	Для Каждого элОтбора Из СтрПоиска Цикл
		Если Не мТаблица[0].Свойство(элОтбора.Ключ) Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Поля %1 не существует';
				|en='Field %1 not exist'"),
				 элОтбора.Ключ);
...
Процедура ПолучитьOutlookObj(ТекстОшибки, ОписаниеОповещенияОЗавершении)
	...
	Если ВРег(ИмяКомпьютера()) <> ВРег(ИмяКомпьютераОбъекта) Тогда
		...
		СписокКнопки.Добавить("ИзменилосьРасположение",
			 НСтр("ru='Изменилось расположение MS Outlook';
			|en='MS Outlook were reposition'"));
		СписокКнопки.Добавить("1СПредприятиеЗапущеноВТерминале",
			 НСтр("ru='1С:Предприятие запущено в терминале';
			|en='1С:Enterprise is running in the terminal mode'"));
		СписокКнопки.Добавить("Отмена", НСтр("ru='Отмена';en='Cancel'"));
		ОписаниеОповещения = Новый ОписаниеОповещения("ПолучитьOutlookObjОтветКомп", ЭтотОбъект,
...
Процедура ПолучитьOutlookObjОтветКомп(Ответ, ОписаниеОповещенияОЗавершении) Экспорт
	...
	ИначеЕсли Ответ = "Отмена" Тогда
		...
		ВыполнитьОбработкуОповещения(ОписаниеОповещенияОЗавершении, Новый Структура("Результат,
			| ТекстОшибки", Ложь,
			 ТекстОшибки));
		ОтказТемп = Истина;
		Возврат;
	...
	Если ЗначениеЗаполнено(Объект.ПрограммаOutlook) И Не ЭтоНоваяНастройка Тогда
		Если ПроверкаПрограммыMSOutlook(Объект.ПрограммаOutlook)  Тогда
			...
			ОписаниеОповещения = Новый ОписаниеОповещения("ПолучитьOutlookObjОтветПродолжить", ЭтотОбъект,
				 ОписаниеОповещенияОЗавершении);
...
Процедура ПолучитьOutlookObjОтветПродолжить(Ответ, ОписаниеОповещенияОЗавершении) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ПолучитьOutlookObjОтветЗавершение", ЭтотОбъект,
			 ОписаниеОповещенияОЗавершении);
...
Процедура ЗаполнитьНачальныеНастройки()
	...
	Для НомСтр = 3 По Макет.ВысотаТаблицы Цикл
		...
		Если  НЕ ПринадлежностьКИ1С = "" Тогда
			...
			Результат = CRM_ОбщегоНазначенияСервер.ВыполнитьЗапрос(Запрос);
			...
			Пока Выборка.Следующий() Цикл
				...
				МассивЗанятыхВКИ = Объект.СоответствиеКонтактнойИнформации.НайтиСтроки(Новый Структура("ВидКИOutlook,
					|ВидКонтакта", ВидКИOutlook,
					 ВидКонтакта));
...
Процедура ПроверитьСкорректироватьТаблицуСоответствий(Таб)
	...
	Пока Ном > 0 Цикл
		...
		ИначеЕсли ЗначениеЗаполнено(Таб[Ном - 1].ВидКИ1С) Тогда
			Если (Таб[Ном - 1].Тип = Перечисления.ТипыКонтактнойИнформации.Факс)
				 И НЕ (Таб[Ном - 1].ВидКИOutlook = "BusinessFaxNumber") Тогда
...
Процедура ЗагрузитьОбщиеНастройкиОбмена(СообщатьЕслиНастройкаНеНайдена = Ложь)
	...
	Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
	               |	CRM_НастройкиОбменаСOutlook.ПрограммаOutlook КАК ПрограммаOutlook,
	               |	CRM_НастройкиОбменаСOutlook.Наименование КАК Наименование,
	               |	CRM_НастройкиОбменаСOutlook.Настройка КАК Настройка
	               |ИЗ
