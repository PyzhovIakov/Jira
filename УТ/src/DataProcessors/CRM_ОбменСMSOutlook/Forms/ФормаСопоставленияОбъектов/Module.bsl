...
Процедура ПриОткрытии(Отказ)
	...
	CRM_ОбщегоНазначенияКлиентСервер.ОбновлениеТаблицыФормы(Элементы.ТаблицаКЛК);
	CRM_ОбщегоНазначенияКлиентСервер.ОбновлениеТаблицыФормы(Элементы.ТаблицаК);
	CRM_ОбменСMSOutlookКлиент.ВосстановитьТаблицуCOMОбъектов(мСтруктураСвойств.OutlookObj,
...
Процедура СобытияПредставление_OutlookНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
	...
	КонецЦикла;
	ОписаниеОповещения = Новый ОписаниеОповещения("СобытияПредставление_OutlookНачалоВыбораИзСпискаЗавершение",
		 ЭтотОбъект,
		 ТекущиеДанные);
...
Процедура ПисьмаПредставление_OutlookНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
	...
	Для Каждого ЭлементМассива Из мСтруктураСвойств.ТаблицаПисемOutlook Цикл
		СписокДляВыбора.Добавить(ЭлементМассива, ЭлементМассива.Представление_Outlook);
	КонецЦикла;
	ОписаниеОповещения = Новый ОписаниеОповещения("ПисьмаПредставление_OutlookНачалоВыбораИзСпискаЗавершение",
		 ЭтотОбъект,
		 ТекущиеДанные);
	ПоказатьВыборИзСписка(ОписаниеОповещения, СписокДляВыбора, Элемент);
КонецПроцедуры
...
Процедура КомандаПродолжить(Команда)
	...
	Попытка
		...
		CRM_ОбменСMSOutlookКлиент.СохранитьДатуСинхронизацииДляТекущейБазыOutlook(мНастройка.ПрограммаOutlook,
			 мСтруктураСвойств);
	Исключение
	КонецПопытки;
...
Процедура ВосстановитьCOMОбъекты()
	...
	CRM_ОбменСMSOutlookКлиент.ВосстановитьТаблицуCOMОбъектов(мСтруктураСвойств.OutlookObj,
		 мСтруктураСвойств.СОМОбъектыКонтактов,
		 мСтруктураСвойств.СоответствиеСОМОбъектыКонтактов);
	
	CRM_ОбменСMSOutlookКлиент.ВосстановитьТаблицуОбъектовOutlook(мСтруктураСвойств.OutlookObj,
		 мСтруктураСвойств.ТаблицаКонтактовOutlook);
	
	CRM_ОбменСMSOutlookКлиент.ВосстановитьТаблицуCOMОбъектов(мСтруктураСвойств.OutlookObj,
		 мСтруктураСвойств.СОМОбъектыСобытий,
		 мСтруктураСвойств.СоответствиеСОМОбъектыСобытий);
	
	CRM_ОбменСMSOutlookКлиент.ВосстановитьТаблицуОбъектовOutlook(мСтруктураСвойств.OutlookObj,
		 мСтруктураСвойств.ТаблицаСобытийOutlook);
	
	CRM_ОбменСMSOutlookКлиент.ВосстановитьТаблицуCOMОбъектов(мСтруктураСвойств.OutlookObj,
		 мСтруктураСвойств.СОМОбъектыПисем,
		 мСтруктураСвойств.СоответствиеСОМОбъектыПисем);
