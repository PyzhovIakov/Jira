		...
		Если стрДанныеЗаполнения.Свойство("Объект") Тогда
			Если ЗначениеЗаполнено(стрДанныеЗаполнения.Объект) Тогда
				Если ТипЗнч(стрДанныеЗаполнения.Объект) = Тип("Строка") Тогда
					...
					ПереданныйОбъектАдресВоВременномХранилище = ПоместитьВоВременноеХранилище(Новый Структура("ОбъектЗначение",
						 ПереданныйОбъект),
			...
			Если CRM_ШаблоныОбъектов.ПроверитьСуществованиеРеквизита("Тема", ВыгружаемыйОбъект)
				 И ПустаяСтрока(Объект.Наименование) Тогда
				Объект.Наименование = ВыгружаемыйОбъект.Тема;
				Если Не ЗначениеЗаполнено(Объект.Наименование) И ПереданныйОбъект <> Неопределено
					И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ПереданныйОбъект, "Тема") Тогда
					Объект.Наименование = ПереданныйОбъект.Тема;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Не ЕстьПометки(СписокРеквизитов) Тогда
		...
		Сообщение.Текст = Нстр("ru='Необходимо отметить хотя бы один реквизит!';
			|en='It is necessary to mark at least one attribute!'");
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура ДеревоРеквизитовЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если (ТекущиеДанные.ТипРеквизита.Типы().Количество() = 1)
		 И (ТекущиеДанные.ТипРеквизита.Типы()[0] = Тип("Строка")) Тогда
...
"Данные будут записаны.'");
		ОписаниеОповещения = Новый ОписаниеОповещения("СписокПередНачаломДобавленияЗавершение",
			 ЭтотОбъект,
...
Процедура ЗавершениеВыбораГруппыПользователей(ГруппаПользователей, ВключаяВложенные) Экспорт
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ЗаполнитьПоГруппеПользователейЗавершение",
		 ЭтотОбъект, Новый Структура("ВключаяВложенные, ГруппаПользователей", ВключаяВложенные,
		 ГруппаПользователей));
...
Процедура ЗаполнитьПоГруппеПользователейЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	Если ДополнительныеПараметры.ВключаяВложенные Тогда
		...
		КонецЕсли;			
	КонецЕсли;	
	мСписокПользователей = ПолучитьСписокПользователей(ДополнительныеПараметры.ГруппаПользователей,
		 ДополнительныеПараметры.ВключаяВложенные);
	ЗаполнитьСписокПользователей(мСписокПользователей);
КонецПроцедуры
...
Функция ПроверитьНаПрава()
	...
	Возврат (Пользователи.РолиДоступны("CRM_СозданиеОбщихШаблоновДокументов, ПолныеПрава")
...
Процедура ЗаполнитьСписок()
	...
	КонецЕсли;
	CRM_ЕстьИдентификатор =
		Метаданные.Справочники[ИмяСправочникаХранилищаФайлов].Реквизиты.Найти("ИДФайлаЭлектронногоПисьма");
...
Процедура ПолучитьВладельцевШаблона()
	...
	Если НЕ ЭтоНовыйШаблон Тогда
		...
		Для Каждого ЗаписьНабора Из НаборЗаписей Цикл
			Если НЕ ЗначениеЗаполнено(ЗаписьНабора.Владелец) Тогда
				Продолжить;
			КонецЕсли;
			НоваяСтрока = СписокПользователей.Добавить();
...
Процедура ЗаполнитьДеревоРеквизитов(ПереданныйОбъект = Неопределено)
	...
	Если (ЗначениеЗаполнено(ВыгружаемыйОбъект)
		 И НЕ ТипЗнч(ВыгружаемыйОбъект) = Тип("СправочникСсылка.CRM_ШаблоныОбъектов"))
		 ИЛИ (НЕ ПереданныйОбъект = Неопределено) Тогда
		ДеревоМетаданных = CRM_ШаблоныОбъектов.ПолучитьДеревоРеквизитов(?(ПереданныйОбъект = Неопределено,
			 ВыгружаемыйОбъект,
			 ПереданныйОбъект));
	Иначе
...
Процедура ЗаписатьШаблон(Владелец)
	...
	Иначе
		Если Не ЗначениеЗаполнено(ВыгружаемыйОбъект)
			 И ЭтоАдресВременногоХранилища(ПереданныйОбъектАдресВоВременномХранилище) Тогда
			ТекстШаблона = CRM_ШаблоныОбъектов.ВыгрузитьРеквизитыДокументаВШаблон(ПереданныйОбъектАдресВоВременномХранилище,
				 ДеревоМетаданных);
		Иначе
...
Процедура УстановитьСвойстваЭлементовФормы()
	...
	Элементы.ГруппаСтраниц.ОтображениеСтраниц	= ?((ЭтоОбщийШаблон И ЕстьПраво),
		 ОтображениеСтраницФормы.Нет, ОтображениеСтраницФормы.ЗакладкиСверху);
