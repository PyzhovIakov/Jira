...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.Список.МножественныйВыбор = ?(Параметры.ЗакрыватьПриВыборе = Неопределено, Ложь,
...
Процедура УстановитьДоступныеКалендари()
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь() Или Параметры.ТолькоСвои Тогда
		ДоступныеКалендари =
			Справочники.CRM_КалендариСотрудников.ДоступныеПользователюКалендари().ВыгрузитьКолонку("Календарь");
