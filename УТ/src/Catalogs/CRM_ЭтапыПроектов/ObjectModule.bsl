...
Процедура ОпределитьДатыПакетаЗадач() Экспорт
	...
	УчитыватьКалендарь = Владелец.CRM_УчитыватьПроизводственныйКалендарь;
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		...
		Если ЗначениеЗаполнено(СуммарнаяДатаОкончанияПлановая) Тогда
			СуммарнаяДлительностьПлановая = CRM_УправлениеПроектамиСервер.ПолучитьПродолжительностьПериода(
				СуммарнаяДатаНачалаПлановая, СуммарнаяДатаОкончанияПлановая, УчитыватьКалендарь);
		Иначе
			СуммарнаяДлительностьПлановая = 0;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(СуммарнаяДатаОкончанияФакт) Тогда
			СуммарнаяДлительностьФакт = CRM_УправлениеПроектамиСервер.ПолучитьПродолжительностьПериода(
				СуммарнаяДатаНачалаФакт, СуммарнаяДатаОкончанияФакт, УчитыватьКалендарь);
		Иначе
