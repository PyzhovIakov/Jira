...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РазделительИменПапок = "/";
	ПредопределенныеПапки = CRM_УправлениеЭлектроннойПочтой.ИменаПредопределенныхПапок();
	ПолученныеПапки = Новый ФиксированныйМассив(Новый Массив);
	Элементы.ИспользоватьУчетнуюЗапись.Видимость = Ложь;
	ЗагружатьВложенияПриПолученииПисем = Истина;
	Элементы.CRM_Логотип.Видимость = Не Перенастройка;
	// -CRM
...
Процедура ПарольПриИзменении(Элемент)
	ПарольДляПолученияПисем = УдалитьЛишниеСимволы(ПарольДляПолученияПисем);
...
Процедура АдресЭлектроннойПочтыПриИзменении(Элемент)
	...
	АдресЭлектроннойПочты = УдалитьЛишниеСимволы(АдресЭлектроннойПочты);
КонецПроцедуры
...
Процедура ПапкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ВидПапки = СтрЗаменить(Элемент.Имя, "Папка", "");
	
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("ВидПапки", ВидПапки);
	
	ОповещениеЗавершения = Новый ОписаниеОповещения("ПапкаНачалоВыбораЗавершение", ЭтотОбъект, ДополнительныеПараметры);
	
	СписокВыбора = Новый СписокЗначений;
	СписокВыбора.ЗагрузитьЗначения(Новый Массив(ПолученныеПапки));
	
	СписокВыбора.Добавить("СоздатьПапку", НСтр("ru = 'Создать папку'"), , БиблиотекаКартинок.СоздатьЭлементСписка);
	
	ПоказатьВыборИзМеню(ОповещениеЗавершения, СписокВыбора, Элемент);
	
КонецПроцедуры

&НаКлиенте
Процедура ПапкаНачалоВыбораЗавершение(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	
	Если ВыбранныйЭлемент = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ОповещениеЗавершения = Новый ОписаниеОповещения("ПапкаНачалоВыбораПослеВводаСтроки", ЭтотОбъект,
		 ДополнительныеПараметры);
	
	Если ВыбранныйЭлемент.Значение = "СоздатьПапку" Тогда
		ТекстПодсказки = НСтр("ru = 'Укажите имя папки'");
		ПоказатьВводСтроки(ОповещениеЗавершения, "", ТекстПодсказки, 25, Ложь);
	Иначе
		ВыполнитьОбработкуОповещения(ОповещениеЗавершения, ВыбранныйЭлемент.Значение);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ПапкаНачалоВыбораПослеВводаСтроки(Строка, ДополнительныеПараметры) Экспорт
	
	Если Не ЗначениеЗаполнено(Строка) Тогда
		Возврат;
	КонецЕсли;
	
	ВидПапки = ДополнительныеПараметры.ВидПапки;
	ЭтотОбъект["Папка" + ВидПапки] = Строка;
	
	Если ЗначениеЗаполнено(ПапкаВходящие) И ЗначениеЗаполнено(ПапкаОбработанные) И ЗначениеЗаполнено(ПапкаИсходящие)
		И ЗначениеЗаполнено(ПапкаОтправленные) И ЗначениеЗаполнено(ПапкаУдаленные) И ЗначениеЗаполнено(ПапкаЧерновики)
		И ЗначениеЗаполнено(ПапкаНежелательные) Тогда
		ОпределеныПапкиIMAP = Истина;
	Иначе
		ОпределеныПапкиIMAP = Ложь;
	КонецЕсли;
...
Процедура СерверВходящейПочтыПриИзменении(Элемент)
	СерверВходящейПочты = УдалитьЛишниеСимволы(СерверВходящейПочты);
КонецПроцедуры

&НаКлиенте
Процедура СерверИсходящейПочтыПриИзменении(Элемент)
	СерверИсходящейПочты = УдалитьЛишниеСимволы(СерверИсходящейПочты);
КонецПроцедуры
...
Процедура Назад(Команда)
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ПроверкаНастроекУчетнойЗаписи
		 Или ТекущаяСтраница = Элементы.ТехническиеПодробностиОшибки Тогда
		ПредыдущаяСтраница = Элементы.НастройкаУчетнойЗаписи;
	...
	// +CRM
	ИначеЕсли ТекущаяСтраница = Элементы.НастройкаПапок Тогда
		ПредыдущаяСтраница = Элементы.НастройкаУчетнойЗаписи;
	// -CRM
	КонецЕсли;
...
Процедура ЗаполнитьСвойстваУчетнойЗаписи()
	...
	"ВЫБРАТЬ
// +CRM
	|	УчетныеЗаписиЭлектроннойПочты.Пользователь КАК ИмяПользователяДляПолученияПисем,
	|	УчетныеЗаписиЭлектроннойПочты.ПользовательSMTP КАК ИмяПользователяДляОтправкиПисем,
	|	УчетныеЗаписиЭлектроннойПочты.ПротоколВходящейПочты КАК Протокол,
	|	УчетныеЗаписиЭлектроннойПочты.ПортСервераВходящейПочты КАК ПортСервераВходящейПочты,
	|	УчетныеЗаписиЭлектроннойПочты.ПортСервераИсходящейПочты КАК ПортСервераИсходящейПочты,
	|	УчетныеЗаписиЭлектроннойПочты.СерверВходящейПочты КАК СерверВходящейПочты,
	|	УчетныеЗаписиЭлектроннойПочты.СерверИсходящейПочты КАК СерверИсходящейПочты,
	|	УчетныеЗаписиЭлектроннойПочты.ВремяОжидания КАК ДлительностьОжиданияСервера,
	|	УчетныеЗаписиЭлектроннойПочты.ИспользоватьЗащищенноеСоединениеДляВходящейПочты КАК ИспользоватьЗащищенноеСоединениеДляВходящейПочты,
	|	УчетныеЗаписиЭлектроннойПочты.ИспользоватьЗащищенноеСоединениеДляИсходящейПочты КАК ИспользоватьЗащищенноеСоединениеДляИсходящейПочты,
	|	ВЫБОР КОГДА УчетныеЗаписиЭлектроннойПочты.ИспользоватьЗащищенноеСоединениеДляВходящейПочты ТОГДА ""SSL"" ИНАЧЕ ""Авто"" КОНЕЦ КАК ШифрованиеПриПолученииПочты,
	|	ВЫБОР КОГДА УчетныеЗаписиЭлектроннойПочты.ИспользоватьЗащищенноеСоединениеДляИсходящейПочты ТОГДА ""SSL"" ИНАЧЕ ""Авто"" КОНЕЦ ШифрованиеПриОтправкеПочты,
	|	УчетныеЗаписиЭлектроннойПочты.ТребуетсяВходНаСерверПередОтправкой КАК ТребуетсяВходНаСерверПередОтправкой,
// -CRM
...
Процедура ПерейтиНаСледующуюСтраницу(Команда = Неопределено)
	...
	Если ТекущаяСтраница = Элементы.НастройкаУчетнойЗаписи
		...
		Если Не Отказ И ВариантАутентификации = "OAuth" Тогда
			...
			Если Не ЗначениеЗаполнено(АдресПеренаправления) 
				...
				СообщенияОбОшибках =
					НСтр("ru = 'Не найдены настройки авторизации почтового сервиса. Используйте авторизацию по паролю.'");
	...
	ИначеЕсли ТекущаяСтраница = Элементы.Авторизация Тогда
		Если ЭтоВебКлиент() И Не ЗначениеЗаполнено(НастройкиАвторизации.АдресРегистрацииУстройства) Тогда
			Если Не ПолучитьКлючиДоступаКПочтовомуСерверу(КодПодтверждения,
				 НастройкиАвторизации.АдресПолученияКлюча,
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ПроверкаНастроекУчетнойЗаписи Тогда
		Если Команда = Неопределено Тогда
			...
			Иначе
				...
				Если ОпределеныПапкиIMAP Тогда
					СледующаяСтраница = Элементы.УчетнаяЗаписьУспешноНастроена;
				Иначе
					СледующаяСтраница = Элементы.НастройкаПапок;
				КонецЕсли;
				// -CRM
			КонецЕсли;
...
Процедура ПроверитьЗаполнениеНаСтраницеНастройкаУчетнойЗаписи(Отказ)
	...
	Если ВидУчетнойЗаписи = "Персональная" Тогда
		Если Не ЗначениеЗаполнено(CRM_Пользователь) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Необходимо указать пользователя';
				|en='It is necessary to specify user'"), , "CRM_Пользователь", ,
				 Отказ);
		КонецЕсли;
	ИначеЕсли ВидУчетнойЗаписи = "Групповая" Тогда
		Если CRM_Пользователи.Количество() = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Необходимо указать пользователей';
				|en='It is necessary to specify users'"), , "CRM_Пользователи", ,
				 Отказ);
		КонецЕсли;
	КонецЕсли;
	// -CRM
...
Процедура НастроитьЭлементыТекущейСтраницы()
	...
	Элементы.КнопкаДалее.Видимость = Не (ТекущаяСтраница = Элементы.ПроверкаНастроекУчетнойЗаписи
		 И СпособНастройки = "Вручную")
...
Функция НастройкиПоУмолчанию(АдресЭлектроннойПочты, Пароль)
	...
	НастройкиIMAPПоУмолчанию =
		Справочники.УчетныеЗаписиЭлектроннойПочты.ВариантыНастройкиПодключенияКСерверуIMAP(АдресЭлектроннойПочты)[0];
	НастройкиSMTPПоУмолчанию =
		Справочники.УчетныеЗаписиЭлектроннойПочты.ВариантыНастройкиПодключенияКСерверуSMTP(АдресЭлектроннойПочты)[0];
...
Процедура ПроверитьНастройкиУчетнойЗаписи()
	...
	ОпределитьПапкиIMAP(ПрофильВходящейПочты);
	Если ОпределеныПапкиIMAP И Не ПроверкаЗавершиласьСОшибками Тогда
	// -CRM
		Попытка
			СоздатьУчетнуюЗапись();
		Исключение
...
Процедура СоздатьУчетнуюЗапись()
	...
	Попытка
		...
		УчетнаяЗапись.ПериодХраненияСообщенийНаСервере = ?(ОставлятьКопииПисемНаСервере
			 И УдалятьПисьмаССервера И Протокол = "POP", ПериодХраненияСообщенийНаСервере,
			 0);
		...
		ИначеЕсли ВидУчетнойЗаписи = "Групповая" Тогда
			...
			УчетнаяЗапись.CRM_ИсточникЛидов = Истина;
		КонецЕсли;
		УчетнаяЗапись.CRM_ПериодСинхронизацииПапок = 7;
		УчетнаяЗапись.CRM_ЗагружатьВложенияПриПолученииПисем = ЗагружатьВложенияПриПолученииПисем;
		
		Если ПолучитьФункциональнуюОпцию("CRM_ИспользоватьПотенциальныхКлиентов") Тогда
			УчетнаяЗапись.CRM_ВариантАвтосозданияКлиентов = Перечисления.CRM_ВариантыАвтосозданияКлиентов.СоздаватьПК;
		Иначе
			УчетнаяЗапись.CRM_ВариантАвтосозданияКлиентов = Перечисления.CRM_ВариантыАвтосозданияКлиентов.СоздаватьКлиента;
		КонецЕсли;
		// -CRM
		...
		Если ВидУчетнойЗаписи = "Групповая" Тогда
			...
			Для Каждого ПользовательСсылка Из ПользователиМассив Цикл
				...
				КонецЕсли;
			КонецЦикла;
			Если ПользователиМассив.Количество() = 1 Тогда
				НоваяЗапись.Администрирование	= Истина;
			КонецЕсли;
		...
		Если УчетнаяЗаписьСсылка.ПротоколВходящейПочты = "IMAP" Тогда
			...
			ПериодЗагрузки = 7 * 24 * 60 * 60;
			МенеджерЗаписи.ДатаЗагрузкиПисем = НачалоДня(ТекущаяДатаСеанса()) - ПериодЗагрузки;
			МенеджерЗаписи.Записать(Истина);
		КонецЕсли;
		
		Если Не Перенастройка Тогда
			
			НаборЗаписей = РегистрыСведений.CRM_ПапкиУчетныхЗаписей.СоздатьНаборЗаписей();
			НаборЗаписей.Отбор.УчетнаяЗапись.Установить(УчетнаяЗаписьСсылка);
			
			НоваяЗапись = НаборЗаписей.Добавить();
			НоваяЗапись.УчетнаяЗапись	= УчетнаяЗаписьСсылка;
			НоваяЗапись.ВидПапки = Перечисления.CRM_ВидыПапокЭлектроннойПочты.Входящие;
			НоваяЗапись.Папка = СоздатьПапкуЭлектроннойПочты(ПапкаВходящие);
			
			НоваяЗапись = НаборЗаписей.Добавить();
			НоваяЗапись.УчетнаяЗапись	= УчетнаяЗаписьСсылка;
			НоваяЗапись.ВидПапки = Перечисления.CRM_ВидыПапокЭлектроннойПочты.Обработанные;
			НоваяЗапись.Папка = СоздатьПапкуЭлектроннойПочты(ПапкаОбработанные);
			
			НоваяЗапись = НаборЗаписей.Добавить();
			НоваяЗапись.УчетнаяЗапись	= УчетнаяЗаписьСсылка;
			НоваяЗапись.ВидПапки = Перечисления.CRM_ВидыПапокЭлектроннойПочты.Исходящие;
			НоваяЗапись.Папка = СоздатьПапкуЭлектроннойПочты(ПапкаИсходящие);
			
			НоваяЗапись = НаборЗаписей.Добавить();
			НоваяЗапись.УчетнаяЗапись	= УчетнаяЗаписьСсылка;
			НоваяЗапись.ВидПапки = Перечисления.CRM_ВидыПапокЭлектроннойПочты.Черновики;
			НоваяЗапись.Папка = СоздатьПапкуЭлектроннойПочты(ПапкаЧерновики);
			
			НоваяЗапись = НаборЗаписей.Добавить();
			НоваяЗапись.УчетнаяЗапись	= УчетнаяЗаписьСсылка;
			НоваяЗапись.ВидПапки = Перечисления.CRM_ВидыПапокЭлектроннойПочты.Отправленные;
			НоваяЗапись.Папка = СоздатьПапкуЭлектроннойПочты(ПапкаОтправленные);
			
			НоваяЗапись = НаборЗаписей.Добавить();
			НоваяЗапись.УчетнаяЗапись	= УчетнаяЗаписьСсылка;
			НоваяЗапись.ВидПапки = Перечисления.CRM_ВидыПапокЭлектроннойПочты.Удаленные;
			НоваяЗапись.Папка = СоздатьПапкуЭлектроннойПочты(ПапкаУдаленные);
			
			НоваяЗапись = НаборЗаписей.Добавить();
			НоваяЗапись.УчетнаяЗапись	= УчетнаяЗаписьСсылка;
			НоваяЗапись.ВидПапки = Перечисления.CRM_ВидыПапокЭлектроннойПочты.НежелательнаяПочта;
			НоваяЗапись.Папка = СоздатьПапкуЭлектроннойПочты(ПапкаНежелательные);
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Работа с почтовыми сообщениями'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, , УчетнаяЗаписьСсылка,
...
Функция ПараметрыЗапросаАвторизации()
	...
	Если НастройкиАвторизации.ИспользоватьКлючПроверкиПодлинностиPKCE Тогда
		ПараметрыЗапроса.Вставить("code_challenge",
			 РаботаСПочтовымиСообщениямиСлужебный.КодироватьСтрокуМетодомS256(ПроверочныйКод));
...
Функция НастройкиПодключенияПоАдресуЭлектроннойПочты()
	...
	УстановитьПривилегированныйРежим(Истина);
	Возврат Справочники.УчетныеЗаписиЭлектроннойПочты.НастройкиПодключенияПоАдресуЭлектроннойПочты(АдресЭлектроннойПочты,
		 ПарольДляПолученияПисем);
...
&НаСервере
Процедура ЗаполнитьЗначениеПапки(ВидПапки, ЗаполнятьПоУмолчанию)
	
	Если ЗначениеЗаполнено(ЭтотОбъект["Папка" + ВидПапки]) Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого ИмяПапки Из ПолученныеПапки Цикл
		Для Каждого ИмяПредопределеннойПапки Из ПредопределенныеПапки[ВидПапки] Цикл
			Если НРег(ИмяПапки) = НРег(ИмяПредопределеннойПапки) Тогда
				ЭтотОбъект["Папка" + ВидПапки] = ИмяПапки;
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	
	Если ЗаполнятьПоУмолчанию И Не ЗначениеЗаполнено(ЭтотОбъект["Папка" + ВидПапки]) Тогда
		ЭтотОбъект["Папка" + ВидПапки] = ПредопределенныеПапки[ВидПапки][0];
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Функция СоздатьПапкуЭлектроннойПочты(ПолноеИмяПапки)
	
	ЧастиИмени = СтрРазделить(ПолноеИмяПапки, РазделительИменПапок, Ложь);
	ИмяТекущейПапки = ЧастиИмени[ЧастиИмени.ВГраница()];
	
	ПапкаОбъект = Справочники.ПапкиЭлектронныхПисем.СоздатьЭлемент();
	ПапкаОбъект.Наименование = ИмяТекущейПапки;
	ПапкаОбъект.Владелец = УчетнаяЗаписьСсылка;
	ПапкаОбъект.CRM_ИмяНаСервере = ПолноеИмяПапки;
...
&НаСервере
Процедура ОпределитьПапкиIMAP(Знач ПрофильВходящейПочты = Неопределено)
	...
	ЗаполнятьПапкиПоУмолчанию = Истина;
	Если ИспользоватьДляПолучения И Протокол = "IMAP" Тогда
		
		Почта = Новый ИнтернетПочта;
		Попытка
			Почта.Подключиться(ПрофильВходящейПочты, ПротоколИнтернетПочты.IMAP);
		Исключение
			ПроверкаЗавершиласьСОшибками = Истина;
			СообщенияОбОшибках = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			Возврат;
		...
		Попытка
			РазделительИменПапок = Почта.СимволРазделитель;
		Исключение
			РазделительИменПапок = "/"; 
			// Некоторые почтовые сервера не поддерживают эту команду.
		КонецПопытки;
		
		ПолученныеПапки = Новый ФиксированныйМассив(Почта.ПолучитьПочтовыеЯщики());
		ЗаполнятьПапкиПоУмолчанию = Ложь;
		
		Почта.Отключиться();
		
	КонецЕсли;
	
	ЗаполнитьЗначениеПапки("Входящие", ЗаполнятьПапкиПоУмолчанию);
	ЗаполнитьЗначениеПапки("Обработанные", Истина);
	ЗаполнитьЗначениеПапки("Исходящие", Истина);
	ЗаполнитьЗначениеПапки("Отправленные", ЗаполнятьПапкиПоУмолчанию);
	ЗаполнитьЗначениеПапки("Удаленные", ЗаполнятьПапкиПоУмолчанию);
	ЗаполнитьЗначениеПапки("Черновики", ЗаполнятьПапкиПоУмолчанию);
	ЗаполнитьЗначениеПапки("Нежелательные", ЗаполнятьПапкиПоУмолчанию);
...
&НаКлиенте
Процедура ИмяПользователяДляПолученияПисемПриИзменении(Элемент)
	ИмяПользователяДляОтправкиПисем = ИмяПользователяДляПолученияПисем;
КонецПроцедуры

// Возвращает переданное значение, очищенное от пробелов слева/справа
// и специальных символов (перенос строки, табуляция)
//
// Параметры:
//  ТекущееЗначение  - Строка - Текущее значение реквизита.
//
// Возвращаемое значение:
//   Результат  - Строка - Обработанное значение реквизита.
//
&НаКлиенте
Функция УдалитьЛишниеСимволы(ТекущееЗначение)
	
	Результат = СокрЛП(ТекущееЗначение);
	
	УдаляемыеСимволы = Новый Массив;
	УдаляемыеСимволы.Добавить(Символы.ВК);
	УдаляемыеСимволы.Добавить(Символы.ВТаб);
	УдаляемыеСимволы.Добавить(Символы.НПП);
	УдаляемыеСимволы.Добавить(Символы.ПС);
	УдаляемыеСимволы.Добавить(Символы.ПФ);
	УдаляемыеСимволы.Добавить(Символы.Таб);
	
	Для Каждого УдаляемыйСимвол Из УдаляемыеСимволы Цикл
		Результат = СтрЗаменить(Результат, УдаляемыйСимвол, "");
	КонецЦикла;
