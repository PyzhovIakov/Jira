...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ CRM_ЛицензированиеЭкспортныеМетоды.ПодсистемаCRMИспользуется() Тогда
		CRM_ЛицензированиеЭкспортныеМетоды.ВывестиУведомлениеОНедоступности(НСтр("ru = 'Обмен данными с MS Outlook'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	#Если ВебКлиент Тогда
		ПоказатьПредупреждение(,
			 НСтр("ru='Обмен с MS Outlook невозможен в Веб-клиенте!';
			|en='Exchange with MS Outlook is not possible in  Web Client!'"),
			 15);
		Отказ = Истина;
		Возврат;
	#КонецЕсли
...
Процедура НастройкиОбменаOutlookПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если НЕ ЗначениеЗаполнено(ПрограммаOutlook) Тогда
		Возврат;
	КонецЕсли;
...
Процедура НастройкиОбменаOutlookПередУдалением(Элемент, Отказ)
	...
	ПоказатьВопрос(ОписаниеОповещения,
		 НСтр("ru='Удалить выбранную настройку?';en='Delete the select settings?'"),
...
Процедура КомандаПометитьНастройкуОсновной(Команда)
	...
	Если  ТекущаяПрограммаOutlook = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьДатыИзменения(СтруктураНастройки)
	ПрограммаOutlook = Элементы.Список.ТекущаяСтрока;
	Если НЕ СтруктураНастройки.Свойство("ФильтрДатаИзмененияКонтактаOutlook")
		...
		СтруктураНастройки.Вставить("ФильтрДатаИзмененияКонтактаOutlook",
			 ПрограммаOutlook.ДатаПоследнейСинхронизацииКонтактов);
	КонецЕсли;
