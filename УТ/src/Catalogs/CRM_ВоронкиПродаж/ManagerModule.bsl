...
Процедура Печать(МассивОбъектов, ПараметрыПечати, 
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПроверкаВоронокПродаж") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм,
			 "ПроверкаВоронокПродаж", "Проверка воронок продаж",
...
Функция ПечатьЭтаповВоронки(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ГДЕ
	               |	ТочкиМаршрута.ТочкаМаршрута.Вид В (&ВидыТочекМаршрута)
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	ТочкиМаршрута.КартаМаршрута.Наименование,
	               |	ТочкиМаршрута.РеквизитДопУпорядочивания";       
	
	ВидыТочекМаршрута = Новый Массив;
	ВидыТочекМаршрута.Добавить(Перечисления.CRM_ВидыТочекМаршрута.Действие);
	ВидыТочекМаршрута.Добавить(Перечисления.CRM_ВидыТочекМаршрута.Старт);
	Запрос.УстановитьПараметр("ВидыТочекМаршрута", ВидыТочекМаршрута);
...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	Если ВидФормы = "ФормаОбъекта" Тогда
		Если Параметры.Свойство("ЗначениеКопирования") Тогда
			Если ТипЗнч(Параметры.ЗначениеКопирования) = Тип("СправочникСсылка.CRM_ВоронкиПродаж")
				 И ЗначениеЗаполнено(Параметры.ЗначениеКопирования.Родитель) Тогда
		...
		ИначеЕсли Параметры.Свойство("ЗначенияЗаполнения") Тогда
			Если ТипЗнч(Параметры.ЗначенияЗаполнения) = Тип("Структура")
				 ...
				 И ЗначениеЗаполнено(Параметры.ЗначенияЗаполнения.Родитель) Тогда
				СтандартнаяОбработка = Ложь;
				ВыбраннаяФорма = "ФормаЭлементаСостав";
			КонецЕсли;
		...
		ИначеЕсли Параметры.Свойство("Ключ") Тогда
			...
			ИначеЕсли ТипЗнч(Параметры.Ключ) = Тип("СправочникСсылка.CRM_ВоронкиПродаж")
				 И ЗначениеЗаполнено(Параметры.Ключ.Родитель) Тогда
				СтандартнаяОбработка = Ложь;
				ВыбраннаяФорма = "ФормаЭлементаСостав";
			КонецЕсли;
