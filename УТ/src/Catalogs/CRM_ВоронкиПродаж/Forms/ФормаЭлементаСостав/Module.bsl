...
Процедура СоставПриАктивизацииСтроки(Элемент)
	...
	Если ТД = Неопределено Тогда
		Возврат;
	КонецЕсли;
	Элементы.СоставПредставлениеТочкиМаршрута.КнопкаВыбора	=
		(ТипЗнч(ТД.КартаМаршрута) = Тип("СправочникСсылка.CRM_КартыМаршрутов"));
...
Процедура СоставПредставлениеТочкиМаршрутаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	СтруктураПараметров = Новый Структура("Отбор,ТекущаяСтрока", Новый Структура("Владелец",
		 Элементы.Состав.ТекущиеДанные.КартаМаршрута),
		 Элементы.Состав.ТекущиеДанные.ТочкаМаршрута);
	ОписаниеОповещения = Новый ОписаниеОповещения("СоставПредставлениеТочкиМаршрутаНачалоВыбораЗавершение", ЭтотОбъект);
	ОткрытьФорму("Справочник.CRM_ВоронкиПродаж.Форма.ФормаВыбораТочкиМаршрута", СтруктураПараметров,
...
Процедура СоставПредставлениеТочкиМаршрутаОткрытие(Элемент, СтандартнаяОбработка)
	...
	КонецЕсли;
	СтандартнаяОбработка = Ложь;
	Если ЗначениеЗаполнено(ТекущиеДанные.ТочкаМаршрута)
		 И ТипЗнч(ТекущиеДанные.ТочкаМаршрута) = Тип("СправочникСсылка.CRM_ТочкиМаршрутов") Тогда
...
Процедура ЗаполнитьПредставленияИАктуальностьТочекМаршрута()
	...
	ОбъектБизнесПроцессаСчет = Неопределено;
	Если CRM_ОбщегоНазначенияПовтИсп.ЭтоCRM() Тогда
		ТипЭтапаВоронки = Тип("ДокументСсылка." + "CRM_СчетНаОплатуПокупателю");
		ОбъектБизнесПроцессаСчет = CRM_ВоронкиПродажСервер.ПолучитьВидОбъектаБизнесПроцессаПоТипу(ТипЭтапаВоронки);
	КонецЕсли;
	...
	Для Каждого СтрокаСостава Из Объект.Состав Цикл
		...
		СтрокаСостава.ТочкаМаршрутаНеАктуальна		= НЕ ПолучитьАктуальностьТочкиМаршрута(СтрокаСостава.КартаМаршрута,
			 СтрокаСостава.ТочкаМаршрута);
	КонецЦикла;
...
Процедура ДобавитьЭтапЗавершение(Этап, ДополнительныеПараметры) Экспорт
	...
	КонецЕсли;
		...
		ИначеЕсли ДополнительныеПараметры = "КартаМаршрута" Тогда
			...
			ОписаниеОповещения = Новый ОписаниеОповещения("ДобавитьТочкуЗавершение", ЭтотОбъект,
...
Процедура ДобавитьВариантСчета(Вариант, ДополнительныеПараметры) Экспорт
	...
	Если Вариант = КодВозвратаДиалога.Отмена Тогда
		Возврат;
	КонецЕсли;
...
Функция ПолучитьМассивДляОтбора()
	...
	Если Объект.ВидЭтапа = ПредопределенноеЗначение("Перечисление.CRM_ВидыЭтаповВоронкиПродаж.ПоДокументам") Тогда
		...
		СписокИсключаемыхТипов = CRM_ВоронкиПродажСервер.ВоронкаПродажСписокИсключаемыхТипов();
		
		ЕстьДокументCRM_СчетНаОплатуПокупателю = CRM_ОбщегоНазначенияСервер.НайтиВМетаданныхПоИмени("Документы",
			 "CRM_СчетНаОплатуПокупателю");
		ИмяДокументаCRM_СчетНаОплатуПокупателю = "ДокументСсылка.CRM_СчетНаОплатуПокупателю";	
		
		Пока Выборка.Следующий() Цикл
			Если ЕстьДокументCRM_СчетНаОплатуПокупателю
				 И Выборка.ТочкаМаршрута.ТипЗначения = Новый ОписаниеТипов(ИмяДокументаCRM_СчетНаОплатуПокупателю) Тогда
				Продолжить;
			КонецЕсли;
		...
		Для Каждого СтрокаСостава Из Объект.Состав Цикл
			Если ЕстьДокументCRM_СчетНаОплатуПокупателю
				 И СтрокаСостава.ТочкаМаршрута.ТипЗначения = Новый ОписаниеТипов(ИмяДокументаCRM_СчетНаОплатуПокупателю) Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ВидВоронкиПриИзмененииСервер()
	Если Объект.ВидЭтапа = Перечисления.CRM_ВидыЭтаповВоронкиПродаж.ПоСостояниямИнтересов Тогда
		...
		ПараметрыВыбораВида.Добавить(Новый ПараметрВыбора("Отбор.Родитель",
