...
Процедура Подключаемый_СписокПриАктивизацииСтроки()
	...
	Если ЭлементСписка = Неопределено Тогда
		Элементы.ФормаВосстановитьИзМакета.Доступность = Ложь;
	Иначе	
		Элементы.ФормаВосстановитьИзМакета.Доступность = (ЭлементСписка.ИзМакета
			 ИЛИ ЭтоПредопределенноеЗначение(ВыделенныеСтроки[0]));
	КонецЕсли;	
...
Процедура ВосстановитьИзМакета(Команда)
	...
	Если ВыделенныеСтроки.Количество() > 0 Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения,
			 НСтр("ru='Выделенные элементы будут перезаполнены из макета справочника. Продолжить?';
			|en='Selected items will be re-written from catalog template. Continue?'"),
