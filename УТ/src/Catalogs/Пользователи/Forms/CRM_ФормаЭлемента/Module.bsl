...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() И Параметры.Свойство("ПараметрыНового") Тогда
		...
		Поз = СтрНайти(Параметры.ПараметрыНового.АдресПочты, "@");
		Если Поз <> 0 Тогда
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
		...
		МодульУправлениеКонтактнойИнформациейБСП = ОбщегоНазначения.ОбщийМодуль("УправлениеКонтактнойИнформацией");
		...
		Если ПользователиСлужебный.ДоступнаНастройкаВосстановленияПароля(УровеньДоступа) Тогда
			...
			ИмяРеквизитаСПочтойДляВосстановленияПароля = МодульУправлениеКонтактнойИнформациейБСП.ОпределитьЭлементСПочтойДляВосстановленияПароля(
			// -CRM
				ЭтотОбъект,
				ПользовательИБАдресЭлектроннойПочты,
	...
	Если Не Объект.Ссылка.Пустая() Тогда
		РегистрыСведений.CRM_ОтсутствиеСотрудников.СформироватьАктивнуюНадпись(ЭтотОбъект,
			 "CRM_ГруппаОтсутствиеПользователя", Ложь,
			 Объект.Ссылка);
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	// +CRM
	Если ИмяСобытия = "ЗавершениеОтсутствияПользователя" Тогда
		Элементы.CRM_ГруппаОтсутствиеПользователя.Видимость = Ложь;
	КонецЕсли;
	// -CRM
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
		...
		МодульУправлениеКонтактнойИнформациейCRM = ОбщегоНазначения.ОбщийМодуль("CRM_УправлениеКонтактнойИнформацией");
		МодульУправлениеКонтактнойИнформациейCRM.ПриЧтенииНаСервере(ЭтотОбъект, ТекущийОбъект, "КонтактнаяИнформация");
		// -CRM
...
&НаКлиенте
Процедура НаименованиеОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, СтандартнаяОбработка)
...
Процедура Подключаемый_КонтактнаяИнформацияПриИзменении(Элемент)
	...
	// +CRM
	// Если это адрес электронной почты, то надо проверить на корректность заполнения.
...
Процедура Подключаемый_ОткрытьАдресНаИнтернетКарте(Элемент)
	...
	НайденныеСтроки = КонтактнаяИнформацияОписаниеДополнительныхРеквизитов.НайтиСтроки(СтруктураПоиска);
...
Процедура ОпределитьНесоответствияПользователяСПользователемИБ(ПараметрыЗаписи = Неопределено)
	...
	ИначеЕсли УровеньДоступа.УправлениеСписком Тогда
		...
		Если ПользовательИБПолноеИмя <> Объект.Наименование Тогда
			...
			СтрокаПолноеИмя = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Полное имя ""%1""'"),
				ПользовательИБПолноеИмя);
			УточнениеСвойств.Вставить(0, СтрокаПолноеИмя);
			// -CRM Рефакторинг кода
		КонецЕсли;
		...
		Если УровеньДоступа.ИзменениеТекущего Тогда
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
				...
				Если СтрокаОписания = Неопределено Тогда
					...
					// +CRM Рефакторинг кода
					СтрокаПочтаДляВосстановленияПароля = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Почта для восстановления пароля ""%1""'"),
						ПользовательИБАдресЭлектроннойПочты);
					УточнениеСвойств.Вставить(0, СтрокаПочтаДляВосстановленияПароля);
					// -CRM Рефакторинг кода
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
