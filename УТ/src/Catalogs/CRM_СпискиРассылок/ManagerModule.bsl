...
//  МассивРезультат - Массив - Возвращаяемый список контактов.
//
Функция ПолучитьСоставСпискаРассылки(Ссылка, МассивРезультат = Неопределено,
	 МассивОбработанныхСписков = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			МассивРезультат.Добавить(Новый Структура("Адрес,Контакт", Выборка.Адрес,
				 ?(ЗначениеЗаполнено(Выборка.Контакт), Выборка.Контакт,
