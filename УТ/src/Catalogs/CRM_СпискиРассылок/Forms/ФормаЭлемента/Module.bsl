...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьКолонкуТипКонтакта(Объект.Состав);
...
Процедура СоставАдресНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ТипЗнч(ТекДанные.Контакт) = Тип("СправочникСсылка.Партнеры") Тогда
		...
		СписокАдресов = СформироватьСписокАдресов(Новый Структура("Партнер, КонтактноеЛицо",
			 ТекДанные.Контакт,
			 ВернутьОсновноеКЛКлиента(ТекДанные.Контакт)));
...
Процедура ЗаполнитьПоРезультатамОтчёта(Команда)
	...
	ОткрытьФорму("Справочник.CRM_СпискиРассылок.Форма.ФормаВыбораРезультатаОтчета", ПараметрыФормы,
...
Процедура ЗаполнитьПоСегменту(Команда)
	...
	ОткрытьФорму("Справочник.CRM_СпискиРассылок.Форма.ФормаВыборСегмента", ПараметрыФормы, ЭтотОбъект,
		 , , , ОписаниеОповещения,
		 РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	...
	Если НЕ (Результат = Неопределено) Тогда
		Если Объект.Состав.Количество() > 0 Тогда
			ТекстВопроса = Нстр("ru='Cписок получателей содержит записи. Заменить или дополнить?';
				|en='The recipient list contains entries. Replace or supplement?'");
...
Процедура ЗагрузкаКлиентовИзФайла(Команда)
	...
	Если CRM_ЛицензированиеКлиент.ПроверитьПодключениеОтраслевыхСервисовИОткрыть() Тогда
		...
		ПараметрыФормы  = Новый Структура("РежимЗагрузкиВДокумент, ЗагрузкаВДокумент", Истина, "СписокРассылок");
...
Процедура ЗагрузкаКлиентов(МассивЭлементов)
	...
	Для Каждого ЭлементМассива Из МассивЭлементов Цикл
		Если ТипЗнч(ЭлементМассива) = Тип("Структура") Тогда
			...
			НоваяСтрока.Контакт			= ЭлементМассива.Партнер;
			НоваяСтрока.КонтактСтрокой	= ЭлементМассива.ПартнерСтрокой;
			НоваяСтрока.Адрес			= ЭлементМассива.Почта;
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьФизическихЛицГруппы(ГруппаФизЛиц)
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	СправочникФизЛица.Ссылка КАК Ссылка,
	|	СправочникФизЛица.ПометкаУдаления КАК ПометкаУдаления,
	|	СправочникФизЛица.Предопределенный КАК Предопределенный,
	|	СправочникФизЛица.Наименование КАК Наименование
	|ИЗ
...
Процедура ОчиститьТабличнуюЧасть(ИмяТаблицы = "Состав")
	Если Объект[ИмяТаблицы].Количество() > 0 Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения,
			 НСтр("ru='Табличная часть содержит записи. Продолжить?';
			|en='Tabular section contain records. Continue?'"), РежимДиалогаВопрос.ДаНет, ,
...
Процедура ЗаполнитьКолонкуТипКонтакта(Состав)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("СправочникСсылка.Партнеры") 
		...
		Если ЗначениеЗаполнено(ВыбранноеЗначение) Тогда
			...
			ИначеЕсли ТипЗнч(ВыбранноеЗначение) = Тип("СправочникСсылка.КонтактныеЛицаПартнеров") Тогда
				СписокАдресов = СформироватьСписокАдресов(Новый Структура("Партнер, КонтактноеЛицо",
					 ВернутьВладельцаКЛ(ТекДанные.Контакт),
					 ТекДанные.Контакт));
			Иначе
				Возврат;
			КонецЕсли;				
