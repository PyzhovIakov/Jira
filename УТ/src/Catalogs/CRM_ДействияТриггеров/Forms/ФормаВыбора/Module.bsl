		...
		Иначе
			...
			Пока Выборка.Следующий() Цикл
				...
				Если ОписаниеТипов <> Неопределено И (ОписаниеТипов.Типы().Количество() = 0
					 ИЛИ CRM_ОбщегоНазначенияКлиентСервер.ПересечениеМножеств(ОписаниеТипов.Типы(), ПроверяемыеТипы).Количество() > 0) Тогда
					МассивДоступных.Добавить(Выборка.Ссылка);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Список, "Ссылка",
			 МассивДоступных, ВидСравненияКомпоновкиДанных.ВСписке, ,
			 Истина);
	Иначе
