...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	CRM_КлассификаторыЭкспортныеМетоды.ПриСозданииНаСервере(ЭтотОбъект);
...
Процедура Сформировать(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ПоказатьПредупреждение(,
			 НСтр("ru='Сначала необходимо записать значение.';en='First,
			| you need to write down the value.'"));
		Возврат;
	КонецЕсли;
	...
	Если Модифицированность Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения,
				НСтр("ru='Перед формированием необходимо записать значение. Записать?';en='Before forming,
					| you need to write down the value. Write down?'"),
...
Процедура Настройки(Команда)
	...
	ЗаголовокФормыНастройкиСхемыКомпоновкиДанных = НСтр("ru='Настройки значения классификатора ""%ИмяЗначения%""';
		|en='The settings of the classifier value ""%ИмяЗначения%""'");
...
Процедура УстановитьВидимостьДоступность(Форма)
	...
	Форма.Элементы.ГруппаСКД.Видимость = 
		Форма.СпособФормирования =
			ПредопределенноеЗначение("Перечисление.СпособыФормированияСегментов.ПериодическиОбновлять");
	...
	Форма.Элементы.ДополнительныйРеквизит.Видимость = 
		Форма.ВидОтображения =
			ПредопределенноеЗначение("Перечисление.CRM_ВидыОтображенияКлассификаторов.ДополнительныйРеквизит");
