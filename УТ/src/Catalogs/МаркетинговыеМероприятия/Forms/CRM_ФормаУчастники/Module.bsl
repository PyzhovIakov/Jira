...
Процедура КомандаЗаполнитьСтороннихУчастниковПоРезультатамОтчета(Команда)
	Если ПартнерыИКонтактныеЛица.Количество() > 0 Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения,
			 НСтр("ru='Список будет очищен. Продолжить?';en='The list will be cleared. Continue?'"),
	...
	Если НаименованиеСохраненногоСписка <> Неопределено Тогда
		...
		Если СохраненныйСписок <> Неопределено Тогда
			Для Каждого СтрокаСписка Из СохраненныйСписок Цикл
				...
				ИначеЕсли (ТипЗнч(СтрокаСписка.Значение)) = Тип("СправочникСсылка.КонтактныеЛицаПартнеров") Тогда
					НоваяСтрока = ПартнерыИКонтактныеЛица.Добавить();
					НоваяСтрока.КонтактноеЛицо = СтрокаСписка.Значение;
					НоваяСтрока.Партнер = CRM_ОбщегоНазначенияСервер.ЗначениеРеквизитаОбъекта(НоваяСтрока.КонтактноеЛицо, "Владелец");
				КонецЕсли;
...
Процедура СвоиУчастникиДатаОкончанияПриИзменении(Элемент)
	...
	Если Не ЗначениеЗаполнено(ТекДанные.ДатаОкончания) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПартнерыИКонтактныеЛица.Загрузить(Параметры.ПартнерыИКонтактныеЛица.Выгрузить());
	CRM_СвоиУчастники.Загрузить(Параметры.CRM_СвоиУчастники.Выгрузить());
...
Процедура Применить(Команда)
	...
	КонецЕсли;
	 Результат = Новый Структура("CRM_СвоиУчастники,ПартнерыИКонтактныеЛица",
	 	 ЭтотОбъект.CRM_СвоиУчастники,
	 	 ЭтотОбъект.ПартнерыИКонтактныеЛица);
	Закрыть(Результат);
