...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	Если ТипЗнч(СохраненныйОтбор) = Тип("ОтборКомпоновкиДанных") Тогда
		СкопироватьОтборКомпоновкиДанных(НастройкиКонтактов.Настройки.Отбор.Элементы,
			 СохраненныйОтбор.Элементы,
			 НастройкиКонтактов.Настройки.Отбор.ДоступныеПоляОтбора);
	КонецЕсли;
	...
	Если ТипЗнч(СохраненныйОтбор) = Тип("ОтборКомпоновкиДанных") Тогда
		СкопироватьОтборКомпоновкиДанных(НастройкиСобытий.Настройки.Отбор.Элементы,
			 СохраненныйОтбор.Элементы,
			 НастройкиСобытий.Настройки.Отбор.ДоступныеПоляОтбора);
	КонецЕсли;
	...
	Если ТипЗнч(СохраненныйОтбор) = Тип("ОтборКомпоновкиДанных") Тогда
		СкопироватьОтборКомпоновкиДанных(НастройкиПочтовыхСообщений.Настройки.Отбор.Элементы,
			 СохраненныйОтбор.Элементы,
			 НастройкиПочтовыхСообщений.Настройки.Отбор.ДоступныеПоляОтбора);
	КонецЕсли;
	...
	Если ТипЗнч(СохраненныйОтбор) = Тип("ОтборКомпоновкиДанных") Тогда
		СкопироватьОтборКомпоновкиДанных(НастройкиЗадач.Настройки.Отбор.Элементы,
			 СохраненныйОтбор.Элементы,
			 НастройкиЗадач.Настройки.Отбор.ДоступныеПоляОтбора);
	КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	Если Объект.Подключение.СерверMSExchange <> Объект.УчетнаяЗапись.СерверMSExchange Тогда
		Отказ = Истина;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Реквизит ""Сервер MS Exchange"" в подключении отличается от значения,
	...
	Если Не Отказ Тогда
		...
		Если НастройкиКонтактов.Настройки.Отбор.Элементы.Количество() > 0 Тогда
			ТекущийОбъект.ХранилищеОтборКонтактов = Новый ХранилищеЗначения(НастройкиКонтактов.Настройки.Отбор,
		...
		Если НастройкиСобытий.Настройки.Отбор.Элементы.Количество() > 0 Тогда
			ТекущийОбъект.ХранилищеОтборСобытий = Новый ХранилищеЗначения(НастройкиСобытий.Настройки.Отбор,
				 Новый СжатиеДанных(9));
		Иначе
		...
		Если НастройкиПочтовыхСообщений.Настройки.Отбор.Элементы.Количество() > 0 Тогда
			ТекущийОбъект.ХранилищеОтборПочтовыхСообщений = Новый ХранилищеЗначения(НастройкиПочтовыхСообщений.Настройки.Отбор,
				 Новый СжатиеДанных(9));
		Иначе
...
Процедура ДекорацияНастроитьСоответствияВидовКИНажатие(Элемент)
	...
	ОткрытьФорму("Справочник.CRM_НастройкиОбменаСMSExchange.Форма.НастройкаСоответствияКонтактнойИнформации",
		 ПараметрыФормы, ЭтотОбъект, , , ,
		 Новый ОписаниеОповещения("ДекорацияНастроитьСоответствияВидовКИЗавершение", ЭтотОбъект,
		 МассивТаблиц));
...
Процедура ВыборПапкиMSExchange(Элемент)
	...
	Если Не СтруктураПапокЗаполненаДляУчетнойЗаписи() Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Для учетной записи ';en='For account '") + """" + Строка(Объект.УчетнаяЗапись) 
			+ """ " 
			+ НСтр("ru='еще не была загружена структура папок."
...
Процедура Далее(Команда)
	...
	Иначе
		...
		Пока (Не СледующаяСтраница.Видимость Или Не СледующаяСтраница.Доступность)
			 И Не ЭтоПоследняяСтраница(СледующаяСтраница) Цикл
...
Процедура ЗаполнитьСписокЧасовыхПоясов()
	...
	Пока Истина Цикл
		...
		Иначе
			ПредставлениеСмещения = "(GMT " 
				+ CRM_MSExchangeСерверЭкспортныеМетоды.ПолучитьСмещениеВремениСтрокойДляMSExchange(Минуты) 
				+ ")";
		КонецЕсли;
...
Функция СформироватьТекстЗапросаСобытия()
	...
	|ИЗ
	|	Документ.CRM_Взаимодействие КАК Мероприятия
	|";
...
Функция СформироватьТекстЗапросаПочтовыеСообщения()
	Если Не Объект.СинхронизироватьПочтовыеСообщенияВходящие
		 И Не Объект.СинхронизироватьПочтовыеСообщенияОтправленные Тогда
		Возврат "ВЫБРАТЬ NULL";
	КонецЕсли;
	...
	Если Объект.СинхронизироватьПочтовыеСообщенияВходящие Тогда
		...
		|	ЭлектронноеПисьмоВходящее.Ссылка КАК ЭлектронноеПисьмоВходящее" 
			+ ?(Объект.СинхронизироватьПочтовыеСообщенияОтправленные, ",", "") + "
...
Процедура УправлениеЭлементамиФормы(Форма)
	...
	КонецЕсли;
	Доступность = ((Форма.Объект.СинхронизироватьКлиенты
		 ИЛИ Форма.Объект.СинхронизироватьКонтактныеЛица) ИЛИ (Форма.Объект.СинхронизироватьСобытия
		 И Форма.Объект.СинхронизироватьКонтактыСобытий));
	...
	// Почтовые сообщения
	Доступность = Форма.Объект.СинхронизироватьПочтовыеСообщенияВходящие
		 ИЛИ Форма.Объект.СинхронизироватьПочтовыеСообщенияОтправленные;
	...
	Форма.Элементы.ИскатьКонтактыПочтовыхСообщенийПоАдресамEMail.Доступность = Доступность
		И (Форма.Объект.СинхронизироватьКонтактыПочтовыхСообщенийВходящие
			 Или Форма.Объект.СинхронизироватьКонтактыПочтовыхСообщенийОтправленные);
	...
	Форма.Элементы.ПапкиВходящихПисемПапка1С.Доступность = Форма.Элементы.ПапкиВходящихПисем.Доступность
		 И ЗначениеЗаполнено(ЗначениеЗаполнено(Форма.Объект.УчетнаяЗаписьЭлектроннойПочты));
	Форма.Элементы.ПапкиОтправленныхПисемПапка1С.Доступность = Форма.Элементы.ПапкиОтправленныхПисем.Доступность
		 И ЗначениеЗаполнено(ЗначениеЗаполнено(Форма.Объект.УчетнаяЗаписьЭлектроннойПочты));
	Форма.Элементы.СоздаватьСобытияПоПочтовымСообщениям.Доступность = Доступность
		 И ЗначениеЗаполнено(Форма.Объект.УчетнаяЗаписьЭлектроннойПочты);
