...
Функция ПолучитьДеревоПапокЭлектроннойПочты(УчетнаяЗапись, СерверMSExchange) Экспорт
	...
	|	И СтруктурыПапокMSExchange.DistinguishedFolderId В (&МассивDistinguishedFolderId)
	|");
	Запрос.УстановитьПараметр("СерверMSExchange", СерверMSExchange);
	Запрос.УстановитьПараметр("УчетнаяЗапись", УчетнаяЗапись);
	Запрос.УстановитьПараметр("МассивDistinguishedFolderId", МассивDistinguishedFolderId);
	...
	Для НомерСтроки = 0 По МассивРодителей.ВГраница() Цикл
		...
		|" + ?(НомерСтроки = 0, "", "ИЛИ ") + "(СтруктурыПапокMSExchange.Ссылка В ИЕРАРХИИ (&" 
			+ ИмяПараметра + ") И СтруктурыПапокMSExchange.FolderClass = ""IPF.Note"")" 
			+ "
		|";
...
//
Процедура ДобавитьПапкиMSExchangeВСправочникПапкиЭлектронныхПисем(УчетнаяЗаписьЭлектроннойПочты,
	 УчетнаяЗаписьMSExchange,
	 СерверMSExchange) Экспорт
	Если Не ЗначениеЗаполнено(УчетнаяЗаписьЭлектроннойПочты)
		 Или Не ЗначениеЗаполнено(УчетнаяЗаписьMSExchange)
		 Или Не ЗначениеЗаполнено(СерверMSExchange) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьПапку(УчетнаяЗаписьЭлектроннойПочты, СтрокаДерева)
	...
	Если Не ЗначениеЗаполнено(Родитель) И ЗначениеЗаполнено(СтрокаДерева.ВидПапкиЭлектроннойПочты1СРодителя) Тогда
		Родитель = ПолучитьПапкуЭлектронныхПисемПоВиду(УчетнаяЗаписьЭлектроннойПочты,
			 СтрокаДерева.ВидПапкиЭлектроннойПочты1СРодителя);
	КонецЕсли;
