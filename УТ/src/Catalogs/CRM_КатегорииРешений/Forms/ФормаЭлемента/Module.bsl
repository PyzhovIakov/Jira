...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		Объект.ЦветИндекс			= 12;
		Объект.ЦветПредставление 	= НСтр("ru='Светло-серый';en='Light grey'");
		Объект.ЦветКрасный  		= 250;
		Объект.ЦветЗеленый  		= 250;
		Объект.ЦветСиний  			= 250;
		Элементы.Цвет.ЦветФона		= Новый Цвет(Объект.ЦветКрасный, Объект.ЦветЗеленый, Объект.ЦветСиний); 
		Элементы.Цвет.ЦветТекста	= CRM_ОбщегоНазначенияКлиентСервер.ПолучитьКонтрастныйЦвет(Элементы.Цвет.ЦветФона);
	КонецЕсли;
...
Процедура ЦветНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("Справочник.CRM_КатегорииРешений.Форма.ФормаВыбораЦвета", ПараметрыФормы, Элемент, , ,
		 , ОписаниеОповещения,
