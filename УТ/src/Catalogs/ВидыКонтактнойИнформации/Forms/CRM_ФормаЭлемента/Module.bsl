...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЗапретРедактированияРеквизитовОбъектов") Тогда
			...
			МодульЗапретРедактированияРеквизитовОбъектов.ЗаблокироватьРеквизиты(ЭтотОбъект, ,
				 НСтр("ru = 'Разрешить редактирование типа и группы'"));
	...
	Если Метаданные.Обработки.Найти("РасширенныйВводКонтактнойИнформации") <> Неопределено Тогда
		МодульРасширенныйВводКонтактнойИнформации = ОбщегоНазначения.ОбщийМодуль(
			"Обработки.РасширенныйВводКонтактнойИнформации");
		ДоступныДополнительныеНастройкиАдреса = 
			МодульРасширенныйВводКонтактнойИнформации.ДоступныДополнительныеНастройкиАдреса();
	Иначе
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если НЕ ПараметрыЗаписи.Свойство("КогдаИдентификаторДляФормулУжеИспользуется")
		...
		Если ЗначениеЗаполнено(ТекстВопроса) Тогда
			...
			ПоказатьВопрос(
				Новый ОписаниеОповещения("ПослеОтветаНаВопросКогдаИдентификаторДляФормулУжеИспользуется",
...
Процедура ОбновитьДоступностьЭлементовТелефонаФакса(Форма)
	...
	Форма.Элементы.МаскаПриВводеНомераТелефона.Доступность = (Не Форма.Объект.ТелефонСДобавочнымНомером
		И Не Форма.Объект.ВидРедактирования = "Диалог") ИЛИ Форма.Объект.ВводитьНомерПоМаске;
	Форма.Элементы.ТелефонСДобавочнымНомером.Доступность = Не Форма.Объект.ВводитьНомерПоМаске
		Или Форма.Объект.ТелефонСДобавочнымНомером;
...
&НаСервереБезКонтекста
Функция ИдентификаторДляФормулУжеИспользуется(Знач ИдентификаторДляФормул,
	 Знач ТекущийВидКонтактнойИнформации, Знач Родитель)
