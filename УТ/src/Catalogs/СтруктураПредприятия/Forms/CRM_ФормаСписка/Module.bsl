...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	CRM_ОбщегоНазначенияСервер.ВыделитьЖирнымОсновнойЭлемент(Пользователи.ТекущийПользователь().Подразделение,
		 Список,
...
Процедура СписокПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	МассивРезультатов = УстановитьПодразделениеПользователю(ПараметрыПеретаскивания.Значение, Строка,
				...
				Исключение
					...
					Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Пользователю %1 не удалось установить подразделение %2';
							|en='The user %1 doing not manage to install department %2'"),
						Пользователь,
						Подразделение
					);
	...
	Если ВыбранныйЭлемент <> Пользователь.Подразделение Тогда
		...
		Исключение
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Ошибка при установке основного подразделения.';
				|en='An error occurred while installing main department.'"));
			Успех = Ложь;
		КонецПопытки;
