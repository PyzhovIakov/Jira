...
Процедура СформироватьМаршрутИИсполнителей()
	...
	Если НастройкиМаршрута.Количество() > 0 Тогда
		...
		СтруктураНастроек = Новый Структура("НастройкиМаршрута, Исполнители, Объекты",
			 НастройкиМаршрута_Значение, Исполнители_Значение,
			 Объекты_Значение);
...
Функция ПроверитьНаНаличиеИзменений()
	...
	Для Каждого Реквизит Из РеквизитыОбъекта Цикл
		...
		Если ИмяРеквизита = "ХранилищеСхемыКомпоновкиДанных"
			 ИЛИ ИмяРеквизита = "ХранилищеНастроекКомпоновкиДанных" Тогда Продолжить; КонецЕсли;
...
Функция ЗаполнитьДеревоТочекПоВариантуСтарта(СтруктураПараметров)
	...
	Пока НЕ Конец Цикл
		...
		Если ИсходящаяТочка.ВидЭлемента = Перечисления.CRM_ВидыТочекМаршрута.Действие
			...
			МассивСтрок = ПройденныеТочки.НайтиСтроки(Новый Структура("ВходящаяТочка, ТочкаМаршрута",
				 ТекущаяТочка.ИмяЭлемента,
				 ИсходящаяТочка.ИмяЭлемента));
		...
		ИначеЕсли ИсходящаяТочка.ВидЭлемента = Перечисления.CRM_ВидыТочекМаршрута.ВыборВарианта Тогда
			...
			Для Каждого Вариант Из ИсходящаяТочка.ИсходящиеТочки Цикл
				...
				Если Результат.Свойство("ТекТочка") 
					И Результат.ТекТочка.ВидЭлемента = Перечисления.CRM_ВидыТочекМаршрута.Слияние Тогда
...
Функция ПрочитатьГрафическуюСхемуСформироватьДеревоТочек()
	...
	СтруктураВозврата = Новый Структура("КритичныеОшибки, НеКритичныеОшибки",
		 СтруктураДанных.КритичныеОшибки,
		 СтруктураДанных.НеКритичныеОшибки);
	Возврат СтруктураВозврата;
	
КонецФункции

&НаСервере
Функция ПолучитьМассивТочекМаршрута()
...
Процедура УстановитьСвойстваЭлементовФормы()
	...
	Элементы.УказыватьСрокВыполненияЭтапов.Доступность =
		(Объект.ТипПроцесса <> ПредопределенноеЗначение("Перечисление.bpmТипыПроцессов.ПроцессОбъекта"));
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ CRM_ЛицензированиеЭкспортныеМетоды.ПодсистемаCRMИспользуется() Тогда
		CRM_ЛицензированиеЭкспортныеМетоды.ВывестиУведомлениеОНедоступности(НСтр("ru = 'Карту маршрута'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура УстановитьСвойстваЭлементовПоТипу(ПриСоздании = Ложь)
	...
	Элементы.ВидОбъектаСтрокой.АвтоОтметкаНезаполненного		=
		(Объект.ТипПроцесса = Перечисления.bpmТипыПроцессов.ПроцессОбъекта);
	Элементы.РеквизитОбъектаСтрокой.АвтоОтметкаНезаполненного	=
		(Объект.ТипПроцесса = Перечисления.bpmТипыПроцессов.ПроцессОбъекта);
	// Элементы.Событие.АвтоОтметкаНезаполненного					= (Объект.ТипПроцесса = Перечисления.bpmТипыПроцессов.ПроцессОбъекта);
	
	Элементы.ИспользоватьПриПринятииОбращения.Видимость			=
		(Объект.ТипПроцесса = Перечисления.bpmТипыПроцессов.НезависимыйПроцесс);
	Элементы.ПоляОтборов.Видимость = (Объект.ТипПроцесса = Перечисления.bpmТипыПроцессов.НезависимыйПроцесс)
		 И Объект.ИспользоватьПриПринятииОбращения;
	
	Элементы.СостояниеДосрочногоЗавершения.Видимость = (Объект.РазрешеноДосрочноеЗавершение
		 И Объект.ТипПроцесса = Перечисления.bpmТипыПроцессов.ПроцессОбъекта);
	Если Объект.РазрешеноДосрочноеЗавершение И ЗначениеЗаполнено(Объект.ВидОбъектаСтрокой)
		 И ЗначениеЗаполнено(Объект.РеквизитОбъектаСтрокой) Тогда
		...
		Если СтрНайти(Объект.РеквизитОбъектаСтрокой, "%Свойство%") > 0 Тогда
			СсылкаНаСвойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.ПолучитьСсылку(Новый УникальныйИдентификатор(СтрЗаменить(Объект.РеквизитОбъектаСтрокой,
				 "%Свойство%",
				 "")));
			...
			Элементы.СостояниеДосрочногоЗавершения.Заголовок = СсылкаНаСвойство.Заголовок + " (" 
		...
		Иначе
			...
			КонецЕсли;
			Элементы.СостояниеДосрочногоЗавершения.ПараметрыВыбора = Новый ФиксированныйМассив(ПараметрыВыбораСостояния);
			Элементы.СостояниеДосрочногоЗавершения.ОграничениеТипа =
				Новый ОписаниеТипов(МетаданныеОбъекта.Реквизиты[Объект.РеквизитОбъектаСтрокой].Тип.Типы());
			Элементы.СостояниеДосрочногоЗавершения.Заголовок =
				МетаданныеОбъекта.Реквизиты[Объект.РеквизитОбъектаСтрокой].Синоним;
		КонецЕсли;
	...
	Если Объект.ОписаниеБизнесПроцессаВСодержаниеЗадачи
		 И Объект.ТипПроцесса = Перечисления.bpmТипыПроцессов.ПроцессОбъекта Тогда
		Объект.ОписаниеБизнесПроцессаВСодержаниеЗадачи = Ложь;
		Модифицированность = Истина;
	КонецЕсли;
	Элементы.ОписаниеБизнесПроцессаВСодержаниеЗадачи.Доступность =
		(Объект.ТипПроцесса <> Перечисления.bpmТипыПроцессов.ПроцессОбъекта);
	
	//Если Объект.РедактироватьСрокВыполненияЗадач
		// И Объект.ТипПроцесса = Перечисления.bpmТипыПроцессов.ПроцессОбъекта Тогда
	...
	Если Объект.УказыватьПричинуНеудачногоЗавершения
		 И Объект.ТипПроцесса = Перечисления.bpmТипыПроцессов.ПроцессОбъекта
		 И Объект.ВидОбъектаСтрокой = "CRM_Интерес" Тогда
		Объект.УказыватьПричинуНеудачногоЗавершения = Ложь;
		Модифицированность = Истина;
	КонецЕсли;
	Элементы.УказыватьПричинуНеудачногоЗавершения.Доступность = НЕ (Объект.ТипПроцесса = Перечисления.bpmТипыПроцессов.ПроцессОбъекта
		 И Объект.ВидОбъектаСтрокой = "CRM_Интерес");
	
	Если Объект.РедактироватьДатуВыполненияЗадач
		 И Объект.ТипПроцесса = Перечисления.bpmТипыПроцессов.ПроцессОбъекта
		 И Объект.ВидОбъектаСтрокой	= "CRM_Интерес" Тогда
		Объект.РедактироватьДатуВыполненияЗадач = Ложь;
		Модифицированность = Истина;
	КонецЕсли;
	Элементы.РедактироватьДатуВыполненияЗадач.Доступность = НЕ (Объект.ТипПроцесса = Перечисления.bpmТипыПроцессов.ПроцессОбъекта
		 И Объект.ВидОбъектаСтрокой = "CRM_Интерес");
	...
	Если Объект.РедактироватьИсполнителейЭтапов И Объект.ТипПроцесса = Перечисления.bpmТипыПроцессов.ПроцессОбъекта Тогда
		...
		Модифицированность = Истина;
	КонецЕсли;
	Элементы.РедактироватьИсполнителейЭтапов.Доступность =
		НЕ (Объект.ТипПроцесса = Перечисления.bpmТипыПроцессов.ПроцессОбъекта);
	...
	Элементы.СценарийИнтереса.Видимость = (Объект.ТипПроцесса = Перечисления.bpmТипыПроцессов.ПроцессОбъекта
		 И Объект.ВидОбъектаСтрокой = "CRM_Интерес");
...
Процедура ПолучитьСхемуИНастройкиКомпоновкиДанных(ПриСоздании = Ложь)
	...
	Попытка
		НастройкиКомпоновки	= CRM_БизнесПроцессыПереопределяемый.СкомпоноватьНастройки(КомпоновщикНастроек,
			 АдресСхемыКомпоновкиДанных);
	Исключение
	...
	Если Объект.Ссылка.Пустая() Тогда
		Если АдресХранилищаНастройкиКомпоновщика = "" Тогда
			Если Объект.ТипПроцесса = Перечисления.bpmТипыПроцессов.ПроцессОбъекта Тогда
				CRM_БизнесПроцессыПереопределяемый.ДобавитьЭлементыОтборовВНастройкиСхемы(НастройкиКомпоновки,
					 Объект.ВидОбъектаСтрокой);
			ИначеЕсли Объект.ТипПроцесса = Перечисления.bpmТипыПроцессов.НезависимыйПроцесс Тогда
	...
	Иначе
		...
		КонецЕсли;
	КонецЕсли;
	АдресХранилищаНастройкиКомпоновщика = ПоместитьВоВременноеХранилище(КомпоновщикНастроек.ПолучитьНастройки(),
...
Функция ПолучитьАдресаСхемыКомпоновкиДанныхВоВременномХранилище()
	...
	Адреса.НастройкиКомпоновкиДанных = ПоместитьВоВременноеХранилище(КомпоновщикНастроек.ПолучитьНастройки(),
		 Новый УникальныйИдентификатор());
...
Процедура ОбновитьЭлементыОтбора()
	...
	Для Ном = 1 По 3 Цикл
		Если Элементы.Найти("Использование" + Строка(Ном)) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ДобавитьОтборНаФОрму(ИндексЭлемента, ИндексОтбора)
	...
	НоваяГруппа = Элементы.Добавить("ФлагЛевоеЗначение" + Строка(ИндексЭлемента), Тип("ГруппаФормы"),
		 Элементы.ЛевоеЗнчение);
...
Процедура ОбновитьСписокСвойствТекущегоНабора()
	...
	|	Свойства.ВладелецДополнительныхЗначений КАК ВладелецДополнительныхЗначений,
	|	Свойства.ЗаполнятьОбязательно КАК ЗаполнятьОбязательно,
...
Процедура ПолучитьНастройкиСвойств()
	...
	|	bpmДополнительныеРеквизитыНастройки.Свойство КАК Свойство,
	|	bpmДополнительныеРеквизитыНастройки.Настройки КАК Настройки
	|ИЗ
...
&НаСервере
Процедура ЗагрузитьНастройкиЭтапов()
...
Процедура ОбработчикОжиданияСвойстваПриАктивизацииСтроки()
	...
	CRM_ОбщегоНазначенияКлиентСервер.ОбновлениеТаблицыФормы(Элементы.НастройкиСвойств);
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ТекущийОбъект.ТипПроцесса = Перечисления.bpmТипыПроцессов.НезависимыйПроцесс Тогда
		...
		ТекущийОбъект.ХранилищеНастроекКомпоновкиДанных =
			Новый ХранилищеЗначения(ПолучитьИзВременногоХранилища(АдресХранилищаНастройкиКомпоновщика));
	Иначе
	...
	Если ЗавершениеРаботы Тогда
		...
		Если Объект.Редактируется Тогда
	        ТекстПредупреждения = ТекстПредупреждения 
	        	+ НСтр("ru='Карта маршрута находится в режиме редактирования. ';en='Route map is in edit mode. '");
		КонецЕсли;
		...
		Если НЕ ПустаяСтрока(ТекстПредупреждения) Тогда
	        Отказ = Истина;
			ТекстПредупреждения = ТекстПредупреждения 
				+ НСтр("ru='При закрытии формы все данные будут утеряны. Продолжить?';en='When you close form,
	...
	Если НЕ (ОтветПередЗакрытием = Ложь) Тогда
		...
		Если Объект.Редактируется Тогда
			...
			ПоказатьВопрос(ОбратныйВызов,
				 НСтр("ru='Внимание! Карта маршрута находится в режиме редактирования. Продолжить c закрытием формы?';
				|en='Attention! The route map is in edit mode. Continue with closing the form?'"),
...
Процедура ПередЗакрытиемЗавершение(Результат, ДополнительныеПараметры) Экспорт
	Если Результат = КодВозвратаДиалога.Да Тогда
		Если Не ТолькоПросмотр И Объект.Редактируется Тогда
	...
	Если ПроверитьНаНаличиеИзменений() Тогда 
		...
		ПоказатьВопрос(ОбратныйВызов,
			 Нстр("ru='Форма была изменена. Сохранить изменения?';en='The form was changed. Save changes?'"),
			 РежимДиалогаВопрос.ДаНет);
	Иначе
...
Процедура КартыМаршрутаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ОткрытьФорму("Справочник.CRM_КартыМаршрутов.Форма.ФормаВерсии", СтруктураПараметров, Элемент, , , ,
		 ОписаниеОповещения,
...
Процедура ЗагрузитьИзФайлаНаДиске(Команда)
	...
	Если РасширениеПодключено Тогда
		...
		Если Объект.Редактируется Тогда
			ВыборФайла.Фильтр =
				НСтр("ru='Графическая схема (.grs) (*.grs)|*.grs';en='The graphical scheme (.grs) (* .grs) | * .grs'");
		Иначе
			ВыборФайла.Фильтр =
...
"Editing is not possible!'"));
	...
	Если СтруктураДанных.НеКритичныеОшибки.Количество() > 0 Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='При проверке карты маршрута обнаружено'") + " " 
			+ Строка(СтруктураДанных.НеКритичныеОшибки.Количество()) 
			+ НСтр("ru=' предупреждений. |Выполнить обновление точек маршрута?'"), РежимДиалогаВопрос.ДаНет);
	Иначе
...
Процедура ЗакончитьРедактированиеЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ДатаНовойВерсии = ОбщегоНазначенияКлиент.ДатаСеанса();
	Объект.НомерВерсии = НомерНовойВерсии;
	
	// Запишем или обновим точки маршрута.
	СформироватьТочкиМаршрута();
	
	НовыеВерсииТочекМаршрута = ПолучитьМассивТочекМаршрута();
	Для Каждого ТекТочка Из НовыеВерсииТочекМаршрута Цикл
		ОповеститьОбИзменении(ТекТочка);
	КонецЦикла;
...
Процедура ОтменитьРедактирование(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Будут отменены все изменения,
		| сделанные в карте маршрута. Продолжить?'"),
		 РежимДиалогаВопрос.ДаНет);
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	// в регистре сведений относительно это версии
		...
		Для Каждого СтрокаСвойства Из Свойства Цикл
			...
			МассивСтрок				= НастройкиСвойствЗнач.НайтиСтроки(Новый Структура("Свойство,ЭтоШапка",
			...
			МассивСтрок		= НастройкиСвойствЗнач.НайтиСтроки(Новый Структура("Свойство,Этап",
				 СтрокаСвойства.Свойство,
				 Справочники.CRM_ТочкиМаршрутов.ПустаяСсылка()));
			СтрокаНастройки	= НастройкиСвойств.Добавить();
			Если МассивСтрок.Количество() > 0 Тогда
				ЗаполнитьЗначенияСвойств(СтрокаНастройкиШапка, МассивСтрок[0]);
			Иначе
...
Процедура ОбработатьОповещение(Параметр)
	...
	Если Параметр.Ссылка.НаборСвойств = Объект.НаборСвойств
		
		 	 ИЛИ Параметр.Ссылка.НаборСвойств = УправлениеСвойствами.НаборСвойствПоИмени("Справочник_CRM_КартыМаршрутов_Общие") Тогда
	...
	Если ЗначениеЗаполнено(Объект.ВидОбъектаСтрокой) Тогда
		Если НЕ Объект.ВидОбъектаСтрокой = "CRM_Интерес" Тогда
			...
			Для Каждого МетаРекизит Из ОбъектМетаданных.Реквизиты Цикл
				...
				Если МетаРекизит.Тип.Типы().Количество() > 1 Тогда
					Продолжить;
				КонецЕсли;
			...
			Если НЕ ОбъектМетаданных.ТабличныеЧасти.Найти("ДополнительныеРеквизиты") = Неопределено Тогда
				...
				НаборыСвойствОбъекта = УправлениеСвойствамиСлужебный.ПолучитьНаборыСвойствОбъекта(МенеджерОбъекта.ПустаяСсылка(),
					 КлючНазначения);
				...
				Для Каждого СтрокаТаблицы Из ОписаниеСвойств Цикл
					Если СтрокаТаблицы.Свойство.ЭтоДополнительноеСведение ИЛИ СтрокаТаблицы.Удалено
						 ИЛИ НЕ СтрокаТаблицы.Виден Тогда Продолжить; КонецЕсли;
					
					Если СтрокаТаблицы.ТипЗначения.Типы().Количество() > 1 Тогда
						Продолжить;
					КонецЕсли;
					...
					Если ОбщегоНазначения.ЭтоСправочник(МетаОбъект)
						ИЛИ ОбщегоНазначения.ЭтоПеречисление(МетаОбъект) Тогда
						Элементы.РеквизитОбъектаСтрокой.СписокВыбора.Добавить("%Свойство%" 
							+ Строка(СтрокаТаблицы.Свойство.УникальныйИдентификатор()), "Дополнительный реквизит: " + СтрокаТаблицы.Наименование);
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
		Иначе
...
Процедура ИспользоватьПриПринятииОбращенияПриИзменении(Элемент)
	Элементы.ПоляОтборов.Видимость	= (Объект.ТипПроцесса = ПредопределенноеЗначение("Перечисление.bpmТипыПроцессов.НезависимыйПроцесс"))
		 И Объект.ИспользоватьПриПринятииОбращения;
КонецПроцедуры
...
Процедура СхемаМаршрутаПриИзменении(Элемент)
	Если РедактируемыйЭлементСхемы <> Неопределено Тогда
		Если РедактируемыйЭлементСхемы.Имя = ИмяТекущегоЭлементаСхемы Тогда
			Если ТипЗнч(РедактируемыйЭлементСхемы) = Тип("ЭлементГрафическойСхемыДействие") Тогда
				РедактируемыйЭлементСхемы.Наименование = Лев(РедактируемыйЭлементСхемы.Наименование, 150);
			КонецЕсли;
		Иначе
