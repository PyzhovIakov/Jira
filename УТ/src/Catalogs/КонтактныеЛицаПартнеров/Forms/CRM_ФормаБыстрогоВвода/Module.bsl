...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ПараметрыНового") Тогда
		Если Параметры.ПараметрыНового.Свойство("ЭтоКонтактныйКлиент")
			 И Параметры.ПараметрыНового.ЭтоКонтактныйКлиент = Истина Тогда
			Заголовок						= НСтр("ru='Регистрация новой связи с юридическим лицом';
				|en='Logging of new communication with the legal entity'");
		...
		Иначе
			...
			Если Параметры.ПараметрыНового.Свойство("Телефон") Тогда
				СтруктураТелефона 			=
					сфпСофтФонПроСервер.сфпЗаполнитьСтруктуруПолейПоПредставлениюТелефон(Параметры.ПараметрыНового.Телефон);
				КодСтраныКонтактногоЛица 	= СтруктураТелефона.КодСтраны;
				КодГородаКонтактногоЛица 	= СтруктураТелефона.КодГорода;
				ТелефонКонтактногоЛица		= СтруктураТелефона.НомерТелефона;
				ДобНомерКонтактногоЛица		= СтруктураТелефона.Добавочный;
			КонецЕсли;
			Если ЗначениеЗаполнено(Наименование) Тогда
				ФИО_Темп = Наименование;
				...
				ФИО_Темп = СтрЗаменить(ФИО_Темп, "[", "");
				ФИО_Темп = СтрЗаменить(ФИО_Темп, "]", "");
				Фамилия = CRM_КлиентыСервер.ВыделитьЧастьНаименованияКонтактногоЛица(ФИО_Темп);
				Имя = CRM_КлиентыСервер.ВыделитьЧастьНаименованияКонтактногоЛица(ФИО_Темп);
				Отчество = CRM_КлиентыСервер.ВыделитьЧастьНаименованияКонтактногоЛица(ФИО_Темп);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	// +Софтфон
	Если Параметры.Свойство("Основание") И ТипЗнч(Параметры.Основание) = Тип("Структура") Тогда
		...
		КонецЕсли;	
		Клиент = ?(Параметры.Основание.Свойство("Владелец"), Параметры.Основание.Владелец,
...
Функция ПроверкаЗаполнения()
	...
	Если НЕ ЗначениеЗаполнено(Наименование) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заполнено наименование';
			|en='The name is not filled in'"), ,
			 "Наименование");
		ОшибокНет = Ложь;
	КонецЕсли;
...
Функция СоздатьНовоеКонтактноеЛицо()
		...
		Если ЗначениеЗаполнено(ВидТелефонаКонтактногоЛица) И ЗначениеЗаполнено(ТелефонКонтактногоЛица) Тогда
			...
			Если Не ПустаяСтрока(КодГородаКонтактногоЛица) Тогда
				Представление = Представление + ?(ПустаяСтрока(Представление), "", " ") + "(" 
					+ СокрЛП(КодГородаКонтактногоЛица) 
					+ ")";
			КонецЕсли;
			...
			Если НЕ ПустаяСтрока(ДобНомерКонтактногоЛица) Тогда
				Представление = Представление + ?(ПустаяСтрока(Представление), "", ", ") 
					+ НСтр("ru='доб. ';en='Ext. '") 
					+ СокрЛП(ДобНомерКонтактногоЛица);
			КонецЕсли;
	...
	Попытка
		...
		Если ЭтоКонтактныйКлиент И ЗначениеЗаполнено(КонтактныйКлиент) Тогда
			РегистрыСведений.CRM_СвязиФизЛицСКонтактнымиЛицамиПартнеров.ЗаписатьФизЛицо(НовоеКонтактноеЛицо.Ссылка,
				 КонтактныйКлиент);
		КонецЕсли;
	Исключение
