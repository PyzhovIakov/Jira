...
Процедура ПередЗаписью(Отказ)
	...
	Если ЭтоНовый() И ЗначениеЗаполнено(КартаМаршрута) И НЕ ОбменДанными.Загрузка Тогда
		...
		ПланируемаяДатаЗавершения = CRM_БизнесПроцессыСервер.РассчитатьПлановуюДатуЗавершенияБП(НомерВерсииКартыМаршрута,
			 Организация, ТочкаСтарта, Проект, КартаМаршрута,
			 ДатаСтарта);
	...
	мСоздатьОповещениеОЗавершении = (Завершен И (НЕ Ссылка.Завершен)
		 И (НЕ Ответственный = CRM_ОбщегоНазначенияСервер.ТекущийПользователь()));
	
	// Эмуляция подписки на событие "УстановитьПометкуУдаленияПрисоединенныхФайловДокументов".
	РаботаСФайлами.УстановитьПометкуУдаленияФайловДокументовПередЗаписью(ЭтотОбъект, Ложь,
...
Процедура ПриЗаписи(Отказ)
	...
	Если мТребуетсяСохранитьНастройкиЭтапов Тогда
		...
		Если ЗначениеЗаполнено(ВедущаяЗадача) Тогда
			мКорректироватьДлительностьВедущего =
				ВедущаяЗадача.БизнесПроцесс.КартаМаршрута.ДинамическиРассчитыватьСрокиВыполненияЭтапов;
		КонецЕсли;
		
		CRM_БизнесПроцессыСервер.ЗаполнитьНастройкиЭтаповБизнесПроцесса(Ссылка, Отказ,
			 мКорректироватьДлительностьВедущего,
			 мПараметрыПериодическогоСтарта);
	...
	Если НЕ Отказ И мСоздатьОповещениеОЗавершении Тогда
		ПараметрыОповещения = CRM_ОповещенияСервер.ПолучитьПараметрыОповещения(Ответственный,
			 Справочники.CRM_ВидыОповещений.ЗавершениеБизнесПроцесса,
			 Ссылка);
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ПодставлятьОсновнойПроектВПоручения =
		CRM_ОбщегоНазначенияПовтИсп.ПолучитьЗначениеНастройки("ПодставлятьОсновнойПроектВПоручения");
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
		...
		ИмяСвойстваКартаМаршрута = "КартаМаршрута";
		
		Если ДанныеЗаполнения.Свойство("Основание") И ДанныеЗаполнения.Свойство(ИмяСвойстваКартаМаршрута) Тогда
			...
			ИначеЕсли CRM_ОбщегоНазначенияПовтИсп.ЭтоCRM()
				 И ТипЗнч(ДанныеЗаполнения.Основание) = Тип("ДокументСсылка." + "CRM_СчетНаОплатуПокупателю") Тогда
			...
			ИначеЕсли ТипЗнч(ДанныеЗаполнения.Основание) = Тип("СправочникСсылка.CRM_ПотенциальныеКлиенты") Тогда
				
				ПотенциальныйКлиент = ДанныеЗаполнения.Основание;
			...
			ИначеЕсли ТипЗнч(ДанныеЗаполнения.Основание) = Тип("ДокументСсылка.CRM_Заявка") Тогда
				
				ДокументЗаявка = ДанныеЗаполнения.Основание;
				
				Наименование = ДокументЗаявка.Тема;
				
				Если Не ЗначениеЗаполнено(Партнер) И ТипЗнч(ДокументЗаявка.Контакт) = Тип("СправочникСсылка.Партнеры") Тогда
					Партнер = ДокументЗаявка.Контакт;
				ИначеЕсли Не ЗначениеЗаполнено(Партнер) И ТипЗнч(ДокументЗаявка.Контакт) = Тип("СправочникСсылка.CRM_ПотенциальныеКлиенты") Тогда
					ПотенциальныйКлиент = ДокументЗаявка.Контакт;
				ИначеЕсли Не ЗначениеЗаполнено(КонтактноеЛицо) И ТипЗнч(ДокументЗаявка.Контакт) = Тип("СправочникСсылка.КонтактныеЛицаПартнеров") Тогда
					КонтактноеЛицо = ДокументЗаявка.Контакт;
					Партнер = ДокументЗаявка.Контакт.Владелец;
				КонецЕсли;
		...
		ИначеЕсли ДанныеЗаполнения.Свойство(ИмяСвойстваКартаМаршрута) Тогда
		...
		ИначеЕсли ДанныеЗаполнения.Свойство(ИмяСвойстваКартаМаршрута) Тогда
			Описание = КартаМаршрута.ОписаниеДляБП;
		КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ Стартован И ДатаСтарта < ТекущаяДатаСеанса() И ДатаСтарта <> Дата('00010101')
		 И (НачалоДня(ДатаСтарта) <> НачалоДня(Дата)) Тогда
		
		ТекстСообщения = НСтр("ru='Дата старта не может быть меньше текущей даты.';
			|en='Start date must not be earlier than current date!'");
...
Процедура ЗадачаБизнесПроцессаПриСозданииЗадач(ТочкаМаршрутаБизнесПроцесса, ФормируемыеЗадачи, Отказ)
	...
	Если мПараметрыПериодическогоСтарта = Неопределено Тогда
		МассивИсходящихТочек = CRM_БизнесПроцессыЭкспортныеМетоды.ПолучитьСледующиеТочкиМаршрута(Ссылка,
			 ТочкаСтарта, , , , ,
			 Истина);
	Иначе
	...
	ДополнительныеСвойства.Свойство("ОбъектПроцесса", ОбъектПроцесса);
	CRM_БизнесПроцессыСервер.СформироватьМассивЗадач(Ссылка, ТочкаМаршрутаБизнесПроцесса,
		 МассивИсходящихТочек, ФормируемыеЗадачи_CRM, , ТаблицаЭтапов, Отказ, , , ,
		 ОбъектПроцесса);
...
Процедура ЗаполнитьПоЗадаче(Основание)
	...
	Если ЗначениеЗаполнено(Основание.БизнесПроцесс) И ЗначениеЗаполнено(Основание.БизнесПроцесс.Предмет) Тогда
		...
		ИначеЕсли CRM_ОбщегоНазначенияПовтИсп.ЭтоCRM()
			 И ТипЗнч(Основание.БизнесПроцесс.Предмет) = Тип("ДокументСсылка." + "CRM_СчетНаОплатуПокупателю") Тогда
	...
	Иначе	
		...
		Если ЗначениеЗаполнено(Основание.БизнесПроцесс)
			 И ТипЗнч(Основание.БизнесПроцесс) = Тип("БизнесПроцессСсылка.CRM_БизнесПроцесс") Тогда
...
Процедура ЗаполнитьПоИнтересу(Основание)
	...
	Если ЗначениеЗаполнено(Основание.Партнер) Тогда
		Партнер             = Основание.Партнер;
		КонтактноеЛицо      = Основание.КонтактноеЛицо;
	Иначе
		ПотенциальныйКлиент = Основание.ПотенциальныйКлиент;
	КонецЕсли;
...
Процедура ЗаполнитьПоСчетуНаОплатуПокупателю(Основание)
	...
	Сумма = РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(Основание.СуммаДокумента,
...
Процедура ЗаполнитьПоМаркетинговомуМероприятию(Основание)
	...
	Сумма = РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(Основание.CRM_ПлановаяСуммаПродаж,
		 СтруктураКурсовСтаройВалюты,
		 СтруктураКурсовНовойВалюты);
