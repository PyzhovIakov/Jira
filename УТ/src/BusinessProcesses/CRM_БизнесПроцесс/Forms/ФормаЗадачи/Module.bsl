...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СтрНайти(Строка(Объект.БизнесПроцесс), НСтр("ru='<Объект не найден>';en='<Object not found>'")) > 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Нстр("ru='У пользователя нет доступа к этой задаче!';
			|en='The user does not have access to this task!'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) И ЗначениеЗаполнено(Объект.БизнесПроцесс) Тогда
		ВидКонтроляЗадачиПоИсправлениюКлиентскойБазы =
			РегистрыСведений.CRM_ЗадачиПоИсправлениюКлиентскойБазы.ЭтоЗадачаПоИсправлениюКлиентскойБазы(Объект.Ссылка);
	...
	Если ЗначениеЗаполнено(Объект.БизнесПроцесс) Тогда
		ДинамическиРассчитыватьСрокиВыполненияЭтапов =
			Объект.БизнесПроцесс.КартаМаршрута.ДинамическиРассчитыватьСрокиВыполненияЭтапов;
	КонецЕсли;
	...
	Если ТипЗнч(Исполнитель) = Тип("СправочникСсылка.РолиИсполнителей") Тогда
		СписокВозможныхИсполнителей = CRM_БизнесПроцессыСервер.ПолучитьВозможныхИсполнителейПоРоли(Исполнитель,
			 Объект.Ссылка);
	Иначе
	...
	Если ЗначениеЗаполнено(Объект.БизнесПроцесс) Тогда
		...
		Элементы.ФормаЗавершитьБизнесПроцессДосрочно.Видимость =
			Объект.БизнесПроцесс.КартаМаршрута.РазрешеноДосрочноеЗавершение;
	...
	Элементы.ПеренестиСрок.Видимость		= ЭтоПоручение И РедактироватьСрокВыполненияЗадач
		 И НЕ ЗначениеЗаполнено(Объект.CRM_ПеренестиСрокИсполненияНа);
	...
	Элементы.ГруппаОтложитьЭтапДатаВыполнения.Видимость	= (Элементы.ДатаИсполнения.Видимость
		 ИЛИ Элементы.ГруппаОтложить.Видимость);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ОбновленыДанныеЭлектронногоПисьмаВходящего"
		 Или ИмяСобытия = "ОбновленыДанныеТелефонногоЗвонка" Тогда
		СсылкаНаОбъект = Параметр;
	КонецЕсли;		

	Если Элементы.ПанельОсновная.Видимость Тогда
		Если	(ИмяСобытия = "ЗаписьПартнера" Или ИмяСобытия = "НовоеКонтактноеЛицоЗаписано"
				Или ((ИмяСобытия = "ОбновленыДанныеСобытия"
					 Или ИмяСобытия = "ОбновленыДанныеЭлектронногоПисьмаВходящего"
					 Или ИмяСобытия = "ОбновленыДанныеТелефонногоЗвонка") 
	...
	Если ИмяСобытия = "ЗадачаПеренаправлена" И ТипЗнч(Параметр.МассивЗадач) = Тип("Массив")
		 И (НЕ Параметр.МассивЗадач.Найти(Объект.Ссылка) = Неопределено) Тогда
		Закрыть();
	КонецЕсли;
...
Процедура ОтложитьНачалоЭтапаПриИзменении(Элемент)
	...
	Иначе
		ОтложитьНаДату = НачалоДня(ОбщегоНазначенияКлиент.ДатаСеанса()) + (ОтложитьНачалоЭтапаДней * 86400) 
			+ (ОтложитьНачалоЭтапаЧасов * 3600) 
			+ (ОтложитьНачалоЭтапаМинут * 60);
	КонецЕсли;
...
Процедура ОтложитьНачалоЭтапаДнейОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ВыбранноеЗначение = 0 Тогда
		...
		ПоказатьВводДаты(Оповещение, ?(ЗначениеЗаполнено(ОтложитьНаДату), ОтложитьНаДату,
			 ДатаСеанса),
			 "Укажите дату",
			 ЧастиДаты.Дата);
		Возврат;
...
Процедура ОтложитьНаДатуНажатие(Элемент, СтандартнаяОбработка)
	...
	Оповещение = Новый ОписаниеОповещения("ВыполнитьПослеВыбораДаты", ЭтотОбъект);
	ПоказатьВводДаты(Оповещение, ?(ЗначениеЗаполнено(ОтложитьНаДату), ОтложитьНаДату, ОбщегоНазначенияКлиент.ДатаСеанса()),
		 "Укажите дату",
		 ЧастиДаты.Дата);
	Возврат;
...
Процедура ВвестиОбъект(Команда)
	...
	Если СтрНайти(Команда.Имя, "Команда_Ввести_Документ_") > 0 Тогда
		...
		Если ИмяДокумента = СтрЗаменить(ИмяМетаданных("Документы", "ЭлектронноеПисьмоВходящее"), ".", "_") Тогда
			ПоказатьПредупреждение(,
				 НСтр("ru='Создание Входящих писем запрещено. Воспользуйтесь кнопкой ""Выбрать...""';
				|en='Incoming mail is not allowed. Use the ""Select ..."" button'"));
			Возврат;
		КонецЕсли;
		...
		Иначе
			...
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли СтрНайти(Команда.Имя, "Команда_Ввести_Справочник_") > 0 Тогда
	...
	ИначеЕсли СтрНайти(Команда.Имя, "Команда_Добавить_Документ_") > 0 Тогда
		...
		Иначе
			...
			ОткрытьФорму("Документ." + ИмяДокумента + ".ФормаОбъекта", ПараметрыФормы, ЭтотОбъект);
		КонецЕсли;
	ИначеЕсли СтрНайти(Команда.Имя, "Команда_Добавить_Справочник_") > 0 Тогда
...
Процедура ВыбратьОбъект(Команда)
	...
	Если СтрНайти(Команда.Имя, "Команда_Выбрать_Документ_") > 0 Тогда
		...
		Иначе
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ФормаВыбора, "ИмяРеквизитаСписка") Тогда
				СписокИмяРеквизитаФормы = ФормаВыбора["ИмяРеквизитаСписка"];
			Иначе
				СписокИмяРеквизитаФормы = "Список";	
			КонецЕсли;			
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ФормаВыбора, СписокИмяРеквизитаФормы) Тогда
	...
	ИначеЕсли СтрНайти(Команда.Имя, "Команда_Выбрать_Справочник_") > 0 Тогда
...
Процедура ПринятьКИсполнениюЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	бТолькоПросмотр = (НЕ Объект.ПринятаКИсполнению ИЛИ НЕ ЗначениеЗаполнено(Объект.Исполнитель)
		 ИЛИ ЗначениеЗаполнено(ВложенныйБизнесПроцесс));
...
Процедура СоздатьПоручениеЗавершение(Продолжать, ДополнительныеПараметры) Экспорт
	...
	Если Продолжать Тогда
		СтруктураОснования = Новый Структура("Основание,КартаМаршрута", Объект.Ссылка,
...
Процедура ПоказыватьСкрыватьДопИнформацию(Команда)
	...
	CRM_ХранилищеНастроек.Сохранить("ПоказыватьСкрыватьПанельИнформацииЗадачаБП", ,
		 Элементы.ПоказыватьСкрыватьПанельИнформации.Пометка); 
КонецПроцедуры
...
Процедура ПересчитатьСрокиИсполнения()
	...
	ТаблицаЭтапов = CRM_БизнесПроцессыСервер.ПересчитатьСрокиИсполненияЗадачБизнесПроцесса(Объект.Ссылка,
	...
	Если ЗначениеЗаполнено(Объект.БизнесПроцесс.ВедущаяЗадача) Тогда
		...
		ТаблицаЭтаповВложенного = CRM_БизнесПроцессыСервер.ПересчитатьСрокиИсполненияЗадачБизнесПроцесса(Объект.БизнесПроцесс.ВедущаяЗадача,
			 ТаблицаЭтапов[0].ДатаВыполнения);
...
Процедура ЗаполнитьДеревоКлиенты()
	...
	Иначе
		...
		|		НЕОПРЕДЕЛЕНО КАК Контакт
		|	ИЗ
		|		РегистрСведений.CRM_ЗадачиПоИсправлениюКлиентскойБазы КАК Таблица
		|	ГДЕ
		|		Таблица.БизнесПроцесс = &БизнесПроцесс
		|		И Таблица.Партнер ССЫЛКА Справочник.Партнеры
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|	ГДЕ
		|		Таблица.БизнесПроцесс = &БизнесПроцесс
		|		И Таблица.Партнер ССЫЛКА Документ.ТелефонныйЗвонок
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|	ГДЕ
		|		Таблица.БизнесПроцесс = &БизнесПроцесс
		|		И Таблица.Партнер ССЫЛКА Документ.ЭлектронноеПисьмоВходящее
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|	ГДЕ
		|		Таблица.БизнесПроцесс = &БизнесПроцесс
		|		И Таблица.Партнер ССЫЛКА Справочник.КонтактныеЛицаПартнеров) КАК Подзапрос
		|
		|УПОРЯДОЧИТЬ ПО
		|	Подзапрос.Партнер.Наименование,
		|	Подзапрос.КонтактноеЛицо.Наименование
		|ИТОГИ ПО
		|	Партнер,
		|	КонтактноеЛицо");
		Запрос.УстановитьПараметр("БизнесПроцесс", Объект.БизнесПроцесс);
...
Функция ДеревоКлиентовЗавершеноПриИзмененииНаСервере(СтруктураКлюч, Знач ФлагЗавершено)
	...
	Исключение
		...
		ОбщегоНазначения.СообщитьПользователю(СтрОшибка);
		Возврат Ложь;
	КонецПопытки;
...
Процедура ЗаписатьЗаполненностьРеквизитовВРегистр(ЭлементПартнерКЛ, ПартнерИлиКЛ)
	...
	ИначеЕсли	ВидКонтроляЗадачиПоИсправлениюКлиентскойБазы = Перечисления.CRM_ВидыМастераПроверкиКлиентскойБазы.КонтрольРегистрацииКлиентов
		...
		Если ТипЗнч(ПартнерИлиКЛ) = Тип("ДокументСсылка.ЭлектронноеПисьмоВходящее") Тогда
			бВсеРеквизитыЗаполнены = (ЗначениеЗаполнено(ПартнерИлиКЛ.ОтправительКонтакт)
				 И ТипЗнч(ПартнерИлиКЛ.ОтправительКонтакт) <> Тип("Строка"));
		...
		Для Каждого ЭлементРеквизит Из ЭлементыРеквизиты Цикл
			Если	ТипЗнч(ЭлементРеквизит.Партнер) = Тип("СправочникСсылка.ВидыКонтактнойИнформации")
				...
				Если ТипЗнч(ЭлементРеквизит.Партнер) = Тип("Строка") Тогда
					РеквизитЗаполнен = CRM_ОбщегоНазначенияСервер.ПроверитьЗаполнениеРеквизитаОбъектаКлиентскойБазы(ПартнерИлиКЛ,
						 ЭлементРеквизит.Реквизит);
				Иначе
					РеквизитЗаполнен = CRM_ОбщегоНазначенияСервер.ПроверитьЗаполнениеРеквизитаОбъектаКлиентскойБазы(ПартнерИлиКЛ,
						 ЭлементРеквизит.Партнер);
				КонецЕсли;
...
Процедура ДобавлениеКнопокНаПанельОбъектов()
	...
	Если ТаблицаТипов.Количество() = 1 Тогда
		...
		НовыйПунктМеню = Элементы.Вставить(ИмяКнопкиВвести, Тип("КнопкаФормы"),
			 Элементы.ОбъектыПоБизнесПроцессам.КоманднаяПанель,
			 Элементы.ОбъектыПоБизнесПроцессамУдалить);
		...
		НовыйПунктМеню = Элементы.Вставить(ИмяКнопкиВыбрать, Тип("КнопкаФормы"),
			 Элементы.ОбъектыПоБизнесПроцессам.КоманднаяПанель,
			 Элементы.ОбъектыПоБизнесПроцессамУдалить);
		...
		НовыйПунктМеню = Элементы.Вставить(ИмяКнопкиДобавить, Тип("КнопкаФормы"),
			 Элементы.ОбъектыПоБизнесПроцессам.КоманднаяПанель,
			 Элементы.ОбъектыПоБизнесПроцессамУдалить);
	...
	ИначеЕсли ТаблицаТипов.Количество() > 1 Тогда
		...
		ПунктМенюВвести		= Элементы.Вставить("Кнопка_Ввести", Тип("ГруппаФормы"),
			 Элементы.ОбъектыПоБизнесПроцессам.КоманднаяПанель,
			 Элементы.ОбъектыПоБизнесПроцессамУдалить);
		...
		ПунктМенюВыбрать	= Элементы.Вставить("Кнопка_Выбрать", Тип("ГруппаФормы"),
			 Элементы.ОбъектыПоБизнесПроцессам.КоманднаяПанель,
			 Элементы.ОбъектыПоБизнесПроцессамУдалить);
		ПунктМенюВыбрать.Заголовок = НСтр("ru='Выбрать документ'");
		
		ПунктМенюДобавить = Элементы.Вставить("Кнопка_Добавить", Тип("ГруппаФормы"),
			 Элементы.ОбъектыПоБизнесПроцессам.КоманднаяПанель,
			 Элементы.ОбъектыПоБизнесПроцессамУдалить);
...
Процедура ВыполнениеЗадачиПоУсловиюЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(Объект.БизнесПроцесс) Тогда
		...
		Если НЕ ПродолжитьВыполнение Тогда
			ТекстПредупреждения = НСтр("ru='Невозможно выполнить, так как задача: '") + Результат.ПредставлениеЗадачи 
...
Процедура ВыполнитьПослеВыбораДаты(ВыбраннаяДата, Параметры) Экспорт
	...
	Если ЗначениеЗаполнено(ВыбраннаяДата) Тогда
		...
		Если ВыбраннаяДата < ОбщегоНазначенияКлиент.ДатаСеанса() Тогда
		...
		Иначе
			...
			ОтложитьНачалоЭтапаДней	= (ДеньГода(ВыбраннаяДата) - ДеньГода(ОбщегоНазначенияКлиент.ДатаСеанса()));
		КонецЕсли;
