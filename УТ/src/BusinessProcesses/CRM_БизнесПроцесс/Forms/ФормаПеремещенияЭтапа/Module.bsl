...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если CRM_ЛицензированиеЭкспортныеМетоды.ВариантПоставкиСтартИлиСтандарт() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Функция доступна только для ""ПРОФ"" и ""КОРП"" поставки конфигурации!';
...
Процедура КартаМаршрутаВыбор(Элемент)
	...
	Если (ТипЗнч(Элемент.ТекущийЭлемент) = Тип("ЭлементГрафическойСхемыДействие"))
		...
		ТекстВопроса = НСтр("ru='Установить';en='Set'") + " """ + ТекДанные.ТочкаМаршрута + """" 
			+ НСтр("ru=' как текущий этап бизнес-процесса?'") 
			+ "
		|Задачи текущих этапов будут выполнены автоматически.";
		ОписаниеОповещения = Новый ОписаниеОповещения("КартаМаршрутаВыборПродолжение", ЭтотОбъект,
			 Новый Структура("Элемент, МассивСтрок", Элемент,
			 МассивСтрок));
...
Функция ПрочитатьМаршрутБизнесПроцесса()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	Маршрут.ВариантВыполнения КАК ВариантВыполнения,
	               |	Маршрут.Используется КАК Используется,
	               |	Маршрут.Шаблон КАК ПроцессШаблон,
	               |	Маршрут.Порядок КАК Порядок,
...
Функция ПроверитьНаПринадлежностьРазделениюПолучитьТочкуРазделения(БизнесПроцесс, ТочкаМаршрута)
	...
	Если НЕ МассивСтрок.Количество() = 0 Тогда
		...
		МассивСтрок = ТочкаСлияния.ТочкиРазделенияСлияния.НайтиСтроки(Новый Структура("НомерВерсии, Вид",
			 БизнесПроцесс.НомерВерсииКартыМаршрута,
			 Перечисления.CRM_ВидыТочекМаршрута.Разделение));
...
Процедура ОбновитьКартуМаршрутаЗаполнитьТаблицу()
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		СтрокаСхемыМаршрута = Объект.Ссылка.КартаМаршрута.ВерсииКартыМаршрута.Найти(Объект.Ссылка.НомерВерсииКартыМаршрута,
			 "НомерВерсии");
	...
	Для Каждого СтрокаМаршрута Из Маршрут Цикл		
		...
		ТочкаРазделения = ПроверитьНаПринадлежностьРазделениюПолучитьТочкуРазделения(Объект.Ссылка,
			 СтрокаМаршрута.ТочкаМаршрута);
	...
	|	CRM_ТочкиМаршрутовИсходящиеТочки.Ссылка.Имя КАК Имя
	|ПОМЕСТИТЬ Точки
	|ИЗ
...
Процедура ЗавершитьВсеЗадачиБизнесПроцесса(БизнесПроцесс, МассивИсключаемыхТочек, ТочкаМаршрута = Неопределено)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Ссылка.CRM_ТочкаМаршрута.Вид = Перечисления.CRM_ВидыТочекМаршрута.ВложенныйБизнесПроцесс Тогда
			...
			ВложенныйБП = CRM_БизнесПроцессыСервер.ПолучитьПодчиненныйБизнесПроцесс(Выборка.Ссылка);
...
Процедура СоздатьЗадачуВТочкеОтката(БизнесПроцесс, ТочкаМаршрута)
	...
	Если Выборка.Следующий() Тогда
		...
		Попытка
			Если БизнесПроцесс.Завершен Тогда
				...
				БПОбъект.ЗавершенДосрочно = Ложь;
				БПОбъект.ДатаЗавершения = Дата(1,1,1);
				БПОбъект.ПричинаДосрочногоЗавершения = Справочники.CRM_ПричиныОтказов.ПустаяСсылка();
				БПОбъект.ЭтапДосрочногоЗавершения = Справочники.CRM_ТочкиМаршрутов.ПустаяСсылка();
				БПОбъект.Записать();
			КонецЕсли;
	...
	Иначе
		...
		CRM_БизнесПроцессыСервер.СформироватьМассивЗадач(БизнесПроцесс,
...
Функция ПолучитьИсходящиеТочкиРазделения(БизнесПроцесс, ТочкаМаршрута, МассивТочек = Неопределено)
	...
	МассивСтруктурТочек = CRM_БизнесПроцессыЭкспортныеМетоды.ПолучитьИсходящиеТочки(ТочкаМаршрута,
		 БизнесПроцесс.НомерВерсииКартыМаршрута);
...
Функция ПроверитьНаличиеСледующегоИсполнителяУЭтапов(МассивПроверяемыхЭтапов)
	...
	Возврат CRM_БизнесПроцессыСервер.ПроверитьНаличиеСледующегоИсполнителяУЭтапов(ОбъектЗнач.Ссылка,
		 МассивПроверяемыхЭтапов);
...
Процедура КартаМаршрутаВыборПродолжение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ИЛИ (ТипЗнч(ДополнительныеПараметры.Элемент.ТекущийЭлемент) = Тип("ЭлементГрафическойСхемыВложенныйБизнесПроцесс")) Тогда
		...
		МассивИсключаемыхТочек = ПроверитьНаПринадлежностьРазделениюПолучитьИсключаемыеТочки(Объект.Ссылка,
			 ДополнительныеПараметры.МассивСтрок[0].ТочкаМаршрута);
	...
	ИначеЕсли (ТипЗнч(ДополнительныеПараметры.Элемент.ТекущийЭлемент) = Тип("ЭлементГрафическойСхемыРазделение")) Тогда
		...
		МассивИсходящихТочек = ПолучитьИсходящиеТочкиРазделения(Объект.Ссылка,
			 ДополнительныеПараметры.МассивСтрок[0].ТочкаМаршрута);
...
Процедура КартаМаршрутаВыборЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если (ТипЗнч(ДополнительныеПараметры.Элемент.ТекущийЭлемент) = Тип("ЭлементГрафическойСхемыВложенныйБизнесПроцесс"))
		Или (ТипЗнч(ДополнительныеПараметры.Элемент.ТекущийЭлемент) = Тип("ЭлементГрафическойСхемыДействие")) Тогда
	...
	Иначе
		Оповестить("ВыполненОткатБизнесПроцесса", Объект.Ссылка);
	КонецЕсли;
