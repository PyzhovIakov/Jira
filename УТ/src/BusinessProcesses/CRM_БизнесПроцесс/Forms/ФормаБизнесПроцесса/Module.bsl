...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.КартаМаршрута) Тогда
		...
		Заголовок = Объект.КартаМаршрута.Наименование;
	...
	Если ЭтоНовыйОбъект Тогда
		...
		Если	Параметры.Свойство("Основание")
			И	ТипЗнч(Параметры.Основание) = Тип("СправочникСсылка.CRM_КартыМаршрутов")
			И	Параметры.Основание.ТипПроцесса = Перечисления.bpmТипыПроцессов.ПроцессОбъекта Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Для бизнес-процесса с типом ""Процесс объекта"" запрещено интерактивное создание.'"));
		...
		Если	Параметры.Свойство("Основание")
			...
			//			ТаблицаИсполнителей[ИндексСтроки].Исполнитель 				= ?(ЗначениеЗаполнено(СтрокаТЧ.Исполнитель),
				// СтрокаТЧ.Исполнитель,
				// ТаблицаИсполнителей[ИндексСтроки].Исполнитель);
			...
			Если Параметры.Основание.Свойство("КартаМаршрута")
				...
				КонецЕсли;
				Если ПризнакОтказыватьсяОтОткрытияФормыСОповещениемОЗаписиНового
					 И (Не ЗначениеЗаполнено(Объект.Ссылка)
					 Или Не Объект.Стартован) Тогда
	...
	Если ЗначениеЗаполнено(Объект.ГлавнаяЗадача) Тогда
		ОписаниеТочкиМаршрута	= CRM_БизнесПроцессыПереопределяемый.СформироватьИсториюБизнесПроцесса(Объект.ГлавнаяЗадача,
			 Истина);
	Иначе
...
Процедура ПриОткрытии(Отказ)
	...
	Если ПризнакОтказыватьсяОтОткрытияФормыСОповещениемОЗаписиНового Тогда
		...
		Отказ = Истина;
		Попытка
			Закрыть(); // Нужно, чтобы разблокировать объект, связанный с формой.
		Исключение 
			Возврат;
		КонецПопытки;
		Возврат;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ЗадачаВыполнена" Тогда
		Если Параметр.Свойство("СсылкаНаОбъект")
			 И ТипЗнч(Параметр.СсылкаНаОбъект) = Тип("ЗадачаСсылка.ЗадачаИсполнителя")
			 И ЗадачаПринадлежитБизнесПроцессу(Параметр, Объект.Ссылка) Тогда
			ЗаполнитьТаблицуИсполнителей();
			ПолучитьЗадачуБПДляИстории(); 			
		КонецЕсли;
	ИначеЕсли ИмяСобытия = "СохраненыНастройкиЭтаповВложенногоБизнесПроцесса" Тогда
		Если ТипЗнч(Параметр) = Тип("БизнесПроцессСсылка.CRM_БизнесПроцесс")
			 И БизнесПроцессПринадлежитБизнесПроцессу(Параметр, Объект.Ссылка) Тогда
			ЗаполнитьТаблицуИсполнителей();
		КонецЕсли;
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ИзмененаПричина Тогда
		...
		Если ЗаписьРегистра.Выбран() Тогда
			...
			ПредставлениеЭтапа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Завершен досрочно %1;
				| по причине: %2; на этапе : %3';en='It is completed ahead of schedule %1;
				| for the reason: %2; at a stage: %3'"),
	...
	CRM_БизнесПроцессыСервер.ЗаписатьИсториюРеквизитовБизнесПроцесса(ТекущийОбъект.Ссылка,
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Исполнители.Количество() > 0 Тогда
		Если ЗначениеЗаполнено(Объект.ТочкаСтарта) И ЗначениеЗаполнено(Объект.Проект) Тогда
			Попытка
				МассивИсходящихТочек = CRM_БизнесПроцессыЭкспортныеМетоды.ПолучитьСледующиеТочкиМаршрута(Объект.Проект,
					 Объект.ТочкаСтарта);
			Исключение
				МассивИсходящихТочек = Неопределено;
			КонецПопытки;
...
Процедура ПланируемаяДатаЗавершенияПриИзменении(Элемент)
	Если ЗначениеЗаполнено(Объект.ПланируемаяДатаЗавершения) Тогда
		Если НачалоДня(Объект.ПланируемаяДатаЗавершения) < НачалоДня(Объект.ДатаСтарта) Тогда
			СтрокаСообщения = Нстр("ru='Планируемая дата завершения не может быть меньше даты старты,
				| она будет скорректирована.';en='The planned completion date can not be less than the start date,
				| it will be adjusted.'");
...
Функция СформироватьСтрокуСостоянияБизнесПроцесса()
	...
	Иначе
		...
		ИначеЕсли Объект.ЗавершенДосрочно Тогда
			...
			СтрокаСостояния = СтрокаСостояния + "Завершен досрочно " + "(" + Объект.ВариантЗавершения 
				+ ") по причине: " 
				+ Строка(Объект.ПричинаДосрочногоЗавершения);
...
Процедура УстановитьДоступностьРеквизитов()
	...
	//Элементы.КартаМаршрута.ТолькоПросмотр	= ((ЗначениеЗаполнено(Объект.Проект)
		// И Проект_КартаМаршрута = Объект.КартаМаршрута)
		// ИЛИ Объект.Стартован)
	...
	Элементы.ФормаЗавершитьБизнесПроцессДосрочно.Видимость		= (РазрешеноДосрочноеЗавершение
		 И Объект.Стартован
		 И НЕ Объект.Завершен);
	Элементы.ФормаЗавершитьБизнесПроцессДосрочно.Доступность	= (РазрешеноДосрочноеЗавершение
		 И Объект.Стартован
		 И НЕ Объект.Завершен);
