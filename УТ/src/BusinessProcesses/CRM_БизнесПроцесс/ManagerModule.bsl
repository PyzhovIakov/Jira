	...
	Пока ЕстьСледующаяПорция Цикл
		Для СчетчикЭлементов = 0 По КонечнаяПозиция Цикл
			...
			//Если НЕ Элемент.Метаданные = Метаданные.Справочники.ФизическиеЛица Тогда
				Если НЕ ДобавитьБизнесПроцессВСписокНайденныхПолнотекстовымПоиском(СписокБизнесПроцессов,
					 БизнесПроцесс, Основание,
					 ЭлементСсылка) Тогда
					Возврат НСтр("ru='Слишком много результатов, уточните запрос.';en='It is too much results, specify query.'");					
				КонецЕсли;
			//КонецЕсли;
...
Функция ДобавитьБизнесПроцессВСписокНайденныхПолнотекстовымПоиском(СписокБизнесПроцессов,
	 БизнесПроцесс, Основание,
	 ЭлементСсылка)
...
Функция ВернутьСсылкуНаЗадачу(Объект)
	...
	|	Выполнена,
	|	ЗадачаПользователя,
	|	Задачи.Ссылка.ДатаИсполнения УБЫВ";
...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Ключ") Тогда
		...
		И ЗначениеЗаполнено(Параметры.Ключ.Интерес) Тогда
			...
			Если Метаданные.Документы.Содержит(Параметры.Ключ.Метаданные()) Тогда
				CRM_МетодыМодулейМенеджеровДокументов.ОбработкаПолученияФормОбъектовCRM(ИсточникНов, ВидФормы,
					 Параметры, ВыбраннаяФорма, ДополнительнаяИнформация,
					 СтандартнаяОбработка);
			Иначе
				CRM_МетодыМодулейМенеджеровСправочников.ОбработкаПолученияФормОбъектовCRM(ИсточникНов, ВидФормы,
					 Параметры, ВыбраннаяФорма, ДополнительнаяИнформация,
					 СтандартнаяОбработка);
			КонецЕсли;
	...
	ИначеЕсли Параметры.Свойство("ЗначенияЗаполнения")
		 ...
		 И Параметры.ЗначенияЗаполнения.Свойство("КартаМаршрута") Тогда
