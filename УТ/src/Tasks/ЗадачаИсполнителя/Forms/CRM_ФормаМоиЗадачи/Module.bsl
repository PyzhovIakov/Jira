...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	CRM_ОбщегоНазначенияСервер.НастройкиПолейОтображенияСодержанияПриСозданииФормыСпискаНаСервере(ЭтотОбъект,
		 , "ГруппаПолеОтображенияСодержания",
		 "КнопкаПоказатьСкрытьПолеОтображенияСодержания");
	CRM_ОбщегоНазначенияСервер.НастройкиПолейОтображенияСодержанияПриСозданииФормыСпискаНаСервере(ЭтотОбъект,
		 , "ГруппаПолеОтображенияСодержанияСегодня",
		 "СписокСегодняПоказатьСкрытьПолеОтображенияСодержанияСегодня");
...
Процедура ПриАктивизацииСтроки(Элемент)
	...
	Если Элементы["ГруппаПолеОтображенияСодержания" + СтрЗаменить(ИмяСписка, "Список", "")].Видимость Тогда
		// Поле отображения содержания.
		Если НЕ Элементы[ИмяСписка].ТекущиеДанные.Свойство("ГруппировкаСтроки")
			 И (ИмяСписка = "СписокСегодня" ИЛИ (ИмяСписка = "Список"
			 И Элементы[ИмяСписка].ТекущиеДанные.Личная)) Тогда
			CRM_ОбщегоНазначенияКлиент.НастройкиПолейОтображенияСодержанияПриАктивизацииСтроки(ЭтотОбъект,
				 Элементы[ИмяСписка].ТекущаяСтрока, Новый ОписаниеТипов("ЗадачаСсылка.ЗадачаИсполнителя"), ,
				 ИмяПоля);
		Иначе
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭтотОбъект, ИмяПоля) Тогда
				ЭтотОбъект[ИмяПоля] =
					CRM_БизнесПроцессыПереопределяемый.СформироватьОписаниеЗадачи(Элементы[ИмяСписка].ТекущаяСтрока);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	
	ПометкаУдаленияДоступна = Истина;
	Для Каждого ТекСтрока Из Элементы[ИмяСписка].ВыделенныеСтроки Цикл
		ДанныеСтроки = Элементы[ИмяСписка].ДанныеСтроки(ТекСтрока);
		Если ДанныеСтроки.Свойство("ГруппировкаСтроки") Или Не ДанныеСтроки.Личная Тогда
			ПометкаУдаленияДоступна = Ложь;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Элементы[ИмяСписка + "КонтекстноеМенюУстановитьПометкуУдаления"].Доступность = ПометкаУдаленияДоступна;
...
Процедура СоздатьБизнесПроцесс(Команда)
	ОткрытьФорму("БизнесПроцесс.CRM_БизнесПроцесс.Форма.ФормаВыборИзДерева", , ЭтотОбъект, ,
...
Процедура УстановитьПометкуУдаления(Команда)
	...
	ВыделенныеСтрокиСписка = ТекущийЭлемент.ВыделенныеСтроки;
	
	Если ВыделенныеСтрокиСписка.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ПараметрыОповещения = Новый Структура("ВыделенныеСтрокиСписка", ВыделенныеСтрокиСписка);
	ОписаниеОповещения = Новый ОписаниеОповещения("УстановитьПометкуУдаленияОтвет", ЭтотОбъект, ПараметрыОповещения);
	ТекстВопроса = НСтр("ru='Установить/снять пометку на удаление?';en='Set/remove delete mark?'");
...
Процедура УстановитьПометкуУдаленияОтвет(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		УстановитьПометкуНаСервере(ДополнительныеПараметры.ВыделенныеСтрокиСписка);
	КонецЕсли;
...
Процедура ОбновитьОтборыНаСервере(Список, ОтборПринадлежность, ОтборСтатус, ТекущийПользователь, ТекущееПодразделение)
	...
	Для Каждого ЭлементОтбора Из тОтбор Цикл
		Если ТипЗнч(ЭлементОтбора) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
			Если ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ИсполнительПотенциальный")
			...
			ИначеЕсли ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Автор") И ЭлементОтбора.Использование Тогда
				ВыбранАвтор = Истина;
			ИначеЕсли ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ИсполнительПотенциальный.ТекущееПодразделение")
				 И ЭлементОтбора.Использование Тогда
				ВыбранТекущееПодразделение = Истина;
			ИначеЕсли ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СрокИсполнения")
				 И ЭлементОтбора.Использование Тогда
			...
			ИначеЕсли ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ПринятаКИсполнению")
				 И ЭлементОтбора.Использование Тогда
	...
	Если ОтборПринадлежность = "Мои" И НЕ ВыбранИсполнитель Тогда
		CRM_ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораСписка(Список, "ИсполнительПотенциальный",
			 ТекущийПользователь); 
	...
	Если ОтборСтатус = "Просроченные" Тогда
		Если НЕ ВыбранСрокИсполнения Тогда
			CRM_ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораСписка(Список, "СрокИсполнения",
				 '00010101',
				 ВидСравненияКомпоновкиДанных.НеРавно); 
			CRM_ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораСписка(Список, "СрокИсполнения",
				 НачалоДня(CRM_ОбщегоНазначенияСервер.ПолучитьТекущуюДатуСеанса()),
	...
	ИначеЕсли ОтборСтатус = "ВРаботе" Тогда
		...
		ОбщаяГруппаОтбора	= CRM_ОбщегоНазначенияКлиентСервер.УстановитьГруппуОтбораСписка(Список,
			 ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли);
		ГруппаИ1			= CRM_ОбщегоНазначенияКлиентСервер.УстановитьГруппуОтбораСписка(Список,
			 ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ,
			 ОбщаяГруппаОтбора); 
		ГруппаИ2			= CRM_ОбщегоНазначенияКлиентСервер.УстановитьГруппуОтбораСписка(Список,
			 ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ,
			 ОбщаяГруппаОтбора); 		
		...
		Если НЕ ВыбранСрокИсполнения Тогда
			CRM_ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораСписка(Список, "СрокИсполнения",
				 ...
				 ГруппаИ2); 
			CRM_ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораСписка(Список, "СрокИсполнения",
				 '00010101', ВидСравненияКомпоновкиДанных.НеРавно,
				 ГруппаИ2); 			
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура УстановитьПометкуНаСервере(МассивЗадач)
	
	Для Каждого ТекЗадача Из МассивЗадач Цикл
		ЗадачаОбъект = ТекЗадача.ПолучитьОбъект();
		ЗадачаОбъект.УстановитьПометкуУдаления(Не ЗадачаОбъект.ПометкаУдаления);
	КонецЦикла;
