...
Процедура ОтборИсполнительПриИзменении(Элемент)
	...
	Иначе
		Список.Параметры.УстановитьЗначениеПараметра("Исполнитель",
			 CRM_УправлениеДоступомПовтИсп.ПолучитьПользователейПодразделения(ОтборПодразделение));
	КонецЕсли;
...
Процедура СписокПриАктивизацииСтрокиНаСервере(ТекущаяСтрокаСписка)
	...
	Иначе
		Если НЕ Пользователи.РолиДоступны("ПолныеПрава") Тогда
			...
			Если ЗначениеЗаполнено(ТекущаяСтрокаСписка.РольИсполнителя)
				...
				Элементы.ПринятьКИсполнению.Доступность	=
					(СписокВозможныхИсполнителей.НайтиПоЗначению(тПользователь) <> Неопределено);
				...
				Элементы.СписокКонтекстноеМенюОтменитьПринятиеКИсполнению.Доступность =
					(СписокВозможныхИсполнителей.НайтиПоЗначению(тПользователь) <> Неопределено);
				Элементы.СписокКонтекстноеМенюЗадачаЗадачаИсполнителяВыполнено.Доступность =
					(СписокВозможныхИсполнителей.НайтиПоЗначению(тПользователь) <> Неопределено);
				Элементы.СписокКонтекстноеМенюЗадачаЗадачаИсполнителяПеренаправить.Доступность	=
					(СписокВозможныхИсполнителей.НайтиПоЗначению(тПользователь) <> Неопределено);
			Иначе
				Элементы.ПринятьКИсполнению.Доступность											= (ТекущаяСтрокаСписка.Исполнитель = тПользователь);
				Элементы.СписокКонтекстноеМенюПринятьКИсполнению.Доступность					=
					(ТекущаяСтрокаСписка.Исполнитель = тПользователь);
				Элементы.СписокКонтекстноеМенюОтменитьПринятиеКИсполнению.Доступность			=
					(ТекущаяСтрокаСписка.Исполнитель = тПользователь);
				Элементы.СписокКонтекстноеМенюЗадачаЗадачаИсполнителяВыполнено.Доступность		=
					(ТекущаяСтрокаСписка.Исполнитель = тПользователь);
		...
		Элементы.СписокКонтекстноеМенюУстановитьПометкуУдаления.Доступность = (ТекущаяСтрокаСписка.CRM_Личная
...
Процедура КомандаСписокЗадачСоздатьЗадачу(Команда)
	...
	СтруктураДанных.Вставить("Дата", ОбщегоНазначенияКлиент.ДатаСеанса());
	СтруктураДанных.Вставить("ДатаНачала", ОбщегоНазначенияКлиент.ДатаСеанса());
...
Процедура УстановитьПометкуУдаления(Команда)
	...
	Если НЕ Элементы.Список.ТекущиеДанные = Неопределено Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("УстановитьПометкуУдаленияОтвет", ЭтотОбъект,
			 Новый Структура("ТекущаяСтрока, ПометкаУдаления", Элементы.Список.ТекущаяСтрока,
			 Элементы.Список.ТекущиеДанные.ПометкаУдаления));
		Если Элементы.Список.ТекущиеДанные.ПометкаУдаления Тогда
			ТекстВопроса = НСтр("ru='Снять с ""';en='Remove from ""'") 
				+ Строка(Элементы.Список.ТекущаяСтрока) 
				+ НСтр("ru='"" пометку на удаление?'");
			
		Иначе
			ТекстВопроса = НСтр("ru='Пометить ""';en='Mark ""'") + Строка(Элементы.Список.ТекущаяСтрока) 
				+ НСтр("ru='"" на удаление?'");
