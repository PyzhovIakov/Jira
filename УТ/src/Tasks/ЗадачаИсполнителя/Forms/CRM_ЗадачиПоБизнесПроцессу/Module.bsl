...
Процедура СохранитьНастройкиНаСервере(ИмяНастроек)
	...
	ПараметрыОтбора.Вставить("ПоИсполнителю", ПоИсполнителю);
	ПараметрыОтбора.Вставить("ПоказыватьЗадачи", ПоказыватьЗадачи);
	ХранилищеСистемныхНастроек.Сохранить("Задачи.ЗадачаИсполнителя.Формы.CRM_ЗадачиПоБизнесПроцессу",
		 ИмяНастроек,
		 ПараметрыОтбора);
КонецПроцедуры
...
Функция СформироватьОписаниеБизнесПроцесса(Задача)
	...
	СписокРеквизитов_1.Добавить("ПланируемаяДатаЗавершения"	,
		 НСтр("ru='Планируемая дата завершения:';
		|en='Schedulable date of completion:'"));
...
Функция СформироватьИсториюБизнесПроцесса(Задача)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТабОбъектов.Количество() > 0 Тогда
			СтрокаОбъектовПоЗадаче = "
			|<P><span style='font-size: 10pt;'><EM>&nbsp&nbsp&nbsp" 
		...
		Если Выборка.Выполнена Тогда
			...
			|<P><span style='font-size: 10pt;'>" + Формат(Выборка.ДатаИсполнения, "ДФ='дд.ММ.гггг ЧЧ:мм'") 
		...
		Иначе
			ИсторияВыполнения = ИсторияВыполнения + "
			|<P><span style='font-size: 10pt;'>" + Формат(Выборка.Дата, "ДФ='дд.ММ.гггг ЧЧ:мм'") 
...
Функция СформироватьОписаниеЗадачи(Задача)
	...
	Если НЕ Задача.CRM_Личная Тогда
		ОписаниеБизнесПроцесса = ?(Задача.БизнесПроцесс.КартаМаршрута.ОписаниеБизнесПроцессаВСодержаниеЗадачи,
			 СформироватьОписаниеБизнесПроцесса(Задача),
			 "");
		ИсторияБизнесПроцесса = ?(Задача.БизнесПроцесс.КартаМаршрута.ИсторияБизнесПроцессаВСодержаниеЗадачи,
			 СформироватьИсториюБизнесПроцесса(Задача),
			 "");
	Иначе
		ОписаниеБизнесПроцесса = "";
...
Процедура СписокПриАктивизацииСтрокиНаСервере(ТекущаяСтрокаСписка)
	...
	Иначе
		Если НЕ Пользователи.РолиДоступны("ПолныеПрава") Тогда
			Если ЗначениеЗаполнено(ТекущаяСтрокаСписка.РольИсполнителя)
				 И НЕ ЗначениеЗаполнено(ТекущаяСтрокаСписка.Исполнитель) Тогда
				СписокВозможныхИсполнителей	= CRM_БизнесПроцессыСервер.ПолучитьВозможныхИсполнителейПоРоли(ТекущаяСтрокаСписка.РольИсполнителя,
					 ТекущаяСтрокаСписка.Ссылка);
				Элементы.ПринятьКИсполнению.Доступность	=
					(СписокВозможныхИсполнителей.НайтиПоЗначению(Пользователи.ТекущийПользователь()) <> Неопределено);
			...
			Иначе
				Элементы.ПринятьКИсполнению.Доступность											=
					(ТекущаяСтрокаСписка.Исполнитель = Пользователи.ТекущийПользователь());
				Элементы.СписокКонтекстноеМенюПринятьКИсполнению.Доступность					=
					(ТекущаяСтрокаСписка.Исполнитель = Пользователи.ТекущийПользователь());
				Элементы.СписокКонтекстноеМенюОтменитьПринятиеКИсполнению.Доступность			=
					(ТекущаяСтрокаСписка.Исполнитель = Пользователи.ТекущийПользователь());
