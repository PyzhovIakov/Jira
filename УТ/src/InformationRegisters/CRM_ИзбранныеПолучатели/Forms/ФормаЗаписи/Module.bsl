...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	ИначеЕсли Запись.Получатель = Запись.Пользователь Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заполнен получательи пользователь не должны совпадать';
			|en='The recipient is not full and the user does not have to match'"));
		Отказ = Истина;
	КонецЕсли;		
КонецПроцедуры
