...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если СоздатьКонтрольнуюТочкуНачала Тогда
		...
		КТНачала = РегистрыСведений.CRM_ПланыПроектов.СоздатьКонтрольнуюТочку(Запись.Проект, Запись.Этап,
			 Запись.Исполнитель, Запись.Подразделение, Запись.ДатаНачала,
			 Истина);
		
		Если ТипЗнч(КТНачала) = Тип("Строка") Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Ошибка при создании контрольной точки начала:';en='Error during creation of check point of the beginning:'") + " """ + КТНачала + """",
				 ,
				 "СоздатьКонтрольнуюТочкуНачала");
	...
	Если СоздатьКонтрольнуюТочкуОкончания И Не бОшибкаПриСозданииКонтрольнойТочкиНачала Тогда
		...
		КТОкончания = РегистрыСведений.CRM_ПланыПроектов.СоздатьКонтрольнуюТочку(Запись.Проект,
			 Запись.Этап, Запись.Исполнитель, Запись.Подразделение, Запись.ДатаКонца,
			 Ложь);
		
		Если ТипЗнч(КТОкончания) = Тип("Строка") Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Ошибка при создании контрольной точки окончания:';en='Error during creation of completion check point:'") + " """ + КТОкончания + """",
				 ,
				 "СоздатьКонтрольнуюТочкуОкончания");
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	CRM_ОбщегоНазначенияКлиентСервер.ОбновлениеТаблицыФормы(Элементы.КонтрольныеТочки);
...
Процедура ПроцентЗагрузкиПриИзменении(Элемент)
	Если Запись.ПроцентЗагрузки > 100 Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения,
			 НСтр("ru='Занятость ресурса > 100%! Продолжить?';en='Seizure of a resource> 100 %! Continue?'"),
...
Процедура СкорректироватьДатуНачалаДатуКонца(ПриоритетДатыНачала = Истина)
	...
	Если Запись.ДатаКонца - Запись.ДатаНачала > КоличествоСекундВГоду Тогда
		ПоказатьПредупреждение(,
			 НСтр("ru='Нельзя указывать период больше 1 года!';
			|en='It is impossible to specify the period more than 1 year!'"),
			 5);
...
Процедура РассчитатьПродолжительность(Форма)
	...
	Иначе
		Форма.Запись.Продолжительность = (НачалоДня(Форма.Запись.ДатаКонца) - НачалоДня(Форма.Запись.ДатаНачала)) / СекундВДне 
			+ 1;
	КонецЕсли;
КонецПроцедуры
...
Процедура РассчитатьТрудоемкость()
	ДанныеТрудозатрат = CRM_ОбщегоНазначенияСервер.ПолучитьТрудозатратыЗаПериод(НачалоДня(Запись.ДатаНачала),
		 КонецДня(Запись.ДатаКонца),
		 Запись.Исполнитель);
