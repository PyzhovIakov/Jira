...
Процедура СостояниеПриИзменении(Элемент)
	...
	Если Запись.Состояние = ПредопределенноеЗначение("Перечисление.CRM_СостоянияСинхронизацииПользователя.Блокирован") Тогда
		ПоказатьВопрос(ОписаниеОповещения,
			 НСтр("ru='Внимание! Пользователю будет временно запрещено работать в приложении. Продолжить ?';
			|en='Attention! The user will be temporarily prohibited from working in the application. Continue ?'"),
	...
	ИначеЕсли Запись.Состояние = ПредопределенноеЗначение("Перечисление.CRM_СостоянияСинхронизацииПользователя.Отключен") Тогда
		ПоказатьВопрос(ОписаниеОповещения,
			 НСтр("ru='Внимание! При отключении пользователя ВСЕ ДАННЫЕ будут удалены из приложения при первом же запуске. Продолжить ?';
			|en='Attention! If you disconnect the user,
			| ALL DATA will be removed from the application at the first start. Continue ?'"),
			 РежимДиалогаВопрос.ДаНет);
	КонецЕсли;	
КонецПроцедуры
