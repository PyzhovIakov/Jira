...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если (Запись.Пользователь <> Пользователи.АвторизованныйПользователь())
		 И (НЕ РольДоступна("ПолныеПрава"))
		 И (НЕ РуководительПодразделения = Пользователи.АвторизованныйПользователь()) Тогда
...
Процедура ТрудозатратыВремяНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	CRM_ОбщегоНазначенияКлиентСервер.ВыбратьВремяИзСписка(ЭтотОбъект, ТрудозатратыВремя, Элемент, ,
		 Истина,
		 ОписаниеОповещения);
КонецПроцедуры
...
Процедура КомандаСтарт(Команда)
	...
	ВремяСтарта = ОбщегоНазначенияКлиент.ДатаСеанса();
	Элементы.ФормаКоммандаСтоп.Доступность = Истина;
	Элементы.ФормаКомандаСтарт.Доступность = Ложь;
	Элементы.ТрудозатратыВремя.Доступность = Ложь;
	
	ПодключитьОбработчикОжидания("Подключаемый_ОбновитьВремяТрудозатрат", 60);
...
Процедура КоммандаСтоп(Команда)
	...
	ОтключитьОбработчикОжидания("Подключаемый_ОбновитьВремяТрудозатрат");
	
	Запись.Трудозатраты = Запись.Трудозатраты + (ОбщегоНазначенияКлиент.ДатаСеанса() - ВремяСтарта);
...
Процедура КомандаОткрытьСписокПоОбъекту(Команда)
	...
	ОткрытьФорму("РегистрСведений.CRM_РабочееВремяПользователей.ФормаСписка", ПараметрыФормы,
...
&НаКлиенте
Процедура Подключаемый_ОбновитьВремяТрудозатрат()
	
	ТрудозатратыВремя = Дата('00010101') + (ОбщегоНазначенияКлиент.ДатаСеанса() - ВремяСтарта);
...
Процедура ИсточникПриИзмененииНаСервере()
	Если ТипЗнч(Запись.Источник) = Тип("ЗадачаСсылка.ЗадачаИсполнителя") Тогда
		Если ЗначениеЗаполнено(Запись.Источник.БизнесПроцесс)
			И ТипЗнч(Запись.Источник.БизнесПроцесс) = Тип("БизнесПроцессСсылка.CRM_БизнесПроцесс") Тогда
