...
Процедура ПеренестиОбъект_Задача_ЗадачаИсполнителя(Ошибки)
	...
	Если РезультатВыполнения И ЗначениеЗаполнено(ПользовательОтбор) И ПользовательОтбор <> Объект.Исполнитель Тогда
		...
		CRM_БизнесПроцессыИЗадачиВызовСервера.ПеренаправитьЗадачи(МассивЗадач, ПользовательОтбор, Ложь,
...
Процедура ПеренестиОбъектПоУмолчанию(Ошибки)
	...
	Если ТаблицаСобытия.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
Функция ПеренестиОбъект() Экспорт
	...
	КонецЕсли;
	Если ТипЗнч(Параметры) = Тип("Структура") И (Не Параметры.Свойство("ПеренестиНаДатуВремя")
		 Или Не ЗначениеЗаполнено(Параметры.ПеренестиНаДатуВремя)) Тогда
		Ошибки = Ошибки + ?(ПустаяСтрока(Ошибки), "", Символы.ПС);
		Ошибки = Ошибки 
			+ НСтр("ru='Не указана дата на которую переносится объект!';
				|en='Date on which are not specified the object are transfer!'");
	КонецЕсли;
...
Процедура УдалитьСсылочныйОбъектПоУмолчанию(УдаляемыйОбъект, Ошибки, ОчищатьЗаписиРегистраВручную = Ложь)
	Попытка
		...
		ОбъектДляУдаления.ДополнительныеСвойства.Вставить("ОбъектЗаписанПользователем");
...
Функция УдалитьОбъект() Экспорт
	...
	Для Каждого УдаляемыйОбъект Из УдаляемыеОбъекты Цикл
		Если ТипЗнч(УдаляемыйОбъект.Объект) = Тип("ДокументСсылка.CRM_ЭтапКалендарногоПлана") Тогда
