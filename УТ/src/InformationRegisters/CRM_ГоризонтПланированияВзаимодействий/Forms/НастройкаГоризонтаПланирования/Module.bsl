...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ДеревоИзменено Тогда
		...
		ПоказатьВопрос(ОповещениеОЗавершении,
			 "Данные были изменены! Продолжить без сохранения изменений?",
...
Процедура ПостроитьДеревоНаСервере()
	...
	ЗапросПоВажности.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	                         |	CRM_ВажностьКлиентов.Ссылка КАК Ссылка,
	...
	ЗапросКэша.Текст = "ВЫБРАТЬ
	                   |	CRM_ГоризонтПланированияВзаимодействий.Важность КАК Важность,
	                   |	CRM_ГоризонтПланированияВзаимодействий.ТипУслуги КАК ТипУслуги,
	                   |	CRM_ГоризонтПланированияВзаимодействий.Дней КАК Дней
	                   |ИЗ
...
Процедура СохранитьДанныеДереваВРегистр()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               |	CRM_ГоризонтПланированияВзаимодействий.Важность КАК Важность,
	               |	CRM_ГоризонтПланированияВзаимодействий.ТипУслуги КАК ТипУслуги,
	               |	CRM_ГоризонтПланированияВзаимодействий.КомпанияОфис КАК КомпанияОфис
	               |ИЗ
	               |	РегистрСведений.CRM_ГоризонтПланированияВзаимодействий КАК CRM_ГоризонтПланированияВзаимодействий
	               |ГДЕ
	               |	CRM_ГоризонтПланированияВзаимодействий.КомпанияОфис = &КомпанияОфис";
