...
Функция ПолучитьРасписаниеВыполненияКлассификатора(Классификатор) Экспорт
	...
	РегламентноеЗаданиеОбъект =
		CRM_КлассификаторыЭкспортныеМетоды.НайтиРегламентноеЗадание(Классификатор.РегламентноеЗаданиеGUID);
...
Функция СоздатьРегламентноеЗаданиеПриНеобходимости(Отказ, ТекущийОбъект)
	...
	РегламентноеЗаданиеОбъект =
		CRM_КлассификаторыЭкспортныеМетоды.НайтиРегламентноеЗадание(ТекущийОбъект.РегламентноеЗаданиеGUID);
	
	// При необходимости создаем регламентное задание.
	Если РегламентноеЗаданиеОбъект = Неопределено Тогда
		
		РегламентноеЗаданиеОбъект =
			РегламентныеЗадания.СоздатьРегламентноеЗадание("CRM_ФормированиеСоставаЗначенийКлассификаторов");
...
Процедура УстановитьПараметрыРегламентногоЗадания(РегламентноеЗаданиеОбъект,
	...
	НаименованиеРегламентногоЗадания = НСтр("ru='Формирование состава значений классификатора: %1';
		|en='Formation of the classifier values:%1'");
	НаименованиеРегламентногоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НаименованиеРегламентногоЗадания,
		 СокрЛП(ТекущийОбъект.Наименование));
...
Процедура ЗаписатьРегламентноеЗадание(Отказ, РегламентноеЗаданиеОбъект)
	...
	Исключение
		...
		СтрокаСообщения = НСтр("ru='Произошла ошибка при сохранении расписания. Подробное описание ошибки: %1';
			|en='An error occurred while saving the schedule. Detailed error description:%1'");
...
Функция АдресаСхемыКомпоновкиДанныхИНастроекВоВременномХранилище(ЭлементНастройки) Экспорт
	...
	НаборДанных.Запрос =
		CRM_КлассификаторыЭкспортныеМетоды.СформироватьТекстЗапросаПринадлежностиКлассификатора(ЭлементНастройки);
