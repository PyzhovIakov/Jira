...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ПустаяСтрока(Объект.CRM_ИмяСправочника) Тогда
		...
		CRM_ЗаполнитьСписокВыбораРеквизитовПоВидуСправочника(Элементы.CRM_ИмяРеквизитаСправочника.СписокВыбора,
			 ЭтотОбъект[ИмяТаблицыРеквизитов]);
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Объект.ТипОтвета = ПредопределенноеЗначение("Перечисление.ТипыОтветовНаВопрос.Число") Тогда
		...
		//	Если Объект.МинимальноеЗначение = Объект.МаксимальноеЗначение Тогда
		Если Объект.ИспользоватьМинимальноеЗначение И Объект.ИспользоватьМаксимальноеЗначение
		   И Объект.МинимальноеЗначение = Объект.МаксимальноеЗначение Тогда
		// -CRM Рефакторинг
			ОбщегоНазначенияКлиент.СообщитьПользователю(
		...
		Если Объект.ПоказыватьПолосуРегулирования
		   И Объект.ШагПолосыРегулирования > Объект.МаксимальноеЗначение - Объект.МинимальноеЗначение Тогда
		// -CRM Рефакторинг
			ОбщегоНазначенияКлиент.СообщитьПользователю(
...
Процедура ПредставлениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиент.ПоказатьФормуРедактированияМногострочногоТекста(ОповещениеОЗакрытии,
		 Элемент.ТекстРедактирования, НСтр("ru = 'Формулировка'"));
...
Процедура ДоступностьТаблицыВариантыОтветов(Форма)
	...
	Если Форма.Объект.Ссылка.Пустая() Тогда
		...
		Форма.ИнформацияВариантыОтветов                       =
			НСтр("ru = 'Для редактирования вариантов ответов запишите вопрос для анкетирования'");
	Иначе
	...
	Для каждого ЗначениеПеречисления Из Метаданные.Перечисления.ТипыОтветовНаВопрос.ЗначенияПеречисления Цикл
		...
		Если Перечисления.ТипыОтветовНаВопрос[ЗначениеПеречисления.Имя] = Перечисления.ТипыОтветовНаВопрос.ЗначениеИнформационнойБазы Тогда 
			...
			Для каждого ДоступныйТип Из РеквизитФормыВЗначение("Объект").Метаданные().Тип.Типы() Цикл
				...
				Если ДоступныйТип = Тип("Строка") Или ДоступныйТип = Тип("Булево")
					 Или ДоступныйТип = Тип("Число") Или ДоступныйТип = Тип("Дата")
					 Или ДоступныйТип = Тип("СправочникСсылка.ВариантыОтветовАнкет") Тогда
					Продолжить;
				КонецЕсли;
...
Процедура УстановитьПредставлениеДиапазонаПодсказокЧисловогоВопроса()
	...
	Для Каждого ТекущаяСтрока Из ТаблицаДиапазонов Цикл
		...
		ТекущаяСтрока.ЗначениеПредставление =
			CRM_ПолучитьПредставлениеДиапазонаПодсказокЧисловогоВопроса(ТекущаяСтрока, ПредыдущаяСтрока, ТекущийИндекс);
		// -CRM
	КонецЦикла;
...
Процедура CRM_ЗаполнитьТаблицуРеквизитовСправочника(ИмяСправочника)
	...
	Для Каждого РеквизитСправочника Из МетаданныеРеквизитыСправочника Цикл
		...
		Для Каждого ТипРеквизитаСправочника Из ТипыРеквизитаСправочника Цикл
			Если ОписаниеТиповОтветовНаВопросы.СодержитТип(ТипРеквизитаСправочника) Тогда
				...
				ТаблицаРеквизитовНоваяСтрока.Представление = ?(ПустаяСтрока(РеквизитСправочника.Синоним) ,
					 РеквизитСправочника.Имя,
					 РеквизитСправочника.Синоним);
	...
	Если ИмяСправочника = "Партнеры" Тогда
		НаборыСвойств.Добавить(УправлениеСвойствами.НаборСвойствПоИмени("Справочник_Партнеры_Общие"));
		НаборыСвойств.Добавить(УправлениеСвойствами.НаборСвойствПоИмени("Справочник_Партнеры_Компании_CRM"));
		НаборыСвойств.Добавить(УправлениеСвойствами.НаборСвойствПоИмени("Справочник_Партнеры_ЧастныеЛица_CRM"));
	ИначеЕсли ИмяСправочника = "КонтактныеЛицаПартнеров" Тогда
		НаборыСвойств.Добавить(УправлениеСвойствами.НаборСвойствПоИмени("Справочник_КонтактныеЛицаПартнеров"));
	КонецЕсли;
...
Процедура CRM_УстановитьТипВопросаПоРеквизиту()
	ИмяТаблицыРеквизитов = "CRM_ТаблицаРеквизитов" + Объект.CRM_ИмяСправочника;
	МассивСтрокРеквизита = ЭтотОбъект[ИмяТаблицыРеквизитов].НайтиСтроки(Новый Структура("Значение",
...
&НаКлиенте
Функция CRM_ПолучитьПредставлениеДиапазонаПодсказокЧисловогоВопроса(ТекущаяСтрока, ПредыдущаяСтрока, ТекущийИндекс)
	
	Результат = "";
	
	Если ТекущаяСтрока.ЗначениеДо = ЗначениеПорогаДиапазонаПодсказокЧисловогоВопроса() Тогда
		Если ТекущийИндекс = 0 Тогда
			Результат = НСтр("ru = 'любое значение'");
		ИначеЕсли ПредыдущаяСтрока.ЗначениеДо = ЗначениеПорогаДиапазонаПодсказокЧисловогоВопроса() Тогда
			Результат = ПредыдущаяСтрока.ЗначениеПредставление;
		Иначе
			ШаблонСтроки = НСтр("ru = 'более %1'");
			Результат = СтрШаблон(ШаблонСтроки, Строка(ПредыдущаяСтрока.ЗначениеДо));
		КонецЕсли;
	Иначе
		Если ТекущийИндекс = 0 Тогда
			ШаблонСтроки = НСтр("ru = '%1 и менее'");
			Результат = СтрШаблон(ШаблонСтроки, Строка(ТекущаяСтрока.ЗначениеДо));
		ИначеЕсли ПредыдущаяСтрока.ЗначениеДо = ТекущаяСтрока.ЗначениеДо Тогда
			Результат = ПредыдущаяСтрока.ЗначениеПредставление;
		Иначе
			Результат = СтрШаблон(НСтр("ru = 'от %1 до %2'"),
