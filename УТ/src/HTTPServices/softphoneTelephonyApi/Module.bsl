	...
	Попытка
		Если НРег(ПараметрыЗапросаJSON.location) = "abonent" Тогда
			...
			Если ПараметрыЗапросаJSON.Свойство("origin") Тогда
				Входящий = (ПараметрыЗапросаJSON.origin <> 1);
            КонецЕсли;
			
			НомерКонтакта = "";
			Если Входящий = Истина И ПараметрыЗапросаJSON.from.Свойство("number", НомерКонтакта) Тогда
				МенеджерЗаписи = РегистрыСведений.сфпТекущиеЗвонки.СоздатьМенеджерЗаписи();
				МенеджерЗаписи.ИдентификаторЗвонка = "Исходящий_" + Прав(НомерКонтакта, 7) + "_" + НомерЛинии;
				МенеджерЗаписи.Прочитать();
				Если МенеджерЗаписи.Выбран() Тогда
					Если (ТекущаяДата() - МенеджерЗаписи.ДатаЗвонка) < 30 Тогда
						Входящий = Ложь;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
			...
			НомерКонтакта = "";
			Если Входящий = Истина И ПараметрыЗапросаJSON.to.Свойство("number", НомерКонтакта) Тогда
				МенеджерЗаписи = РегистрыСведений.сфпТекущиеЗвонки.СоздатьМенеджерЗаписи();
				МенеджерЗаписи.ИдентификаторЗвонка = "Исходящий_" + Прав(НомерКонтакта, 7) + "_" + НомерЛинии;
				МенеджерЗаписи.Прочитать();
				Если МенеджерЗаписи.Выбран() Тогда
