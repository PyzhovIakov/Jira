...
Процедура ВывестиДанныеПоПроекту()
	...
	Область.Параметры.ПроектПроцесс = НСтр("ru='проекта';en='project'");
	...
	Если ЗначениеЗаполнено(ДанныеПоПроекту.ПлановаяДатаНачала)
		 И ЗначениеЗаполнено(ДанныеПоПроекту.ПлановаяДатаОкончания) Тогда
		Область.Параметры.ЗаголовокПлановаяДатаНачалаОкончания = НСтр("ru='Плановая дата начала/окончания';
			|en='Planned date of start/end'");
	...
	ИначеЕсли ЗначениеЗаполнено(ДанныеПоПроекту.ПлановаяДатаНачала)
		 И Не ЗначениеЗаполнено(ДанныеПоПроекту.ПлановаяДатаОкончания) Тогда
		Область.Параметры.ЗаголовокПлановаяДатаНачалаОкончания = НСтр("ru='Плановая дата начала';en='Planned start date'");
		
		Область.Параметры.СтрокаПлановаяДатаНачалаОкончания = Формат(ДанныеПоПроекту.ПлановаяДатаНачала, "ДФ=dd.MM.yyyy");
		
	ИначеЕсли Не ЗначениеЗаполнено(ДанныеПоПроекту.ПлановаяДатаНачала)
		 И ЗначениеЗаполнено(ДанныеПоПроекту.ПлановаяДатаОкончания) Тогда
...
Процедура ВывестиДанныеПоЗаказчику()
	...
	Если СтруктураДанные.Свойство("ДанныеПоКонтактномуЛицуЗаказчика") Тогда
		...
		Если ДанныеПоКонтактномуЛицуЗаказчика.Следующий() Тогда
			...
			Если ТабДокументКИ.ВысотаТаблицы > 0 Тогда
				Область = Макет.ПолучитьОбласть("ЗаголовокПодраздела");
				Область.Параметры.Заголовок = НСтр("ru='Контактное лицо заказчика';en='Contact person of customer'") + ": " 
...
Процедура ВывестиДанныеПоЭтапам(ТабДокументТаблицаЭтапов = Неопределено, ДеревоЭтапы = Неопределено,
	 Знач УровеньВложенности = -1,
	...
	Иначе
		...
		Для Каждого СтрокаДерева Из ДеревоЭтапы.Строки Цикл
			...
			ВывестиДанныеПоЭтапам(ТабДокументТаблицаЭтапов, СтрокаДерева, УровеньВложенности,
				 МассивВыведенныеЭтапы, Валюта,
				 ОбластьСтрока);
		КонецЦикла;
