...
Процедура ТаблицаОтчетаОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	Если ЗначениеЗаполнено(Расшифровка) Тогда
			...
			Если Расшифровка = "ПлановыеЗатратыПоПроекту" Тогда
				Если Проекты.Количество() > 0 И ЗначениеЗаполнено(Проекты[0].Значение) Тогда
					ПараметрыФормы = Новый Структура("Отбор,КлючВарианта,СформироватьПриОткрытии",
						 Новый Структура("Проект", Проекты[0].Значение), "Основной",
						 Истина);
					ОткрытьФорму("Отчет.CRM_ПлановыеЗатратыПроектов.Форма", ПараметрыФормы, ЭтотОбъект,
		...
		ИначеЕсли ТипЗнч(Расшифровка) = Тип("СправочникСсылка.CRM_ЭтапыПроектов") Тогда
			...
			ПараметрыФормы = Новый Структура("Отбор,КлючВарианта,СформироватьПриОткрытии",
				 Новый Структура("Этап", Расшифровка), "Основной",
				 Истина);
...
Процедура СохранитьНастройки()
	...
	CRM_ХранилищеНастроек.Сохранить("Отчет.CRM_КарточкаПроекта", "CRM_КарточкаПроекта", Настройки);
КонецПроцедуры
...
Процедура ЗагрузитьНастройки()
	...
	ЗначениеНастроек = CRM_ХранилищеНастроек.Загрузить("Отчет.CRM_КарточкаПроекта", "CRM_КарточкаПроекта");
