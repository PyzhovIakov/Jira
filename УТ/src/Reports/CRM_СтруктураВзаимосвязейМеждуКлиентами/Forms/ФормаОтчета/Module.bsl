...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Свойство("Партнер") Тогда
		ВызватьИсключение НСтр("ru='Отчет не предназначен для непосредственного использования.';
			|en='The report is not intended for direct use.'");
	КонецЕсли;
...
Функция ВернутьПроектыПоКлиенту(Клиент)
	...
	|	CRM_БизнесПроцесс.Проект КАК Проект
...
Процедура СформироватьРекурсивно(ПартнерСсылка, ТаблицаСвязейПартнеров, СтрокиДерева, ТаблицаВыведенныхСвязей, Дерево)
	...
	Для Каждого НайденнаяСтрока Из НайденныеСтроки1 Цикл
		...
		НайденныеСтрокиВыведенные = ТаблицаВыведенныхСвязей.НайтиСтроки(Новый Структура("ПервыйПартнер,
			|ВторойПартнер,ВидСвязи", Партнер1, Партнер2,
			 НайденнаяСтрока.ВидСвязи));
		Если НайденныеСтрокиВыведенные.Количество() > 0 Тогда
			Продолжить;
		КонецЕсли;
		НайденныеСтрокиВыведенные = ТаблицаВыведенныхСвязей.НайтиСтроки(Новый Структура("ПервыйПартнер,
			|ВторойПартнер,ВидСвязи", Партнер2, Партнер1,
			 НайденнаяСтрока.ВидСвязи));
		Если НайденныеСтрокиВыведенные.Количество() > 0 Тогда
			Продолжить;
		КонецЕсли;
		...
		СформироватьРекурсивно(НоваяСтрока.Партнер2, ТаблицаСвязейПартнеров, НоваяСтрока.Строки,
			 ТаблицаВыведенныхСвязей,
			 Дерево);
