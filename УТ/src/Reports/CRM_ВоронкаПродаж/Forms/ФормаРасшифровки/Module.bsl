...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Этап.ВидЭтапа = Перечисления.CRM_ВидыЭтаповВоронкиПродаж.ПоДокументам Тогда
		...
		СхемаКомпоновкиДанных =
			Отчеты.CRM_ВоронкаПродаж.ПолучитьМакет("СхемаКомпоновкиДанныхРасшифровкаДиаграммыПоДокументам");
	...
	ИначеЕсли Параметры.Этап.ВидЭтапа = Перечисления.CRM_ВидыЭтаповВоронкиПродаж.ПоЭтапам Тогда
		...
		Если Параметры.Свойство("ПараметрТипВоронки") Тогда
			...
			Если Параметры.ПараметрТипВоронки = "Активные" Тогда
				ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "//%УСЛОВИЯ_ПО_ЗАДАЧАМ",
					 "И (ЗадачаИсполнителя.БизнесПроцесс.Стартован И НЕ ЗадачаИсполнителя.БизнесПроцесс.Завершен)
																				  |И (НЕ ЗадачаИсполнителя.Выполнена)
																				  |И ЗадачаИсполнителя.СрокИсполнения <= &ГоризонтВоронки
																				  |И ЗадачаИсполнителя.Дата <= &КонецПериода");
				ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "//%УСЛОВИЯ_ПО_СОБЫТИЯМ",
					 "И (Событие.СостояниеСобытия.ТипСостояния = ЗНАЧЕНИЕ(Перечисление.CRM_ТипыСостоянийСобытий.Запланировано)
			...
			ИначеЕсли Параметры.ПараметрТипВоронки = "Закрытые" Тогда
				Если Параметры.Свойство("ПараметрУспешно")
					 И НЕ ТипЗнч(Параметры.ПараметрУспешно) = Тип("Массив")
					 И Параметры.ПараметрУспешно = 1 Тогда
				...
				КонецЕсли;
				ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "//%УСЛОВИЯ_ПО_СОБЫТИЯМ",
					 "И (Событие.СостояниеСобытия.ТипСостояния = ЗНАЧЕНИЕ(Перечисление.CRM_ТипыСостоянийСобытий.Завершено)
	...
	Если Параметры.Свойство("Вариант") И ТипЗнч(Параметры.Вариант) = Тип("Строка")
		 И ЗначениеЗаполнено(Параметры.Вариант) Тогда
	...
	Если Параметры.Свойство("Пользователь") И ЗначениеЗаполнено(Параметры.Пользователь) Тогда
		УстановитьЗначениеВПользовательскомОтборе("Пользователь", ВидСравненияКомпоновкиДанных.ВСписке,
	...
	Если Параметры.Свойство("Подразделение") И ЗначениеЗаполнено(Параметры.Подразделение) Тогда
		УстановитьЗначениеВПользовательскомОтборе("Подразделение", ВидСравненияКомпоновкиДанных.Равно,
	...
	Если Параметры.Свойство("ФиксированныйОтборПоПользователюПодразделению")
		 И Параметры.ФиксированныйОтборПоПользователюПодразделению = Истина Тогда
		Для Каждого ЭлементОтбора Из НастройкиКомпоновщика.Настройки.Отбор.Элементы Цикл
			Если ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Пользователь")
				...
				Элементы.НастройкиКомпоновщикаПользовательскиеНастройки.ВысотаВСтрокахТаблицы =
					Элементы.НастройкиКомпоновщикаПользовательскиеНастройки.ВысотаВСтрокахТаблицы - 1;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура РезультатОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Попытка
		Для Каждого КлючЗначение Из ЗначениеПоляРасшифровки Цикл
			...
			Если ЗначениеЗаполнено(ЗначениеПоляРасшифровки[КлючЗначение.Ключ])
				 И ЭтоСсылка(ТипЗнч(ЗначениеПоляРасшифровки[КлючЗначение.Ключ])) Тогда
...
&НаКлиенте
Процедура НастройкиКомпоновщикаПользовательскиеНастройкиПриОкончанииРедактирования(Элемент,
	 НоваяСтрока,
...
Процедура ОбновитьРезультат()
	...
	МакетКомпоновки = КомпоновщикМакета.Выполнить(СхемаКомпоновкиДанных,
		 НастройкиКомпоновщика.ПолучитьНастройки(),
		 Данныерасшифровки);
...
Процедура УстановитьЗначениеВПользовательскомОтборе(ИмяПоля, ВидСравнения, Значение)
	...
	Для Каждого ЭлементНастройки Из НастройкиКомпоновщика.ПользовательскиеНастройки.Элементы Цикл
		Если ТипЗнч(ЭлементНастройки) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
			ЭлементВОтборе =
				ПолучитьЭлементОтбораПоИдентификаторуПользовательскойНастройки(ЭлементНастройки.ИдентификаторПользовательскойНастройки);
			Если ТипЗнч(ЭлементВОтборе) = Тип("ЭлементОтбораКомпоновкиДанных")
				 И ЭлементВОтборе.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(ИмяПоля) Тогда
