...
Процедура ПередЗакрытием_Продолжение()
	Оповещение = Новый ОписаниеОповещения("ПередЗакрытием_Завершение", ЭтотОбъект);
	ПоказатьВопрос(Оповещение,
		 НСтр("ru='Настройки были изменены. Записать изменения настроек?';
...
Процедура УстановитьНадписьКоличествоКолонокМонитораПоказателей(КоличествоКолонокМонитораПоказателей, Элементы)
	...
	Иначе
		...
		Если СтрНайти(СловоПрописью, "колонки") Тогда
			СловоПрописью = "колонки";
		ИначеЕсли СтрНайти(СловоПрописью, "колонок") Тогда
