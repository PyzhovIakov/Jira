...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ CRM_ЛицензированиеЭкспортныеМетоды.ВариантПоставкиКОРП() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Монитор показателей доступен только для ""КОРП"" поставки конфигурации!';
			|en='The indicator monitor is only available for ""CORP"" configuration delivery!'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если НЕ ПустаяСтрока(Параметры.ИдентификаторВариантаАнализа) Тогда
		...
		АдресаДинамическихПараметров =  Новый ХранилищеЗначения(ДанныеФормыВЗначение(Параметры.СоставМонитора,
			 Тип("ТаблицаЗначений")));
...
Процедура ОтборСтатусПриИзменении(Элемент)
	...
	CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики(
		"CRM_Статистика.Показатели.МониторПоказателей.КоличествоВыбораВариантаСтрокаСтатус");
...
Процедура КнопкаШаблоныПоказателейНажатие(Элемент)
	...
	CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики(
		"CRM_Статистика.Показатели.МониторПоказателей.КоличествоНажатийИконкаШаблоныПоказателей");
...
Процедура КнопкаУстановкаПоказателейНажатие(Элемент)
	...
	CRM_ЦентрМониторингаКлиент.ЗаписатьОперациюБизнесСтатистики(
		"CRM_Статистика.Показатели.МониторПоказателей.КоличествоНажатийИконкаУстановкаПоказателей");
...
Процедура СводныйОтчет(Команда)
	...
	//	Предупреждение(НСтр("ru='Сформировать сводный отчет невозможно,
		//| т.к. монитор показателей не настроен.';en='Generate a summary report is impossible,
		//| because indicators monitor is not configured.'"),,
		// НСтр("ru='Ошибка формирования сводного отчета'"));
	//	
	// ИначеЕсли СоставМонитораПустойПоОтбору Тогда
	//	Предупреждение(НСтр("ru='Сформировать сводный отчет невозможно,
		//| т.к. в текущий отбор не входит ни один показатель.';
		//|en='Generate a consolidated report is impossible,
		//| because no indicator is included in the current selection.'"),,
		// НСтр("ru='Ошибка формирования сводного отчета'"));
	//	
	// Иначе 
	//	
...
Процедура ВыбратьПериод(Команда)
	...
	ОткрытьФорму("ОбщаяФорма.CRM_ВыборСтандартногоПериода", ПараметрыВыбора, Элементы.ВыбратьПериод, ,
		 , ,
		 ОписаниеОповещения);
...
Функция ГоризонтальнаяГруппа(Имя, Заголовок, РодительскаяГруппа = Неопределено, ОдинаковыеПоШирине = Ложь)
	...
	ГруппаСтрокаЭлементов.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	ГруппаСтрокаЭлементов.Отображение = ОтображениеОбычнойГруппы.Нет;
	ГруппаСтрокаЭлементов.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
	ГруппаСтрокаЭлементов.Заголовок = Заголовок;
...
//	ВариантОтображенияПоказателейСтроки - ПеречислениеСсылка.ВариантыОтображенияВариантовАнализа, Неопределено - указывает на краткое отображение
	// или в виде диаграммы/таблицы
//	КлючУникальностиГруппы - Строка - уникальный идентификатор группы, основанный на имени зоны внимания
//
&НаСервере 
Процедура ДобавитьГруппыКолонкиЭлементов(РодительскаяГруппа,
	 ВариантОтображенияПоказателейСтроки = Неопределено,
	 КлючУникальностиГруппы)
...
//	ВариантОтображения - ПеречислениеСсылка.ВариантыОтображенияВариантовАнализа - указывает на краткое отображение
	// или в виде диаграммы/таблицы
//
&НаСервере
Процедура ДобавитьКомандыВариантовАнализа(Показатель, Пользователь, ГруппаЭлементов,
	...
	Если ВариантОтображения = ВариантыОтображения.Диаграмма 
		...
		КоманднаяПанельКомандПоказателя = Элементы.Добавить("КоманднаяПанельКомандПоказателя__" 
			+ ИдентификаторЭлементаФормы, Тип("ГруппаФормы"), Элементы["Статусы__" + ИдентификаторЭлементаФормы]);
		...
		// ПодменюВсеДействия = Элементы.Добавить("ВсеДействия__" + ИдентификаторЭлементаФормы,
			// Тип("ГруппаФормы"), КоманднаяПанельКомандПоказателя);
		...
		Если КоличествоВариантовОтчетов Тогда
			Для Каждого ВариантОтчетаПоказателя Из ВариантыОтчетовПоказателя Цикл 
				...
				КомандаОткрытьОтчет = Команды.Добавить("ОткрытьОтчет__" 
					+ СтрЗаменить(Строка(Пользователь.УникальныйИдентификатор()), "-", "_") + "__" + ИдентификаторЭлементаФормы + "__" 
		...
		Если КоличествоВариантовОтчетов Тогда
			...
			Для Каждого ВариантОтчетаПоказателя Из ВариантыОтчетовПоказателя Цикл 
				...
				КнопкаОтчета = Элементы.Добавить("ФормаОткрытьОтчет__" + ИдентификаторЭлементаФормы + "__" 
					+ ИдентификаторВарианта, Тип("КнопкаФормы"), КоманднаяПанельКомандПоказателя);
				КнопкаОтчета.ИмяКоманды = "ОткрытьОтчет__" 
					+ СтрЗаменить(Строка(Пользователь.УникальныйИдентификатор()), "-", "_") + "__" + ИдентификаторЭлементаФормы + "__" 
					+ ИдентификаторВарианта;
	...
	ИначеЕсли ВариантОтображения = ВариантыОтображения.Кратко Тогда
		...
		Если КоличествоВариантовОтчетов Тогда
			Для Каждого ВариантОтчетаПоказателя Из ВариантыОтчетовПоказателя Цикл 
				...
				КомандаОткрытьОтчет = Команды.Добавить("ОткрытьОтчет__" + ИдентификаторЭлементаФормы + "__" 
					+ ИдентификаторВарианта);
				КомандаОткрытьОтчет.Заголовок = НаименованиеВарианта;
				КомандаОткрытьОтчет.Действие = "Подключаемый_ОткрытьОтчет";
				КомандаОткрытьОтчет.Картинка = БиблиотекаКартинок.Отчет;
		...
		Если КоличествоВариантовОтчетов Тогда
			ПодменюОтчеты = Элементы.Добавить("Отчеты__" + ИдентификаторЭлементаФормы, Тип("ГруппаФормы"),
				 КонтекстноеМенюТекстовогоПоказателя);
			ПодменюОтчеты.Вид = ВидГруппыФормы.Подменю;
			ПодменюОтчеты.Заголовок = НСтр("ru='Отчеты';en='Reports'");
			ПодменюОтчеты.Картинка = БиблиотекаКартинок.Отчет;
			...
			Для Каждого ВариантОтчетаПоказателя Из ВариантыОтчетовПоказателя Цикл 
				...
				КнопкаОтчета = Элементы.Добавить("ФормаОткрытьОтчет__" + ИдентификаторЭлементаФормы + "__" 
					+ ИдентификаторВарианта, Тип("КнопкаФормы"), ПодменюОтчеты);
...
Процедура НарисоватьДиаграмму(ИсточникДанных, ГруппаЭлементов, ИдентификаторЭлементаФормы)
	...
	МассивРеквизитов.Добавить(Новый РеквизитФормы("Диаграмма__" + ИдентификаторЭлементаФормы,
		 ОписаниеТипов, "", "Диаграмма для '" + ИсточникДанных + "'"));
	...
	МассивРеквизитов.Добавить(Новый РеквизитФормы("КартинкаСостояния__" + ИдентификаторЭлементаФормы,
		 ОписаниеТипов, "", НСтр("ru='Картинка состояния';en='Status Picture'")));
	МассивРеквизитов.Добавить(Новый РеквизитФормы("КартинкаТренда__" + ИдентификаторЭлементаФормы,
		 ОписаниеТипов, "", НСтр("ru='Картинка тренда';en='Trend picture'")));
	...
	МассивРеквизитов.Добавить(Новый РеквизитФормы("НаименованиеПоказателя__" 
		+ ИдентификаторЭлементаФормы, ОписаниеТипов, "", "Наименование показателя"));
	...
	// ПолеЗначенияИндикатора = Элементы.Вставить("КартинкаСостояния__" + ИдентификаторЭлементаФормы,
		// Тип("ПолеФормы"), ГруппаСтатусов);
	...
	Динамика = ПолучитьДинамикуПоказателя(ИсточникДанных.Показатель,
		 ?(ОтборКонецПериода < CRM_ОбщегоНазначенияСервер.ПолучитьТекущуюДатуСеанса(), ОтборКонецПериода,
	...
	ПолеЗначенияИндикатора = Элементы.Вставить("КартинкаТренда__" + ИдентификаторЭлементаФормы,
		 Тип("ПолеФормы"), ГруппаСтатусов);
	ПолеЗначенияИндикатора.Вид = ВидПоляФормы.ПолеКартинки;
	...
	// ЭтаФорма["КартинкаСостояния__" 
		//+ ИдентификаторЭлементаФормы] = Отчеты.CRM_МониторЦелевыхПоказателей.ПолучитьКартинкуСостоянияДляПоказателя(ИсточникДанных);
	...
	Если СтруктураНастроек.ВариантОтображенияДеталей = "ПоказыватьТекущееЗначениеИДетали" Тогда
		ЭтотОбъект["НаименованиеПоказателя__" 
			+ ИдентификаторЭлементаФормы] = ИсточникДанных.Показатель.КраткоеНаименование 
			+ ДобавитьЗнач(Отчеты.CRM_МониторЦелевыхПоказателей.ПолучитьСтрокуПредставленияЧисел(ИсточникДанных.Показатель.КратностьЗначений)) 
			+ ДобавитьЗнач(ИсточникДанных.Показатель.ЕдиницаИзмерения) 
			+ ДобавитьЗнач(ИсточникДанных.ПроцентВыполнения);
	Иначе
		ЭтотОбъект["НаименованиеПоказателя__" 
			+ ИдентификаторЭлементаФормы] = ИсточникДанных.Показатель.КраткоеНаименование;
	КонецЕсли;
	...
	Иначе
		// Если Не ПустойИсточникДанных Тогда
			...
			CRM_УправлениеЦелевымиПоказателямиСервер.ПостроитьДиаграммуПоказателя(ИсточникДанных.Показатель,
				 Диаграмма,
				 ТаблицаДанных);
...
Процедура НарисоватьКратко(ИсточникДанных, ГруппаЭлементов, ИдентификаторЭлементаФормы)
	...
	МассивРеквизитов.Добавить(Новый РеквизитФормы("КраткоеНаименование__" + ИдентификаторЭлементаФормы,
		 ОписаниеТиповСтрок, "", НСтр("ru='Краткое наименование';en='Short name'")));
	МассивРеквизитов.Добавить(Новый РеквизитФормы("Значение__" + ИдентификаторЭлементаФормы,
		 ОписаниеТиповСтрок, "", НСтр("ru='Значение';en='Value'")));
	...
	Если ПоказыватьТекущееЗначениеИДеталиПоказателя
		...
		МассивРеквизитов.Добавить(Новый РеквизитФормы("ОтнИзменение__" + ИдентификаторЭлементаФормы,
			 ОписаниеТиповСтрок, "", НСтр("ru='Отн. изменение';en='Rel. change'")));
		МассивРеквизитов.Добавить(Новый РеквизитФормы("АбсИзменение__" + ИдентификаторЭлементаФормы,
			 ОписаниеТиповСтрок, "", НСтр("ru='Абс. изменение';en='Abs. change'")));
	КонецЕсли;
	...
	МассивРеквизитов.Добавить(Новый РеквизитФормы("КартинкаСостояния__" + ИдентификаторЭлементаФормы,
		 ОписаниеТиповКартинок, "", НСтр("ru='Картинка состояния';en='Status Picture'")));
	МассивРеквизитов.Добавить(Новый РеквизитФормы("КартинкаТренда__" + ИдентификаторЭлементаФормы,
		 ОписаниеТиповКартинок, "", НСтр("ru='Картинка тренда';en='Trend picture'")));
	
	ИзменитьРеквизиты(МассивРеквизитов);
	
	// Добавим элементы формы
	ГруппаСтатусов = ГоризонтальнаяГруппа("Статусы__" + ИдентификаторЭлементаФормы,
		 НСтр("ru='Статусы';en='Statuses'"), ГруппаЭлементов);
	...
	ПолеИндикатора.Подсказка = ИсточникДанных.СвойстваЗоныАнализаИТренда.ПояснениеКСтатусу 
		+ Символы.ПС + Символы.ПС 
		+ ИсточникДанных.СвойстваЗоныАнализаИТренда.ПояснениеКТренду;
	
	ПолеКрНаименования = Элементы.Вставить("КраткоеНаименование__" + ИдентификаторЭлементаФормы,
	...
	ЭтотОбъект["КартинкаСостояния__" 
		+ ИдентификаторЭлементаФормы] =
			Отчеты.CRM_МониторЦелевыхПоказателей.ПолучитьКартинкуСостоянияДляПоказателя(ИсточникДанных);
	ЭтотОбъект["КартинкаТренда__" 
		+ ИдентификаторЭлементаФормы] =
			Отчеты.CRM_МониторЦелевыхПоказателей.ПолучитьКартинкуТрендаДляПоказателя(ИсточникДанных);
	
	НачалоПериода = Формат(ИсточникДанных.СвойстваДанныхПоПериодам.ДатаПервогоЗначения, "ДФ=dd.MM.yy");
	КонецПериода = Формат(ИсточникДанных.СвойстваДанныхПоПериодам.ДатаПоследнегоЗначения, "ДФ=dd.MM.yy");
	ТекстовоеПредставлениеПериода = НачалоПериода + " - " + КонецПериода;
	// Если ВариантАнализа.КратностьЗначений = ВидыПредставленияЧисел.БезИзменений Тогда
		ЭтотОбъект["КраткоеНаименование__" 
			+ ИдентификаторЭлементаФормы] = ИсточникДанных.Владелец.КраткоеНаименованиеЦелевогоПоказателя 
	...
	// Иначе
	//	ЭтаФорма["КраткоеНаименование__" 
		//+ ИдентификаторЭлементаФормы] = ВариантАнализа.Владелец.КраткоеНаименованиеЦелевогоПоказателя 
		//+ ": " + ВариантАнализа.Наименование + ", " 
		//+ Отчеты.CRM_МониторЦелевыхПоказателей.ПолучитьСтрокуПредставленияЧисел(ВариантАнализа) 
		//+ ?(НЕ ПустаяСтрока(ДинамическиеСвойстваВариантаАнализа.Размерность), " " 
			//+ ДинамическиеСвойстваВариантаАнализа.Размерность, "") + ", " 
			//+ ТекстовоеПредставлениеПериода;
	// КонецЕсли;
	...
	Если НЕ ПустойИсточникДанных И НЕ РасчетыНеактуальны И НЕ ОшибкаРасчета Тогда
		...
		ЭтотОбъект["Значение__" 
			+ ИдентификаторЭлементаФормы] =
				СокрЛП(Отчеты.CRM_МониторЦелевыхПоказателей.ПолучитьСокращенноеПредставлениеЧисла(ОтображаемоеЗначение,
		...
		Если ПоказыватьТекущееЗначениеИДеталиПоказателя
			...
			Если НЕ ПустойИсточникДанных И НЕ РасчетыНеактуальны Тогда
				ПоследнееЦелевоеЗначение = Окр(ИсточникДанных.СвойстваДанныхПоПериодам.ЦелевоеЗначение,
			...
			ЭтотОбъект["АбсИзменение__" + ИдентификаторЭлементаФормы] = ЗнакИзменения 
				+ Отчеты.CRM_МониторЦелевыхПоказателей.ПолучитьСокращенноеПредставлениеЧисла(АбсолютноеИзменение,
					 ТочностьРасчетаДробнойЧасти, КратностьЗначений) 
					+ СтрокаРазмерности;
...
//
&НаСервере 
Процедура НарисоватьПустойЭлемент(ГруппаЭлементов, ИдентификаторЭлементаФормы, Высота, Ширина,
...
Процедура НарисоватьТаблицу(ИсточникДанных, ГруппаЭлементов, ИдентификаторЭлементаФормы)
	...
	МассивРеквизитов.Добавить(Новый РеквизитФормы("НаименованиеПоказателя__" 
		+ ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "", НСтр("ru='Наименование показателя';en='Indicator name'")));
	
	Если НЕ ПустойИсточникДанных И НЕ РасчетыНеактуальны Тогда
		МассивРеквизитов.Добавить(Новый РеквизитФормы("КолонкаОбъектАнализа" + "__" 
			+ ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "ТаблицаЗначений__" + ИдентификаторЭлементаФормы, ИсточникДанных.ДинамическиеСвойстваВариантаАнализа.ОбъектАнализаПредставление));
		МассивРеквизитов.Добавить(Новый РеквизитФормы("КолонкаЗначениеАнализа" + "__" 
			+ ИдентификаторЭлементаФормы, ОписаниеТиповСтрок, "ТаблицаЗначений__" + ИдентификаторЭлементаФормы, ИсточникДанных.ДинамическиеСвойстваВариантаАнализа.ЗначениеАнализаПредставление));
		
		Если ВыводитьИтогВТаблице Тогда
			МассивРеквизитов.Добавить(Новый РеквизитФормы("ИтогТаблицаЗначений__" 
				+ ИдентификаторЭлементаФормы, ОписаниеТиповСтрок));
	...
	//	ЭтаФорма["НаименованиеПоказателя__" 
		//+ ИдентификаторЭлементаФормы] = ВариантАнализа.Владелец.КраткоеНаименованиеЦелевогоПоказателя 
		//+ ": " + ВариантАнализа.Наименование + ", " 
		//+ Отчеты.CRM_МониторЦелевыхПоказателей.ПолучитьСтрокуПредставленияЧисел(ВариантАнализа) 
		//+ ?(НЕ ПустаяСтрока(ДинамическиеСвойстваВариантаАнализа.Размерность), " " 
	...
	Если НЕ ПустойИсточникДанных И НЕ РасчетыНеактуальны И НЕ ОшибкаРасчета Тогда
		...
		ПолеПоляТаблицы.ПутьКДанным = "ТаблицаЗначений__" + ИдентификаторЭлементаФормы + "." 
			+ "КолонкаОбъектАнализа" + "__" 
			+ ИдентификаторЭлементаФормы;
		...
		Если ВыводитьИтогВТаблице Тогда 
			...
			ЭтотОбъект["ИтогТаблицаЗначений__" 
				+ ИдентификаторЭлементаФормы] =
					Отчеты.CRM_МониторЦелевыхПоказателей.ПолучитьСокращенноеПредставлениеЧисла(ИсточникДанных.Данные.Итог(ИсточникДанных.ДинамическиеСвойстваВариантаАнализа.ЗначениеАнализа),
						 ТочностьРасчетаДробнойЧасти, КратностьЗначений) + " " 
						+ Размерность;
...
Процедура НарисоватьЭлементыМонитора(СоставМонитора)
	...
	Если РежимАнализаВзаимосвязанных Тогда
		...
		// НарисоватьЭлементыМонитораСУчетомВидаАнркЗоныВниманияВариантовАнализаализаВзаимосвязаных(Анализируемые,
			// Перечисления.ВидыПоискаСвязанныхПоказателей.ПустаяСсылка());
		//
		...
		//	 НарисоватьЭлементыМонитораСУчетомВидаАнализаВзаимосвязаных(Зависимые,
		...
		//	 НарисоватьЭлементыМонитораСУчетомВидаАнализаВзаимосвязаных(Влияющие,
	...
	Иначе 
		Если СтруктураНастроек.ВариантГруппировкиПоказателей = "ПоВажности" Тогда
			...
			НаборИсточниковДанныхНеКраткие = Отчеты.CRM_МониторЦелевыхПоказателей.ПрименитьКТаблицеУсловиеНЕ(НаборИсточниковДанных.Скопировать(),
				 Отбор);
			...
			Если ПоказателиКритическоеСостояниеКратко.Количество() > 0 Тогда
				НарисоватьЭлементыМонитораСУчетомЗоныВнимания(ПоказателиКритическоеСостояниеКратко,
					 ЗоныВнимания.КритическоеСостояние);
				
			КонецЕсли;
			Если ПоказателиКритическоеСостояниеПодробно.Количество() > 0 Тогда
				НарисоватьЭлементыМонитораСУчетомЗоныВнимания(ПоказателиКритическоеСостояниеПодробно,
					 ЗоныВнимания.КритическоеСостояние);
			...
			Если ПоказателиРассчитанныеСОшибкамиКратко.Количество() > 0 Тогда
				НарисоватьЭлементыМонитораСУчетомЗоныВнимания(ПоказателиРассчитанныеСОшибкамиКратко,
					 ЗоныВнимания.РассчитанныеСОшибками);
				
			КонецЕсли;
			Если ПоказателиРассчитанныеСОшибкамиПодробно.Количество() > 0 Тогда
				НарисоватьЭлементыМонитораСУчетомЗоныВнимания(ПоказателиРассчитанныеСОшибкамиПодробно,
					 ЗоныВнимания.РассчитанныеСОшибками);
		...
		ИначеЕсли СтруктураНастроек.ВариантГруппировкиПоказателей = "ПоКатегориямЦелей" Тогда
			...
			НаборГруппИсточниковДанныхТекстовые.Колонки.Добавить("Количество", Новый ОписаниеТипов("Число",
				 Новый КвалификаторыЧисла(2,
				 0)));
			НаборГруппИсточниковДанныхТекстовые.Колонки.Добавить("КоличествоГруппСтрок",
				 Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(2,
				 0)));
			...
			ОтборТекстовых = Новый Структура("ВариантОтображения",
				 Перечисления.CRM_ВариантыОтображенияАнализаПоказателей.Кратко);
			НаборГруппИсточниковДанныхТекстовые = Отчеты.CRM_МониторЦелевыхПоказателей.ПрименитьКТаблицеУсловие(НаборГруппИсточниковДанныхТекстовые,
				 ОтборТекстовых);
			...
			НаборГруппИсточниковДанныхПодробные.Колонки.Добавить("Количество", Новый ОписаниеТипов("Число",
				 Новый КвалификаторыЧисла(2,
				 0)));
			НаборГруппИсточниковДанныхПодробные.Колонки.Добавить("КоличествоГруппСтрок",
				 Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(2,
				 0)));
			...
			НаборГруппИсточниковДанныхПодробные = Отчеты.CRM_МониторЦелевыхПоказателей.ПрименитьКТаблицеУсловиеНЕ(НаборГруппИсточниковДанныхПодробные,
				 ОтборТекстовых);
			...
			Для Каждого ГруппаТекстовогоПоказателя Из НаборГруппИсточниковДанныхТекстовые Цикл
				...
				ОтборПоГруппеТекстовые = Новый Структура("Группа, ВариантОтображения", ГруппаПоказателя,
					 ВариантыОтображенияВариантовАнализа.Кратко);
			...
			Для Каждого ГруппаПодробногоПоказателя Из НаборГруппИсточниковДанныхПодробные Цикл
				...
				НаборИсточниковДанныхНеТекстовые = Отчеты.CRM_МониторЦелевыхПоказателей.ПрименитьКТаблицеУсловиеНЕ(НаборИсточниковДанных.Скопировать(),
					 ОтборТекстовых);
...
Процедура НарисоватьЭлементыМонитораСУчетомГрупп(КоллекцияВариантовАнализа, ИдентификаторГруппы)
	...
	КоличествоПоказателейВПоследнейСтроке =
		КоличествоПоказателейВКоллекции % СтруктураНастроек.КоличествоКолонокМонитораПоказателей;
	...
	Для Каждого ЭлементМонитора Из КоллекцияВариантовАнализа Цикл 
		...
		ИдентификаторЭлементаФормы = СтрЗаменить(Строка(Показатель.УникальныйИдентификатор()), "-", "_") + "__" 
			+ ЭлементМонитора.Позиция;
		...
		Если ВариантОтображения = ВариантыОтображения.Кратко Тогда
			ГруппаСтолбецЭлементов = Элементы["ГруппаКолонкаЭлементовКратко" + ИдентификаторГруппы + "__" 
			...
			// Иначе
			//	ЗаголовокЭлемента = ВариантАнализа.Владелец.КраткоеНаименованиеЦелевогоПоказателя + " (" 
				//+ Отчеты.CRM_МониторЦелевыхПоказателей.ПолучитьСтрокуПредставленияЧисел(ВариантАнализа) 
				//+ ?(НЕ ПустаяСтрока(ДинамическиеСвойстваВариантаАнализа.Размерность), " " + ДинамическиеСвойстваВариантаАнализа.Размерность, "") + ")";
			// КонецЕсли;
			
			ГруппаЭлемент   = ГоризонтальнаяГруппа("Элемент__" + ИдентификаторЭлементаФормы,
				 ЗаголовокЭлемента, ГруппаСтолбецЭлементов);
			ГруппаСотрудник = ГоризонтальнаяГруппаСотрудника("Элемент__Сотрудник__" 
				+ ИдентификаторЭлементаФормы, Строка(ЭлементМонитора.Сотрудник), ГруппаЭлемент);
			...
			Если КоличествоНенарисованныхПоказателей = 0
				...
				Для СчВыравнивающихЭлементов = 1 По (СтруктураНастроек.КоличествоКолонокМонитораПоказателей - СчетчикЭлементовСтроки) Цикл 
					ГруппаСтрокаЭлементов = Элементы["ГруппаКолонкаЭлементовКратко" + ИдентификаторГруппы + "__" 
						+ (СчетчикЭлементовСтроки 
						+ СчВыравнивающихЭлементов)];
					
					ГруппаЭлемент = Элементы.Добавить("КонтейнерВыравнивающегоЭлемента__" 
						+ ИдентификаторЭлементаФормы + "__" + СчВыравнивающихЭлементов, Тип("ГруппаФормы"),
						 ГруппаСтрокаЭлементов);
					...
					НарисоватьПустойЭлемент(ГруппаЭлемент, ИдентификаторЭлементаФормы + "__" 
						+ СчВыравнивающихЭлементов, ВысотаЭлемента, 0,
						 Истина);
		...
		Иначе
			ГруппаСтрокаЭлементов = Элементы["ГруппаКолонкаПодробныхЭлементов" + ИдентификаторГруппы + "__" 
				+ СчетчикЭлементовСтроки];
			ВысотаЭлемента = 10;
		
			ГруппаЭлемент = Элементы.Добавить("Элемент__" + ИдентификаторЭлементаФормы, Тип("ГруппаФормы"),
				 ГруппаСтрокаЭлементов);
			ГруппаЭлемент.Вид = ВидГруппыФормы.ОбычнаяГруппа;
			...
			ГруппаСотрудник = ВертикальнаяГруппаСотрудника("Элемент__Сотрудник__" 
				+ ИдентификаторЭлементаФормы, Строка(ЭлементМонитора.Сотрудник), ГруппаЭлемент);
			...
			Если КоличествоНенарисованныхПоказателей = 0
				 И СчетчикЭлементовСтроки < СтруктураНастроек.КоличествоКолонокМонитораПоказателей Тогда
				Для СчВыравнивающихЭлементов = 1 По (СтруктураНастроек.КоличествоКолонокМонитораПоказателей - СчетчикЭлементовСтроки) Цикл 
					ГруппаСтрокаЭлементов = Элементы["ГруппаКолонкаПодробныхЭлементов" + ИдентификаторГруппы 
						+ "__" + (СчетчикЭлементовСтроки 
						+ СчВыравнивающихЭлементов)];
					ГруппаЭлемент = Элементы.Добавить("Элемент__" + ИдентификаторЭлементаФормы + "__" 
						+ (СчетчикЭлементовСтроки + СчВыравнивающихЭлементов), Тип("ГруппаФормы"),
						 ГруппаСтрокаЭлементов);
					ГруппаЭлемент.Вид = ВидГруппыФормы.ОбычнаяГруппа;
					...
					ГруппаСотрудник = ВертикальнаяГруппаСотрудника("Элемент__Сотрудник__" 
						+ ИдентификаторЭлементаФормы + "__" + (СчетчикЭлементовСтроки + СчВыравнивающихЭлементов), "",
						 ГруппаЭлемент, ОтображениеОбычнойГруппы.СлабоеВыделение);
					
					// Наполним элемент контентом по шаблонам вариантов
					НарисоватьПустыеЭлементыПоказателя(ЭлементМонитора, ВариантОтображения, ГруппаСотрудник, ИдентификаторЭлементаФормы + "__" 
						+ (СчетчикЭлементовСтроки 
						+ СчВыравнивающихЭлементов));
...
Процедура НарисоватьЭлементыМонитораСУчетомЗоныВнимания(КоллекцияВариантовАнализа, ЗонаВнимания)
	...
	Для Каждого ЭлементМонитора Из КоллекцияВариантовАнализа Цикл 
		...
		ИдентификаторЭлементаФормы = СтрЗаменить(Строка(ИсточникДанных.УникальныйИдентификатор()), "-", "_") + "__" 
			+ ЭлементМонитора.Позиция;
		...
		Если ВариантОтображения = ВариантыОтображения.Кратко Тогда
			...
			Если КоличествоНенарисованныхПоказателей = 0
				 И СчетчикЭлементовСтроки < СтруктураНастроек.КоличествоКолонокМонитораПоказателей Тогда
				Для СчВыравнивающихЭлементов = 1 По (СтруктураНастроек.КоличествоКолонокМонитораПоказателей - СчетчикЭлементовСтроки) Цикл 
					ГруппаСтрокаЭлементов = Элементы["ГруппаКолонкаЭлементовКратко" + ИмяЗоныВнимания + "__" 
						+ (СчетчикЭлементовСтроки 
						+ СчВыравнивающихЭлементов)];
		...
		Иначе
			...
			ГруппаСтрокаЭлементов = Элементы["ГруппаКолонкаПодробныхЭлементов" + ИмяЗоныВнимания + "__" 
				+ СчетчикЭлементовСтроки];
			ВысотаЭлемента = 10;
...
Процедура НарисоватьЭлементыПоказателя(ИсточникДанных, ВариантОтображения, ГруппаЭлементов, ИдентификаторЭлементаФормы)
	...
	ДобавитьКомандыВариантовАнализа(ИсточникДанных.Показатель, ИсточникДанных.Сотрудник,
		 ГруппаЭлементов, ИдентификаторЭлементаФормы,
		 ВариантОтображения);
	
КонецПроцедуры

&НаСервере
Процедура НарисоватьПустыеЭлементыПоказателя(ИсточникДанных, ВариантОтображения, ГруппаЭлементов,
	 ИдентификаторЭлементаФормы)
...
Процедура СоздатьБазовуюСтруктуруЭлементовФормы(СоставМонитора)
	...
	ГруппаПоказатели = ВертикальнаяГруппа("ГруппаПоказатели", НСтр("ru='Показатели';en='Indicators'"),
		 Элементы.ОсновнаяСтраницаМонитора);
	
	Если РежимАнализаВзаимосвязанных Тогда
	Иначе
		ГруппаПоказателиКратко = ВертикальнаяГруппа("ГруппаПоказателиКратко",
			 НСтр("ru='Показатели (кратко)';en='Indicators (briefly)'"),
			 ГруппаПоказатели);
		ГруппаПоказателиПодробно = ВертикальнаяГруппа("ГруппаПоказателиПодробно",
			 НСтр("ru='Показатели (подробно)';en='Indicators (in detail)'"),
			 ГруппаПоказатели);
		...
		Если СтруктураНастроек.ВариантГруппировкиПоказателей = "ПоВажности" Тогда
			...
			Если СтатистикаПоСтрокам.КритическоеСостояниеКратко > 0 Тогда
				...
				ГруппаКритическоеСостояниеКратко = ГоризонтальнаяГруппа("ГруппаКритическоеСостояниеКратко",
					 ЗоныВнимания.КритическоеСостояние, ГруппаПоказателиКратко,
					 Истина); 
			...
			Если СтатистикаПоСтрокам.КСведениюКратко > 0 Тогда
				...
				ГруппаКСведениюКратко = ГоризонтальнаяГруппа("ГруппаКСведениюКратко", ЗоныВнимания.КСведению,
			...
			Если СтатистикаПоСтрокам.РассчитанныеСОшибкамиКратко > 0 Тогда
				...
				ГруппаРассчитанныеСОшибкамиКратко = ГоризонтальнаяГруппа("ГруппаРассчитанныеСОшибкамиКратко",
					 ЗоныВнимания.РассчитанныеСОшибками, ГруппаПоказателиКратко,
					 Истина); 
			...
			Если СтатистикаПоСтрокам.КритическоеСостояниеПодробно > 0 Тогда
				...
				ГруппаКритическоеСостояниеПодробно = ГоризонтальнаяГруппа("ГруппаКритическоеСостояниеПодробно",
					 ЗоныВнимания.КритическоеСостояние, ГруппаПоказателиПодробно,
					 Истина); 
			...
			Если СтатистикаПоСтрокам.ВажноПодробно > 0 Тогда
				...
				ГруппаВажноПодробно = ГоризонтальнаяГруппа("ГруппаВажноПодробно", ЗоныВнимания.Важно,
			...
			Если СтатистикаПоСтрокам.КСведениюПодробно > 0 Тогда
				...
				ГруппаКСведениюПодробно = ГоризонтальнаяГруппа("ГруппаКСведениюПодробно",
					 ЗоныВнимания.КСведению, ГруппаПоказателиПодробно,
					 Истина); 
			...
			Если СтатистикаПоСтрокам.РассчитанныеСОшибкамиПодробно > 0 Тогда
				...
				ГруппаРассчитанныеСОшибкамиПодробно = ГоризонтальнаяГруппа("ГруппаРассчитанныеСОшибкамиПодробно",
					 ЗоныВнимания.РассчитанныеСОшибками, ГруппаПоказателиПодробно,
					 Истина); 
		...
		ИначеЕсли СтруктураНастроек.ВариантГруппировкиПоказателей = "ПоКатегориямЦелей" Тогда
			...
			Для Каждого ГруппаТекстовогоПоказателя Из НаборГруппИсточниковДанныхТекстовые Цикл 
				...
				ГруппаТекстовыеЗаголовок = ГоризонтальнаяГруппа("ГруппаТекстовыеЗаголовок__" 
					+ ИдентификаторГруппы, ИмяГруппы, ГруппаПоказателиКратко); 
			...
			Для Каждого ГруппаТабличногоИлиДиаграммногоПоказателя Из НаборГруппИсточниковДанныхПодробные Цикл 
				...
				ИдентификаторГруппы = СтрЗаменить(Строка(ГруппаТабличногоИлиДиаграммногоПоказателя.Группа.УникальныйИдентификатор()),
					 "-",
					 "_");
				КоличествоГруппТаблицИДиаграмм = ГруппаТабличногоИлиДиаграммногоПоказателя.КоличествоГруппСтрок;
				
				ГруппаТабличныеИДиаграммные = ГоризонтальнаяГруппа("ГруппаТабличныеИДиаграммные__" 
					+ ИдентификаторГруппы, ИмяГруппы, ГруппаПоказателиПодробно); 
...
Процедура ОбновитьСоставМонитора(ПолученныйСоставМонитора)
	...
	Для Каждого Команда Из Команды Цикл 
		...
		Если СтрНайти(ИмяКоманды, "Обновить") 
			ИЛИ СтрНайти(ИмяКоманды, "СводныйОтчет")
			ИЛИ СтрНайти(ИмяКоманды, "Настроить") 
			ИЛИ СтрНайти(ИмяКоманды, "ВыбратьПериод")   
			ИЛИ СтрНайти(ИмяКоманды, "ПерейтиВперед")   
			ИЛИ СтрНайти(ИмяКоманды, "КнопкаТумблера")
			ИЛИ СтрНайти(ИмяКоманды, "НастройкиРабочегоСтола")
			ИЛИ СтрНайти(ИмяКоманды, "скИндикаторНапоминаний")
			ИЛИ СтрНайти(ИмяКоманды, "Напоминания")
			ИЛИ СтрНайти(ИмяКоманды, "Заметки")
			ИЛИ СтрНайти(ИмяКоманды, "CRM_Открыть")
			ИЛИ СтрНайти(ИмяКоманды, "ПерейтиНазад") Тогда
			Продолжить;     
...
Функция ПолучитьВариантыОтчетовПоказателя(ВариантАнализа)
	...
	ВариантыОтчетовПоказателя.Добавить("ОтчетРасшифровкаПоказателя",
		 НСтр("ru='Расшифровка показателя';en='Decoding of the benchmark'"));
...
Процедура Подключаемый_ОткрытьОтчет(Команда)
	...
	Если СтрНайти(Команда.Имя, "ОтчетРасшифровкаПоказателя") > 0 Тогда
		Если ЗначениеЗаполнено(ПоказательПериодАнализа) Тогда
			CRM_УправлениеЦелевымиПоказателямиКлиент.ОткрытьРасшифровкуПоказателя(Показатель, Пользователь);
		Иначе
			НачалоКонецПериода = Новый Структура("НачалоПериода, КонецПериода", ОтборНачалоПериода, ОтборКонецПериода);
			CRM_УправлениеЦелевымиПоказателямиКлиент.ОткрытьРасшифровкуПоказателя(Показатель, Пользователь, НачалоКонецПериода);
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьКоличествоГруппСтрок(КоличествоВыводимыхЭлементов, МножительРазмера = 1)
	...
	ПредварительноеКоличествоСтрок = КоличествоВыводимыхЭлементов / ?(КоличествоНеделимыхЭлементовВСтроке < 1,
		 1, Окр(КоличествоНеделимыхЭлементовВСтроке, 0,
...
Процедура ОбновитьСоставМонитораКлиент(ПринудительноОбновитьДанные = Ложь)
	...
	Иначе
		ПараметрыСоставаМонитора = Новый Структура("РежимАнализаВзаимосвязанных, ОтборВариантовАнализа,
