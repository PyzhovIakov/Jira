...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ CRM_ЛицензированиеЭкспортныеМетоды.ПодсистемаCRMИспользуется() Тогда
		CRM_ЛицензированиеЭкспортныеМетоды.ВывестиУведомлениеОНедоступности(НСтр("ru = 'форму Планирования воронок'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Элементы.ПланыСуммаПотенциальная.Заголовок = Элементы.ПланыСуммаПотенциальная.Заголовок + " (" 
...
&НаКлиенте
Процедура ОбъектПланированияНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	
	Если Не ИспользоватьПодразделения Тогда
		СтандартнаяОбработка = Ложь;
		
		СписокДоступных = Новый СписокЗначений;
		СписокДоступных.Добавить(ПредопределенноеЗначение("Перечисление.CRM_ОбъектыПланированияВоронкиПродаж.Предприятие"));
		СписокДоступных.Добавить(ПредопределенноеЗначение("Перечисление.CRM_ОбъектыПланированияВоронкиПродаж.Менеджер"));
...
&НаКлиенте
Процедура ОбъектПланированияАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
...
Процедура НастроитьВидимостьИДоступностьФормы(Форма)
	Если Не Форма.ДоступноИзменениеМенеджераПодразделения Тогда
		...
		Если	(Форма.Объект.ОбъектПланирования <> ПредопределенноеЗначение("Перечисление.CRM_ОбъектыПланированияВоронкиПродаж.Менеджер"))
			Или	(Форма.Объект.ОбъектПланирования = ПредопределенноеЗначение("Перечисление.CRM_ОбъектыПланированияВоронкиПродаж.Менеджер")
				 И Не ЗначениеЗаполнено(Форма.Объект.Менеджер)) Тогда
			//
			Форма.Элементы.ВоронкаПродаж.ТолькоПросмотр = Истина;
			Форма.Элементы.Планы.ТолькоПросмотр = Истина;
		КонецЕсли;
	Иначе
		Форма.Элементы.Подразделение.Видимость	=
			(Форма.Объект.ОбъектПланирования = ПредопределенноеЗначение("Перечисление.CRM_ОбъектыПланированияВоронкиПродаж.Подразделение"));
		Форма.Элементы.Подразделение.АвтоОтметкаНезаполненного =
			(Форма.Объект.ОбъектПланирования = ПредопределенноеЗначение("Перечисление.CRM_ОбъектыПланированияВоронкиПродаж.Подразделение"));
		Форма.Элементы.Менеджер.Видимость		=
			(Форма.Объект.ОбъектПланирования = ПредопределенноеЗначение("Перечисление.CRM_ОбъектыПланированияВоронкиПродаж.Менеджер"));
		Форма.Элементы.Менеджер.АвтоОтметкаНезаполненного		=
			(Форма.Объект.ОбъектПланирования = ПредопределенноеЗначение("Перечисление.CRM_ОбъектыПланированияВоронкиПродаж.Менеджер"));
	КонецЕсли;
КонецПроцедуры
...
Процедура ПолучитьДанныеЗаПериодРассчитать()
	...
	ДатаОкончанияПериода	= ?(ЗначениеЗаполнено(ПериодАнализа.ДатаОкончания),
		 ПериодАнализа.ДатаОкончания,
		 ТекущаяДатаСеанса());
	ГоризонтВоронки			= ДатаОкончанияПериода;
	ГоризонтВоронкиПрогноз	= ГоризонтВоронки;
	ДатаКонецПериода		= ?(ЗначениеЗаполнено(ПериодАнализа.ДатаОкончания), ПериодАнализа.ДатаОкончания,
		 ТекущаяДатаСеанса());
	...
	Если Объект.ОбъектПланирования = Перечисления.CRM_ОбъектыПланированияВоронкиПродаж.Подразделение Тогда
		УстановитьЗначениеВОтборе(НастройкиСКД.Отбор, "Подразделение", ВидСравненияКомпоновкиДанных.Равно,
			 Объект.Подразделение,
			 Истина);
	КонецЕсли;
	
	Если Объект.ОбъектПланирования = Перечисления.CRM_ОбъектыПланированияВоронкиПродаж.Менеджер Тогда
		УстановитьЗначениеВОтборе(НастройкиСКД.Отбор, "Пользователь", ВидСравненияКомпоновкиДанных.Равно,
			 Объект.Менеджер,
			 Истина);
	КонецЕсли;
	...
	Для Каждого СтрокаЭтап Из ЭтапыТекущейВоронки Цикл
		...
		СерияНеудачно.Подсказка = НСтр("ru='Неудачно: ';en='Failed: '") + Строка(СерияНеудачно.Показатель) 
			+ РассчитанныеПоказатели.ПоказательПроцент;
		...
		СерияУспешно.Подсказка = НСтр("ru='Успешно: ';en='Successfully: '") + Строка(СерияУспешно.Показатель) 
			+ РассчитанныеПоказатели.ПоказательПроцент;
			...
			Пока ВыполнятьЦикл Цикл
				...
				ИЛИ СтрокаТочки_1.Значение.ВидЭтапа = Перечисления.CRM_ВидыЭтаповВоронкиПродаж.ПоСостояниямИнтересов Тогда
					...
					Если СтрокаТочки_1.Текст = "Неудачно" ИЛИ СтрокаТочки_1.Текст = "Успешно" Тогда
						...
						ПоказательСуммаПотенциальнаяВсего	= ПоказательСуммаПотенциальнаяВсего 
							+ СтрокаТочки_1.ПоказательСуммаПотенциальная;
					КонецЕсли;
				Иначе
	...
	Для Каждого СтрокаЭтап Из ЭтапыТекущейВоронки Цикл
		...
		Если НайденныеСтрокиТекущие.Количество() > 0 Тогда
			...
			Для Каждого НайденнаяСтрока Из НайденныеСтрокиТекущие Цикл
				...
				НоваяСтрока.Количество = НоваяСтрока.Количество + НайденнаяСтрока.КоличествоНеудачных 
					+ НайденнаяСтрока.КоличествоУспешных;
			КонецЦикла;
	...
	Если ТаблицаИнформация.Количество() > 0 Тогда
		ПлановоеКоличество	= (ТаблицаИнформация[ТаблицаИнформация.Количество() - 1].Количество * ПлановаяСумма) / ?(ТаблицаИнформация[ТаблицаИнформация.Количество() - 1].Сумма = 0,
			 1,
			 ТаблицаИнформация[ТаблицаИнформация.Количество() - 1].Сумма);
...
&НаСервере
Функция РассчитатьПоказательДляЭтапа(ДанныеТекущие, ДанныеСтатистика = Неопределено, Этап,
	 ВидПоказателя = Неопределено, ЗначениеТекущейГруппировки = Неопределено,
	 ИмяТекущейГруппировки = Неопределено, Успешные = Неопределено,
	 Активные = Ложь)
	
	РассчитанныеПоказатели = Новый Структура("Показатель,ПоказательКоличество,ПоказательСумма,
		|ПоказательСуммаПотенциальная,ПоказательПроцент", 0, 0, 0, 0,
		 "");
	...
	Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
		...
		РассчитанныеПоказатели.ПоказательКоличество = РассчитанныеПоказатели.ПоказательКоличество + ?(Успешные = Неопределено,
																									НайденнаяСтрока.КоличествоАктивных + НайденнаяСтрока.КоличествоУспешно 
																										+ НайденнаяСтрока.КоличествоНеудачных,
		...
		РассчитанныеПоказатели.ПоказательСумма		= РассчитанныеПоказатели.ПоказательСумма + ?(Успешные = Неопределено,
																							   НайденнаяСтрока.СуммаАктивных + НайденнаяСтрока.СуммаУспешных 
																							   	+ НайденнаяСтрока.СуммаНеудачных,
...
Процедура УстановитьВидимостьПолейРасчета()
	...
	Элементы.ПериодАнализа.Видимость	=
		(Объект.ВариантПланирования = ПредопределенноеЗначение("Перечисление.CRM_ВариантыПланированияВоронки.ПоКоличеству"));
	Элементы.ЦелеваяСумма.Видимость		=
		(Объект.ВариантПланирования = ПредопределенноеЗначение("Перечисление.CRM_ВариантыПланированияВоронки.ПоКоличеству"));
	Элементы.ПланыРассчитать.Видимость	=
		(Объект.ВариантПланирования = ПредопределенноеЗначение("Перечисление.CRM_ВариантыПланированияВоронки.ПоКоличеству"));
	Элементы.ПланыКонверсия.Видимость	=
		(Объект.ВариантПланирования = ПредопределенноеЗначение("Перечисление.CRM_ВариантыПланированияВоронки.ПоКоличеству"));
