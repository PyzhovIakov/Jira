...
Процедура ОчиститьНеиспользуемыеРеквизитыТоваров(Объект, Параметры) Экспорт
	...
	Иначе
		...
		ЗапретПревышенияПоСумме =
			(Параметры.ПоказательТоваров = ПоказателиПродаж.Сумма
		...
		Для Каждого СтрокаТЧ Из Объект.Товары Цикл
			...
			Если Параметры.ОтборТоваров = Перечисления.СоставыСписковРетроБонусов.Выбранные Тогда
				...
				Если (Параметры.ПоказательТоваров <> ПоказателиПродаж.Количество
					  И Параметры.ПоказательТоваров <> ПоказателиПродаж.КоличествоСовокупно
					  И Параметры.ПоказательТоваров <> ПоказателиПродаж.ПакетноеПредложение) Тогда
...
Процедура НастройкаПоляСуммаПлан(Настройки, МодульНастройки)
	...
	МодульНастройки.НовыйОтбор(Элемент.Условие, "Дополнительно.ПоказательТоваров", ПоказателиПродаж.Сумма);
	МодульНастройки.НовыйОтбор(Элемент.Условие, "Дополнительно.ЕстьНачисления", Истина);
