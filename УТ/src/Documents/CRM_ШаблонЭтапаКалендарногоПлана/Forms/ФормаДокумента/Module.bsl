...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если объект.Ссылка.Пустая() Тогда
		...
		НастроитьДоступностьВидимостьРежимВводаОтЗадачи();
	...
	ИначеЕсли Объект.ТипЭтапа = Перечисления.CRM_ТипыЭтапов.КонтрольнаяТочка Тогда
		...
		Элементы.Тема.ПодсказкаВвода = НСтр("ru='Введите наименование шаблона контрольной точки';
			|en='Enter name of check points template'");
	...
	Иначе
		...
		Элементы.Тема.ПодсказкаВвода = НСтр("ru='Введите наименование шаблона задачи проекта';
			|en='Enter name of project task template'");
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Запись_Файл"
		 И (ТипЗнч(Источник) = Тип("СправочникСсылка.CRM_ШаблонЭтапаКалендарногоПланаПрисоединенныеФайлы")
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ЗначениеЗаполнено(ТекущийОбъект.ШаблонЭтапаПроекта)
		 И ТекущийОбъект.ШаблонЭтапаПроекта.ШаблонПроекта <> ТекущийОбъект.ШаблонПроекта
		Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Выбранный пакет задач не соответствует Проекту! Запись невозможна.';
			|en='Selected package of tasks does not match to Project! Recording is not possible.'"), , , ,
			 Отказ);
	КонецЕсли; 
	
	Если ТекущийОбъект.ТипЭтапа = Перечисления.CRM_ТипыЭтапов.Этап И ТекущийОбъект.ПродолжительностьДней = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Длительность этапа должна быть не менее 1 дня! Запись невозможна.';
			|en='Duration of stage shall be not less than 1 day! Recording is not possible.'"), , , ,
			 Отказ);
	КонецЕсли; 
	...
	Если Объект.Участники.Количество() > 0 Тогда
		...
		Если Строки.Количество() = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
							НСтр("ru='Необходимо указать хотя бы одного ответственного!';
...
Процедура УчастникиДобавитьОтветственных()
	...
	Если НЕ Объект.ШаблонПроекта.Пустая() И Не Объект.ШаблонПроекта.ЭтоГруппа
		 И ЗначениеЗаполнено(Объект.ШаблонПроекта.Ответственный) Тогда
...
Процедура НастроитьДоступностьВидимостьРежимВводаОтЗадачи()
	
	РежимВводаОтЗадачи = Ложь;
	Если ЗначениеЗаполнено(Объект.ШаблонПроекта) Тогда
		РежимВводаОтЗадачи = Объект.ШаблонПроекта.РежимВводаОтЗадачи;
	ИначеЕсли ЗначениеЗаполнено(Объект.ШаблонЭтапаПроекта)
		И ЗначениеЗаполнено(Объект.ШаблонЭтапаПроекта.ШаблонПроекта) Тогда
		РежимВводаОтЗадачи = Объект.ШаблонЭтапаПроекта.ШаблонПроекта.РежимВводаОтЗадачи;
	КонецЕсли;
	
	Элементы.Смещение.Доступность = НЕ РежимВводаОтЗадачи;
	Элементы.СмещениеОтЗадачи.Видимость = РежимВводаОтЗадачи;
