...
Процедура ПриКопировании(ОбъектКопирования)
	...
	ДатаНачалаВыполненияРассылки = Дата;
...
Процедура ЗаполнитьПоНазначениюОпросов(ДанныеЗаполнения)
	...
	Если Основание.СвободныйОпрос Тогда
		...
		Пока ВыборкаПоРеспондентам.Следующий() Цикл
			...
			Если ТипЗнч(Основание.ТипРеспондентов) = Тип("СправочникСсылка.Пользователи") Тогда
				Таб = CRM_ОбщегоНазначенияСервер.ПолучитьКонтактнуюИнформациюПоТипуКИ(НоваяСтрока.Партнер,
			...
			Если Таб.Количество() > 0 Тогда
				...
				НоваяСтрока.ПредставлениеАдреса	= ?(Таб[0].Ссылка = НоваяСтрока.Партнер,
					 Строка(НоваяСтрока.Партнер), Строка(НоваяСтрока.КонтактноеЛицо)) + " <" + Таб[0].Представление 
					+ ">";
			КонецЕсли;
	...
	Иначе
		...
		Иначе
			...
			ЗапросПоРеспондентам.Текст = СтрЗаменить(ЗапросПоРеспондентам.Текст,
				 "И НЕ(НазначениеОпросовРеспонденты.Респондент В (&РеспондентыИсключения))",
				 "");
	...
	НаименованиеОрганизации = ?(ПустаяСтрока(Организация.НаименованиеПолное), Организация.Наименование,
		...
		Для Каждого элСп Из СписокПартнеров Цикл
			...
			Если ЗначениеЗаполнено(элСп.Партнер) Тогда
				...
				Если Таб.Количество() > 0 Тогда
					...
					ПредставлениеАдреса	= ?(Таб[0].Ссылка = элСп.Партнер, Строка(элСп.Партнер), Строка(элСп.КонтактноеЛицо)) + " <" 
						+ Таб[0].Представление 
						+ ">";
