...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	//	ТаблицаРабочихДней = CRM_УправлениеЦелевымиПоказателямиСервер.ПолучитьТаблицуРабочихДней(НачалоПериодаПланирования,
		// "Месяц",
		// Подразделение); 
	// Иначе
		ТаблицаРабочихДней = CRM_УправлениеЦелевымиПоказателямиСервер.ПолучитьТаблицуРабочихДней(НачалоПериодаПланирования,
		...
		Если ТаблицаРабочихДней.Количество() = 0 Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заполнен производственный календарь для подразделения ""';
				|en='Production schedule not fill for department ""'")
