...
Функция ПолучитьСписокДоступныхРезультатов()
	...
	МассивВозможныхТипов.Добавить(Тип("СправочникСсылка.Партнеры"));
	МассивВозможныхТипов.Добавить(Тип("СправочникСсылка.CRM_ПотенциальныеКлиенты"));
	МассивВозможныхТипов.Добавить(Тип("СправочникСсылка.КонтактныеЛицаПартнеров"));
	...
	Если СписокСуществующихРезультатов.Количество() > 0 Тогда
		Для Каждого ЭлементСписка Из СписокСуществующихРезультатов Цикл
			...
			Если СохраненныйСписок.Количество() > 0
				И МассивВозможныхТипов.Найти(ТипЗнч(СохраненныйСписок[0].Значение)) <> Неопределено Тогда
