...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("БизнесПроцессСсылка.CRM_БизнесПроцесс")
		...
		Иначе
			CRM_ОбщегоНазначенияСервер.ЗаполнитьОбъектБизнесПроцесса(ЭтотОбъект,
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	// Очистка взаимодействий с участниками.
	Для Каждого ТекУчастник Из Участники Цикл
		ТекУчастник.Обработан = Ложь;
		ТекУчастник.НеДозвонились = Ложь;
		ТекУчастник.Отказ = Ложь;
		ТекУчастник.ПричинаОтказа = Справочники.CRM_ПричиныОтказовПриОбзвоне.ПустаяСсылка();
		ТекУчастник.ПерезвонитьПозднее = Ложь;
		ТекУчастник.КоличествоЗвонков = 0;
		ТекУчастник.Звонок = Документы.ТелефонныйЗвонок.ПустаяСсылка();
		ТекУчастник.Взаимодействие = Документы.CRM_Взаимодействие.ПустаяСсылка();
		ТекУчастник.Интерес = Документы.CRM_Интерес.ПустаяСсылка();
		ТекУчастник.Примечание = "";
	КонецЦикла;
...
Процедура ЗаполнитьПоМаркетинговойКампании(ПараметрДанныеЗаполнения)
	...
	Если ТипЗнч(ПараметрДанныеЗаполнения) = Тип("Структура")
		 И ПараметрДанныеЗаполнения.Свойство("МаркетинговаяКампания") Тогда
