...
Процедура ПриИзмененииНоменклатурыХарактеристикиКоличестваЛокализация()
	...
	//++ Локализация
	Если (ОбщегоНазначенияЕГАИСКлиентСерверПовтИсп.ИспользуетсяМаркируемаяПродукция()
			ИЛИ ИнтеграцияИСМПКлиентСерверПовтИсп.ВестиУчетМаркируемойПродукции())
...
Процедура УказатьРазрешениеНаПродажуВСценарииОднойСтроки()
	...
	Если ТекущаяСтрока <> Неопределено И ТекущаяСтрока.ПродаетсяПоРазрешению Тогда 
