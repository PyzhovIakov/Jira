...
Функция ГотовоСервер()
	...
	Попытка
		...
		Для Каждого ТекСтрока Из ЗаказыПоставщикам Цикл
				...
				ОбрабатываемыеСтрокиСумм = Новый Массив;
				ОбрабатываемыеСтрокиСуммБезПересчетаРучнойСкидки = Новый Массив;
				ОбрабатываемыеСтрокиЦен = Новый Массив;
				ОбрабатываемыеСтрокиЦенСПересчетомРучнойСкидки = Новый Массив;
				ОбрабатываемыеСтрокиЦенБезПересчетаРучнойСкидки = Новый Массив;
						...
						Если НЕ СтрокаОтклонений.ЦенаИзменилась И СтрокаОтклонений.СуммаИзменилась Тогда
							...
							Если НайденнаяСтрока <> Неопределено Тогда
								...
								Если СтрокаОтклонений.ПроцентРучнойСкидкиПоступления <> 0 Тогда
									...
									ОбрабатываемыеСтрокиСумм.Добавить(НайденнаяСтрока);
									
								ИначеЕсли СтрокаОтклонений.СуммаРучнойСкидкиПоступления <> 0 Тогда
									ОбрабатываемыеСтрокиСуммБезПересчетаРучнойСкидки.Добавить(НайденнаяСтрока);
						...
						Иначе
							Если НайденнаяСтрока <> Неопределено Тогда
								...
								Если СтрокаОтклонений.ПроцентРучнойСкидкиПоступления <> 0 Тогда
									...
									ОбрабатываемыеСтрокиЦен.Добавить(НайденнаяСтрока);
								ИначеЕсли СтрокаОтклонений.СуммаРучнойСкидкиПоступления <> 0 Тогда
									ОбрабатываемыеСтрокиЦенСПересчетомРучнойСкидки.Добавить(НайденнаяСтрока);
									НайденнаяСтрока.СуммаРучнойСкидки = СтрокаОтклонений.СуммаРучнойСкидкиПоступления;
									НайденнаяСтрока.ПроцентРучнойСкидки = 0;
								Иначе
									ОбрабатываемыеСтрокиЦенБезПересчетаРучнойСкидки.Добавить(НайденнаяСтрока);
									НайденнаяСтрока.ПроцентРучнойСкидки = 0;
								КонецЕсли;
								ЦеныСкорректированы = Истина;
				...
				ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
					ОбрабатываемыеСтрокиСуммБезПересчетаРучнойСкидки,
				...
				ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
					ОбрабатываемыеСтрокиЦенБезПересчетаРучнойСкидки,
				...
				ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
					ОбрабатываемыеСтрокиЦенСПересчетомРучнойСкидки,
					СтруктураДействийИзмененияЦен,
					Объект.Товары,
					КэшированныеЗначения);
