...
Процедура ЗаполнитьПоРезультатамОтчета(Команда)
	...
	ОткрытьФорму("Документ.НазначениеОпросов.Форма.CRM_ФормаВыбораРезультатаОтчета", ПараметрыФормы,
...
Процедура ЗаполнитьПоГруппе(Команда)
	...
	Если ТипЗнч(Объект.ТипРеспондентов) = Тип("СправочникСсылка.Пользователи") Тогда
		...
		ПараметрыФормы = Новый Структура(
			"РежимВыбора,ЗакрыватьПриВыборе,ВыборГруппПользователей,РежимПодбораПоГруппировкам",
...
Процедура ЗаполнитьПоСегменту(Команда)
	...
	Если ТипЗнч(Объект.ТипРеспондентов) = Тип("СправочникСсылка.Пользователи") Тогда
		ПоказатьПредупреждение(,
			 НСтр("ru='Для пользователей заполнение по сегменту не используется!';
			|en='For users filling on a segment are not us!'"),
			 10);
		Возврат;
	КонецЕсли;
	ПараметрыФормы = Новый Структура();
	...
	ОткрытьФорму("Документ.НазначениеОпросов.Форма.CRM_ФормаВыборСегмента", ПараметрыФормы, ЭтотОбъект,
		 , , , ОписаниеОповещения,
		 РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
"Создать новую рассылку только для новых респондентов?'");
		ОписаниеОповещения = Новый ОписаниеОповещения("СоздатьРассылкуЗавершение", ЭтотОбъект,
			 РассылкаТолькоДляНовыхРеспонденов);	
