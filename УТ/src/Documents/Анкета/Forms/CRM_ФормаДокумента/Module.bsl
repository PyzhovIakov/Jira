...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.CRM_ШаблонАнкеты) Тогда
		...
		Если (НЕ Объект.Проведен) И ЗначениеЗаполнено(Объект.РедактируемыйРаздел) Тогда
			Если ТипЗнч(Объект.РедактируемыйРаздел) = Тип("СправочникСсылка.ВопросыШаблонаАнкеты") Тогда
				НомерТекущегоРаздела = АнкетированиеКлиентСервер.НайтиСтрокуВДанныхФормыДерево(ДеревоРазделов,
					 Объект.РедактируемыйРаздел, "Ссылка",
					 Истина);
			Иначе
				НомерТекущегоРаздела = АнкетированиеКлиентСервер.НайтиСтрокуВДанныхФормыДерево(ДеревоРазделов,
					 Объект.РедактируемыйРаздел, "ТипСтроки",
					 Истина);
			КонецЕсли;
			Если НомерТекущегоРаздела = -1 Тогда
				НомерТекущегоРаздела = 0;
			...
			Если НЕ (ТекущийВопросТипОтвета = Перечисления.ТипыОтветовНаВопрос.НесколькоВариантовИз) 
				...
				Если НЕ ПустаяСтрока(СтрокаВопросРаздела.ЭлементарныйВопрос.CRM_ИмяСправочника) Тогда
					Если НЕ (СтрокаВопросРаздела.ТипВопроса = Перечисления.ТипыВопросовШаблонаАнкеты.Табличный) Тогда
						ИмяВопроса = АнкетированиеКлиентСервер.ИмяВопроса(СтрокаВопросРаздела.КлючСтроки);
						Если (НЕ ЗначениеЗаполнено(ЭтотОбъект[ИмяВопроса]))
							...
							ЗначениеИзСправочника = CRM_АнкетированиеПривилегированный.ПолучитьРеквизитИзСправочника(Объект.Респондент,
								 СтрокаВопросРаздела.ЭлементарныйВопрос);
