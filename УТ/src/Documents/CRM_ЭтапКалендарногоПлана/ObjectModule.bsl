...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
		...
		Если ДанныеЗаполнения.Свойство("Исполнитель") Тогда
			...
			Если  (ДанныеЗаполнения.Свойство("Проект")
				 И ДанныеЗаполнения.Проект.Ответственный = ДанныеЗаполнения.Исполнитель) ИЛИ
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Проект.CRM_Статус = Перечисления.CRM_СтатусыПроектов.ВРаботе И ЗначениеЗаполнено(ПлановаяДатаНачала) 
		 И ПлановаяДатаНачала <= НачалоДня(ТекущаяДатаСеанса())
		 	 И Статус = Перечисления.CRM_СтатусыЭтаповКалендарногоПлана.Запланирована Тогда
	...
	Если ЗначениеЗаполнено(ФактическаяДатаНачала) 
		И ФактическаяДатаНачала <= НачалоДня(ТекущаяДатаСеанса())
			 И Статус = Перечисления.CRM_СтатусыЭтаповКалендарногоПлана.Запланирована Тогда
		
		Статус = Перечисления.CRM_СтатусыЭтаповКалендарногоПлана.ВРаботе;
	
	ИначеЕсли ТипЭтапа = Перечисления.CRM_ТипыЭтапов.Этап
		 И Статус = Перечисления.CRM_СтатусыЭтаповКалендарногоПлана.ВРаботе 
	...
	// Минимизируем процедуру обновления дат пакетов задач
 	Если ПлановаяДатаНачала <> Ссылка.ПлановаяДатаНачала ИЛИ ПлановаяДатаОкончания <> Ссылка.ПлановаяДатаОкончания ИЛИ
		 ФактическаяДатаНачала <> Ссылка.ФактическаяДатаНачала
		 	 ИЛИ ФактическаяДатаОкончания <> Ссылка.ФактическаяДатаОкончания 
		 	 ИЛИ Этап <> Ссылка.Этап
		 ИЛИ (Статус <> Ссылка.Статус И (Статус = Перечисления.CRM_СтатусыЭтаповКалендарногоПлана.Отменена
		 	 ИЛИ Ссылка.Статус = Перечисления.CRM_СтатусыЭтаповКалендарногоПлана.Отменена))
...
Процедура ПриЗаписи(Отказ)
	...
	Если ДополнительныеСвойства.Свойство("ОбновитьДатыПакетов") Тогда
		...
		КонецЕсли;
		Если ДополнительныеСвойства.Свойство("ПредыдущийЭтап")
			 И ЗначениеЗаполнено(ДополнительныеСвойства.ПредыдущийЭтап) Тогда
	...
	Если ДополнительныеСвойства.Свойство("МассивУчастниковДляОповещенияОЗадаче") Тогда
		...
		ПараметрыОповещения = CRM_ОповещенияСервер.ПолучитьПараметрыОповещения(ДополнительныеСвойства.МассивУчастниковДляОповещенияОЗадаче,
			 ВидОповещения,
			 Ссылка);
