...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ CRM_ЛицензированиеЭкспортныеМетоды.ПодсистемаCRMИспользуется() Тогда
		CRM_ЛицензированиеЭкспортныеМетоды.ВывестиУведомлениеОНедоступности(НСтр("ru = 'форму списка Контрольных точек'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	
	CRM_ЛицензированиеЭкспортныеМетоды.ПолучитьЗащищеннуюОбработку().
		Документ_CRM_ЭтапКалендарногоПлана_ФормаСписка_ПриСозданииНаСервере(ЭтотОбъект, Отказ, СтандартнаяОбработка);
...
Процедура ПриОткрытии(Отказ)
	...
	КонецЕсли;
	CRM_ТегиКлиент.БыстрыйОтборИзменение(ЭтотОбъект, ОтборПроект, ОтборТипЭтапа, ОтборСтатус,
		 ОтборСтатусПроекта, ОтборРуководительПроекта, ОтборОтветственный,
		 ОтборУчастник);
	CRM_ТегиКлиент.БыстрыйОтборИзменение(ЭтотОбъект, ВидОтбораДатаНачалаПлан, ВидОтбораДатаНачалаФакт,
		 ВидОтбораДатаОкончанияПлан, ВидОтбораДатаОкончанияФакт, , , , , , ,
		 "ОтборПериод");
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	Если ЗначениеЗаполнено(ОтборСтатусСписокРеквизитов) Тогда
		Для каждого Элемент Из ОтборСтатусСписокРеквизитов Цикл
			Если Метаданные.Перечисления.CRM_СтатусыЭтаповКалендарногоПлана.ЗначенияПеречисления.Найти(Сред(Элемент.Значение,
				 СтрНайти(Элемент.Значение, "__") 
				+ 2)) = Неопределено Тогда
	...
	Если ЗначениеЗаполнено(ОтборСтатус) Тогда
		Для каждого Элемент Из ОтборСтатусСписокРеквизитов Цикл
			Если ОтборСтатус.НайтиПоЗначению(ПредопределенноеЗначение("Перечисление." 
				+ СтрЗаменить(Элемент.Значение, "__", "."))) <> Неопределено Тогда
				ЭтотОбъект[Элемент.Значение] = Истина;
			КонецЕсли;
		КонецЦикла;
		CRM_ОбщегоНазначенияКлиентСервер.ИзменитьЭлементОтбораСписка(Список, "Статус", ОтборСтатус,
			 Истина,
			 ВидСравненияКомпоновкиДанных.ВСписке);
	КонецЕсли;
	Если ЗначениеЗаполнено(ОтборСтатусПроекта) Тогда
		Для каждого Элемент Из ОтборСтатусПроектаСписокРеквизитов Цикл
			Если ОтборСтатусПроекта.НайтиПоЗначению(ПредопределенноеЗначение("Перечисление." 
				+ СтрЗаменить(Элемент.Значение, "__", "."))) <> Неопределено Тогда
				ЭтотОбъект[Элемент.Значение] = Истина;
			КонецЕсли;
		КонецЦикла;
		CRM_ОбщегоНазначенияКлиентСервер.ИзменитьЭлементОтбораСписка(Список, "Проект.CRM_Статус",
			 ОтборСтатусПроекта, Истина,
			 ВидСравненияКомпоновкиДанных.ВСписке);
	КонецЕсли;
