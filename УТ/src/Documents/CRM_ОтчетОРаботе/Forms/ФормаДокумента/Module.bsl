...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ CRM_ЛицензированиеЭкспортныеМетоды.ПодсистемаCRMИспользуется() Тогда
		CRM_ЛицензированиеЭкспортныеМетоды.ВывестиУведомлениеОНедоступности(НСтр("ru = 'форму Отчета о работе'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	
	CRM_ЛицензированиеЭкспортныеМетоды.ПолучитьЗащищеннуюОбработку().
		Документ_CRM_ОтчетОРаботе_ФормаДокумента_ПриСозданииНаСервере(ЭтотОбъект, Отказ, СтандартнаяОбработка);
	...
	Элементы.СтрокиОтчетаКомандаЗаполнитьПоРабочемуВремени.Видимость =
		Константы.CRM_ИспользоватьУчетРабочегоВремени.Получить();
		...
		// Проверка рабочих дат на выходные дни.
			...
			Если Не ПустаяСтрока(СтрСтроки) Тогда
				...
				ПоказатьВопрос(ОписаниеОповещения,
					 НСтр("ru='Рабочие даты попадают на выходные дни. Строки:';
					|en='Working dates got for the weekend. Lines:'")
					+ " " + СтрСтроки + ". "
...
Процедура СтрокиОтчетаПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		Если Не ЗначениеЗаполнено(ТекДанные.ДатаР) И Не ЗначениеЗаполнено(ТекДанные.Проект) Тогда
			...
			Если НачалоНедели(Объект.Дата) = НачалоНедели(ОбщегоНазначенияКлиент.ДатаСеанса()) Тогда
				ТекДанные.ДатаР = НачалоДня(ОбщегоНазначенияКлиент.ДатаСеанса());
			Иначе
				ТекДанные.ДатаР = НачалоДня(Объект.НачалоПериода);
			КонецЕсли;
			
			ТекДанные.Проект = CRM_ОбщегоНазначенияПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(Объект.Сотрудник,
				 "ОсновнойПроект");
...
Процедура КомандаЗаполнитьПоСуществующемуОтчету(Команда)
	Если Объект.СтрокиОтчета.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("КомандаЗаполнитьПоСуществующемуОтчетуЗавершение",
			 ЭтотОбъект),
			 НСтр("ru='Заполнить с очисткой табличной части?';en='Fill with cleaning of tabular section?'"),
...
Процедура КомандаЗаполнитьПоСуществующемуОтчетуФрагмент(Знач Ответ)
	...
	ОткрытьФорму("Документ.CRM_ОтчетОРаботе.ФормаВыбора", , ЭтотОбъект, , , ,
		 Новый ОписаниеОповещения("КомандаЗаполнитьПоСуществующемуОтчетуФрагментЗавершение", ЭтотОбъект,
		 Ответ));
...
Процедура КомандаЗаполнитьПоРабочемуВремениНаСервере()
	...
	|	CRM_РабочееВремяПользователей.Этап КАК Этап,
	|	CRM_РабочееВремяПользователей.Проект КАК Проект,
...
Процедура КомандаЗаполнитьПоРабочемуВремени(Команда)
	Если Объект.СтрокиОтчета.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("КомандаЗаполнитьПоРабочемуВремениЗавершение",
			 ЭтотОбъект),
			 НСтр("ru='Заполнить с очисткой табличной части?';en='Fill with cleaning of tabular section?'"),
			 РежимДиалогаВопрос.ДаНетОтмена);
        Возврат;
...
Процедура ЗаполнитьДни()
	...
	Если МинДата <> Неопределено И МаксДата <> Неопределено Тогда
		...
		СоответствиеКалендарныеДни = CRM_ОбщегоНазначенияСервер.ПолучитьСоответствиеКалендарныхГрафиков(МассивДат, ,
			 Объект.Сотрудник);
		
		Для Каждого СтрокаТаблицы Из Объект.СтрокиОтчета Цикл
			Если ЗначениеЗаполнено(СтрокаТаблицы.ДатаР) Тогда
				СтрокаТаблицы.ЭтоНеРабочийДень = ЭтоНеРабочийДень(СтрокаТаблицы.ДатаР, Объект.Сотрудник,
					 СоответствиеКалендарныеДни);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры
...
Функция ПолучитьТекущийЭтапПроекта(Проект)
	...
	Если ЗначениеЗаполнено(Проект.CRM_ТекущийЭтап) Тогда
...
Функция ЭтоНеРабочийДень(Дата, Сотрудник, Знач СоответствиеКалендарныеДни = Неопределено)
	Если ТипЗнч(СоответствиеКалендарныеДни) <> Тип("Соответствие") Тогда
		СоответствиеКалендарныеДни = CRM_ОбщегоНазначенияСервер.ПолучитьСоответствиеКалендарныхГрафиков(НачалоДня(Дата), ,
			 Сотрудник);
	КонецЕсли;
	Если ТипЗнч(СоответствиеКалендарныеДни) = Тип("Соответствие") Тогда
...
Функция ПроверкаКорректностиЗаполненияНаСервере()
	...
	// Если у проекта есть хотя бы один этап - в строке ТЧ обязательно должен быть указан этап.
	Запрос = Новый Запрос(
		...
		Если СтрокаТаблицы.ДатаР < Объект.НачалоПериода Или СтрокаТаблицы.ДатаР > Объект.КонецПериода Тогда
			Ошибки = Ошибки
				...
				+ НСтр("ru='дата работ находится за пределами выбранного периода!';
					|en='date of operations are outside of the select period!'");
			//
		КонецЕсли;
	КонецЦикла;
	Если Не ПустаяСтрока(Ошибки) Тогда
