...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.МакетMS.СписокВыбора.Добавить("CRM_ПФ_DOC_КоммерческоеПредложение",
		 "Коммерческое предложение (Microsoft Word)");
	Элементы.МакетOpenOffice.СписокВыбора.Добавить("CRM_ПФ_ODT_КоммерческоеПредложение",
...
Процедура ВариантПредставлениеПриИзменении(Элемент)
	...
	Для Каждого ЭлементСписка Из СписокВариантов Цикл
		Если НЕ ЭлементСписка.Представление = ВариантПредставление Тогда
			Продолжить;
		КонецЕсли;
...
Процедура КнопкаОК(Команда)
	...
	Если НЕ ФорматODT И НЕ ФорматDOC Тогда
		ПоказатьПредупреждение(,
			 НСтр("ru='Необходимо указать как минимум один из предложенных форматов.';
			|en='It is necessary to specify at least one of the offer formats.'"));
		Возврат;
	КонецЕсли;
