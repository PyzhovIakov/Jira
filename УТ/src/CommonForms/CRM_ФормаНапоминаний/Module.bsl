...
//
Процедура ОбновитьТаблицуНапоминаний(СтруктураНовыеНапоминания, // АПК:78 - исключить из проверки.
	 ОткрыватьАктивизироватьФорму = Истина) Экспорт
	...
	НапоминанияОСообщенияхМессенджеров = СтруктураНовыеНапоминания.Свойство("НапоминанияОСообщенияхМессенджеров");
	...
	Если (НапоминанияОВходящихПисьмах Или НапоминанияОСообщенияхМессенджеров)
		И ПрочиеНапоминания Тогда
		
		ТаблицаНапоминаний.Очистить();
		
	Иначе		
		
		ВидОповещенияОповещатьОНовыхВходящихПисьмах =
			ПредопределенноеЗначение("Справочник.CRM_ВидыОповещений.ОповещатьОНовыхВходящихПисьмах");
		ВидОповещенияОповещатьОСообщенияхМессенджеров =
		...
		Пока ТаблицаНапоминанийИндексЭлемента < ТаблицаНапоминаний.Количество() Цикл
			...
			ТекущееОповещениеОВходящихПисьмах =
				ТекущаяСтрокаНапоминаний.ВидОповещения = ВидОповещенияОповещатьОНовыхВходящихПисьмах;
			ТекущееОповещениеОСообщенияхМессенджеров =
				ТекущаяСтрокаНапоминаний.ВидОповещения = ВидОповещенияОповещатьОСообщенияхМессенджеров;
			
			Если (НапоминанияОВходящихПисьмах И ТекущееОповещениеОВходящихПисьмах)
				Или (НапоминанияОСообщенияхМессенджеров И ТекущееОповещениеОСообщенияхМессенджеров)
				Или (ПрочиеНапоминания И Не ТекущееОповещениеОВходящихПисьмах И Не ТекущееОповещениеОСообщенияхМессенджеров) Тогда
	...
	Если ИмяСобытия = "ОбновитьНапоминания" Тогда
		...
		Если ТипЗнч(Параметр) = Тип("Структура") Тогда
			...
			Если Параметр.Свойство("ОбновитьНапоминанияОВходящихПисьмах") И Параметр.ОбновитьНапоминанияОВходящихПисьмах Тогда
				...
				МассивНапоминанийОВходящихПисьмах = CRM_НапоминанияСервер.ПолучитьНапоминания("НапоминанияОВходящихПисьмах");
			...
			ИначеЕсли Параметр.Свойство("ОбновитьНапоминанияОСообщенияхМессенджеров") 
					И Параметр.ОбновитьНапоминанияОСообщенияхМессенджеров Тогда
				
				СтруктураНовыхНапоминаний.Вставить("НапоминанияОСообщенияхМессенджеров"); 
				
				МассивНапоминанийОСообщенияхМессенджеров = 
					CRM_НапоминанияСервер.ПолучитьНапоминания("НапоминанияОСообщенияхМессенджеров");
				
				Если (Не МассивНапоминанийОСообщенияхМессенджеров = Неопределено) Тогда
					
					Для каждого НапоминаниеОСообщенияхМессенджеров Из МассивНапоминанийОСообщенияхМессенджеров Цикл
					
						МассивНапоминаний.Добавить(НапоминаниеОСообщенияхМессенджеров);
...
Функция КнопкаОтложитьНажатиеНаСервере(ВыделеннаяСтрока, ПараметрСмещение)
	...
	Если РегистрСведенийМенеджерЗаписи.Выбран() Тогда
		...
		СтруктураНапоминания.Вставить("ЭтоНапоминаниеОВходящихПисьмах",
			 РегистрСведенийМенеджерЗаписи.ВидОповещения = Справочники.CRM_ВидыОповещений.ОповещатьОНовыхВходящихПисьмах);
			 
		СтруктураНапоминания.Вставить("ЭтоНапоминаниеОСообщенияхМессенджеров",
			 РегистрСведенийМенеджерЗаписи.ВидОповещения = Справочники.CRM_ВидыОповещений.ОповещатьОНовыхСообщенияхМессенджеров);
	...
	ИначеЕсли ТипЗнч(ВыделеннаяСтрока.Предмет) = Тип("СправочникСсылка.УчетныеЗаписиЭлектроннойПочты") Тогда
		...
		МассивКлючейНапоминаний = CRM_НапоминанияСервер.ПолучитьКлючиНапоминанийОВходящихЭлектронныхПисьмахУчетнойЗаписиПользователя(ВыделеннаяСтрока.Предмет,
			 ВыделеннаяСтрока.Пользователь);
		...
		Для каждого КлючНапоминанияУчетнойЗаписи Из МассивКлючейНапоминаний Цикл
			...
			Если РегистрСведенийМенеджерЗаписи.Выбран() Тогда
				...
				Если РегистрСведенийМенеджерЗаписи.ВидОповещения = Справочники.CRM_ВидыОповещений.ОповещатьОНовыхВходящихПисьмах 
					Или РегистрСведенийМенеджерЗаписи.ВидОповещения = Справочники.CRM_ВидыОповещений.ОповещатьОНовыхСообщенияхМессенджеров Тогда
							
					РегистрСведенийМенеджерЗаписи.ДатаОповещения = НоваяДатаНапоминания 
						+ ЗначениеОтложенногоВремени 
						+ ПараметрСмещение;
					РегистрСведенийМенеджерЗаписи.Счетчик = РегистрСведенийМенеджерЗаписи.Счетчик + 1;
					РегистрСведенийМенеджерЗаписи.Записать(Истина);
	...
	ИначеЕсли ТипЗнч(ВыделеннаяСтрока.Предмет) = Тип("СправочникСсылка.CRM_УчетныеЗаписиМессенджеров") Тогда
		
		МассивКлючейНапоминаний = 
			CRM_НапоминанияСервер.ПолучитьКлючиНапоминанийОСообщенияхМессенджеровУчетнойЗаписиПользователя(
			ВыделеннаяСтрока.Предмет, ВыделеннаяСтрока.Пользователь);
		...
		СтруктураНапоминания.Вставить("ЭтоНапоминаниеОСообщенияхМессенджеров", Истина);
...
Функция КнопкаПрекратитьНажатиеНаСервере(ВыделеннаяСтрока)
	...
	Если РегистрСведенийМенеджерЗаписи.Выбран() Тогда
		...
		Если РегистрСведенийМенеджерЗаписи.ВидОповещения = Справочники.CRM_ВидыОповещений.ОповещенияОДняхРождения Тогда
			...
			ДатаНачала = CRM_ОбщегоНазначенияСервер.ПолучитьДатуПоКалендарю(НачалоДня(ДобавитьМесяц(РегистрСведенийМенеджерЗаписи.ДатаНачала,
		...
		Иначе
			РегистрСведенийМенеджерЗаписи.Просмотрено = Истина;
			РегистрСведенийМенеджерЗаписи.Записать();
		КонецЕсли;
	ИначеЕсли ТипЗнч(ВыделеннаяСтрока.Предмет) = Тип("СправочникСсылка.УчетныеЗаписиЭлектроннойПочты") Тогда
	...
	ИначеЕсли ТипЗнч(ВыделеннаяСтрока.Предмет) = Тип("СправочникСсылка.CRM_УчетныеЗаписиМессенджеров") Тогда
		...
		Для каждого КлючНапоминанияУчетнойЗаписи Из МассивКлючейНапоминаний Цикл
			...
			Если РегистрСведенийМенеджерЗаписи.Выбран() Тогда
				...
				Если РегистрСведенийМенеджерЗаписи.ВидОповещения = Справочники.CRM_ВидыОповещений.ОповещатьОНовыхСообщенияхМессенджеров Тогда
...
Процедура ПолучитьФормуОткрытьПредмет(ТекущийПредмет)
	...
	Если ТипЗнч(ТекущийПредмет) = Тип("ДокументСсылка.ЭлектронноеПисьмоВходящее") Тогда
		ОткрытьФорму("Документ.ЭлектронноеПисьмоВходящее.Форма.CRM_ФормаДокумента",
			 Новый Структура("Ключ", ТекущийПредмет),
			 ЭтотОбъект);
...
Процедура КнопкаПрекратитьВсеНажатие(Команда)
	...
	БылиИзмененыОповещенияОСообщенияхМессенджеров = Ложь;
	...
	Для Каждого Строка Из ТаблицаНапоминаний Цикл
		...
		Если НЕ СтруктураНапоминания.Выбран Тогда
			ПоказатьПредупреждение(,
				 НСтр("ru='Напоминание удалено или модифицировано!';
				|en='The reminder is deleted or modified!'"));
			Продолжить;
		КонецЕсли;
		Если СтруктураНапоминания.Свойство("ЭтоНапоминаниеОВходящихПисьмах")
			И СтруктураНапоминания.ЭтоНапоминаниеОВходящихПисьмах Тогда
			БылиИзмененыОповещенияОВходящихПисьмах = Истина;
		ИначеЕсли СтруктураНапоминания.Свойство("ЭтоНапоминаниеОСообщенияхМессенджеров")
			И СтруктураНапоминания.ЭтоНапоминаниеОСообщенияхМессенджеров Тогда
			БылиИзмененыОповещенияОСообщенияхМессенджеров = Истина;
		КонецЕсли; 
	КонецЦикла;
	Оповестить("ОбновитьНапоминания",
		Новый Структура("ОбновитьНапоминанияОВходящихПисьмах, ОбновитьНапоминанияОСообщенияхМессенджеров",
		БылиИзмененыОповещенияОВходящихПисьмах, БылиИзмененыОповещенияОСообщенияхМессенджеров));
...
Процедура КнопкаРедактироватьНажатие(Команда)
	...
	Для Каждого ВыделеннаяСтрока Из Элементы.Напоминания.ВыделенныеСтроки Цикл
		...
		Если ТипЗнч(ВыделеннаяСтрока.Предмет) = Тип("СправочникСсылка.УчетныеЗаписиЭлектроннойПочты") Тогда
			...
			ПерейтиКЧерновикам = (СтрНайти(ВыделеннаяСтрока.Содержание, "Письма перенесены в ""Черновики""") > 0);
		Иначе
...
Процедура ПредметПредставлениеНажатие(Элемент, СтандартнаяОбработка)
	...
	Если ТипЗнч(Предмет) = Тип("СправочникСсылка.УчетныеЗаписиЭлектроннойПочты") Тогда
		...
		ПерейтиКЧерновикам = (СтрНайти(Элементы.Напоминания.ТекущиеДанные.Содержание,
			 "Письма перенесены в ""Черновики""") > 0);
...
Процедура ОткрытьМенеджерПочты(УчетнаяЗапись, ПерейтиКЧерновикам)
	...
	Если ФормаПочтовогоМенеджера = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ВидПапки = ?(ПерейтиКЧерновикам, "Черновики", "Входящие");
	
	Если ФормаПочтовогоМенеджера.Открыта() Тогда
		ФормаПочтовогоМенеджера.УстановитьПапкуУчетнойЗаписи(УчетнаяЗапись, ВидПапки);
