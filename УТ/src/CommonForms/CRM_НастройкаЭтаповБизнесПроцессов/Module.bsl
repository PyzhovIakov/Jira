...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		Заголовок = НСтр("ru='Настройка этапов';en='Stages Settings'") + " " + НСтр("ru='для'") + " " + 
		СокрЛП(Строка(Процесс)) + НСтр("ru=' (Карта маршрута)';en=' (Flow Chart)'");
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если ИмяСобытия = "ЗаписанТриггер" И Параметр = ТочкаМаршрута Тогда
		CRM_ОбщегоНазначенияКлиентСервер.ОбновлениеТаблицыФормы(Элементы.Триггеры);
	КонецЕсли;
КонецПроцедуры
...
Процедура ТаблицаУсловийЦельНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ТекДанные.ТипДействия = ПредопределенноеЗначение("Перечисление.bpmТипыДействий.ВводДокумента") Тогда
		СтандартнаяОбработка = Ложь;
...
Процедура ПерейтиКСпискуЭтапов(Команда)
	...
	МаршрутПослеОжиданияПриАктивацииСтроки();
...
Процедура МаршрутПослеОжиданияПриАктивацииСтроки()
	...
	Если Элементы.ГруппаСтраниц.ТекущаяСтраница = Элементы.СтраницаТаблицаМаршрута Тогда
	...
	CRM_ОбщегоНазначенияКлиентСервер.ОбновлениеТаблицыФормы(Элементы.табИсполнители_);
	...
	CRM_ОбщегоНазначенияКлиентСервер.ОбновлениеТаблицыФормы(Элементы.ТаблицаУсловий);
...
&НаКлиенте
Процедура СхемаПриАктивизацииЭлемента()
	
	Элемент = Элементы.СхемаМаршрута;
...
Функция ПроверитьНаРоль(Исполнитель)
	...
	Возврат НЕ (ТипЗнч(Исполнитель)=Тип("СправочникСсылка.РолиИсполнителей") И Исполнитель <> Справочники.РолиИсполнителей.CRM_ДинамическаяАдресация);
...
Процедура УстановитьДоступностьРеквизитовЭтапа()
	...
	Элементы.ТочкаМаршрутаЗадание.ТолькоПросмотр	= Ложь;
	Элементы.ТочкаМаршрутаЗадание3.ТолькоПросмотр		= Ложь;
	Элементы.СписокВариантов1.ТолькоПросмотр			= Ложь;
	Элементы.СписокВариантов3.ТолькоПросмотр			= Ложь;
	...
	Если ТочкаМаршрута_ЭтоГрупповаяТочка Тогда
		Элементы.ГруппаСтраниц_1.ТекущаяСтраница = Элементы.ИсполнительГрупповойТочки;
		Элементы.табИсполнители_.ВысотаВСтрокахТаблицы = 4;
	Иначе
		Элементы.ГруппаСтраниц_1.ТекущаяСтраница = Элементы.ИсполнительНеГрупповойТочки;
		Элементы.табИсполнители_.ВысотаВСтрокахТаблицы = 1; // чтобы высота всей группы стала меньше
	КонецЕсли;
	...
	Если Модифицированность Тогда
		...
		Если НЕ ЕстьОшибки Тогда
			...
			//Если ТипЗнч(Процесс) = Тип("СправочникСсылка.Проекты") Тогда
				Оповестить("ЗавершенаНастройкаМаршрута", Процесс, ВладелецФормы);
			//КонецЕсли;
...
Функция ПроверитьПроцессИТочку()
	...
	Если ТипПроцесса = ПредопределенноеЗначение("Перечисление.bpmТипыПроцессов.ПроцессОбъекта")
		ИЛИ (ТипПроцесса = ПредопределенноеЗначение("Перечисление.bpmТипыПроцессов.НезависимыйПроцесс")
		И Процесс.ИспользоватьПриПринятииОбращения) Тогда
		
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("Ссылка", ТочкаМаршрута);
		Запрос.УстановитьПараметр("НомерВерсии", Процесс.НомерВерсии);
		Запрос.Текст = "ВЫБРАТЬ
...
&НаКлиенте
Процедура ТочкаМаршрутаЗадание1НачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	Оповещение = Новый ОписаниеОповещения("ТочкаМаршрутаЗаданиеНовый", ЭтотОбъект);
	ОбщегоНазначенияКлиент.ПоказатьФормуРедактированияМногострочногоТекста(Оповещение,ТочкаМаршрута_Задание, Нстр("ru = 'Изменить задания'"));
	
КонецПроцедуры

&НаКлиенте
Процедура ТочкаМаршрутаЗаданиеНовый(Параметр1, Параметр2) Экспорт
	
	Если Параметр1 <> Неопределено Тогда
		ТочкаМаршрута_Задание = Параметр1;
		Модифицированность = Истина;
		ТекущиеДанные = Элементы.табМаршрут_.ТекущиеДанные;	
		ТекущиеДанные.Задание = Параметр1;
	КонецЕсли
