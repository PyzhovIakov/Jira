...
Процедура Поиск(Команда)
	Если ПустаяСтрока(ТекущийСправочник) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Нстр("ru='Не выбран источник поиска!';
			|en='No search source selected!'"), ,
			 "ТекущийСправочник");
		Возврат;
	КонецЕсли;
	Если ПустаяСтрока(ПолеПоиска) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Нстр("ru='Не заполнено значение поиска!';
			|en='The search value is not filled!'"), ,
			 "ПолеПоиска");
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьСостоянияЛиний(МассивЛиний)
	Для Каждого ЭлементМассива Из МассивЛиний Цикл
		Если ПустаяСтрока(ЭлементМассива.Number) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ВыполнитьПоискНаСервере()
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Нстр("ru='Не выбран режим поиска!';
			|en='No search mode selected!'"), ,
			 "ПереключательВыборПоляПоиска");
		Возврат;
	КонецЕсли;
	Если ПереключательВыборПоляПоиска = "ПоТелефону" Тогда
		Выборка = Запрос.Выполнить().Выбрать();
