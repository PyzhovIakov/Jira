...
&НаСервере
Процедура ДобавитьСтрокуВДеревоДокументов(СтрокиДереваДокументов, СодержаниеСтроки,
	 ЦветТекстаСтроки = Неопределено, Текст,
	 Содержание)
...
Процедура ДобавитьОписаниеЗадача(СтрокаДерева, ТекущаяЗадача)
	...
	СтрокаРезультат = "РЕЗУЛЬТАТ: " + ?(Не ПустаяСтрока(ТекущаяЗадача.РезультатВыполнения),
		 ТекущаяЗадача.РезультатВыполнения, "");
	Текст		= "Результат: ";
...
Процедура ДобавитьОписаниеЭлектронноеПисьмоВходящее(СтрокаДерева, ЭлектронноеПисьмо)
	...
	Текст		= "Содержание: ";
	//Содержание	= ?(Не ПустаяСтрока(ЭлектронноеПисьмо.ТекстHTML),
	//	УправлениеЭлектроннойПочтой.ПолучитьПростойТекстИзHTML(ЭлектронноеПисьмо.ТекстHTML),
	//	"");
...
Процедура ЗаполнитьДерево(СтрокаДерева, Основание)
	...
	ИначеЕсли ТипЗнч(Основание) = ТИП("ЗадачаСсылка.ЗадачаИсполнителя")
		 И НЕ Основание.CRM_ТочкаМаршрута.Вид = Перечисления.CRM_ВидыТочекМаршрута.ВложенныйБизнесПроцесс Тогда
	...
	Для Каждого ЭлементМассива Из МассивДокументов Цикл
		...
		Если ТипЗнч(ЭлементМассива) = ТИП("ЗадачаСсылка.ЗадачаИсполнителя")  Тогда
			...
			Если ЭлементМассива.CRM_ТочкаМаршрута.Вид = Перечисления.CRM_ВидыТочекМаршрута.ВложенныйБизнесПроцесс Тогда
				ВложенныйБП = CRM_БизнесПроцессыСервер.ПолучитьПодчиненныйБизнесПроцесс(ЭлементМассива, Истина);
				Если ВложенныйБП <> Неопределено Тогда
					ЭлементМассива  =  ВложенныйБП;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
			...
			Если СтрокаДереваЗначений.СсылкаНаОбъект <> Неопределено Тогда
				Если ТипЗнч(СтрокаДереваЗначений.СсылкаНаОбъект) = Тип("БизнесПроцессСсылка.CRM_БизнесПроцесс") Тогда
					Если СтрокаДереваЗначений.СсылкаНаОбъект.Стартован Тогда
						Если НЕ СтрокаДереваЗначений.СсылкаНаОбъект.ПометкаУдаления
							 И НЕ СтрокаДереваЗначений.СсылкаНаОбъект.Завершен Тогда
							ОбластьМакета = Макет.ПолучитьОбласть("ПредставлениеБП|Картинака5");
						ИначеЕсли СтрокаДереваЗначений.СсылкаНаОбъект.ПометкаУдаления
							 И НЕ СтрокаДереваЗначений.СсылкаНаОбъект.Завершен Тогда
							ОбластьМакета = Макет.ПолучитьОбласть("ПредставлениеБП|Картинака6");
						ИначеЕсли НЕ СтрокаДереваЗначений.СсылкаНаОбъект.ПометкаУдаления
							 И СтрокаДереваЗначений.СсылкаНаОбъект.Завершен Тогда
