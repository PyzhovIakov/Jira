...
Процедура ПредставлениеПоляОтбораНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтруктураПараметры = Новый Структура("Режим, КомпоновщикНастроек, НеДляОтчетов", "ВыборПоляОтбора",
		 КомпоновщикНастроек,
		 НеДляОтчетов);
	...
	Форма = ПолучитьФорму("ОбщаяФорма.CRM_ЗагрузкаРезультатовОтчетовВОтбор", СтруктураПараметры,
		 ЭтотОбъект,
...
Функция ПроверитьНаКорректность()
	...
	Если Не ЗначениеЗаполнено(ПолеОтбора) Тогда
		СообщитьПользователю(НСтр("ru='Поле отбора должно быть заполнено!';
			|en='Field of filter should be fill!'"),
			 "ПредставлениеПоляОтбора");
		Возврат Ложь;
	КонецЕсли;
	...
	Если Типы.Количество() > 0 Тогда
		Попытка
			ДоступноеПолеОтбора =
				КомпоновщикНастроек.Настройки.ДоступныеПоляОтбора.Элементы.Найти(Новый ПолеКомпоновкиДанных(ПолеОтбора));
		Исключение
		...
		Если ДоступноеПолеОтбора <> Неопределено Тогда
			...
			Если Не бСодержитТип Тогда
				СообщитьПользователю(НСтр("ru='Тип загружаемых значений не соответствует типу выбранного поля!';
					|en='The type of load values did not correspond to type of the select field!'"),
					 "ПредставлениеПоляОтбора");
				Возврат Ложь;
			КонецЕсли;
		КонецЕсли;
