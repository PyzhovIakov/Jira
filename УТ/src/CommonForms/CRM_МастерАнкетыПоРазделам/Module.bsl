...
Процедура ПриОткрытии(Отказ)
	...
	// +CRM
	Элементы.СкрытьРазделы.Заголовок = ?(Элементы.ГруппаДеревоРазделов.Видимость,
		"Скрыть разделы", "Показать разделы");
	// -CRM
...
Процедура ИзменитьВидимостьДеревоРазделов()
	...
	Элементы.СкрытьРазделы.Заголовок = ?(Элементы.ГруппаДеревоРазделов.Видимость,
		 НСтр("ru = 'Скрыть разделы'"), НСтр("ru = 'Показать разделы'"));
	...
	Для каждого ЭлементКоллекции Из ПодчиненныеВопросы Цикл
		...
		Для каждого ПодчиненныйВопрос Из ЭлементКоллекции.Подчиненные Цикл
			...
			Если СтрЧислоВхождений(ПодчиненныйВопрос.ИмяЭлементаПодчиненногоВопроса, "Реквизит") = 0 Тогда
				...
				// +CRM
				//Попытка
				Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементПодчиненногоВопроса,
					"АвтоОтметкаНезаполненного") Тогда
				// -CRM
