...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Если Параметры.Свойство("СтруктураПоиска") Тогда
		Если НайтиДубли(Параметры.СтруктураОбъекта, Параметры.СтруктураПоиска, Параметры.СписокКонтактов,
			 Параметры.Контакт,
			 Параметры.Клиент) Тогда
			Для Каждого СтрокаНайденого Из _ТаблицаНайдено Цикл
				...
				Если НайдСтрока.Количество() = 0 Тогда
					...
					Строка.Доступен			= (СтрНайти(Строка(СтрокаНайденого.Объект), "<Объект не найден>") = 0);
				КонецЕсли;
			КонецЦикла;
		Иначе
			Отказ = Истина;
	...
	Если НЕ РольДоступна("ПолныеПрава") И Константы.CRM_ЗапретитьВводНовогоКонтактаПриНайденныхСовпадениях.Получить() Тогда
		...
		Если Команда <> Неопределено Тогда
			Команда.Подсказка =
				НСтр("ru = 'Необходимо обратиться к администратору базы для предоставления прав доступа к имеющемуся контакту.'");
		КонецЕсли;
	КонецЕсли;
