...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// +CRM
	CRM_УправлениеПечатью.ПриСозданииФормыРедактированияТабличногоДокумента(ЭтотОбъект);
	// -CRM
КонецПроцедуры
...
Функция ЗаписатьМакет(ПрерватьЗаписьПриНаличииОшибокВМакете = Ложь, АдресМакетаВоВременномХранилище = "")
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Печать") Тогда
		...
		// +CRM
		ОписаниеМакета.CRM_ИмяМакета = ЭтотОбъект["CRM_ИмяМакета"];
		ОписаниеМакета.CRM_ПоставляемыйМакет = ЭтотОбъект["CRM_ПоставляемыйМакет"];
		ОписаниеМакета.Используется = ЭтотОбъект["CRM_МакетИспользуется"];
		// -CRM
...
Процедура Подключаемый_ВставитьТэг(Команда)
	...
	МассивСтрок = ЭтотОбъект["CRM_ТаблицаАвтотекста"].НайтиСтроки(Новый Структура("ИмяКоманды", ИмяКоманды));
	Тэг = МассивСтрок[0].Тэг;
	
	Элементы.ТабличныйДокумент.ТекущаяОбласть.Текст = Элементы.ТабличныйДокумент.ТекущаяОбласть.Текст + Тэг;
...
Процедура Подключаемый_СоздатьТег(Команда)
	...
	CRM_АвтотекстКлиент.СоздатьТегАвтотекста(ЭтотОбъект, Команда, ЭтотОбъект["CRM_ОбъектыАвтотекста"], , ОписаниеОповещения);
