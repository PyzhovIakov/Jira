...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбновитьЗначениеПараметраКомпоновкиДанных(СписокПользователей, "ПланируемаяДата", ПланируемаяДата);
	...
	ЭтоПолноправныйПользователь = Пользователи.ЭтоПолноправныйПользователь(ТекущийПользовательСеанса, , Ложь);
...
Процедура РежимПодбораПриИзменении(Элемент)
	...
	Иначе
		...
		Если НастройкаПоТипуУслуги Тогда
			...
			Элементы.НастройкаПоТипуУслуги.Заголовок = Нстр("ru='Использовать отбор подразделений по типу услуг ';
				|en='Use filter of department by type of services'") 
				+ Строка(ТипУслуги);
		КонецЕсли;					
	КонецЕсли;
...
Процедура ПользователиСписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ТекДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	Если НЕ ВключеныОграниченияДоступа Или ЭтоПолноправныйПользователь(ТекДанные.Ссылка) Тогда 
		ОповеститьОВыборе(ТекДанные.Ссылка);
	Иначе
		
		Если НЕ CRM_ОбщегоНазначенияСервер.ЕстьОбщийМодуль("CRM_УправлениеДоступомУровниДоступа") Тогда
			Возврат;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(Партнер)
			 И МодульУправлениеДоступомУровниДоступа.РежимДоступаКПартнерам(Партнер, ТекДанные.Ссылка, "И") < РежимДоступа Тогда
			Если ЭтоПолноправныйПользователь
				 ИЛИ МодульУправлениеДоступомУровниДоступа.РежимДоступаКПартнерам(Партнер, ТекущийПользовательСеанса, "И") = 1 Тогда
		...
		Если ЗначениеЗаполнено(КонтактноеЛицо)
			 И МодульУправлениеДоступомУровниДоступа.РежимДоступаККонтактам(КонтактноеЛицо,
			 	 ТекДанные.Ссылка) < РежимДоступа Тогда
			Если ЭтоПолноправныйПользователь
				 ИЛИ МодульУправлениеДоступомУровниДоступа.РежимДоступаККонтактам(КонтактноеЛицо,
				 	 ТекущийПользовательСеанса) = 1 Тогда
...
Процедура ПользователиСписокВыборЗавершение(Результат, ДопПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.ОК Тогда
		МодульУправлениеДоступомУровниДоступа = ОбщегоНазначенияКлиент.ОбщийМодуль("CRM_УправлениеДоступомУровниДоступа");
		Если МодульУправлениеДоступомУровниДоступа <> Неопределено Тогда
...
Процедура Подключаемый_ОбработчикОжиданияСписокПриАктивизацииСтроки()
	...
	Если ТекущиеДанные <> Неопределено Тогда
		ПользовательОтсутствует = ТекущиеДанные.Отсутствует;
	КонецЕсли;
...
Процедура СписокПользователейУстановитьОтбор()
	...
	Если Не НастройкаПоТипуУслуги Тогда
		...
		// Рассматриваем 2 варианта: софтфон доступен и недоступен
		
		Если Не сфпИспользоватьСофтфон Тогда
			// 1 Вариант (недоступен):
			...
			Если ТекущееПодразделение.Количество() > 0 И Не (ТекущееПодразделение = Неопределено)
				 И Не ЗначениеЗаполнено(ТекущееПодразделение[0]) Тогда
				// Это корень дерева, значит в списке пользователей должен виден весь их список.
				ЭлементОтбора =
					СписокПользователей.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[0].Элементы[0].Элементы[0];
				...
				ЭлементОтбора2 =
					СписокПользователей.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[0].Элементы[0].Элементы[1];
				ЭлементОтбора2.Использование = Истина;
			ИначеЕсли ТекущееПодразделение.Количество() > 0 Тогда
				ЭлементОтбора =
					СписокПользователей.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[0].Элементы[0].Элементы[0];
				ЭлементОтбора.ПравоеЗначение = ТекущееПодразделение;
				ЭлементОтбора.ВидСравнения = ?(ВыбиратьИерархически,
		...
		Иначе
			...
			Если ТекущееПодразделение.Количество() > 0 И Не (ТекущееПодразделение = Неопределено)
				...
				МассивПользователей = ПолучитьПользователейПодразделения(ПредопределенноеЗначение("Справочник.СтруктураПредприятия.ПустаяСсылка"),
					 Истина);
...
Процедура СписокПользователейУстановитьОтборНаСервере(Подразделение)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Подразделение.ТекущийРуководитель) Тогда
			...
			Иначе
				РуководительПодразделения = Справочники.Пользователи.НайтиПоРеквизиту("ФизическоеЛицо",
					Подразделение.ТекущийРуководитель);
			КонецЕсли;
...
Процедура ФильтроватьДеревоПоТипуУслуги(ЭлементыДерева)
	Для Каждого ЭлементДерева Из ЭлементыДерева Цикл
		...
		Попытка
			ТекЭлементРодитель = ЭлементДерева.ПолучитьРодителя();
		...
		Пока ТекЭлементРодитель <> Неопределено Цикл
			...
			Попытка
				ТекЭлементРодитель = ТекЭлементРодитель.ПолучитьРодителя();
...
Процедура ЗаполнитьМассивПользователейПоТипуУслуги()
	...
	Запрос = Новый Запрос;
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
	|	СтруктураПредприятияCRM_ТипУслуги.Ссылка КАК Ссылка
	|ИЗ
