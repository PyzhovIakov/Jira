...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ИдентификаторЗвонка") Тогда
		...
		Если ЗначениеЗаполнено(Параметры.Контакт)
			 И ТипЗнч(Параметры.Контакт) = Тип("СправочникСсылка.КонтактныеЛицаПартнеров") Тогда
			ТекущийЗвонокВладелецКонтакта = Параметры.Контакт.Владелец;
		КонецЕсли;	
		ТекущийЗвонокНомерАбонента = Параметры.НомерТелефона;
		ТекущийЗвонок = Параметры.НовыйЗвонок;
	...
	Если ЗначениеЗаполнено(ТекущийЗвонокКонтакт) Тогда
		ДанныеКонтакта = сфпСофтФонПроСервер.ПолучитьДанныеПредставленияКонтакта(ТекущийЗвонокКонтакт,
	...
	КонецЕсли;
	Заголовок = "" + ТекущийПользователь + " (" + ЗаголовокСтатуса + ")";
...
Процедура ПриОткрытии(Отказ)
	...
	ТекущийЗвонокПредставлениеКонтакта = ?(ЗначениеЗаполнено(ТекущийЗвонокКонтакт),
		 ТекущийЗвонокКонтакт,
		 НСтр("ru='Неизвестный абонент';en='Unknown subscriber'"));
	
	УстановитьОтборыТелефонныхЗвонков();
	сфпСофтФонПроКлиент.УстановитьДоступностьКомандПанелиЗвонка(ЭтотОбъект);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "сфпКонецРазговора" Тогда
		Если Параметр.ИдентификаторЗвонка = ИдентификаторЗвонка ИЛИ ПустаяСтрока(ИдентификаторЗвонка) Тогда
	...
	сфпСофтФонПроКлиент.ОбработкаОповещенияПанелиЗвонка(ЭтотОбъект, ИмяСобытия, Параметр, Источник,
		 ИспользуетсяОблачнаяАТС);
...
Процедура АбонентыТелефонныхКнигПриАктивизации()
	...
	Если НЕ ЗначениеЗаполнено(ИдентификаторЗвонка) Тогда
		...
		сфпСофтФонПроКлиент.УстановитьДоступностьКомандПанелиЗвонка(ЭтотОбъект, ТекДанные <> Неопределено);
			...
			Если ИспользуетсяОблачнаяАТС Тогда
				Если ТекущийЗвонокДлительность = '00010101' Тогда
					РезультатЗапроса = сфпЛицензированиеЭкспортныеМетоды.ВыполнитьЗапросОблачнойАТС("ПереводНеотвеченногоЗвонка",
						 ИдентификаторЗвонка, СтруктураЗвонка.НомерТелефона, ВнутреннийНомер,
						 Истина);
...
Процедура ПриВыбореНомера(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныйЭлемент <> Неопределено Тогда
		...
		Если сфпСофтФонПроКлиент.сфпПозвонить(ВыбранныйЭлемент.Значение,
			 ДополнительныеПараметры.АбонентКонтакт,
			 ДополнительныеПараметры.ДанныеЗаполнения) Тогда
			//ОбработкаОповещенияПанелиЗвонка(ЭтотОбъект, "сфпНовыйИсходящийЗвонок", ДополнительныеПараметры, ЭтотОбъект);
...
Процедура НомеронабирательНажатие(Команда)
	...
	ИмяЭлемента = СтрЗаменить(ЭтотОбъект.ТекущийЭлемент.Имя, "Номеронабиратель", "");
...
Процедура УстановитьОтображениеГруппыЕще(Группа)
	...
	Элементы.ПоказатьНомеронабиратель.ЦветФона = ?(Группа = "Номеронабиратель",
		 ЦветФонаВыбраннойГиперссылки,
		 ЦветФонаГиперссылки);
	Элементы.ПоказатьНомеронабиратель.Картинка = ?(Группа = "Номеронабиратель",
	...
	Если ВыбранныйЭлемент <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ИдентификаторЗвонка) Тогда
			СтруктураЗвонка = Новый Структура("ИдентификаторЗвонка,Контакт,НомерТелефона,НовыйЗвонок",
				ИдентификаторЗвонка, ТекущийЗвонокКонтакт, ТекущийЗвонокНомерАбонента, ТекущийЗвонок);
