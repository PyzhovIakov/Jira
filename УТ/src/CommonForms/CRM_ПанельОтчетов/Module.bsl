...
Процедура Подключаемый_ЗаголовокРазделаНажатие(Элемент)
	...
	Если ПараметрыКлиента.ВыполнятьЗамеры Тогда
		// +CRM_Модуль
		Замер = НачатьЗамер("ПанельОтчетов.Открытие", ПутьКПодсистеме);
		// -CRM_Модуль
	КонецЕсли;
...
Функция НачатьЗамер(Событие, Комментарий = Неопределено)
	...
	Если Событие = "СписокОтчетов.Открытие" Или Событие = "ПанельОтчетов.Открытие" Тогда
		Замер.Имя = Событие;
		Комментарий = ?(ЗначениеЗаполнено(Комментарий), Комментарий + "; ", "") + НСтр("ru = 'Из панели отчетов:'") + " " + ПараметрыКлиента.ПутьКПодсистеме;
	Иначе
		...
		КонецЕсли;
		Комментарий = ?(ЗначениеЗаполнено(Комментарий), Комментарий + "; ", "") + ПараметрыКлиента.ПутьКПодсистеме;
		Комментарий = ?(ЗначениеЗаполнено(Комментарий), Комментарий + "; ", "") + НСтр("ru = 'Подсказки:'") + " " + Строка(ПоказыватьПодсказки);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
		Комментарий = ?(ЗначениеЗаполнено(Комментарий), Комментарий + "; ", "")
			+ НСтр("ru = 'Поиск:'") + " " + Строка(СтрокаПоиска)
			+ НСтр("ru = 'Во всех разделах:'") + " " + Строка(ИскатьВоВсехРазделах);
	Иначе
		Комментарий = ?(ЗначениеЗаполнено(Комментарий), Комментарий + "; ", "") + НСтр("ru = 'Без поиска'");
	КонецЕсли;
	
	Если Событие = "ВыходИзРежимаНастройки" Тогда
		Комментарий = ?(ЗначениеЗаполнено(Комментарий), Комментарий + "; ", "") + НСтр("ru = 'Выход из режима настройки'");
	КонецЕсли;
	// -CRM_Модуль
...
Функция ОбновитьПанельОтчетовНаСервере(Знач Событие = "")
	...
	Элементы.БыстрыйДоступШапкаНадпись.ОтображениеПодсказки = ?(РежимНастройки,
...
Функция ЗаполнитьПанельОтчетовВФоне()
	...
	НазначениеВариантаОтчета = ?(ОбщегоНазначения.ЭтоМобильныйКлиент(),
	...
	НазначенияВариантовОтчетов.Добавить(НазначениеВариантаОтчета);
	// -CRM Рефакторинг кода
...
Процедура ИнициализироватьПараметрыЗаполнения(ПараметрыЗаполнения)
	...
	Если РежимНастройки Тогда
		ПараметрыЗаполнения.Вставить("КонтекстноеМеню", Новый Структура("УбратьИзБыстрогоДоступа,
			| ПереместитьВБыстрыйДоступ, Изменить"));
...
Процедура ОпределитьГруппыИДекорацииДляВыводаВариантов(ПараметрыЗаполнения)
	...
	Если ПараметрыЗаполнения.ИспользоватьПодсветку Тогда
		ПараметрыПодсветки =
			ПараметрыЗаполнения.РезультатПоиска.ПодсветкаПодсистем.Получить(ПараметрыЗаполнения.РазделСсылка);
...
Процедура ВывестиВариантыСГруппой(ПараметрыЗаполнения)
	...
	Для Каждого ВариантРодитель Из НайденныеРодители Цикл
		НайденныеПодчиненные = Варианты.НайтиСтроки(Новый Структура("Родитель, Подсистема",
			ВариантРодитель.Ссылка, ВариантРодитель.Подсистема));
...
Процедура ЗаполнитьПорядокВывода(ПорядокВывода, СтрокаРодителя, СтрокаДерева, Рекурсия, ПараметрыЗаполнения)
	...
	Если (СтрокаДерева.КоличествоВариантов > 0 Или СтрокаДерева.ВсегоВложенныхВариантов > 0)
		 И Рекурсия.НачатВыводВТекущейКолонке
		 И СтрокаРодителя.НачатВывод Тогда
		// Вывод пустой строки.
	...
	Если СтрокаДерева.КоличествоВариантов > 0 Тогда
		...
		Для Каждого Вариант Из СтрокаДерева.Варианты Цикл
			...
			Если ВозможноПродолжение
				...
				Иначе
					КоличествоВТекущуюКолонку = Макс(Мин(Рекурсия.СвободноСтрок,
						 СтрокаДерева.КоличествоВариантов - ВариантовВыведено), 3);
				КонецЕсли;
				ВариантовВыведено = 0;
...
Функция ДобавитьГруппуПодсистемы(ПараметрыЗаполнения, СтрокаПорядкаВывода, ВГруппу)
	...
	Если ТребуетсяПодсветка Тогда
		...
		ЗаголовокПодсистемы = Элементы.Вставить(ГруппаПодсистемы.Имя + "_РасширеннаяПодсказка",
			 Тип("ДекорацияФормы"), ГруппаПодсистемы);
	...
	Для Каждого ЭлементСписка Из ОбластьПоиска.ПодсветкаСлов Цикл
		...
		Если Подсвечивать И КоличествоОткрыто = 1 Тогда
			...
			ФрагментОбычногоТекста = Сред(ИсходныйТекст, ПозицияНачалаОбычногоТекста,
				 ПозицияНачалаПодсветки - ПозицияНачалаОбычногоТекста);
		...
		ИначеЕсли Не Подсвечивать И КоличествоОткрыто = 0 Тогда
			...
			ПодсвечиваемыйФрагмент = Сред(ИсходныйТекст, ПозицияНачалаПодсветки,
				 ПозицияНачалаОбычногоТекста - ПозицияНачалаПодсветки);
