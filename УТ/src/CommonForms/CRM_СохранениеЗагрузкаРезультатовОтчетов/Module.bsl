...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ CRM_ЛицензированиеЭкспортныеМетоды.ПодсистемаCRMИспользуется() Тогда
		CRM_ЛицензированиеЭкспортныеМетоды.ВывестиУведомлениеОНедоступности(НСтр("ru = 'или сохранить результат отчета'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Иначе
		...
		ПараметрыСохранения = ЗащищеннаяОбработка.
			ОбщаяФорма_CRM_ОбщаяФормаОтчетов_СформироватьРезультатДляСохраненияСервер(ЭтотОбъект);
...
Функция СформироватьНаименование(Форма)
	...
	Если ЗначениеЗаполнено(Форма.НазваниеВарианта) Тогда
		СтрокаНаименование = СтрокаНаименование + ?(ЗначениеЗаполнено(СтрокаНаименование), " ", "") + "(" 
			+ Форма.НазваниеВарианта 
			+ ")";
	КонецЕсли;
...
Процедура ЗаполнитьСписокРезультат()
	CRM_ЛицензированиеЭкспортныеМетоды.ПолучитьЗащищеннуюОбработку().
		ОбщаяФорма_CRM_ЗагрузкаРезультатовОтчетовВОтбор_ЗаполнитьСписокРезультат(ЭтотОбъект);
КонецПроцедуры
