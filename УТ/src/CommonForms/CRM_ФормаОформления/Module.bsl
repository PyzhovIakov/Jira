...
Процедура СохранитьВБыстрыхНастройках(Команда)
	...
	КонецЕсли; 
	ИмяНастройки = "";
	Оповещение = Новый ОписаниеОповещения("СохранитьВБыстрыхНастройкахЗавершение", ЭтотОбъект,
		 Новый Структура("ИмяНастройки",
		 ИмяНастройки));
...
Процедура СохранитьВБыстрыхНастройкахЗавершение(Результат, ДополнительныеПараметры) Экспорт
    ...
    Если НЕ (Результат <> Неопределено) Тогда
        Возврат;
    КонецЕсли;
    СохранитьВБыстрыхНастройкахНаСервере(ИмяНастройки,
    	 Элементы.КомпоновщикНастроекНастройкиУсловноеОформление.ТекущаяСтрока);
