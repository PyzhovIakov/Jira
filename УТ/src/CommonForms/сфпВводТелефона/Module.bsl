...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВXML(ЗначенияПолей) Тогда
			...
			КонтактнаяИнформация = CRM_УправлениеКонтактнойИнформацией.КонтактнаяИнформацияИзXML(ЗначенияПолей,
		...
		Иначе
			Если ТипКонтактнойИнформации = Перечисления.ТипыКонтактнойИнформации.Телефон Тогда
				КонтактнаяИнформация = CRM_УправлениеКонтактнойИнформацией.ДесериализацияТелефона(ЗначенияПолей,
					 Параметры.Представление,
					 ТипКонтактнойИнформации);
			Иначе
				КонтактнаяИнформация = CRM_УправлениеКонтактнойИнформацией.ДесериализацияФакса(ЗначенияПолей,
					 Параметры.Представление,
					 ТипКонтактнойИнформации);
			КонецЕсли;
		КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Данные.CountryCode) Тогда
		СтруктураПолей = сфпСофтФонПроСервер.сфпЗаполнитьСтруктуруПолейПоПредставлениюТелефон(Данные.value);
		Данные.countryCode = СтруктураПолей.КодСтраны;
		Данные.areaCode    = СтруктураПолей.КодГорода;
		Данные.number      = СтруктураПолей.НомерТелефона;
		Данные.extNumber   = СтруктураПолей.Добавочный;
	КонецЕсли;
	
	ЗначениеРеквизитовПоКонтактнойИнформации(Данные);
	
	Элементы.Добавочный.Видимость = ВидКонтактнойИнформации.ТелефонСДобавочнымНомером;
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоМобильныйКлиент() Тогда // Временное решение для работы в мобильном клиенте, будет удалено в следующих версиях
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Представление",
			 "ПодсказкаВвода",
				НСтр("ru='Представление';en='Presentation'"));
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "КомандаОК",
					"Отображение",
					ОтображениеКнопки.Картинка);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Отмена",		"Видимость",		Ложь);
		
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "КодСтраны",
					"ПоложениеЗаголовка",
			 ПоложениеЗаголовкаЭлементаФормы.Лево);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "КодГорода",
					"ПоложениеЗаголовка",
			 ПоложениеЗаголовкаЭлементаФормы.Лево);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "НомерТелефона",
				"ПоложениеЗаголовка",
			 ПоложениеЗаголовкаЭлементаФормы.Лево);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Добавочный",
				"ПоложениеЗаголовка",
			 ПоложениеЗаголовкаЭлементаФормы.Лево);
	...
	КонецЕсли;
	сфпВремяЗадержкиНабораДобавочногоНомера = ?(сфпВремяЗадержкиНабораДобавочногоНомера = 0, 5,
		 сфпВремяЗадержкиНабораДобавочногоНомера);
	
	ИспользоватьПрефиксы = Константы.сфпИспользоватьПрефиксыИсходящихВызов.Получить();
	Элементы.сфпПрефиксНабора.Видимость = ИспользоватьПрефиксы;
	
	Если Параметры.Свойство("сфпПрефиксНабора") Тогда
		сфпПрефиксНабора = Параметры.сфпПрефиксНабора;
	КонецЕсли;
	// -СофтФон
...
Процедура КодГородаПриИзменении(Элемент)
	...
	Если (КодСтраны = "+7" ИЛИ КодСтраны = "8") И СтрНачинаетсяС(КодГорода, "9") И СтрДлина(КодГорода) <> 3 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Кода мобильных телефонов начинающиеся на цифру 9 имеют фиксированную длину в 3 цифры,
			| например - 916.';en='If code of a cell phone begins with 9,
			| it has a fixed length 3 digits. For example, 916.'"), ,
			 "КодГорода");
	КонецЕсли;
	ЗаполнитьПредставлениеТелефона();
...
&НаКлиенте
Процедура сфпПрефиксНабораПриИзменении(Элемент)
...
Функция РезультатВыбора()
	...
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("Обработка.ВводКонтактнойИнформации.Форма.ВводТелефона",
		 "КодыСтраныИГорода", Коды,
		 НСтр("ru='Коды страны и города';en='Codes of country and city'"));
	...
	Результат.Вставить("КонтактнаяИнформация",
		 УправлениеКонтактнойИнформацией.КонтактнаяИнформацияВXML(ДанныеВыбора, КонтактнаяИнформация.Value,
		 ТипКонтактнойИнформации));
	...
	Результат.Вставить("сфпПрефиксНабора", сфпПрефиксНабора);
	// -СофтФон
...
Функция РезультатВыбораТолькоКомментария()
	...
	Если ПустаяСтрока(КонтактнаяИнфо) Тогда
		Если ТипКонтактнойИнформации = Перечисления.ТипыКонтактнойИнформации.Телефон Тогда
			КонтактнаяИнфо = CRM_УправлениеКонтактнойИнформацией.ДесериализацияТелефона("", "", ТипКонтактнойИнформации);
		Иначе
			КонтактнаяИнфо = CRM_УправлениеКонтактнойИнформацией.ДесериализацияФакса("", "", ТипКонтактнойИнформации);
		КонецЕсли;
...
Процедура ЗаполнитьПредставлениеТелефонаСейчас()
	...
	Представление = УправлениеКонтактнойИнформациейКлиентСервер.СформироватьПредставлениеТелефона(КодСтраны,
		КодГорода, НомерТелефона, Добавочный, Комментарий);
...
Функция ОшибкиЗаполненияТелефона()
	...
	Если НомерТелефонаСодержитНедопустимыеСимволы(ПолныйНомерТелефона) Тогда
		СписокОшибок.Добавить("НомерТелефона",
			 НСтр("ru='Номер телефона содержит недопустимые символы';
	...
	Если КодСтраны = "7" ИЛИ КодСтраны = "+7" Тогда
		Если СтрДлина(ТолькоЦифры(НомерТелефона)) > 7 Тогда
			СписокОшибок.Добавить("НомерТелефона",
				 НСтр("ru='В России номер телефона не может быть больше 7 цифр';
	...
	Если СтрНачинаетсяС(КодГорода, "9") И СтрДлина(КодГорода) <> 3 Тогда
		СписокОшибок.Добавить("НомерТелефона",
			 НСтр("ru='В России номера мобильных телефонов должны содержать 3 цифры';
