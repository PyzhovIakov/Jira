...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(ВнутреннийНомер) Тогда
		Элементы.НадписьЛинияПодключения.Заголовок				= НСтр("ru='Внутренний номер: ';en='Internal number: '") 
			+ ВнутреннийНомер;
...
Процедура HoldUnHold(Команда)
	...
	Если ТекЗвонок = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьОбновитьЗвонок(СтруктураЗвонка)
	// Звонки
	Если СтруктураЗвонка.hCall = 0 Тогда
		Возврат;
	КонецЕсли;
	ОбъектЗвонки = РеквизитФормыВЗначение("Звонки");
...
//	ДополнительныеПараметры	- Структура	- Структура дополнительных параметров
//
Процедура ВыполнитьПереключитьЗвонок(СтруктураКонтакта, // АПК:78 - исключить из проверки.
...
Процедура ВыбратьКонтактИзАдреснойКниги(ПереводЗвонка, ДополнительныеПараметры)
	...
	КонецЕсли;	
	ОповещениеВыбора = Новый ОписаниеОповещения(ДополнительныеПараметры.ИмяОповещения, ЭтотОбъект,
		 ДополнительныеПараметры);
	ОткрытьФорму("ОбщаяФорма.сфпАдреснаяКнига", ПараметрыФормы, ЭтотОбъект, ,
...
Процедура ОбновитьОтображениеЗвонков()
	Если Звонки.Количество() = 0 Тогда
		Элементы.ГруппаСтраницыЗвонков.ТекущаяСтраница =
			Элементы.ГруппаСтраницыЗвонков.ПодчиненныеЭлементы.ГруппаПропущенныеЗвонки;
...
Процедура СтатусПользователяНажатие(Элемент, СтандартнаяОбработка)
	...
	Если сфпПанельУправления <> Неопределено Тогда
		...
		Оповещение = Новый ОписаниеОповещения("ПриВыбореСтатусаПользователя", ЭтотОбъект);
