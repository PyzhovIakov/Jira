...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РежимПодбора = ?(ИспользоватьПодразделения, "1", "0");
	
	Элементы.РежимПодбора.Видимость = ИспользоватьПодразделения;
	
	Если Параметры.Свойство("ТекущаяСтрока")
		 И ТипЗнч(Параметры.ТекущаяСтрока) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
	...
	Иначе
		...
		ОбновитьЗначениеПараметраКомпоновкиДанных(ПользователиСписок, "ГруппаПользователей",
	...
	Если Параметры.Свойство("Пользователи") И ТипЗнч(Параметры.Пользователи) = Тип("Массив") Тогда
		...
		Для Каждого Пользователь Из Параметры.Пользователи Цикл
			Если ТипЗнч(Пользователь) = Тип("Структура") Тогда
				Если ТаблицаВыбранныеПользователи.НайтиСтроки(Новый Структура("Значение",
					 Пользователь.Пользователь)).Количество() = 0 Тогда
				...
				СтруктураПоиска = Новый Структура("Значение,НачалоПериода,ОкончаниеПериода",
					 Пользователь.Пользователь, Пользователь.НачалоПериода,
					 Пользователь.ОкончаниеПериода);
	...
	Если Параметры.Свойство("ЗакрыватьФормуПослеВыбораПериодов") Тогда
		...
		Элементы.КалендарьТабличныйДокументКонтекстноеМенюКомандаДобавитьПользователяСПериодом.Заголовок =
			НСтр("ru='Выбрать период';
			|en='Select period'");
	КонецЕсли;
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	Если Настройки["РежимПодбора"] = Неопределено
		 ...
		 И Настройки["РежимПодбора"] <> "1") Тогда
...
Процедура КалендарьОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если ТипЗнч(Расшифровка) = Тип("Число")
		И ТаблицаРасшифровок.Количество() > Расшифровка Тогда
		ЗначениеРасшифровки = ТаблицаРасшифровок[Расшифровка];
	Иначе
		Возврат;
	КонецЕсли;
	...
	Если ЗначениеРасшифровки.ТипЯчейки = "Событие" Тогда
		Если ЗначениеЗаполнено(ЗначениеРасшифровки.Событие) Тогда
			ПоказатьЗначение(, ЗначениеРасшифровки.Событие);
		КонецЕсли;
	ИначеЕсли ЗначениеРасшифровки.ТипЯчейки = "ЗаголовокРесурс" Тогда
		Если ЗначениеЗаполнено(ЗначениеРасшифровки.Пользователь) Тогда
			НайденныеСтроки = ТаблицаВыбранныеПользователи.НайтиСтроки(Новый Структура("Значение",
				 ЗначениеРасшифровки.Пользователь));
			Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
				ТаблицаВыбранныеПользователи.Удалить(НайденнаяСтрока);
			КонецЦикла;
			НайденныеСтроки = ТаблицаВыбранныеПользователиСПериодами.НайтиСтроки(Новый Структура("Значение",
				 ЗначениеРасшифровки.Пользователь));
			Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
...
Процедура ГруппыПользователейПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если НЕ Копирование Тогда
		...
		Если ЗначениеЗаполнено(Элементы.ГруппыПользователей.ТекущаяСтрока) Тогда
			...
			ПараметрыФормы.Вставить("ЗначенияЗаполнения", Новый Структура("Родитель",
...
Процедура ГруппыПользователейНачалоПеретаскивания(Элемент, ПараметрыПеретаскивания, Выполнение)
	...
	ПодключитьОбработчикОжидания("Подключаемый_ОбработчикОжиданияКалендарьТабличныйДокументОтменитьРежимРедактирования",
		 0.1,
		 Истина);
КонецПроцедуры
...
Процедура КомандаДобавитьПользователяСПериодом(Команда)
	...
	КонПериода = ТекущиеДанные.МассивДат[ТекущиеДанные.МассивДат.ВГраница()] 
		+ НастройкиОтображения.РасписаниеИнтервалШкалыВремени * 60;
...
Процедура КомандаУдалитьПользователя(Команда)
	...
	Для Каждого ПользовательДляУдаления Из МассивРесурсы Цикл
		Если Не ЗначениеЗаполнено(ПользовательДляУдаления) Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ПризнакЗакрыватьФормуПослеВыбораПериодов И ТаблицаВыбранныеПользователиСПериодами.Количество() = 0 Тогда
			Для Каждого СтрокаТаблицы Из ТаблицаРесурсы Цикл
				Если ЗначениеЗаполнено(СтрокаТаблицы.Значение) И Массив.Найти(СтрокаТаблицы.Значение) = Неопределено Тогда
					Массив.Добавить(Новый Структура(?(ПризнакВыбиратьПомещения, "Помещение", "Пользователь") + ",
						|НачалоПериода,ОкончаниеПериода", СтрокаТаблицы.Значение, ПериодДокументаНачало,
						 ПериодДокументаОкончание));
				КонецЕсли;
			КонецЦикла;
		Иначе
			Для Каждого СтрокаТаблицы Из ТаблицаВыбранныеПользователиСПериодами Цикл
				Массив.Добавить(Новый Структура(?(ПризнакВыбиратьПомещения, "Помещение", "Пользователь") + ",
					|НачалоПериода,ОкончаниеПериода", СтрокаТаблицы.Значение, СтрокаТаблицы.НачалоПериода,
					 СтрокаТаблицы.ОкончаниеПериода));
			КонецЦикла;
		КонецЕсли;
	Иначе
		Для Каждого СтрокаТаблицы Из ТаблицаРесурсы Цикл
...
Процедура НастроитьВидимостьИДоступностьФормы(Форма)
	...
	Если Форма.Элементы.КалендарьТабличныйДокументКонтекстноеМенюКомандаДобавитьПользователяСПериодом.Видимость <> Форма.ПризнакВыбиратьПериоды Тогда
		Форма.Элементы.КалендарьТабличныйДокументКонтекстноеМенюКомандаДобавитьПользователяСПериодом.Видимость =
			Форма.ПризнакВыбиратьПериоды;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбновитьСодержимоеФормыПриИзмененииГруппы()
	...
	Если НЕ ИспользоватьГруппы
	 ИЛИ Элементы.ГруппыПользователей.ТекущаяСтрока = ГруппаПользователейВсеПользователи Тогда
		...
		ОбновитьЗначениеПараметраКомпоновкиДанных(ПользователиСписок, "ВыбиратьИерархически", Истина);
		ОбновитьЗначениеПараметраКомпоновкиДанных(ПользователиСписок, "ГруппаПользователей",
			 ГруппаПользователейВсеПользователи);
	Иначе
...
Функция ПолучитьНастройкиОтображенияПоУмолчанию()
	...
	СтруктураНастройкиОтображения.КалендарьШрифтТекстаЗавершенногоСобытия	= Новый Шрифт("Arial", 8,
		 Ложь, Ложь, Ложь,
		 Истина);
...
Процедура ЗагрузитьНастройкиКалендаря()
	...
	Если ПризнакПоказыватьПериодДокумента И ЗначениеЗаполнено(ПериодДокументаНачало)
		 И ЗначениеЗаполнено(ПериодДокументаОкончание) Тогда
		СтруктураНастройкиОтображения.НастройкиОтображения.НачалоДняКалендаряЧас = Мин(Час(ПериодДокументаНачало),
			 СтруктураНастройкиОтображения.НастройкиОтображения.НачалоДняКалендаряЧас);
		СтруктураНастройкиОтображения.НастройкиОтображения.КонецДняКалендаряЧас = Макс(Час(ПериодДокументаОкончание),
			 СтруктураНастройкиОтображения.НастройкиОтображения.КонецДняКалендаряЧас);
	КонецЕсли;
	
	// Настройки пользователя
	НастройкиРабочееВремяПользователя =
		CRM_ОбщегоНазначенияСервер.ПолучитьСтруктуруРабочихВременПользователя(ТекущийПользовательСеанса);
...
Функция ПолучитьСтруктуруДанныхЗапроса()
	...
	|	// Фильтр на дату
	|	И (ВЫБОР КОГДА СобытияКалендаряПользователя.ПериодОкончание = ДАТАВРЕМЯ(1,1,1) 
	|		ТОГДА СобытияКалендаряПользователя.ПериодНачало 
	|		ИНАЧЕ СобытияКалендаряПользователя.ПериодОкончание 
	|		КОНЕЦ) >= &ПериодНачало И СобытияКалендаряПользователя.ПериодНачало <= &ПериодОкончание
	|	
...
&НаСервере
Процедура СоздатьОтборПоОписанию(ЭлементыОтбора, ОписанияЭлементовОтбора,
	 ДоступныеПоля = Неопределено,
	 ЭтоОтборДляДанныхИзКэша = Ложь)
	Для Каждого ОписаниеЭлементаОтбораКлючЗначение Из ОписанияЭлементовОтбора Цикл
		...
		Иначе
			Если ДоступныеПоля <> Неопределено Тогда
				Если ДоступныеПоля.НайтиПоле(Новый ПолеКомпоновкиДанных(ОписаниеЭлементаОтбораКлючЗначение.Ключ)) =
					Неопределено Тогда Продолжить; КонецЕсли;
			КонецЕсли;
...
&НаСервере
Функция ПолучитьДанныеСобытийЗаПериодПоДням(СтруктураДанныхКлиент, НастройкиОтображения,
	 МаксимальныйИндексСобытияНаВесьДень = -1, ПериодНачало = Неопределено,
	...
	УстановитьЗначениеВОтборе(Настройки.Отбор, "СобытиеНаВесьДень", ВидСравненияКомпоновкиДанных.Равно,
		 Ложь, Истина,
		 КомпоновщикНастроек.Настройки.ДоступныеПоляВыбора);
...
Функция ПолучитьМакетНаСервере(ИмяМакета)
	Возврат Обработки.УдалитьCRM_Календарь.ПолучитьМакет(ИмяМакета);
КонецФункции
...
Функция ПолучитьКалендарныйГрафик(	ДатаИлиМассивДат,
	...
	Возврат CRM_ОбщегоНазначенияСервер.ПолучитьСоответствиеКалендарныхГрафиков(ДатаИлиМассивДат,
		 ПроизводственныйКалендарь,
		 Пользователь);
КонецФункции

&НаСервереБезКонтекста
Функция ПолучитьСтруктуруДополнительногоОписанияИндексаВремени(СтруктураОписаниеКалендарногоДня,
	 ВремяИндексаНачало,
	 ВремяИндексаКонец)
	...
	Если бРабочийДеньДляРесурса = Истина Тогда
		Если СтруктураОписаниеКалендарногоДня.МассивПериодов = Неопределено
			 Или СтруктураОписаниеКалендарногоДня.МассивПериодов.Количество() = 0 Тогда
			// отклонений нет
		Иначе
			// TODO:
		КонецЕсли;
...
&НаСервере
Процедура СвернутьТаблицуДанныхПоПериодуПоРесурсам(ТаблицаДанные,
	 СтруктураОтбораПоРесурсу = Неопределено,
	 бСворачиватьПоРесурсам = Неопределено)
	...
	Если бИспользуютсяВыбранныеРесурсы И РесурсИмя = "Пользователь" Тогда
		...
		Для Каждого СтрокаСобытие Из ТаблицаДанные Цикл
			Если СоответствиеОбработанныеСтроки[СтрокаСобытие] <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
...
Функция ПолучитьПредставлениеСобытия(Событие, ОписаниеСобытия, НастройкиОтображения, ВысотаОбластиСобытия = 1)
	...
	ПредставлениеПериод = Формат(ОписаниеСобытия.ПериодНачало, "ДФ=ЧЧ:мм") + " - " 
		+ Формат(ОписаниеСобытия.ПериодОкончание,
		 "ДФ=ЧЧ:мм");
	...
	Для Каждого СтрСписка Из СписокПредставлениеСобытияВКалендаре Цикл
		Если Не СтрСписка.Пометка Тогда
			Продолжить;
		КонецЕсли;
		Если СтрСписка.Значение = "Время" 
			И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ОписаниеСобытия, "СобытиеНаВесьДень")
			И ОписаниеСобытия.СобытиеНаВесьДень Тогда
			Продолжить;
		КонецЕсли;
		...
		ИначеЕсли СтрСписка.Значение = "Тема" Тогда
			...
			КонецЕсли;
		Иначе
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ОписаниеСобытия, СтрСписка.Значение) Тогда
...
Процедура СформироватьРасписаниеНаСервере(СтруктураДанныхКлиент)
	...
	КоличествоИнтерваловШкалыВремениВДне = КоличествоИнтерваловШкалыВремениВЧасе * (ИндексЧасОкончание - ИндексЧасНачало 
		+ 1);
	КоличествоИнтерваловШкалыВремениВПериоде = ((НачалоДня(МаксимальнаяДата) - НачалоДня(МинимальнаяДата)) / КоличествоСекундВДне 
		+ 1) * КоличествоИнтерваловШкалыВремениВДне;
	
	МинимальнаяДатаВремяНаШкале = НачалоДня(МинимальнаяДата) + ИндексЧасНачало * 60 * 60;
	МаксимальнаяДатаВремяНаШкале = НачалоДня(МаксимальнаяДата) + ИндексЧасОкончание * 60 * 60 
		+ ИнтервалШкалыВремениВСекундах * (КоличествоИнтерваловШкалыВремениВЧасе - 1);
	
	ДанныеЗаПериод = ПолучитьДанныеСобытийЗаПериодПоДням(СтруктураДанныхКлиент, НастройкиОтображения, ,
		 МинимальнаяДатаВремяНаШкале,
		 МаксимальнаяДатаВремяНаШкале);
	...
	ВремяОкончаниеШкалыВремени = ПустаяДата + ИндексЧасОкончание * 60 * 60 
		+ КоличествоИнтерваловШкалыВремениВЧасе * ИнтервалШкалыВремени * 60 - 1;
	...
	ОбластьОтрисовка = ТабДокументЯчейкаОбластиОтрисовки.Область(1, 1,
		 ТабДокументЯчейкаОбластиОтрисовки.ВысотаТаблицы,
		 ТабДокументЯчейкаОбластиОтрисовки.ШиринаТаблицы);
	ОбластьОтрисовка.Объединить();
	ОбластьОтрисовкаЛево = ТабДокументЯчейкаОбластиОтрисовки.Область(1, 1, 1, 1);
	ОбластьОтрисовкаПраво = ТабДокументЯчейкаОбластиОтрисовки.Область(1, КоличестоЯчеекВИнтервале, 1,
		 КоличестоЯчеекВИнтервале);
	...
	ОбластьЧасПраваяЧасть = ТабДокументЧас.Область(1, КоличестоЯчеекВИнтервале,
		 ТабДокументЧас.ВысотаТаблицы,
		 ТабДокументЧас.ШиринаТаблицы);
	ОбластьЧасПраваяЧастьИндикатор = ТабДокументЧас.Область(НомерСтрокиИндикатора, 1,
		 НомерСтрокиИндикатора,
		 ТабДокументЧас.ШиринаТаблицы);
	ОбластьИнтервал = ТабДокументИнтервал.Область(1, 1, ТабДокументИнтервал.ВысотаТаблицы,
		 ТабДокументИнтервал.ШиринаТаблицы);
	ОбластьИнтервалВерх = ТабДокументИнтервал.Область(1, 1, 1, ТабДокументИнтервал.ШиринаТаблицы);
	ОбластьИнтервалНиз = ТабДокументИнтервал.Область(2, 1, ТабДокументИнтервал.ВысотаТаблицы,
		 ТабДокументИнтервал.ШиринаТаблицы);
	
	ТекущееВремяОкругленное = ПустаяДата 
		+ Цел((ТекущееВремя - ПустаяДата) / ИнтервалШкалыВремениВСекундах) * ИнтервалШкалыВремениВСекундах;
	...
	СлужебнаяОбластьИнтервал = ТабДокументСлужебнаяОбластьИнтервал.Область(1, 1,
		 ТабДокументСлужебнаяОбластьИнтервал.ВысотаТаблицы,
		 ТабДокументСлужебнаяОбластьИнтервал.ШиринаТаблицы);
	
	МаксимальныйИндексВремениНаШкале = Неопределено;
	ШиринаОбластиДняВЯчейках = Неопределено;
	
	бВыделятьПериодДокумента = (ПризнакПоказыватьПериодДокумента
		 И ЗначениеЗаполнено(ПериодДокументаНачало)
		 И ЗначениеЗаполнено(ПериодДокументаОкончание));
	...
	Если бВыделятьПериодДокумента Тогда
		...
		ПериодДокументаНачалоОкругленный = НачалоДня(ПериодДокументаНачало) 
			+ Цел(ВремяДокументаНачалоВСекундах / ИнтервалШкалыВремениВСекундах) * ИнтервалШкалыВремениВСекундах;
		Если Цел(ВремяДокументаОкончаниеВСекундах / ИнтервалШкалыВремениВСекундах) = ВремяДокументаОкончаниеВСекундах / ИнтервалШкалыВремениВСекундах Тогда
			ПериодДокументаОкончаниеОкругленный = НачалоДня(ПериодДокументаОкончание) 
				+ Цел(ВремяДокументаОкончаниеВСекундах / ИнтервалШкалыВремениВСекундах) * ИнтервалШкалыВремениВСекундах;
		Иначе
			ПериодДокументаОкончаниеОкругленный = НачалоДня(ПериодДокументаОкончание) 
				+ Цел(ВремяДокументаОкончаниеВСекундах / ИнтервалШкалыВремениВСекундах) * ИнтервалШкалыВремениВСекундах 
				+ ИнтервалШкалыВремениВСекундах;
		КонецЕсли;
	...
	Пока ТекДата <= НачалоДня(МаксимальнаяДата) Цикл
		...
		Для ИндексЧас = ИндексЧасНачало По ИндексЧасОкончание Цикл
			...
			Если	(ИндексЧас < НастройкиОтображения.НачалоРабочегоДняЧас Или ИндексЧас > НастройкиОтображения.КонецРабочегоДняЧас)
				Или	(НастройкиОтображения.ЕстьОбеденныйПерерыв = Истина
					 И (ИндексЧас >= НастройкиОтображения.НачалоОбеденногоПерерываЧас
					 И ИндексЧас < НастройкиОтображения.КонецОбеденногоПерерываЧас)) Тогда
				//
				ТекущийЦветФона = ?(бТекущееВремя,
					 НастройкиОтображения.КалендарьЦветФонаТекущееВремяНеРабочееВремя,
					 НастройкиОтображения.КалендарьЦветШкалыВремениНеРабочегоВремени);
			Иначе
				ТекущийЦветФона = ?(бТекущееВремя,
					 НастройкиОтображения.КалендарьЦветФонаТекущееВремяРабочееВремя,
					 НастройкиОтображения.КалендарьЦветШкалыВремениРабочегоВремени);
			КонецЕсли;
			...
			Если бВыделятьПериодДокумента Тогда
				Если ДатаВремяВИндексе = ПериодДокументаНачалоОкругленный Тогда
					НачалоОбластиВыделенияПериодаДокумента = Новый Структура("НомерДня,ИндексВремени",
						 ТекущийНомерДня,
						 ТекущийИндексВремени);
				КонецЕсли;
				Если ДатаВремяВИндексе = ПериодДокументаОкончаниеОкругленный Тогда
					КонецОбластиВыделенияПериодаДокумента = Новый Структура("НомерДня,ИндексВремени",
						 ТекущийНомерДня,
						 ТекущийИндексВремени - 1);
				КонецЕсли;
			КонецЕсли;
			...
			Для ИндексИнтервал = 1 По КоличествоИнтерваловШкалыВремениВЧасе - 1 Цикл
				...
				Если	(ИндексЧас < НастройкиОтображения.НачалоРабочегоДняЧас
					 Или ИндексЧас > НастройкиОтображения.КонецРабочегоДняЧас)
					Или	(НастройкиОтображения.ЕстьОбеденныйПерерыв = Истина
						 И (ИндексЧас >= НастройкиОтображения.НачалоОбеденногоПерерываЧас
						 И ИндексЧас < НастройкиОтображения.КонецОбеденногоПерерываЧас)) Тогда
					//
		...
		Если МаксимальныйИндексВремениНаШкале =
			Неопределено Тогда МаксимальныйИндексВремениНаШкале = ТекущийИндексВремени - 1; КонецЕсли;
		Если ШиринаОбластиДняВЯчейках =
			Неопределено Тогда ШиринаОбластиДняВЯчейках = ТабДокументШкалаВремениДень.ШиринаТаблицы; КонецЕсли;
		
		ОбластьЛинияСнизу = ТабДокументШкалаВремениДень.Область(ВысотаШкалыВремени + 1, 1,
			 ВысотаШкалыВремени + 1,
			 ТабДокументШкалаВремениДень.ШиринаТаблицы);
		...
		Если бИспользоватьКалендарныеГрафики Тогда // Цвет заголовка дня в зависимости от календарного графика.
			...
			Если СтруктураОписаниеКалендарногоДня = Неопределено
				 Или СтруктураОписаниеКалендарногоДня.ДеньРабочийПоПроизводственномуКалендарю = Истина Тогда
	...
	Если НачалоОбластиВыделенияПериодаДокумента <> Неопределено
		 И КонецОбластиВыделенияПериодаДокумента <> Неопределено Тогда
		ИндексОбластиНачало = НачалоОбластиВыделенияПериодаДокумента.НомерДня * (МаксимальныйИндексВремениНаШкале 
			+ 1) 
			+ НачалоОбластиВыделенияПериодаДокумента.ИндексВремени;
		ИндексОбластиКонец = КонецОбластиВыделенияПериодаДокумента.НомерДня * (МаксимальныйИндексВремениНаШкале 
			+ 1) 
			+ КонецОбластиВыделенияПериодаДокумента.ИндексВремени;
	...
	ТабДокументСвободнаяОбластьСлева = МакетЯчейки.ПолучитьОбласть(1, 1,
		 ТабДокументШкалаВремениДень.ВысотаТаблицы,
		 ШиринаОбластиРесурсовВЯчейках);
	Область = ТабДокументСвободнаяОбластьСлева.Область(1, 1,
		 ТабДокументСвободнаяОбластьСлева.ВысотаТаблицы,
		 ТабДокументСвободнаяОбластьСлева.ШиринаТаблицы);
	Область.Объединить();
	...
	Для нИндекс = 0 По КоличествоИнтерваловШкалыВремениВПериоде - 1 Цикл
		ТаблицаЗанятыеИндексыПоРесурсуБланк.Колонки.Добавить("ИндексИнтервала" + Формат(нИндекс,
			 "ЧН=0; ЧГ="), ОписаниеТиповЧисло);
	КонецЦикла;
	...
	Для Каждого Ресурс Из ТаблицаРесурсов Цикл
		...
		Если ЗначениеЗаполнено(ТекущийПользователь) Тогда
			...
			Иначе
				...
				Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
					МассивВыбранныеПериодыПользователя.Добавить(Новый Структура("НачалоПериода,ОкончаниеПериода",
						 НайденнаяСтрока.НачалоПериода,
						 НайденнаяСтрока.ОкончаниеПериода));
				КонецЦикла;
			КонецЕсли;
		Иначе
			МассивВыбранныеПериодыПользователя = Неопределено;
		...
		Если бИспользоватьКалендарныеГрафики Тогда
			...
			КалендарныйГрафик = ПолучитьКалендарныйГрафик(МассивДат, ,
				 ПараметрКалендарногоГрафикаПользователь,
				 ПараметрКалендарногоГрафикаПомещение);
			Если КалендарныйГрафик = Неопределено Тогда
				бИспользоватьКалендарныеГрафики = Ложь;
			КонецЕсли;
		КонецЕсли;
		...
		Пока ТекДата <= НачалоДня(МаксимальнаяДата) Цикл
			...
			Для ИндексЧас = ИндексЧасНачало По ИндексЧасОкончание Цикл
				...
				Если СтруктураОписаниеКалендарногоДня <> Неопределено Тогда
					ДопСтруктураОписаниеИндекса = ПолучитьСтруктуруДополнительногоОписанияИндексаВремени(СтруктураОписаниеКалендарногоДня,
						 ВремяВИндексе, ВремяВИндексе 
						+ ИнтервалШкалыВремени * 60);
				КонецЕсли;
				...
				Если	(ИндексЧас < НастройкиОтображения.НачалоРабочегоДняЧас
					...
					Или	(НастройкиОтображения.ЕстьОбеденныйПерерыв = Истина
						 И (ИндексЧас >= НастройкиОтображения.НачалоОбеденногоПерерываЧас
						 И ИндексЧас < НастройкиОтображения.КонецОбеденногоПерерываЧас))
				...
				Если МассивВыбранныеПериодыПользователя <> Неопределено Тогда
					Для Каждого ВыбранныйПериодПользователя Из МассивВыбранныеПериодыПользователя Цикл
						Если ДатаВремяВИндексе >= ВыбранныйПериодПользователя.НачалоПериода
							 И ДатаВремяВИндексе < ВыбранныйПериодПользователя.ОкончаниеПериода Тогда
		...
		Для Каждого СтрокаСобытие Из ТаблицаСобытия Цикл
			...
			Если ОписаниеСобытия = Неопределено Тогда
				ОписаниеСобытия = Обработки.УдалитьCRM_Календарь.ПолучитьСтруктуруОписаниеСобытияБланк();
				...
				Иначе
					...
					Иначе
						...
						ПериодНачалоСобытияОкругленный = НачалоДня(ОписаниеСобытия.ПериодНачало) 
							+ Цел(НачалоСобытияВСекундах / ИнтервалШкалыВремениВСекундах) * ИнтервалШкалыВремениВСекундах;
					КонецЕсли;
				КонецЕсли;
				...
				Иначе
					...
					Иначе
						...
						ПериодОкончанияСобытияОкругленный = НачалоДня(ОписаниеСобытия.ПериодОкончание) 
							+ Цел(ОкончаниеСобытияВСекундах / ИнтервалШкалыВремениВСекундах) * ИнтервалШкалыВремениВСекундах 
							+ ИнтервалШкалыВремениВСекундах;
					КонецЕсли;
					...
					КонецЕсли;
				КонецЕсли;
				ВремяНачалоСобытияОкругленное = ПустаяДата 
					+ (ПериодНачалоСобытияОкругленный - НачалоДня(ПериодНачалоСобытияОкругленный));
				ВремяОкончанияСобытияОкругленное = ПустаяДата 
					+ (ПериодОкончанияСобытияОкругленный - НачалоДня(ПериодОкончанияСобытияОкругленный));
				
				НомерДняВИнтервалеНачалоСобытия = СоответствиеНомеровДней[НачалоДня(ПериодНачалоСобытияОкругленный)];
				Если НомерДняВИнтервалеНачалоСобытия = Неопределено Тогда
					НомерДняВИнтервалеНачалоСобытия = 0;
				КонецЕсли;
				НомерДняВИнтервалеОкончаниеСобытия = СоответствиеНомеровДней[НачалоДня(ПериодОкончанияСобытияОкругленный)];
				Если НомерДняВИнтервалеОкончаниеСобытия =
					Неопределено Тогда НомерДняВИнтервалеОкончаниеСобытия = МаксимальныйНомерДня; КонецЕсли;
				...
				Если ИндексВремениНачалоСобытия = Неопределено И ИндексВремениОкончаниеСобытия = Неопределено
					 И НачалоДня(ПериодНачалоСобытияОкругленный) = НачалоДня(ПериодОкончанияСобытияОкругленный) Тогда
				...
				ОписаниеСобытия.ИндексНачало = НомерДняВИнтервалеНачалоСобытия * КоличествоИнтерваловШкалыВремениВДне 
					+ ИндексВремениНачалоСобытия;
				ОписаниеСобытия.ИндексКонец = НомерДняВИнтервалеОкончаниеСобытия * КоличествоИнтерваловШкалыВремениВДне 
					+ ИндексВремениОкончаниеСобытия;
				...
				Обработки.УдалитьCRM_Календарь.ЗаполнитьДополнительныеСвойстваСобытия(ОписаниеСобытия, НастройкиОтображения);
			...
			НоваяСтрока.КонецОбластиКолонка		= (ОписаниеСобытия.ИндексКонец * КоличестоЯчеекВИнтервале) 
				+ КоличестоЯчеекВИнтервале;
		КонецЦикла;
		
		// Оформление табличного документа по ресурсу с областями событий + одна строчка свободной области.
		ВысотаОбластиПоРесурсу = Макс(МаксимальныйГоризонтальныйИндексСобытия + 1,
			 ВысотаОбластиРесурсовВЯчейках) + ?(МаксимальныйГоризонтальныйИндексСобытия = 0, 0,
			 1);
		...
		Для номер = 1 По ВысотаОбластиПоРесурсу Цикл
			Если номер = ВысотаОбластиПоРесурсу Тогда
				ТабДокументСтрокаФонаОтрисовки.Область(1, 1, 1,
					 ТабДокументСтрокаФонаОтрисовки.ШиринаТаблицы).ГраницаСнизу = ЛинияГраницаЖирная;
			КонецЕсли;
			...
			СлужебнаяОбластьСправа = ТабДокументСобытияПоРесурсу.Область(номер,
				 КолонкаСлужебнаяОбластьСправа, номер,
				 КолонкаСлужебнаяОбластьСправа);
			Расшифровка = ПолучитьСтруктуруРасшифровкиЯчейкиБланк();
			Расшифровка.ТипЯчейки = "СлужебнаяОбласть";
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Расшифровка, Ресурс.Имя + "Отбор") Тогда
				Расшифровка[Ресурс.Имя + "Отбор"] = Ресурс.Значение;
			КонецЕсли;
			СлужебнаяОбластьСправа.Расшифровка = ТаблицаРасшифровок.Индекс(Расшифровка);
			СлужебнаяОбластьСправа.Текст = " ";
		КонецЦикла;
		ТабДокументСтрокаФонаОтрисовки.Область(1, 1, 1,
			 ТабДокументСтрокаФонаОтрисовки.ШиринаТаблицы).ГраницаСнизу = ЛинияНетЛинии;
		
		// Оформление и вывод областей событий.
		Для Каждого СтрокаОбласть Из ТаблицаОбластиВыводаСобытий Цикл
			ОбластьСобытие = ТабДокументСобытияПоРесурсу.Область(СтрокаОбласть.НачалоОбластиСтрока,
				 СтрокаОбласть.НачалоОбластиКолонка, СтрокаОбласть.НачалоОбластиСтрока,
				 СтрокаОбласть.КонецОбластиКолонка);
			ОбластьСобытие.Объединить();
			ОбластьСобытие.Обвести(	ЛинияСобытие,
									ЛинияСобытие,
									?(СтрокаОбласть.КонецОбластиКолонка = ШиринаОбластиДняВЯчейках
										 Или СтрокаОбласть.КонецОбластиКолонка = ТабДокументСобытияПоРесурсу.ШиринаТаблицы - 1, ЛинияГраницаЖирная, ЛинияСобытие),
									ЛинияСобытие);
			//
			...
			Если НастройкиОтображения.КалендарьОтображатьПиктограммы И Не бВебКлиент Тогда
				Если РасположитьКартинкуВНачалеОбласти(ТабДокументСобытияПоРесурсу, ОбластьСобытие,
					 СтрокаОбласть.ОписаниеСобытия.Пиктограмма) Тогда
					СтрДобавитьКПараметру = "     ";
				КонецЕсли;
			КонецЕсли;
			ОбластьСобытие.Текст = СтрДобавитьКПараметру 
				+ ПолучитьПредставлениеСобытия(СтрокаОбласть.ОписаниеСобытия.Событие,
					 СтрокаОбласть.ОписаниеСобытия, НастройкиОтображения,
					 1);
		КонецЦикла;
		
		// Оформление области заголовка ресурса.
		ТабДокументСобытияПоРесурсуЗаголовок = МакетЯчейки.ПолучитьОбласть(1, 1, ВысотаОбластиПоРесурсу,
			 ШиринаОбластиРесурсовВЯчейках);
	...
	Для нИндекс = 0 По МассивИндикаторЗанятостиВремени.ВГраница() Цикл
		Если МассивИндикаторЗанятостиВремени[нИндекс] <> Неопределено Тогда
			Область = ТабДокументШкалаВремени.Область(НомерСтрокиИндикатора, ШиринаОбластиРесурсовВЯчейках + нИндекс * КоличестоЯчеекВИнтервале + 1, НомерСтрокиИндикатора,
				 ШиринаОбластиРесурсовВЯчейках 
				+ нИндекс * КоличестоЯчеекВИнтервале 
				+ КоличестоЯчеекВИнтервале);
	...
	ТабДокументСлужебнаяОбластьСнизуСЗаголовком.Присоединить(МакетЯчейки.ПолучитьОбласть(1, 1, 1,
		 ШиринаОбластиРесурсовВЯчейках));
...
Процедура КалендарьТабличныйДокументПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Область)
	...
	Для Каждого ЗначениеПеретаскивания Из МассивЗначений Цикл
		Если ЗначениеЗаполнено(ЗначениеПеретаскивания)
...
Процедура ДобавитьПользователяИлиГруппуИлиПомещение(ПользовательГруппаПользователейИлиПомещениеИлиМассив)
	...
	Для Каждого РесурсСсылка Из МассивРесурсов Цикл
		...
		ИначеЕсли ТипЗнч(РесурсСсылка) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
			МассивПользователиГруппы = CRM_ОбщегоНазначенияСервер.ПолучитьПользователейГруппы(РесурсСсылка,
	...
	Для нИндексСтрока = ИндексСтрокаНачало По ИндексСтрокаКонец Цикл
		Для нИндексКолонка = ИндексКолонкаНачало По ИндексКолонкаКонец Цикл
			...
			Если ТипЗнч(Область.Расшифровка) = Тип("Число") Тогда
				Попытка
					Расшифровка = ТаблицаРасшифровок[Область.Расшифровка];
			...
			Если Расшифровка <> Неопределено Тогда
				...
				Иначе
					...
					Иначе
						СтруктураРезультат.ИндексВремениОкончание = Макс(СтруктураРезультат.ИндексВремениНачало,
							 Расшифровка.ИндексВремени);
					КонецЕсли;
				...
				Если ЗначениеЗаполнено(Расшифровка.ПользовательОтбор)
					 И СтруктураРезультат.МассивПользователей.Найти(Расшифровка.ПользовательОтбор) = Неопределено Тогда
					СтруктураРезультат.МассивПользователей.Добавить(Расшифровка.ПользовательОтбор);
				КонецЕсли;
				
				Если ЗначениеЗаполнено(Расшифровка.Пользователь)
					 И СтруктураРезультат.МассивПользователей.Найти(Расшифровка.Пользователь) = Неопределено Тогда
					СтруктураРезультат.МассивПользователей.Добавить(Расшифровка.Пользователь);
				КонецЕсли;
				
				Если ЗначениеЗаполнено(Расшифровка.ПомещениеОтбор)
					 И СтруктураРезультат.МассивПомещений.Найти(Расшифровка.ПомещениеОтбор) = Неопределено Тогда
		...
		СлужебнаяОбласть = КалендарьТабличныйДокумент.Область(нИндексСтрока, ШиринаТабличногоДокумента,
			 нИндексСтрока,
			 ШиринаТабличногоДокумента);
		Если ТипЗнч(СлужебнаяОбласть.Расшифровка) = Тип("Число") Тогда
			Попытка
				Расшифровка = ТаблицаРасшифровок[СлужебнаяОбласть.Расшифровка];
...
Процедура СписокПользователейУстановитьОтбор()
	...
	СписокПользователейВидСравнения = ?(ВыбиратьИерархически,
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(СписокПользователей,
		 "Подразделение", Подразделение, СписокПользователейВидСравнения, ,
		 ЗначениеЗаполнено(Подразделение[0]));
