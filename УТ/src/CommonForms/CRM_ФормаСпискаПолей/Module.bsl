...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Параметры.Свойство("Режим", Режим) ИЛИ НЕ Параметры.Свойство("АдресСхемы")
		 ИЛИ НЕ Параметры.Свойство("АдресНастроек") Тогда
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьФильтры()
	...
	Для каждого Поле Из Компоновщик.Настройки.ДоступныеПоляОтбора.Элементы Цикл
		...
		Если Поле.ТипЗначения.Типы().Количество() = 1
			 И Перечисления.ТипВсеСсылки().СодержитТип(Поле.ТипЗначения.Типы().Получить(0)) Тогда
