...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ЗаполнитьПоУмолчанию") И Параметры.ЗаполнитьПоУмолчанию Тогда
		ЗаполнитьПоУмолчаниюТипСтрокой =
			CRM_ОбщегоНазначенияСервер.НастройкиПолейОтображенияСодержанияСсылочныйТипСтрокой(ТипЗнч(Параметры.Объект));
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	Если ЗначениеЗаполнено(ЗаполнитьПоУмолчаниюТипСтрокой) Тогда
		НайденныеСтроки = CRM_ОбщегоНазначенияКлиентСервер.НайтиВЭлементахДерева(ДеревоОбъекты.ПолучитьЭлементы(),
			 Новый Структура("Объект", ЗаполнитьПоУмолчаниюТипСтрокой),
			 Истина);
...
Процедура ЦветФонаОчистка(Элемент, СтандартнаяОбработка)
	...
	ТекущиеДанные.ЦветФона = Новый Цвет(250, 250, 250);
	Модифицированность = Истина;
КонецПроцедуры
...
Процедура КомандаСохранитьВФайл(Команда)
	Если ПризнакВключенОтбор Тогда
		Возврат;
	КонецЕсли;
	...
	Если Модифицированность Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения,
			 НСтр("ru='Данные должны быть записаны. Записать данные?';
			|en='Data should be written down. Write down the data?'"), РежимДиалогаВопрос.ДаНет, ,
			 КодВозвратаДиалога.Да,
			 НСтр("ru='Сохранение настроек в файл'"));
	Иначе
...
Процедура КомандаСохранитьВФайлЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если Не ПроверитьСохранить() Тогда
		Возврат;
	КонецЕсли;
...
Процедура КомандаОтключитьОтбор(Команда)
	...
	Если ТекущийОбъектВДереве <> Неопределено Тогда
		НайденныеСтроки = CRM_ОбщегоНазначенияКлиентСервер.НайтиВЭлементахДерева(ДеревоОбъекты.ПолучитьЭлементы(),
			 Новый Структура("Объект", ТекущийОбъектВДереве),
			 Истина);
...
Процедура КомандаЗаполнитьПоУмолчанию(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("КомандаЗаполнитьПоУмолчаниюЗавершение", ЭтотОбъект,
		 ТекущиеДанныеКорень);
...
Функция ПолучитьШрифтПоУмолчанию(ЭтоСсылка = Ложь)
	...
	Возврат Новый Шрифт("Arial", 13, Истина, Ложь, Ложь, Ложь);
КонецФункции
...
Функция ПолучитьСписокОбъектов(ОграничениеТипов = Неопределено)
	...
	Если ОграничениеТипов = Неопределено Тогда
		Для Каждого МдОбъект Из Метаданные.Документы Цикл
			Если Не ПравоДоступа("Чтение", МдОбъект) Тогда
				Продолжить;
			КонецЕсли;
	...
	Иначе
		Для Каждого Тип Из ОграничениеТипов.Типы() Цикл
			...
			Список.Добавить(CRM_ОбщегоНазначенияСервер.НастройкиПолейОтображенияСодержанияСсылочныйТипСтрокой(Тип),
				 МдОбъект.Представление());
		КонецЦикла;
...
&НаСервере
Функция ДобавитьОбъектВДерево(Корень, ТипОбъектаСтрокой, Представление = Неопределено,
	 СохраненноеТекущееДеревоНастроек = Неопределено)
	Попытка
		МетаданныеОбъекта = Метаданные.НайтиПоТипу(Тип(ТипОбъектаСтрокой));
	...
	Попытка
		РеквизитыАдресации = МетаданныеОбъекта.РеквизитыАдресации;
	...
	Если бЕстьРеквизитПартнер Тогда
		Если ДобавленныеРеквизиты = Неопределено
			 Или ДобавленныеРеквизиты.Найти("Партнеры_СегментКлиентов") = Неопределено Тогда
			НоваяСтрока = КореньОбъекта.Строки.Добавить();
	...
	Для Каждого ТабЧасть Из МетаданныеОбъекта.ТабличныеЧасти Цикл
		...
		Попытка
			СтандартныеРеквизиты = ТабЧасть.СтандартныеРеквизиты;
		
		Исключение	СтандартныеРеквизиты = Неопределено;
		КонецПопытки;
		Если СтандартныеРеквизиты <> Неопределено Тогда
			Для Каждого Реквизит Из СтандартныеРеквизиты Цикл
				Если ДобавленнаяСтрокаТЧ <> Неопределено И ДобавленнаяСтрокаТЧ.Строки.Найти(Реквизит.Имя,
				...
				НоваяСтрока.Представление				= ?(ЗначениеЗаполнено(Реквизит.Представление()),
					 Реквизит.Представление(),
					 Реквизит.Имя);
				НоваяСтрока.ЭтоСсылка					=
					CRM_ОбщегоНазначенияСервер.НастройкиПолейОтображенияСодержанияРеквизитЭтоСсылка(Реквизит);
			КонецЦикла;
		КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьДеревоОбъекты(ОграничениеТипов = Неопределено, ОчищатьДерево = Истина,
	 ПолучитьТаблицуНастроекИзВременногоХранилищаПоАдресу = Неопределено)
	ДеревоОбъектыЭлементы = ДеревоОбъекты.ПолучитьЭлементы();
	...
	ОбъектыВерхнийУровень.Добавить("БизнесПроцессСсылка",	НСтр("ru='Бизнес-процессы'"), ,
	...
	Для Каждого ДобавляемыйОбъект Из ДобавляемыеОбъекты Цикл
		...
		Для Каждого СтрокаДерева Из Дерево.Строки Цикл
			Если СтрНайти(ДобавляемыйОбъект.Значение, СтрокаДерева.Объект + ".") > 0 Тогда
		...
		Если СтрокаНастройки <> Неопределено
			 И ТипЗнч(СтрокаНастройки.ДеревоНастроек) = Тип("ДеревоЗначений")
		...
		КореньОбъекта = ДобавитьОбъектВДерево(НайденнаяСтрока, ДобавляемыйОбъект.Значение,
			 ДобавляемыйОбъект.Представление,
...
&НаСервере
Функция ПроверитьСуществованиеЭлементаОформленияЦвета(ПроверяемыйЦвет, ИмяЭлементаОформления,
	 ИмяОформляемогоПоля,
	 ИмяПоляЭлемнтаОтбора)
	бЭлементОформленияНайден = Ложь;
	
	Для Каждого ЭлементУсловногоОформления Из УсловноеОформление.Элементы Цикл
		
		Если ЭлементУсловногоОформления.Использование
			 И ЭлементУсловногоОформления.РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный Тогда
		
			Для Каждого ЭлементОформления Из ЭлементУсловногоОформления.Оформление.Элементы Цикл
				
				Если ЭлементОформления.Использование
					 И ЭлементОформления.Параметр = Новый ПараметрКомпоновкиДанных(ИмяЭлементаОформления)
					 И ЭлементОформления.Значение = ПроверяемыйЦвет Тогда
						...
						Если ОформляемоеПоле.Использование И ОформляемоеПоле.Поле = Новый ПолеКомпоновкиДанных(ИмяОформляемогоПоля) Тогда
							...
							Для Каждого ЭлементОтбора Из ЭлементУсловногоОформления.Отбор.Элементы Цикл
								...
								Если		ЭлементОтбора.Использование
										...
										И	ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ДеревоОтображаемыеРеквизиты." 
											+ ИмяПоляЭлемнтаОтбора)
...
Процедура ОбработкаОбновленияДереваОтображаемыхРеквизитовСервер()
	...
	ДополнительныеПараметры = Новый Структура("ШиринаЛевойКолонки,ШиринаЛевойКолонкиАбсолютная,
		|ШиринаПравойКолонки,ШиринаПравойКолонкиАбсолютная",
		ШиринаЛевойКолонки, ШиринаЛевойКолонкиАбсолютная, ШиринаПравойКолонки, ШиринаПравойКолонкиАбсолютная);
	
	Результат = CRM_ОбщегоНазначенияСервер.НастройкиПолейОтображенияСодержанияПолучитьСодержание(Дерево, ,
...
Процедура ЗаполнитьНастройкиПоУмолчанию(ЗаполнитьДляОбъекта)
	...
	ТаблицаНастройки = CRM_ОбщегоНазначенияСервер.НастройкиПолейОтображенияСодержанияПолучитьНастройки(ЗаполнитьДляОбъекта,
		 Истина);
...
Процедура ЗаполнитьНовуюСтрокуРеквизитаПоУмолчанию(СтрокаРеквизита)
	...
	СтрокаРеквизита.ЦветФона	= Новый Цвет(250, 250, 250);
	СтрокаРеквизита.ЦветТекста	= Новый Цвет(0, 0, 0);
КонецПроцедуры
...
Функция ПолучитьСтрокуТабличнойЧастиВОтображаемыхРеквизитах(КореньТекущихДанных, ТекущиеДанные, ЭлементыДерева)
	...
	НайденныеСтроки = CRM_ОбщегоНазначенияКлиентСервер.НайтиВЭлементахДерева(ЭлементыДерева,
		 Новый Структура("Реквизит,ЭтоТабличнаяЧасть", КореньТЧ.Объект,
		 Истина));
...
&НаКлиенте
Процедура ДобавитьСтрокиВОтображаемыхРеквизитах(КореньТекущихДанных, ТекущиеДанные,
	 ЭлементыДляДобавления, ДобавляемыеДанные,
	 ЭтоРеквизитТабличнойЧасти)
	Если КореньТекущихДанных = Неопределено Или ТекущиеДанные = Неопределено
...
Процедура ДобавитьРеквизитВДеревоОтображаемыхРеквизитов(ТекущиеДанные)
	...
	Если КореньТекущихДанныхЭлементы.Количество() > 0 Тогда
		...
		ИначеЕсли ПоследнийЭлемент.ЭтоТабличнаяЧасть И ТекущиеДанные.ЭтоРеквизитТабличнойЧасти
	...
	Если ТекущиеДанные.ЭтоТабличнаяЧасть Тогда
		...
		ДобавитьСтрокиВОтображаемыхРеквизитах(КореньТекущихДанных, ТекущиеДанные,
			 СтрокаТабличнаяЧасть.ПолучитьЭлементы(), ТекущиеДанные.ПолучитьЭлементы(),
			 Истина);
	...
	ИначеЕсли ТекущиеДанные.ЭтоРеквизитТабличнойЧасти Тогда
		...
		ДобавитьСтрокиВОтображаемыхРеквизитах(КореньТекущихДанных, ТекущиеДанные,
			 СтрокаТабличнаяЧасть.ПолучитьЭлементы(), ТекущиеДанные,
			 Истина);
		
	Иначе
		ДобавитьСтрокиВОтображаемыхРеквизитах(КореньТекущихДанных, ТекущиеДанные,
			 ДеревоОтображаемыеРеквизиты.ПолучитьЭлементы(), ТекущиеДанные,
			 Ложь);
		
	КонецЕсли;
	
	Если бЭтоПоследнийЭлемент Или ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() Тогда
		Если КореньТекущихДанныхЭлементы.Количество() > 0 Тогда
...
Функция ПолучитьСтрокуТабличнойЧастиВДеревеОбъектов(КореньОбъекта, ТекущиеДанные)
	...
	НайденныеСтроки = CRM_ОбщегоНазначенияКлиентСервер.НайтиВЭлементахДерева(ЭлементыДерева,
		 Новый Структура("Объект,ЭтоТабличнаяЧасть", КореньТЧ.Реквизит,
		 Истина));
	...
	Если НайденныеСтроки.Количество() = 0 Тогда
		...
		НоваяСтрока.Представление		= ?(ЗначениеЗаполнено(КореньТЧ.ОригинальноеПредставлениеРеквизита),
			 КореньТЧ.ОригинальноеПредставлениеРеквизита,
			 КореньТЧ.ПредставлениеРеквизита);
...
&НаКлиенте
Процедура ДобавитьСтрокиВДеревоОбъектов(КореньОбъекта, ТекущиеДанные, ЭлементыДляДобавления,
	 ДобавляемыеДанные,
	 ЭтоРеквизитТабличнойЧасти)
	Если КореньОбъекта = Неопределено Или ТекущиеДанные = Неопределено
		 Или ЭлементыДляДобавления = Неопределено
		 Или ДобавляемыеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	Для нИндекс = 0 По МассивДанных.ВГраница() Цикл
		...
		НоваяСтрока.Представление	= ?(ЗначениеЗаполнено(ДобавляемыйЭлемент.ОригинальноеПредставлениеРеквизита),
			 ДобавляемыйЭлемент.ОригинальноеПредставлениеРеквизита,
			 ДобавляемыйЭлемент.ПредставлениеРеквизита);
		НоваяСтрока.ЭтоСсылка		= ДобавляемыйЭлемент.ЭтоСсылка;
		Если ЭтоРеквизитТабличнойЧасти Тогда
			НоваяСтрока.ЭтоРеквизитТабличнойЧасти	= Истина;
		КонецЕсли;
		...
		Если ДобавляемыйЭлемент = ТекущиеДанные Тогда
			ТекущиеДанные = Неопределено;
		КонецЕсли;
		ЭлементыДляУдаления = ?(ДобавляемыйЭлементРодитель = Неопределено,
			 ДеревоОтображаемыеРеквизиты.ПолучитьЭлементы(),
...
Процедура УдалитьРеквизитИзДереваОтображаемыхРеквизитов(ТекущиеДанные)
	...
	НайденныеСтроки = CRM_ОбщегоНазначенияКлиентСервер.НайтиВЭлементахДерева(ДеревоОбъекты.ПолучитьЭлементы(),
		 Новый Структура("Объект,ЭтоОбъект", ТекущиеДанные.Объект, Истина),
		 Истина);
	Если НайденныеСтроки.Количество() = 0 Тогда
	...
	Иначе
		Если ТекущиеДанные.ЭтоТабличнаяЧасть Тогда
			...
			ДобавитьСтрокиВДеревоОбъектов(КореньОбъекта, ТекущиеДанные,
				 СтрокаТабличнаяЧасть.ПолучитьЭлементы(), ТекущиеДанные.ПолучитьЭлементы(),
				 Истина);
...
Процедура ПриСменеТекущегоОбъекта(Знач НовыйТекущийОбъект, Знач ПредыдущийТекущийОбъект)
	...
	Иначе
		НайденныеСтроки = CRM_ОбщегоНазначенияКлиентСервер.НайтиВЭлементахДерева(ДеревоОбъекты.ПолучитьЭлементы(),
			 Новый Структура("Объект", НовыйТекущийОбъект),
			 Истина);
		Если НайденныеСтроки.Количество() > 0 Тогда
	...
	Иначе
		НайденныеСтроки = CRM_ОбщегоНазначенияКлиентСервер.НайтиВЭлементахДерева(ДеревоОбъекты.ПолучитьЭлементы(),
			 Новый Структура("Объект", ПредыдущийТекущийОбъект),
			 Истина);
		Если НайденныеСтроки.Количество() > 0 Тогда
...
Процедура ВыбратьЦвет(ОписаниеОповещенияЗавершение)
	...
	ОткрытьФорму("Справочник.CRM_Категории.Форма.ФормаВыбораЦвета", Новый Структура("ТекущийЦвет", 0),
