...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Если Параметры.Свойство("СвойстваДинамическогоСписка") Тогда
		...
		CRM_ОбщегоНазначенияСервер.ПользовательскиеНастройкиСпискаОбновитьДеревоНастроек(ЭтотОбъект,
...
Процедура ТаблицаОтборПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	Если Не ОтменаРедактирования Тогда
		...
		Если ТекущиеДанные <> Неопределено И Не ТекущиеДанные.ЭтоКорневойЭлемент Тогда
			СохранитьТекущиеНастройкиВоВременномХранилище(ТекущиеДанные.Идентификатор,
				 ПолучитьСтруктуруТекущиеНастройки(ЭтотОбъект));
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ДеревоПользовательскиеНастройкиСпискаПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	CRM_ОбщегоНазначенияКлиентСервер.РазвернутьТаблицуФормы(Элементы.ДеревоПользовательскиеНастройкиСписка,
		НоваяСтрока.ПолучитьРодителя().ПолучитьИдентификатор(), Истина);
...
Процедура ДеревоПользовательскиеНастройкиСпискаПередУдалением(Элемент, Отказ)
	СвойстваДинамическогоСписка = ПолучитьСвойстваДинамическогоСпискаКлиент();
	CRM_ОбщегоНазначенияКлиент.ДеревоПользовательскиеНастройкиСпискаПередУдалением(ЭтотОбъект,
		 СвойстваДинамическогоСписка.ИдентификаторСписка, Элемент,
		 Отказ);
...
Процедура ДеревоПользовательскиеНастройкиСпискаПриАктивизацииСтроки(Элемент)
	Если ПредыдущийТекущийИдентификаторВДереве <> Неопределено Тогда
		...
		Если ПредыдущиеДанные <> Неопределено Тогда
			СохранитьТекущиеНастройкиВоВременномХранилище(ПредыдущиеДанные.Идентификатор,
				 ПолучитьСтруктуруТекущиеНастройки(ЭтотОбъект));
		КонецЕсли;
	КонецЕсли;
	...
	Если ТекущиеДанные.ЭтоКорневойЭлемент Тогда
		CRM_ОбщегоНазначенияКлиентСервер.ПрименитьПользовательскиеНастройкиКДинамическомуСписку(ДинамическийСписок,
			 Неопределено, СвойстваДинамическогоСписка.ИдентификаторСписка, Истина, Истина,
			 Истина);
	...
	Иначе
		...
		Если ТипЗнч(СтруктураНастройки) = Тип("Структура") Тогда
			CRM_ОбщегоНазначенияКлиентСервер.ПрименитьПользовательскиеНастройкиКДинамическомуСписку(ДинамическийСписок,
				 СтруктураНастройки, СвойстваДинамическогоСписка.ИдентификаторСписка, Истина, Истина,
				 Истина);
		Иначе
			CRM_ОбщегоНазначенияКлиентСервер.ПрименитьПользовательскиеНастройкиКДинамическомуСписку(ДинамическийСписок,
				 ТекущиеДанные.Идентификатор, СвойстваДинамическогоСписка.ИдентификаторСписка, Истина, Истина,
				 Истина);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Функция ПолучитьСтруктуруТекущиеНастройки(Форма)
	Возврат Новый Структура("Отбор,Группировка,УсловноеОформление", Форма.ДинамическийСписок.Отбор,
		 Форма.ДинамическийСписок.Группировка,
		 Форма.ДинамическийСписок.УсловноеОформление);
КонецФункции
...
Процедура СохранитьНастройки()
	...
	Для Каждого СтрокаНастройка Из СтрокиНастроек Цикл
		Если Не ЗначениеЗаполнено(СтрокаНастройка.Идентификатор) Тогда
			Продолжить;
		КонецЕсли;
		Если Не ЭтоАдресВременногоХранилища(СтрокаНастройка.Адрес) Тогда
			Продолжить;
		КонецЕсли;
		...
		CRM_ОбщегоНазначенияСервер.СохранитьПользовательскиеНастройкиДинамическогоСписка(ИдентификаторСписка,
			 СтрокаНастройка.Идентификатор,
			 СтруктураНастройки);
	КонецЦикла;
	...
	Если ЭтоАдресВременногоХранилища(АдресВоВременномХранилище) Тогда
		Попытка
			...
			Если ТипЗнч(СохраненныеДанные) = Тип("Структура") Тогда
				...
				Если СохраненныеДанные.Свойство("Группировка")
					 И ТипЗнч(СохраненныеДанные.Группировка) = Тип("ХранилищеЗначения") Тогда
...
Процедура СохранитьТекущиеНастройкиВоВременномХранилище(ИдентификаторНастройки, СтруктураНастройки = Неопределено)
	...
	НайденныеСтроки = CRM_ОбщегоНазначенияКлиентСервер.НайтиВЭлементахДерева(ДеревоПользовательскиеНастройкиСписка.ПолучитьЭлементы(),
		 Новый Структура("Идентификатор", ИдентификаторНастройки),
		 Истина);
...
Процедура СкопироватьНастройкуПоИдентификатору(ИдентификаторИсточник, ИдентификаторПриемник)
	...
	НайденныеСтроки = CRM_ОбщегоНазначенияКлиентСервер.НайтиВЭлементахДерева(ДеревоПользовательскиеНастройкиСписка.ПолучитьЭлементы(),
		 Новый Структура("Идентификатор", ИдентификаторИсточник),
		 Истина);
	Если НайденныеСтроки.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
Процедура СохранитьДеревоПользовательскихНастроекСервер()
	Дерево = РеквизитФормыВЗначение("ДеревоПользовательскиеНастройкиСписка");
	СвойстваДинамическогоСписка = ПолучитьСвойстваДинамическогоСпискаСервер();
	CRM_ОбщегоНазначенияСервер.СохранитьДеревоПользовательскихНастроекДинамическогоСписка(СвойстваДинамическогоСписка.ИдентификаторСписка,
		 Дерево);
КонецПроцедуры
