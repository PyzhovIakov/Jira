...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Запись.ИсходныйКлючЗаписи.Идентификатор = ""
		...
		Если Параметры.Свойство("АдресНастройкаКомбинацииУсловий")
			 И ЗначениеЗаполнено(Параметры.АдресНастройкаКомбинацииУсловий) Тогда
	...
	Элементы.Страницы.ТекущаяСтраница = Элементы["Страница" 
		+ ОбщегоНазначения.ИмяЗначенияПеречисления(Запись.СпособЗаданияУсловия)];
	...
	Элементы.КомпоновщикУсловийНастройкиОтборПравоеЗначение.ОтображениеКнопкиВыбора =
	...
	Элементы.КомпоновщикУсловийНастройкиОтборПравоеЗначение.УстановитьДействие("НачалоВыбора",
		 "КомпоновщикУсловийНастройкиОтборПравоеЗначениеНачалоВыбора");
	Элементы.КомпоновщикУсловийНастройкиОтборПравоеЗначение.УстановитьДействие("ОбработкаВыбора",
		 "КомпоновщикУсловийНастройкиОтборПравоеЗначениеОбработкаВыбора");
...
Процедура ВидСравненияПриИзменении(Элемент)
	...
	ЭлементОтбора =
		Компоновщик.Настройки.Отбор.ПолучитьОбъектПоИдентификатору(Элементы.КомпоновщикНастройкиОтбор.ТекущаяСтрока);
		...
		Если Результат Тогда
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Нельзя добавить условие ""%1"". Произошло зацикливание условий.';
					|en='You can not add condition ""%1"". There was a looping of conditions.'"),
				ДобавленноеУсловие);
...
&НаКлиенте
Процедура КомпоновщикУсловийНастройкиОтборПравоеЗначениеОбработкаВыбора(Элемент, ВыбранноеЗначение,
...
Функция ВыполнитьПоискУсловияВОдномУровнеКомбинацииУсловий(Элементы, ИскомоеУсловие)
	...
	Для Каждого ЭлементОтбора Из Элементы Цикл
		...
		Иначе			
			...
			ИначеЕсли  ЭлементОтбора.ПравоеЗначение <> Неопределено Тогда
				...
				Если НаборУсловий.Количество() > 0 Тогда
					Если НаборУсловий[0].СпособЗаданияУсловия = Перечисления.bpmСпособыЗаданияУсловия.КомбинацияИзДругихУсловий
						И ВыполнитьПоискУсловияВОдномУровнеКомбинацииУсловий(НаборУсловий[0].НастройкаКомбинацииУсловий.Получить().Отбор.Элементы,
			...
			//	И ВыполнитьПоискУсловияВОдномУровнеКомбинацииУсловий(ЭлементОтбора.ПравоеЗначение.НастройкаКомбинацииУсловий.Получить().Отбор.Элементы,
				// ИскомоеУсловие) Тогда
			//		Возврат Истина;
			КонецЕсли;
		КонецЕсли;	
	...
	Для Каждого ЭлементОтбора Из Элементы Цикл
		...
		ИначеЕсли ЭлементОтбора.ПравоеЗначение <> Неопределено Тогда
			...
			ИначеЕсли СпособЗаданияУсловия = Перечисления.bpmСпособыЗаданияУсловия.КомбинацияИзДругихУсловий Тогда
            	ВыполнитьПоискИУдалениеДляОдногоУровня(ЭлементОтбора.ПравоеЗначение.НастройкаКомбинацииУсловий.Получить().Отбор.Элементы,
            		 ИмяУсловия);
			КонецЕсли;
...
Функция ВернутьСтруктруНастроекУсловия()
		...
		СтруктураНастроекУсловия.АдресНастройкаКомбинацииУсловий	= ПоместитьВоВременноеХранилище(КомпоновщикУсловий.ПолучитьНастройки(),
			 Новый УникальныйИдентификатор());
		СтруктураНастроекУсловия.АдресНастройкаУсловия				= ПоместитьВоВременноеХранилище(Компоновщик.ПолучитьНастройки(),
			 Новый УникальныйИдентификатор());
