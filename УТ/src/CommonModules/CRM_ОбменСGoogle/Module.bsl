...
Процедура ЗагрузитьДанныеКалендаря(Календарь, СеансовыеДанные) Экспорт
	...
	Пока ЗначениеЗаполнено(СтраничныйТокен) Или СтраничныйТокен = "" Цикл
		...
		Если СобытияКалендаря["error"] <> Неопределено Тогда
			ВызватьИсключение 
				СтрШаблон(НСтр("ru='Произошла ошибка синхронизации с Google Calendar: %1';
					|en='Synchronization error occurred with Google Calendar: %1'"),
		...
		Если ЗначениеЗаполнено(СобытияКалендаря["nextSyncToken"]) Тогда
			РегистрыСведений.CRM_ИнкрементнаяСинхронизацияGoogle.ЗаписатьСинхроТокен(Календарь,
				 СобытияКалендаря["nextSyncToken"]);
		КонецЕсли;
...
Процедура УдалитьВзаимодействиеИзКалендаря(Календарь, Взаимодействие) Экспорт
	...
	HTTPСоединение.ОтправитьДляОбработки(ПакетныйЗапросHTTP);
...
Функция ДанныеДляИзмененияЗаписиКалендаря(ДанныеВзаимодействия)
	...
	ДанныеДляИзменения["start"]		= ОписаниеДатыСобытия(ДанныеВзаимодействия.ДатаНачала, ДанныеВзаимодействия.НаВесьДень);
	ДанныеДляИзменения["end"]		= ОписаниеДатыСобытия(ДанныеВзаимодействия.ДатаОкончания, ДанныеВзаимодействия.НаВесьДень);
...
Процедура ВыгрузитьДанныеВУзел(Календарь, Узел, ДанныеДляВыгрузки, СеансовыеДанные)
	...
	ТелоПакетногоЗапросаИзменениеДанныхКалендаря = ТелоПакетногоЗапросаИзменениеДанныхКалендаря(Календарь,
		 ВыборкаДанных,
		 ГраницаСекций);
...
Функция ОписаниеДатыСобытия(Знач ДатаВремя, НаВесьДень)
...
Функция ВосстановитьДатуJSON(Свойство, Значение, Параметры) Экспорт
	...
	Исключение
		...
		НСтр("ru='Не удалось прочитать дату JSON для свойства ""%1"": ""%2"".';
			|en='Failed to read date JSON for property ""%1"": ""%2"".'"), 
		Свойство,
		Значение));
...
Функция ВызовМетода(АдресAPI, СеансовыеДанные = Неопределено, HTTPМетод = Неопределено,
