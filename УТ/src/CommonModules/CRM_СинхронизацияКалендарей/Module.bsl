...
Процедура СинхронизироватьКалендарь(ДанныеАвторизации, Знач АдресХранилища = Неопределено,
	...
	Календарь = CRM_ОбщегоНазначенияПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(Пользователь,
	...
	ИначеЕсли СервисКалендарей = ПредопределенноеЗначение("Справочник.CRM_СервисыКалендарей.Яндекс") 
		...
		CRM_ОбменСВнешнимиКалендарями.СинхронизироватьDAVКалендарьДляУчетнойЗаписи(ДанныеАвторизации, Пользователь);
	Иначе
...
Процедура УдалитьВзаимодействиеИзКалендаря(Взаимодействие) Экспорт
	...
	Иначе
		CRM_СинхронизацияКалендарейПереопределяемый.УдалитьВзаимодействиеИзКалендаря(СервисКалендарей,
			 Календарь,
			 Взаимодействие);
	КонецЕсли;
...
Процедура ПрочитатьДанныеАвторизации(СервисКалендарей, ДанныеАвторизации, Пользователь) Экспорт
	...
	ИначеЕсли СервисКалендарей = ПредопределенноеЗначение("Справочник.CRM_СервисыКалендарей.Яндекс") 
		Или СервисКалендарей = ПредопределенноеЗначение("Справочник.CRM_СервисыКалендарей.MailRu") 
		Или СервисКалендарей = ПредопределенноеЗначение("Справочник.CRM_СервисыКалендарей.iCloud") Тогда
		CRM_ОбменСВнешнимиКалендарями.ПрочитатьДанныеАвторизации(СервисКалендарей, ДанныеАвторизации, Пользователь);
	Иначе
		CRM_СинхронизацияКалендарейПереопределяемый.ПрочитатьДанныеАвторизации(СервисКалендарей,
			 ДанныеАвторизации,
			 Пользователь);
	КонецЕсли;
...
Процедура ОбновитьДанныеАвторизации(СервисКалендарей, ДанныеАвторизации, Пользователь) Экспорт
	...
	ИначеЕсли СервисКалендарей = ПредопределенноеЗначение("Справочник.CRM_СервисыКалендарей.Яндекс") 
		Или СервисКалендарей = ПредопределенноеЗначение("Справочник.CRM_СервисыКалендарей.MailRu") 
		Или СервисКалендарей = ПредопределенноеЗначение("Справочник.CRM_СервисыКалендарей.iCloud") Тогда
		CRM_ОбменСВнешнимиКалендарями.ОбновитьДанныеАвторизации(СервисКалендарей, ДанныеАвторизации, Пользователь);
	Иначе
		CRM_СинхронизацияКалендарейПереопределяемый.ОбновитьДанныеАвторизации(СервисКалендарей,
			 ДанныеАвторизации,
			 Пользователь);
	КонецЕсли;
...
Процедура ЗаписатьДанныеАвторизации(СервисКалендарей, ДанныеАвторизации, Пользователь, 
		УчетнаяЗаписьВнешнегоКалендаря = Неопределено, ДанныеВнешнегоКалендаря = Неопределено) Экспорт
	...
	ИначеЕсли СервисКалендарей = ПредопределенноеЗначение("Справочник.CRM_СервисыКалендарей.Яндекс") 
		Или СервисКалендарей = ПредопределенноеЗначение("Справочник.CRM_СервисыКалендарей.MailRu") 
		Или СервисКалендарей = ПредопределенноеЗначение("Справочник.CRM_СервисыКалендарей.iCloud") Тогда
		CRM_ОбменСВнешнимиКалендарями.ЗаписатьДанныеАвторизации(ДанныеАвторизации, Пользователь, 
			СервисКалендарей, УчетнаяЗаписьВнешнегоКалендаря, ДанныеВнешнегоКалендаря);
	Иначе
		CRM_СинхронизацияКалендарейПереопределяемый.ЗаписатьДанныеАвторизации(СервисКалендарей,
			 ДанныеАвторизации,
			 Пользователь);
	КонецЕсли;
...
Процедура УдалитьДанныеАвторизации(СервисКалендарей, Пользователь) Экспорт
	...
	ИначеЕсли СервисКалендарей = ПредопределенноеЗначение("Справочник.CRM_СервисыКалендарей.Яндекс") 
		Или СервисКалендарей = ПредопределенноеЗначение("Справочник.CRM_СервисыКалендарей.MailRu")  
		Или СервисКалендарей = ПредопределенноеЗначение("Справочник.CRM_СервисыКалендарей.iCloud") Тогда
		CRM_ОбменСВнешнимиКалендарями.УдалитьДанныеАвторизации(СервисКалендарей, Пользователь);
	Иначе
...
Функция ДанныеАвторизацииДействительны(СервисКалендарей, ДанныеАвторизации) Экспорт
	...
	Иначе
		Возврат CRM_СинхронизацияКалендарейПереопределяемый.ДанныеАвторизацииДействительны(СервисКалендарей,
			 ДанныеАвторизации);
	КонецЕсли;
...
Процедура ЗаполнитьВзаимодействие(Календарь, ДанныеЗаполнения) Экспорт
	...
	Если ВзаимодействиеОбъект.СтатусВзаимодействия = СтатусОтменено
		 И Не ЗначениеЗаполнено(ВзаимодействиеОбъект.Ссылка) Тогда
		Возврат;
	КонецЕсли;
	...
	Если ВзаимодействиеОбъект.СтатусВзаимодействия = СтатусОтменено
		 Или ВзаимодействиеОбъект.СтатусВзаимодействия = СтатусЗавершено Тогда
	...
	Иначе
		...
		ВзаимодействиеОбъект.НаВесьДень				= Ложь;
	КонецЕсли;
	...
	Для Каждого Участник Из ДанныеЗаполнения.Участники Цикл
		...
		Если ДанныеКонтакта <> Неопределено Тогда
			...
			ИначеЕсли ТипЗнч(ДанныеКонтакта.Контакт) = Тип("СправочникСсылка.CRM_ПотенциальныеКлиенты") Тогда
				НоваяСтрока = СторонниеЛица.Добавить();
				НоваяСтрока.ПотенциальныйКлиент	= ДанныеКонтакта.Контакт;
			Иначе
				Продолжить;
			КонецЕсли;
	...
	Иначе
		ВзаимодействиеОбъект.СторонниеЛица.Загрузить(СторонниеЛица);
	КонецЕсли;
	...
	Если СторонниеЛица.Количество() > 0 Тогда
		...
		Запрос.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	СторонниеЛица.Партнер КАК СтороннееЛицо,
		|	СторонниеЛица.КонтактноеЛицо КАК КонтактноеЛицо
		|ПОМЕСТИТЬ втПартнеры
		|ИЗ
		|	&СторонниеЛица КАК СторонниеЛица
		|ГДЕ
		|	СторонниеЛица.Партнер <> ЗНАЧЕНИЕ(Справочник.Партнеры.ПустаяСсылка)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	СторонниеЛица.ПотенциальныйКлиент КАК СтороннееЛицо
		|ПОМЕСТИТЬ втПотенциальныеКлиенты
		|ИЗ
		|	&СторонниеЛица КАК СторонниеЛица
		|ГДЕ
		|	СторонниеЛица.ПотенциальныйКлиент <> ЗНАЧЕНИЕ(Справочник.CRM_ПотенциальныеКлиенты.ПустаяСсылка)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	втПартнеры.СтороннееЛицо КАК СтороннееЛицо,
		|	втПартнеры.КонтактноеЛицо КАК КонтактноеЛицо
		|ИЗ
		...
		|
		|ВЫБРАТЬ
		|	втПотенциальныеКлиенты.СтороннееЛицо,
		|	ЗНАЧЕНИЕ(Справочник.КонтактныеЛицаПартнеров.ПустаяСсылка)
		|ИЗ
		|	втПотенциальныеКлиенты КАК втПотенциальныеКлиенты
		|
		|УПОРЯДОЧИТЬ ПО
		|	КонтактноеЛицо
		|ИТОГИ
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СтороннееЛицо),
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ КонтактноеЛицо)
		|ПО
		|	ОБЩИЕ";
		
		Запрос.УстановитьПараметр("СторонниеЛица", СторонниеЛица);
		...
		Если Не РезультатЗапроса.Пустой() Тогда
			...
			Если ВыборкаОбщийИтог.СтороннееЛицо = 1 Тогда
				
				ВыборкаДетальныеЗаписи = ВыборкаОбщийИтог.Выбрать();
				ВыборкаДетальныеЗаписи.Следующий(); 
				ВзаимодействиеОбъект.Партнер = ВыборкаДетальныеЗаписи.СтороннееЛицо;
				
				Если ВыборкаОбщийИтог.КонтактноеЛицо = 1 Тогда
					ВзаимодействиеОбъект.КонтактноеЛицо = ВыборкаДетальныеЗаписи.КонтактноеЛицо;
				ИначеЕсли ВыборкаОбщийИтог.КонтактноеЛицо = 2
					И Не ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.КонтактноеЛицо) Тогда
					ВыборкаДетальныеЗаписи.Следующий();
					ВзаимодействиеОбъект.КонтактноеЛицо = ВыборкаДетальныеЗаписи.КонтактноеЛицо;
				КонецЕсли;
...
Функция ПрочитатьВзаимодействие(Календарь, Взаимодействие) Экспорт
	...
	ДанныеСобытия = РегистрыСведений.CRM_СобытияКалендаря.ИдентификаторыПоСобытиюКалендаря(Взаимодействие.Ссылка,
		 Календарь.Пользователь);
	...
	Если Взаимодействие.ВидВзаимодействия = Справочники.CRM_ВидыВзаимодействий.Встреча Тогда
		...
		Для Каждого ДанныеУчастника Из Взаимодействие.СвоиЛица Цикл
			...
			Если ЗначениеЗаполнено(УчастникАдрес) Тогда
				НовыйУчастник = Новый Структура;
				НовыйУчастник.Вставить("УчастникИмя", ?(ЗначениеЗаполнено(ДанныеУчастника.Лицо),
					 Строка(ДанныеУчастника.Лицо),
					 ДанныеУчастника.Адрес));
		...
		Для Каждого ДанныеУчастника Из Взаимодействие.СторонниеЛица Цикл
			...
			Если ЗначениеЗаполнено(УчастникАдрес) Тогда
				НовыйУчастник = Новый Структура;
				НовыйУчастник.Вставить("УчастникИмя", ?(ЗначениеЗаполнено(ДанныеУчастника.КонтактноеЛицо),
					 Строка(ДанныеУчастника.КонтактноеЛицо),
					 Строка(ДанныеУчастника.Партнер)));
				НовыйУчастник.Вставить("УчастникАдрес", УчастникАдрес);
				НовыйУчастник.Вставить("УчастникОтклик", ДанныеУчастника.ПосетитМероприятие);
				ДанныеВзаимодействия.Участники.Добавить(НовыйУчастник);
			КонецЕсли;
...
Функция ПолучитьЦветКалендаря(Пользователь = Неопределено, ИндексЦвета = Неопределено) Экспорт
	...
	КонецЕсли;		
	КалендарьПользователя = CRM_ОбщегоНазначенияПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(Пользователь,
		 "КалендарьДляСинхронизации");
	Если НЕ ЗначениеЗаполнено(КалендарьПользователя) Тогда
		Возврат ЦветФона;
	КонецЕсли;
	Если ПустаяСтрока(КалендарьПользователя.backgroundColor) Тогда
		Возврат ЦветФона;
	КонецЕсли;
	СтрокаЦвета	= КалендарьПользователя.backgroundColor;
	Если НЕ (СтрДлина(СтрокаЦвета) = 7) Тогда
		Возврат ЦветФона;
	КонецЕсли;
	Возврат Новый Цвет(ЧислоИзШестнадцатеричнойСтроки("0x" + Сред(СтрокаЦвета, 2, 2 )),
		 ЧислоИзШестнадцатеричнойСтроки("0x" + Сред(СтрокаЦвета, 4, 2 )),
		 ЧислоИзШестнадцатеричнойСтроки("0x" + Сред(СтрокаЦвета, 6,
		 2 )));
КонецФункции
...
Процедура ЗаданиеСинхронизироватьКалендарь() Экспорт
	...
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.CRM_СинхронизацияКалендарей);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не CRM_СинхронизацияКалендарейКлиентСервер.ДанныеАвторизацииЗаполнены(Выборка.СервисКалендарей,
			 ДанныеАвторизации) Тогда
			Продолжить;
		КонецЕсли;
...
Функция КартинкаСервисаКалендарей(СервисКалендарей) Экспорт
	...
	ИначеЕсли СервисКалендарей = Справочники.CRM_СервисыКалендарей.Яндекс Тогда
		Возврат БиблиотекаКартинок.CRM_СервисЯндекс;
	ИначеЕсли СервисКалендарей = Справочники.CRM_СервисыКалендарей.MailRu Тогда
		Возврат БиблиотекаКартинок.CRM_СервисMailRu;
	ИначеЕсли СервисКалендарей = Справочники.CRM_СервисыКалендарей.iCloud Тогда
		Возврат БиблиотекаКартинок.CRM_СервисiCloud;
	Иначе
...
Процедура УдалитьУзелПланаОбмена(Календарь) Экспорт
	
	УзелСсылка = ПланыОбмена.CRM_СинхронизацияКалендарей.УзелДляКалендаря(Календарь);
...
Функция КонтактПоEMail(EMail)
	...
	|	НайденныеКонтакты.CRM_Состояние";
...
Функция EmailКонтакта(Контакт)
	...
	Если ТипЗнч(Контакт) = Тип("СправочникСсылка.Пользователи") Тогда
		ВидКонтактнойИнформации =
			CRM_ОбщегоНазначенияПовтИсп.ВидEMailДляОповещений(Справочники.ВидыКонтактнойИнформации.СправочникПользователи);
	ИначеЕсли ТипЗнч(Контакт) = Тип("СправочникСсылка.ФизическиеЛица") Тогда
		ВидКонтактнойИнформации =
			CRM_ОбщегоНазначенияПовтИсп.ВидEMailДляОповещений(Справочники.ВидыКонтактнойИнформации.СправочникФизическиеЛица);
	ИначеЕсли ТипЗнч(Контакт) = Тип("СправочникСсылка.КонтактныеЛицаПартнеров") Тогда
		ВидКонтактнойИнформации =
			CRM_ОбщегоНазначенияПовтИсп.ВидEMailДляОповещений(Справочники.ВидыКонтактнойИнформации.СправочникКонтактныеЛицаПартнеров);
	ИначеЕсли ТипЗнч(Контакт) = Тип("СправочникСсылка.Партнеры") Тогда
		ВидКонтактнойИнформации =
			CRM_ОбщегоНазначенияПовтИсп.ВидEMailДляОповещений(Справочники.ВидыКонтактнойИнформации.СправочникПартнеры);
	Иначе
		Возврат "";
	КонецЕсли;
