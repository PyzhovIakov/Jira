...
Процедура ЗаписатьРеквизитыОбъектаАнкетирования(Анкета) Экспорт
	...
	Для Каждого СтрокаСостав Из Анкета.Состав Цикл
		...
		Если ЗначениеЗаполнено(СтрокаСостав.ЭлементарныйВопрос) Тогда
			...
			Если СтрНайти(ИмяРеквизита, "ДополнительныйРеквизит_") = 1 Тогда
				
				// дополнительный реквизит.
				
				СтрокаТаблицаРеквизитовОбъекта.ТипОтвета = "ДР";
				
			ИначеЕсли СтрНайти(ИмяРеквизита, "КонтактнаяИнформация_") = 1 Тогда
	...
	Если ТаблицаРеквизитовОбъекта.Количество() > 0 Тогда
		...
		Для Каждого СтрокаТаблицаРеквизитовОбъекта Из МассивСтрокРеквизитов Цикл
			...
			Если СтрокаТаблицаРеквизитовОбъекта.ИмяСправочника = ИмяСправочникаМетаданные Тогда
				...
				Попытка
					...
					ОбъектАнкетирования[СтрокаТаблицаРеквизитовОбъекта.ИмяРеквизита] =
						СтрокаТаблицаРеквизитовОбъекта.ЗначениеРеквизита;
		...
		Для Каждого СтрокаТаблицаРеквизитовОбъекта Из МассивСтрокРеквизитов Цикл
			...
			Если СтрокаТаблицаРеквизитовОбъекта.ИмяСправочника = ИмяСправочникаМетаданные Тогда
				...
				Попытка
					...
					Если НЕ ЗначениеЗаполнено(ЗначениеДопРеквизита)
						 ИЛИ (ТипЗнч(ЗначениеДопРеквизита) = Тип("Булево")
						 И ЗначениеДопРеквизита = Ложь) Тогда
					...
					Иначе						
						...
						Если УправлениеСвойствамиСлужебный.ИспользоватьНеограниченнуюСтроку(Свойство.ТипЗначения,
							 Свойство.МногострочноеПолеВвода) Тогда
		...
		Попытка
			...
			СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения,
				 ПервыйПараметр,
				 ОбъектАнкетирования.Наименование);
		...
		Если ИмяСправочника = ИмяСправочникаМетаданные Тогда
			...
			ИначеЕсли СтрНайти(ИмяРеквизита, "КонтактнаяИнформация_") = 1 Тогда
				...
				Если ИмяСправочника = "Партнеры" Тогда
					...
					ГруппаВидовКИ.Добавить(Справочники.ВидыКонтактнойИнформации["CRM_Справочник" 
						+ ИмяСправочникаМетаданные 
						+ "Компания"]);
					ГруппаВидовКИ.Добавить(Справочники.ВидыКонтактнойИнформации["CRM_Справочник" 
						+ ИмяСправочникаМетаданные 
						+ "ЧастноеЛицо"]);
