...
Процедура ТегСоздание(Элемент, СтандартнаяОбработка, ТекущееПодразделение) Экспорт
	...
	ОткрытьФорму("Справочник.CRM_Теги.ФормаОбъекта", ПарамФормы, Элемент);
КонецПроцедуры
...
//
Функция СозданыОтборНажатие(Форма, ИмяСписка, ВариантПериода, Элемент = Неопределено, ПолеДата = "",
	 ВключатьПросроченные = Ложь) Экспорт
	...
	Если Элемент <> Неопределено Тогда
		...
		Для Индекс = 0 По Форма.ОтборСозданы.Количество() - 1 Цикл
			...
			Если ЭлементСпискаПериодов.Пометка Тогда
				Если ТипЗнч(ЭлементОтображенияПериода) <> Тип("ПолеФормы") Тогда
					...
					// Цвета стиля недоступны на клиенте
		...
		СформироватьЗаголовокВариантаОтбора(Форма.Элементы.ОтборПериод,
			 ОбщегоНазначенияКлиентСервер.ОтмеченныеЭлементы(Форма.ОтборСозданы).Количество());
		
	КонецЕсли;
	
	ГруппаОтбораПериод = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(Форма[ИмяСписка].Отбор.Элементы,
		 "ОтборПериод",
...
Процедура ОчиститьОтборНажатие(Форма, ИмяСписка, ПолеДата = "") Экспорт
	...
	Если ПолеДата = "" Тогда
		Если Форма[ИмяСписка].Отбор.ДоступныеПоляОтбора.Элементы.Найти("Дата") = Неопределено Тогда
			ОбщегоНазначенияКлиентСервер.УдалитьЭлементыГруппыОтбораДинамическогоСписка(Форма[ИмяСписка],
				 "ДатаНачала",
				 "ДатаНачала");
			ОбщегоНазначенияКлиентСервер.УдалитьЭлементыГруппыОтбораДинамическогоСписка(Форма[ИмяСписка],
				 "ДатаОкончания",
...
// Процедура - помещает тег в табличную часть объекта.
//
// Параметры:
//  Форма	 - форма объекта
	...
	Для каждого ЭлементГруппы Из ГруппаФормы.ПодчиненныеЭлементы Цикл
		...
		ИначеЕсли ТипЗнч(ЭлементГруппы) = Тип("ПолеФормы") Тогда
			Попытка
				...
				КонецЕсли;
			Исключение
				Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементГруппы, "ТекстРедактирования")
					И ЗначениеЗаполнено(ЭлементГруппы.ТекстРедактирования) Тогда
					Количество = Количество + 1;
				КонецЕсли;
			КонецПопытки;
		КонецЕсли;
...
// Возвращаемое значение:
//  Булево - включен или выключен отбор по данному элементу.
Функция ОтборПоПеридуНажатие(СписокПериодов, ВариантПериода, ЭлементВариантаОтбора,
	 ГруппаЗначенияОтбораПериод,
	...
	Для каждого ЭлементФормы Из ГруппаЗначенияОтбораПериод.ПодчиненныеЭлементы Цикл
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементФормы, "ЦветФона") Тогда
	...
	СформироватьЗаголовокВариантаОтбора(ЭлементВариантаОтбора,
		 ОбщегоНазначенияКлиентСервер.ОтмеченныеЭлементы(СписокПериодов).Количество());
	
	Возврат Период.Пометка;
	
КонецФункции

Процедура УстановитьОтборПоПериоду(Форма, ИмяСписка, СписокПериодов, ВариантПериода, ПолеДата = "",
	 ВключатьПросроченные = Ложь) Экспорт
