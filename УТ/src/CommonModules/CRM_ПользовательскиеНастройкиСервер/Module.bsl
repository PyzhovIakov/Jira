...
Процедура ДобавитьПодменюПользовательскихНастроек(Форма, Список, КлючОбъектаНастроек) Экспорт
	...
	Для Каждого ЭлементКнопка Из Форма.Элементы.ГруппаСКоммандыНастроек.ПодчиненныеЭлементы Цикл
		Если ЭлементКнопка.Имя = "СписокНастройкаСписка" Тогда
			Продолжить;
		КонецЕсли;
		СписокНаУдаление.Добавить(ЭлементКнопка);
	КонецЦикла;	
	...
	КонецЦикла;
	Сч = 0;
	СписокНастроек = ХранилищеПользовательскихНастроекДинамическихСписков.ПолучитьСписок(КлючОбъектаНастроек,
		 ИмяПользователя());
	Если СписокНастроек.Количество() = 0 Тогда
		...
		ХранилищеПользовательскихНастроекДинамическихСписков.Сохранить(КлючОбъектаНастроек,
			 "Стандартные_Настройки", Список.КомпоновщикНастроек.ПользовательскиеНастройки, ОписаниеНастроек,
			 ИмяПользователя()); 
		СписокНастроек = ХранилищеПользовательскихНастроекДинамическихСписков.ПолучитьСписок(КлючОбъектаНастроек,
			 ИмяПользователя());
	...
	Для Каждого ЭлементНастройки Из СписокНастроек Цикл	
		...
		Если ЭлементНастройки.Значение = "Стандартные_Настройки" Тогда
			НовыйЭлемент = Форма.Элементы.Добавить("Настройка_" + Формат(Сч, "ЧН=; ЧГ="), Тип("КнопкаФормы"),
				 Форма.Элементы.ГруппаСтандартныеНастройки);
			НовыйЭлемент.Заголовок = НСтр("ru='Без отборов';en='No Filters'");
		Иначе	
			НовыйЭлемент = Форма.Элементы.Добавить("Настройка_" + Формат(Сч, "ЧН=; ЧГ="), Тип("КнопкаФормы"),
				 Форма.Элементы.ГруппаКоммандВыбораНастроек);
	...
	// Добавляем элемент "КомандаПредупредить" с типом "Кнопка формы"
	НовыйЭлемент = Форма.Элементы.Добавить("Подключаемый_СохранитьТекущиеНастройки",
		 Тип("КнопкаФормы"),
		 Форма.Элементы.ГруппаСКоммандыНастроек);
	// Присваиваем команду для созданной кнопке
	...
	// Добавляем элемент "КомандаПредупредить" с типом "Кнопка формы"
	НовыйЭлемент = Форма.Элементы.Добавить("Подключаемый_УдалитьТекущуюНастройку", Тип("КнопкаФормы"),
		 Форма.Элементы.ГруппаСКоммандыНастроек);
	// Присваиваем команду для созданной кнопке
...
Функция ПолучитьПредставлениеНастройки(КлючНастройки, КлючОбъектаНастроек) Экспорт
	ОписаниеНастроек = Новый ОписаниеНастроек;
	ХранилищеПользовательскихНастроекДинамическихСписков.Загрузить(КлючОбъектаНастроек,
		 КлючНастройки, ОписаниеНастроек,
		 ИмяПользователя());
...
Процедура УстановитьПользовательскиеНастройки(Список, КлючНастройки, КлючОбъектаНастроек) Экспорт
	ТекущиеНастройкиСписка = ХранилищеПользовательскихНастроекДинамическихСписков.Загрузить(КлючОбъектаНастроек,
		 КлючНастройки, ,
		 ИмяПользователя());
...
Процедура ОбработатьВыборНастройкиНаСервере(НомерНастройки, Список, Форма, КлючОбъектаНастроек) Экспорт
	...
	СписокНастроек = ХранилищеПользовательскихНастроекДинамическихСписков.ПолучитьСписок(КлючОбъектаНастроек,
		 ИмяПользователя());
	Для Каждого ЭлементНастройки Из СписокНастроек Цикл	
		Форма.Элементы["Настройка_" + Формат(Сч,
			 "ЧН=; ЧГ=")].Пометка = ЭлементНастройки.Значение = Форма.ИдентификаторПользовательскойНастройки;
...
Процедура СохранитьТекущиеНастройкиНаСервере(ИмяНастройки, Список, Форма, КлючОбъектаНастроек) Экспорт
	...
	ОписаниеНастроек.КлючОбъекта = КлючОбъектаНастроек;
	ОписаниеНастроек.Пользователь = ИмяПользователя();
	ХранилищеПользовательскихНастроекДинамическихСписков.Сохранить(КлючОбъектаНастроек, ИДНастройки,
		 Список.КомпоновщикНастроек.ПользовательскиеНастройки, ОписаниеНастроек,
		 ИмяПользователя()); 
