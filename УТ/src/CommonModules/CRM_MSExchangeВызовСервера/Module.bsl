...
// BSLLS:LogicalOrInTheWhereSectionOfQuery-off
		...
		|	И (ПапкиЭлектронныхПисем.CRM_ПапкаMSExchange.ID = &ПапкаMSExchangeИлиFolderId
		|		ИЛИ ПапкиЭлектронныхПисем.CRM_ПапкаMSExchange.DistinguishedFolderId = &ПапкаMSExchangeИлиFolderId)
...
Функция ПолучитьУчетнуюЗаписьMSExchangeПоАккаунтуMSExchange(Аккаунт) Экспорт
	...
	ЭтоАдресEMail = (СтрНайти(Аккаунт, "@") > 0);
...
Функция ПолучитьEMailКонтакта(Контакт)
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Контакт, "КонтактнаяИнформация") Тогда
	...
	НайденныеСтроки = КонтактнаяИнформация.НайтиСтроки(Новый Структура("Тип",
...
Функция ПолучитьСинхронизируемыеЗадачи1С(Настройка,
	 МассивIDЗагруженныхОбновленныхЗадач = Неопределено,
	 МассивСсылокЗагруженныхОбновленныхЗадач = Неопределено) Экспорт
	...
	ДатаПоследнейМодификации = Макс(Настройка.ЗадачиМодифицированныеПосле,
		 Настройка.ДатаПоследнегоОбменаЗадач1СMSExchange);
...
Функция ПолучитьСинхронизируемыеЗадачиMSExchange(Настройка, Подключение, СтрокаОшибки = "",
	 МассивIDЗагруженныхОбновленныхЗадач = Неопределено) Экспорт
	Если МассивIDЗагруженныхОбновленныхЗадач = Неопределено Тогда
		МассивIDЗагруженныхОбновленныхЗадач = Новый Массив();
	КонецЕсли;
	...
	ДатаПоследнейМодификации = Макс(Настройка.ЗадачиМодифицированныеПосле,
		 Настройка.ДатаПоследнегоОбменаЗадачMSExchange1С);
	Если ЗначениеЗаполнено(ДатаПоследнейМодификации) Тогда
		ДатаВФорматеMSExchange = CRM_MSExchangeСерверЭкспортныеМетоды.Дата1СВДатуMsExchange(ДатаПоследнейМодификации,
			 Настройка.СмещениеВремениВМинутах);
	...
	Если ТипЗнч(ТаблицаID) = Тип("ТаблицаЗначений") Тогда
		Если ТаблицаID.Количество() > 0 Тогда
			...
			Если ТаблицаID.Количество() > 0 Тогда
				...
				Таблица = CRM_MSExchangeСерверЭкспортныеМетоды.ПолучитьЗадачу(Подключение, МассивID, ,
					 "AllProperties",
					 СтрокаОшибки);
...
Функция СформироватьДанныеДляСозданияОбновленияЗадачи(Настройка, Задача, ДляОбновления = Ложь)
	...
	CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(ДанныеЗадачи.Строки,
		 "t:Subject",
		 Задача.Наименование);
	...
	Если ЗначениеЗаполнено(Задача.ДатаИсполнения) Тогда
		CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(ДанныеЗадачи.Строки,
			 "t:CompleteDate", CRM_MSExchangeСерверЭкспортныеМетоды.Дата1СВДатуMsExchange(Задача.ДатаИсполнения,
			 Настройка.СмещениеВремениВМинутах));
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Задача.СрокИсполнения) Тогда
		CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(ДанныеЗадачи.Строки,
			 "t:DueDate", CRM_MSExchangeСерверЭкспортныеМетоды.Дата1СВДатуMsExchange(Задача.СрокИсполнения,
			 Настройка.СмещениеВремениВМинутах));
	КонецЕсли;
	
	CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(ДанныеЗадачи.Строки,
		 "t:PercentComplete", Формат(Задача.CRM_ПроцентВыполненияЗадачи,
		 "ЧН=0; ЧГ="));
	
	CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(ДанныеЗадачи.Строки,
		 "t:StartDate", CRM_MSExchangeСерверЭкспортныеМетоды.Дата1СВДатуMsExchange(Задача.ДатаНачала,
		 Настройка.СмещениеВремениВМинутах));
...
Функция ЗагрузитьОбновитьЗадачуВMSExchange(Настройка, Подключение, СтрокаТаблицы, СтрокаОшибки = "",
	 ДатаВремяНачалаОбмена = Неопределено)
	...
	Если ЗначениеЗаполнено(СтрокаТаблицы.ID) Тогда
		Если СтрокаТаблицы.Задача.ПометкаУдаления И Настройка.УдалятьПомеченныеНаУдаленияЗадачи Тогда
			Если CRM_MSExchangeСерверЭкспортныеМетоды.УдалитьЭлемент(Подключение, СтрокаТаблицы.ID) Тогда
				CRM_MSExchangeСерверЭкспортныеМетоды.УдалитьСоответствиеОбъектовMSExchange(Настройка.УчетнаяЗапись,
		...
		Иначе
			...
			Если ТипЗнч(ОбъектMSExchange) = Тип("ТаблицаЗначений") И ОбъектMSExchange.Количество() > 0 Тогда
				Если Не Настройка.ЗадачиОбмениватьсяТолькоНовымиОбъектами Тогда
						...
						СтрокаТаблицы.ДанныеОбновления = СформироватьДанныеДляСозданияОбновленияЗадачи(Настройка,
							 СтрокаТаблицы.Задача,
							 Истина);
			...
			Иначе
				...
				Если Не СтрокаТаблицы.Задача.ПометкаУдаления Тогда
					...
					Если Результат = Неопределено Тогда
						ДобавитьВСтрокуОшибки(СтрокаОшибки,
							 НСтр("ru='Ошибка создания/обновления задачи в MS Exchange: ';
							|en='Error of create/update tasks in MS Exchange: '") 
							+ СтрокаТаблицы.Задача.Наименование);
					Иначе
						CRM_MSExchangeСерверЭкспортныеМетоды.ЗаписатьСоответствиеОбъектовMSExchange(Настройка.УчетнаяЗапись,
							 СтрокаТаблицы.Задача, Результат[0].ID, Результат[0].ChangeKey,
							 ?(ЗначениеЗаполнено(ДатаВремяНачалаОбмена), ДатаВремяНачалаОбмена - 1,
							 Неопределено));
...
Функция ЗагрузитьОбновитьЗадачиВMSExchange(Настройка, Подключение, Данные, СтрокаОшибки = "",
	 ДатаВремяНачалаОбмена = Неопределено) Экспорт
	МассивID = Новый Массив();
	Если НЕ ТипЗнч(Данные) = Тип("ТаблицаЗначений") Тогда
		Возврат МассивID;
	КонецЕсли;
	Для Каждого СтрокаТаблицы Из Данные Цикл
		IDСозданногоИзмененногоОбъекта = ЗагрузитьОбновитьЗадачуВMSExchange(Настройка, Подключение,
			 СтрокаТаблицы, СтрокаОшибки,
			 ДатаВремяНачалаОбмена);
...
Функция ЗагрузитьОбновитьЗадачуВ1С(Настройка, СтрокаТаблицы, СтрокаОшибки,
	 МассивСсылокЗагруженныхОбновленныхЗадач,
	 ДатаВремяНачалаОбмена = Неопределено)
	...
	ЗадачаСсылка = CRM_MSExchangeСерверЭкспортныеМетоды.ПолучитьОбъект1СПоID(Настройка.УчетнаяЗапись,
		 СтрокаТаблицы.ID,
		 ChangeKey);
	...
	Попытка
		ЗадачаЗавершена = Булево(СтрокаТаблицы.IsComplete);
	
	Исключение	ЗадачаЗавершена = Ложь;
	КонецПопытки;
	
	Попытка
		Status = СтрокаТаблицы.Status;
	...
	Попытка
		DueDate = СтрокаТаблицы.DueDate;
	
	Исключение	DueDate = Неопределено;
	КонецПопытки;
	
	Попытка
		CompleteDate = СтрокаТаблицы.CompleteDate;
	
	Исключение	CompleteDate = Неопределено;
	КонецПопытки;
	
	СтруктураЗаполнения.Вставить("Дата",
		 CRM_MSExchangeСерверЭкспортныеМетоды.ДатаMSExchangeВДату1С(StartDate,
		 Настройка.СмещениеВремениВМинутах));
	СтруктураЗаполнения.Вставить("ДатаНачала",
		 CRM_MSExchangeСерверЭкспортныеМетоды.ДатаMSExchangeВДату1С(StartDate,
		 Настройка.СмещениеВремениВМинутах));
	Если ЗначениеЗаполнено(DueDate) Тогда
		СтруктураЗаполнения.Вставить("СрокИсполнения",
			 CRM_MSExchangeСерверЭкспортныеМетоды.ДатаMSExchangeВДату1С(DueDate,
			 Настройка.СмещениеВремениВМинутах));
	КонецЕсли;
	Если ЗначениеЗаполнено(CompleteDate) Тогда
		СтруктураЗаполнения.Вставить("ДатаИсполнения",
			 CRM_MSExchangeСерверЭкспортныеМетоды.ДатаMSExchangeВДату1С(CompleteDate,
			 Настройка.СмещениеВремениВМинутах));
	КонецЕсли;
	...
	Если ОбновляетсяСуществующийОбъект И ЗадачаСсылка.CRM_Личная Или Не ОбновляетсяСуществующийОбъект Тогда
		Попытка
			PercentComplete = Число(СтрокаТаблицы.PercentComplete);
	...
	Если ОбновляетсяСуществующийОбъект Тогда
		CRM_MSExchangeСерверЭкспортныеМетоды.ОбновитьChangeKeyВСоответствииОбъектовMSExchange(Настройка.УчетнаяЗапись,
			 ЗадачаСсылка,
			 СтрокаТаблицы.ChangeKey);
	Иначе
		CRM_MSExchangeСерверЭкспортныеМетоды.ЗаписатьСоответствиеОбъектовMSExchange(Настройка.УчетнаяЗапись,
			 ЗадачаСсылка, СтрокаТаблицы.ID, СтрокаТаблицы.ChangeKey,
			 ?(ЗначениеЗаполнено(ДатаВремяНачалаОбмена), ДатаВремяНачалаОбмена - 1,
			 Неопределено));
	КонецЕсли;
...
Функция ЗагрузитьОбновитьЗадачиВ1С(Настройка, Данные, СтрокаОшибки,
	 МассивСсылокЗагруженныхОбновленныхЗадач = Неопределено,
	 ДатаВремяНачалаОбмена = Неопределено) Экспорт
	Если МассивСсылокЗагруженныхОбновленныхЗадач = Неопределено Тогда
		МассивСсылокЗагруженныхОбновленныхЗадач = Новый Массив();
	КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из Данные Цикл
		СсылкаСозданногоИзмененногоОбъекта = ЗагрузитьОбновитьЗадачуВ1С(Настройка, СтрокаТаблицы,
			 СтрокаОшибки, МассивСсылокЗагруженныхОбновленныхЗадач,
			 ДатаВремяНачалаОбмена);
...
Функция ПолучитьСинхронизируемыеКонтакты1С(Настройка,
	 МассивIDЗагруженныхОбновленныхКонтактов = Неопределено,
	 МассивСсылокЗагруженныхОбновленныхКонтактов = Неопределено) Экспорт
	...
	ДатаПоследнейМодификации = Макс(Настройка.КонтактыМодифицированныеПосле,
		 Настройка.ДатаПоследнегоОбменаКонтактов1СMSExchange);
	...
	СоответствиеПараметры.Вставить("МассивСсылокЗагруженныхОбновленныхКонтактов",
		 МассивСсылокЗагруженныхОбновленныхКонтактов);
...
Функция ПолучитьСинхронизируемыеКонтактыMSExchange(Настройка, Подключение, СтрокаОшибки = "",
	 МассивIDЗагруженныхОбновленныхКонтактов = Неопределено) Экспорт
	Если МассивIDЗагруженныхОбновленныхКонтактов = Неопределено Тогда
		МассивIDЗагруженныхОбновленныхКонтактов = Новый Массив();
	КонецЕсли;
	...
	ДатаПоследнейМодификации = Макс(Настройка.КонтактыМодифицированныеПосле,
		 Настройка.ДатаПоследнегоОбменаКонтактовMSExchange1С);
	Если ЗначениеЗаполнено(ДатаПоследнейМодификации) Тогда
		ДатаВФорматеMSExchange = CRM_MSExchangeСерверЭкспортныеМетоды.Дата1СВДатуMsExchange(ДатаПоследнейМодификации,
			 Настройка.СмещениеВремениВМинутах);
	...
	Таблица = CRM_MSExchangeСерверЭкспортныеМетоды.НайтиКонтакты(Подключение, , Ограничение, , ,
		 "AllProperties",
		 СтрокаОшибки);
...
Функция СформироватьДанныеДляСозданияОбновленияКонтакта(Настройка, Контакт, ДляОбновления = Ложь)
	...
	CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(ДанныеКонтакта.Строки,
		 "t:FileAs",
	...
	Если (ТипЗнч(Контакт) = Тип("СправочникСсылка.Партнеры"))
		 И (Контакт.ЮрФизЛицо = Перечисления.КомпанияЧастноеЛицо.Компания) Тогда
		CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(ДанныеКонтакта.Строки,
			 "t:GivenName",
				Контакт.Наименование);
	...
	Иначе	
		...
		CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(ДанныеКонтакта.Строки,
			 "t:MiddleName",
				Контакт.CRM_Отчество);
	КонецЕсли;
	
	Если ТипЗнч(Контакт) = Тип("СправочникСсылка.КонтактныеЛицаПартнеров") Тогда
		CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(ДанныеКонтакта.Строки,
			 "t:CompanyName",
			 Контакт.Владелец.Наименование);
	Иначе
		CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(ДанныеКонтакта.Строки, "t:CompanyName", "");
	КонецЕсли;
	
	// Адреса E-Mail
	СтрокиEMail = Контакт.КонтактнаяИнформация.НайтиСтроки(Новый Структура("Тип",
		 Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты));
	Если СтрокиEMail.Количество() > 0 Тогда
		СтрокаEmailAddresses = CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(ДанныеКонтакта.Строки,
			 "t:EmailAddresses",
			 Неопределено);
		НомерСтроки = 1;
		Для Каждого СтрокаEMail Из СтрокиEMail Цикл
			CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(СтрокаEmailAddresses.Строки, "t:Entry Key=""EmailAddress" + Формат(НомерСтроки, "ЧН=0; ЧГ=") 
				+ """",
	...
	ИначеЕсли ДляОбновления Тогда
		CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(ДанныеКонтакта.Строки,
			 "t:EmailAddresses",
			 CRM_MSExchangeСерверЭкспортныеМетоды.UpdateItemЭтоПолеДляУдаления());
	КонецЕсли;
	
	// Адреса
	Если Настройка.КИСинхронизироватьАдреса Тогда
		СтрокиАдрес = Контакт.КонтактнаяИнформация.НайтиСтроки(Новый Структура("Тип",
		...
		Если СтрокиАдрес.Количество() > 0 Тогда
			...
			Если бДобавлять Тогда
				СтрокаPhysicalAddresses = CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(ДанныеКонтакта.Строки,
					 "t:PhysicalAddresses",
					 Неопределено);
				Для Каждого СтрокаАдрес Из СтрокиАдрес Цикл
					Если ЗначениеЗаполнено(СтрокаАдрес.Представление) Тогда
						ВидАдресаMSExchange = ОпределитьВидАдресаMSExchange(СтрокаАдрес.Вид, Настройка);
						Если ВидАдресаMSExchange <> Неопределено Тогда
							СтрокаEntry = CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(СтрокаPhysicalAddresses.Строки, "t:Entry Key=""" + ВидАдресаMSExchange 
								+ """",
								 Неопределено);
							CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(СтрокаEntry.Строки,
								 "t:Street",
								 СтрокаАдрес.Представление);
						КонецЕсли;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
	...
	Если Настройка.КИСинхронизироватьТелефоны Тогда
		СтрокиТелефон = Контакт.КонтактнаяИнформация.НайтиСтроки(Новый Структура("Тип",
		...
		Если СтрокиТелефон.Количество() > 0 Тогда
			Для Каждого СтрокаТелефон Из СтрокиТелефон Цикл
				...
				Если СтрокаPhoneNumbers = Неопределено Тогда
					СтрокаPhoneNumbers = CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(ДанныеКонтакта.Строки,
						 "t:PhoneNumbers",
						 Неопределено);
				КонецЕсли;
				CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(СтрокаPhoneNumbers.Строки, "t:Entry Key=""" + ВидТелефонаMSExchange 
					+ """",
	...
	Если ЗначениеЗаполнено(ДатаРожденияКонтакта) Тогда
		CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(ДанныеКонтакта.Строки,
			 "t:Birthday", CRM_MSExchangeСерверЭкспортныеМетоды.Дата1СВДатуMsExchange(ДатаРожденияКонтакта,
			 Настройка.СмещениеВремениВМинутах));
	ИначеЕсли ДляОбновления Тогда
		CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(ДанныеКонтакта.Строки,
			 "t:Birthday",
	...
	СтрокиВебСтраница = Контакт.КонтактнаяИнформация.НайтиСтроки(Новый Структура("Тип",
	...
	Если СтрокиВебСтраница.Количество() > 0 Тогда
		Если ЗначениеЗаполнено(СтрокиВебСтраница[0].Представление) Тогда
			CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(ДанныеКонтакта.Строки,
				 "t:BusinessHomePage",
				 СтрокиВебСтраница[0].Представление);
...
Функция ЗагрузитьОбновитьКонтактВMSExchange(Настройка, Подключение, СтрокаТаблицы,
	 СтрокаОшибки = "", ЭтоКонтактыОбъекта = Ложь,
	 ДатаВремяНачалаОбмена = Неопределено)
	IDСозданногоИзмененногоОбъекта = Неопределено;
	Если ЗначениеЗаполнено(СтрокаТаблицы.ID) Тогда
		...
		Иначе
			...
			Если (ТипЗнч(ОбъектMSExchange) = Тип("ТаблицаЗначений")) И (ОбъектMSExchange.Количество() > 0) Тогда
				Если НЕ Настройка.КонтактыОбмениватьсяТолькоНовымиОбъектами Тогда
					...
					Если НЕ ЭтоКонтактыОбъекта Тогда
						...
						КонецЕсли;
						СтрокаТаблицы.ChangeKey = ОбъектMSExchange[0].ChangeKey;
						СтрокаТаблицы.ДанныеОбновления = СформироватьДанныеДляСозданияОбновленияКонтакта(Настройка,
							 СтрокаТаблицы.Контакт,
							 Истина);
			...
			Иначе
				...
				Если НЕ СтрокаТаблицы.Контакт.ПометкаУдаления Тогда
					...
					Если Результат = Неопределено Тогда
						ДобавитьВСтрокуОшибки(СтрокаОшибки,
							 НСтр("ru='Ошибка создания/обновления контакта в MS Exchange: ';
							|en='Error create/update contact in MS Exchange: '") 
							+ СтрокаТаблицы.Контакт.Наименование);
					Иначе
						CRM_MSExchangeСерверЭкспортныеМетоды.ЗаписатьСоответствиеОбъектовMSExchange(Настройка.УчетнаяЗапись,
							 СтрокаТаблицы.Контакт, Результат[0].ID, Результат[0].ChangeKey,
							 ?(ЗначениеЗаполнено(ДатаВремяНачалаОбмена), ДатаВремяНачалаОбмена - 1,
							 Неопределено));
						IDСозданногоИзмененногоОбъекта = Результат[0].ID;
					КонецЕсли;
...
Функция ЗагрузитьОбновитьКонтактыВMSExchange(Настройка, Подключение, Данные, СтрокаОшибки = "",
	 ЭтоКонтактыОбъекта = Ложь,
	 ДатаВремяНачалаОбмена = Неопределено) Экспорт
	МассивID = Новый Массив();
	...
	Для Каждого СтрокаТаблицы Из Данные Цикл
		IDСозданногоИзмененногоОбъекта = ЗагрузитьОбновитьКонтактВMSExchange(Настройка, Подключение,
			 СтрокаТаблицы, СтрокаОшибки, ЭтоКонтактыОбъекта,
			 ДатаВремяНачалаОбмена);
		Если ЗначениеЗаполнено(IDСозданногоИзмененногоОбъекта) Тогда
			МассивID.Добавить(IDСозданногоИзмененногоОбъекта);
		КонецЕсли;
...
Функция ЗагрузитьОбновитьКонтактВ1С(Настройка, СтрокаТаблицы, СтрокаОшибки,
	 МассивСсылокЗагруженныхОбновленныхКонтактов,
	 ДатаВремяНачалаОбмена = Неопределено)
	ChangeKey = Неопределено;
	КонтактСсылка = CRM_MSExchangeСерверЭкспортныеМетоды.ПолучитьОбъект1СПоID(Настройка.УчетнаяЗапись,
		 СтрокаТаблицы.ID,
		 ChangeKey);
	Если ЗначениеЗаполнено(КонтактСсылка)
		 И НЕ (МассивСсылокЗагруженныхОбновленныхКонтактов.Найти(КонтактСсылка) = Неопределено) Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	КонецЕсли;
	Если ЗначениеЗаполнено(КонтактСсылка)
		 И НЕ (ТипЗнч(КонтактСсылка) = Тип("СправочникСсылка.Партнеры"))
		 И НЕ (ТипЗнч(КонтактСсылка) = Тип("СправочникСсылка.КонтактныеЛицаПартнеров")) Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	КонецПопытки;
 	Если (ТипЗнч(CompleteName) = Тип("ТаблицаЗначений")) И (CompleteName.Количество() > 0) Тогда
		...
		// </CompleteName>
		ФИО = CompleteName[0];
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ФИО, "FirstName")
			И ЗначениеЗаполнено(ФИО.FirstName) Тогда
			Имя = ФИО.FirstName;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ФИО, "LastName")
			И ЗначениеЗаполнено(ФИО.LastName) Тогда
			Фамилия = ФИО.LastName;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ФИО, "MiddleName")
			И ЗначениеЗаполнено(ФИО.MiddleName) Тогда
			Отчество = ФИО.MiddleName;
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ФИО, "FullName")
			И ЗначениеЗаполнено(ФИО.FullName) Тогда
	...
	Если НЕ ЗначениеЗаполнено(Фамилия)
		И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаблицы, "Surname") Тогда
		Фамилия = СтрокаТаблицы.Surname;
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(Имя)
		И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаблицы, "GivenName") Тогда
		Имя = СтрокаТаблицы.GivenName;
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(Отчество)
		И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаблицы, "MiddleName") Тогда
		Отчество = СтрокаТаблицы.MiddleName;
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(ПолноеНаименование)
		И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаблицы, "DisplayName") Тогда
		ПолноеНаименование = СтрокаТаблицы.DisplayName;
	КонецЕсли;
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаблицы, "Birthday") Тогда
		ДатаРождения = CRM_MSExchangeСерверЭкспортныеМетоды.ДатаMSExchangeВДату1С(СтрокаТаблицы.Birthday,
			 Настройка.СмещениеВремениВМинутах);
	Иначе
	...
	Если (ТипЗнч(КонтактСсылка) = Тип("СправочникСсылка.Партнеры")
		 ИЛИ ТипЗнч(КонтактСсылка) = Тип("СправочникСсылка.КонтактныеЛицаПартнеров"))
	...
	КонецПопытки;
	Если Настройка.КИСинхронизироватьТелефоны И (ТипЗнч(PhoneNumbers) = Тип("ТаблицаЗначений"))
		 И (PhoneNumbers.Количество() > 0) Тогда
		//		  <t:PhoneNumbers>
		//			<t:Entry Key="BusinessPhone">4255550199</t:Entry>
		//		  </t:PhoneNumbers>
	...
	КонецПопытки;
	Если Настройка.КИСинхронизироватьАдреса И (ТипЗнч(PhysicalAddresses) = Тип("ТаблицаЗначений"))
		 И (PhysicalAddresses.Количество() > 0) Тогда
	...
	Если ОбновляетсяСуществующийОбъект Тогда
		CRM_MSExchangeСерверЭкспортныеМетоды.ОбновитьChangeKeyВСоответствииОбъектовMSExchange(Настройка.УчетнаяЗапись,
			 КонтактОбъект.Ссылка,
			 СтрокаТаблицы.ChangeKey);
	Иначе
		CRM_MSExchangeСерверЭкспортныеМетоды.ЗаписатьСоответствиеОбъектовMSExchange(Настройка.УчетнаяЗапись,
			 КонтактОбъект.Ссылка, СтрокаТаблицы.ID, СтрокаТаблицы.ChangeKey,
			 ?(ЗначениеЗаполнено(ДатаВремяНачалаОбмена), ДатаВремяНачалаОбмена - 1,
			 Неопределено));
	КонецЕсли;
...
Функция ЗагрузитьОбновитьКонтактыВ1С(Настройка, Данные, СтрокаОшибки,
	 МассивСсылокЗагруженныхОбновленныхКонтактов = Неопределено,
	 ДатаВремяНачалаОбмена = Неопределено) Экспорт
	Если МассивСсылокЗагруженныхОбновленныхКонтактов = Неопределено Тогда
		МассивСсылокЗагруженныхОбновленныхКонтактов = Новый Массив();
	КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из Данные Цикл
		...
		Если (ItemClass = Неопределено) ИЛИ (НРег(ItemClass) = "ipm.contact") Тогда
			...
			СсылкаСозданногоИзмененногоОбъекта = ЗагрузитьОбновитьКонтактВ1С(Настройка, СтрокаТаблицы,
				 СтрокаОшибки, МассивСсылокЗагруженныхОбновленныхКонтактов,
				 ДатаВремяНачалаОбмена);
			Если ЗначениеЗаполнено(СсылкаСозданногоИзмененногоОбъекта) Тогда
				МассивСсылок.Добавить(СсылкаСозданногоИзмененногоОбъекта);
			КонецЕсли;
...
Функция ПолучитьУчастниковСобытия1С(СтрокаТаблицы, Настройка, СоответствияАдресовИКонтактов)
	...
	Для НомерСтроки = 1 По 2 Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Возврат Новый Структура("Организатор,Участники", Новый Структура("Участник,Адрес", Организатор,
		 АдресEMailОрганизатора),
		 МассивУчастников);
КонецФункции
...
Функция ПолучитьСинхронизируемыеСобытия1С(Настройка,
	 МассивIDЗагруженныхОбновленныхСобытий = Неопределено,
	 МассивСсылокЗагруженныхОбновленныхСобытий = Неопределено) Экспорт
	...
	ТаблицаСобытий = CRM_ОбщегоНазначенияСервер.ВыполнитьЗапросВКомпоновкеДанных(ТекстЗапроса, Отбор,
		 СоответствиеПараметры);
...
Функция ПолучитьКонтактыСобытий1С(Настройка, СинхронизируемыеСобытия,
	 МассивIDЗагруженныхОбновленныхКонтактов = Неопределено,
	 МассивСсылокЗагруженныхОбновленныхКонтактов = Неопределено) Экспорт
...
Функция ПолучитьСинхронизируемыеСобытияMSExchange(Настройка, Подключение, СтрокаОшибки = "",
	 МассивIDЗагруженныхОбновленныхСобытий = Неопределено) Экспорт
	Если МассивIDЗагруженныхОбновленныхСобытий = Неопределено Тогда
		МассивIDЗагруженныхОбновленныхСобытий = Новый Массив();
	КонецЕсли;
	...
	ДатаПоследнейМодификации = Макс(Настройка.СобытияМодифицированныеПосле,
		 Настройка.ДатаПоследнегоОбменаСобытийMSExchange1С);
	Если ЗначениеЗаполнено(ДатаПоследнейМодификации) Тогда
		ДатаВФорматеMSExchange = CRM_MSExchangeСерверЭкспортныеМетоды.Дата1СВДатуMsExchange(ДатаПоследнейМодификации,
			 Настройка.СмещениеВремениВМинутах);
	...
	Если ТипЗнч(ТаблицаID) = Тип("ТаблицаЗначений") Тогда
		Если ТаблицаID.Количество() > 0 Тогда
			...
			Если ТаблицаID.Количество() > 0 Тогда
				МассивID = ТаблицаID.ВыгрузитьКолонку("ID");
				Таблица = CRM_MSExchangeСерверЭкспортныеМетоды.ПолучитьСобытие(Подключение, МассивID, ,
					 "AllProperties",
					 СтрокаОшибки);
				...
				Если ТипЗнч(Таблица) = Тип("ТаблицаЗначений") И Таблица.Колонки.Найти("CalendarItemType") <> Неопределено Тогда
					...
					нИндекс = 0;
					Пока нИндекс < Таблица.Количество() Цикл
						Если НРег(Таблица[нИндекс].CalendarItemType) = "occurrence"
							 Или НРег(Таблица[нИндекс].CalendarItemType) = "recurringmaster" Тогда
							Таблица.Удалить(нИндекс);
						Иначе
							нИндекс = нИндекс + 1;
						КонецЕсли;
...
Функция СформироватьСписокУчастников(Событие)
	...
	Для Каждого СтрокаТаблицы Из Событие.СторонниеЛица Цикл
		Если ЗначениеЗаполнено(СтрокаТаблицы.Партнер) Или ЗначениеЗаполнено(СтрокаТаблицы.КонтактноеЛицо) Тогда
			МассивСтороннихУчастников.Добавить(Новый Структура("Участник, Контакт",
			СтрокаТаблицы.Партнер, СтрокаТаблицы.КонтактноеЛицо));
		КонецЕсли;
	КонецЦикла;
	СтрУчастники = "";
	...
	Если МассивСтороннихУчастников.Количество() > 0 Тогда
		...
		Для Каждого ЭлементМассива Из МассивСтороннихУчастников Цикл
			СтрУчастники = СтрУчастники + ?(ЗначениеЗаполнено(СтрУчастники), Символы.ПС, "") +
				"  - " + ?(ЗначениеЗаполнено(ЭлементМассива.Контакт), Строка(ЭлементМассива.Контакт) +
				" " + "(" + Строка(ЭлементМассива.Участник) + ")", Строка(ЭлементМассива.Участник));
		КонецЦикла;
	КонецЕсли;
...
Функция СформироватьДанныеДляСозданияОбновленияСобытия(Настройка, Событие, ДляОбновления = Ложь)
	...
	CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(ДанныеСобытия.Строки,
		 "Body BodyType=""Text""",
		 ТекстОписания);
	...
	CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(ДанныеСобытия.Строки,
		 "Start", CRM_MSExchangeСерверЭкспортныеМетоды.Дата1СВДатуMsExchange(Событие.ПлановаяДата,
		 Настройка.СмещениеВремениВМинутах));
	CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(ДанныеСобытия.Строки,
		 "End",   CRM_MSExchangeСерверЭкспортныеМетоды.Дата1СВДатуMsExchange(Событие.ПлановаяДатаЗавершение,
		 Настройка.СмещениеВремениВМинутах));
	
	НаВесьДень = Событие.НаВесьДень;
	CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(ДанныеСобытия.Строки,
		 "IsAllDayEvent", ?(НаВесьДень, "true",
		 "false"));
	
	//Отменено = (Событие.СтатусВзаимодействия = Справочники.CRM_СостоянияСобытий.Отменено);
	//CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(ДанныеСобытия.Строки,
		// "IsCancelled", ?(Отменено, "true",
		// "false"));
	
	Если ЗначениеЗаполнено(Событие.Помещение) Тогда
		CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(ДанныеСобытия.Строки,
			 "Location",
			 Строка(Событие.Помещение));
	...
	Для Каждого СтрокаТаблицы Из Событие.СторонниеЛица Цикл
		Если ЗначениеЗаполнено(СтрокаТаблицы.Адрес) И СтрокаТаблицы.ОтправлятьПоПочте Тогда
			Если ЗначениеЗаполнено(СтрокаТаблицы.КонтактноеЛицо)
				 И (МассивУчастники.Найти(СтрокаТаблицы.КонтактноеЛицо) = Неопределено) Тогда
				МассивУчастники.Добавить(СтрокаТаблицы.КонтактноеЛицо);
			ИначеЕсли ЗначениеЗаполнено(СтрокаТаблицы.Партнер)
				 И (МассивУчастники.Найти(СтрокаТаблицы.Партнер) = Неопределено) Тогда
	...
	Если МассивУчастники.Количество() > 0 Тогда
		...
		СтрокаRequiredAttendees = CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(ДанныеСобытия.Строки,
			 "RequiredAttendees",
			 Неопределено);
		...
		Для Каждого Участник Из МассивУчастники Цикл
			...
			Если ЗначениеЗаполнено(EMailУчастника) Тогда
				СтрокаAttendee = CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(СтрокаRequiredAttendees.Строки,
					 "Attendee",
					 Неопределено);
				
				СтрокаMailbox = CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(СтрокаAttendee.Строки,
					 "Mailbox",
					 Неопределено);
				...
				Если ЗначениеЗаполнено(EMailУчастника) Тогда
					CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(СтрокаMailbox.Строки,
						 "Name",
						 Строка(Участник));
					CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(СтрокаMailbox.Строки,
						 "EmailAddress",
				...
				//CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(СтрокаAttendee.Строки,
					// "ResponseType",
					// "Organizer");
	...
	СмещениеВремениСтрокой =
		CRM_MSExchangeСерверЭкспортныеМетоды.ПолучитьСмещениеВремениСтрокойДляMSExchange(Настройка.СмещениеВремениВМинутах);
	Если ЗначениеЗаполнено(СмещениеВремениСтрокой) Тогда
		СтрокаMeetingTimeZone = CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(ДанныеСобытия.Строки, "MeetingTimeZone TimeZoneName=""(UTC" + СмещениеВремениСтрокой 
			+ ")""",
			 Неопределено);
		CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(СтрокаMeetingTimeZone.Строки,
			 "BaseOffset",
			 CRM_MSExchangeСерверЭкспортныеМетоды.ПолучитьBaseOffsetdurationСтрокой(Настройка.СмещениеВремениВМинутах));
	КонецЕсли;
...
Функция ЗагрузитьОбновитьСобытиеВMSExchange(Настройка, Подключение, СтрокаТаблицы,
	 СтрокаОшибки = "",
	 ДатаВремяНачалаОбмена = Неопределено)
	IDСозданногоИзмененногоОбъекта = Неопределено;
	Если ЗначениеЗаполнено(СтрокаТаблицы.ID) Тогда
		Если (СтрокаТаблицы.Событие.ПометкаУдаления 
			Или СтрокаТаблицы.Событие.СтатусВзаимодействия.ТипСостояния = Перечисления.CRM_ТипыСостоянийСобытий.Отменено)
			И Настройка.УдалятьПомеченныеНаУдаленияСобытия Тогда
		...
		Иначе
			// Здесь не передаем в ф-ю СтрокаОшибки чтобы не плодить ошибки, вида "такого объекта нет", которые могут сбить с
			// толку.
			ОбъектMSExchange = CRM_MSExchangeСерверЭкспортныеМетоды.ПолучитьСобытие(Подключение,
				 СтрокаТаблицы.ID, ,
				 "AllProperties");
			Если (ТипЗнч(ОбъектMSExchange) = Тип("ТаблицаЗначений")) И (ОбъектMSExchange.Количество() > 0) Тогда
				...
				Иначе
					...
					КонецЕсли;
					СтрокаТаблицы.ChangeKey = ОбъектMSExchange[0].ChangeKey;
					СтрокаТаблицы.ДанныеОбновления = СформироватьДанныеДляСозданияОбновленияСобытия(Настройка,
						 СтрокаТаблицы.Событие,
						 Истина);
					...
					ИначеЕсли СтрокаТаблицы.Событие.СтатусВзаимодействия.ТипСостояния = Перечисления.CRM_ТипыСостоянийСобытий.Завершено Тогда
						...
						ОбновленоУспешно = CRM_MSExchangeСерверЭкспортныеМетоды.ОбновитьСобытие(Подключение,
							 СтрокаТаблицы, СтрокаОшибки,
			...
			Иначе
				...
				Если НЕ СтрокаТаблицы.Событие.ПометкаУдаления
					 И (СтрокаТаблицы.Событие.Ответственный = Настройка.УчетнаяЗапись.Пользователь) Тогда
					...
					КонецЕсли;	
					Если Результат = Неопределено Тогда
						ДобавитьВСтрокуОшибки(СтрокаОшибки, НСтр("ru='Ошибка создания/обновления события в MS Exchange: ';en='Error creating/updating events in MS Exchange: '") 
							+ Строка(СтрокаТаблицы.Событие) 
							+ НСтр("ru=' Возможно, у клиента(ов) не указан ни один e-mail.'"));
					Иначе
						CRM_MSExchangeСерверЭкспортныеМетоды.ЗаписатьСоответствиеОбъектовMSExchange(Настройка.УчетнаяЗапись,
							 СтрокаТаблицы.Событие, Результат[0].ID, Результат[0].ChangeKey,
							 ?(ЗначениеЗаполнено(ДатаВремяНачалаОбмена), ДатаВремяНачалаОбмена - 1,
							 Неопределено));
						IDСозданногоИзмененногоОбъекта = Результат[0].ID;
					КонецЕсли;
...
Функция ЗагрузитьОбновитьСобытияВMSExchange(Настройка, Подключение, Данные, СтрокаОшибки = "",
	 ДатаВремяНачалаОбмена = Неопределено) Экспорт
	...
	КонецЕсли;
	Для Каждого СтрокаТаблицы Из Данные Цикл
		IDСозданногоИзмененногоОбъекта = ЗагрузитьОбновитьСобытиеВMSExchange(Настройка, Подключение,
			 СтрокаТаблицы, СтрокаОшибки,
			 ДатаВремяНачалаОбмена);
		Если ЗначениеЗаполнено(IDСозданногоИзмененногоОбъекта) Тогда
			МассивID.Добавить(IDСозданногоИзмененногоОбъекта);
...
Функция ЗагрузитьОбновитьСобытиеВ1С(Настройка, СтрокаТаблицы, СтрокаОшибки,
	 СоответствияАдресовИКонтактов, МассивСсылокЗагруженныхОбновленныхСобытий,
	 ДатаВремяНачалаОбмена = Неопределено)
	// <CalendarItem>
	//   <MimeContent/>
	//   <ItemId/>
	...
	// </CalendarItem>
	ChangeKey = Неопределено;
	СобытиеСсылка = CRM_MSExchangeСерверЭкспортныеМетоды.ПолучитьОбъект1СПоID(Настройка.УчетнаяЗапись,
		 СтрокаТаблицы.ID,
		 ChangeKey);
	...
	Если НЕ ЗначениеЗаполнено(Start) ИЛИ НЕ ЗначениеЗаполнено(End) Тогда
		...
		Возврат Неопределено;
	КонецЕсли;
	ДанныеЗаполнения.Вставить("ПлановаяДата",
		 CRM_MSExchangeСерверЭкспортныеМетоды.ДатаMSExchangeВДату1С(Start,
		 Настройка.СмещениеВремениВМинутах));
	ДанныеЗаполнения.Вставить("ПлановаяДатаЗавершение",
		 CRM_MSExchangeСерверЭкспортныеМетоды.ДатаMSExchangeВДату1С(End,
		 Настройка.СмещениеВремениВМинутах));
	ДанныеЗаполнения.Вставить("Дата",
		 CRM_MSExchangeСерверЭкспортныеМетоды.ДатаMSExchangeВДату1С(DateTimeCreated,
		 Настройка.СмещениеВремениВМинутах));
	Попытка
		Subject = СтрокаТаблицы.Subject;
	Исключение
	...
	Если ЗначениеЗаполнено(Body) Тогда
		...
		Иначе
			ТекстОписания = Body;
		КонецЕсли;
		ПозицияКлиент = СтрНайти(ТекстОписания, ПолучитьОткрывающийТегКлиент());
		Если ПозицияКлиент > 0 Тогда
			СтрКлиент = Сред(ТекстОписания, ПозицияКлиент);
			ПозицияКлиентКонец = СтрНайти(СтрКлиент, ПолучитьЗакрывающийТегКлиент());
			...
			КонецЕсли;
		КонецЕсли;
		ПозицияУчастники = СтрНайти(ТекстОписания, ПолучитьОткрывающийТегУчастниковСобытия());
		Если ПозицияУчастники > 0 Тогда
			СтрУчастники = Сред(ТекстОписания, ПозицияУчастники);
			ПозицияУчастникиКонец = СтрНайти(СтрУчастники, ПолучитьЗакрывающийТегУчастниковСобытия());
	...
	Если ОбновляетсяСуществующийОбъект Тогда
		CRM_MSExchangeСерверЭкспортныеМетоды.ОбновитьChangeKeyВСоответствииОбъектовMSExchange(Настройка.УчетнаяЗапись,
			 СобытиеОбъект.Ссылка,
			 СтрокаТаблицы.ChangeKey);
	Иначе
		CRM_MSExchangeСерверЭкспортныеМетоды.ЗаписатьСоответствиеОбъектовMSExchange(Настройка.УчетнаяЗапись,
			 СобытиеОбъект.Ссылка, СтрокаТаблицы.ID, СтрокаТаблицы.ChangeKey,
			 ?(ЗначениеЗаполнено(ДатаВремяНачалаОбмена), ДатаВремяНачалаОбмена - 1,
			 Неопределено));
	КонецЕсли;
...
Функция ЗагрузитьОбновитьСобытияВ1С(Настройка, Данные, СтрокаОшибки,
	...
	Для Каждого СтрокаТаблицы Из Данные Цикл
		...
		Если ТипЗнч(Organizer) = Тип("ТаблицаЗначений") Тогда
			...
			КонецЕсли;
		КонецЕсли;
		СсылкаСозданногоИзмененногоОбъекта = ЗагрузитьОбновитьСобытиеВ1С(Настройка, СтрокаТаблицы,
			 СтрокаОшибки, СоответствияАдресовИКонтактов, МассивСсылокЗагруженныхОбновленныхСобытий,
			 ДатаВремяНачалаОбмена);
		Если ЗначениеЗаполнено(СсылкаСозданногоИзмененногоОбъекта) Тогда
			МассивСсылок.Добавить(СсылкаСозданногоИзмененногоОбъекта);
		КонецЕсли;
...
Функция СформироватьТекстЗапросаПочтовыеСообщения(Настройка) Экспорт
	...
	Если Не Настройка.СинхронизироватьПочтовыеСообщенияВходящие
		 И Не Настройка.СинхронизироватьПочтовыеСообщенияОтправленные Тогда
	...
	Если Настройка.СинхронизироватьПочтовыеСообщенияВходящие Тогда
		...
		|	ЭлектронноеПисьмоВходящее.Ссылка КАК ЭлектронноеПисьмоВходящее" 
			+ ?(Настройка.СинхронизироватьПочтовыеСообщенияОтправленные, ",", "") + "
...
Функция ПолучитьСинхронизируемыеПочтовыеСообщения1С(Настройка, ТипПочтовыхСообщений,
	 СтрокаОшибки = "", МассивIDЗагруженныхОбновленныхПочтовыхСообщений = Неопределено,
	 МассивСсылокЗагруженныхОбновленныхПочтовыхСообщений = Неопределено) Экспорт
	Если Не Настройка.СинхронизироватьПочтовыеСообщенияВходящие
		 И Не Настройка.СинхронизироватьПочтовыеСообщенияОтправленные Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	СоответствиеПараметры.Вставить("СерверMSExchange", Настройка.УчетнаяЗапись.СерверMSExchange);
	СоответствиеПараметры.Вставить("ДатаПоследнейМодификации", ДатаПоследнейМодификации);
	СоответствиеПараметры.Вставить("МассивIDЗагруженныхОбновленныхПочтовыхСообщений",
		 МассивIDЗагруженныхОбновленныхПочтовыхСообщений);
	СоответствиеПараметры.Вставить("МассивСсылокЗагруженныхОбновленныхПочтовыхСообщений",
		 МассивСсылокЗагруженныхОбновленныхПочтовыхСообщений);
	...
	|///////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ 
	|	ПочтовыеСообщения." + ?(ТипПочтовыхСообщений = "Входящие", "ЭлектронноеПисьмоВходящее",
		 "ЭлектронноеПисьмоИсходящее") 
		+ " КАК ПочтовоеСообщение,
	|	ЕСТЬNULL(СоответствиеОбъектовMSExchange.ID, НЕОПРЕДЕЛЕНО) КАК ID,
	|	ЕСТЬNULL(СоответствиеОбъектовMSExchange.ДатаПоследнейМодификации, ПочтовыеСообщения." 
		+ ?(ТипПочтовыхСообщений = "Входящие", "ЭлектронноеПисьмоВходящее", "ЭлектронноеПисьмоИсходящее") 
		+ ".Дата) КАК ДатаПоследнейМодификации
	|ИЗ
	...
	|		ПочтовыеСообщения." + ?(ТипПочтовыхСообщений = "Входящие", "ЭлектронноеПисьмоВходящее",
		 "ЭлектронноеПисьмоИсходящее") 
		+ " = СоответствиеОбъектовMSExchange.Объект1С
	|		И СоответствиеОбъектовMSExchange.СерверMSExchange = &СерверMSExchange
	|	
	|ГДЕ
	|	ЕСТЬNULL(СоответствиеОбъектовMSExchange.ДатаПоследнейМодификации, ПочтовыеСообщения." 
		+ ?(ТипПочтовыхСообщений = "Входящие", "ЭлектронноеПисьмоВходящее", "ЭлектронноеПисьмоИсходящее") 
		+ ".Дата) >= &ДатаПоследнейМодификации
	|";
...
Функция ПолучитьКонтактыПочтовыхСообщений1С(Настройка, ТипПочтовыхСообщений,
	 СинхронизируемыеПочтовыеСообщения, МассивIDЗагруженныхОбновленныхКонтактов = Неопределено,
	 МассивСсылокЗагруженныхОбновленныхКонтактов = Неопределено) Экспорт
...
// BSLLS:LogicalOrInTheWhereSectionOfQuery-off
	...
	|	ИЗ
	|		Документ_ИМЯ_ДОКУМЕНТА_ПолучателиПисьма КАК ТаблицаКонтакты
	|	ГДЕ
	...
	|		ТаблицаКонтакты.Контакт.ПометкаУдаления КАК КонтактПометкаУдаления
	|	ИЗ
	|		Документ_ИМЯ_ДОКУМЕНТА_ПолучателиКопий КАК ТаблицаКонтакты
	|	ГДЕ
	|		ТаблицаКонтакты.Ссылка В (&МассивПочтовыхСообщений)
	|		И
	|			(ТаблицаКонтакты.Контакт ССЫЛКА Справочник.Партнеры ИЛИ ТаблицаКонтакты.Контакт ССЫЛКА Справочник.КонтактныеЛицаПартнеров)
	...
	|		ТаблицаКонтакты.Контакт.ПометкаУдаления КАК КонтактПометкаУдаления
	|	ИЗ
	|		Документ_ИМЯ_ДОКУМЕНТА_ПолучателиОтвета КАК ТаблицаКонтакты
	|	ГДЕ
	|		ТаблицаКонтакты.Ссылка В (&МассивПочтовыхСообщений)
	|		И
	|			(ТаблицаКонтакты.Контакт ССЫЛКА Справочник.Партнеры ИЛИ ТаблицаКонтакты.Контакт ССЫЛКА Справочник.КонтактныеЛицаПартнеров)
	...
	|	ИЗ
	|		Документ_ИМЯ_ДОКУМЕНТА_ КАК ЭлектронноеПисьма
	|	ГДЕ
...
// BSLLS:LogicalOrInTheWhereSectionOfQuery-on
	...
	ИмяДокумента = ?(ТипПочтовыхСообщений = "Входящие", "ЭлектронноеПисьмоВходящее", "ЭлектронноеПисьмоИсходящее");
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Документ_ИМЯ_ДОКУМЕНТА_ПолучателиПисьма",
		 СтрШаблон("Документ.%1.ПолучателиПисьма",
		 ИмяДокумента));
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Документ_ИМЯ_ДОКУМЕНТА_ПолучателиКопий",
		 СтрШаблон("Документ.%1.ПолучателиКопий",
		 ИмяДокумента));
    	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Документ_ИМЯ_ДОКУМЕНТА_ПолучателиОтвета",
    		 СтрШаблон("Документ.%1.ПолучателиОтвета",
    		 ИмяДокумента));
      	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Документ_ИМЯ_ДОКУМЕНТА_", СтрШаблон("Документ.%1", ИмяДокумента));
...
Функция ПолучитьПапкиСинхронизацииПочтовыхСообщений(Настройка, ТипПочтовыхСообщений)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаПапок Цикл
		Если ЗначениеЗаполнено(СтрокаТаблицы.DistinguishedFolderId) Тогда
			НайденныеСтроки = ТаблицаРезультат.НайтиСтроки(Новый Структура("DistinguishedFolderId",
				 СтрокаТаблицы.DistinguishedFolderId));
			Если НайденныеСтроки.Количество() = 0 Тогда
...
Функция ПолучитьСинхронизируемыеПочтовыеСообщенияMSExchange(Настройка, ТипПочтовыхСообщений,
	 Подключение, СтрокаОшибки = "",
	 МассивIDЗагруженныхОбновленныхПочтовыхСообщений = Неопределено) Экспорт
	Если МассивIDЗагруженныхОбновленныхПочтовыхСообщений = Неопределено Тогда
		МассивIDЗагруженныхОбновленныхПочтовыхСообщений = Новый Массив();
	КонецЕсли;
	...
	Для Каждого СтрокаТаблицыПапка Из ПапкиСинхронизацииПочтовыхСообщений Цикл
		Если ЗначениеЗаполнено(СтрокаТаблицыПапка.DistinguishedFolderId) Тогда
			ТаблицаID = CRM_MSExchangeСерверЭкспортныеМетоды.НайтиПочтовыеСообщения(Подключение, ,
				 Ограничение, , СтрокаТаблицыПапка.DistinguishedFolderId, "IdOnly",
				 СтрокаОшибки);
		ИначеЕсли ЗначениеЗаполнено(СтрокаТаблицыПапка.FolderID) Тогда
			ТаблицаID = CRM_MSExchangeСерверЭкспортныеМетоды.НайтиПочтовыеСообщения(Подключение, ,
				 Ограничение, СтрокаТаблицыПапка.FolderID, , "IdOnly",
				 СтрокаОшибки);
		КонецЕсли;
		
		Если ТипЗнч(ТаблицаID) = Тип("ТаблицаЗначений") И ТаблицаID.Количество() > 0
			 И ТаблицаID.Колонки.Найти("ID") <> Неопределено Тогда
			Для Каждого СтрокаТаблицы Из ТаблицаID Цикл
				Если МассивIDЗагруженныхОбновленныхПочтовыхСообщений.Найти(СтрокаТаблицы.ID) = Неопределено
					 И МассивID.Найти(СтрокаТаблицы.ID) = Неопределено Тогда
	...
	Если МассивID.Количество() = 0 Тогда
		Возврат Новый ТаблицаЗначений();
	Иначе
		Таблица = CRM_MSExchangeСерверЭкспортныеМетоды.ПолучитьПочтовоеСообщение(Подключение, МассивID, ,
			 "AllProperties",
			 СтрокаОшибки);
		Возврат Таблица;
	КонецЕсли;
...
Функция СформироватьДанныеДляСозданияОбновленияПочтовогоСообщения(Настройка, ПочтовоеСообщение, ДляОбновления = Ложь)
	...
	//
	//CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(ДанныеСобытия.Строки,
		// "Start", CRM_MSExchangeСерверЭкспортныеМетоды.Дата1СВДатуMsExchange(Событие.Дата,
		// Настройка.СмещениеВремениВМинутах));
	//CRM_MSExchangeСерверЭкспортныеМетоды.ДобавитьСтрокуДереваСозданияДанных(ДанныеСобытия.Строки,
		// "End", CRM_MSExchangeСерверЭкспортныеМетоды.Дата1СВДатуMsExchange(Событие.ОкончаниеМероприятия,
		// Настройка.СмещениеВремениВМинутах));
	//
	//НаВесьДень = Событие.НаВесьДень;
	//
	...
	//	Если ЗначениеЗаполнено(СтрокаТаблицы.Лицо)
		...
		// И МассивУчастники.Найти(СтрокаТаблицы.Лицо) = Неопределено Тогда
	//		МассивУчастники.Добавить(СтрокаТаблицы.Лицо);
	//	КонецЕсли;
	//КонецЦикла;
	//
...
Функция ЗагрузитьОбновитьПочтовоеСообщениеВMSExchange(Настройка, ТипПочтовогоСообщения, Подключение,
	 СтрокаТаблицы, СтрокаОшибки = "",
	 ДатаВремяНачалаОбмена = Неопределено)
	IDСозданногоИзмененногоОбъекта = Неопределено;
	
	Если ТипПочтовогоСообщения = "Входящие" Тогда
		ПочтовыеСообщенияОбмениватьсяТолькоНовымиОбъектами =
			Настройка.ПочтовыеСообщенияОбмениватьсяТолькоНовымиОбъектамиВходящие;
	Иначе
		ПочтовыеСообщенияОбмениватьсяТолькоНовымиОбъектами =
			Настройка.ПочтовыеСообщенияОбмениватьсяТолькоНовымиОбъектамиОтправленные;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтрокаТаблицы.ID) Тогда
		Если СтрокаТаблицы.ПочтовоеСообщение.ПометкаУдаления И Настройка.УдалятьПомеченныеНаУдаленияПочтовыеСообщения Тогда
			...
			CRM_MSExchangeСерверЭкспортныеМетоды.УдалитьСоответствиеОбъектовMSExchange(Настройка.УчетнаяЗапись,
				 СтрокаТаблицы.ПочтовоеСообщение);
		Иначе
			// Здесь не передаем в ф-ю СтрокаОшибки чтобы не плодить ошибки, вида "такого объекта нет", которые могут сбить с
			// толку.
			ОбъектMSExchange = CRM_MSExchangeСерверЭкспортныеМетоды.ПолучитьПочтовоеСообщение(Подключение,
				 СтрокаТаблицы.ID, ,
				 "IdOnly");
			...
			Если ТипЗнч(ОбъектMSExchange) = Тип("ТаблицаЗначений") И ОбъектMSExchange.Количество() > 0 Тогда
				Если Не ПочтовыеСообщенияОбмениватьсяТолькоНовымиОбъектами Тогда
					...
					КонецЕсли;
					СтрокаТаблицы.ChangeKey = ОбъектMSExchange[0].ChangeKey;
					СтрокаТаблицы.ДанныеОбновления = СформироватьДанныеДляСозданияОбновленияПочтовогоСообщения(Настройка,
						 СтрокаТаблицы.ПочтовоеСообщение,
						 Истина);
			...
			Иначе
				...
				Если Не СтрокаТаблицы.ПочтовоеСообщение.ПометкаУдаления Тогда
					...
					Если Результат = Неопределено Тогда
						ДобавитьВСтрокуОшибки(СтрокаОшибки,
							 НСтр("ru='Ошибка создания/обновления письма в MS Exchange: ';
							|en='Error of create/update emails in MS Exchange: '") 
							+ Строка(СтрокаТаблицы.ПочтовоеСообщение));
					Иначе
						CRM_MSExchangeСерверЭкспортныеМетоды.ЗаписатьСоответствиеОбъектовMSExchange(Настройка.УчетнаяЗапись,
							 СтрокаТаблицы.ПочтовоеСообщение, Результат[0].ID, Результат[0].ChangeKey,
							 ?(ЗначениеЗаполнено(ДатаВремяНачалаОбмена), ДатаВремяНачалаОбмена - 1,
							 Неопределено));
						IDСозданногоИзмененногоОбъекта = Результат[0].ID;
					КонецЕсли;
...
Функция ЗагрузитьОбновитьПочтовыеСообщенияВMSExchange(Настройка, ТипПочтовогоСообщения, Подключение,
	 Данные, СтрокаОшибки = "",
	 ДатаВремяНачалаОбмена = Неопределено) Экспорт
	...
	Для Каждого СтрокаТаблицы Из Данные Цикл
		IDСозданногоИзмененногоОбъекта = ЗагрузитьОбновитьПочтовоеСообщениеВMSExchange(Настройка,
			 ТипПочтовогоСообщения, Подключение, СтрокаТаблицы, СтрокаОшибки,
			 ДатаВремяНачалаОбмена);
		Если ЗначениеЗаполнено(IDСозданногоИзмененногоОбъекта) Тогда
			МассивID.Добавить(IDСозданногоИзмененногоОбъекта);
		КонецЕсли;
...
Функция ЗагрузитьОбновитьПочтовоеСообщениеВ1С(Настройка, ТипПочтовогоСообщения, СтрокаТаблицы,
	 СтрокаОшибки, СоответствияАдресовИКонтактов, МассивСсылокЗагруженныхОбновленныхПочтовыхСообщений,
	 ДатаВремяНачалаОбмена = Неопределено,
	 УчетнаяЗаписьЭлектроннойПочты = Неопределено)
	ChangeKey = Неопределено;
	ПочтовоеСообщениеСсылка = CRM_MSExchangeСерверЭкспортныеМетоды.ПолучитьОбъект1СПоID(Настройка.УчетнаяЗапись,
		 СтрокаТаблицы.ID,
		 ChangeKey);
	Если ЗначениеЗаполнено(ПочтовоеСообщениеСсылка)
		 И МассивСсылокЗагруженныхОбновленныхПочтовыхСообщений.Количество() > 0 Тогда
	...
	Если ЗначениеЗаполнено(ПочтовоеСообщениеСсылка) И ТипЗнч(ПочтовоеСообщениеСсылка) <> ТипДокумента
		 И ТипЗнч(ПочтовоеСообщениеСсылка) <> Тип("ДокументСсылка.CRM_Взаимодействие") Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ПочтовоеСообщениеСсылка) И ЗначениеЗаполнено(ChangeKey)
		 И ChangeKey = СтрокаТаблицы.ChangeKey Тогда
		// Объект был загружен из 1С в прошлом сеансе обмена и не менялся - пропускаем его.
	...
	Если ОбновляетсяСуществующийОбъект Тогда
		CRM_MSExchangeСерверЭкспортныеМетоды.ОбновитьChangeKeyВСоответствииОбъектовMSExchange(Настройка.УчетнаяЗапись,
			 ПочтовоеСообщениеОбъект.Ссылка,
			 СтрокаТаблицы.ChangeKey);
	Иначе
		CRM_MSExchangeСерверЭкспортныеМетоды.ЗаписатьСоответствиеОбъектовMSExchange(Настройка.УчетнаяЗапись,
			 ПочтовоеСообщениеОбъект.Ссылка, СтрокаТаблицы.ID, СтрокаТаблицы.ChangeKey,
			 ?(ЗначениеЗаполнено(ДатаВремяНачалаОбмена), ДатаВремяНачалаОбмена - 1,
			 Неопределено));
	КонецЕсли;
...
Функция ЗагрузитьОбновитьПочтовыеСообщенияВ1С(Настройка, ТипПочтовогоСообщения, Данные,
	 СтрокаОшибки, МассивСсылокЗагруженныхОбновленныхПочтовыхСообщений = Неопределено,
	 ДатаВремяНачалаОбмена = Неопределено)
	Если МассивСсылокЗагруженныхОбновленныхПочтовыхСообщений = Неопределено Тогда
	...
	УчетнаяЗаписьЭлектроннойПочты =
		ПолучитьУчетнуюЗаписьЭлектроннойПочтыПоУчетнойЗаписиMSExchange(Настройка.УчетнаяЗапись);
	...
	Для Каждого СтрокаТаблицы Из Данные Цикл
		СсылкаСозданногоИзмененногоОбъекта = ЗагрузитьОбновитьПочтовоеСообщениеВ1С(Настройка,
			 ТипПочтовогоСообщения, СтрокаТаблицы, СтрокаОшибки, СоответствияАдресовИКонтактов,
			 МассивСсылокЗагруженныхОбновленныхПочтовыхСообщений, ДатаВремяНачалаОбмена,
			 УчетнаяЗаписьЭлектроннойПочты);
		Если ЗначениеЗаполнено(СсылкаСозданногоИзмененногоОбъекта) Тогда
			МассивСсылок.Добавить(СсылкаСозданногоИзмененногоОбъекта);
		КонецЕсли;
...
Функция ЗагрузитьОбновитьСписокРассылкиВ1С(Настройка, Подключение, СтрокаТаблицы, СтрокаОшибки,
	 ТолькоОбъектыБезСостава = Ложь,
	 ДатаВремяНачалаОбмена = Неопределено)
	СписокРассылкиСсылка = CRM_MSExchangeСерверЭкспортныеМетоды.ПолучитьОбъект1СПоID(Настройка.УчетнаяЗапись,
		 СтрокаТаблицы.ID);
	...
	Если ЗначениеЗаполнено(СписокРассылкиСсылка)
		 И ТипЗнч(СписокРассылкиСсылка) = Тип("СправочникСсылка.CRM_СпискиРассылок") Тогда
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаблицы, "DisplayName") Тогда
		СписокРассылкиОбъект.Наименование = СтрокаТаблицы.DisplayName;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(СписокРассылкиОбъект.Наименование)
		И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаблицы, "FileAs") Тогда
		СписокРассылкиОбъект.Наименование = СтрокаТаблицы.FileAs;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(СписокРассылкиОбъект.Наименование)
		И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаблицы, "FileAs") Тогда
		СписокРассылкиОбъект.Наименование = СтрокаТаблицы.Subject;
	КонецЕсли;
	...
	Если Не ТолькоОбъектыБезСостава Тогда
		СоставСпискаРассылки = CRM_MSExchangeСерверЭкспортныеМетоды.ПолучитьСоставГруппыКонтактов(Подключение,
			 СтрокаТаблицы.ID,
			 СтрокаОшибки);
		...
		Для Каждого СтрокаСостава Из СоставСпискаРассылки Цикл
			Если СтрокаСостава.MailboxType = "PrivateDL" Или СтрокаСостава.MailboxType = "PublicDL" Тогда
				ВложенныйСписокРассылкиСсылка = CRM_MSExchangeСерверЭкспортныеМетоды.ПолучитьОбъект1СПоID(Настройка.УчетнаяЗапись,
					 СтрокаСостава.ID);
				Если Не ЗначениеЗаполнено(ВложенныйСписокРассылкиСсылка)
					 Или ТипЗнч(ВложенныйСписокРассылкиСсылка) <> Тип("СправочникСсылка.CRM_СпискиРассылок") Тогда
					Продолжить;
				КонецЕсли;
				
				НайденныеСтроки = СписокРассылкиОбъект.Состав.НайтиСтроки(Новый Структура("Контакт",
					 ВложенныйСписокРассылкиСсылка));
				Если НайденныеСтроки.Количество() = 0 Тогда
	...
	CRM_MSExchangeСерверЭкспортныеМетоды.ЗаписатьСоответствиеОбъектовMSExchange(Настройка.УчетнаяЗапись,
		 СписокРассылкиОбъект.Ссылка, СтрокаТаблицы.ID, СтрокаТаблицы.ChangeKey,
		 ?(ЗначениеЗаполнено(ДатаВремяНачалаОбмена), ДатаВремяНачалаОбмена - 1,
		 Неопределено));
...
Функция ЗагрузитьОбновитьСпискиРассылокВ1С(Настройка, Подключение, Данные, СтрокаОшибки,
	 ТолькоОбъектыБезСостава = Ложь,
	 ДатаВремяНачалаОбмена = Неопределено)
	МассивСсылок = Новый Массив();
	...
	Для Каждого СтрокаТаблицы Из Данные Цикл
		...
		Если ItemClass = Неопределено Или НРег(ItemClass) = "ipm.distlist" Тогда
			СсылкаСозданногоИзмененногоОбъекта = ЗагрузитьОбновитьСписокРассылкиВ1С(Настройка, Подключение,
				 СтрокаТаблицы, СтрокаОшибки, ТолькоОбъектыБезСостава,
				 ДатаВремяНачалаОбмена);
			Если ЗначениеЗаполнено(СсылкаСозданногоИзмененногоОбъекта) Тогда
				МассивСсылок.Добавить(СсылкаСозданногоИзмененногоОбъекта);
			КонецЕсли;
...
Функция ПолучитьЭлементСтруктурыПапокMSExchangeПоID(СерверMSExchange, УчетнаяЗапись, ID)
	...
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	СтруктурыПапок.Ссылка КАК Ссылка,
	|	СтруктурыПапок.ID КАК ID
	|ИЗ
	|	Справочник.CRM_СтруктурыПапокMSExchange КАК СтруктурыПапок
	|ГДЕ
	|	СтруктурыПапок.СерверMSExchange = &СерверMSExchange
	|	И СтруктурыПапок.УчетнаяЗапись = &УчетнаяЗапись
	|	И СтруктурыПапок.ID = &ID");
	...
	Если Не РезультатаЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			// Дополнительная проверка на случай совпадения 
			// идентификаторов с точностью до регистра символов.
...
Процедура ЗагрузитьЭлементыСтруктурыПапокMSExchangeРекурсивно(СтрокиДерева, СерверMSExchange,
	 УчетнаяЗапись, СоответствиеDistinguishedFolderIds,
	 МассивID)
	Для Каждого СтрокаДерева Из СтрокиДерева Цикл
		...
		Попытка
			ParentFolderId = СтрокаДерева.ParentFolderId;
		...
		Если СтрокаДерева.Строки.Количество() > 0 Тогда
			ЗагрузитьЭлементыСтруктурыПапокMSExchangeРекурсивно(СтрокаДерева.Строки, СерверMSExchange,
				 УчетнаяЗапись, СоответствиеDistinguishedFolderIds,
				 МассивID);
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗагрузитьОбновитьСтруктуруПапокВ1С(Настройка, Подключение, СтрокаОшибки = "",
	 ПроверятьПодключение = Ложь) Экспорт
	...
	Если ТипЗнч(ДеревоПапок) = Тип("ДеревоЗначений") И ДеревоПапок.Строки.Количество() > 0 Тогда
		...
		Попытка
			...
			ЗагрузитьЭлементыСтруктурыПапокMSExchangeРекурсивно(ДеревоПапок.Строки,
				 Настройка.УчетнаяЗапись.СерверMSExchange, Настройка.УчетнаяЗапись,
				 СоответствиеDistinguishedFolderIds,
				 МассивID);
			
			ПометитьНеИспользуемыеЭлементыСтруктурыПапокMSExchangeНаУдаление(Настройка.УчетнаяЗапись.СерверMSExchange,
				 Настройка.УчетнаяЗапись,
				 МассивID);
...
Процедура СинхронизироватьПоНастройке(Настройка, СтрокаОшибки = "") Экспорт
	Если НЕ ЗначениеЗаполнено(Настройка.УчетнаяЗапись) Тогда
		ВызватьИсключение НСтр("ru='В настройке не заполнена учетная запись!';
			|en='In setting the login account is not filled in!'");
	КонецЕсли;
	...
	Если НЕ (Настройка.Подключение.СерверMSExchange = Настройка.УчетнаяЗапись.СерверMSExchange) Тогда
		ВызватьИсключение НСтр("ru='Реквизит ""Сервер MS Exchange"" в подключении отличается от значения,
			| указанного в учетной записи!';en='Attribute ""server MS Exchange"" in connection differs from the value specified in account!'");
	КонецЕсли;
	...
	Если Настройка.СинхронизироватьКлиенты ИЛИ Настройка.СинхронизироватьКонтактныеЛица Тогда
		Если НаправлениеMSExchange Тогда
			...
			СинхронизируемыеКонтакты = ПолучитьСинхронизируемыеКонтакты1С(Настройка,
				 МассивIDЗагруженныхОбновленныхКонтактов,
				 МассивСсылокЗагруженныхОбновленныхКонтактов);
			Если ТипЗнч(СинхронизируемыеКонтакты) = Тип("ТаблицаЗначений") Тогда
				ЗагруженныеОбновленныеID = ЗагрузитьОбновитьКонтактыВMSExchange(Настройка, Подключение,
					 СинхронизируемыеКонтакты, СтрокаОшибки, Ложь, ?(ДвустороннийОбмен,
					 ДатаВремяНачалаДвустороннегоОбмена,
					 ДатаВремяНачалаОбмена));
				...
				Если СинхронизируемыеКонтакты.Колонки.Найти("Контакт") <> Неопределено Тогда
					...
					КонецЦикла;
				КонецЕсли;
				CRM_MSExchangeСерверЭкспортныеМетоды.ЗаписатьДатыПоследнегоОбмена(Настройка,
					 ?(ДвустороннийОбмен, ДатаВремяНачалаДвустороннегоОбмена,
					 ДатаВремяНачалаОбмена));
			КонецЕсли;
		КонецЕсли;
		Если Направление1С Тогда
			ДатаВремяНачалаОбмена = ТекущаяДатаСеанса();
			СинхронизируемыеКонтакты = ПолучитьСинхронизируемыеКонтактыMSExchange(Настройка, Подключение,
				 СтрокаОшибки,
				 МассивIDЗагруженныхОбновленныхКонтактов);
			Если ТипЗнч(СинхронизируемыеКонтакты) = Тип("ТаблицаЗначений") Тогда
				ЗагруженныеОбновленныеСсылки = ЗагрузитьОбновитьКонтактыВ1С(Настройка, СинхронизируемыеКонтакты,
					 СтрокаОшибки, МассивСсылокЗагруженныхОбновленныхКонтактов, ?(ДвустороннийОбмен,
					 ДатаВремяНачалаДвустороннегоОбмена,
					 ДатаВремяНачалаОбмена));
				...
				Если СинхронизируемыеКонтакты.Колонки.Найти("ID") <> Неопределено Тогда
					...
					КонецЦикла;
				КонецЕсли;
				CRM_MSExchangeСерверЭкспортныеМетоды.ЗаписатьДатыПоследнегоОбмена(Настройка, Неопределено,
					 ?(ДвустороннийОбмен, ДатаВремяНачалаДвустороннегоОбмена,
					 ДатаВремяНачалаОбмена));
			Иначе
	...
	Если Настройка.СинхронизироватьСобытия Тогда
		Если НаправлениеMSExchange Тогда
			ДатаВремяНачалаОбмена = ТекущаяДатаСеанса();
			СинхронизируемыеСобытия = ПолучитьСинхронизируемыеСобытия1С(Настройка,
				 МассивIDЗагруженныхОбновленныхСобытий,
				 МассивСсылокЗагруженныхОбновленныхСобытий);
			Если ТипЗнч(СинхронизируемыеСобытия) = Тип("ТаблицаЗначений") Тогда
				Если Настройка.СинхронизироватьКонтактыСобытий И (СинхронизируемыеСобытия.Количество() > 0) Тогда
					СинхронизируемыеКонтактыСобытий1С = ПолучитьКонтактыСобытий1С(Настройка,
						 СинхронизируемыеСобытия, МассивIDЗагруженныхОбновленныхКонтактов,
						 МассивСсылокЗагруженныхОбновленныхКонтактов);
					ЗагруженныеОбновленныеID = ЗагрузитьОбновитьКонтактыВMSExchange(Настройка, Подключение,
						 СинхронизируемыеКонтактыСобытий1С, СтрокаОшибки, Истина, ?(ДвустороннийОбмен,
						 ДатаВремяНачалаДвустороннегоОбмена,
						 ДатаВремяНачалаОбмена));
					Если ТипЗнч(ЗагруженныеОбновленныеID) = Тип("Массив") Тогда
						Для Каждого ID Из ЗагруженныеОбновленныеID Цикл
					...
					КонецЕсли;
				КонецЕсли;
				ЗагруженныеОбновленныеID = ЗагрузитьОбновитьСобытияВMSExchange(Настройка, Подключение,
					 СинхронизируемыеСобытия, СтрокаОшибки, ?(ДвустороннийОбмен, ДатаВремяНачалаДвустороннегоОбмена,
					 ДатаВремяНачалаОбмена));
				Если ТипЗнч(ЗагруженныеОбновленныеID) = Тип("Массив") Тогда
					Для Каждого ID Из ЗагруженныеОбновленныеID Цикл
				...
				КонецЕсли;
				CRM_MSExchangeСерверЭкспортныеМетоды.ЗаписатьДатыПоследнегоОбмена(Настройка, Неопределено,
					 Неопределено, ?(ДвустороннийОбмен, ДатаВремяНачалаДвустороннегоОбмена,
					 ДатаВремяНачалаОбмена));
			КонецЕсли;
		КонецЕсли;
		Если Направление1С Тогда
			ДатаВремяНачалаОбмена = ТекущаяДатаСеанса();
			СинхронизируемыеСобытия = ПолучитьСинхронизируемыеСобытияMSExchange(Настройка, Подключение,
				 СтрокаОшибки,
				 МассивIDЗагруженныхОбновленныхСобытий);
			Если ТипЗнч(СинхронизируемыеСобытия) = Тип("ТаблицаЗначений") Тогда
				ЗагруженныеОбновленныеСсылки = ЗагрузитьОбновитьСобытияВ1С(Настройка, СинхронизируемыеСобытия,
					 СтрокаОшибки, МассивСсылокЗагруженныхОбновленныхСобытий, ?(ДвустороннийОбмен,
					 ДатаВремяНачалаДвустороннегоОбмена,
					 ДатаВремяНачалаОбмена));
				Если ТипЗнч(ЗагруженныеОбновленныеСсылки) = Тип("Массив") Тогда
					Для Каждого ОбъектСсылка Из ЗагруженныеОбновленныеСсылки Цикл
				...
				КонецЕсли;
				CRM_MSExchangeСерверЭкспортныеМетоды.ЗаписатьДатыПоследнегоОбмена(Настройка, Неопределено,
					 Неопределено, Неопределено, ?(ДвустороннийОбмен, ДатаВремяНачалаДвустороннегоОбмена,
					 ДатаВремяНачалаОбмена));
			Иначе
				Если ЗначениеЗаполнено(СтрокаОшибки) Тогда
					ВызватьИсключение СтрокаОшибки;
	...
	Если Ложь Тогда
		...
		// Почтовые сообщения
		Если Настройка.СинхронизироватьПочтовыеСообщенияВходящие
			 Или Настройка.СинхронизироватьПочтовыеСообщенияОтправленные Тогда
			Если Настройка.СинхронизироватьПочтовыеСообщенияВходящие Тогда
				ДатаВремяНачалаДвустороннегоОбмена = ТекущаяДатаСеанса();
				...
				Если НаправлениеMSExchange Тогда
					...
					СинхронизируемыеПочтовыеСообщения = ПолучитьСинхронизируемыеПочтовыеСообщения1С(Настройка,
						 "Входящие", СтрокаОшибки, МассивIDЗагруженныхОбновленныхПочтовыхСообщенийВходящих,
						 МассивСсылокЗагруженныхОбновленныхПочтовыхСообщенийВходящих);
					
					Если ТипЗнч(СинхронизируемыеПочтовыеСообщения) = Тип("ТаблицаЗначений") > 0 Тогда
						Если Настройка.СинхронизироватьКонтактыПочтовыхСообщенийВходящие
							 И СинхронизируемыеПочтовыеСообщения.Количество() > 0 Тогда
							// BSLLS:UnusedLocalVariable-off
							СинхронизируемыеКонтактыПочтовыхСообщений1С = ПолучитьКонтактыПочтовыхСообщений1С(Настройка,
								 "Входящие", СинхронизируемыеПочтовыеСообщения, МассивIDЗагруженныхОбновленныхКонтактов,
								 МассивСсылокЗагруженныхОбновленныхКонтактов);
						...
						ЗагруженныеОбновленныеID = ЗагрузитьОбновитьПочтовыеСообщенияВMSExchange(Настройка,
							 "Входящие", Подключение, СинхронизируемыеПочтовыеСообщения, СтрокаОшибки, ?(ДвустороннийОбмен,
							 ДатаВремяНачалаДвустороннегоОбмена,
							 ДатаВремяНачалаОбмена));
				...
				Если Направление1С Тогда
					...
					Если ЗначениеЗаполнено(УчетнаяЗаписьЭлектроннойПочты) Или Настройка.СоздаватьСобытияПоПочтовымСообщениям Тогда
						...
						СинхронизируемыеПочтовыеСообщения = ПолучитьСинхронизируемыеПочтовыеСообщенияMSExchange(Настройка,
							 "Входящие", Подключение, СтрокаОшибки,
							 МассивIDЗагруженныхОбновленныхПочтовыхСообщенийВходящих);
						
						Если ТипЗнч(СинхронизируемыеПочтовыеСообщения) = Тип("ТаблицаЗначений") Тогда
							ЗагруженныеОбновленныеСсылки = ЗагрузитьОбновитьПочтовыеСообщенияВ1С(Настройка, "Входящие",
								 СинхронизируемыеПочтовыеСообщения, СтрокаОшибки,
								 МассивСсылокЗагруженныхОбновленныхПочтовыхСообщенийВходящих, ?(ДвустороннийОбмен,
								 ДатаВремяНачалаДвустороннегоОбмена,
								 ДатаВремяНачалаОбмена));
							Если ТипЗнч(ЗагруженныеОбновленныеСсылки) = Тип("Массив") Тогда
								Для Каждого ОбъектСсылка Из ЗагруженныеОбновленныеСсылки Цикл
						...
						Иначе
							...
							Иначе
								ВызватьИсключение НСтр("ru='Ошибка при получении списка писем!';
									|en='Error when obtaining the list of letters!'");
							КонецЕсли;
						КонецЕсли;
	...
	Если Настройка.СинхронизироватьЗадачи Тогда
		Если НаправлениеMSExchange Тогда
			...
			СинхронизируемыеЗадачи = ПолучитьСинхронизируемыеЗадачи1С(Настройка,
				 МассивIDЗагруженныхОбновленныхЗадач,
				 МассивСсылокЗагруженныхОбновленныхЗадач);
			Если ТипЗнч(СинхронизируемыеЗадачи) = Тип("ТаблицаЗначений") Тогда
				ЗагруженныеОбновленныеID = ЗагрузитьОбновитьЗадачиВMSExchange(Настройка, Подключение,
					 СинхронизируемыеЗадачи, СтрокаОшибки, ?(ДвустороннийОбмен, ДатаВремяНачалаДвустороннегоОбмена,
					 ДатаВремяНачалаОбмена));
				Если ТипЗнч(ЗагруженныеОбновленныеID) = Тип("Массив") Тогда
					Для Каждого ID Из ЗагруженныеОбновленныеID Цикл
		...
		Если Направление1С Тогда
			...
			СинхронизируемыеЗадачи = ПолучитьСинхронизируемыеЗадачиMSExchange(Настройка, Подключение,
				 СтрокаОшибки,
				 МассивIDЗагруженныхОбновленныхЗадач);
			Если ТипЗнч(СинхронизируемыеЗадачи) = Тип("ТаблицаЗначений") Тогда
				ЗагруженныеОбновленныеСсылки = ЗагрузитьОбновитьЗадачиВ1С(Настройка, СинхронизируемыеЗадачи,
					 СтрокаОшибки, МассивСсылокЗагруженныхОбновленныхЗадач, ?(ДвустороннийОбмен,
					 ДатаВремяНачалаДвустороннегоОбмена,
					 ДатаВремяНачалаОбмена));
				Если ТипЗнч(ЗагруженныеОбновленныеСсылки) = Тип("Массив") Тогда
					Для Каждого ОбъектСсылка Из ЗагруженныеОбновленныеСсылки Цикл
			...
			Иначе
				...
				Иначе
					ВызватьИсключение НСтр("ru='Ошибка при получении списка задач!';
						|en='An error occurred while retrieving the task list!'");
				КонецЕсли;
			КонецЕсли;
	...
	Если Настройка.ЗагружатьСпискиРассылок Тогда
		...
		СинхронизируемыеСпискиРассылок = ПолучитьСинхронизируемыеСпискиРассылокMSExchange(Настройка,
			 Подключение,
			 СтрокаОшибки);
		...
		Если ТипЗнч(СинхронизируемыеСпискиРассылок) = Тип("ТаблицаЗначений") Тогда
			...
			ЗагруженныеОбновленныеСсылки = ЗагрузитьОбновитьСпискиРассылокВ1С(Настройка, Подключение,
				 СинхронизируемыеСпискиРассылок, СтрокаОшибки, Истина,
				 ДатаВремяНачалаОбмена);
			...
			Если ТипЗнч(ЗагруженныеОбновленныеСсылки) = Тип("Массив") И ЗагруженныеОбновленныеСсылки.Количество() > 0 Тогда
				ЗагрузитьОбновитьСпискиРассылокВ1С(Настройка, Подключение, СинхронизируемыеСпискиРассылок,
					 СтрокаОшибки, Ложь,
					 ДатаВремяНачалаОбмена);
			КонецЕсли;
			
			CRM_MSExchangeСерверЭкспортныеМетоды.ЗаписатьДатыПоследнегоОбмена(Настройка, Неопределено,
				 Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено,
				 ДатаВремяНачалаОбмена);
...
Процедура ВыполнитьРегламентныйОбменДаннымиСMSExchange() Экспорт
	...
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.CRM_ОбменДаннымиСMSExchange);
