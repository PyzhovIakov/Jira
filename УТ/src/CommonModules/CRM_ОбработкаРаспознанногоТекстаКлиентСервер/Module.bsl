...
// Функция возвращает текст, полученный из результата распознавания.
//
// Параметры:
...
Функция ТекстИзРезультатаРаспознавания(РезультатРаспознавания) Экспорт
	...
	ВозможныеКоманды = CRM_ОбработкаРаспознанногоТекстаКлиентСерверПовтИсп.Команды();
	ВозможныеТокены = CRM_ОбработкаРаспознанногоТекстаКлиентСерверПовтИсп.ЗаменяемыеЗнаки();
...
// Функция возвращает переданную строку следующим образом: у каждого слова строки первый символ преобразуется
// к верхнему регистру. Все остальные символы преобразуются к нижнему регистру.
//
// Параметры:
//  Слово	 - Строка	 - Слово.
// 
...
Функция ТРегВнутр(Слово) Экспорт
	...
	Соответствие = CRM_ОбработкаРаспознанногоТекстаКлиентСерверПовтИсп.СоответствиеДляТрег();
