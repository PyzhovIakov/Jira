...
//
Функция ВыполнитьЗапрос(Адрес, Метод, Заголовки, ПараметрыЗапроса = Неопределено, ИмяФайла = "",
	 ПоказыватьСообщенияОбОшибках = Истина) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПолучениеФайловИзИнтернета") Тогда
		...
		Протокол = ?(ПараметрыСоединения["ЗащищенноеСоединение"], "https", "http");
	...
	HTTPСоединение = Новый HTTPСоединение(ПараметрыСоединения["Сервер"],
										ПараметрыСоединения["Порт"], , , Прокси, 30,
		...
		Исключение
			...
			Если ПоказыватьСообщенияОбОшибках Тогда
				...
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%2",
					 НСтр("ru='Проверьте соединение с Интернет. Установите корректное время на устройстве.';
					|en='Please check your Internet connection and set current time on your device.'"));
...
Функция ПолучитьПараметрыСоединения(Знач Адрес)
	...
	НайденнаяПозиция = СтрНайти(Адрес, "/");
	...
	НайденнаяПозиция = СтрНайти(Сервер, ":");
...
Функция ПолучитьСтрокуПараметров(ПараметрыЗапроса, Разделитель = "&") Экспорт
	...
	Для Каждого ЭлементКоллекции Из ПараметрыЗапроса Цикл
		...
		Результат = Результат + ЭлементКоллекции.Ключ + "=" + КодироватьСтроку(ЭлементКоллекции.Значение,
...
Функция ПолучитьОбъектJSON(Текст)
	...
	Возврат CRM_СинхронизацияСiCRMНовый.ЗначениеИзСтрокиJSON(Текст, Истина, ,
		 "ВосстановлениеЗначенийJSON",
		 CRM_ВзаимодействиеСИнтернетСервер);
...
Функция СоответствуетСигнатуре(Значение, ПозицияСимвола, Сигнатура)
	...
	Для НомерСимвола = 1 По СтрДлина(Сигнатура) Цикл
		...
		Если Символ <> СимволСигнатуры
			И СимволСигнатуры <> "*"
			И СтрНайти("0123456789ABCDEF", ВРег(Символ) = 0) Тогда
...
Функция HexToDec(Знач Значение)
	...
	Пока НомерСимвола > 0 Цикл
		...
		Результат = Результат + (СтрНайти("0123456789ABCDEF", Сред(Значение, НомерСимвола, 1)) - 1) * Основание;
