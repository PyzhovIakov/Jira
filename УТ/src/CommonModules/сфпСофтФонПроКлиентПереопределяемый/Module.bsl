...
Процедура сфпВыполнитьАвтоматическоеДействие(СтруктураЗвонка, ОткрыватьЗвонок = Ложь) Экспорт
	...
	Если Не СтруктураЗвонка.НовыйЗвонок = Неопределено Тогда
		РеквизитыЗвонка = сфпСофтФонПроСервер.ЗначенияРеквизитовОбъекта(СтруктураЗвонка.НовыйЗвонок, "сфпНецелевойЗвонок");
		Если сфпСофтФонПроСервер.НеВыполнятьАвтоматическоеДействие(СтруктураЗвонка.НовыйЗвонок)
			 ИЛИ РеквизитыЗвонка.сфпНецелевойЗвонок Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Иначе НаименованиеДействия =
		сфпСофтФонПроСервер.сфпПолучитьЗначениеНастройкиПользователя("сфпДействиеПриИсходящемЗвонке");
	КонецЕсли;
	...
	Для Каждого ЭлементМассива Из МассивАвтоматическихДействий Цикл
		Если ЭлементМассива.Наименование = НаименованиеДействия Тогда
			Если НЕ ПустаяСтрока(ЭлементМассива.Действие) Тогда
				Попытка
					ЗвонокСделанИзТМ = Ложь;
					НомерИзКеша = "";
					ДлинаНомера = 0;
					ПараметрыЗвонкаТМ = ПараметрыПриложения.Получить("ПараметрыЗвонкаИзТелемаркетинга");
					Если Не ПараметрыЗвонкаТМ = Неопределено Тогда
						ЗвонокСделанИзТМ =
							(СтруктураЗвонка.ДатаОтвета - ПараметрыЗвонкаТМ.ДатаНачала) <= ПараметрыЗвонкаТМ.ВремяАктуальности;
						НомерИзКеша = ПараметрыЗвонкаТМ.Номер;
						Если СтруктураЗвонка.ВнешнийЗвонок Тогда
							ДлинаНомера = ПараметрыЗвонкаТМ.ДлинаНомераВнеш;
						Иначе
							ДлинаНомера = ПараметрыЗвонкаТМ.ДлинаНомераВнутр;
						КонецЕсли;
					КонецЕсли;
					
					Если ЗвонокСделанИзТМ И Прав(СтруктураЗвонка.НомерТелефона, ДлинаНомера) = Прав(НомерИзКеша, ДлинаНомера) Тогда
						
						СтруктураЗвонка.Вставить("КлиентСтрокой", ПараметрыЗвонкаТМ.КлиентСтрокой);
						СтруктураЗвонка.Вставить("КонтактСтрокой", ПараметрыЗвонкаТМ.КонтактСтрокой);
						
						сфпОткрытьСтикер(СтруктураЗвонка);
					
					ИначеЕсли ЭлементМассива.Действие = "сфпОткрытьПанельЗвонка" Тогда
						сфпОткрытьПанельЗвонка(СтруктураЗвонка);
...
//	СтруктураЗвонка	- Структура	- Структура данных звонка
//
Процедура сфпОткрытьПанельЗвонка(СтруктураЗвонка) Экспорт
	
	Если СтруктураЗвонка.Свойство("ИдентификаторЗвонка") Тогда
		ИдентификаторФормы = СтруктураЗвонка.ИдентификаторЗвонка;
		
	ИначеЕсли СтруктураЗвонка.Свойство("hCall") Тогда
		ИдентификаторФормы = СтруктураЗвонка.hCall;
		
	Иначе	
		ИдентификаторФормы = Новый УникальныйИдентификатор();
	КонецЕсли;
	
	ОткрытьФорму("ОбщаяФорма.сфпПанельЗвонка", СтруктураЗвонка, , ИдентификаторФормы);
...
Процедура сфпОткрытьФормуОбъектаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(Результат) Тогда
		...
		ИначеЕсли ТипЗнч(Результат) = Тип("СправочникСсылка.CRM_ПотенциальныеКлиенты") Тогда
			...
			Иначе
				Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ФормаОбъекта.Объект, "ПотенциальныйКлиент") Тогда
...
Процедура сфпОткрытьСтикерЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = Неопределено Тогда
		КомментарийЗвонка = НСтр("ru = 'Звонили клиенту, которого нет в базе: %1.
		                               |Контактное лицо: %2.'");
		КомментарийЗвонка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(КомментарийЗвонка,
		                                                                            СтруктураЗвонка.КлиентСтрокой,
		                                                                            СтруктураЗвонка.КонтактСтрокой);
		СтруктураЗвонка.Вставить("Комментарий", КомментарийЗвонка);
	КонецЕсли;
	...
	ИмяФормы = "Документ.ТелефонныйЗвонок.Форма.сфпФормаДокументаКраткая";
	ОткрытьФорму(ИмяФормы, СтруктураЗвонка, , ИдентификаторФормы, , , , РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
...
Процедура сфпОткрытьСписокАнкетЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(Результат) Тогда
		...
		Если ТипЗнч(Результат) = Тип("СправочникСсылка.Партнеры") ИЛИ
			...
			ОткрытьФорму("Документ.Анкета.Форма.CRM_ФормаСписка", ПараметрыФормы);
		Иначе
			Если сфпСофтФонПроСерверПовтИсп.ЭтоНеCRM() И сфпСофтФонПроСервер.сфпЯвляетсяКонтрагентом(Результат) Тогда
				Клиент = сфпСофтФонПроСервер.сфпПолучитьПартнераКонтрагента(Результат);
				Если ЗначениеЗаполнено(Клиент) Тогда
					ПараметрыФормы = Новый Структура();
					ПараметрыФормы.Вставить("Отбор", Новый Структура("Респондент", Клиент));
					ПараметрыФормы.Вставить("Респондент", Клиент);
					ОткрытьФорму("Документ.Анкета.Форма.CRM_ФормаСписка", ПараметрыФормы);
				КонецЕсли;
			КонецЕсли;
...
//
// Параметры:
//	Результат				- СправочникСсылка	- Выбранный пользователем объект для подстановки в поле "Партнер"
	// или "Контактное лицо"
//	ДополнительныеПараметры	- Структура			- Структура дополнительных переданных параметров
//
...
Процедура сфпОбработкаКомандыКарточкаCoMagic(ПараметрКоманды, ПараметрыВыполненияКоманды = Неопределено) Экспорт
	...
	Адрес = "http://" + ПараметрыОператораКоллтрекинга.ДоменAPP 
		+ "/analytics/auditory/cmvisitor/?session_key=" + КлючСессии + "&visitor_id=" 
		+ CoMagicID;
...
Процедура сфпОбработкаКомандыПереводЗвонков(ПараметрКоманды, ПараметрыВыполненияКоманды) Экспорт
	...
	ИначеЕсли НЕ сфпСофтФонПроСервер.сфпРолиДоступны("ПолныеПрава, сфпУправлениеМаршрутизацией") Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Недостаточно прав для настройки перевода звонков';
			|en='There are not enough rights to set up call transfer'"));
		Возврат;
	КонецЕсли;
	ПараметрыФормы	= Новый Структура;
...
Функция сфпИнициализироватьФормуСчета(СтруктураЗвонка, ДанныеЗаполнения)
	...
	ИмяПредопределенногоЗначения = Нстр("ru='Перечисление.ВариантыВажностиВзаимодействия.Обычная';
		|en='Перечисление.ВариантыВажностиВзаимодействия.Обычная'"); 
	...
	Если ЗначениеЗаполнено(СтруктураЗвонка.Контакт) Тогда
		Если СтрНайти(Строка(СтруктураЗвонка.Контакт), НСтр("ru='<Объект не найден>';en='<Object not found>'")) > 0 Тогда
			ФормаСчета.Объект["Комментарий"]	= ФормаСчета.Объект["Комментарий"] + Символы.ПС  
		...
		Иначе
			Если сфпСофтФонПроСерверПовтИсп.ЭтоНеCRM() И сфпСофтФонПроСервер.сфпЯвляетсяКонтрагентом(СтруктураЗвонка.Контакт) Тогда
				ФормаСчета.Объект["Партнер"] = сфпСофтФонПроСервер.сфпПолучитьПартнераКонтрагента(СтруктураЗвонка.Контакт);
			КонецЕсли;
			ФормаСчета.Объект["Комментарий"] = ФормаСчета.Объект["Комментарий"] + Символы.ПС  
				+ Строка(ТипЗнч(СтруктураЗвонка.Контакт)) 
				+ ": "
...
Функция сфпИнициализироватьФормуЗаказаКлиента(СтруктураЗвонка, ДанныеЗаполнения)
	...
	Иначе
		...
		Если МодульCRM_Модуль_МетодыМодулейОбъектовДокументов <> Неопределено Тогда
			СтруктураОбъекта = Новый Структура("Ссылка, Организация",
				ФормаЗаказа.Объект.Ссылка, ФормаЗаказа.Объект.Организация);
			МодульCRM_Модуль_МетодыМодулейОбъектовДокументов.ЗаполнитьОрганизацию(СтруктураОбъекта);
			ФормаЗаказа.Объект.Организация = СтруктураОбъекта.Организация;
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтруктураЗвонка.Контакт) Тогда
		Если СтрНайти(Строка(СтруктураЗвонка.Контакт), НСтр("ru='<Объект не найден>';en='<Object not found>'")) > 0 Тогда
			ФормаЗаказа.Объект["Комментарий"]	= ФормаЗаказа.Объект["Комментарий"] + Символы.ПС  
				+ Строка(ТипЗнч(СтруктураЗвонка.Контакт)) 
				+ ": "
					+ сфпСофтФонПроСервер.сфпПолучитьНаименованиеКонтакта(СтруктураЗвонка.Контакт) + НСтр("ru=',
						| номер: ';en=', number: '") 
		...
		ИначеЕсли ТипЗнч(СтруктураЗвонка.Контакт) = Тип("СправочникСсылка.КонтактныеЛицаПартнеров") Тогда
			...
			ФормаЗаказа.Объект["Контрагент"] 		=
				сфпСофтФонПроСервер.сфпПолучитьКонтрагентаПартнера(ФормаЗаказа.Объект["Партнер"]);
		Иначе
			Если сфпСофтФонПроСерверПовтИсп.ЭтоНеCRM() И сфпСофтФонПроСервер.сфпЯвляетсяКонтрагентом(СтруктураЗвонка.Контакт) Тогда
				ФормаЗаказа.Объект["Партнер"] = сфпСофтФонПроСервер.сфпПолучитьПартнераКонтрагента(СтруктураЗвонка.Контакт);
				ФормаЗаказа.Объект["Контрагент"] =
					сфпСофтФонПроСервер.сфпПолучитьКонтрагентаПартнера(ФормаЗаказа.Объект["Партнер"]);
			КонецЕсли;
			ФормаЗаказа.Объект["Комментарий"]	= ФормаЗаказа.Объект["Комментарий"] + Символы.ПС  
...
Функция сфпИнициализироватьФормуИнтереса(СтруктураЗвонка, ДанныеЗаполнения)
	...
	ОбъектФормы.Тема = НСтр("ru='#Создан автоматически средствами СофтФон';
		|en='# Created automatically with SoftPhone tools'") 
	+ ?(ЗначениеЗаполнено(СтруктураЗвонка.НомерТелефона), Нстр("ru=', номер телефона: ';en=', phone number: '") 
		+ СтруктураЗвонка.НомерТелефона,
		 "");
	...
	Если НЕ (ДанныеЗаполнения = Неопределено) И ДанныеЗаполнения.Свойство("сфпСтруктураCoMagic") Тогда
		...
		Если ФиксироватьПервичныйИнтерес Тогда
			...
			ФормаИнтереса.ИсточникПервичногоИнтереса 	=
...
Процедура сфпОткрытьФормуОбъекта(СтруктураЗвонка, ОбработчикЗавершения, ОбработчикИнициализацииФормы = "")
	...
	ИначеЕсли СтруктураЗвонка.Свойство("МассивЗвонящих")
		И ТипЗнч(СтруктураЗвонка["МассивЗвонящих"]) = Тип("Массив")
		И СтруктураЗвонка["МассивЗвонящих"].Количество() > 1 Тогда
		...
		Иначе
			Если Не ОбработчикЗавершения = "сфпОткрытьТелефонныйЗвонокЗавершение"
				 И Не ОбработчикЗавершения = "сфпРегистрацияОбращенияЗавершение" Тогда
...
//
Процедура сфпОткрытьОтчет(СтруктураЗвонка, ИмяОтчета, КлючВариантаОтчета = "",
	 ИмяОтбораКлиента = "",
	 ИмяСвойстваКлиента = "")
	...
	ИначеЕсли ЗначениеЗаполнено(СтруктураЗвонка["МассивЗвонящих"])
		 И СтруктураЗвонка["МассивЗвонящих"].Количество() > 1 Тогда
		ОписаниеВыбора = Новый ОписаниеОповещения("сфпОткрытьОтчетЗавершение", ЭтотОбъект,
			 Новый Структура("СтруктураЗвонка",
			 СтруктураЗвонка));
		СписокКонтактов = сфпСофтФонПроСервер.сфпСформироватьСписокОбъектовДляВыбораПоМассивуЗвонящих(СтруктураЗвонка["МассивЗвонящих"],
			 Истина);
		Если СписокКонтактов.Количество() = 1 Тогда
