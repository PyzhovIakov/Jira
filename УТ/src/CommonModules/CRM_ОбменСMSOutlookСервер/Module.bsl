...
Процедура УстановитьДлинуGUIDOutlook(ПрограммаOutlook, GUIDOutlook) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации("Обмен данными с MS Outlook",
...
Функция ДобавитьНовуюНастройку(ПрограммаOutlook, Копирование = Ложь, ТекущаяНастройка = Неопределено) Экспорт
	...
	Пока НЕ ПолученоИмяНастройки Цикл
		...
		// Вызывается только при настройке нового обмена
...
Функция ПолучитьНастройкуОбмена(ПрограммаOutlook, ИмяНастройки) Экспорт 
	...
	Возврат CRM_ЛицензированиеЭкспортныеМетоды.ПолучитьЗащищеннуюОбработку().
		ОбменСMSOutlookПолучитьНастройкуОбмена(ИмяНастройки, ПрограммаOutlook);
...
//
Функция ПолучитьСинхронизируемыеКонтакты1С(ТекстЗапроса, Отбор, Параметры = Неопределено,
	 Знач ДатаПоследнегоОбмена = Неопределено) Экспорт
	Если НЕ (ТипЗнч(ДатаПоследнегоОбмена) = Тип("Дата")) Тогда
		ДатаПоследнегоОбмена = Дата("00010101");
	КонецЕсли;
...
Функция ПолучитьСвойствоПоСсылке(Ссылка, ИмяСвойства) Экспорт
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Ссылка, ИмяСвойства) Тогда 
		Возврат Ссылка[ИмяСвойства];
	Иначе
		// Временная заглушка
		Если (ИмяСвойства = "ДатаРегистрации") ИЛИ (ИмяСвойства = "ДатаРегистрацииСвязи") Тогда
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Ссылка, "ДатаРегистрации") Тогда 
				Возврат Ссылка["ДатаРегистрации"];
			ИначеЕсли ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Ссылка, "ДатаРегистрацииСвязи") Тогда 
...
//
Процедура УдалитьОбъекты1СИзБазы(мОбъекты1С, мПрограммаOutlook) Экспорт
...
Функция СоздатьСобытие(СтруктураЗаполнения) Экспорт
	Если СтруктураЗаполнения = Неопределено Тогда 
		Возврат Неопределено;
	КонецЕсли;
	Событие = Документы.CRM_Взаимодействие.СоздатьДокумент();	
...
Функция ЗаполнитьМероприятиеПоСтруктуре(Ссылка, СтруктураОбъекта, ОчиститьУчастников = Ложь, СтрокаСобытия) Экспорт
		...
		Для Каждого СтрокаТЧ Из ТабЧасть.Значение Цикл
			...
			Для Каждого КлючЗначение Из СтрокаТЧ Цикл
				Если ЗначениеЗаполнено(КлючЗначение.Значение) 
					И МДТЧ.Реквизиты.Найти(КлючЗначение.Ключ) <> Неопределено Тогда
			...
			Для Каждого КлючЗначение Из СтрокаТЧ Цикл
				Если МДТЧ.Реквизиты.Найти(КлючЗначение.Ключ) <> Неопределено Тогда
					мСтрокаТЧ[КлючЗначение.Ключ] = КлючЗначение.Значение;
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
	КонецЦикла;
	...
	Исключение
		...
		ОбщегоНазначения.СообщитьПользователю(СтрОписаниеОшибки);
	КонецПопытки;
...
Функция ЗаполнитьПисьмоПоСтруктуре(Ссылка, СтруктураОбъекта, ОчиститьУчастников = Ложь, СтрокаСобытия) Экспорт
	...
	Попытка
		мОбъект.Записать();
		Взаимодействия.УстановитьПапкуЭлектронногоПисьма(мОбъект.Ссылка, СтруктураОбъекта.Реквизиты.Папка, Ложь);
		СтрокаСобытия.Объект_1С			= мОбъект.Ссылка;
		СтрокаСобытия.ДатаИзменения_1С	= мОбъект.Дата;
		Если СтруктураОбъекта.Реквизиты.Непрочитано Тогда
			CRM_УправлениеЭлектроннойПочтой.УстановитьСнятьПометкуОПрочтении(мОбъект.Ссылка, Ложь);
		КонецЕсли;
	Исключение
		СтрОписаниеОшибки = ОписаниеОшибки();
...
Функция ЗаполнитьОбъект(мСсылка, СтруктураОбъекта) Экспорт
	...
	Для Каждого ТабЧасть Из СтруктураОбъекта.ТабличныеЧасти Цикл
		МДТЧ = МД.ТабличныеЧасти.Найти(ТабЧасть.Ключ);
		Если МДТЧ = Неопределено
			ИЛИ МДТЧ.Реквизиты.Найти("Тип") = Неопределено
			ИЛИ МДТЧ.Реквизиты.Найти("Вид") = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Для Каждого СтрокаТЧ Из ТабЧасть.Значение Цикл
...
Функция ПолучитьКонтактыСобытия(Ссылка) Экспорт
	...
	Для Каждого Контакт Из Ссылка.СторонниеЛица  Цикл
		...
		Если ТипЗнч(Контакт1С) <> Тип("Строка") И Контакт1С.ПометкаУдаления Тогда
			Продолжить;
		КонецЕсли;	
...
Функция ПолучитьСтруктуруСвойствКонтакта(Ссылка, ВидКонтакта) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Ссылка, "CRM_Фамилия") Тогда
		СтруктураОбъекта.Вставить("CRM_Фамилия", Ссылка.CRM_Фамилия);
	КонецЕсли;
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Ссылка, "CRM_Имя") Тогда
		СтруктураОбъекта.Вставить("CRM_Имя", Ссылка.CRM_Имя);
	КонецЕсли;
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Ссылка, "CRM_Отчество") Тогда
		СтруктураОбъекта.Вставить("CRM_Отчество", Ссылка.CRM_Отчество);
	КонецЕсли;
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Ссылка, "Наименование") Тогда
		СтруктураОбъекта.Вставить("Наименование", Ссылка.Наименование);
	КонецЕсли;
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Ссылка, "ДатаРождения") Тогда
		СтруктураОбъекта.Вставить("ДатаРождения", Ссылка.ДатаРождения);
	КонецЕсли;
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Ссылка, "Комментарий") Тогда
		СтруктураОбъекта.Вставить("Комментарий", Ссылка.Комментарий);
	КонецЕсли;
	Если ВидКонтакта = "К" Тогда
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Ссылка, "ЮрФизЛицо") Тогда
