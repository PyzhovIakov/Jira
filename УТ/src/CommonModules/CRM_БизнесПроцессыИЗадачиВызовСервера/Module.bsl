...
Функция ЗаблокироватьДанныеДляРедактированияВедущейЗадачи(БизнесПроцесс, ПродолжитьВыполнение = Истина) Экспорт
	...
	Если БизнесПроцесс.КартаМаршрута = Справочники.CRM_КартыМаршрутов.ЗапросНаИзменениеСроков Тогда
		...
		Исключение
			...
			ТекстОшибки = Прав(ТекстОшибки, СтрДлина(ТекстОшибки) - (?(СтрНайти(ТекстОшибки,
				 "по причине:") > 0, СтрНайти(ТекстОшибки, "по причине:") - 1,
				 0)));
...
//
Процедура Выполнить_Задачу(ЗадачаСсылка, ДействиеПоУмолчанию = Ложь,
	 РазрешитьВыполнитьСПодчиненнымиЗадачами = Ложь) Экспорт
	
	Если Не РазрешитьВыполнитьСПодчиненнымиЗадачами И ЕстьПодчиненныеЗадачи(ЗадачаСсылка) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='У задачи';en='At the task'") + " """ 
			+ Строка(ЗадачаСсылка) 
			+ """ "
...
//
Процедура ВыполнитьЗадачу(ЗадачаСсылка, ВариантВыполнения = Неопределено,
	 ОтложитьНаДату = Неопределено, РезультатВыполнения = "",
	...
	Попытка
		...
		Если НЕ ДополнительныеПараметры = Неопределено Тогда
			...
			Если ДополнительныеПараметры.Свойство("ПричинаДосрочногоЗавершения") Тогда
				ЗадачаОбъект.ДополнительныеСвойства.Вставить("ПричинаДосрочногоЗавершения",
					 ДополнительныеПараметры.ПричинаДосрочногоЗавершения);
			КонецЕсли;
		КонецЕсли;
...
//  			физического перенаправления задач, а только проверит возможность перенаправления.
//  МассивПеренаправленныхЗадач	 - Массив -  Перенаправленные задачи.
//  			Может отличаться по составу элементов от массива МассивЗадач, если какие-то задачи
		...
		Если ЗадачаВыполнена Тогда
			...
			Если ТолькоПроверка Тогда
				ПричинаОтказа = НСтр("ru='Невозможно перенаправить уже выполненную задачу.';
					|en='It is impossible to redirect already carried  out task.'");
				Прервать;
			КонецЕсли;
		КонецЕсли;	
		Если НЕ CRM_БизнесПроцессыСервер.ЗадачаДоступнаДляПринятияКИсполнению(Задача.Ссылка) Тогда
			...
			Если ТолькоПроверка Тогда
				ПричинаОтказа = НСтр("ru='Невозможно перенаправить чужую задачу.';
					|en='It is impossible to redirect someone else task.'");
				Прервать;
			КонецЕсли;
		КонецЕсли;
...
Процедура ПринятьЗадачиКИсполнению(Задачи) Экспорт
	...
	Попытка
		...
		Для каждого Задача Из Задачи Цикл
			...
			Если НЕ ЗначениеЗаполнено(ЗадачаОбъект.Исполнитель)
				 ИЛИ НЕ (ЗадачаОбъект.Исполнитель = Пользователи.ТекущийПользователь()) Тогда
...
Функция ФормаВыполненияЗадачи(Знач ЗадачаСсылка, Знач ИзКалендаря = 0) Экспорт
	...
	Если ТипЗнч(ЗадачаСсылка) <> Тип("ЗадачаСсылка.ЗадачаИсполнителя") Тогда
		...
		ТекстСообщения =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Неправильный тип параметра ЗадачаСсылка (передан: %1;
			| ожидается: %2)';en='Invalid type of parameter ЗадачаСсылка (passed: %1; expected: %2)'"),
	...
	Если ТипБизнесПроцесса.Имя = "CRM_БизнесПроцесс" Тогда
		...
		Если НЕ ЗадачаСсылка.CRM_ТочкаМаршрута.Вид = Перечисления.CRM_ВидыТочекМаршрута.ВложенныйБизнесПроцесс Тогда
			...
			ИначеЕсли ЗадачаСсылка.БизнесПроцесс.КартаМаршрута.ТипПроцесса = Перечисления.bpmТипыПроцессов.ПроцессОбъекта
				...
				ПараметрыФормы.ПараметрыФормы.Ключ = ЗадачаСсылка.БизнесПроцесс.Интерес;
				ПараметрыФормы.Вставить("ИмяФормы", "Документ." + ЗадачаСсылка.БизнесПроцесс.Интерес.Метаданные().Имя 
					+ ".ФормаОбъекта");
				ПараметрыФормы.ПараметрыФормы.Вставить("ОткрываемаяЗадача", ЗадачаСсылка);
...
Функция ПроверитьВозможностьПереходаБПИнтереса(Знач Объект, ЗадачаИсполнителя, ВариантВыполнения) Экспорт
	...
	СледующееСостояниеИнтереса = CRM_БизнесПроцессыСервер.ПолучитьСледующееСостояниеОбъектаБП(ЗадачаИсполнителя,
		 ВариантВыполнения);
	Если СледующееСостояниеИнтереса <> Неопределено Тогда
		...
		ПроверяемыеРеквизитыИнтереса =
			CRM_КонтрольЗаполненностиРеквизитовОбъектовПовтИсп.ПроверяемыеРеквизитыИнтересаВСостоянии(СледующееСостояниеИнтереса);
		...
		Если Объект.СостояниеИнтереса.ИспользоватьЧекЛист И Объект.СостояниеИнтереса.ПроверкаВыполненияЧекЛиста
			...
			ТекстСообщенияПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Для изменения состояния %1 необходимо выполнить все задачи чек-листа!'"),
												?(Объект.ЭтоПоддержка, НСтр("ru = 'обращения'"), НСтр("ru = 'интереса'")));
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщенияПользователю, Объект.Ссылка, "Объект.ЧекЛист", "Объект.ЧекЛист");
		КонецЕсли;
...
Процедура ПолучитьТекущиеЗадачиПеренаправитьНаОтветственного(ОбъектБП, СтарыйОтветственный) Экспорт
	...
	ПеренаправитьЗадачи(МассивЗадач, Исполнитель, Ложь, МассивПеренаправленныхЗадач);
...
Функция ПеренестиСобытияЗадачи(СобытияЗадачи, ДатаЗадачи,
	 Знач ВариантЗавершенияСобытий = Неопределено, Знач ДатаПереноса = Неопределено,
...
Функция ВыполнитьПереносЗадачи(ЭтаФормаИлиЗадача, ВариантПереноса, Знач ДатаПереноса = Неопределено,
	 ПроверятьНаРабочийДень = Истина,
...
// BSLLS:WrongUseOfRollbackTransactionMethod-on
		...
		СтрОшибка = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнфОбОшибке);
		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось перенести задачу: ';
			|en='Failed to relocate task:'")
...
Функция ПеренестиЗадачу(ЭтаФормаИлиЗадача, ВариантПереноса, Знач ДатаПереноса = Неопределено,
	 ПроверятьНаРабочийДень = Истина,
	ПереноситьСобытияЗадачи = Ложь, БылиПеренесеныСобытия = Ложь, ЗаписыватьВФорме = Истина) Экспорт
...
Процедура НазначитьРеквизитыЗадачам(Задача, Партнер = Неопределено, КонтактноеЛицо = Неопределено,
	 Проект = Неопределено, ДополнительныеРеквизиты = Неопределено,
	 НазначитьВсемВИерархии = Истина) Экспорт
	...
	Если Не Задача.CRM_Личная И (ТипЗнч(ДополнительныеРеквизиты) <> Тип("Структура")
	...
	Если ТипЗнч(ДополнительныеРеквизиты) = Тип("Структура") И ДополнительныеРеквизиты.Количество() > 0 Тогда
		Для Каждого КлючЗначение Из ДополнительныеРеквизиты Цикл
			...
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЗадачаОбъект, КлючЗначение.Ключ) Тогда 
				ЗадачаОбъект[КлючЗначение.Ключ] = КлючЗначение.Значение;
				БылиИзменения = Истина;                                                                                         
			КонецЕсли;
			
			Если КлючЗначение.Ключ = "CRM_ОсновнаяКатегория" И ЗначениеЗаполнено(КлючЗначение.Значение)
				 И ТипЗнч(КлючЗначение.Значение) = Тип("СправочникСсылка.CRM_КатегорииПользователей") Тогда
	...
	Если НазначитьВсемВИерархии И Задача.CRM_Личная Тогда
		...
		Если ТаблицаПодчиненныеЗадачи.Количество() > 0 Тогда
			Для Каждого СтрокаПодчиненнаяЗадача Из ТаблицаПодчиненныеЗадачи Цикл
				НазначитьРеквизитыЗадачам(СтрокаПодчиненнаяЗадача.Ссылка, Партнер, КонтактноеЛицо, Проект,
					 ДополнительныеРеквизиты,
					 НазначитьВсемВИерархии);
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры

Процедура НазначитьРодителя(Задача, Родитель, Партнер = Неопределено, КонтактноеЛицо = Неопределено,
	 Проект = Неопределено, ДополнительныеРеквизиты = Неопределено,
	 НазначитьВсемВИерархии = Истина) Экспорт
	...
	Если бОшибочныйРодитель Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Нельзя переносить задачу внутрь собственной иерархии';
			|en='It is impossible to transfer the task in own hierarchy'")
			+ ": """ + Строка(Задача) + """!");
		//
		Возврат;
	КонецЕсли;
	...
	Попытка
		...
		Если (ЗначениеЗаполнено(Партнер) Или ЗначениеЗаполнено(КонтактноеЛицо)
			 Или ЗначениеЗаполнено(Проект) Или (ТипЗнч(ДополнительныеРеквизиты) = Тип("Структура")
	...
	Исключение
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось назначить нового родителя задаче: ';
			|en='Failed to appoint new parent for task: '")
			+ """" + Строка(Задача) + """"
			+ ?(ЗначениеЗаполнено(СтрОшибка), ": " + СтрОшибка, "."));
		//
	КонецПопытки;
...
Функция ПолучитьПодчиненныеЗадачиРекурсивно(Задача, ПолучитьПомеченныеНаУдаление = Ложь,
	 ТаблицаЗадач = Неопределено) Экспорт
...
Процедура ПересчитатьПоказателиЗадачиПоПодчиненнымЗадачам(Задача) Экспорт
	...
	Если Не бЕстьПодчиненныеЗадачи Тогда
		Возврат;
	КонецЕсли;
...
Функция ПометитьНаУдаление(Задача, ПометкаУдаления) Экспорт
	Если Задача.ПометкаУдаления = ПометкаУдаления Тогда
		Возврат Истина;
	КонецЕсли;
...
Функция ОпределитьПринадлежностьЗадачи(Задача) Экспорт
	...
	Если ЗначениеЗаполнено(Задача.БизнесПроцесс)
		 И ТипЗнч(Задача.БизнесПроцесс) = Тип("БизнесПроцессСсылка.CRM_БизнесПроцесс") Тогда
...
Процедура ИзменитьСобытие(СобытиеСсылка)  Экспорт
	...
	Исключение
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось изменить состояние,
			| вероятно событие заблокировано';en='It were not possible to change status,
			| possibly event it is disabl'"));	
	КонецПопытки;
...
Процедура ИзменитьМаркетинговоеМероприятие(СправСсылка) Экспорт
	...
	Исключение
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось изменить состояние,
			| вероятно элемент справочника заблокирован';en='Failed to change state,
			| probably catalog item blocked'"));	
	КонецПопытки;
...
Процедура ЗадачаИсполнителяМодульОбъектаОбработкаЗаполнения(Объект, ДанныеЗаполнения) Экспорт
	...
	Или ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ТелефонныйЗвонок")
	...
	ИЛИ ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.CRM_Телемаркетинг")
	ИЛИ ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.CRM_Заявка") Тогда
		...
		Если НЕ ЗначениеЗаполнено(Объект.CRM_Партнер) Тогда
			...
			ИначеЕсли ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеЗаполнения, "CRM_Партнер")
				И ЗначениеЗаполнено(ДанныеЗаполнения.CRM_Партнер) Тогда
		...
		Если НЕ ЗначениеЗаполнено(Объект.CRM_КонтактноеЛицо) Тогда
			...
			ИначеЕсли ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеЗаполнения, "CRM_КонтактноеЛицо")
				И ЗначениеЗаполнено(ДанныеЗаполнения["CRM_КонтактноеЛицо"]) Тогда	
				Объект.CRM_КонтактноеЛицо = ДанныеЗаполнения.CRM_КонтактноеЛицо;
			КонецЕсли;
		КонецЕсли;		
		Если НЕ ЗначениеЗаполнено(Объект.CRM_Проект) Тогда
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеЗаполнения, "Проект")
				И ЗначениеЗаполнено(ДанныеЗаполнения["Проект"]) Тогда
				Объект.CRM_Проект = ДанныеЗаполнения.Проект;
			ИначеЕсли ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеЗаполнения, "CRM_Проект")
				И ЗначениеЗаполнено(ДанныеЗаполнения["CRM_Проект"]) Тогда	
		...
		Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЭлектронноеПисьмоВходящее")
			...
			// В случае писем в описание не переносим текст писем, только тему и информацию, что
		...
		Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ТелефонныйЗвонок") Тогда
			Объект.Наименование = ДанныеЗаполнения.Тема;
			Объект.Описание = "Телефонный звонок: " + ДанныеЗаполнения.Тема + "
			|Введено на основании: " + ДанныеЗаполнения.Ссылка;
		КонецЕсли;
		Объект.Дата = ТекущаяДатаСеанса();
		...
		ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ТелефонныйЗвонок") Тогда
			Если ЗначениеЗаполнено(ДанныеЗаполнения.АбонентКонтакт) Тогда
				Если ТипЗнч(ДанныеЗаполнения.АбонентКонтакт) = Тип("СправочникСсылка.КонтактныеЛицаПартнеров") Тогда
					Объект.CRM_КонтактноеЛицо	= ДанныеЗаполнения.АбонентКонтакт;
					Объект.CRM_Партнер			= Объект.CRM_КонтактноеЛицо.Владелец;
				ИначеЕсли ТипЗнч(ДанныеЗаполнения.АбонентКонтакт) = Тип("СправочникСсылка.Партнеры") Тогда
					Объект.CRM_Партнер			= ДанныеЗаполнения.АбонентКонтакт;
					Объект.CRM_КонтактноеЛицо	= Объект.CRM_Партнер.CRM_ОсновноеКонтактноеЛицо;
				КонецЕсли;
			КонецЕсли;
		ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.CRM_Заявка") Тогда
			
			Объект.Наименование = ДанныеЗаполнения.Тема;
			
			Если Не ЗначениеЗаполнено(Объект.CRM_Партнер) И ТипЗнч(ДанныеЗаполнения.Контакт) = Тип("СправочникСсылка.Партнеры") Тогда
				Объект.CRM_Партнер = ДанныеЗаполнения.Контакт;
			ИначеЕсли Не ЗначениеЗаполнено(Объект.CRM_Партнер) И ТипЗнч(ДанныеЗаполнения.Контакт) = Тип("СправочникСсылка.CRM_ПотенциальныеКлиенты") Тогда
				Объект.CRM_ПотенциальныйКлиент = ДанныеЗаполнения.Контакт;
			ИначеЕсли Не ЗначениеЗаполнено(Объект.CRM_КонтактноеЛицо) И ТипЗнч(ДанныеЗаполнения.Контакт) = Тип("СправочникСсылка.КонтактныеЛицаПартнеров") Тогда
				Объект.CRM_КонтактноеЛицо = ДанныеЗаполнения.Контакт;
				Объект.CRM_Партнер = ДанныеЗаполнения.Контакт.Владелец;
			КонецЕсли;
	...
	КонецЕсли;
	Если ЗначениеЗаполнено(Объект.CRM_ОсновнаяКатегория)
		 И Объект.CRM_Категории.Найти(Объект.CRM_ОсновнаяКатегория, "Категория") = Неопределено Тогда
...
Процедура ЗадачаИсполнителяМодульОбъектаПередЗаписью(Объект, Отказ) Экспорт
	...
	КонецЕсли;
	Если Объект.ДополнительныеСвойства.Свойство("мСоздатьОповещениеОбИзмененииСрокаИсполнения") Тогда
		Объект.ДополнительныеСвойства["мСоздатьОповещениеОбИзмененииСрокаИсполнения"] = Ложь;
	Иначе	
		Объект.ДополнительныеСвойства.Вставить("мСоздатьОповещениеОбИзмененииСрокаИсполнения", Ложь);
	КонецЕсли;	
	...
	Если НЕ ИсходныеРеквизиты.Выполнена И Объект.Выполнена Тогда
		...
		Если Объект.СостояниеБизнесПроцесса = Перечисления.СостоянияБизнесПроцессов.Остановлен Тогда
			ВызватьИсключение НСтр("ru='Нельзя выполнять задачи остановленных бизнес-процессов.';
				|en='It is impossible to carry out of the task of the stop business processes.'");
		КонецЕсли;	
	...
	УстановитьПривилегированныйРежим(Истина);
	БизнесПроцессыИЗадачиСервер.ГруппаИсполнителейЗадач(Объект.РольИсполнителя, 
	...
	Если ЗначениеЗаполнено(Объект.БизнесПроцесс)
		...
		КонецЕсли;
	КонецЕсли;
	Если Объект.СрокИсполнения <> Объект.Ссылка.СрокИсполнения 
		И Не Объект.Исполнитель = Пользователи.ТекущийПользователь() Тогда
		Объект.ДополнительныеСвойства.Вставить("мСоздатьОповещениеОбИзмененииСрокаИсполнения", Истина);
		Объект.ДополнительныеСвойства.Вставить("СтарыйСрокИсполнения", Объект.Ссылка.СрокИсполнения);
	КонецЕсли;
	Если Объект.Выполнена Тогда
	...
	КонецЕсли;
	Если Объект.CRM_Личная И ЗначениеЗаполнено(Объект.CRM_Родитель) И НЕ ИсходныеРеквизиты.Выполнена
		 И НЕ Объект.Выполнена
		 И (Объект.CRM_ПроцентВыполненияЗадачи >= 100) Тогда
...
Процедура ОбновитьСостояниеСтрокой(Объект)
	...
	СтруктураТочки = CRM_БизнесПроцессыЭкспортныеМетоды.ПолучитьИсходящуюТочку(Объект.CRM_ТочкаМаршрута,
		 Объект.БизнесПроцесс.НомерВерсииКартыМаршрута,
		 0);
...
Процедура ЗадачаИсполнителяМодульОбъектаПриЗаписи(Объект, Отказ) Экспорт
	...
	Если НЕ Объект.ДополнительныеСвойства.Свойство("мСоздатьОповещениеОНовойЗадаче") Тогда
		Объект.ДополнительныеСвойства.Вставить("мСоздатьОповещениеОНовойЗадаче", Ложь);
	КонецЕсли;	
	Если НЕ Объект.ДополнительныеСвойства.Свойство("мСоздатьОповещениеОбИзмененииСрокаИсполнения") Тогда
		Объект.ДополнительныеСвойства.Вставить("мСоздатьОповещениеОбИзмененииСрокаИсполнения", Ложь);
	КонецЕсли;	
	
	Если НЕ Отказ И (Объект.ДополнительныеСвойства["мСоздатьОповещениеОНовойЗадаче"] 
		Или Объект.ДополнительныеСвойства["мСоздатьОповещениеОбИзмененииСрокаИсполнения"]) Тогда
		ВидОповещения = ?(ЗначениеЗаполнено(Объект.CRM_НачалоПереадресации),
			 Справочники.CRM_ВидыОповещений.ОповещатьОПереадресованныхДокументахЗадачах,
			 Справочники.CRM_ВидыОповещений.ОповещатьОНовыхЗадачах);
		Если ЗначениеЗаполнено(Объект.РольИсполнителя) И НЕ ЗначениеЗаполнено(Объект.Исполнитель) Тогда
			ПараметрыОповещения = CRM_ОповещенияСервер.ПолучитьПараметрыОповещения(Объект.РольИсполнителя,
				 ВидОповещения,
				 Объект.Ссылка,
				?(Объект.ДополнительныеСвойства["мСоздатьОповещениеОбИзмененииСрокаИсполнения"],
				Объект.ДополнительныеСвойства.СтарыйСрокИсполнения, Неопределено));
		ИначеЕсли ЗначениеЗаполнено(Объект.Исполнитель) Тогда
			ПараметрыОповещения = CRM_ОповещенияСервер.ПолучитьПараметрыОповещения(Объект.Исполнитель,
				 ВидОповещения,
				 Объект.Ссылка,
				?(Объект.ДополнительныеСвойства["мСоздатьОповещениеОбИзмененииСрокаИсполнения"],
				Объект.ДополнительныеСвойства.СтарыйСрокИсполнения, Неопределено));
			...
			Если ЗначениеЗаполнено(Объект.Исполнитель) И ПравоДоступа("Изменение", Объект.Предмет.Метаданные()) Тогда
				Если Объект.Предмет.Ответственный <> Объект.Исполнитель Тогда
					...
					Если бОтветственныйИзменен Тогда
						бОтправитьУведомлениеПредыдущемуОтветственному = ЗавершитьЛичныеЗадачиПоПредметуВПривилигированномРежмиме(Объект.Ссылка,
							 ПредыдущийОтветственный,
							 Объект.Предмет);
						Если бОтправитьУведомлениеПредыдущемуОтветственному Тогда
							ПараметрыОповещения = CRM_ОповещенияСервер.ПолучитьПараметрыОповещения(ПредыдущийОтветственный,
								 Справочники.CRM_ВидыОповещений.ОповещатьОПереадресованныхДокументахЗадачах,
								 Объект.Предмет);
							Если НЕ (ПараметрыОповещения = Неопределено) Тогда
								CRM_ОповещенияСервер.ДобавитьОповещение(ПараметрыОповещения);
	...
	Если НЕ Объект.CRM_Личная И Не Отказ И Объект.ДополнительныеСвойства.Свойство("ЭтоНовый")
			...
			Если ЗначениеЗаполнено(НастройкаЭтапа.Состояние) Тогда
				Попытка
					...
					Если СтрНайти(Объект.БизнесПроцесс.РеквизитОбъектаСтрокой, "%Свойство%") Тогда
						...
						СсылкаНаСвойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.ПолучитьСсылку(Новый УникальныйИдентификатор(СтрЗаменить(Объект.БизнесПроцесс.РеквизитОбъектаСтрокой,
							 "%Свойство%",
							 "")));
						...
						Если МассивСтрок.Количество() > 0 Тогда
							Если МассивСтрок[0].Значение <> НастройкаЭтапа.Состояние Тогда
								...
								ОбъектБП.ДополнительныеСвойства.Вставить("ЗаписьОбъектаУправляющимБП", Истина);
								ОбъектБП.Записать();
							КонецЕсли;
						Иначе
...
Функция ЗадачаДоступнаДляПринятияКИсполнению(Задача) Экспорт
	Возврат CRM_БизнесПроцессыСервер.ЗадачаДоступнаДляПринятияКИсполнению(Задача);
КонецФункции

Функция ПартнерБизнесПроцесса(БизнесПроцесс) Экспорт
...
Функция КонтактноеЛицоБизнесПроцесса(БизнесПроцесс) Экспорт
...
Функция КонтактныеЛицаПартнера(Партнер)
...
Функция УчастникиБизнесПроцесса(БизнесПроцесс) Экспорт
	
	СписокУчастников = Новый СписокЗначений;
	
	Партнер = ПартнерБизнесПроцесса(БизнесПроцесс);
	Если ЗначениеЗаполнено(Партнер) Тогда
		СписокУчастников.Добавить(Партнер);
	КонецЕсли;	
	
	КонтактноеЛицо = КонтактноеЛицоБизнесПроцесса(БизнесПроцесс);
	Если ЗначениеЗаполнено(КонтактноеЛицо) Тогда
		СписокУчастников.Добавить(КонтактноеЛицо);
	Иначе
		Для Каждого КонтактноеЛицо Из КонтактныеЛицаПартнера(Партнер) Цикл
...
Функция ЕстьРеквизитПартнера(ИмяДокумента) Экспорт
...
Процедура ДобавитьЗаписьПоОбъекту(НовыйОбъект, Задача, БизнесПроцесс, ТочкаМаршрута,
	 ДатаРегистрации,
	 ВидОбъекта) Экспорт
	...
	ЗаписьРегистра.ТочкаМаршрута	= ТочкаМаршрута;
	ЗаписьРегистра.Объект			= НовыйОбъект;
	ЗаписьРегистра.ДатаРегистрации	= ДатаРегистрации;
	ЗаписьРегистра.ВидОбъекта		= ВидОбъекта;
	ЗаписьРегистра.Записать();
	
КонецПроцедуры

Процедура УдалитьЗаписьПоОбъекту(Объект, Задача, БизнесПроцесс, ТочкаМаршрута) Экспорт
...
Функция ПроверитьНаличиеОснования(ИмяДокумента, БизнесПроцесс, ДанныеТиповОбъектов) Экспорт
	
	СинонимДокумента	= Метаданные.НайтиПоТипу(Тип("ДокументСсылка." + ИмяДокумента)).Синоним;
	
	СписокОснований = Новый СписокЗначений;
	
	Для Каждого ДанныеТипа Из ДанныеТиповОбъектов Цикл
		Если ДанныеТипа.ВидОбъекта.ТипЗначения.СодержитТип(Тип("ДокументСсылка." + ИмяДокумента)) Тогда
			Если ЗначениеЗаполнено(ДанныеТипа.ЭтапОснования) Тогда
				
				Если ДанныеТипа.ЭтапОснования.Вид = Перечисления.CRM_ВидыТочекМаршрута.Старт Тогда
					
					МассивСсылок = Новый Массив;
					
					Если ЗначениеЗаполнено(БизнесПроцесс.Предмет) Тогда
						МассивСсылок.Добавить(БизнесПроцесс.Предмет);
					Иначе
						МассивСсылок.Добавить(БизнесПроцесс.Ссылка);
					КонецЕсли;
					СписокОснований.Добавить(МассивСсылок, "Основание бизнес-процесса");
				Иначе
					Набор = РегистрыСведений.CRM_ОбъектыПоБизнесПроцессам.СоздатьНаборЗаписей();
					Набор.Отбор.БизнесПроцесс.Установить(БизнесПроцесс);
					Набор.Отбор.ТочкаМаршрута.Установить(ДанныеТипа.ЭтапОснования);
					Набор.Прочитать();
					...
					Если Набор.Количество() > 0 Тогда
						...
						СписокОснований.Добавить(МассивСсылок, НСтр("ru='Документы этапа: ';en='Documents of stage: '") 
							+ ДанныеТипа.ЭтапОснования.Наименование);
					КонецЕсли;
				КонецЕсли;
			Иначе
				Если ЗначениеЗаполнено(БизнесПроцесс.Предмет) Тогда
