...
//
Функция ПолучитьТаблицуПоказателя(Показатель, ДатаРасчета, Менеджер, Подразделение = Неопределено,
	 ЗаДень = Ложь) Экспорт
	
	ПустаяТаблицаПоказателя = Новый ТаблицаЗначений;
	ПустаяТаблицаПоказателя.Колонки.Добавить("Менеджер");
	ПустаяТаблицаПоказателя.Колонки.Добавить("Подразделение");
	...
	Для каждого ВыборЭлемент Из НастройкиСКД.Выбор.Элементы Цикл
		Если ВыборЭлемент.Поле = ПолеПодразделение Тогда
			ЕстьПодразделение = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	Если Не ЕстьПодразделение Тогда
		ПолеПодразделение = НастройкиСКД.Выбор.Элементы.Добавить(Тип("ВыбранноеПолеКомпоновкиДанных"));
		ПолеПодразделение.Использование = Истина;
		ПолеПодразделение.Заголовок = "Подразделение";
		ПолеПодразделение.Поле = Новый ПолеКомпоновкиДанных("Менеджер.Подразделение");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Подразделение) Тогда
		НовыйЭлементОтбора = НастройкиКомпоновщика.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		Если ЕстьПодразделение Тогда
			ПолеОтбора = Новый ПолеКомпоновкиДанных("Подразделение");
		Иначе
			ПолеОтбора = Новый ПолеКомпоновкиДанных("Менеджер.Подразделение");
		КонецЕсли;
	...
	Для Каждого ТекущийПараметрСКД Из СКД.Параметры Цикл
		Если Не ТекущийПараметрСКД.ОграничениеИспользования Тогда
			ПараметрКомпоновки	= Новый ПараметрКомпоновкиДанных(ТекущийПараметрСКД.Имя);
			ЗначениеПараметра	= ПараметрыНастройки.НайтиЗначениеПараметра(ПараметрКомпоновки);
	...
	Если Не ЕстьПодразделение Тогда
		КолонкаПодразделение = ТаблицаРезультат.Колонки.Найти("МенеджерПодразделение");
		Если КолонкаПодразделение <> Неопределено Тогда
			КолонкаПодразделение.Имя = "Подразделение";
		КонецЕсли;
	КонецЕсли;
...
Процедура ПостроитьДиаграммуПоказателя(Показатель, Диаграмма, ТаблицаДанных) Экспорт
	...
	Подразделение = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Менеджер, "Подразделение");
	
	Если ТаблицаДанных = Неопределено Тогда
		ТаблицаДанных = ПолучитьТаблицуПоказателя(Показатель, ДатаРасчета, Менеджер, Подразделение);
	КонецЕсли;
...
Процедура РасчетКлючевыхПоказателей() Экспорт
	...
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(
		Метаданные.РегламентныеЗадания.CRM_РасчетКлючевыхПоказателей);
...
Функция ЗапуститьПересчетПоказателей() Экспорт
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Расчет ключевых показателей';
		|en='Calculation of key indicators'");
...
Функция ЗапуститьПересчетПоказателя(Показатель) Экспорт
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(Новый УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Расчет показателя ';en='Calculation of indicator'") 
		+ Строка(Показатель);
...
Функция ПолучитьПараметрыРасшифровки(Показатель, ПараметрыОтбора) Экспорт
	...
	Если Показатель = Справочники.CRM_ШаблоныКлючевыхПоказателей.ПросроченоВзаимодействий
		...
		Если Показатель = Справочники.CRM_ШаблоныКлючевыхПоказателей.ПросроченоВзаимодействий Тогда
			...
			ПараметрыОтбора.Вставить("ПлановаяДатаЗавершения", Новый Структура("ВидСравнения, ПравоеЗначение",
				ВидСравненияКомпоновкиДанных.Меньше,
				ТекущаяДатаСеанса()));
		Иначе
			ПараметрыОтбора.Вставить("Запланировано", Ложь);
			ПараметрыОтбора.Вставить("ДатаЗавершения", Новый Структура("ВидСравнения, ПравоеЗначение",
				ВидСравненияКомпоновкиДанных.БольшеИлиРавно,
				ПараметрыОтбора.ПериодОтчета.ДатаНачала));
			ПараметрыОтбора.Вставить("ДатаДеньЗавершения", Новый Структура("ВидСравнения, ПравоеЗначение",
				ВидСравненияКомпоновкиДанных.МеньшеИлиРавно,
				ПараметрыОтбора.ПериодОтчета.ДатаОкончания));
		КонецЕсли;
		ПараметрыОтбора.Удалить("ПериодОтчета");
	...
	ИначеЕсли Показатель = Справочники.CRM_ШаблоныКлючевыхПоказателей.КасанияНаОдногоКлиента Тогда
		...
		ПараметрыОтбора.Вставить("ДатаСледующегоДействия", Новый Структура("ВидСравнения, ПравоеЗначение",
			 ВидСравненияКомпоновкиДанных.Меньше,
			 ТекущаяДатаСеанса()));
	...
	ИначеЕсли Показатель = Справочники.CRM_ШаблоныКлючевыхПоказателей.ЗабытыеКлиенты Тогда
		...
		ПараметрыОтбора.Вставить("ЭтоКлиент", Истина);
		Если ПараметрыОтбора.Свойство("Менеджер") Тогда
			ПараметрыОтбора.Вставить("МенеджерКлиенты", ПараметрыОтбора.Менеджер);
			ПараметрыОтбора.Удалить("Менеджер");
		КонецЕсли;
		Если ПараметрыОтбора.Свойство("Подразделение") Тогда
			ПараметрыОтбора.Вставить("ПодразделениеКлиенты", ПараметрыОтбора.Подразделение);
			ПараметрыОтбора.Удалить("Подразделение");
		КонецЕсли;
		Если ПараметрыОтбора.Свойство("ПериодОтчета") Тогда
			ПараметрыОтбора.Удалить("ПериодОтчета");
		КонецЕсли;
	...
	ИначеЕсли Показатель = Справочники.CRM_ШаблоныКлючевыхПоказателей.Взаимодействия Тогда
		...
		Если ПараметрыОтбора.Свойство("Менеджер") Тогда
			ПараметрыОтбора.Вставить("МенеджерПоВзаимодействиям", ПараметрыОтбора.Менеджер);
		КонецЕсли;
		Если ПараметрыОтбора.Свойство("Подразделение") Тогда
			ПараметрыОтбора.Вставить("ПодразделениеПоВзаимодействиям", ПараметрыОтбора.Подразделение);
		КонецЕсли;
	...
	ИначеЕсли Показатель = Справочники.CRM_ШаблоныКлючевыхПоказателей.ИсходящиеПисьма
		...
		Если ПараметрыОтбора.Свойство("Менеджер") Тогда
			ПараметрыОтбора.Вставить("Ответственный", ПараметрыОтбора.Менеджер);
			ПараметрыОтбора.Удалить("Менеджер");
		КонецЕсли;
		Если Показатель = Справочники.CRM_ШаблоныКлючевыхПоказателей.ИсходящиеПисьма Тогда
			ПараметрыОтбора.Вставить("ТипОбъекта",
				 ПланыВидовХарактеристик.CRM_ОбъектыЖурналаДокументов.НайтиПоНаименованию(Метаданные.Документы.ЭлектронноеПисьмоИсходящее.Синоним));
			ПараметрыОтбора.Вставить("СтатусПисьма", Новый Структура("ПолеКомпоновкиДанных, ВидСравнения,
				| ПравоеЗначение", "Объект.СтатусПисьма",
		...
		ИначеЕсли Показатель = Справочники.CRM_ШаблоныКлючевыхПоказателей.ИсходящиеСообщения Тогда
			ПараметрыОтбора.Вставить("ТипОбъекта",
				 ПланыВидовХарактеристик.CRM_ОбъектыЖурналаДокументов.НайтиПоНаименованию("Сообщение"));
	...
	ИначеЕсли Показатель = Справочники.CRM_ШаблоныКлючевыхПоказателей.ИнтересовВРаботе
		...
		Если Показатель = Справочники.CRM_ШаблоныКлючевыхПоказателей.УспешныхИнтересов
			...
			ПараметрыОтбора.Вставить("ДатаЗакрытия", Новый Структура("ВидСравнения, ПравоеЗначение",
				ВидСравненияКомпоновкиДанных.БольшеИлиРавно,
				ПараметрыОтбора.ПериодОтчета.ДатаНачала));
			ПараметрыОтбора.Вставить("ВРаботе", Ложь);
			ПараметрыОтбора.Вставить("ВидСостояния", Новый Структура("ПолеКомпоновкиДанных, ВидСравнения,
				| ПравоеЗначение", "СостояниеИнтереса.ВидСостояния",
																ВидСравненияКомпоновкиДанных.Равно, Перечисления.CRM_ВидыСостоянияИнтереса.УспешноеЗавершение));
		Иначе
			ПараметрыОтбора.Вставить("ДатаСоздания", Новый Структура("ВидСравнения, ПравоеЗначение",
				ВидСравненияКомпоновкиДанных.БольшеИлиРавно,
				ПараметрыОтбора.ПериодОтчета.ДатаНачала));
		КонецЕсли;
		ПараметрыОтбора.Удалить("ПериодОтчета");
	...
	ИначеЕсли Показатель = Справочники.CRM_ШаблоныКлючевыхПоказателей.ТелефонныеЗвонки
		...
		Если ПараметрыОтбора.Свойство("Менеджер") Тогда
			ПараметрыОтбора.Вставить("Сотрудник", ПараметрыОтбора.Менеджер);
			ПараметрыОтбора.Удалить("Менеджер");
		КонецЕсли;
		...
		Если Показатель = Справочники.CRM_ШаблоныКлючевыхПоказателей.ТелефонныеЗвонки Тогда
			ПараметрыОтбора.Вставить("ДлительностьЗвонка", 20);
		Иначе
			ПараметрыОтбора.Вставить("ДлительностьЗвонка", 1);
		КонецЕсли;
		
		ПараметрыОтбора.Вставить("Показатель", Показатель);
		
	ИначеЕсли Показатель = Справочники.CRM_ШаблоныКлючевыхПоказателей.Продажи Тогда
		Если CRM_ОбщегоНазначенияПовтИсп.ЭтоCRM() Тогда
			ПараметрыФормы.Вставить("КлючВарианта", "ПоДокументамПродажи");
			ФормаРасшифровки = "Отчет.CRM_АнализПродажПланФакт.Форма";
		Иначе
			ПараметрыФормы.Вставить("КлючВарианта", "ПродажиПоПартнерамПоОрганизациям");
			ФормаРасшифровки = "Отчет.ВыручкаИСебестоимостьПродаж.Форма";
			ПараметрыОтбора.Вставить("Период", ПараметрыОтбора.ПериодОтчета);
			ПараметрыОтбора.Удалить("ПериодОтчета");
		КонецЕсли;
	Иначе
	...
	Возврат Новый Структура("Форма, ИмяПредопределенных, ПараметрыФормы, ПараметрыОтбора",
		 ФормаРасшифровки, Показатель.ИмяПредопределенныхДанных, ПараметрыФормы,
		 ПараметрыОтбора);
КонецФункции

Функция ПолучитьТаблицуПланаПоказателя(Показатель, ДатаРасчета, Менеджер, Подразделение = Неопределено,
	 ЗаДень = Ложь) Экспорт
	...
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ИспользуемыеПоказатели.Менеджер.Подразделение В ИЕРАРХИИ (&Подразделение)
	|		КОНЕЦ
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура НастроитьВариантыОтчетов(Настройки) Экспорт
	...
	НастройкиВарианта = ВариантыОтчетов.ОписаниеВарианта(Настройки,
		 Метаданные.Отчеты.CRM_АнализАктивныхПоказателей,
		 "ОтчетПоСотрудникам");
...
Функция ПолучитьТаблицуРабочихДней(ДатаРасчета, Период, Подразделение) Экспорт
	...
	Возврат CRM_ЛицензированиеЭкспортныеМетоды.ПолучитьЗащищеннуюОбработку().ПолучитьТаблицуРабочихДней(Истина,
		 ДатаРасчета, Период,
		 Подразделение);
...
Функция ДоступныеПользователи(ТолькоТекущий = Истина) Экспорт
	...
	Возврат CRM_ЛицензированиеЭкспортныеМетоды.ПолучитьЗащищеннуюОбработку().ПолучитьМассивПользователей(Истина,
		 ТолькоТекущий);
	
КонецФункции

Функция ПолучитьДинамикуПоказателя(Показатель, ДатаРасчета, Менеджер = Неопределено,
	 Подразделение = Неопределено) Экспорт
	
	Возврат CRM_ЛицензированиеЭкспортныеМетоды.ПолучитьЗащищеннуюОбработку().ПолучитьДинамикуПоказателя(Показатель,
		 ДатаРасчета, "ДП", Менеджер,
		 Подразделение);
	
КонецФункции

Функция ПолучитьЦветПоказателя(Показатель, ДатаРасчета, Менеджер = Неопределено,
	 Подразделение = Неопределено,
	 ТаблицаДанных = Неопределено) Экспорт
	
	Возврат CRM_ЛицензированиеЭкспортныеМетоды.ПолучитьЗащищеннуюОбработку().ПолучитьЦветПоказателя(Истина,
		 Показатель, ДатаРасчета, Менеджер, Подразделение,
		 ТаблицаДанных);
