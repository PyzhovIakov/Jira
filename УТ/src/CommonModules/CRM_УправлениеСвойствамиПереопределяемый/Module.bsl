...
Функция ПолучитьНаборыСвойствДляКартМаршрута(Объект) Экспорт
	...
	НаборСвойств = УправлениеСвойствами.НаборСвойствПоИмени("Справочник_CRM_КартыМаршрутов_Общие");
...
Функция ПолучитьНаборыСвойствДляПартнеров(ВидПартнера) Экспорт
	...
	Если CRM_ОбщегоНазначенияПовтИсп.ЭтоCRM() Тогда
		Наборы.Добавить(УправлениеСвойствами.НаборСвойствПоИмени("Справочник_Партнеры_Общие"),
			 НСтр("ru='Общие для всех партнеров';en='Common for all partners'"));
	КонецЕсли;
	Если ВидПартнера = Перечисления.КомпанияЧастноеЛицо.Компания Тогда
		Наборы.Добавить(УправлениеСвойствами.НаборСвойствПоИмени("Справочник_Партнеры_Компании_CRM"),
			    НСтр("ru='Для компаний';en='For the companies'"));
	ИначеЕсли ВидПартнера = Перечисления.КомпанияЧастноеЛицо.ЧастноеЛицо Тогда
		Наборы.Добавить(УправлениеСвойствами.НаборСвойствПоИмени("Справочник_Партнеры_ЧастныеЛица_CRM"),
			 НСтр("ru='Для частных лиц';en='For private persons'"));
	КонецЕсли;
	Возврат Наборы;
...
Функция ПолучитьНаборыСвойствНоменклатуры(НоменклатурнаяГруппа = Неопределено) Экспорт
	Наборы = Новый СписокЗначений;
	Наборы.Добавить(УправлениеСвойствами.НаборСвойствПоИмени("Справочник_Номенклатура"),
		 НСтр("ru='Общие для номенклатуры';en='Nomenclature common'"));
	Если ЗначениеЗаполнено(НоменклатурнаяГруппа) Тогда
		Наборы.Добавить(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НоменклатурнаяГруппа,
			 "НаборСвойствХарактеристики"),
			 НСтр("ru='Для номенклатурной группы';en='For nomenclature group'"));
	КонецЕсли;
	Возврат Наборы;
КонецФункции // ПолучитьНаборыСвойствДляПартнеров()
...
Функция ЗаполнитьДеревоЗначенийСвойств(Ссылка, ДополнительныеРеквизиты, ДляДополнительныхРеквизитов, Наборы) Экспорт
	...
	Иначе
		...
		КонецЕсли;
	КонецЕсли;
	Дерево = ПолучитьДеревоДляРедактированияЗначенийСвойств(спсНаборов, ДополнительныеРеквизиты,
		 ДляДополнительныхРеквизитов);
	Возврат Дерево;
...
// BSLLS:LogicalOrInTheWhereSectionOfQuery-on	
	...
	КонецЦикла;
	СкопироватьДеревоЗначенийСтрок(новДерево.Строки, Дерево.Строки,
