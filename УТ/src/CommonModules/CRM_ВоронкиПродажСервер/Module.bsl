	...
	Для Каждого ЭлементЭтапа Из ЭтапВоронки.Состав Цикл
		...
		Для Каждого ТипДокумента Из МассивТипов Цикл
			...
			ИначеЕсли ТипДокумента = Тип("ДокументСсылка.CRM_Телемаркетинг") Тогда
				...
				|	CRM_ТелемаркетингУчастники.Ссылка.Проект КАК Проект,
				|	1 КАК Количество,
				|	0 КАК Сумма,
				|	&ВалютаРегламентированногоУчета КАК Валюта,
			...
			ИначеЕсли ТипДокумента = Тип("ДокументСсылка.КоммерческоеПредложениеКлиенту") Тогда
				...
				|ГДЕ
				|	НЕ КоммерческоеПредложениеКлиентуТовары.Ссылка.ПометкаУдаления
				|	И КоммерческоеПредложениеКлиентуТовары.Ссылка.Дата МЕЖДУ &НачалоПериода И &КонецПериода
...
Функция ЭтоТипОбъектаВоронкиПродаж(Тип) Экспорт
	...
	СписокИсключаемыхТипов = ВоронкаПродажСписокИсключаемыхТипов();
...
// Функция возвращает список типов объектов, не принадлежащих воронке продаж.
// 
// Возвращаемое значение:
//   - СписокЗначений
//
Функция ВоронкаПродажСписокИсключаемыхТипов() Экспорт
	
	СписокИсключаемыхТипов = Новый СписокЗначений;
	СписокИсключаемыхТипов.Добавить(Тип("СправочникСсылка.МаркетинговыеМероприятия"));
	СписокИсключаемыхТипов.Добавить(Тип("СправочникСсылка.ДоговорыКонтрагентов"));
	СписокИсключаемыхТипов.Добавить(Тип("СправочникСсылка.CRM_ЖурналыВстреч"));
	...
	Если Не CRM_ОбщегоНазначенияПовтИсп.ЭтоCRM() Тогда
		СписокИсключаемыхТипов.Добавить(Тип("СправочникСсылка." + "Претензии"));
		СписокИсключаемыхТипов.Добавить(Тип("СправочникСсылка." + "СделкиСКлиентами"));
	КонецЕсли;
	
	Возврат СписокИсключаемыхТипов;
...
Процедура ПриЗаписиОбъектаВоронкиПродаж(Источник, Отказ) Экспорт
	...
	Если ТипЗнч(Источник.Ссылка) = Тип("ДокументСсылка.CRM_Интерес") Тогда
		...
		Если ЗначениеЗаполнено(Источник.СостояниеИнтереса) Тогда
			...
			Если ТипЗнч(Источник) = Тип("ДокументОбъект.CRM_Интерес")
				 И Источник.ДополнительныеСвойства.Свойство("ОчиститьСостоянияВоронкиПродаж") Тогда
	...
	ИначеЕсли ТипЗнч(Источник.Ссылка) = Тип("ЗадачаСсылка.ЗадачаИсполнителя")
		...
		Если Не CRM_ВоронкиПродажПовтИсп.ОбъектВключенВЭтапВоронки(Источник.Ссылка) Тогда
			Возврат;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(Источник.ДатаИсполнения) Тогда
			НоваяЗапись.СостояниеЭтапа = ?(Источник.CRM_Неудача,
				 Перечисления.CRM_СостоянияЭтаповВоронки.ЗавершенНеудачно,
				 Перечисления.CRM_СостоянияЭтаповВоронки.ЗавершенУспешно);
		Иначе	
			НоваяЗапись.СостояниеЭтапа	= Перечисления.CRM_СостоянияЭтаповВоронки.Активный;
		КонецЕсли;
	...
	ИначеЕсли CRM_ВоронкиПродажПовтИсп.ОбъектВключенВЭтапВоронки(Источник.Ссылка) Тогда
		...
		ЭтапВоронки = CRM_ВоронкиПродажПовтИсп.ПолучитьСсылкуПВХВидыОбъектовБизнесПроцессовПоТипу(ТипЗнч(Источник.Ссылка));
		...
		ИначеЕсли ТипЗнч(Источник.Ссылка) = Тип("ДокументСсылка.CRM_Телемаркетинг") Тогда
			...
			СтруктураЗаписи.Вставить("Этап", ЭтапВоронки);
		...
		ИначеЕсли CRM_ОбщегоНазначенияПовтИсп.ЭтоCRM() И ТипЗнч(Источник.Ссылка) = Тип("ДокументСсылка." 
			+ "CRM_СчетНаОплатуПокупателю") Тогда
			
			НаборЗаписей = РегистрыСведений.CRM_СостоянияВоронкиПродаж.СоздатьНаборЗаписей();
			НаборЗаписей.Отбор.Объект.Установить(Источник.Ссылка);
			
			ЭтапВоронкиПродаж = ПолучитьВидОбъектаБизнесПроцессаПоТипу(ТипЗнч(Источник.Ссылка));
			...
			СтруктураЗаписи.Вставить("Этап", ЭтапВоронкиПродаж);
...
// Процедура заполняет и проверяет корректность элементов ПВХ CRM_ВидыОбъектовБизнесПроцессов,
// вызывается при обновлении конфигурации.
//
// Параметры:
//	ПерезаписыватьНаименования	- Булево	- Признак перезаписи наименования.
//
Процедура ЗаполнитьПроверитьПВХВидыОбъектовБизнесПроцессов(ПерезаписыватьНаименования = Ложь) Экспорт
	Типы = Метаданные.ПланыВидовХарактеристик.CRM_ВидыОбъектовБизнесПроцессов.Тип.Типы();
	Запрос = Новый Запрос(
	"ВЫБРАТЬ  
	...
	Если МассивТиповДляСоздания.Количество() > 0 Тогда
		Для Каждого Тип Из МассивТиповДляСоздания Цикл
			...
			Объект = ПланыВидовХарактеристик.CRM_ВидыОбъектовБизнесПроцессов.СоздатьЭлемент();
	...
	Для Каждого Тип Из МассивДублирующихсяТипов Цикл
		...
		CRM_ВоронкиПродажПовтИсп.ПолучитьСсылкуПВХВидыОбъектовБизнесПроцессовПоТипу(Тип, , МассивДублирующихсяСсылок);
...
// Функция возвращает вид объекта бизнес-процесса по переданному типу.
//
// Параметры:
//  ТипОбъектаБП - Тип	 - Тип объекта бизнес-процесса.
// 
// Возвращаемое значение:
//   - ПланВидовХарактеристикСсылка.CRM_ВидыОбъектовБизнесПроцессов
//
Функция ПолучитьВидОбъектаБизнесПроцессаПоТипу(ТипОбъектаБП) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.ТипЗначения.СодержитТип(ТипОбъектаБП) Тогда
				...
				Для Каждого ЭлементЭтапа Из ЭтапВоронки.Состав Цикл
					...
					Для Каждого ТипДокумента Из МассивТипов Цикл
						...
						Если ТипДокумента = Тип("ДокументСсылка.CRM_РассылкаЭлектронныхПисем") Тогда
							...
							|", "") 
																				+ "ВЫБРАТЬ
							                                                      ...
							                                                      |	ПартнерыСегмента.Сегмент КАК Сегмент,
							                                                      |	CRM_РассылкаЭлектронныхПисем.Дата КАК Дата
							                                                      |ИЗ
						...
						ИначеЕсли ТипДокумента = Тип("ДокументСсылка.CRM_Телемаркетинг") Тогда
							...
							|", "") 
																				+ "ВЫБРАТЬ
							                                                      |	CRM_Телемаркетинг.Ссылка КАК Ссылка,
							                                                      |	CRM_ТелемаркетингУчастники.Партнер КАК Партнер,
							                                                      ...
							                                                      |	ПартнерыСегмента.Сегмент КАК Сегмент,
							                                                      |	CRM_Телемаркетинг.Дата КАК Дата
							                                                      |ИЗ
...
Функция ВоронкаПродажПолучитьТекстЗапросаРасшифрокиПоСостояниямИнтересов() Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВсеИнтересы.Этап КАК Этап,
	|	ВсеИнтересы.Пользователь КАК Пользователь,
	|	ВсеИнтересы.Подразделение КАК Подразделение,
	|	ВсеИнтересы.Документ КАК Документ,
	|	ВсеИнтересы.ТипУслуги КАК ТипУслуги,
	|	ВсеИнтересы.Клиент КАК Клиент,
	|	ВсеИнтересы.БизнесРегион КАК БизнесРегион,
	|	ВсеИнтересы.Проект КАК Проект,
	|	ВсеИнтересы.Количество КАК Количество,
	|	ВсеИнтересы.Сумма КАК Сумма,
	|	ВсеИнтересы.Дата КАК Дата
	|ИЗ
