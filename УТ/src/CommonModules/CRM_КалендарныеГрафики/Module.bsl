...
//
Функция ПолучитьМассивДатПоКалендарю(Знач Календарь, Знач ДатаОт, Знач МассивДней,
	 Знач РассчитыватьСледующуюДатуОтПредыдущей = Ложь,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ГрафикЗаГод.Год КАК Год,
	|	ГрафикЗаГод.КоличествоДнейВГрафикеСНачалаГода КАК КоличествоДнейВГрафикеСНачалаГода,
	|	КоличествоРабочихДнейВГоду.КоличествоДнейВсего КАК КоличествоДнейВсего,
	|	ГрафикЗаГод.ДеньВключенВГрафик КАК ДеньВключенВГрафик
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	КалендарныйГрафик.ИндексСтроки КАК ИндексСтроки,
	|	КалендарныйГрафик.КоличествоДней КАК КоличествоДней,
	...
	|					ТОГДА ВЫБОР
	|							КОГДА КалендарныйГрафик.ДеньВключенВГрафик
	...
	|							ИНАЧЕ КалендарныйГрафик.КоличествоДнейВсего - (КалендарныйГрафик.КоличествоДнейВГрафикеСНачалаГода - ТаблицаДат.КоличествоДней) - 1
	|						КОНЕЦ
	|				КОГДА КалендарныйГрафик.КоличествоДнейВГрафикеСНачалаГода - ТаблицаДат.КоличествоДней = 0
	|					ТОГДА 0
	|				КОГДА КалендарныйГрафик.КоличествоДнейВГрафикеСНачалаГода - ТаблицаДат.КоличествоДней < 0
	...
	|							ИНАЧЕ (КалендарныйГрафик.КоличествоДнейВГрафикеСНачалаГода - ТаблицаДат.КоличествоДней) * -1 - 1
	|						КОНЕЦ
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.ДатаПоКалендарю = Неопределено Тогда
			СообщениеОбОшибке =
				НСтр("ru='Необходимо заполнить календарь ""%1"" за указанный год (в разделе Общие настройки)!';en='It is necessary to fill a calendar ""%1"" for the specified year (in section Inline tunings)!'");
...
//
Функция ПолучитьСоответствиеДатРабочихДней(ДатаИлиМассивДат, График = Неопределено,
	 ПолучатьПредшествующие = Ложь) Экспорт
...
//
Функция ПолучитьДатуПоКалендарю(Знач Календарь, Знач ДатаОт, Знач КоличествоДней,
	 ВызыватьИсключение = Истина,
	 Знач ВпередПоКалендарю = Истина) Экспорт
	ДатаОт = НачалоДня(ДатаОт);
