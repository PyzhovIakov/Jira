...
//
Функция НайтиВЭлементахДерева(ЭлементыСтрокДерева, СтруктураПоиска, Рекурсивно = Ложь,
	 МассивНайденныхСтрок = Неопределено) Экспорт
...
//
Процедура СкопироватьОтборКомпоновкиДанных(ЭлементыПриемник, ЭлементыИсточник,
	 ДоступныеПоляОтбора = Неопределено) Экспорт
...
Функция ПолучитьДатуИзСтроки(СтрокаСДатой) Экспорт 
	...
	Если СтрНайти(СтрокаСДатой, ".") > 0 Тогда
		
		Если СтрНайти(СтрокаСДатой, ":") > 0 И СтрНайти(СтрокаСДатой, " ") > 0 Тогда 
	...
	ИначеЕсли СтрНайти(СтрокаСДатой, "/") > 0 Тогда
...
//
Процедура ОбработкаКомандыПользовательскихНастроекДинамическогоСписка(ЭтаФорма,
	 СвойстваДинамическогоСписка,
	 ПризнакИзмененыНастройки = Неопределено) Экспорт
	Если ПризнакИзмененыНастройки = Истина Тогда
		CRM_ОбщегоНазначенияСервер.ПользовательскиеНастройкиСпискаЗаполнитьПодменюНастроек(ЭтаФорма,
			 СвойстваДинамическогоСписка);
	КонецЕсли;
	
	Если ПризнакИзмененыНастройки = Неопределено Или  (ПризнакИзмененыНастройки = Истина
		 И ЭтаФорма[СвойстваДинамическогоСписка.ИмяПеременнойТекущейНастройки] <> "") Тогда
	...
	Если НЕ CRM_ОбщегоНазначенияСервер.ПользовательскиеНастройкиСпискаУстановитьНастройкуПоУмолчанию(СвойстваДинамическогоСписка.ИдентификаторСписка,
		 ЭтаФорма[СвойстваДинамическогоСписка.ИмяПеременнойТекущейНастройки]) Тогда
...
Процедура ПрименитьПользовательскиеНастройкиКДинамическомуСписку(ДинамическийСписок, Настройки,
	...
	Если ТипЗнч(Настройки) = Тип("Строка") Тогда
		Если ИдентификаторСписка = Неопределено Тогда
			Возврат;
		КонецЕсли;
		СтруктураНастройки = CRM_ОбщегоНазначенияСервер.ВосстановитьПользовательскиеНастройкиДинамическогоСписка(ИдентификаторСписка,
			 Настройки);
	Иначе
	...
	Если СтруктураНастройки.Свойство("Отбор") И ТипЗнч(СтруктураНастройки.Отбор) = Тип("ОтборКомпоновкиДанных") Тогда
		Если НЕ ОчищатьОтбор Тогда
			ДинамическийСписок.Отбор.Элементы.Очистить();
		КонецЕсли;
		СкопироватьОтборКомпоновкиДанных(ДинамическийСписок.Отбор.Элементы,
			 СтруктураНастройки.Отбор.Элементы,
			 ДинамическийСписок.Отбор.ДоступныеПоляОтбора);
	КонецЕсли;
	Если СтруктураНастройки.Свойство("Группировка")
		 И ТипЗнч(СтруктураНастройки.Группировка) = Тип("ПоляГруппировкиКомпоновкиДанных") Тогда
		Если НЕ ОчищатьГруппировки Тогда
			ДинамическийСписок.Группировка.Элементы.Очистить();
		КонецЕсли;
		СкопироватьГруппировкуКомпоновкиДанных(ДинамическийСписок.Группировка.Элементы,
			 СтруктураНастройки.Группировка.Элементы);
	КонецЕсли;
	Если СтруктураНастройки.Свойство("УсловноеОформление")
		 И ТипЗнч(СтруктураНастройки.УсловноеОформление) = Тип("УсловноеОформлениеКомпоновкиДанных") Тогда
		Если НЕ ОчищатьУсловноеОформление Тогда
			ДинамическийСписок.УсловноеОформление.Элементы.Очистить();
		КонецЕсли;
		СкопироватьУсловноеОформлениеКомпоновкиДанных(ДинамическийСписок.УсловноеОформление.Элементы,
			 СтруктураНастройки.УсловноеОформление.Элементы);
	КонецЕсли;
...
Функция БыстрыйОтборОпределитьИмяПоляПоЭлементу(Элемент) Экспорт
	...
	ПозицияСпецСтроки = СтрНайти(ИмяЭлемента, СпецСтрока);
...
Процедура БыстрыйОтборПрименитьОтборКСписку(Список, Элемент = Неопределено, ЭтаФорма = Неопределено,
	...
	Если Значение = Неопределено Тогда
		Если ЭтаФорма = Неопределено Или Элемент = Неопределено Тогда
			Возврат;
		КонецЕсли;
		Попытка
...
//
Процедура БыстрыйОтборПрименитьГруппуОтборовКСписку(Список, ЭлементГруппа, ЭтаФорма,
	 Использование = Неопределено,
	 ИгнорироватьПустыеЗначения = Ложь) Экспорт
...
//
Процедура ЗаполнитьСписокВыбораВремени(Список, ВремяНачало = Неопределено,
	 ДобавлятьПредставлениеДлительности = Ложь,
	 УчитыватьДату = Ложь) Экспорт
...
//
Процедура ВыбратьВремяИзСписка(Форма, Знач СтароеЗначение, ЭлементФормы,
	 Знач ВремяНачало = Неопределено, ДобавлятьПредставлениеДлительности = Ложь, ОписаниеОповещения,
	 УчитыватьДату = Ложь) Экспорт
	...
	Если УчитыватьДату Тогда
		...
		КонецЕсли;
	Иначе
		СтароеЗначениеОкругленное = Дата("00010101") 
			+ ( Цел((СтароеЗначение - НачалоДня(СтароеЗначение)) / (30 * 60)) * (30 * 60) );
	КонецЕсли;
...
//
Процедура УстановитьЭлементОтбораКоллекции(КоллекцияЭлементов, ИмяПоля, ПравоеЗначение,
...
//  Родитель			- ГруппаЭлементовОтбораКомпоновкиДанных     - Родитель добавляемой группы отбора.
//
Процедура УстановитьЭлементОтбораСписка(Список, ИмяПоля, ПравоеЗначение,
	 ВидСравнения = Неопределено,
...
Процедура ИзменитьЭлементОтбораГруппыСписка(Группа, ИмяПоля, ПравоеЗначение = Неопределено,
	 Установить = Ложь,
	 ВидСравнения = Неопределено) Экспорт
...
"E-mail addresses must not contain: !#$%^&*()`~|\/=;,'");
	...
	СообщениеСимволыРусскогоАлфавита					=
		НСтр("ru='Адреса электронной почты должны состоять из букв английского алфавита'");
	ПредыдущийСимвол = "";
	
	Пока Индекс <= СтрДлина(СтрокаАдреса) Цикл
		Символ = Сред(СтрокаАдреса, Индекс, 1);
		ЭтоЦифра = (СтрНайти(СимволыЦифры, НРег(Символ)) > 0);
		ЭтоСимволАлфавита = (СтрНайти(СимволыЛатинскогоАлфавита, НРег(Символ)) > 0);
		...
		ИначеЕсли СтрНайти(НедопустимыеСимволы, Символ) > 0 Тогда
			Отказ = Истина;
			Результат.Вставить("Отказ", Отказ);
			Результат.Вставить("Сообщение", СообщениеНедопустимыеСимволы);
			Возврат Результат;
		ИначеЕсли СтрНайти(СимволыРусскогоАлфавита, НРег(Символ)) > 0 Тогда
			Отказ = Истина;
			Результат.Вставить("Отказ", Отказ);
	...
	Если СтрНайти(Домен, "+") > 0 Тогда
		Отказ = Истина;
		СообщениеНедопустимыеСимволыСПлюсом = СтрЗаменить(СообщениеНедопустимыеСимволы, "()", "()+");
		Результат.Вставить("Отказ", Отказ);
		Результат.Вставить("Сообщение", СообщениеНедопустимыеСимволыСПлюсом);
		Возврат Результат;
	КонецЕсли;
...
Функция ПолучитьЛогин(Текст, ТекущийСимвол, Терм_Конец, НедопустимыеСимволы, СимволыРусскогоАлфавита = "")
	...
	Пока ТекущийСимвол <> Терм_Конец Цикл
		...
		Если ТекущийСимвол = " " Тогда
			...
			Возврат СтруктураВозврата;
		КонецЕсли;
		Если СтрНайти(НедопустимыеСимволы, ТекущийСимвол) Тогда
			СтруктураВозврата.КодОшибки = 1;
			Возврат СтруктураВозврата;
		КонецЕсли;
		Если СтрНайти(СимволыРусскогоАлфавита, ТекущийСимвол) Тогда
...
"Адреса электронной почты не должен содержать: !#$%^&*()+`~|\/=;,'");
	...
	// Символы 
	НедопустимыеСимволыДляЛогина	= "!#$%^&*()`~|\/=;,";
	НедопустимыеСимволыДляДомена	= "!#$%^&*()+`~|\/=;,";
	...
	ПозицияНачалаАдреса = СтрНайти(Текст, "<");
	Если ПозицияНачалаАдреса > 0 Тогда
		
		ПозицияОкончанияАдреса = СтрНайти(Текст, ">");
		...
		Текст = ПропуститьПробелыВНачале(Сред(Текст, ПозицияНачалаАдреса + 1,
			 ПозицияОкончанияАдреса - ПозицияНачалаАдреса - 1));
	Иначе
		ПозицияОкончанияАдреса = СтрНайти(Текст, ">");
	...
	СтрЛогин = ПолучитьЛогин(Текст, ТекущийСимвол, Терм_Домен, НедопустимыеСимволыДляЛогина, СимволыРусскогоАлфавита);
	Если СтрЛогин.КодОшибки = 0 Тогда
		// Все в порядке продолжаем
		// Пытаемся получить домены
		ВозврСтруктура.Логин = СтрЛогин.Логин;
		СтрДомены = ПолучитьДомены(Текст, ТекущийСимвол, "", НедопустимыеСимволыДляДомена, СимволыРусскогоАлфавита);
...
Функция АнализСтрокиАдресовЭП(Знач СтрокаАдресов, СимволРазделитель = ",", ЕстьОшибки) Экспорт
	...
	Для Каждого Адрес Из МассивПодстрок Цикл
		...
		Иначе
			...
			ИмяПартнера = ПолучитьИмяПартнераИзАдресаЭП(СборнаяСтрокаАдреса);
			СтруктураВозврата.Вставить("ИмяПартнера", ИмяПартнера);
...
//
Функция ПолучитьДоменныйАдресДляПоиска(Адрес) Экспорт
...
//
Функция НастройкиПолейОтображенияСодержанияПолучитьПустоеСодержание(СсылкаНаОбъект = Неопределено,
	 ТекстКомментария = "") Экспорт
	...
	ПутьККартинке = ПоместитьВоВременноеХранилище(
		БиблиотекаКартинок.CRM_НастройкиРабочегоСтола, Новый УникальныйИдентификатор);
	...
	|<title></title>
	|</head>
	|<body><div id=""main-container"">" + ТекстКомментария + ГиперссылкаНастройки + "</div></body>
	|</html>";
...
// Функция возвращает структуру цвета по ключу.
//
// Параметры:
//	Ключ - Число | Строка | Цвет - Ключ по которому будет получаться цвет.
//
// Возвращаемое значение:
//	Структура{
...
Функция ПолучитьЦветПоКлючу(Ключ) Экспорт
	...
	Если ТипЗнч(Ключ) = Тип("Число") Тогда
		Возврат Цвета.Получить(Ключ);
	ИначеЕсли ТипЗнч(Ключ) = Тип("Строка") Тогда
		Если СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Ключ) Тогда
			ЧислоКлюча = Число(Ключ);
			Возврат Цвета.Получить(ЧислоКлюча);
		КонецЕсли;
	ИначеЕсли ТипЗнч(Ключ) = Тип("Цвет") Тогда
		Для Каждого ТекущийЦветОбхода Из Цвета Цикл
			Если ТекущийЦветОбхода.Цвет = Ключ Тогда
				Возврат ТекущийЦветОбхода
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция ПолучитьСписокЦветов() Экспорт
	...
	сЦвет.Вставить("Цвет", Новый Цвет(250, 250, 250));
...
// Функция возвращает цвет преобразованный из строки.
// СтрокаRGB - Строка - "145;233;122".
// Возвращаемое значение: Цвет
// }
Функция ЦветИзСтрокиRGB(СтрокаRGB) Экспорт
	Если ЗначениеЗаполнено(СтрокаRGB) Тогда
		МассивRGB = СтрРазделить(СтрокаRGB, ";");
		Если МассивRGB.Количество() = 3 Тогда
			Цвет = Новый Цвет(Число(МассивRGB[0]), Число(МассивRGB[1]), Число(МассивRGB[2]));
		КонецЕсли;
	Иначе
...
Процедура НайтиРекурсивно(КоллекцияЭлементов, МассивЭлементов, СпособПоиска, ЗначениеПоиска)
	...
	Для каждого ЭлементОтбора Из КоллекцияЭлементов Цикл
		...
		Если ТипЗнч(ЭлементОтбора) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
			...
			КонецЕсли;
		// +CRM
		ИначеЕсли ТипЗнч(ЭлементОтбора) = Тип("ЭлементПорядкаКомпоновкиДанных") Тогда
			
			Если СпособПоиска = 1 Тогда
				Если ЭлементОтбора.Поле = ЗначениеПоиска Тогда
					МассивЭлементов.Добавить(ЭлементОтбора);
				КонецЕсли;
			ИначеЕсли СпособПоиска = 2 Тогда
				Если ЭлементОтбора.Представление = ЗначениеПоиска Тогда
...
#Область ОбновлениеТаблицыФормы
...
// Процедура развернуть строки таблицы формы.
//
// Параметры:
//  Элемент			 - ТаблицаФормы	 - Таблица формы.
//  Строка			 - Произвольный	 - Идентификатор строки таблицы.
//  СПодчиненными	 - Булево	 - Признак необходимости развернуть строки таблицы.
//
Процедура РазвернутьТаблицуФормы(Элемент, Строка, СПодчиненными = Ложь) Экспорт
	
	Если ТипЗнч(Элемент) = Тип("ТаблицаФормы") Тогда
		// BSLLS:MissingCodeTryCatchEx-off
		Попытка
			Элемент.Развернуть(Строка, СПодчиненными);
		Исключение
			// Обработка исключения не требуется
		КонецПопытки;
		// BSLLS:MissingCodeTryCatchEx-on
...
// Процедура свернуть строки таблицы формы.
//
// Параметры:
//  Элемент			 - ТаблицаФормы	 - Таблица формы.
//  Строка			 - Произвольный	 - Идентификатор строки таблицы.
//
Процедура СвернутьТаблицуФормы(Элемент, Строка) Экспорт
...
Процедура УстановитьЭлементПорядкаДинамическогоСписка(ДинамическийСписок, ИмяПоля,
	...
	Если РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный Тогда
		ПорядокДинамическогоСписка = ДинамическийСписок.КомпоновщикНастроек.ФиксированныеНастройки.Порядок;
	Иначе
		ПорядокДинамическогоСписка = ДинамическийСписок.КомпоновщикНастроек.Настройки.Порядок;
	КонецЕсли;
	
	УстановитьЭлементПорядка(
		ПорядокДинамическогоСписка,
		ИмяПоля,
		ТипУпорядочивания,
		Представление,
		Использование,
...
Процедура УстановитьЭлементПорядка(ОбластьПоискаДобавления,
								Знач ИмяПоля,
								Знач ТипУпорядочивания = Неопределено,
								Знач Представление = Неопределено,
								Знач Использование = Неопределено,
								Знач РежимОтображения = Неопределено) Экспорт
	
	ЧислоИзмененных = ИзменитьЭлементыПорядка(ОбластьПоискаДобавления, ИмяПоля, ТипУпорядочивания,
	...
	Если ЧислоИзмененных = 0 Тогда
		...
		Если РежимОтображения = Неопределено Тогда
			РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный;
		КонецЕсли;
		ДобавитьЭлементПорядкаКомпоновки(ОбластьПоискаДобавления, ИмяПоля, ТипУпорядочивания,
			Представление, Использование, РежимОтображения);
	КонецЕсли;
	
КонецПроцедуры

Функция ИзменитьЭлементыПорядка(ОбластьПоиска,
								Знач ИмяПоля = Неопределено,
								Знач ТипУпорядочивания = Неопределено,
								Знач Представление = Неопределено,
								Знач Использование = Неопределено,
	...
	Для Каждого Элемент Из МассивЭлементов Цикл
		...
		КонецЕсли;
		Если ТипУпорядочивания <> Неопределено Тогда
			Элемент.ТипУпорядочивания = ТипУпорядочивания;
		КонецЕсли;
		Если РежимОтображения <> Неопределено Тогда
			Элемент.РежимОтображения = РежимОтображения;
		КонецЕсли;
...
Функция ДобавитьЭлементПорядкаКомпоновки(ОбластьДобавления,
									Знач ИмяПоля,
									Знач ТипУпорядочивания,
									Знач Представление  = Неопределено,
									Знач Использование  = Неопределено,
									знач РежимОтображения = Неопределено) Экспорт
	
	Элемент = ОбластьДобавления.Элементы.Добавить(Тип("ЭлементПорядкаКомпоновкиДанных"));
	Элемент.Поле = Новый ПолеКомпоновкиДанных(ИмяПоля);
...
Функция ПолучитьИмяПартнераИзАдресаЭП(СтрокаАдресаЭП)
	
	ИмяПартнера = "";
	Если СтрНайти(СтрокаАдресаЭП, "(") > 0 И СтрНайти(СтрокаАдресаЭП, ")") > 0 Тогда
		НачалоИмяПартнера = СтрНайти(СтрокаАдресаЭП, "(") + 1;
		КонецИмяПартнера  = СтрНайти(СтрокаАдресаЭП, ")");
		ЧислоСимволов     = КонецИмяПартнера - НачалоИмяПартнера;
		
		ИмяПартнера = Сред(СтрокаАдресаЭП, НачалоИмяПартнера, ЧислоСимволов);
	КонецЕсли;
