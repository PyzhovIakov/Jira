...
Процедура ПередЗавершениемРаботыСистемы(Отказ) Экспорт
	...
	Если ТекущийСоставОткрытыхФорм.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого ОткрытоеОкно Из ОткрытыеОкна Цикл
		...
		Если ТипЗнч(УпрФорма) <> Тип("ФормаКлиентскогоПриложения") Тогда
			Продолжить;
		КонецЕсли;
		
		// Если у формы определен владелец, значит форма открыта из другой формы как вспомогательная - пропускаем.
		Если УпрФорма.ВладелецФормы <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(УпрФорма, "ИмяФормы") Тогда
			ИмяФормы = УпрФорма.ИмяФормы;
		Иначе
			Продолжить;
		КонецЕсли;
		...
		Если ИмяФормы = "ОбщаяФорма.ПанельОтчетов" Тогда
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(УпрФорма, "Подсистемы") Тогда
				Параметры.Вставить("Подсистемы", УпрФорма.Подсистемы);
			КонецЕсли;
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(УпрФорма, "ИмяФормыОтчетов") Тогда
				Параметры.Вставить("ИмяФормы", УпрФорма.ИмяФормыОтчетов);
			КонецЕсли;
		КонецЕсли;
...
Процедура ПриНачалеРаботыСистемы() Экспорт
	...
	Для Каждого ОписаниеФормы Из ТекущийСоставОткрытыхФорм Цикл
		Если ТипЗнч(ОписаниеФормы) <> Тип("Структура") Тогда
			Продолжить;
		КонецЕсли;
		Если Не ОписаниеФормы.Свойство("ИмяФормы") Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ОписаниеФормы.Свойство("Параметры") И ТипЗнч(ОписаниеФормы.Параметры) = Тип("Структура") Тогда
			...
			Если ОписаниеФормы.ИмяФормы = "ОбщаяФорма.ПанельОтчетов" Тогда
				Попытка
					Форма = ПолучитьФорму(ОписаниеФормы.ИмяФормы, ОписаниеФормы.Параметры, , Истина);
				
				Исключение	Продолжить;
				КонецПопытки;
			Иначе
				Попытка
					Форма = ПолучитьФорму(ОписаниеФормы.ИмяФормы, ОписаниеФормы.Параметры);
		...
		Иначе
			Попытка
				Форма = ПолучитьФорму(ОписаниеФормы.ИмяФормы, Новый Структура("_НачалоРаботыСистемы"));
