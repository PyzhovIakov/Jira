...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники,
	...
	ФормыИсключенияКоманд = Новый Массив;
	ФормыИсключенияКоманд.Добавить("Справочник.Партнеры.Форма.CRM_ФормаЭлементаНовая");
	ФормыИсключенияКоманд.Добавить("Справочник.Партнеры.Форма.CRM_ФормаСписка");
	ФормыИсключенияКоманд.Добавить("Справочник.Партнеры.Форма.CRM_ФормаСпискаМобильныйКлиент");
	ФормыИсключенияКоманд.Добавить("Справочник.Партнеры.Форма.CRM_Модуль_ФормаЭлементаРеквизитыКонтрагентаНовая");
	ФормыИсключенияКоманд.Добавить("Справочник.Партнеры.Форма.CRM_Модуль_ФормаСписка");
	ФормыИсключенияКоманд.Добавить("Справочник.Партнеры.Форма.CRM_Модуль_ФормаСпискаМобильныйКлиент");
	ФормыИсключенияКоманд.Добавить("Документ.CRM_Интерес.Форма.ФормаДокументаНовая");
	ФормыИсключенияКоманд.Добавить("Документ.CRM_Интерес.Форма.ФормаСписка");
	
	Если ФормыИсключенияКоманд.Найти(НастройкиФормы.ИмяФормы) <> Неопределено Тогда
		СтруктураПоиска = Новый Структура;
		СтруктураПоиска.Вставить("Обработчик", "ЗаметкиПользователяСлужебныйКлиент.СоздатьЗаметкуПоПредмету");
		НайденныеСтроки = Команды.НайтиСтроки(СтруктураПоиска);
		Для Каждого ТекСтрока Из НайденныеСтроки Цикл
			Команды.Удалить(ТекСтрока);
		КонецЦикла;
	КонецЕсли;
	
	// Работа с буфером обмена
	Команда = Команды.Добавить();
	Команда.Вид = "РаботаСБуферомОбмена";
	Команда.ИмяФормы = "Обработка.РаботаСФайлами.Форма.ПрисоединенныеФайлы";
	Команда.Идентификатор = "CRM_СкопироватьФайлВБуфер";
	Команда.ИмяВФорме = "CRM_СкопироватьФайлВБуфер";
	Команда.ИзменяетВыбранныеОбъекты = Ложь;
	Команда.МножественныйВыбор = Истина;
	Команда.Обработчик = "CRM_ПрисоединенныеФайлыКлиент.СкопироватьФайлыВБуфер";
	Команда.Назначение = "ДляСписка";
	
	Команда = Команды.Добавить();
	Команда.Вид = "РаботаСБуферомОбмена";
	Команда.ИмяФормы = "Обработка.РаботаСФайлами.Форма.ПрисоединенныеФайлы";
	Команда.Идентификатор = "CRM_ВставитьФайлИзБуфера";		
	Команда.ИмяВФорме = "CRM_ВставитьФайлИзБуфера";
	Команда.ИзменяетВыбранныеОбъекты = Ложь;
	Команда.МножественныйВыбор = Ложь;
	Команда.Обработчик = "CRM_ПрисоединенныеФайлыКлиент.ВставитьФайлыИзБуфера";
	Команда.Назначение = "ДляСписка";
