		...
		Если НЕ ПараметрыВыбора = Неопределено И ПараметрыВыбора.Количество() > 0 Тогда
			...
			Для каждого Параметр Из ПараметрыВыбора Цикл
				...
				Если МетаданныеОбъекта.Реквизиты.Найти(ИмяРеквизита) = Неопределено
					 И НЕ СуществуетСтандартныйРеквизит(МетаданныеОбъекта.СтандартныеРеквизиты, ИмяРеквизита) Тогда
					Продолжить;
				КонецЕсли; 
				Условия = Условия +
				?(ПустаяСтрока(Условия), "ГДЕ ", Символы.ПС + "	И") +
				" Объект." + ИмяРеквизита +
				?(ТипЗнч(Параметр.Значение) = Тип("СписокЗначений")
					 ИЛИ ТипЗнч(Параметр.Значение) = Тип("ФиксированныйМассив"), " В (&Значение" + Индекс + ")", " = &Значение" + Индекс);
...
//
Процедура ДобавитьОписаниеПривязки(СвязанныеПоля, Поле, Объект, ДополнительныйРазрез = Неопределено,
	 СложныйАлгоритм = Ложь,
	 Рекомендуемый = Ложь) Экспорт
...
//
Процедура ДобавитьВариантОформления(ВариантыОформления, Поле, КлючВариантаОформления,
	 ЭлементОформления,
	 ЗначениеОформления) Экспорт
...
Процедура ДобавитьВариантыОформленияСумм(ВариантыОформления, Поля, НастройкиКД = Неопределено) Экспорт
	...
	Если НЕ НастройкиКД = Неопределено Тогда
		Для каждого ИмяПоля Из МассивПолей Цикл
			CRM_ОтчетыКлиентСервер.ДобавитьУсловноеОформление(НастройкиКД, ИмяПоля,
...
Функция ПараметрыФормированияОтчета(НастройкиОтчета) Экспорт
	...
	НевыводимыеПараметры = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("НачалоПериода,
		|КонецПериода,Период,СтПериод,Периодичность,ТипДиаграммы,
		|ВыводитьЗаголовок");
...
Процедура СтандартизироватьСхему(СхемаКомпоновкиДанных) Экспорт
	...
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодДекада",
		 НСтр("ru='Начало недели';en='Beginning of week'"),
		 НСтр("ru='ДФ=дд.ММ.гггг'"));
...
Процедура ОбработатьСхемуМультивалютногоОтчета(СхемаКомпоновкиДанных, Настройки) Экспорт
	...
	Для каждого ПолеВыбор Из Настройки.ДоступныеПоляВыбора.Элементы Цикл
		...
		Если СтрНайти(ИмяПоля, "Вал") > 0 Тогда
			Продолжить;
		КонецЕсли; 
		Суффиксы = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("НачальныйОстаток,Приход,
			|Расход,Оборот,КонечныйОстаток,Остаток,
			|");
		...
		Для каждого Суффикс Из Суффиксы Цикл
			...
			Иначе
				Позиция = СтрНайти(ИмяПоля, Суффикс);
...
//
Процедура ВывестиКартинкиВЭлементРезультата(ЭлементРезультата, ДанныеРасшифровки, ДокументРезультат,
	 ПараметрыФормирования) Экспорт
		...
		Если ТипЗнч(ЭлементПараметра.Значение) = Тип("ИдентификаторРасшифровкиКомпоновкиДанных") Тогда
			...
			Для Каждого Поле Из Поля Цикл
				Если НЕ ПараметрыФормирования.ПорядокГруппировокЗаполнен Тогда
					...
					ИначеЕсли НЕ Поле.Поле = ПараметрыФормирования.ГруппировкаКолонок
						 И НЕ Поле.Поле = ПараметрыФормирования.Сравнение Тогда
						ПараметрыФормирования.ПорядокГруппировокЗаполнен = Истина;
					Иначе
						Если НЕ ПараметрыФормирования.Свойство("ПерваяКолонка") Тогда
							ПараметрыФормирования.Вставить("ПерваяКолонка",
								 ДокументРезультат.ШиринаТаблицы - ПараметрыФормирования.СтруктураПолейСравнения.Количество() 
								+ 1);
						КонецЕсли; 
				...
				Если ТипЗнч(Поле.Значение) = Тип("ХранилищеЗначения") Тогда
					...
					Для НомерКолонки = 1 По ДокументРезультат.ШиринаТаблицы Цикл
						ОбластьЯчейки = ДокументРезультат.Область(ДокументРезультат.ВысотаТаблицы, НомерКолонки,
...
// Возвращает преобразованную дату, для которой актуальными остается только месяц
	// и день, остальные части даты сбрасываются 
//
// Параметры:
...
Функция ТекстБезТегов(Текст) Экспорт
	...
	Пока Позиция > 0 Цикл
		...
		ПозицияКонца = СтрНайти(ОстатокСтроки, ">");
...
Функция СтрокФиксироватьСлева(КомпоновщикНастроек) Экспорт
	...
	КонецЕсли; 
    СтрокФиксироватьСлева = МасксимальноеКоличествоПолейРекурсивно(КомпоновщикНастроек.Настройки.Структура,
    	 ГруппировкиОтдельно,
    	 РеквизитыОтдельно);

    ПараметрВывода =
    	КомпоновщикНастроек.Настройки.ПараметрыВывода.Элементы.Найти("ГоризонтальноеРасположениеОбщихИтогов");
...
Функция МасксимальноеКоличествоПолейРекурсивно(ЭлементыСтруктуры, ГруппировкиОтдельно, РеквизитыОтдельно)
	...
	Для каждого Элемент Из ЭлементыСтруктуры Цикл
		Если НЕ Элемент.Использование Тогда
			Продолжить;
		КонецЕсли; 
		Если НЕ ТипЗнч(Элемент) = Тип("ГруппировкаКомпоновкиДанных")
			 И НЕ ТипЗнч(Элемент) = Тип("ГруппировкаТаблицыКомпоновкиДанных") Тогда
			Продолжить;
		КонецЕсли;
			...
			Если НЕ РеквизитыОтдельно И Позиция > 0 Тогда
				...
				Для каждого ПолеПроверки Из Элемент.ПоляГруппировки.Элементы Цикл
					...
					Если ИмяПоля = ИмяПоляПроверки Тогда
						Прервать;
					КонецЕсли;
					ПозицияПроверки = СтрНайти(ИмяПоляПроверки, ".");
	...
	Для каждого Элемент Из ЭлементыСтруктуры Цикл
		Если ТипЗнч(Элемент) = Тип("ГруппировкаКомпоновкиДанных")
			 ИЛИ ТипЗнч(Элемент) = Тип("ГруппировкаТаблицыКомпоновкиДанных") Тогда
			КоличествоПолей = Макс(КоличествоПолей, МасксимальноеКоличествоПолейРекурсивно(Элемент.Структура,
				 ГруппировкиОтдельно,
				 РеквизитыОтдельно));
		ИначеЕсли ТипЗнч(Элемент) = Тип("ТаблицаКомпоновкиДанных") Тогда
			КоличествоПолей = Макс(КоличествоПолей, МасксимальноеКоличествоПолейРекурсивно(Элемент.Строки,
				 ГруппировкиОтдельно,
				 РеквизитыОтдельно));
		КонецЕсли; 
	КонецЦикла; 
...
Функция СоздатьГруппу(Стр, Отображение = Неопределено, ЭтаФорма, Элементы) Экспорт
	...
	Группа = Элементы.Добавить("ГруппаФильтр" + Идентификатор, Тип("ГруппаФормы"),
...
Процедура СоздатьФильтрПолеВвода(Стр, ЭтаФорма, Элементы, ПараметрыВыбора, СвязиПараметровВыбора) Экспорт
	...
	Для каждого Реквизит Из Стр.Реквизиты Цикл
		...
		ИначеЕсли НЕ Стр.ТипЗначения.Типы().Количество() = 0
			 И ОбщегоНазначения.ЭтоСсылка(Стр.ТипЗначения.Типы().Получить(0)) Тогда
		...
		Если Стр.ДополнительныеПараметры.Свойство("АвтоОтметкаНезаполненного")
			 И Стр.ДополнительныеПараметры.АвтоОтметкаНезаполненного Тогда
...
Процедура НастроитьДинамическийПериод(СхемаКомпоновкиДанных, ПараметрыОтчета, ДополнятьПериод = Ложь) Экспорт
	...
	Если Не ЗначениеЗаполнено(ПараметрПериодичность.Значение) Тогда
		ПараметрПериодичность.Значение = ПолучитьЗначениеПериодичности(ПараметрыОтчета.НачалоПериода,
	...
	Для Каждого Рисунок Из ДокументРезультат.Рисунки Цикл
		...
		Попытка
			Если Рисунок.ТипРисунка = ТипРисункаТабличногоДокумента.Диаграмма
				...
				Рисунок.Объект.ПоложениеПодписейШкалыЗначенийИзмерительнойДиаграммы =
...
Процедура ИсправитьНастройкиГруппировок(КомпоновщикНастроек, ПользовательскиеНастройкиМодифицированы) Экспорт
	...
	Для Каждого ПользовательскаяНастройка Из ПользовательскиеНастройки.Элементы Цикл
		Если (ТипЗнч(ПользовательскаяНастройка) = Тип("ГруппировкаКомпоновкиДанных") 
			...
			ИсправитьНастройкиРодительскойГруппировки(ПользовательскаяНастройка, ПользовательскиеНастройки,
				 Настройки,
...
Процедура ИсправитьНастройкиРодительскойГруппировки(ПользовательскаяНастройка,
	...
	Если ТипЗнч(РодительОбъектаНастройки) = Тип("ГруппировкаКомпоновкиДанных") 
		...
		Если Не ПустаяСтрока(ИдентификаторПользовательскойНастройкиРодителя) Тогда
			ПользовательскаяНастройкаРодитель = НайтиПользовательскуюНастройку(ПользовательскиеНастройки,
				 ИдентификаторПользовательскойНастройкиРодителя);
			ПользовательскаяНастройкаРодитель.Использование = Истина;
			ПользовательскиеНастройкиМодифицированы = Истина;
			
			ИсправитьНастройкиРодительскойГруппировки(ПользовательскаяНастройкаРодитель,
				 ПользовательскиеНастройки, Настройки,
				 ПользовательскиеНастройкиМодифицированы);
		Иначе
			ИсправитьНастройкиРодительскойГруппировки(РодительОбъектаНастройки, ПользовательскиеНастройки,
				 Настройки,
				 ПользовательскиеНастройкиМодифицированы);
		КонецЕсли;
	КонецЕсли;
...
//
Функция ПолучитьОбъектПоПользовательскомуИдентификатору(Настройки,
	...
	Если ТипНастройки <> Тип("НастройкиКомпоновкиДанных") Тогда
		...
		ИначеЕсли ТипНастройки = Тип("НастройкиВложенногоОбъектаКомпоновкиДанных") Тогда
			...
			Возврат ПолучитьОбъектПоПользовательскомуИдентификатору(Настройки.Настройки,
				 ИдентификаторПользовательскойНастройки,
				 Иерархия);
		...
		ИначеЕсли ТипНастройки = Тип("КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных")
			...
			Для Каждого ВложенныйЭлемент Из Настройки Цикл
				РезультатПоиска = ПолучитьОбъектПоПользовательскомуИдентификатору(ВложенныйЭлемент,
					 ИдентификаторПользовательскойНастройки,
					 Иерархия);
	...
	Если ТипНастройки = Тип("ТаблицаКомпоновкиДанных") Тогда
		...
		РезультатПоиска = ПолучитьОбъектПоПользовательскомуИдентификатору(Настройки.Строки,
			 ИдентификаторПользовательскойНастройки,
			 Иерархия);
		Если РезультатПоиска <> Неопределено Тогда
			Возврат РезультатПоиска;
		...
		РезультатПоиска = ПолучитьОбъектПоПользовательскомуИдентификатору(Настройки.Колонки,
			 ИдентификаторПользовательскойНастройки,
			 Иерархия);
		Если РезультатПоиска <> Неопределено Тогда
			Возврат РезультатПоиска;
	...
	ИначеЕсли ТипНастройки = Тип("ДиаграммаКомпоновкиДанных") Тогда
		...
		РезультатПоиска = ПолучитьОбъектПоПользовательскомуИдентификатору(Настройки.Точки,
			 ИдентификаторПользовательскойНастройки,
			 Иерархия);
		Если РезультатПоиска <> Неопределено Тогда
			Возврат РезультатПоиска;
		...
		РезультатПоиска = ПолучитьОбъектПоПользовательскомуИдентификатору(Настройки.Серии,
			 ИдентификаторПользовательскойНастройки,
			 Иерархия);
		Если РезультатПоиска <> Неопределено Тогда
			Возврат РезультатПоиска;
	...
	Иначе
		...
		РезультатПоиска = ПолучитьОбъектПоПользовательскомуИдентификатору(Настройки.Структура,
			 ИдентификаторПользовательскойНастройки,
			 Иерархия);
		Если РезультатПоиска <> Неопределено Тогда
			Возврат РезультатПоиска;
...
Функция ПолучитьПараметрВывода(Настройка, ИмяПараметра) Экспорт
	...
	Если ТипЗнч(Настройка) = Тип("КомпоновщикНастроекКомпоновкиДанных") Тогда
		ЗначениеПараметра =
			Настройка.Настройки.ПараметрыВывода.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных(ИмяПараметра));
	Иначе
...
Процедура ОтключитьИспользованиеПользовательскойНастройки(КомпоновщикНастроек, Настройка) Экспорт
	...
	ПользовательскаяНастройка =
		КомпоновщикНастроек.ПользовательскиеНастройки.Элементы.Найти(ИскомаяНастройка.ИдентификаторПользовательскойНастройки);
