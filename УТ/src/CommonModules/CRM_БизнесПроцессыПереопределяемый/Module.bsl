...
Процедура ТочкаСтартаПередСтартом(ТочкаМаршрута, БизнесПроцесс, Отказ, ТекстСообщения = "") Экспорт
	...
	Обработчик = ПолучитьОбработчикТочкиМаршрута(ТочкаМаршрута,
		 Справочники.CRM_ОбработчикиСобытийТочекМаршрута.СтартПередСтартом);
	Если ЗначениеЗаполнено(Обработчик) Тогда
		Попытка
			Выполнить(Обработчик);
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка выполнения обработчика ""Перед стартом"" в точке';en='Run error of the handler ""Before start"" in a point'") + " " 
				+ ТочкаМаршрута.Наименование 
				+ Символы.ПС 
				+ ОписаниеОшибки();
...
Процедура ТочкаСтартаПриСтарте(ТочкаМаршрута, БизнесПроцесс, ФормируемыеЗадачи) Экспорт
	// Получим обработчик текущей точки.
	Обработчик = ПолучитьОбработчикТочкиМаршрута(ТочкаМаршрута,
		 Справочники.CRM_ОбработчикиСобытийТочекМаршрута.СтартПриСтарте);
	Если ЗначениеЗаполнено(Обработчик) Тогда
		Попытка
			Выполнить(Обработчик);
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка выполнения обработчика ""При старте"" в точке';en='Run error of the handler ""At start"" in a point'") + " " 
				+ ТочкаМаршрута.Наименование 
				+ Символы.ПС 
				+ ОписаниеОшибки();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Процедура ТочкаСтартаПослеСтарта(ТочкаМаршрута, БизнесПроцесс, ФормируемыеЗадачи) Экспорт
	// Получим обработчик текущей точки.
	Обработчик = ПолучитьОбработчикТочкиМаршрута(ТочкаМаршрута,
		 Справочники.CRM_ОбработчикиСобытийТочекМаршрута.СтартПослеСтарта);
	Если ЗначениеЗаполнено(Обработчик) Тогда
		Попытка
			Выполнить(Обработчик);
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка выполнения обработчика ""После старта"" в точке';en='Run error of the handler ""After start"" in a point'") + " " 
				+ ТочкаМаршрута.Наименование 
				+ Символы.ПС 
				+ ОписаниеОшибки();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Процедура ТочкаЗавершенияПриЗавершении(ТочкаМаршрута, БизнесПроцесс) Экспорт
	// Получим обработчик текущей точки.
	Обработчик = ПолучитьОбработчикТочкиМаршрута(ТочкаМаршрута,
		 Справочники.CRM_ОбработчикиСобытийТочекМаршрута.ЗавершениеПриЗавершении);
	Если ЗначениеЗаполнено(Обработчик) Тогда
		Попытка
			Выполнить(Обработчик);
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка выполнения обработчика ""При завершении"" в точке';en='Run error of the handler ""At completion"" in a point'") + " " 
				+ ТочкаМаршрута.Наименование 
				+ Символы.ПС 
				+ ОписаниеОшибки();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Функция СформироватьОписаниеБизнесПроцесса(Задача) Экспорт
	...
	Если ЗначениеЗаполнено(Задача.БизнесПроцесс.Предмет) Тогда
		...
		|<span style='font-size: 12pt;'><STRONG>" + НСтр("ru='Предмет: ';en='Subject: '") + "</STRONG>" 
			+ ЗначениеСтрокой 
			+ "</span>
		|";
	КонецЕсли;
	...
	Для Каждого ЭлементСписка Из СписокРеквизитов Цикл
		Если НЕ ЗначениеЗаполнено(Задача.БизнесПроцесс[ЭлементСписка.Значение]) Тогда
			Продолжить;
		КонецЕсли;
		...
		Иначе
			Попытка
				Если (СтрНайти(Задача.БизнесПроцесс[ЭлементСписка.Значение].Метаданные().ПолноеИмя(), "Справочник.") > 0)
				ИЛИ (СтрНайти(Задача.БизнесПроцесс[ЭлементСписка.Значение].Метаданные().ПолноеИмя(), "Документ.") > 0) Тогда
					
					Если СтрНайти(Задача.БизнесПроцесс[ЭлементСписка.Значение].Метаданные().ПолноеИмя(), "Документ.") > 0 Тогда
						ПредставлениеОбъекта = CRM_СтруктураПодчиненностиПереопределяемый.ПредставлениеОбъектаДляВыводаВОтчет(Новый Структура("Ссылка",
							 Задача.БизнесПроцесс[ЭлементСписка.Значение]));
		...
		ТаблицаРеквизитов = ТаблицаРеквизитов + "
		|<TR>
		|<TD><span style='font-size: 12pt;'><STRONG>" + ЭлементСписка.Представление + "</STRONG></span></TD>
		|<TD><span style='font-size: 12pt;'>" + ЗначениеСтрокой + "</span></TD></TR>";
	КонецЦикла;
	...
	|<P><span style='font-size: 12pt;'>" + ОписаниеФорматированное + "</span></P>";
...
Функция СформироватьИсториюБизнесПроцесса(Задача, Сделка = Ложь) Экспорт
	...
	ТекущаяЗадача = "";
	ИсторияВыполнения = "";
	// |<P><span style='font-size: 10pt;'><span style='font-size: 10pt;'><EM><STRONG>" 
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТабОбъектов.Количество() > 0 Тогда
			Если НЕ Сделка Тогда
				СтрокаОбъектовПоЗадаче = "
					|<P><span style='font-size: 12pt;'><EM>&ensp;&ensp;&ensp;" 
						+ НСтр("ru='Введенные на этапе объекты:';en='The objects enter at a stage:'") + "</EM></span></P>";
			Иначе
				СтрокаОбъектовПоЗадаче = "
		...
		Для Каждого СтрокаТаб Из ТабОбъектов Цикл
			Попытка
				Если НЕ Сделка И ((СтрНайти(СтрокаТаб.Объект.Метаданные().ПолноеИмя(), "Справочник.") > 0)
					...
					Если ТипЗнч(СтрокаТаб.Объект) <> Тип("ДокументСсылка.CRM_Интерес")
			...
			Если НЕ Сделка Тогда
				СтрокаОбъектовПоЗадаче = СтрокаОбъектовПоЗадаче + "
					|<P><span style='font-size: 12pt;'>&ensp;&ensp;" + ЗначениеСтрокой + "</P>";
		...
		Если Выборка.Выполнена Тогда
			Если НЕ Сделка Тогда
				ИсторияВыполнения = ИсторияВыполнения + "
					|<P><span style='font-size: 12pt;'>" + Формат(Выборка.ДатаИсполнения, "ДФ='дд.ММ.гггг ЧЧ:мм'") 
						+ " / <STRONG>" + Выборка.CRM_ТочкаМаршрута + "</STRONG> / " + Выборка.Исполнитель + " / " 
			...
			Если НЕ Сделка Тогда
				ИсторияВыполнения = ИсторияВыполнения + "
					|<P><span style='font-size: 12pt;'>" + СокрЛП(СтрЗаменить(Выборка.РезультатВыполнения,
						 Символы.ПС, "<br>")) + "</P>";
			Иначе
				ИсторияВыполнения = ИсторияВыполнения + "
		...
		Иначе
			Если НЕ Сделка Тогда
				ТекущаяЗадача = ТекущаяЗадача + "
					|<P><span style='font-size: 12pt;'>" + Формат(Выборка.Дата, "ДФ='дд.ММ.гггг ЧЧ:мм'") 
						+ " / <STRONG>" + Выборка.CRM_ТочкаМаршрута + "</STRONG> / " 
			...
			Если НЕ Сделка Тогда
				ТекущаяЗадача = ТекущаяЗадача + "
					|<P><span style='color: #1955ae; font-size: 12pt;'"">" + СокрЛП(СтрЗаменить(Выборка.Описание,
						 Символы.ПС, "<br>")) + "</span></P>";
			Иначе
				ТекущаяЗадача = ТекущаяЗадача + "
	...
	Если НЕ ИсторияВыполнения = "" Тогда
		Если НЕ Сделка Тогда
			ИсторияВыполнения = "<P><span style='font-size: 12pt;'><span style='font-size: 12pt;'><EM><STRONG>"
...
Функция СформироватьОписаниеЗадачи(Задача, Сделка = Ложь) Экспорт
	...
	Если (НЕ Задача.CRM_Личная) И (Задача <> Задачи.ЗадачаИсполнителя.ПустаяСсылка()) Тогда
		Если НЕ Сделка
			И ТипЗнч(Задача.БизнесПроцесс) = Тип("БизнесПроцессСсылка.CRM_БизнесПроцесс")
			И Задача.БизнесПроцесс.КартаМаршрута.ОписаниеБизнесПроцессаВСодержаниеЗадачи Тогда
			
			ОписаниеБизнесПроцесса = СформироватьОписаниеБизнесПроцесса(Задача);
		КонецЕсли;
		Если ТипЗнч(Задача.БизнесПроцесс) = Тип("БизнесПроцессСсылка.CRM_БизнесПроцесс")
			И Задача.БизнесПроцесс.КартаМаршрута.ИсторияБизнесПроцессаВСодержаниеЗадачи Тогда
			
			ИсторияБизнесПроцесса = СформироватьИсториюБизнесПроцесса(Задача);
		КонецЕсли;
	Иначе
...
Процедура ДобавитьЭлементыОтборовВНастройкиСхемы(НастройкиКомпоновки, ИмяОбъекта) Экспорт
	...
	Для Каждого ИмяПоля Из МассивПолейОтбора Цикл
		...
		МенеджерОбъекта =
			ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(Метаданные.НайтиПоТипу(Элемент.Тип.Типы()[0]).ПолноеИмя());
...
Процедура ТочкаДействияПриВыполнении(ТочкаМаршрута, Задача, Отказ, ТекстСообщения = "") Экспорт
	// Получим обработчик текущей точки.
	Обработчик = ПолучитьОбработчикТочкиМаршрута(ТочкаМаршрута,
		 Справочники.CRM_ОбработчикиСобытийТочекМаршрута.ДействиеПриВыполнении);
	Если ЗначениеЗаполнено(Обработчик) Тогда
		Попытка
			Выполнить(Обработчик);
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка выполнения обработчика ""При выполнении"" в точке';en='Run error of the handler ""At execution"" in a point'") + " " 
				+ ТочкаМаршрута.Наименование 
				+ Символы.ПС 
				+ ОписаниеОшибки();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Процедура ОбработчикТочкиДействияПослеВыполнения(ТекущаяЗадача, ФормируемыеЗадачи) Экспорт
	// Получим обработчик текущей точки.
	Обработчик = ПолучитьОбработчикТочкиМаршрута(ТекущаяЗадача.CRM_ТочкаМаршрута,
		 Справочники.CRM_ОбработчикиСобытийТочекМаршрута.ДействиеПослеВыполнения);
	Если ЗначениеЗаполнено(Обработчик) Тогда
		Попытка
			Выполнить(Обработчик);
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка выполнения обработчика ""После выполнения"" в точке';en='Run error of the handler ""After execution"" in a point'") 
				+ " " 
				+ ТекущаяЗадача.CRM_ТочкаМаршрута.Наименование + Символы.ПС 
				+ ОписаниеОшибки();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
//
Процедура ОбработчикТочкиДействияПередСозданиемЗадач(БизнесПроцесс , ТочкаМаршрута,
	 ФормируемыеЗадачи,
	 СтандартнаяОбработка) Экспорт
	// Получим обработчик текущей точки.
	Обработчик = ПолучитьОбработчикТочкиМаршрута(ТочкаМаршрута,
		 Справочники.CRM_ОбработчикиСобытийТочекМаршрута.ДействиеПередСозданиемЗадач);
	Если ЗначениеЗаполнено(Обработчик) Тогда
		Попытка
			Выполнить(Обработчик);
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка выполнения обработчика ""Перед созданием задач"" в точке';en='Run error of the handler ""Before creation of tasks"" in a point'") 
				+ " " 
				+ ТочкаМаршрута.Наименование + Символы.ПС 
				+ ОписаниеОшибки();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Процедура ОбработчикТочкиДействияПриСозданииЗадач(БизнесПроцесс , ТочкаМаршрута, ФормируемыеЗадачи, Отказ) Экспорт
	...
	Если ЗначениеЗаполнено(Обработчик) Тогда
		Попытка
			Выполнить(Обработчик);
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка выполнения обработчика ""При создании задач"" в точке';en='Run error of the handler ""At creation of tasks"" in a point'") 
				+ " " 
				+ ТочкаМаршрута.Наименование + Символы.ПС 
				+ ОписаниеОшибки();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Функция ОбработчикТочкиВыборВарианта(БизнесПроцесс, ВходящаяТочка, ТочкаМаршрута, Знач ВариантВыполнения) Экспорт
	...
	// Получим обработчик текущей точки.
	Обработчик = ПолучитьОбработчикТочкиМаршрута(ТочкаМаршрута,
		 Справочники.CRM_ОбработчикиСобытийТочекМаршрута.ВыборВарианта);
	Если ЗначениеЗаполнено(Обработчик) Тогда
		Попытка
			Выполнить(Обработчик);
			Возврат ВариантВыполнения;
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка выполнения обработчика ""Выбор варианта"" в точке';en='Run error of the handler ""Choice of an option"" in a point'") 
				+ " " 
				+ ТочкаМаршрута.Наименование + Символы.ПС 
				+ ОписаниеОшибки();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
//	Число	- Индекс варианта
//
Функция ОбработчикТочкиУсловияПроверкаУсловия(БизнесПроцесс, ВходящаяТочка, ТочкаМаршрута,
	 Знач ВариантВыполнения) Экспорт
	...
	// Получим обработчик текущей точки.
	Обработчик = ПолучитьОбработчикТочкиМаршрута(ТочкаМаршрута,
		 Справочники.CRM_ОбработчикиСобытийТочекМаршрута.УсловиеПроверкаУсловия);
	Если ЗначениеЗаполнено(Обработчик) Тогда
		Попытка
			Выполнить(Обработчик);
			...
			Возврат ВариантВыполнения;
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка выполнения обработчика ""Проверка условия"" в точке';en='Run error of the handler ""Check of a condition"" in a point'") 
				+ " " 
				+ ТочкаМаршрута.Наименование + Символы.ПС 
				+ ОписаниеОшибки();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Процедура ОбработчикТочкиВложенныйБизнесПроцессПриСозданииВложенногоБизнесПроцесса(БизнесПроцесс, ВедущаяЗадача) Экспорт
	...
	Если ЗначениеЗаполнено(Обработчик) Тогда
		Попытка
			Выполнить(Обработчик);
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка выполнения обработчика ""При создании вложенного бизнес-процесса"" в точке';en='Run error of the handler ""At creation of nested business process"" in a point'") 
				+ " " 
				+ ВедущаяЗадача.CRM_ТочкаМаршрута.Наименование + Символы.ПС 
				+ ОписаниеОшибки();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
//	ВедущаяЗадача	- ЗадачаСсылка			- Ведущая задача.
//
Процедура ОбработчикТочкиВложенныйБизнесПроцессПослеСозданияВложенногоБизнесПроцесса(БизнесПроцесс,
	 ВедущаяЗадача) Экспорт
	// Получим обработчик текущей точки.
	Обработчик = ПолучитьОбработчикТочкиМаршрута(ВедущаяЗадача.CRM_ТочкаМаршрута,
	...
	Если ЗначениеЗаполнено(Обработчик) Тогда
		Попытка
			Выполнить(Обработчик);
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка выполнения обработчика ""После создания вложенного бизнес-процесса"" в точке';en='Run error of the handler ""After creation of nested business process"" in a point'") 
				+ " " 
				+ ВедущаяЗадача.CRM_ТочкаМаршрута.Наименование + Символы.ПС 
				+ ОписаниеОшибки();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
//						  Значение по умолчанию: Истина.
//
Процедура ОбработчикТочкиВложенныйБизнесПроцессПередСозданиемЗадач(БизнесПроцесс, ТочкаМаршрута,
	 ФормируемыеЗадачи,
	 СтандартнаяОбработка) Экспорт
	// Получим обработчик текущей точки.
	Обработчик = ПолучитьОбработчикТочкиМаршрута(ТочкаМаршрута,
...
//						  Значение по умолчанию: Ложь.
//
Процедура ОбработчикТочкиВложенныйБизнесПроцессПриСозданииЗадач(БизнесПроцесс, ТочкаМаршрута,
	 ФормируемыеЗадачи,
	 Отказ) Экспорт
	// Получим обработчик текущей точки.
	Обработчик = ПолучитьОбработчикТочкиМаршрута(ТочкаМаршрута,
...
Процедура ТочкаОбработкиОбработка(ТочкаМаршрута, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Обработчик) Тогда
		Попытка
			Выполнить(Обработчик);
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка выполнения обработчика ""Обработка"" в точке';en='Run error of the ""Data processor"" handler in a point'") + " " 
				+ ТочкаМаршрута.Наименование 
				+ Символы.ПС 
				+ ОписаниеОшибки();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
//
//
Функция ПолучитьОбъектыПоЗадаче(Задача) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	ТипЗначения(CRM_ОбъектыПоБизнесПроцессам.Объект) КАК ТипОбъекта,
	...
	ТипыОбъектов = ТаблицаЗначений.ВыгрузитьКолонку("ТипОбъекта");
	ТекстЗапроса = "";
	
	Для Каждого ТекТип Из ТипыОбъектов Цикл
		
		ИмяРеквизитаСумма = "0";
		ИмяРеквизитаВалюта = "ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)";
		ОбъектМетаданных = Метаданные.НайтиПоТипу(ТекТип);
		
		Если ОбщегоНазначения.ЭтоДокумент(ОбъектМетаданных) Тогда
			Если Не ОбъектМетаданных.Реквизиты.Найти("СуммаДокумента") = Неопределено Тогда
				ИмяРеквизитаСумма = "ОбъектЗадачи.СуммаДокумента";
			КонецЕсли;
			Если Не ОбъектМетаданных.Реквизиты.Найти("Валюта") = Неопределено Тогда
				ИмяРеквизитаВалюта = "ОбъектЗадачи.Валюта";
			КонецЕсли;
		КонецЕсли;
		...
		Если ТекстЗапроса = "" Тогда
			ТекстЗапроса = "
				...
				|	CRM_ОбъектыПоБизнесПроцессам.Объект,
				|	CRM_ОбъектыПоБизнесПроцессам.ДатаРегистрации КАК ДатаРегистрации,
				|	CRM_ОбъектыПоБизнесПроцессам.ДатаСортировки КАК ДатаСортировки
				|ПОМЕСТИТЬ ОбъектыПоБизнесПроцессам
				|ИЗ
				|	&ТаблицаЗначений КАК CRM_ОбъектыПоБизнесПроцессам
				|;
				|";
		КонецЕсли;
		
		ТекстЗапроса = ТекстЗапроса + СтрокаОбъединить + "
			|ВЫБРАТЬ
			|	ОбъектыПоБизнесПроцессам.Объект,
			|	ОбъектыПоБизнесПроцессам.ДатаРегистрации КАК ДатаРегистрации,
			|	ОбъектыПоБизнесПроцессам.ДатаСортировки КАК ДатаСортировки,
			|	%1 КАК СуммаДокумента,
			|	%2 КАК Валюта
			|ИЗ
			|	ОбъектыПоБизнесПроцессам КАК ОбъектыПоБизнесПроцессам
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ %3 КАК ОбъектЗадачи
			|		ПО ОбъектыПоБизнесПроцессам.Объект = ОбъектЗадачи.Ссылка
			|";
		
		ТекстЗапроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстЗапроса,
			ИмяРеквизитаСумма, ИмяРеквизитаВалюта, ОбъектМетаданных.ПолноеИмя());
