...
Процедура СинхронизироватьСобытия(СтруктураСвойств, Настройка)  Экспорт
	...
	Для Каждого СтрокаСобытия Из СтруктураСвойств.События Цикл
		Если НЕ СтрокаСобытия.Пометка Тогда
			Продолжить;
		КонецЕсли;
		ОбработкаПрерыванияПользователя();
		Состояние(НСтр("ru='Запись событий: ';en='Record of events: '") + Строка(Счетчик) 
			+ НСтр("ru=' из '") 
			+ Строка(ОбщееКоличество));
		Счетчик = Счетчик + 1;
		Если СтрокаСобытия.Направление = "Outlook -> 1С" Тогда
			...
			Если Настройка.СинхронизироватьУчастниковСобытияOutlook  Тогда
				...
				Для Ном = 1 По КолКонтактов Цикл
					...
					Попытка
						...
						Идентификатор	= CRM_ОбменСMSOutlookКлиентСервер.ПолучитьИдентификаторУчастника(УчастникСобытия.AddressEntry.ID,
							 ДлинаGUID);
			...
			// +CRM3
			//СтруктураОбъекта.Реквизиты.Вставить("Партнер",
				//			CRM_ОбщегоНазначенияСервер.ПолучитьЗначениеРеквизита(СтрокаСобытия.Объект_1С,
				// "Партнер"));
			//СтруктураОбъекта.Реквизиты.Вставить("КонтактноеЛицо",
				//	CRM_ОбщегоНазначенияСервер.ПолучитьЗначениеРеквизита(СтрокаСобытия.Объект_1С,
				// "КонтактноеЛицо"));
				...
				Если ЗначениеЗаполнено(СтрокаТЧКонтакты.Объект_1С) Тогда
					Если СтрокаТЧКонтакты.ВидКонтакта = "КЛК" Тогда 
						...
						Если ЗначениеЗаполнено(СтруктураОбъекта.Реквизиты.КонтактноеЛицо) Тогда
							...
							ИначеЕсли НЕ КонтактЕстьВТЧ(СтрокаТЧКонтакты.Объект_1С, СторонниеЛица, "КонтактноеЛицо") Тогда
								...
								СтрокаТЧ.Вставить("Партнер",
									 		CRM_ОбщегоНазначенияСервер.ПолучитьЗначениеРеквизита(СтрокаТЧКонтакты.Объект_1С,
									 "Владелец"));
						...
						ИначеЕсли ЗначениеЗаполнено(СтруктураОбъекта.Реквизиты.Партнер) Тогда
							...
							КонецЕсли;
						Иначе
							СтруктураОбъекта.Реквизиты.Партнер			= CRM_ОбщегоНазначенияСервер.ПолучитьЗначениеРеквизита(СтрокаТЧКонтакты.Объект_1С,
								 "Владелец");
					...
					ИначеЕсли СтрокаТЧКонтакты.ВидКонтакта = "К" Тогда	
						...
						Если ЗначениеЗаполнено(СтруктураОбъекта.Реквизиты.Партнер) Тогда
							Если ЗначениеЗаполнено(СтруктураОбъекта.Реквизиты.КонтактноеЛицо) Тогда
								Если НЕ КонтактЕстьВТЧ(СтрокаТЧКонтакты.Объект_1С, СторонниеЛица, "Партнер") Тогда
									...
									СтрокаТЧ.Вставить("КонтактноеЛицо",
										 ПредопределенноеЗначение("Справочник.КонтактныеЛицаПартнеров.ПустаяСсылка"));
									СтрокаТЧ.Вставить("Адрес",			ПолучитьEMailКонтакта(КонтактOutlook));
									СторонниеЛица.Добавить(СтрокаТЧ);
								КонецЕсли;
						...
						Иначе
							...
							СтруктураОбъекта.Реквизиты.КонтактноеЛицо	=
								ПредопределенноеЗначение("Справочник.КонтактныеЛицаПартнеров.ПустаяСсылка");
						КонецЕсли;	
					КонецЕсли;	
				КонецЕсли;	
			...
			Объект_1С  = CRM_ОбменСMSOutlookСервер.ЗаполнитьМероприятиеПоСтруктуре(СтрокаСобытия.Объект_1С,
				 СтруктураОбъекта, ОчиститьУчастников,
				 СтрокаСобытия);
		ИначеЕсли СтрокаСобытия.Направление = "1С -> Outlook" Тогда
			...
			Для Каждого УчастникСобытия Из КонтактыСобытия Цикл 
				...
				Если СОМОбъектКонтактов.ОбъектOutlook = Неопределено Тогда
					Продолжить;
				КонецЕсли;
				Если Настройка.СинхронизироватьКонтактыСобытияOutlook И OutlookМеньше2013(ОбъектOutlook.Application.Version) Тогда
	...
	СтруктураСвойств.ДатаПоследнейСинхронизацииСобытий = ОбщегоНазначенияКлиент.ДатаСеанса();
КонецПроцедуры	
...
Функция ПолучитьOutlookObj(СтруктураСвойств, Настройка, ТекстОшибки) Экспорт
	...
	Исключение
		...
		Иначе
			ТекстОшибки = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИО);
	...
	Если ЗначениеЗаполнено(Настройка.ПрограммаOutlook) Тогда
		Если CRM_ОбщегоНазначенияСервер.ПолучитьЗначениеРеквизита(Настройка.ПрограммаOutlook, "ДлинаGUID") = 0 Тогда
			...
			Если СтруктураСвойств.FolderContacts.Items.Count > 0 Тогда
				CRM_ОбменСMSOutlookСервер.УстановитьДлинуGUIDOutlook(Настройка.ПрограммаOutlook,
					 СтруктураСвойств.FolderContacts.Items(1).EntryID);			
			КонецЕсли;
		КонецЕсли;	
	КонецЕсли;
	Возврат Истина;
...
Функция ПолучитьСинхронизируемыеКонтактыOutlook(СтруктураСвойств, Настройка) Экспорт
	...
	Если ЗначениеЗаполнено(Настройка.ФильтрДатаИзмененияКонтактаOutlook) Тогда
		Для Каждого ПапкаOutlook Из Настройка.ТаблицаПапокOutlook Цикл
			Если НЕ (ПапкаOutlook.olFolder = "2") Тогда
				Продолжить;
			КонецЕсли;
			Folder = ПолучитьПапкуOutlook(СтруктураСвойств.OutlookObj, ПапкаOutlook.Folder);
			Если Folder = Неопределено Тогда
...
Процедура СинхронизироватьКонтакты(СтруктураСвойств, Настройка)  Экспорт
	...
	Для Каждого СтрокаКонтактов Из СтруктураСвойств.Контакты Цикл
		Если НЕ СтрокаКонтактов.Пометка Тогда
			Продолжить;
		КонецЕсли;
		ОбработкаПрерыванияПользователя();
		Состояние(НСтр("ru='Запись контактов: '") + Строка(Счетчик) + НСтр("ru=' из '") + Строка(ОбщееКоличество));
		Счетчик = Счетчик + 1;
		СинхронизироватьКонтакт(СтруктураСвойств, Настройка, СтрокаКонтактов,
			 ТаблицаСинхронизируемыхРеквизитовК,
			 ТаблицаСинхронизируемыхРеквизитовКЛК);
		СтрокаКонтактов.Пометка = Ложь;
	КонецЦикла;
	// Сохраняем дату синхронизации.
	Если Настройка.СинхронизироватьКонтакты  Тогда
		СтруктураСвойств.ДатаПоследнейСинхронизацииКонтактов = ОбщегоНазначенияКлиент.ДатаСеанса();
	КонецЕсли;
КонецПроцедуры
...
Функция ПолучитьСинхронизируемыеСобытияOutlook(СтруктураСвойств, Настройки) Экспорт
	...
	Если ЗначениеЗаполнено(Настройки.ФильтрДатаИзмененияСобытияOutlook) Тогда
		Для Каждого ПапкаOutlook Из Настройки.ТаблицаПапокOutlook Цикл
			Если НЕ (ПапкаOutlook.olFolder = "1") Тогда
				Продолжить;
			КонецЕсли;
...
Процедура СинхронизироватьПисьма(мСтруктураСвойств, мНастройка)  Экспорт
	Если НЕ мНастройка.СинхронизироватьПисьма Тогда
		Возврат;
	КонецЕсли;	
	...
	Для Каждого СтрокаСобытия Из мСтруктураСвойств.Письма Цикл
		...
		Позиция				= СтрНайти(ПапкаПисьмаТекущая, "\");
		Пока Позиция > 0 Цикл
		...
		Позиция				= СтрНайти(ПапкаПисьмаВходящее, "\");
		Пока Позиция > 0 Цикл
		...
		ЗаполнитьСтруктуруПисьмаПоОбъектуOutlook(СтруктураОбъекта.Реквизиты, ОбъектOutlook,
			 ПапкаOutlook.Folder, СписокПапок,
			 ПапкаПисьмаТекущая = ПапкаПисьмаВходящее);
		...
		Для	Каждого ПолучательПисьма Из ОбъектOutlook.Recipients Цикл
			...
			Попытка 
				КонтактOutlook	= НайтиКонтактOutlookПоАдресу(мСтруктураСвойств, мНастройка, АдресЭП,
					 СоответствиеКонтактовOutlookАдресам);
			Исключение
				КонтактOutlook	= Неопределено;
			КонецПопытки;
		...
		Если ОбъектOutlook.MessageClass = "IPM.Note" Тогда
			...
			СтруктураОбъекта.Реквизиты.Вставить("Текст",
				 CRM_ОбменСMSOutlookСервер.ПолучитьОбычныйТекстИзHTML(ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыXML(ОбъектOutlook.HTMLBody)));
			Если ОбъектOutlook.BodyFormat = 0 Тогда // Unspecified format
				СтруктураОбъекта.Реквизиты.Вставить("ТипТекста",
			...
			ИначеЕсли ОбъектOutlook.BodyFormat = 3 Тогда // Rich text format (RTF)
				СтруктураОбъекта.Реквизиты.Вставить("ТипТекста",
					 ПредопределенноеЗначение("Перечисление.ТипыТекстовЭлектронныхПисем.РазмеченныйТекст"));
			КонецЕсли;
		Иначе
			СтруктураОбъекта.Реквизиты.Вставить("ТекстHTML", ОбъектOutlook.Body);
			СтруктураОбъекта.Реквизиты.Вставить("Текст",
				 CRM_ОбменСMSOutlookСервер.ПолучитьОбычныйТекстИзHTML(ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыXML(ОбъектOutlook.Body)));
			СтруктураОбъекта.Реквизиты.Вставить("ТипТекста",
				 ПредопределенноеЗначение("Перечисление.ТипыТекстовЭлектронныхПисем.HTML"));
		КонецЕсли;
		...
		Попытка
			Объект_1С = CRM_ОбменСMSOutlookСервер.ЗаполнитьПисьмоПоСтруктуре(СтрокаСобытия.Объект_1С,
				 СтруктураОбъекта, Ложь,
				 СтрокаСобытия);
		Исключение
	...
	КонецЦикла;
	// Сохраняем дату синхронизации.
	мСтруктураСвойств.ДатаПоследнейСинхронизацииПисем =  ОбщегоНазначенияКлиент.ДатаСеанса();
КонецПроцедуры
...
Процедура СопоставитьПисьмаOutlook(мСтруктураСвойств, мНастройка, ТаблицаСинхронизируемыхПисемOutlook)
	...
	ФормаТаблицы.ЗаполнитьТаблицуСоответствиеОбъектов(ТаблицаСинхронизируемыхПисемOutlook,
		 мНастройка.ПрограммаOutlook,
		 Истина);
	Сч = 0;
	...
	Для Каждого СтрокаПисемOutlook Из ТаблицаСинхронизируемыхПисемOutlook Цикл
		...
		Если НЕ (СоответствиеОбработанные[СтрокаПисемOutlook.ОбъектOutlookID] = Неопределено) Тогда
			Продолжить;
		КонецЕсли;
		...
		Попытка
			...
			СсылкаНаОбъект1С = ПолучитьОбъект1С(мНастройка.ПрограммаOutlook, EntryID, ФормаТаблицы,
				 ДополнительныеДанныеОбъекта1С);
		Исключение
			сч = сч - 1;
			Продолжить;
		КонецПопытки;
...
Процедура ДобавитьПисьмоOutlookВТаблицуСинхронизируемыхПисем(ТаблицаСинхронизируемыхПисемOutlook,
	 ПисьмоOutlook, ContactFolder,
	 FolderName)
	...
	СтрокаТаблицы.ДатаИзменения	= ?(СтрокаТаблицы.ДатаИзменения = Дата('45010101'), Дата("00010101"),
		 СтрокаТаблицы.ДатаИзменения);
		...
		Если ContactFolder.Items.Count > 0 Тогда
			Для Ном = 1 По ContactFolder.Items.Count Цикл
				...
				Если Повторяющееся = Истина Тогда
					...
					Продолжить;
				КонецЕсли;
				ДобавитьСобытиеOutlookВТаблицуСинхронизируемыхСобытий(СтруктураСвойств.ТаблицаСобытийOutlook,
					 СобытиеOutlook, ContactFolder,
					 FolderName);
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
...
Процедура ДобавитьСобытиеOutlookВТаблицуСинхронизируемыхСобытий(ТаблицаСобытийOutlook,
	 СобытиеOutlook, ContactFolder,
	 FolderName)
	...
	СтрокаТаблицы.Представление_Outlook	= ПолучитьПредставлениеСобытияOutlook(ПолучитьРеквизитОбъектаOutlook(СобытиеOutlook,
		 "ОписаниеСобытия"),
...
Процедура СопоставитьСобытия1С(СтруктураСвойств, Настройка, ТаблицаСинхронизируемыхСобытий1С)
	...
	Для Каждого СтрокаСобытий_1С Из ТаблицаСинхронизируемыхСобытий1С Цикл
		...
		Если НЕ (СоответствиеСопоставленныеСобытия[СтрокаСобытий_1С.Ссылка] = Неопределено) Тогда
			Продолжить;
		КонецЕсли;
		...
		Если (Настройка.Направление = "1СOutlook") И НЕ (СобытиеOutlook = Неопределено) Тогда
			НоваяСтрокаСобытия.Направление	= ?(ПолучитьРеквизитОбъектаOutlook(СобытиеOutlook,
				 "ДатаИзменения") > СтрокаСобытий_1С.Дата, "Outlook -> 1С",
				 "1С -> Outlook");
		Иначе
			НоваяСтрокаСобытия.Направление	= "1С -> Outlook";
		КонецЕсли;
		Если НЕ (СобытиеOutlook = Неопределено) Тогда
			НоваяСтрокаСобытия.Представление_Outlook		= ПолучитьПредставлениеСобытияOutlook(ПолучитьРеквизитОбъектаOutlook(СобытиеOutlook,
				 "ОписаниеСобытия"),
				ПолучитьРеквизитОбъектаOutlook(СобытиеOutlook, "НачалоСобытия"));
...
Процедура СопоставитьСобытияOutlook(СтруктураСвойств, Настройка, ТаблицаСинхронизируемыхСобытийOutlook)
	...
	ФормаТаблицы	= ПолучитьФорму("Обработка.CRM_ОбменСMSOutlook.Форма.Таблицы");
	ФормаТаблицы.СоответствиеОбъектов.Очистить();
	ФормаТаблицы.ЗаполнитьТаблицуСоответствиеОбъектов(ТаблицаСинхронизируемыхСобытийOutlook,
		 Настройка.ПрограммаOutlook,
		 Истина);
	Сч = 0;
	СоответствиеОбработанные = Новый Соответствие();
	Для Каждого СтрокаСобытийOutlook Из ТаблицаСинхронизируемыхСобытийOutlook Цикл
		...
		Если НЕ (СоответствиеОбработанные[СтрокаСобытийOutlook.ОбъектOutlookID] = Неопределено) Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(СсылкаНаОбъект1С) Тогда
			...
			Если ЗначениеЗаполнено(Настройка.ФильтрДатаИзмененияКонтактаOutlook) Тогда
				ДатаИзменения_1С = CRM_ОбменСMSOutlookСервер.ПолучитьДатуМодификацииВ1С(СсылкаНаОбъект1С,
					 Настройка.ПрограммаOutlook);
			КонецЕсли;
		КонецЕсли;
		Если (Настройка.Направление = "1СOutlook") И НЕ (СсылкаНаОбъект1С = Неопределено) Тогда
			НоваяСтрокаСобытия.Направление	= ?(СтрокаСобытийOutlook.ДатаИзменения > ДатаИзменения_1С,
				 "Outlook -> 1С",
				 "1С -> Outlook");
		Иначе
		...
		Иначе
			НоваяСтрокаСобытия.Объект_1С		= ПредопределенноеЗначение("Документ.CRM_Взаимодействие.ПустаяСсылка");
		КонецЕсли;
		СтруктураСвойств.События.Добавить(НоваяСтрокаСобытия);
		НоваяСтрокаСОМОбъекта					= ПолучитьСтруктуруТаблицыCOMОбъектовСобытий();
		НоваяСтрокаСОМОбъекта.СтрокаТЧ			= НоваяСтрокаСобытия.ID;
...
Процедура ЗаполнитьТаблицуСинхронизацииСобытий(СтруктураСвойств, Настройки) Экспорт
	Если НЕ Настройки.СинхронизироватьСобытия Тогда
		Возврат;
	КонецЕсли;	
	...
	Если Настройки.Направление = "1СOutlook" Тогда
		...
		КонецПопытки;
		СинхронизируемыеСобытия1С = CRM_ОбменСMSOutlookСервер.ПолучитьСинхронизируемыеСобытия1С(ТекстЗапроса,
			 Отбор,
...
// Процедура  сопоставляет  контакты outlook с  контактами 1С. 
//
Процедура СопоставитьКонтактыOutlook(СтруктураСвойств, Настройка,
	 ТаблицаСинхронизируемыхКонтактовOutlook,
	 ТолькоНовые = Ложь)
 	Количество		= ТаблицаСинхронизируемыхКонтактовOutlook.Количество();
	ФормаТаблицы	= ПолучитьФорму("Обработка.CRM_ОбменСMSOutlook.Форма.Таблицы");
	ФормаТаблицы.СоответствиеОбъектов.Очистить();
	ФормаТаблицы.ЗаполнитьТаблицуСоответствиеОбъектов(ТаблицаСинхронизируемыхКонтактовOutlook,
		 Настройка.ПрограммаOutlook,
		 Истина);
	ЗаполнитьСоответствиеСОМОбъектыКонтактов(СтруктураСвойств);
	Сч = 0;
	Для Каждого СтрокаКонтактовOutlook Из ТаблицаСинхронизируемыхКонтактовOutlook Цикл
		...
		ДатаИзменения_1С									= ОбщегоНазначенияКлиент.ДатаСеанса();
		Если ЗначениеЗаполнено(СсылкаНаОбъект1С) Тогда
			// Если это не первый обмен, то вместо даты регистрации установим дату изменения объекта.
			Если ЗначениеЗаполнено(Настройка.ФильтрДатаИзмененияКонтактаOutlook) Тогда
				ДатаИзменения_1С = CRM_ОбменСMSOutlookСервер.ПолучитьДатуМодификацииВ1С(СсылкаНаОбъект1С,
		...
		Если (Настройка.Направление = "1СOutlook") И НЕ (СсылкаНаОбъект1С = Неопределено) Тогда
			НоваяСтрокаКонтакта.Направление	= ?(СтрокаКонтактовOutlook.ДатаИзменения > ДатаИзменения_1С,
				 "Outlook -> 1С",
				 "1С -> Outlook");
		Иначе
...
Процедура ЗаполнитьТаблицуСинхронизацииКонтактов(СтруктураСвойств, Настройка) Экспорт
	...
	Если Настройка.СинхронизироватьСобытия 
		...
		Если Настройка.Направление = "1СOutlook" Тогда
			...
			СопоставитьКонтакты1С(СтруктураСвойств, Настройка, СинхронизируемыеКонтактыСобытий1С,
				 НЕ Настройка.СинхронизироватьКонтакты);
			СопоставитьКонтактыOutlook(СтруктураСвойств, Настройка, СинхронизируемыеКонтактыСобытийOutlook,
				 НЕ Настройка.СинхронизироватьКонтакты);
		ИначеЕсли Настройка.Направление = "Outlook" Тогда
			// Из 1С в Outlook
			СинхронизируемыеКонтактыСобытий1С	= ПолучитьСинхронизируемыеКонтактыСобытий1С(СтруктураСвойств, Настройка);
	...
	Если Настройка.СинхронизироватьПисьма И Настройка.СинхронизироватьКонтакты Тогда
		...
		СопоставитьКонтактыOutlook(СтруктураСвойств, Настройка, СинхронизируемыеКонтактыПисемOutlook,
			 НЕ Настройка.СинхронизироватьКонтакты);			 
	КонецЕсли;	
КонецПроцедуры
...
Процедура СинхронизироватьКонтакт(СтруктураСвойств, Настройка, СтрокаКонтактов, ТаблицаСинхронизируемыхРеквизитовК,
	...
	Если СтрокаКонтактов.Направление = "Outlook -> 1С" Тогда
		...
		Если СтрокаКонтактов.ВидКонтакта = "К"  Тогда
			...
			Если ЭтоНовыйОбъект Тогда
				СтруктураОбъекта.Реквизиты.Вставить("ДатаРегистрации", ОбщегоНазначенияКлиент.ДатаСеанса());
			Иначе	
				// Удаление объекта Outlook
		...
		ИначеЕсли СтрокаКонтактов.ВидКонтакта = "КЛК" Тогда
			...
			Если ЭтоНовыйОбъект Тогда
				...
				СтруктураКонтрагента	= Новый Структура("Реквизиты,ТабличныеЧасти",
					...
					ОбщегоНазначенияКлиент.ДатаСеанса(), ВидПартнера, Настройка.ГруппаКонтактов1С), Новый Структура);
			...
			Если Истина Тогда
				...
				СтруктураОбъекта.Реквизиты.Вставить("ДатаРегистрацииСвязи",	ОбщегоНазначенияКлиент.ДатаСеанса());
			КонецЕсли;
	...
	ИначеЕсли СтрокаКонтактов.Направление = "1С -> Outlook" Тогда
		...
		Если СтрокаКонтактов.ВидКонтакта = "КЛК" Тогда	
			Если НЕ ЗначениеЗаполнено(СтруктураОбъекта.CRM_Имя)
				 И НЕ ЗначениеЗаполнено(СтруктураОбъекта.CRM_Фамилия)
				 И НЕ ЗначениеЗаполнено(СтруктураОбъекта.CRM_Отчество) Тогда
			...
			Если Настройка.ПереноситьКомментарийКонтактаВOutlook И ЭтоНовыйОбъект Тогда
				Заметки = Настройка.КомментарийOutlook + Символы.ПС + СтрЗаменить(СтруктураОбъекта.Комментарий,
					 Настройка.Комментарий1С, "");
		...
		Если НайденныйЭлемент = Неопределено Тогда
			...
			СтруктураСвойств.СоответствиеСОМОбъектыКонтактов.Вставить(СтрокаСОМОбъектыКонтактов.СтрокаТЧ,
				 СтрокаСОМОбъектыКонтактов);
		КонецЕсли;
	КонецЕсли;
	// Сохраним связь с объектом Outlook.
...
Функция ФорматироватьПредставлениеКонтакта(Представление)
	 ...
	 Возврат ?(ПредставлениеБезПробелов = "", "", ВРег(Лев(ПредставлениеБезПробелов, 1)) 
	 	+ Прав(ПредставлениеБезПробелов,
	 	 СтрДлина(ПредставлениеБезПробелов) - 1));	
КонецФункции	
...
Функция ПолучитьСинхронизируемыеКонтактыСобытий1С(СтруктураСвойств, Настройка) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ФормаТаблицы.КонтактыСобытий Цикл
		...
		Если НЕ (СоответствиеСинхронизируемыхКонтактов[Объект1С] = Неопределено) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура СопоставитьКонтакты1С(СтруктураСвойств, Настройка, ТаблицаСинхронизируемыхКонтактов1С, ТолькоНовые = Ложь)
	...
	Для Каждого СтрокаКонтактов_1С Из ТаблицаСинхронизируемыхКонтактов1С Цикл
		сч = сч + 1;
		// Возможно контакт уже сопоставлен ранее.
		Если НЕ (СоответствиеСопоставленныхКонтактов[СтрокаКонтактов_1С.Ссылка] = Неопределено) Тогда
			Продолжить;
		КонецЕсли;
		...
		Если  (КонтактOutlook = Неопределено) И ПомеченНаУдаление Тогда
			Продолжить;
		КонецЕсли;	
		// Добавим новую строку в таблицу сопоставления объектов 1С и Outlook.
		НоваяСтрокаКонтакта						= ПолучитьСтруктуруКонтактов();
		НоваяСтрокаКонтакта.Пометка				= Настройка.СинхронизироватьКонтакты И НЕ (ТолькоНовые
			 И НЕ (КонтактOutlook = Неопределено));
		НоваяСтрокаКонтакта.Объект_1С			= СтрокаКонтактов_1С.Ссылка;
		ДатаИзменения_1С						= СтрокаКонтактов_1С.ДатаРегистрации;
		Если ЗначениеЗаполнено(НоваяСтрокаКонтакта.Объект_1С)
			 И ЗначениеЗаполнено(Настройка.ФильтрДатаИзмененияКонтактаOutlook) Тогда
			ДатаИзменения_1С = CRM_ОбменСMSOutlookСервер.ПолучитьДатуМодификацииВ1С(НоваяСтрокаКонтакта.Объект_1С,
				 Настройка.ПрограммаOutlook);
		...
		Если (Настройка.Направление = "1СOutlook") И НЕ (КонтактOutlook = Неопределено) Тогда
			НоваяСтрокаКонтакта.Направление	= ?(ПолучитьРеквизитОбъектаOutlook(КонтактOutlook,
				 "ДатаИзменения") > ДатаИзменения_1С, "Outlook -> 1С",
				 "1С -> Outlook");
		Иначе
...
Процедура ЗаполнитьСтруктуруСобытияПоНастройке(СтруктураРеквизитов, Настройки)
	Если НЕ СтруктураРеквизитов.Свойство("Дата")
		...
		СтруктураРеквизитов.Вставить("Дата", ОбщегоНазначенияКлиент.ДатаСеанса());
	КонецЕсли;
	Если НЕ СтруктураРеквизитов.Свойство("ПлановаяДата")
		...
		СтруктураРеквизитов.Вставить("ПлановаяДата", ОбщегоНазначенияКлиент.ДатаСеанса());
	КонецЕсли;
	...
	//СтруктураРеквизитов.Вставить("Важность",
		//			ПредопределенноеЗначение("Перечисление.ВариантыВажностиВзаимодействия.Обычная"));
...
Процедура ЗаполнитьСтруктуруСобытияПоОбъектуOutlook(СтруктураРеквизитов, ОбъектOutlook)
	...
	СтруктураРеквизитов.Вставить("ПлановаяДатаЗавершение",
			ПолучитьРеквизитОбъектаOutlook(ОбъектOutlook,
			"ОкончаниеМероприятия"));
	СтруктураРеквизитов.Тема		= CRM_ОбменСMSOutlookКлиентСервер.ЗаменитьНедопустимыеСимволыXML(СтруктураРеквизитов.Тема);
	СтруктураРеквизитов.Содержание	=
		CRM_ОбменСMSOutlookКлиентСервер.ЗаменитьНедопустимыеСимволыXML(СтруктураРеквизитов.Содержание);
КонецПроцедуры

// Функция заполняет структуру объекта контактной информацией.
//
Функция ЗаполнитьСтруктуруОбъектаКонтактнойИнформацией(ТаблицаСинхронизируемыхРеквизитов,
	 СтруктураОбъекта,
	 ОбъектOutlook) Экспорт
...
Функция НайтиКонтактOutlookПоАдресу(мСтруктураСвойств, мНастройки, Адрес,
	 СоответствиеКонтактовOutlookАдресам = Неопределено)
...
Функция ПолучитьСинхронизируемыеКонтактыСобытийOutlook(СтруктураСвойств, Настройка) Экспорт
	...
	Для Индекс = 1 По КолвоСтрок Цикл
		НайденныйЭлемент = ПолучитьКонтактПоOutlookID(СтруктураСвойств,
			 СинхронизируемыеКонтактыСобытийOutlook[КолвоСтрок - Индекс].ОбъектOutlookID);
		Если НЕ (НайденныйЭлемент = Неопределено)  Тогда
...
Функция ПолучитьСинхронизируемыеКонтактыПисемOutlook(СтруктураСвойств, Настройка) Экспорт
	...
	Для Каждого СинхронизируемоеПисьмо Из  СтруктураСвойств.Письма Цикл
		...
		Для Ном = 1 По КолКонтактов Цикл
			...
			Попытка
				...
				ContactFolder	= КонтактOutlook.Parent;
				ДобавитьКонтактOutlookВТаблицуСинхронизируемыхКонтактов(СинхронизируемыеКонтактыПисемOutlook,
					 КонтактOutlook, ContactFolder,
					 ContactFolder.Name);
			Исключение
				// Найдем контакт по адресу
				КонтактOutlook = НайтиКонтактOutlookПоАдресу(СтруктураСвойств, Настройка,
					 УчастникСобытия.address,
					 СоответствиеКонтактовOutlookАдресам);
				Если НЕ (КонтактOutlook = Неопределено) Тогда
					ContactFolder = КонтактOutlook.Parent;
					ДобавитьКонтактOutlookВТаблицуСинхронизируемыхКонтактов(СинхронизируемыеКонтактыПисемOutlook,
		...
		Попытка
			// Найдем контакт по адресу
			КонтактOutlook = НайтиКонтактOutlookПоАдресу(СтруктураСвойств, Настройка,
				 ОбъектOutlook.SenderEmailAddress ,
				 СоответствиеКонтактовOutlookАдресам);
		Исключение
			Продолжить;
		КонецПопытки;
	...
	Для Индекс = 1 По КолвоСтрок Цикл
		НайденныйЭлемент = ПолучитьКонтактПоOutlookID(СтруктураСвойств,
			 СинхронизируемыеКонтактыПисемOutlook[КолвоСтрок - Индекс].ОбъектOutlookID);
       	Если НЕ (НайденныйЭлемент = Неопределено) Тогда
	...
	Если СтруктураСвойств.СОМОбъектыКонтактов.Количество() > 0 Тогда
		...
		Для Каждого СтрокаТаблицы Из СтруктураСвойств.СОМОбъектыКонтактов Цикл
			Если ЗначениеЗаполнено(СтрокаТаблицы.ОбъектOutlookID) Тогда
				СтруктураСвойств.СоответствиеСОМОбъектыКонтактовОбъектOutlookID.Вставить(СтрокаТаблицы.ОбъектOutlookID,
					 СтрокаТаблицы);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция ПолучитьОбъект1С(ПрограммаOutlook, СсылкаВOutlook, ФормаТаблицы = Неопределено,
...
Процедура ЗаполнитьСтруктуруПисьмаПоОбъектуOutlook(СтруктураРеквизитов, ОбъектOutlook,
	 ПапкаOutlookEntryID, СписокПапок,
	 Входящее)
	...
	Если НЕ Входящее Тогда
		Если ВидПапки = ПредопределенноеЗначение("Перечисление.CRM_ВидыПапокЭлектроннойПочты.Исходящие") Тогда
			СтруктураРеквизитов.Вставить("СтатусПисьма",
	...
	Папка = CRM_ОбменСMSOutlookСервер.ПолучитьПапкуПисьма(СтруктураРеквизитов.УчетнаяЗапись,
		 ОбъектOutlook.Parent.Name,
		 ВидПапки);
	...
	СтруктураРеквизитов.Тема		= CRM_ОбменСMSOutlookКлиентСервер.ЗаменитьНедопустимыеСимволыXML(СтруктураРеквизитов.Тема);
	СтруктураРеквизитов.Описание	=
		CRM_ОбменСMSOutlookКлиентСервер.ЗаменитьНедопустимыеСимволыXML(СтруктураРеквизитов.Описание);
КонецПроцедуры
...
Процедура ДобавитьВложенияЭлектронногоПисьма(Объект_1С, ОбъектOutlook)
	...
	Для Ном = 1 По ОбъектOutlook.Attachments.Count Цикл
		...
		РаботаСФайламиСлужебныйВызовСервера.ДобавитьФайл(ПараметрыФайла, АдресФайлаВоВременномХранилище);
		// BSLLS:MissingCodeTryCatchEx-off
		Попытка
