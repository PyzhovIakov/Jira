...
Процедура ОбновитьОблакоТегов(Форма, ПараметрыТегов = Неопределено) Экспорт
	...
	Если Не ЭтоСеансВнешнегоПользователя Тогда
		...
		СписокВышестоящихПодразделений.ЗагрузитьЗначения(CRM_УправлениеДоступомПовтИсп.ПолучитьВышестоящиеПодразделения(ТекущийПользователь,
			Истина));
	КонецЕсли;
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	Теги.Тег КАК Тег,
		|	Теги.НомерСтроки КАК НомерСтроки
		|ПОМЕСТИТЬ Теги
		|ИЗ
		|	&Теги КАК Теги
		|;
	...
	Для Каждого СтрокаТегов Из СписокТеговОбработанный Цикл
		...
		Если (НомерСтрокиЭлементов = 1 И ТекущаяДлинаСтроки > МаксДлинаПервойСтроки)
			...
			ГруппаТегов = Элементы.Добавить("СтрокаТегов_" + НомерСтрокиЭлементов, Тип("ГруппаФормы"),
				 ?(НомерСтрокиЭлементов = 1, Элементы.ПерваяСтрока, Элементы.ОблакоТегов));
		...
		Если ЗначениеЗаполнено(СтрокаТегов.ЦветПредставление) Тогда
			ЦветФонаТега = Новый Цвет(СтрокаТегов.ЦветКрасный, СтрокаТегов.ЦветЗеленый, СтрокаТегов.ЦветСиний);
			ЦветТега = CRM_ОбщегоНазначенияКлиентСервер.ПолучитьКонтрастныйЦвет(ЦветФонаТега);
		Иначе
		...
		КомпонентыТега.Добавить(Новый ФорматированнаяСтрока(ПредставлениеТега, , ЦветТега, ЦветФонаТега));
		КомпонентыТега.Добавить(Новый ФорматированнаяСтрока(БиблиотекаКартинок.CRM_УдалитьСиняя, , , , "ТегИД_" 
			+ СтрокаТегов.НомерСтроки));
...
//
Процедура ОбновитьЗначенияОтбораТегов(Форма, МаксимальнаяДлинаСтроки = 150,
	 Подразделение = Неопределено, СписокВышестоящихПодразделений = Неопределено,
	 ИспользоватьТегНет = Ложь) Экспорт
...
Процедура ОбновитьЗначенияОтбораПериодов(Форма, БудущиеПериоды = Ложь) Экспорт
	...
	ПериодПроизвольный.Значение.Вариант = ВариантСтандартногоПериода.ПроизвольныйПериод;
