...
// Процедура выполняет вставку результата распознавания в место, в котором установлен курсор.
//
// Параметры:
//  Форма					 - ФормаКлиентскогоПриложения	 - Форма.
//  Элемент					 - ГруппаФормы, ТаблицаФормы, ПолеФормы, КнопкаФормы - Элемент формы.
//  ИмяРеквизита			 - Строка	 - Имя реквизита.
//  РезультатРаспознавания	 - РезультатРаспознаванияРечи	 - Результат распознавания.
//
...
// Процедура выполняет вставку текста в место, в котором установлен курсор.
//
// Параметры:
//  Форма			 - ФормаКлиентскогоПриложения	 - Форма.
//  Элемент			 - ГруппаФормы, ТаблицаФормы, ПолеФормы, КнопкаФормы - Элемент формы.
//  ИмяРеквизита	 - Строка	 - Имя реквизитаю
//  Текст			 - Строка	 - Текс для вставки.
//  ВыделитьТекст	 - Булево	 - Признак необходимости выделения текста.
//
...
Процедура ВставитьТекстПоКурсоруВПолеВвода(Форма, Элемент, ИмяРеквизита, Знач Текст, ВыделитьТекст)
	...
	Если ПоследнийСимволТребуетТРег(ПредыдущийФрагмент) Или ПустаяСтрока(ПредыдущийФрагмент) Тогда
	// -CRM
		Текст = CRM_ОбработкаРаспознанногоТекстаКлиентСервер.ТРегВнутр(Текст);
	КонецЕсли;
	...
	Если СтрНайти(ИмяРеквизита, ".") > 0 Тогда
		МассивЧастейИмени = СтрРазделить(ИмяРеквизита, ".");
		Если МассивЧастейИмени.Количество() = 2 Тогда
			Форма[МассивЧастейИмени[0]][МассивЧастейИмени[1]] = НовыйТекст;
		ИначеЕсли МассивЧастейИмени.Количество() = 3 Тогда
			Форма[МассивЧастейИмени[0]][МассивЧастейИмени[1]][МассивЧастейИмени[2]] = НовыйТекст;
		ИначеЕсли МассивЧастейИмени.Количество() = 4 Тогда
			Форма[МассивЧастейИмени[0]][МассивЧастейИмени[1]][МассивЧастейИмени[2]][МассивЧастейИмени[3]] = НовыйТекст;
		КонецЕсли;
	Иначе
		Форма[ИмяРеквизита] = НовыйТекст;
	КонецЕсли;
...
Процедура ВставитьТекстПоКурсоруВПолеHTMLДокумента(Форма, Элемент, Знач Текст, ВыделитьТекст)
	...
	Если ПоследнийСимволТребуетТРег(ПредыдущийФрагмент) Или ПервыйПроход Или ПустаяСтрока(ПредыдущийФрагмент) Тогда
	// -CRM
		Текст = CRM_ОбработкаРаспознанногоТекстаКлиентСервер.ТРегВнутр(Текст);
	КонецЕсли;
	...
	Если СтрНайти(Текст, "table") = 0 Тогда
		...
		Для Индекс = 0 По Токены.ВГраница() - 1 Цикл
			...
			Если Не ПустаяСтрока(Токен) Тогда
				// +CRM
				el = ДокументHTML.createElement("div");
				el.innerHTML = Токен;
				// -CRM
				range.insertNode(el);
