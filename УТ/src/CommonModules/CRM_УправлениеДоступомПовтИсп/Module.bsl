...
//
Функция ПолучитьВышестоящиеПодразделения(Пользователь, ВключатьСвоеПодразделение = Ложь,
	 ВключатьРабочиеГруппы = Ложь) Экспорт
...
Функция ПолучитьПодразделенияВКоторыхРуководитель(Пользователь, ПодчиненныеПодразделения = Ложь) Экспорт
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ПодчиненныеПодразделения Тогда
			CRM_МетодыМодулейМенеджеровСправочников.ПолучитьПодчиненныеПодразделенияПользователя(ВыборкаДетальныеЗаписи.Ссылка,
				 МассивПодразделений);
		КонецЕсли;
	КонецЦикла;
