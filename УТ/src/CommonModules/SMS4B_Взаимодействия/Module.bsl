...
Функция ОтправкаSMSПоДокументу(Документ) Экспорт
	...
	Если Не ОтправкаSMS.НастройкаОтправкиSMSВыполнена() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не выполнены настройки отправки SMS.';
			|en='Settings of SMS message sending are not executed.'"), ,
			 "Объект");
	...
	РезультатОтправки = SMS4B_ОтправкаSMS.ОтправитьSMS(НомераПолучателей, Документ.ТекстСообщения,
		 Документ.SMS4B_ИмяОтправителя, Документ.ОтправлятьВТранслите,
...
Процедура ПроверитьСтатусыДоставкиSMS(СообщениеSMS, Модифицированность) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	И СообщениеSMSАдресаты.СостояниеСообщения В(&СостоянияСообщений)";
	
	Запрос.УстановитьПараметр("СообщениеSMS", СообщениеSMS.Ссылка); 
	
	СостоянияСообщений = Новый Массив;
	СостоянияСообщений.Добавить(Перечисления.СостоянияСообщенияSMS.ОтправляетсяПровайдером);
	СостоянияСообщений.Добавить(Перечисления.СостоянияСообщенияSMS.ОтправленоПровайдером);
	СостоянияСообщений.Добавить(Перечисления.СостоянияСообщенияSMS.ОшибкаПолученияСтатусаУПровайдера);
	
	Запрос.УстановитьПараметр("СостоянияСообщений", СостоянияСообщений);
...
Процедура ОтправкаSMS() Экспорт
	...
	Пока ВыборкаДокументы.Следующий() Цикл
		...
		Попытка
			...
			РезультатОтправки = SMS4B_ОтправкаSMS.ОтправитьSMS(ТаблицаАдресатовСообщения, ТекстПоУмолчанию,
				 ИмяОтправителя, ОтправлятьВТранслите,
				 ДополнительныеПараметры);
			
			Если РезультатОтправки.ОтправленныеСообщения.Количество() = 0 Тогда
				РегистрироватьОшибку = Ложь;
				ВызватьИсключение "ПродолжаемВыполнение";
			КонецЕсли;
...
Процедура ОбновлениеСтатусовДоставкиSMS() Экспорт
	...
	Пока ВыборкаДокументы.Следующий() Цикл
		...
		Попытка
			...
			СтатусыДоставки =
				SMS4B_ОтправкаSMS.SMS4B_СтатусыДоставки(ТаблицаСообщений.ВыгрузитьКолонку("ИдентификаторСообщения"));
			Для Каждого СтрокаСтатуса Из СтатусыДоставки Цикл
			...
			Для Каждого ИзменившийсяСтатус Из ТаблицаИзменившихсяСтатусов Цикл
				ДокументОбъект.Адресаты[ИзменившийсяСтатус.НомерСтроки - 1].СостояниеСообщения =
					ИзменившийсяСтатус.СостояниеСообщения;
			КонецЦикла;
...
Процедура SMS4B_ФормаНастроекПриСозданииНаСервере(Форма) Экспорт
	...
	ПодсказкаОписания = НСтр("ru='Узнать больше о расширенных возможностях SMS4B для бизнеса';
		|en='Learn more about the advanced features of SMS4B for business'");
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы, ИмяКонтейнераФормы,
		 "Видимость",
		 ЭтоSMS4B);
	...
	Если ЭтоSMS4B Тогда
		...
		Форма.Элементы.ИмяОтправителя.УстановитьДействие("ОбработкаВыбора",
			 "Подключаемый_SMS4B_ИмяОтправителяОбработкаВыбора");
		...
		Если ИменаОтправителя.Количество() > 0 Тогда
			...
			КонецЦикла;
			Форма.Элементы.ИмяОтправителя.СписокВыбора.Добавить("SMS4B_Подключиться",
				 НСтр("ru='Подключиться и обновить';en='Connect and update'"), ,
				 БиблиотекаКартинок.Обновить);
		Иначе
			Форма.Элементы.ИмяОтправителя.СписокВыбора.Добавить("SMS4B_Подключиться",
				 НСтр("ru='Подключиться и заполнить';en='Connect and fill in'"), ,
				 БиблиотекаКартинок.Обновить);
		КонецЕсли;
...
Функция SMS4B_ЗаполнитьТекстПоШаблонуАвтотекста(ТекстШаблона, Контакт, Знач ТаблицаАвтотекста) Экспорт
	...
	Для Каждого СтрокаАвтотекста Из ТаблицаАвтотекста Цикл
		...
		Если СтрНайти(СформированныйТекст, СтрокаАвтотекста.Тэг) = 0 Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ОтразитьРезультатыОтправкиSMSВДокументе(ДокументОбъект, РезультатОтправки)
	...
	Для Каждого ОтправленноеСообщение Из РезультатОтправки.ОтправленныеСообщения Цикл
		Для Каждого НайденнаяСтрока Из ДокументОбъект.Адресаты.НайтиСтроки(Новый Структура("НомерДляОтправки",
			 ОтправленноеСообщение.НомерПолучателя)) Цикл
...
Функция СтатусДокументаСтрокой(Документ)
	...
	СтатусСтрокой = СтрЗаменить(НСтр("ru='Всего: %ВсегоКол%';en='In total %TotalNumber%'"),
		 "%ВсегоКол%", ВсегоКоличество) 
		+ ";"
