...
Функция ВыгрузкаЗагрузкаШаблоновСформироватьСтруктуруВыбранныхРеквизитов(СписокРеквизитов)
	...
	Для каждого Строка Из СписокРеквизитов.Строки Цикл
		...
		Иначе
			ШаблонныеРеквизиты.Вставить(Строка.ИмяРеквизита,
				 ВыгрузкаЗагрузкаШаблоновСформироватьСтруктуруВыбранныхРеквизитов(Строка));
		КонецЕсли;
	КонецЦикла;
...
Функция ЗагрузитьРеквизитыДокументаИзШаблона(ДокументОбъект, Шаблон) Экспорт
	...
	Пока ФайлXML.Прочитать() Цикл
		...
		ИначеЕсли НЕ ФайлXML.Имя = "ШаблонДокумента" И ФайлXML.ТипУзла = ТипУзлаXML.НачалоЭлемента
			 И ФайлXML.КоличествоАтрибутов() > 0 Тогда
			...
			Пока ФайлXML.ПрочитатьАтрибут() Цикл
				...
				Если ФайлXML.Имя <> "Пометка" Тогда
					...
					Если НЕ (ТипЗнч(Значение) = Тип("Неопределено")) И ОбщегоНазначения.ЭтоСсылка(ТипЗнч(Значение))
...
Процедура ЗаполнитьРеквизитыОбъектаПоШаблону(Объект, Шаблон)
	...
	Для Каждого СтрокаДерева Из ДеревоРеквизитов.Строки Цикл
		...
		Если СтрокаДерева.Строки.Количество() > 0 Тогда
			Если МетаданныеОбъекта.ТабличныеЧасти.Найти(СтрокаДерева.ИмяРеквизита) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
...
Функция ПолучитьДеревоРеквизитовОбъекта(Док)
	...
	СтрокаДереваТабличнаяЧасть.ТипРеквизита = Новый ОписаниеТипов("Дата", , ,
	...
	Для Каждого ТабличнаяЧасть Из МетаданныеДокумента.ТабличныеЧасти Цикл
		Если Док[ТабличнаяЧасть.Имя].Количество() > 0 Тогда
			...
			СтрокаДереваТабличнаяЧасть.ПредставлениеРеквизита = ?(ЗначениеЗаполнено(ТабличнаяЧасть.Синоним),
		...
		ИначеЕсли НЕ ФайлXML.Имя = "ШаблонДокумента" И ФайлXML.ТипУзла = ТипУзлаXML.НачалоЭлемента
			...
			Иначе
				...
				Если ЗаполняемОбъект Тогда
					...
					Попытка
						СтрокаДереваСтрокаТЧ.ПредставлениеРеквизита	=
							ОбъектМетаданных.ТабличныеЧасти[ИмяТЧ].Реквизиты[ИмяРеквизита].Синоним;
			...
			Пока ФайлXML.ПрочитатьАтрибут() Цикл
				...
				Иначе
					Если ЗаполняемОбъект ИЛИ (НЕ ФлагПродолжить И НЕ ИмяТЧ = "ДополнительныеРеквизиты") Тогда
						...
						Если НЕ (ТипЗнч(Значение) = Тип("Неопределено"))
...
Функция ПолучитьШаблон(Владелец, ИмяМетаданного) Экспорт
	...
	|	CRM_ШаблоныОбъектов.ИмяОбъекта КАК ИмяОбъекта,
...
Процедура ЗаполнитьОбъектПоШаблону(Объект, ШаблонСсылка)
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "CRM_Шаблон") Тогда
		Объект.CRM_Шаблон = ШаблонСсылка;
	КонецЕсли;
...
Процедура CRM_ШаблонОбъектовПриЗаписи(Источник, Отказ) Экспорт
	...
	Если Источник.ДополнительныеСвойства.Свойство("ЭтоНовый") И Источник.ДополнительныеСвойства.ЭтоНовый Тогда
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Источник, "CRM_Шаблон")
			И ОбщегоНазначения.ЭтоСсылка(ТипЗнч(Источник.CRM_Шаблон))
			И ЗначениеЗаполнено(Источник.CRM_Шаблон)Тогда
			Шаблон = Источник.CRM_Шаблон;
...
//
Процедура ЗаполнитьПредставлениеОбъекта(ШаблонОбъект, ПредставлениеОбъекта, ОписаниеРеквизитов) Экспорт
...
Процедура СформироватьДополнительныеЭлементыФормы(Форма) Экспорт
	...
	Если Форма.Команды.Найти(ИмяКомандыЗагрузки) = Неопределено Тогда
		...
		Команда.Подсказка = НСтр("ru='Заполнить документ на основании ранее сохраненного шаблона';
			|en='Fill the document on the basis of earlier saved template'");
