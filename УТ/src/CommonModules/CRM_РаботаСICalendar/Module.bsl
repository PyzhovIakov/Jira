...
Функция ДатаФорматаICalendar(Дата, ФорматДаты = "УниверсальноеВремя", ТипДаты = "") Экспорт
	...
	Результат = "";
	ТолькоДатаСтрока = "ТолькоДата";
	ШаблонСтрокиПараметров = "TZID=%1;VALUE=DATE:%2";
	СекундВСутках = 86400;
	
	Если ФорматДаты = ТолькоДатаСтрока И ТипДаты = "ДатаНачала" 
		Или ФорматДаты = ТолькоДатаСтрока Тогда
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСтрокиПараметров,
			ЧасовойПоясСеанса(),
			Формат(Дата, "ДФ=yyyyMMdd"));
	ИначеЕсли ФорматДаты = ТолькоДатаСтрока И ТипДаты = "ДатаОкончания" Тогда
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСтрокиПараметров,
			ЧасовойПоясСеанса(),
			Формат(Дата + СекундВСутках, "ДФ=yyyyMMdd"));
...
// Объединяет некорректно разбитые подстроки строки iCalendar.
//
// Параметры:
//  МассивСтрок - Массив - массив подстрок из СтрокаiCalendar.
//
// Возвращаемое значение:
//  Массив - обработанный массив подстрок.
//
Функция ОбработатьМассивСтрок(МассивСтрок)
	
	Счетчик = МассивСтрок.Количество() - 1;
	Пока Счетчик > 0 Цикл
		Если СтрНачинаетсяС(МассивСтрок[Счетчик], " ") Тогда
			МассивСтрок[Счетчик - 1] = СокрП(МассивСтрок[Счетчик - 1])
				+ Сред(МассивСтрок[Счетчик], 2);
...
Функция СтруктураiCalendar(СтрокаiCalendar) Экспорт
	...
	МассивСтрок = ОбработатьМассивСтрок(МассивСтрок);
	ЧтениеСобытия = Ложь;
	...
	Для Каждого Строка Из МассивСтрок Цикл
		...
		ИначеЕсли ЧтениеОписания И Не СтрНачинаетсяС(Строка, " ") Тогда
			...
			ПерваяСтрокаОписания = CRM_РаботаСICalendar.РаскодироватьСтрокуiCalendar(ПерваяСтрокаОписания);
			ЧтениеОписания = Ложь;
		КонецЕсли;
		...
		ИначеЕсли ЧтениеОстальногоОписания И Не СтрНачинаетсяС(Строка, " ") Тогда
			...
			ОстальноеОписание = CRM_РаботаСICalendar.РаскодироватьСтрокуiCalendar(ОстальноеОписание);
			ЧтениеОстальногоОписания = Ложь;
		КонецЕсли;
		...
		ИначеЕсли СокрЛП(Строка) = "END:VTODO" Тогда
			Если Не Результат.Свойство(ДатаНачалаСтрока) И Не Результат.Свойство(ДатаОкончанияСтрока) Тогда
				Результат.Вставить(ДатаНачалаСтрока, Результат.ДатаСоздания);
				Результат.Вставить(ДатаОкончанияСтрока, Результат.ДатаСоздания);
			КонецЕсли;
		...
		Если ЧтениеСобытия Тогда
			...
			Если ЧтениеНапоминания Тогда
				Если Лев(Строка, СтрДлина("TRIGGER")) = "TRIGGER" Тогда
					// +CRM
					Если СтрНайти(Строка, "VALUE=DATE-TIME") = 0 Тогда
			...
			Иначе
				...
				ИначеЕсли Лев(Строка, СтрДлина(КодДатыНачала)) = КодДатыНачала Тогда
					Результат.Вставить(ДатаНачалаСтрока, ВыделитьВремя(Строка, КодДатыНачала));
					Если Результат.ДатаНачала = Неопределено Тогда
						Возврат Неопределено;
					КонецЕсли;
				ИначеЕсли Лев(Строка, СтрДлина(КодДатыКонца)) = КодДатыКонца Тогда
					Результат.Вставить(ДатаОкончанияСтрока, ВыделитьВремя(Строка, КодДатыКонца));
				...
				ИначеЕсли Лев(Строка, СтрДлина("ATTENDEE")) = "ATTENDEE" Тогда
					Участник = Новый Структура;
					Если СтрНайти(Строка, "PARTSTAT=DECLINED") > 0 Тогда
						Участник.Вставить("УчастникОтклик", "0");
					ИначеЕсли СтрНайти(Строка, "PARTSTAT=ACCEPTED") > 0 Тогда
						Участник.Вставить("УчастникОтклик", "1");
					Иначе
						Участник.Вставить("УчастникОтклик", "2");
					КонецЕсли;
					Участник.Вставить("УчастникЭтоЯ", Ложь);
					Участник.Вставить("УчастникИмя", "");
					УчастникАдрес = "";
					ПозицияАдреса = СтрНайти(Строка, "mailto:");
					Если ПозицияАдреса > 0 Тогда
						УчастникАдрес = Сред(Строка, ПозицияАдреса + СтрДлина("mailto:"));
						Если Прав(УчастникАдрес, 1) = "." Тогда
							УчастникАдрес = Лев(УчастникАдрес, СтрДлина(УчастникАдрес) - 1);
						КонецЕсли;
					КонецЕсли;
					Участник.Вставить("УчастникАдрес", УчастникАдрес);
...
//
// Параметры:
//  СобытияКалендаря - ТаблицаЗначений	 - описание событий календаря во внутреннем формате
//  	см. ОбменСВнешнимиКалендарями.НовоеОписаниеСобытийКалендаря.
//  Организатор		 - Строка, Неопределено	 - адрес электронной почты организатора.
// 
// Возвращаемое значение:
//  Строка - тело файла формата .ics или HTTP запроса.
//
Функция СобытияiCalendar(СобытияКалендаря, Сервис) Экспорт
	...
	Для Каждого Событие Из СобытияКалендаря Цикл
		Результат = Результат + Символы.ПС + СобытиеiCalendar(Событие, Событие.ВремяНапоминания, Сервис);
	КонецЦикла;
...
Функция СобытиеiCalendar(ОписаниеСобытия, ВремяНапоминания, Сервис)
	
	МассивСобытиеiCalendar = Новый Массив;
	ЭтоСервисМэйл = (Сервис = Справочники.CRM_СервисыКалендарей.MailRu);
	...
	Если ОписаниеСобытия.Выполнена = Неопределено Тогда
		...
		Участники = Новый Массив;
		Для Каждого ДанныеУчастника Из ОписаниеСобытия.Ссылка.СвоиЛица Цикл
			Если Не ЭтоСервисМэйл
				И ДанныеУчастника.Лицо = ОписаниеСобытия.Ссылка.Ответственный Тогда
				МассивСобытиеiCalendar.Добавить("ORGANIZER;CN="
					+ ДанныеУчастника.Лицо
					+ ":mailto:" + ДанныеУчастника.Адрес);
			КонецЕсли;
			Если ДанныеУчастника.ПосетитМероприятие = 0 Тогда
				Участие = "PARTSTAT=DECLINED;";
			ИначеЕсли ДанныеУчастника.ПосетитМероприятие = 1 Тогда
				Участие = "PARTSTAT=ACCEPTED;";
			ИначеЕсли ДанныеУчастника.ПосетитМероприятие = 2 Тогда
				Участие = "PARTSTAT=TENTATIVE;";
			Иначе
				Участие = "PARTSTAT=NEEDS-ACTION;";
			КонецЕсли;
			УчастникИмя = ДанныеУчастника.Лицо;
			НовыйУчастник = "ATTENDEE;CUTYPE=INDIVIDUAL;ROLE=REQ-PARTICIPANT;" 
				+ Участие + "RSVP=TRUE;CN=" + УчастникИмя 
				+ ";X-NUM-GUESTS=0:MAILTO:" + ДанныеУчастника.Адрес;
			МассивСобытиеiCalendar.Добавить(НовыйУчастник);
		КонецЦикла;
		Для Каждого ДанныеУчастника Из ОписаниеСобытия.Ссылка.СторонниеЛица Цикл
			Если ДанныеУчастника.ПосетитМероприятие = 0 Тогда
				Участие = "PARTSTAT=DECLINED;";
			ИначеЕсли ДанныеУчастника.ПосетитМероприятие = 1 Тогда
				Участие = "PARTSTAT=ACCEPTED;";
			...
			Иначе
				Участие = "PARTSTAT=NEEDS-ACTION;";
			КонецЕсли;
			Если ЗначениеЗаполнено(ДанныеУчастника.КонтактноеЛицо) Тогда
				УчастникИмя = ДанныеУчастника.КонтактноеЛицо;
			ИначеЕсли ЗначениеЗаполнено(ДанныеУчастника.Партнер) Тогда
				УчастникИмя = ДанныеУчастника.Партнер;
			Иначе
				УчастникИмя = ДанныеУчастника.ПотенциальныйКлиент;
			КонецЕсли;
			НовыйУчастник = "ATTENDEE;CUTYPE=INDIVIDUAL;ROLE=REQ-PARTICIPANT;" 
				+ Участие + "RSVP=TRUE;CN=" + УчастникИмя 
				+ ";X-NUM-GUESTS=0:MAILTO:" + ДанныеУчастника.Адрес;
