...
Процедура сфпПодключитьсяНативнаяКомпонента(ПоказыватьСообщения = Ложь) Экспорт
	...
	Если ТипСистемы = ТипПлатформы.Windows_x86 ИЛИ ТипСистемы = ТипПлатформы.Windows_x86_64 Тогда
		...
		ДополнительныеПараметры = Новый Структура("ПоказыватьСообщения,КаталогСофтфона",
			 ПоказыватьСообщения,
			 КаталогСофтфона);
		Оповещение = Новый ОписаниеОповещения("ПриПроверкеСуществованияКаталогаСофтФона",
			 сфпСофтФонПроКлиентНативнаяКомпонента,
	...
	ИначеЕсли ПоказыватьСообщения Тогда
		ПоказатьОповещениеПользователя(
			...
			НСтр("ru='Работа Софтфона возможна только под управлением ОС Windows!';
				|en='Softphone operation is possible only with Windows OS!'"),
...
Процедура ПриПроверкеСуществованияКаталогаСофтФона(Существует, ДополнительныеПараметры) Экспорт
	...
	#Если НЕ ВебКлиент Тогда
		...
		Оповещение = Новый ОписаниеОповещения("сфпПодключитьсяНативнаяКомпонента_Продолжить",
			 сфпСофтФонПроКлиентНативнаяКомпонента,
			 ДополнительныеПараметры);
		НачатьКопированиеФайла(Оповещение, ДополнительныеПараметры.КаталогСофтфона 
			+ "SoftPhoneClient.ini",
...
Процедура сфпПодключитьсяНативнаяКомпонента_Продолжить(СкопированныйФайл, ДополнительныеПараметры) Экспорт
	...
	НомерСимволаПутиКПанели = СтрНайти(ТекстФайла, "[ClientPath]");
	Если НомерСимволаПутиКПанели = 0 Тогда
		Если ДополнительныеПараметры.ПоказыватьСообщения Тогда
			ПоказатьПредупреждение(, Нстр("ru='Не удалось найти установленную панель Софтфона. Пожалуйста,
				| переустановите панель!';en='The installed Softphone panel could not be found. Please reinstall the panel!'"));
		КонецЕсли;
	...
	СистемнаяИнформация = Новый СистемнаяИнформация();
	ТипСистемы = СистемнаяИнформация.ТипПлатформы;
	Если ТипСистемы = ТипПлатформы.Linux_x86_64 ИЛИ ТипСистемы = ТипПлатформы.MacOS_x86_64
	...
	ПутьККомпоненте = СтрЗаменить(ПутьККомпоненте, "SPPanel3.exe", "NativeComponent\" + ИмяКаталогаКомпоненты 
		+ "\SP_ClientNative.dll");
	...
	Оповещение = Новый ОписаниеОповещения("ПриПроверкеСуществованияКомпоненты",
		 сфпСофтФонПроКлиентНативнаяКомпонента,
		 ДополнительныеПараметры);
		...
		Если НЕ Существует Тогда
			Если ДополнительныеПараметры.ПоказыватьСообщения Тогда
				ПоказатьПредупреждение(,
					 Нстр("ru='Не найдена компонента для работы с Софтфоном. Версия панели Софтфона устарела!';
					|en='No component was found to work with Softphone. The version of the Softphone panel is outdated!'"));
			КонецЕсли;
			
			Возврат;
		КонецЕсли;
		
		Оповещение = Новый ОписаниеОповещения("сфпПодключитьсяНативнаяКомпонента_Закончить",
			 сфпСофтФонПроКлиентНативнаяКомпонента,
			 ДополнительныеПараметры);
...
Процедура сфпПодключитьсяНативнаяКомпонента_Закончить(СкопированныйФайл, ДополнительныеПараметры) Экспорт
	...
	Оповещение = Новый ОписаниеОповещения("сфпПодключитьсяНативнаяКомпонента_Подключить",
		 сфпСофтФонПроКлиентНативнаяКомпонента,
		 ДополнительныеПараметры);
...
Процедура сфпПодключитьсяНативнаяКомпонента_Подключить(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если НЕ Подключено Тогда
		Если ДополнительныеПараметры.ПоказыватьСообщения Тогда
			ПоказатьПредупреждение(,
				 НСтр("ru='Не удалось подключить компоненту Софтфона! Проверьте правильность её установки и наличия на диске.';
				|en='Unable to connect the Softphone component! Check that it is installed correctly and that it is on the disk.'"),
				 5);
		КонецЕсли;
	...
	json = сфпЛицензированиеСервер.ЗаполнитьСистемнуюИнформацию(ВерсияПанели);
		
	// BSLLS:MissingCodeTryCatchEx-off		
	Попытка
		сфпПанельУправления.RegistrationInfo(json);
	Исключение
	...
	ПоказатьОповещениеПользователя("Компонента Софтфон", , "Версия: " + ВерсияПанели,
		 БиблиотекаКартинок.сфпПанельЗвонка_ПустойАватар,
...
Процедура сфпЗаполнитьПрефиксыИНастройкиНативнаяКомпонента() Экспорт
	...
	сфпСтруктураНастроек =
		сфпСофтФонПроСервер.UnJSON(сфпСофтФонПроКлиент.ЗаменитьНедопустимыеСимволыXML(сфпСтрокаНастроек));
	
	Если сфпСтруктураНастроек.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Если СтруктураДанных["UseRouter"] <> Неопределено
		 И СтруктураДанных["UseRouter"] <> сфпПараметрыСервера.ИспользоватьМаршрутизацию Тогда
		ТребуетсяОбновитьИнтерфейс = Истина;
	КонецЕсли;
	Если СтруктураДанных["HistoryOn"] <> Неопределено
		 И СтруктураДанных["HistoryOn"] <> сфпПараметрыСервера.ИспользоватьИсториюЗвонков Тогда
		// Если изменилась видимость отчетов
		ТребуетсяОбновитьИнтерфейс = Истина;
	КонецЕсли;
	Если СтруктураДанных["UseHistory"] <> Неопределено
		 И СтруктураДанных["UseHistory"] <> сфпПараметрыСервера.ИспользоватьИсториюЗвонков Тогда
		ТребуетсяОбновитьИнтерфейс = Истина;
	КонецЕсли;
