...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_ДоступноеВремяФизЛица" Или ИмяСобытия = "Запись_ЗанятостьФизЛица" Или ИмяСобытия
		= "Запись_Мероприятие" Или ИмяСобытия = "Запись_Отсутствие" Или ИмяСобытия
		= "Закрытие_НастройкиСинхронизацииКалендаря" Тогда
...
Процедура СоздатьГлавнуюЗадачу(Команда)
	...
	Если cbr_ОбработчикиФормыЗадачиСервер.ЕстьПользовательскиеНастройки() Тогда
		ПараметрыФормы = Новый Структура("ИсточникПараметров,Исполнитель,УпрощеннаяФорма", Объект.Ссылка,
			Объект.Постановщик, Истина);
...
Процедура СоздатьПодчиненнуюЗадачу(Команда)
	...
	Иначе
		Если cbr_ОбработчикиФормыЗадачиСервер.ЕстьПользовательскиеНастройки() Тогда
			ПараметрыФормы = Новый Структура("ГлавнаяЗадача,ИсточникПараметров,Исполнитель,УпрощеннаяФорма",
				Объект.Ссылка, Объект.Ссылка, Объект.Постановщик, Истина);
...
Процедура НазначитьИсполнителяИзГруппы(Команда)
	...
	ОткрытьФорму("Справочник.Пользователи.Форма.cbr_ФормаВыбора", ПараметрыФормы, , , , ,
		ОписаниеОповещенияОЗакрытииФормыВыбора);
КонецПроцедуры
...
Процедура ОбновитьВидимостьЭлементов()
	...
	ВидимостьКнопкиОтправитьНаИсполнение();
	ОбновитьВидимостьОсновногоМенеджера();
...
&НаСервере
Процедура ВидимостьКнопкиОтправитьНаИсполнение()
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаОтправитьНаИсполнение", "Видимость",
		Ложь);
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Запрос.Текст =
		"ВЫБРАТЬ
		|	cbr_Задача.Автор = &ТекущийПользователь КАК ЭтоАвтор,
		|	cbr_Задача.Постановщик = &ТекущийПользователь КАК ЭтоПостановщик,
		|	cbr_Задача.ГлавныйМенеджер = &ТекущийПользователь КАК ЭтоГлавныйМенеджер,
		|	ЕстьNull(cbr_СтатусыЗадач.Новый, Ложь) как НоваяЗадача
		|ИЗ
		|	Документ.cbr_Задача КАК cbr_Задача
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.cbr_СтатусыЗадач.СрезПоследних(, Задача = &Ссылка) КАК
		...
		Если Выборка.Следующий() Тогда
			ЭтоНужныйПользователь = Выборка.ЭтоАвтор Или Выборка.ЭтоПостановщик Или Выборка.ЭтоГлавныйМенеджер;
			Если ЭтоНужныйПользователь И Выборка.НоваяЗадача Тогда
...
Процедура ВидимостьКнопкиТрудозатраты()
	...
	Если Выборка.Следующий() Тогда
		Если Объект.КонтрольВремени И Не Выборка.Закрыт Тогда
			...
			КонецЕсли;
		Иначе
			Если Объект.УчетВремени И Выборка.ЭтоПользователь И Выборка.ЭтоИсполнитель И Выборка.ЗадачаВзята
				И Не Выборка.Закрыт Тогда
				ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ВвестиТрудозатраты", "Видимость",
					Истина);
			КонецЕсли;
		КонецЕсли;
...
Процедура ПереключениеВидаФормы(Упрощенный = Истина)
	...
	Иначе
		...
		Элементы.Переместить(Элементы.Наименование, Элементы.ГруппаНаименованиеИКнопкаСвязатьЗадачу,
			Элементы.ГруппаСвязанныеВнешняя);
