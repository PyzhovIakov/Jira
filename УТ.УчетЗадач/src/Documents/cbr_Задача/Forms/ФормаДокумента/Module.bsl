...
Процедура ОтклонитьЗадачу(Команда) Экспорт
	ПараметрыФормы = Новый Структура("Постановщик,ГлавныйМенеджер,Основание,Автор", Объект.Постановщик,
		Объект.ГлавныйМенеджер, Объект.Ссылка, Объект.Автор);
...
Процедура ЗадачаВКалендарь(Команда)
	cbr_КалендарьКлиент.СоздатьЗаписьКалендаряЗадача(Объект.Ссылка, Исполнитель,
		ЭтотОбъект.ОписаниеЗадачиДокумент.ПолучитьТекст());
КонецПроцедуры
