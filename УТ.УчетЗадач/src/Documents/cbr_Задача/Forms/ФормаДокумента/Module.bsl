...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если (Не cbr_ОбработчикиФормыЗадачиСервер.ЕстьПользовательскиеНастройки() И Не ЗначениеЗаполнено(Объект.Ссылка))
		...
		Пока Выборка.Следующий() Цикл
			cbr_ЭлементыФормы.СоздатьКоманду(ЭтотОбъект, "МеткаКоманда" + Выборка.Код,
...
Функция ПолучениеВыборкиМеток()
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ПРЕДСТАВЛЕНИЕ(cbr_МеткиЗадач.Наименование) КАК Наименование,
...
Процедура ЗакрытьЗадачуНаСервере()
	...
	Если Объект.ТребуетКонтроляПоЗавершению Тогда
		...
		ПодчиненнаяЗадача.Наименование = "Поверка задачи " + Объект.Наименование + " от " + Формат(Объект.Дата,
			"ДФ=dd.MM.yyyy");
