...
Процедура ПланировщикПередСозданием(Элемент, Начало, Конец, ЗначенияИзмерений, Текст, СтандартнаяОбработка)
	...
	ФизЛицоКалендаря = ПользователиКлиент.АвторизованныйПользователь();
	cbr_КалендарьКлиент.ОбработкаПередСозданием(
		Элемент, Начало, Конец, ЗначенияИзмерений, Текст, СтандартнаяОбработка, ИспользоватьБыстроеРедактирование,
		ФизЛицоКалендаря, НастройкиОтображения, УникальныйИдентификатор, Объект.Ссылка);
	Иначе
		СтандартнаяОбработка = Ложь;
		ОбщегоНазначенияКлиент.СообщитьПользователю("Проведите документ задачи");
	КонецЕсли;
КонецПроцедуры
...
Процедура ПланировщикПриОкончанииРедактирования(Элемент, НовыйЭлемент, ОтменаРедактирования)
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
	ФизЛицоКалендаря = ПользователиКлиент.ТекущийПользователь();
	cbr_КалендарьКлиент.ОбработкаОкончанияРедактированияЭлемента(
		Элемент, НовыйЭлемент, ОтменаРедактирования,
		Планировщик, НастройкиОтображения, УникальныйИдентификатор, ФизЛицоКалендаря, Объект.Ссылка);
	ОбновитьОтображениеКлиент();
	Иначе
		ОбщегоНазначенияКлиент.СообщитьПользователю("Проведите документ задачи");
	КонецЕсли;
...
Процедура ВыбратьГлавнуюЗадачу(Команда)
	...
	СписокЗапрещенных.Добавить(Объект.Ссылка);
...
Процедура ИнициализироватьМетки()
	...
	Если (Не cbr_ОбработчикиФормыЗадачиВызовСервера.ЕстьПользовательскиеНастройки() И Не ЗначениеЗаполнено(Объект.Ссылка))
		...
		Пока Выборка.Следующий() Цикл
			cbr_ЭлементыФормыСервер.СоздатьКоманду(ЭтотОбъект, "МеткаКоманда" + Выборка.Код, Выборка.Наименование,
				"ПрисвоитьЗначениеМетке");

			НоваяКнопка = cbr_ЭлементыФормыСервер.СздКнопка(ЭтотОбъект, "МеткаКоманда" + Выборка.Код, Элементы.Метки,
...
Процедура ОчисткаМетокНаФормеНаСервере()
	...
	Элемент = Элементы.Найти("ВыборМеткиКнопка");
	Если Элемент = Неопределено Тогда
		cbr_ЭлементыФормыСервер.СздКнопка(ЭтотОбъект, "ВыборМеткиКнопка", Элементы.Метки, "Метки", "МеткаЕще", 2);
	Иначе
...
Процедура ОбновитьЗаголовокПостановщика()
	Если ЗначениеЗаполнено(Объект.ВидЗадачи) Тогда
		...
		КонецЕсли;
		cbr_ЭлементыФормыСервер.УстановитьСвойстваЭлементу(Элементы.Постановщик, СтруктураСвойств);
	КонецЕсли;
КонецПроцедуры
#КонецОбласти
...
Процедура ОбработатьЗначенияПараметровФормы()
	...
	ПараметрВидЗадачи = Неопределено;
	Если Параметры.Свойство("ВидЗадачи", ПараметрВидЗадачи) Тогда
		Объект.ВидЗадачи = ПараметрВидЗадачи;
	КонецЕсли;
...
Процедура ОбработкаСменыМенеджера()
	...
	Если МенеджерВБазе <> МенеджерВОбъекте Тогда
		...
		Подписка = ПредопределенноеЗначение(
		"Перечисление.cbr_ПодпискаДляТриггера.СменаМенеджера");
		cbr_ОбработкаТриггеровВызовСервера.ВызовТриггера(Подписка, Объект.Ссылка);
	КонецЕсли;
КонецПроцедуры
