...
Процедура ВидимостьСвязанныхСРолями()
	...
	Запрос.Текст =
		...
		|		ИЛИ cbr_Задача.Автор = &ТекущийПользователь
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		...
		|		ПО вт_Исполнитель.Исполнитель = cbr_ГруппыДоступаКЗадачамПользователи.Пользователь
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.cbr_Роли.Пользователи КАК cbr_РолиПользователи
		|		ПО вт_Исполнитель.Исполнитель = cbr_РолиПользователи.Пользователь
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.cbr_ГруппыДоступаКЗадачам КАК cbr_ГруппыДоступаКЗадачам
		|		ПО вт_Исполнитель.Исполнитель = cbr_ГруппыДоступаКЗадачам.Руководитель
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ЕСТЬNULL(НЕ вт_ЗначенияСтатусов.Закрыта
		...
		|	ЕСТЬNULL(вт_ДанныеДокумента.ЭтоМенеджер, ЛОЖЬ) КАК ОтправитьНаИсполнение,
		...
		|	И (вт_Согласование.ЗаписьСогласование
		|	И вт_Согласование.ЭтоСогласующий
		|	И НЕ вт_Согласование.Согласовано
		|	И НЕ вт_Согласование.Отказ
		|	ИЛИ (вт_Согласование.ЭтоСогласовант
		|	И НЕ вт_Согласование.ЗаписьСогласование
		|	ИЛИ вт_Согласование.Отказ)), ЛОЖЬ) КАК СогласоватьОценку,
		|	ЕСТЬNULL(НЕ вт_ДанныеДокумента.Задача ЕСТЬ NULL
		|	И НЕ вт_ЗначенияСтатусов.Закрыта
		|	И (вт_Согласование.ЭтоСогласующий
		|	И вт_Дедлайн.ЗаписьДедлайн
		|	И НЕ вт_Дедлайн.ОтказДедлайн
		|	И НЕ вт_Дедлайн.СогласованоДедлайн
		|	ИЛИ вт_Согласование.ЭтоСогласовант
		|	И вт_ЗначенияИсполнителя.ЗадачаВзята
		|	И НЕ (вт_Дедлайн.ЗаписьДедлайн
		|	И НЕ вт_Дедлайн.ОтказДедлайн
		|	И НЕ вт_Дедлайн.СогласованоДедлайн)
		|	И вт_Согласование.Согласовано), ЛОЖЬ) КАК СогласоватьДедлайн,
		|	ЕСТЬNULL(вт_ЗначенияИсполнителя.ЭтоИсполнитель
		|	И НЕ вт_ЗначенияСтатусов.Закрыта, ЛОЖЬ) КАК ЗакрытьЗадачу,
