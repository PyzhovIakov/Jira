...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	ТекущийСтатус = cbr_ОбработчикиФормыЗадачиВызовСервера.СтатусВыбраннойЗадачи(Объект.Ссылка);
	...
	Если ТекущийСтатус <> Неопределено Тогда
		...
		Если ЭтоНовая И (ТекущийПользователь = Объект.Автор Или ТекущийПользователь = Объект.Постановщик) Тогда
...
Процедура ДекорацияЧекЛистНажатие(Элемент)
	ДекорацияЧекЛистНажатиеНаСервере();
КонецПроцедуры

&НаКлиенте
Процедура КонтактноеЛицоНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	СтруктураОтбора = Новый Структура("Владелец", Объект.Постановщик);
	ПараметрыФормы = Новый Структура("Отбор", СтруктураОтбора);
	Оповещение = Новый ОписаниеОповещения("ПослеВыбораКонтактногоЛица", ЭтотОбъект);
	ОткрытьФорму("Справочник.КонтактныеЛицаПартнеров.ФормаВыбора", ПараметрыФормы, , , , , Оповещение);
КонецПроцедуры
#КонецОбласти
...
Процедура ОткрытьВопросы(Команда)
	...
	ОткрытьФорму("ОбщаяФорма.cbr_ФормаВопросаПоЗадаче", ПараметрыФормы, , , , , Оповещение,
...
Процедура СоздатьПодчиненнуюЗадачу(Команда)
	...
	Иначе
		Если cbr_ОбработчикиФормыЗадачиВызовСервера.ЕстьПользовательскиеНастройки() Тогда
			...
			ПараметрыФормы.Вставить("УпрощеннаяФорма", Истина);	
...
Процедура ОткрытьИерархиюВНовомОкне(Команда)
	...
	ОткрытьФорму("ОбщаяФорма.cbr_ИерархияЗадачОбщая", ПараметрыФормы, , Новый УникальныйИдентификатор, , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
...
Процедура ОтправитьНаИсполнение(Команда)
	ОтправитьНаИсполнениеНаСервере();
КонецПроцедуры
...
Процедура ИнициализироватьМетки()
	...
	Если (Не cbr_ОбработчикиФормыЗадачиВызовСервера.ЕстьПользовательскиеНастройки() И Не ЗначениеЗаполнено(Объект.Ссылка))
...
Процедура ПостановщикПриИзмененииНаСервере()
	...
	cbr_ОбработчикиФормыЗадачиВызовСервера.ДобавитьНаблюдателяОтМенеджера(ЭтотОбъект);
КонецПроцедуры

&НаСервере
Процедура ОтправитьНаИсполнениеНаСервере()
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Подписка = ПредопределенноеЗначение("Перечисление.cbr_ПодпискаДляТриггера.КнопкаОтправитьНаИсполнение");
		cbr_ОбработкаТриггеровВызовСервера.ВызовТриггера(Подписка, Объект.Ссылка);
		ОбновитьВидимостьЭлементов();
	Иначе
		ОбщегоНазначения.СообщитьПользователю("Проведите документ.");
	КонецЕсли;
КонецПроцедуры
...
Процедура УстановитьОтображаемуюДату(НоваяОтображаемаяДата)
	ОтображаемаяДата =  ОбщегоНазначенияКлиент.ДатаСеанса();
...
Процедура ВидимостьСвязанныхСРолями()
	...
	Запрос.Текст =
		...
		|	ИЛИ вт_ЗначенияИсполнителя.ДоступПоРоли, ЛОЖЬ)
		|	И НЕ вт_ЗначенияСтатусов.Новая КАК НазначитьИсполнителя,
		|	ЕСТЬNULL(вт_ЗначенияИсполнителя.ДоступРуководителя, ЛОЖЬ)
		|	И НЕ вт_ЗначенияСтатусов.Новая КАК НазначитьИсполнителяИзГруппы,
		|	ЕСТЬNULL(вт_ЗначенияИсполнителя.ЭтоИсполнитель
		|	И НЕ вт_ЗначенияИсполнителя.ЗадачаВзята
		|	И НЕ вт_ДанныеДокумента.ЭтоМенеджер, ЛОЖЬ)
		|	И НЕ вт_ЗначенияСтатусов.Новая КАК ВзятьЗадачу,
		...
		|	ИЛИ вт_ЗначенияИсполнителя.ДоступРуководителя), ЛОЖЬ)
		|	И НЕ вт_ЗначенияСтатусов.Новая КАК ОтклонитьЗадачу,
		...
		|	И вт_Согласование.Согласовано), ЛОЖЬ)
		|	И НЕ вт_ЗначенияСтатусов.Новая КАК ВвестиТрудозатраты,
		...
		|	И вт_Согласование.Согласовано), ЛОЖЬ)
		|	И НЕ вт_ЗначенияСтатусов.Новая КАК СогласоватьОценку,
		|	ЕСТЬNULL(НЕ вт_ДанныеДокумента.Задача ЕСТЬ NULL
		|	И НЕ ЕСТЬNULL(вт_ЗначенияСтатусов.Закрыта, ЛОЖЬ)
		|	И ((вт_ДанныеДокумента.ЭтоМенеджер
		|	ИЛИ вт_Согласование.ЭтоСогласующий)
		...
		|	И НЕ ЕСТЬNULL(вт_Дедлайн.ОтказДедлайн, ЛОЖЬ)
		|	И НЕ ЕСТЬNULL(вт_Дедлайн.СогласованоДедлайн, ЛОЖЬ))
		|	И ЕСТЬNULL(вт_Согласование.Согласовано, ЛОЖЬ)), ЛОЖЬ)
		|	И НЕ вт_ЗначенияСтатусов.Новая КАК СогласоватьДедлайн,
		|	ЕСТЬNULL(вт_ЗначенияИсполнителя.ЭтоИсполнитель
		|	И НЕ ЕСТЬNULL(вт_ЗначенияСтатусов.Закрыта, ЛОЖЬ)
		|	И вт_ЗначенияИсполнителя.ЗадачаВзята, ЛОЖЬ)
		|	И НЕ вт_ЗначенияСтатусов.Новая КАК ЗакрытьЗадачу,
		|	ЕСТЬNULL(вт_ЗначенияСтатусовГлавной.ГлавнаяЗакрыта
		|	И вт_ЗначенияИсполнителя.ЭтоИсполнитель
		|	И вт_ЗначенияИсполнителя.ЗадачаВзята, ЛОЖЬ)
		|	И НЕ вт_ЗначенияСтатусов.Новая КАК ВернутьВышестоящуюВРаботу,
	...
	Если Выборка.Следующий() Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ВзятьЗадачу", "Видимость",
			Выборка.НазначитьИсполнителя ИЛИ Выборка.ВзятьЗадачу);
...
Процедура ВидимостьКнопкиВопроса()
    ...
    |	МАКСИМУМ(ЕСТЬNULL(cbr_СтатусыЗадач.Новый, ИСТИНА)) КАК ЭтоЧерновик
    |ИЗ
    ...
    Если Выборка.Следующий() Тогда
    	Если НЕ Выборка.ЭтоЧерновик И Выборка.КоличествоВопросов = 0 Тогда
...
Процедура ВидимостьКнопокСообщений()
    ...
    |ГДЕ
    |    cbr_ВопросыПоЗадачам.Задача = &Задача
    |    И НЕ ЕСТЬNULL(cbr_СтатусыЗадач.Новый, ИСТИНА)
    |СГРУППИРОВАТЬ ПО
    |    cbr_ВопросыПоЗадачам.Задача";
...
&НаСервере
Процедура ОбновитьЗаголовокПостановщика()
	Если ЗначениеЗаполнено(Объект.ВидЗадачи) Тогда
		ЗадачаПользовательская = НЕ ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ВидЗадачи, "Партнер");
		Если ЗадачаПользовательская Тогда
			СтруктураСвойств = Новый Структура("Заголовок", "Постановщик");
		Иначе
			СтруктураСвойств = Новый Структура("Заголовок", "Клиент");
		КонецЕсли;
		cbr_ЭлементыФормы.УстановитьСвойстваЭлементу(Элементы.Постановщик, СтруктураСвойств);
	КонецЕсли;
КонецПроцедуры
#КонецОбласти
...
Процедура ДобавитьНаблюдателяОтИсполнителя()
	cbr_ОбработчикиФормыЗадачиВызовСервера.ДобавитьНаблюдателяОтИсполнителя(ЭтотОбъект);
КонецПроцедуры
...
Процедура ОбработатьЗначенияПараметровФормы()
	...
	Если Параметры.Свойство("ИсточникПараметров", ПараметрИсточникПараметров) Тогда
		cbr_ОбработчикиФормыЗадачиВызовСервера.ЗаполнитьНаследуемыеПараметры(ЭтотОбъект, ПараметрИсточникПараметров);
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Если Не ЗначениеЗаполнено(ПараметрИсточникПараметров) Тогда
			cbr_ОбработчикиФормыЗадачиВызовСервера.ЗаполнитьПользовательскиеНастройки(ЭтотОбъект);
		КонецЕсли;
...
&НаСервере
Процедура ДекорацияЧекЛистНажатиеНаСервере()
	Элементы.ГруппаЧекЛист.Видимость = Не Элементы.ГруппаЧекЛист.Видимость;
КонецПроцедуры

&НаСервере
Процедура ОбработкаСменыМенеджера()
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Возврат;
	КонецЕсли;	
		
	Если ТипЗнч(Объект.Постановщик) = Тип("СправочникСсылка.Пользователи") Тогда
		МенеджерВБазе = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Ссылка, "Постановщик");
		МенеджерВОбъекте = Объект.Постановщик;
	Иначе
		МенеджерВБазе = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Ссылка, "ГлавныйМенеджер");
		МенеджерВОбъекте = Объект.ГлавныйМенеджер;
	КонецЕсли;
	
	Если МенеджерВБазе <> МенеджерВОбъекте Тогда
		РегистрыСведений.cbr_ОценкаЧасов.ОтменитьНесогласованные(Объект.Ссылка, МенеджерВБазе);
		РегистрыСведений.cbr_ДедлайнЗадач.ОтменитьНесогласованные(Объект.Ссылка, МенеджерВБазе);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ПослеВыбораКонтактногоЛица(Результат, ДополнительныеПараметры) Экспорт
	Если ЗначениеЗаполнено(Результат) Тогда
