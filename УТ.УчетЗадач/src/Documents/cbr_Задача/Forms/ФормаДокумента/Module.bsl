...
Процедура ВидимостьКнопкиТрудозатраты()
	...
	|	cbr_СтатусыЗадач.Закрыт как ЗадачаЗакрыта
	|ИЗ
	...
	|		ПО cbr_ОценкаЧасовСрезПоследних.Задача = cbr_СтатусыЗадачСрезПоследних.Задача";
	...
	Если Выборка.Следующий() Тогда
		Если Объект.КонтрольВремени Тогда
			Если Выборка.Согласовано  И Выборка.ЭтоИсполнитель  И Не Выборка.ЗадачаЗакрыта Тогда
				ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ВвестиТрудозатраты", "Видимость",
					Истина);
			КонецЕсли;
		Иначе
			Если Объект.УчетВремени  И Выборка.ЭтоИсполнитель  И Не Выборка.ЗадачаЗакрыта Тогда
				ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ВвестиТрудозатраты", "Видимость",
					Истина);
			КонецЕсли;
		КонецЕсли;
...
Процедура ВидимостьКнопокСогласования()
	...
	Если Объект.КонтрольВремени Тогда
		...
		|	ЕСТЬNULL(вт_Статус.Закрыт, ЛОЖЬ) как ЗадачаЗакрыта
		|ИЗ
		...
		|		ПО Вт_ДанныеПоЗадаче.Задача = вт_Статус.Задача";
		...
		Если Выборка.Следующий() Тогда
			Если Выборка.ЭтоСогласующий Тогда
				...
				УсловиеЕсли = ЗначениеЗаполнено(Выборка.ЗаписьОценка) И Не Выборка.Отказ И Не Выборка.Согласовано;
				Если УсловиеЕсли И Не Выборка.ЗадачаЗакрыта Тогда
			...
			Если Выборка.ЭтоСогласовант Тогда
				...
				УсловиеЕсли = ЗначениеЗаполнено(Выборка.ЗаписьДедлайн) И Не Выборка.ОтказДедлайн
					И Не Выборка.СогласованоДедлайн И Не Выборка.ЗадачаЗакрыта;
				Если Не УсловиеЕсли И Выборка.Согласовано Тогда
					ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СогласоватьДедлайн",
						"Видимость", Истина);
				КонецЕсли;
				УсловиеЕсли = Не ЗначениеЗаполнено(Выборка.ЗаписьОценка) Или Выборка.Отказ;
				Если УсловиеЕсли И Не Выборка.ЗадачаЗакрыта Тогда
					ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СогласоватьОценку",
						"Видимость", Истина);
				КонецЕсли;
...
Процедура ОбновитьВидимостьКнопокСвязанныхСИсполнителем()
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.cbr_СтатусыЗадач КАК cbr_СтатусыЗадач
	|			ПО cbr_СтатусыЗадачСрезПоследних.Статус = cbr_СтатусыЗадач.Ссылка
	|		ПО Центр_ИсполнителиЗадачСрезПоследних.Задача = cbr_СтатусыЗадачСрезПоследних.Задача
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	вт_Исполнитель.Закрыт КАК ЗакрытаЗадача
	|ИЗ
	...
	Если Выборка.Следующий() Тогда
		...
		КонецЕсли;
		Если Выборка.ЭтоГруппа И Выборка.ЭтоРуководитель  И Не Выборка.ЗакрытаЗадача Тогда
			...
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ОтклонитьЗадачу", "Видимость",
				Истина);
		КонецЕсли;
		Если Выборка.ЭтоГруппа И Выборка.ЭтоПользовательГруппы И Не Выборка.ЗакрытаЗадача Тогда
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "НазначитьИсполнителя", "Видимость",
				Истина);
		КонецЕсли;
		Если Выборка.ЭтоРоль И Выборка.ЭтоПользовательРоли И Не Выборка.ЗакрытаЗадача Тогда
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "НазначитьИсполнителя", "Видимость",
				Истина);
		КонецЕсли;
		...
		И Не ТекПользователь = Объект.ГлавныйМенеджер И Не Выборка.ЗакрытаЗадача Тогда
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ОтклонитьЗадачу", "Видимость",
				Истина);
