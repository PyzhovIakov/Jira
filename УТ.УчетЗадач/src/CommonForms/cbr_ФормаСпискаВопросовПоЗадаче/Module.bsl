...
Функция АдресатыВыбраннойЗадачи()
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	Центр_ЗадачаНаблюдатели.Пользователь КАК Пользователь
		|ПОМЕСТИТЬ вт_НесгруппированныеПользователи
		|ИЗ
		|	Документ.cbr_Задача.Наблюдатели КАК Центр_ЗадачаНаблюдатели
		|ГДЕ
		...
		|ИЗ
		|	РегистрСведений.cbr_ИсполнителиЗадач.СрезПоследних(, Задача = &Ссылка
		|	И Исполнитель ССЫЛКА Справочник.cbr_ГруппыДоступаКЗадачам) КАК Центр_ИсполнителиЗадачСрезПоследних
...
Процедура ОтправитьВопросНаСервере()
	...
	МенеджерЗаписи.ВидВопроса = Перечисления.cbr_ВидыВопросов.Вопрос;
