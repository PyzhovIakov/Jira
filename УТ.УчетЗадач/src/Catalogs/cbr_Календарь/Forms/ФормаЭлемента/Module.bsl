...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Запись_ЗаписьКалендаря" Тогда
		...
		Если (ТипЗнч(Параметр) = Тип("СправочникСсылка.cbr_Календарь")
...
Процедура ПовторятьПриИзменении(Элемент)
	...
	Повторять = (Объект.ТипЗаписиКалендаря =
		ПредопределенноеЗначение("Перечисление.cbr_ТипЗаписиКалендаря.ПовторяющеесяСобытие"));
...
Процедура ПланировщикПередНачаломРедактирования(Элемент, НовыйЭлемент, СтандартнаяОбработка)
	...
	cbr_КалендарьКлиент.ОбработкаПередНачаломРедактированиемЭлемента(Элемент,
		НовыйЭлемент, СтандартнаяОбработка, Объект.Сотрудник);
	...
	Если НастройкаРежимОтображенияМестногоВремени <> Перечисления.РежимыОтображенияМестногоВремени.ВремяСеанса Тогда
		...
		Если ЧасовойПоясПользователя <> ЧасовойПоясПоУмолчанию Тогда
			ПредставлениеМестногоЧасовогоПояса =
				cbr_КалендарьСервер.ПредставлениеЧасовогоПоясаДО(ЧасовойПоясПользователя);
		Иначе
			ПредставлениеМестногоЧасовогоПояса = "";
		КонецЕсли;
...
Процедура ИнициализироватьКалендарь()
	...
	ЧасовойПоясТекущегоПользователя =
		cbr_КалендарьСервер.ЧасовойПоясПользователя(Пользователи.ТекущийПользователь());
