...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УчетВремени = Ложь;
	Если Объект.ТипЗаписиКалендаря = ПредопределенноеЗначение("Перечисление.cbr_ТипЗаписиКалендаря.Задача")
		И ЗначениеЗаполнено(Объект.Предмет) Тогда
		УчетВремени = ОбщегоНазначенияУТВызовСервера.ЗначениеРеквизитаОбъекта(Объект.Предмет, "УчетВремени");
	КонецЕсли;	
		
	Если УчетВремени Тогда
		Элементы.СписаниеЧасов.Видимость = Истина;
		
		Если Объект.Состояние = ПредопределенноеЗначение("Перечисление.cbr_СостояниеЗаписиРабочегоКалендаря.Завершена") Тогда
			Элементы.СписаниеЧасов.ТолькоПросмотр = Истина;
		КонецЕсли;
	КонецЕсли;
...
Процедура ИнтервалВремениСтрокойОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ИнтервалВремениСтрокой <> ВыбранноеЗначение Тогда
...
Процедура СостояниеЗавершенно(Команда)  
	Если ПроверитьЗаполнение() Тогда
		...
		Если УчетВремени И Объект.СписаниеЧасов Тогда	
			ОповещениеОбОтмене = новый ОписаниеОповещения("ОбработкаРезультатаТрудозатрат",ЭтотОбъект);
			ПараметрыФормы = Новый Структура("Основание,Исполнитель,ИзКалендаря", Объект.Предмет, Объект.Автор, Истина);
			ОткрытьФорму("Документ.cbr_ТрудозатратыПоЗадаче.Форма.ФормаДокумента", ПараметрыФормы, , ,
				, , ОповещениеОбОтмене, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		Иначе
			ОповещениеОбОтмене = новый ОписаниеОповещения("ОбработкаВопросаРезультата",ЭтотОбъект, );
			ПоказатьВводСтроки(ОповещениеОбОтмене,"","Запишите результат...",,Истина);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры 
...
Процедура ОбработкаВопросаРезультата(Результат, ДополнительныеПараметры) экспорт        
    Если ЗначениеЗаполнено(Результат) Тогда
		...
		Элементы.СписаниеЧасов.ТолькоПросмотр = Истина;
    Иначе
    	ОбщегоНазначенияКлиент.СообщитьПользователю("Введите результат события");
    КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ОбработкаРезультатаТрудозатрат(Результат, ДополнительныеПараметры) экспорт        
    Если ЗначениеЗаполнено(Результат) Тогда
		Объект.Результат = Результат.Описание;	
		Объект.ДокументТрудозатрат = Результат.Документ;
		СостояниеЗавершенноСервер();
		Записать();
		Элементы.СписаниеЧасов.ТолькоПросмотр = Истина;
	КонецЕсли;
