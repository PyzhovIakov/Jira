...
Процедура ВыполнитьНастройку(Команда)
    ...
    ПоказатьВопрос(Оповещение, "Триггеры и этапы контроля будут сброшены. Продолжить?", РежимДиалогаВопрос.ДаНетОтмена,
		0, КодВозвратаДиалога.Нет);
КонецПроцедуры

&НаКлиенте
Процедура ВыполнитьНастройкуКанБан(Команда)
	Оповещение = Новый ОписаниеОповещения("ОбработатьОтветКанБан", ЭтотОбъект);	
 
    ПоказатьВопрос(Оповещение, "Элементы справочников ""Доски"" и ""Виды задач"" будут изменены. Продолжить?", РежимДиалогаВопрос.ДаНетОтмена,
		0, КодВозвратаДиалога.Нет);
КонецПроцедуры
#КонецОбласти
...
#Область КанБан
&НаКлиенте
Процедура ОбработатьОтветКанБан(Результат, Параметры) Экспорт
	Если Результат = КодВозвратаДиалога.Да Тогда
		Попытка
        	ВыполнитьНастройкуКанБанНаСервере();
...
&НаСервере
Процедура ВыполнитьНастройкуКанБанНаСервере()
	Виды = СформироватьВидыЗадач();
	НастроитьВиды(Виды);
	
	Доски = СформироватьДоски();
	НастроитьДоски(Доски);
КонецПроцедуры

&НаСервере
Функция СформироватьВидыЗадач()
	Виды = Новый ТаблицаЗначений;
	Виды.Колонки.Добавить("Имя", Новый ОписаниеТипов("Строка", Новый КвалификаторыСтроки(50)));
	Виды.Колонки.Добавить("Наименование", Новый ОписаниеТипов("Строка", Новый КвалификаторыСтроки(50)));
	Виды.Колонки.Добавить("КонтрольВремени", Новый ОписаниеТипов("Булево"));
	Виды.Колонки.Добавить("Партнер", Новый ОписаниеТипов("Булево"));
	
	СписокТипов = Новый Массив;
	СписокТипов.Добавить(Тип("СправочникСсылка.cbr_ГруппыДоступаКЗадачам"));
    СписокТипов.Добавить(Тип("СправочникСсылка.Пользователи"));
    СписокТипов.Добавить(Тип("СправочникСсылка.cbr_Роли"));
	
	ЛК_Вид = Виды.Добавить();
	ЛК_Вид.Имя = "ЛинияКонсультации";
	ЛК_Вид.КонтрольВремени = Истина;
	ЛК_Вид.Партнер = Истина;
	ЛК_Вид.Наименование = "Линия Консультации";
	
	ЛК_Вид = Виды.Добавить();
	ЛК_Вид.Имя = "IT";
	ЛК_Вид.КонтрольВремени = Истина;
	ЛК_Вид.Партнер = Истина;
	ЛК_Вид.Наименование = "IT";
	
	Возврат Виды;
КонецФункции

&НаСервере
Процедура НастроитьВиды(Виды)
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	Виды.Имя,
	|	Виды.Наименование,
	|	Виды.КонтрольВремени,
	|	Виды.Партнер
	...
	|	вт_ТаблицаВидов.Наименование, 
	|	вт_ТаблицаВидов.КонтрольВремени,
	|	вт_ТаблицаВидов.Партнер,
	|	ВЫБОР
	|		КОГДА вт_ТаблицаВидов.Наименование = cbr_ВидыЗадач.Наименование
	|		И вт_ТаблицаВидов.КонтрольВремени = cbr_ВидыЗадач.КонтрольВремени
	|		И вт_ТаблицаВидов.Партнер = cbr_ВидыЗадач.Партнер
	|			ТОГДА 1
	|		ИНАЧЕ ВЫБОР
	|			КОГДА вт_ТаблицаВидов.Наименование = cbr_ВидыЗадач.Наименование
	|				ТОГДА 2
	|			ИНАЧЕ   
	|				ВЫБОР
	|					КОГДА вт_ТаблицаВидов.Имя ЕСТЬ NULL
	|						ТОГДА 4
	|					ИНАЧЕ 3
	|				КОНЕЦ
	|		КОНЕЦ
	...
	|		ПОЛНОЕ СОЕДИНЕНИЕ вт_ТаблицаВидов КАК вт_ТаблицаВидов
	|		ПО вт_ТаблицаВидов.Наименование = cbr_ВидыЗадач.Наименование";
	
	Запрос.УстановитьПараметр("Виды", Виды);
	Выборка = Запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл
		Если Выборка.Индикатор = 2 Тогда
			Вид = Выборка.Ссылка.ПолучитьОбъект();
			Вид.КонтрольВремени = Выборка.КонтрольВремени;
			Вид.Партнер = Выборка.Партнер;
			Вид.Записать();
		ИначеЕсли Выборка.Индикатор = 3 Тогда
			Вид = Справочники.cbr_ВидыЗадач.СоздатьЭлемент();
...
&НаСервере
Функция СформироватьДоски()
	Доски = Новый ТаблицаЗначений;
	Доски.Колонки.Добавить("Имя", Новый ОписаниеТипов("Строка", Новый КвалификаторыСтроки(50)));
	Доски.Колонки.Добавить("Наименование", Новый ОписаниеТипов("Строка", Новый КвалификаторыСтроки(50)));
	Доски.Колонки.Добавить("ДоскаПоУмолчанию", Новый ОписаниеТипов("Булево"));
	
	ЛК_Доска = Доски.Добавить();
	ЛК_Доска.Имя = "ЛК";
	ЛК_Доска.Наименование = "ЛК";
	ЛК_Доска.ДоскаПоУмолчанию = Ложь;
	
	Возврат Доски;
КонецФункции

&НаСервере
Процедура НастроитьДоски(Доски)
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	Доски.Имя,
	|	Доски.Наименование,
	|	Доски.ДоскаПоУмолчанию
	|ПОМЕСТИТЬ вт_ТаблицаДосок
	|ИЗ
	|	&Доски КАК Доски
	|;
	|
	...
	|ВЫБРАТЬ
	|	cbr_Доски.Ссылка,
	|	вт_ТаблицаДосок.Наименование, 
	|	вт_ТаблицаДосок.ДоскаПоУмолчанию,
	|	ВЫБОР
	|		КОГДА вт_ТаблицаДосок.Наименование = cbr_Доски.Наименование
	|		И вт_ТаблицаДосок.ДоскаПоУмолчанию = cbr_Доски.ДоскаПоУмолчанию
	|			ТОГДА 1
	|		ИНАЧЕ ВЫБОР
	|			КОГДА вт_ТаблицаДосок.Наименование = cbr_Доски.Наименование
	|				ТОГДА 2
	|			ИНАЧЕ   
	|				ВЫБОР
	|					КОГДА вт_ТаблицаДосок.Имя ЕСТЬ NULL
	|						ТОГДА 4
	|					ИНАЧЕ 3
	|				КОНЕЦ
	|		КОНЕЦ
	...
	|		ПОЛНОЕ СОЕДИНЕНИЕ вт_ТаблицаДосок КАК вт_ТаблицаДосок
	|		ПО вт_ТаблицаДосок.Наименование = cbr_Доски.Наименование";
	
	Запрос.УстановитьПараметр("Доски", Доски);
	Выборка = Запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл
		Если Выборка.Индикатор = 2 Тогда
			Доска = Выборка.Ссылка.ПолучитьОбъект();
			Доска.ДоскаПоУмолчанию = Выборка.ДоскаПоУмолчанию;
			Доска.Записать();
		ИначеЕсли Выборка.Индикатор = 3 Тогда
			Доска = Справочники.cbr_Доски.СоздатьЭлемент();
			Доска.Наименование = Выборка.Наименование;
			Доска.ДоскаПоУмолчанию = Выборка.ДоскаПоУмолчанию;
			Доска.Записать();
		Иначе
			Продолжить;
...
Процедура ВыполнитьНастройкуНаСервере()
	...
	Статусы = СоздатьТаблицуСтатусов();
	Статусы = УстановитьЗначенияСтатусов(Статусы);
...
&НаСервере
Функция УстановитьЗначенияСтатусов(Статусы)
	СтруктураСтатусов = Новый Структура;
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	Статусы.Имя,
	|	Статусы.Наименование,
	|	Статусы.Закрыт,
	|	Статусы.Новый
	|ПОМЕСТИТЬ вт_ТаблицаСтатусов
	|ИЗ
	|	&Статусы КАК Статусы
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	cbr_СтатусыЗадач.Ссылка,
	|	вт_ТаблицаСтатусов.Наименование, 
	|	вт_ТаблицаСтатусов.Закрыт,
	|	вт_ТаблицаСтатусов.Новый,
	|	ВЫБОР
	|		КОГДА вт_ТаблицаСтатусов.Наименование = cbr_СтатусыЗадач.Наименование
	|		И вт_ТаблицаСтатусов.Закрыт = cbr_СтатусыЗадач.Закрыт
	|		И вт_ТаблицаСтатусов.Новый = cbr_СтатусыЗадач.Новый
	|			ТОГДА 1
	|		ИНАЧЕ ВЫБОР
	|			КОГДА вт_ТаблицаСтатусов.Наименование = cbr_СтатусыЗадач.Наименование
	|				ТОГДА 2
	|			ИНАЧЕ   
	|				ВЫБОР
	|					КОГДА вт_ТаблицаСтатусов.Имя ЕСТЬ NULL
	|						ТОГДА 4
	|					ИНАЧЕ 3
	|				КОНЕЦ
	|		КОНЕЦ
	|	КОНЕЦ КАК Индикатор,
	|	вт_ТаблицаСтатусов.Имя
	|ИЗ
	|	Справочник.cbr_СтатусыЗадач КАК cbr_СтатусыЗадач
	|		ПОЛНОЕ СОЕДИНЕНИЕ вт_ТаблицаСтатусов КАК вт_ТаблицаСтатусов
	|		ПО вт_ТаблицаСтатусов.Наименование = cbr_СтатусыЗадач.Наименование";
	
	Запрос.УстановитьПараметр("Статусы", Статусы);
	Выборка = Запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл
		Если Выборка.Индикатор = 1 Тогда
			СтруктураСтатусов.Вставить(Выборка.Имя, Выборка.Ссылка);
		ИначеЕсли Выборка.Индикатор = 2 Тогда
			Статус = Выборка.Ссылка.ПолучитьОбъект();
			Статус.Закрыт = Выборка.Закрыт;
			Статус.Новый = Выборка.Новый;
			Статус.Записать();
			СтруктураСтатусов.Вставить(Выборка.Имя, Статус.Ссылка);
		ИначеЕсли Выборка.Индикатор = 3 Тогда
			Статус = Справочники.cbr_СтатусыЗадач.СоздатьЭлемент();
			Статус.Наименование = Выборка.Наименование;
			Статус.Закрыт = Выборка.Закрыт;
			Статус.Новый = Выборка.Новый;
			Статус.Записать();
			СтруктураСтатусов.Вставить(Выборка.Имя, Статус.Ссылка);
	...
	Возврат СтруктураСтатусов;
КонецФункции 

&НаСервере
Функция СоздатьТаблицуСтатусов()
	Статусы = Новый ТаблицаЗначений;
	Статусы.Колонки.Добавить("Имя", Новый ОписаниеТипов("Строка", Новый КвалификаторыСтроки(100)));
	Статусы.Колонки.Добавить("Наименование", Новый ОписаниеТипов("Строка", Новый КвалификаторыСтроки(100)));
	Статусы.Колонки.Добавить("Новый", Новый ОписаниеТипов("Булево"));
	Статусы.Колонки.Добавить("Закрыт", Новый ОписаниеТипов("Булево"));
	
	НаИсполнителе = Статусы.Добавить();
	НаИсполнителе.Имя = "НаИсполнителе";
	НаИсполнителе.Новый = Ложь;
	НаИсполнителе.Закрыт = Ложь;
	НаИсполнителе.Наименование = "На исполнителе";
	
	ВРаботе = Статусы.Добавить();
	ВРаботе.Имя = "ВРаботе";
	ВРаботе.Новый = Ложь;
	ВРаботе.Закрыт = Ложь;
	ВРаботе.Наименование = "В работе";
	
	Закрыта = Статусы.Добавить();
	Закрыта.Имя = "Закрыта";
	Закрыта.Наименование = "Закрыта";
	Закрыта.Закрыт = Истина;
	Закрыта.Новый = Ложь;
	
	Запланирована = Статусы.Добавить();
	Запланирована.Имя = "Запланирована";
	Запланирована.Новый = Ложь;
	Запланирована.Закрыт = Ложь;
	Запланирована.Наименование = "Запланирована";
	
	ЗапросИнформации = Статусы.Добавить();
	ЗапросИнформации.Имя = "ЗапросИнформации";
	ЗапросИнформации.Новый = Ложь;
	ЗапросИнформации.Закрыт = Ложь;
	ЗапросИнформации.Наименование = "Запрос информации";
	
	ЗапросИнформацииЗакрытой = Статусы.Добавить();
	ЗапросИнформацииЗакрытой.Имя = "ЗапросИнформацииЗакрытой";
	ЗапросИнформацииЗакрытой.Наименование = "Запрос информации закрытой";
	ЗапросИнформацииЗакрытой.Закрыт = Истина;
	ЗапросИнформацииЗакрытой.Новый = Ложь;
	
	НаМенеджере = Статусы.Добавить();
	НаМенеджере.Имя = "НаМенеджере";
	НаМенеджере.Новый = Ложь;
	НаМенеджере.Закрыт = Ложь;
	НаМенеджере.Наименование = "На Менеджере";
	
	Новая = Статусы.Добавить();
	Новая.Имя = "Новая";
	Новая.Наименование = "Новая";
	Новая.Новый = Истина;
	Новая.Закрыт = Ложь;
	
	ОжиданиеГлавной = Статусы.Добавить();
	ОжиданиеГлавной.Имя = "ОжиданиеГлавной";
	ОжиданиеГлавной.Новый = Ложь;
	ОжиданиеГлавной.Закрыт = Ложь;
	ОжиданиеГлавной.Наименование = "Ожидание главной";
	
	ОзнакомленИсполнитель = Статусы.Добавить();
	ОзнакомленИсполнитель.Имя = "ОзнакомленИсполнитель";
	ОзнакомленИсполнитель.Новый = Ложь;
	ОзнакомленИсполнитель.Закрыт = Ложь;
...
Функция СформироватьСтруктурыТриггеров(Статусы)
	...
	Триггер.Вставить("Наименование", "Сменился Менеджер");
	Триггер.Вставить("Владелец", Статусы.НаИсполнителе);
	Триггер.Вставить("Уведомлять", Истина);
	Триггер.Вставить("Подписка", ПредопределенноеЗначение("Перечисление.cbr_ПодпискаДляТриггера.СменаМенеджера"));
	Триггер.Вставить("ФорматнаяСтрока", "У задачи сменился менеджер. &Пользователь проверте оценку и дедлайн задачи &Наименование");
	ПараметрыТриггера.Добавить(Новый Структура("Параметр,Значение", "Статус", Статусы.НаМенеджере));
	Триггер.Вставить("Параметры", ПараметрыТриггера);
	Триггер.Вставить("Запрос", "ВЫБРАТЬ
	|	cbr_Задача.Ссылка КАК Ссылка,
	|	cbr_Задача.Наименование КАК Наименование
	...
	|	вт_Задача.Наименование КАК Наименование,
	|	cbr_ИсполнителиЗадачСрезПоследних.Исполнитель КАК Пользователь
	|ИЗ
	...
	|			ТОГДА вт_ПроверкаНаПервыйСтатус.Задача
	|	КОНЕЦ КАК Ссылка
	|ИЗ
	...
	ПараметрыТриггера = Новый Массив;
	Триггер = Новый Структура;
	Триггер.Вставить("Наименование", "После Выполнения главной после ожидания");
	Триггер.Вставить("Владелец", Статусы.НаИсполнителе);
	Триггер.Вставить("Уведомлять", Истина);
	Триггер.Вставить("Подписка", ПредопределенноеЗначение("Перечисление.cbr_ПодпискаДляТриггера.КнопкаЗакрытияЗадачи"));
	Триггер.Вставить("ФорматнаяСтрока", "Задача "" &Наименование "" вернулась на &Пользователь после выполнения вышестоящей.");
	ПараметрыТриггера.Добавить(Новый Структура("Параметр,Значение", "Статус", Статусы.ОжиданиеГлавной));
	Триггер.Вставить("Параметры", ПараметрыТриггера);
	Триггер.Вставить("Запрос", "ВЫБРАТЬ
	|	cbr_Задача.Ссылка КАК Ссылка,
	...
	|		ПО вт_задачи.Ссылка = cbr_ИсполнителиЗадачСрезПоследних.Задача
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.cbr_СтатусыЗадач.СрезПоследних КАК cbr_СтатусыЗадачСрезПоследних
	|		ПО вт_задачи.Ссылка = cbr_СтатусыЗадачСрезПоследних.Задача
	|ГДЕ
	|	cbr_СтатусыЗадачСрезПоследних.Статус = &Статус");
	Триггеры.Добавить(Триггер);
