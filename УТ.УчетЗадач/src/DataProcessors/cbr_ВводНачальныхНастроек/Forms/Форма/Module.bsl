...
Функция СоздатьСтатусы()
	...
	Составляется = Справочники.cbr_СтатусыЗадач.СоздатьЭлемент();
	Составляется.Наименование = "Составляется";
	Составляется.Новый = Истина;
	Составляется.Записать();
	Статусы.Вставить("Составляется", Составляется.Ссылка);
...
Функция СформироватьСтруктурыТриггеров(Статусы)
	...
	Триггер.Вставить("Наименование", "Задача составляется");
	Триггер.Вставить("Владелец", Статусы.Составляется);
	...
	Триггер.Вставить("Параметры", ПараметрыТриггера);
	Триггер.Вставить("Запрос", "ВЫБРАТЬ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ cbr_СтатусыЗадачСрезПоследних.Статус) КАК Количество
	...
	Триггер.Вставить("ФорматнаяСтрока", "Запрос Информации по задаче "" &Наименование "".");
	Триггер.Вставить("Параметры", ПараметрыТриггера);
	Триггер.Вставить("Запрос", "ВЫБРАТЬ
	|	cbr_Задача.Ссылка КАК Ссылка,
	|	ВЫБОР
	...
	Триггер.Вставить("Уведомлять", Ложь);
	Триггер.Вставить("ФорматнаяСтрока", "");
	Триггер.Вставить("Подписка", ПредопределенноеЗначение("Перечисление.cbr_ПодпискаДляТриггера.КнопкаОтправитьНаИсполнение"));
	Триггер.Вставить("Параметры", ПараметрыТриггера);
	Триггер.Вставить("Запрос", "ВЫБРАТЬ
	|	cbr_Задача.Ссылка КАК Ссылка
	|ИЗ
	...
	Триггер.Вставить("Наименование", "Ознакомлен Исполнитель с задачей");
	...
	Триггер.Вставить("ФорматнаяСтрока", "Пришел ответ на вопрос по задаче "" &Наименование "".");
	Триггер.Вставить("Параметры", ПараметрыТриггера);
	Триггер.Вставить("Запрос", "ВЫБРАТЬ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	cbr_ИсполнителиЗадачСрезПервых.Задача КАК Ссылка,
	|	cbr_ИсполнителиЗадачСрезПервых.Исполнитель КАК Пользователь,
	|	cbr_Задача.Наименование КАК Наименование
	|ИЗ
	|	вт_ПоследнийСтатус КАК вт_ПоследнийСтатус
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.cbr_ИсполнителиЗадач.СрезПервых(, Задача = &Задача) КАК
	|			cbr_ИсполнителиЗадачСрезПервых
	|		ПО cbr_ИсполнителиЗадачСрезПервых.Задача = вт_ПоследнийСтатус.Задача
	...
	Триггер.Вставить("ФорматнаяСтрока", "На тебя переведена задача: &Ссылка");
	Триггер.Вставить("Параметры", ПараметрыТриггера);
	Триггер.Вставить("Запрос", "ВЫБРАТЬ
	|	ВЫБОР
	...
	Триггер.Вставить("Наименование", "Исполнителем назначен менеджер");
	Триггер.Вставить("Владелец", Статусы.НаИсполнителе);
	Триггер.Вставить("Уведомлять", Ложь);
	Триггер.Вставить("ФорматнаяСтрока", "");
	Триггер.Вставить("Подписка", ПредопределенноеЗначение("Перечисление.cbr_ПодпискаДляТриггера.ПослеЗаписиНаСервереФормаЗадачи"));
	Триггер.Вставить("Параметры", ПараметрыТриггера);
	Триггер.Вставить("Запрос", "ВЫБРАТЬ
	|	cbr_Задача.Ссылка КАК Ссылка
	|ИЗ
	|	РегистрСведений.cbr_ИсполнителиЗадач.СрезПоследних(, Задача = &Задача) КАК cbr_ИсполнителиЗадачСрезПоследних
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.cbr_Задача КАК cbr_Задача
	|		ПО cbr_ИсполнителиЗадачСрезПоследних.Задача = cbr_Задача.Ссылка
	|		И (cbr_Задача.ГлавныйМенеджер = cbr_ИсполнителиЗадачСрезПоследних.Исполнитель
	|		ИЛИ cbr_Задача.ГлавныйМенеджер = cbr_ИсполнителиЗадачСрезПоследних.Исполнитель)");
	Триггеры.Добавить(Триггер);
	ПараметрыТриггера = Новый Массив;
	Триггер = Новый Структура;
...
Функция СформироватьСтруктурыЭтапов(Статусы)
	...
	Этап.Вставить("Длительность", 3600);
	...
	Этап.Вставить("Длительность", 7200);
