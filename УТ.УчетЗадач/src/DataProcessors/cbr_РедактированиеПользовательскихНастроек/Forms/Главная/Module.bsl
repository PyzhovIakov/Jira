...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)       
	...
	Если Не Отказ Тогда                     
		...
		Запрос.Текст =
			...
			|ИЗ
			|	ПланВидовХарактеристик.cbr_ВидыПользовательскихНастроек КАК Центр_ВидыПользовательскихНастроек
			|ГДЕ
			...
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.cbr_ПользовательскиеНастройки КАК Центр_ПользовательскиеНастройки
...
Процедура ДеревоНастроекЗначениеПриИзменении(Элемент)
	...
	Если ТекущиеДанные.Настройка = ПредопределенноеЗначение("ПланВидовХарактеристик.cbr_ВидыПользовательскихНастроек.Постановщик") Тогда
		НастройкаВида = НастройкаДереваПоЗначению(ПредопределенноеЗначение("ПланВидовХарактеристик.cbr_ВидыПользовательскихНастроек.ВидЗадачи"), "Партнер");
		...
		Если НастройкаВида <> Неопределено И Не НастройкаВида И ЗначениеЗаполнено(ТекущиеДанные.Значение) И ТипЗнч(ТекущиеДанные.Значение) = Тип("СправочникСсылка.Партнеры") Тогда
			//ТекущиеДанные.Значение = Неопределено;
			СообщитьТекст("Постановщиком выбранного вида задачи не может быть партнер");
		КонецЕсли;	
	КонецЕсли;
	
	Если ТекущиеДанные.Настройка = ПредопределенноеЗначение("ПланВидовХарактеристик.cbr_ВидыПользовательскихНастроек.ВидЗадачи") Тогда
		НастройкаВида = НастройкаДереваПоЗначению(ПредопределенноеЗначение("ПланВидовХарактеристик.cbr_ВидыПользовательскихНастроек.Постановщик"), "Ссылка");
		...
		НастройкаКонтроля = НастройкаДереваПоЗначению(ПредопределенноеЗначение("ПланВидовХарактеристик.cbr_ВидыПользовательскихНастроек.КонтрольВремени"));
	...
	Если ТекущиеДанные.Настройка = ПредопределенноеЗначение("ПланВидовХарактеристик.cbr_ВидыПользовательскихНастроек.КонтрольВремени") Тогда
		НастройкаКонтроля = НастройкаДереваПоЗначению(ПредопределенноеЗначение("ПланВидовХарактеристик.cbr_ВидыПользовательскихНастроек.ВидЗадачи"), "КонтрольВремени");
		...
		НастройкаУчета = НастройкаДереваПоЗначению(ПредопределенноеЗначение("ПланВидовХарактеристик.cbr_ВидыПользовательскихНастроек.УчетВремени"));
	...
	Если ТекущиеДанные.Настройка = ПредопределенноеЗначение("ПланВидовХарактеристик.cbr_ВидыПользовательскихНастроек.УчетВремени") Тогда
		НастройкаКонтроля = НастройкаДереваПоЗначению(ПредопределенноеЗначение("ПланВидовХарактеристик.cbr_ВидыПользовательскихНастроек.КонтрольВремени"));
		Если НастройкаКонтроля <> Неопределено И НастройкаКонтроля И ЗначениеЗаполнено(ТекущиеДанные.Значение) И Не ТекущиеДанные.Значение Тогда
			//ТекущиеДанные.Значение = Неопределено;
			СообщитьТекст("Учет времени обязателен при включенном контроле");
