		...
		ИначеЕсли Событие = "СоздатьЗадачу" Тогда
			...
			Если cbr_ОбработчикиФормыЗадачиСервер.ЕстьПользовательскиеНастройки() Тогда
				...
				ОткрытьФорму("Документ.cbr_Задача.Форма.ФормаДокумента", ПараметрыФормы, , , , ,
					ОповещениеПослеСозданияЗадачи, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
			Иначе
...
Процедура ОтобразитьДоску()
	...
	Для Каждого Приоритет Из ТаблицаПриоритетов Цикл
		ТекстСтилей = ТекстСтилей + "#Приоритет" + Приоритет.Код + "{background: rgb(" + Приоритет.ЦветRGB
			+ "); color: rgb(" + Приоритет.ЦветТекстаRGB + ");}" + "#Приоритет" + Приоритет.Код
	...
	Пока ВыборкаДоски.Следующий() Цикл
		Если ВыборкаДоски.Ссылка = ВыбраннаяДоска Тогда
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого Задача Из ТаблицаЗадач Цикл
			...
			ИначеЕсли ТипЗнч(Задача.Исполнитель) = Тип("СправочникСсылка.cbr_Роли") Или ТипЗнч(Задача.Исполнитель)
				= Тип("СправочникСсылка.cbr_ГруппыДоступаКЗадачам") Тогда
...
Процедура СохранитьИзменениеВыбраннойДоски()
	Пользователь = Пользователи.АвторизованныйПользователь(); 
	РегистрыСведений.cbr_НастройкиПользователяКанБан.ИзменениеНастроек(Пользователь, ВыбраннаяДоска);
КонецПроцедуры
