...
Процедура ПолучитьИОбработатьДанные() Экспорт
	...
	Если ОкноДокумента.queueHead < ОкноДокумента.queueTail Тогда
		...
		ИначеЕсли Событие = "Удалить" Тогда
			ПараметрыВопроса = новый Структура("Номер", Номер);
			Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияВопроса", ЭтотОбъект, ПараметрыВопроса);

			ПоказатьВопрос(Оповещение, "Удалить колонку?", РежимДиалогаВопрос.ДаНетОтмена, 0, 
				КодВозвратаДиалога.Да, "Удаление");
		...
		ИначеЕсли Событие = "ДобавитьКолонку" Тогда
			...
			КонецЕсли;
			ДобавитьКолонку();
		ИначеЕсли Событие = "ДобавитьЗадачуЛК" Тогда
			ОткрытьФормуСозданияЗадачи(Истина);
		ИначеЕсли Событие = "ДобавитьЗадачуРазработка" Тогда
			ОткрытьФормуСозданияЗадачи(Ложь);
...
Процедура ПослеЗакрытияВопроса(Результат, ДополнительныеПараметры) Экспорт
	 Если Результат = КодВозвратаДиалога.Да Тогда
        ПроверитьКолонкиПользователя();
		УдалитьКолонкуНаСервере(Число(ДополнительныеПараметры.Номер));
    КонецЕсли;	
КонецПроцедуры
...
Процедура ОткрытьФормуНастройкиДоски()
	...
	ОткрытьФорму("Обработка.cbr_КанбанЛК.Форма.НастройкиКанБанДоски", , , , , , ОповещениеОЗакрытииФормы, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
...
Процедура ОткрытьНастройки(Номер)
	...
	ОткрытьФорму("Обработка.cbr_КанбанЛК.Форма.КонструкторКолонки", СтруктураФормы, , , , , ОповещениеОЗакрытииФормы);
КонецПроцедуры
...
Процедура ДобавитьКолонку()
	...
	ОткрытьФорму("Обработка.cbr_КанбанЛК.Форма.КонструкторКолонки", СтруктураФормы, , , , , ОповещениеОЗакрытии);
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьФормуСозданияЗадачи(ЗадачаЛК)
	ОповещениеПослеСозданияЗадачи = Новый ОписаниеОповещения("ПослеВызоваОбновитьКанБан", ЭтотОбъект);
	ПараметрыФормы =  ПараметрыЗадачи(ЗадачаЛК);
...
Функция ПараметрыЗадачи(ЗадачаЛК) 
	ИсполниетльЗадачи = Неопределено;
	ПараметрыФормы = Новый Структура;
	Если ЗадачаЛК Тогда
		ПараметрыФормы.Вставить("ВидЗадачи", Справочники.cbr_ВидыЗадач.НайтиПоНаименованию("Линия Консультации"));
		ИсполниетльЗадачи = Справочники.cbr_ГруппыДоступаКЗадачам.НайтиПоНаименованию("Линия Консультации");		
	Иначе
		ПараметрыФормы.Вставить("ВидЗадачи", Справочники.cbr_ВидыЗадач.НайтиПоНаименованию("IT"));
		ИсполниетльЗадачи = Справочники.cbr_ГруппыДоступаКЗадачам.НайтиПоНаименованию("Разработка");		
	Конецесли;	
	ПараметрыФормы.Вставить("Исполнитель", ИсполниетльЗадачи);
...
Процедура ПолучитьНастройкиКанБан()
	...
	ВыбраннаяДоска = Справочники.cbr_Доски.НайтиПоНаименованию("ЛК");
