...
Процедура ОтобразитьДоску()
	...
	ТекстЗапросаДС = "ВЫБРАТЬ
	                 ...
	                 |	СТРОКА(cbr_Задача.Номер) + "" "" + ЕСТЬNULL(cbr_СтатусыЗадачСрезПоследних.Статус.Наименование, """") КАК НомерСтатус,
	                 |	"""" КАК Пустая,
	...
	СтруктураКолонок.Вставить("НомерСтатус", "НомерСтатус");
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.ЦветRGB)  Тогда
			...
			НовыйЭлемент.ЦветФона = Новый Цвет(Фон[0], Фон[1], Фон[2]);
		КонецЕсли;
	
		// Таблица	
		cbr_ЭлементыФормы.СздДинамическийСписок(ЭтотОбъект, "Колонка" + Индекс, ГруппаКолонки,
			ТекстЗапросаДС, , "Документ.cbr_Задача", СтруктураКолонок, СтруктураСвойствДС, , , , Истина);
		...
		Пока ДанныеПриоритета.Следующий() Цикл
			...
			ЭлементОформления.Оформление.УстановитьЗначениеПараметра("ЦветФона",Новый Цвет(ЦветПриоритета[0],ЦветПриоритета[1],ЦветПриоритета[2]));  
