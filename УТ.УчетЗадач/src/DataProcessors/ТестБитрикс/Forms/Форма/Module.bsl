...
Процедура ПолучитьЗадачуНаСервере()
	...
	Результат = cbr_OPIBitrix24.УдалитьЭлементЧеклистаЗадачи(URL, ДанныеЗадачи["id"], АйДиЭлемента, Токен);
...
Процедура ДобавитьКаталогВХранилищеНаСервере()
	...
	URL = "https://c1c.bitrix24.ru/rest/3097/ztlcnlwwa060a57v/";
	Результат = cbr_OPIBitrix24.СоздатьКаталогВХранилище(URL, 3943, "TasksFiles");
...
URL = "https://c1c.bitrix24.ru/rest/3089/k6jllepat6a3ly3n/";
	Результат = cbr_OPIBitrix24.ПолучитьХранилище(URL, НомерХранилища);
...
//	Результат = cbr_OPIBitrix24.ПолучитьХранилище(URL, НомерХранилища);
//	Результат = cbr_OPIBitrix24.ПолучитьХранилищеПриложения(URL);
...
//	URL = "https://c1c.bitrix24.ru/rest/3089/hxjd2525sutx3574/";
//	ЗадачаНомер = 35027;
//	Ответ = cbr_OPIBitrix24.ПолучитьЗадачу(URL, ЗадачаНомер);
//	
//	Ответ2 = cbr_OPIBitrix24.ДобавитьЭлементЧеклистаЗадачи(URL, ЗадачаНомер, "Первое что написал");
//	Ответ3 = cbr_OPIBitrix24.ДобавитьЭлементЧеклистаЗадачи(URL, ЗадачаНомер, "Второе что написал");
//	
//КонецПроцедуры
...
Процедура ПоляЗадачиНаСервере()
	URL = "https://c1c.bitrix24.ru/rest/3089/hxjd2525sutx3574/";
	Ответ = cbr_OPIBitrix24.ПолучитьСписокПользовательскихПолейЗадачи(URL);
КонецПроцедуры
...
//	РезультатЗадачи = cbr_OPIBitrix24.СоздатьЗадачу(URL, Результат);
