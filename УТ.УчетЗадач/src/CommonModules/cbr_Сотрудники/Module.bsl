...
Процедура ПриДобавленииПараметровРаботыКлиента(Параметры) Экспорт
	...
	Параметры.Вставить(
		"ОсновнойСотрудник", ОсновнойСотрудник());
//	Параметры.Вставить(
...
Функция ЭтоСотрудникИлиПользовательТекущегоПользователя(ПользовательИлиСотрудник) Экспорт
	...
	ЭтоСотрудникИлиПользовательТекущегоПользователя = ТекущийПользовательИСотрудники.Найти(ПользовательИлиСотрудник)
		<> Неопределено;
...
Функция ЗаменитьПользователяСотрудником(ОбрабатываемаяСсылка, Действительный = Истина) Экспорт
	...
	Если ОбрабатываемаяСсылка = ПользователиСерверПовтИсп.СсылкаНеуказанногоПользователя() Или ОбрабатываемаяСсылка
...
Функция ЗаменитьПользователейСотрудниками(ОбрабатываемыйМассив, Действительные = Истина) Экспорт
	...
	СотрудникиПользователей = СотрудникиПользователей(
		МассивПользователей, Действительные);
...
//  СправочникСсылка.Сотрудники
//
Функция СсылкаНаСотрудникаПоПараметрам(ФизическоеЛицо, Подразделение, Должность, НачалоДействия, ОкончаниеДействия) Экспорт
...
Функция СотрудникиПользователей(МассивПользователей, Действительные = Истина) Экспорт
	...
	Если МассивПользователей.Количество() = 1 И МассивПользователей[0] = Пользователи.ТекущийПользователь() Тогда
	...
	Для Каждого КлючИЗначение Из СотрудникиПоПользователям Цикл
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
			СотрудникиПользователей, СотрудникиПользователя, Истина);
...
// 
// Возвращаемое значение:
//  Соответствие - Соответствие - Сотрудники по пользователям.
...
Функция ПользователиСотрудников(МассивСотрудников) Экспорт
	...
	Для Каждого КлючИЗначение Из ПользователиПоСотрудникам Цикл
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
			ПользователиСотрудников, СотрудникиПользователя, Истина);
...
Функция ЭтоСотрудникТекущегоПользователя(Сотрудник) Экспорт
	...
	ЭтоСотрудникТекущегоПользователя = ВсеСотрудникиТекущегоПользователя.Найти(Сотрудник) <> Неопределено;
...
Функция ОсновнойСотрудникПользователя(Пользователь = "ТекущийПользователь") Экспорт
	...
	Если Пользователь = "ТекущийПользователь" Тогда // Параметр сеанса ещё может быть не установлен.
		Пользователь = Пользователи.ТекущийПользователь();
	ИначеЕсли ТипЗнч(Пользователь) = Тип("СправочникСсылка.Пользователи") И Пользователь
		= Пользователи.ТекущийПользователь() Тогда // Если передается пользователь, то может быть он текущий.
...
// Возвращает сотрудников, относящихся к указанным подразделениям.
//
// Параметры:
//  Подразделения - Массив из СправочникСсылка.СтруктураПредприятия - список подразделений, для которых необходимо получить сотрудников.
//  ТолькоДействующие - Булево - флаг, определяющий, включать ли в результат только действующих сотрудников.
//
// Возвращаемое значение:
//  Соответствие - содержит сотрудников, сгруппированных по подразделениям.
//                 Ключ - СправочникСсылка.СтруктураПредприятия - подразделение.
//                 Значение - Массив из СправочникСсылка.Сотрудники - список сотрудников в подразделении.
//
Функция СотрудникиВПодразделениях(Подразделения, ТолькоДействующие = Истина) Экспорт
	...
	Если ТолькоДействующие Тогда
		Запрос.Текст = СтрЗаменить(ТекстЗапроса, "	И &ТолькоДействующие", "	И Сотрудники.ПометкаУдаления = ЛОЖЬ
																			 |	И Сотрудники.Действует = ИСТИНА");
	Иначе
	...
	Пока ЗначениеЗаполнено(ОчередноеПодразделение) Цикл
		...
		Если ОбработанныеПодразделения[ОчередноеПодразделение] = Истина Тогда
			ВызватьИсключение СтрШаблон(
				НСтр("ru = 'Обнаружено зацикливание подразделений для сотрудника %1 в подразделении %2.'"), Сотрудник,
				ОчередноеПодразделение);
		КонецЕсли;

		ОбработанныеПодразделения[ОчередноеПодразделение] = Истина;

		РуководительПодразделения = СтруктураПредприятия.РуководительПодразделения(ОчередноеПодразделение);
		...
		РеквизитыПользователя = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			ПользовательРуководителя, "Недействителен, ПометкаУдаления");
		ПользовательДействует = РеквизитыПользователя.Недействителен = Ложь И РеквизитыПользователя.ПометкаУдаления
			= Ложь;
...
Функция КонтейнерыВсехСотрудниковТекущегоПользователя() Экспорт
	...
	КонтейнерыВсехСотрудниковТекущегоПользователя = РегистрыСведений.СотрудникиВКонтейнерах.КонтейнерыСотрудников(
...
//
Функция СформироватьДанныеВыбора(Текст, ДополнениеТипа = Неопределено, ТипОбъекта = Неопределено,
	...
	// Роли.
	Если ДополнениеТипа.СодержитТип(Тип("СправочникСсылка.РолиИсполнителей")) Или ДополнениеТипа.СодержитТип(Тип(
		"СправочникСсылка.ПолныеРоли")) Тогда
	...
	// Подразделения.
	Если ДополнениеТипа.СодержитТип(Тип("СправочникСсылка.СтруктураПредприятия")) Или ДополнениеТипа.СодержитТип(Тип(
		"СправочникСсылка.ПодразделенияКонтейнеры")) Тогда
		Запрос.Текст = Запрос.Текст + "
									  |ОБЪЕДИНИТЬ ВСЕ
									  |";
		Запрос.Текст = Запрос.Текст + "
	...
	// Проекты.
	Если ДополнениеТипа.СодержитТип(Тип("СправочникСсылка.Проекты")) Или ДополнениеТипа.СодержитТип(Тип(
		"СправочникСсылка.ПроектыКонтейнеры")) Тогда
		Запрос.Текст = Запрос.Текст + "
									  |ОБЪЕДИНИТЬ ВСЕ
									  |";
		Запрос.Текст = Запрос.Текст + "
	...
	Если ДополнениеТипа.СодержитТип(Тип("СправочникСсылка.АвтоподстановкиДляОбъектов")) Тогда
		...
		Если Не ЗначениеЗаполнено(ТипОбъекта) Тогда
			ВызватьИсключение СтрШаблон(
				НСтр("ru = 'Не указан тип объекта %1.'"), ТипОбъекта);
		КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТипСсылки = Тип("СправочникСсылка.Сотрудники") И (ЗначениеЗаполнено(Выборка.Пояснение1)
			Или ЗначениеЗаполнено(Выборка.Пояснение2)) Тогда

			Если Не СтрНачинаетсяС(
				НРег(СтрШаблон("%1%2", Выборка.ПредставлениеСсылки, ТекстПояснения)), НРег_Текст) Тогда
		...
		ПредставлениеСсылки = СтрНайтиИВыделитьОформлением(
			Выборка.ПредставлениеСсылки, ТекстПоискаСотрудников);

		Если Не ЗначениеЗаполнено(ПредставлениеСсылки) Тогда
			ПредставлениеСсылки = Строка(Выборка.ПредставлениеСсылки);
		КонецЕсли;

		ПредставлениеФорматированнаяСтрока = Новый ФорматированнаяСтрока(ПредставлениеСсылки,
...
Функция ПросклонятьПредставлениеСотрудника(Сотрудник, Падеж) Экспорт
	...
	Возврат ПредставлениеСотрудника(
		СклоняемаяЧасть, РеквизитыСотрудника.ПредставлениеДолжностиПодразделения);
...
Функция ПредставленияВДокументахСотрудниковНаДату(Сотрудники, Дата) Экспорт
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "Сотрудники.Наименование", "Сотрудники.Наименование"
		+ СуффиксТекущегоЯзыка);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ФИОФизическихЛицСрезПоследних.ФИО", "ФИОФизическихЛицСрезПоследних.ФИО"
		+ СуффиксТекущегоЯзыка);
...
Процедура ЗаменитьОтветственныхВидовБизнесСобытийНаСотрудников(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ЗначениеЗаполнено(ВидБизнесСобытия.Ответственный) Тогда
				ВидБизнесСобытия.Ответственный = ЗаменитьПользователяСотрудником(ВидБизнесСобытия.Ответственный, Ложь);
			КонецЕсли;
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьОтветственныхВидовБизнесСобытийНаСотрудников не удалось обработать некоторые виды бизнес-событий (пропущены): %1'"),
			ПроблемныхОбъектов);
...
Процедура ЗаменитьОтветственныхЗаВопросыДеятельностиНаСотрудников(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ЗначениеЗаполнено(ВопросДеятельности.Ответственный) Тогда
				ВопросДеятельности.Ответственный = ЗаменитьПользователяСотрудником(ВопросДеятельности.Ответственный,
					Ложь);
			КонецЕсли;
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьОтветственныхЗаВопросыДеятельностиНаСотрудников не удалось обработать некоторые вопросы деятельности (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьОтветственныхЗаГрифыДоступаНаСотрудников(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ЗначениеЗаполнено(ГрифДоступа.Ответственный) Тогда
				ГрифДоступа.Ответственный = ЗаменитьПользователяСотрудником(ГрифДоступа.Ответственный, Ложь);
			КонецЕсли;
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьОтветственныхЗаГрифыДоступаНаСотрудников не удалось обработать некоторые грифы доступа (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьОтветственныхЗаГруппыДоступаКонтрагентовНаСотрудников(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ЗначениеЗаполнено(ГруппаДоступа.Ответственный) Тогда
				ГруппаДоступа.Ответственный = ЗаменитьПользователяСотрудником(ГруппаДоступа.Ответственный, Ложь);
			КонецЕсли;
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьОтветственныхЗаГруппыДоступаКонтрагентовНаСотрудников не удалось обработать некоторые группы доступа (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьОтветственныхЗаГруппыДоступаФизическихЛицНаСотрудников(Параметры) Экспорт
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьОтветственныхЗаГруппыДоступаФизическихЛицНаСотрудников не удалось обработать некоторые группы доступа (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьОтветственныхЗаДетекторыБизнесСобытийНаСотрудников(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ЗначениеЗаполнено(ДетекторСобытия.Ответственный) Тогда
				ДетекторСобытия.Ответственный = ЗаменитьПользователяСотрудником(ДетекторСобытия.Ответственный, Ложь);
			КонецЕсли;
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьОтветственныхЗаДетекторыБизнесСобытийНаСотрудников не удалось обработать некоторые группы доступа (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьОтветственныхЗаКонтрагентовНаСотрудников(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ЗначениеЗаполнено(Контрагент.Ответственный) Тогда
				Контрагент.Ответственный = ЗаменитьПользователяСотрудником(Контрагент.Ответственный, Ложь);
			КонецЕсли;
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьОтветственныхЗаКонтрагентовНаСотрудников не удалось обработать некоторых контрагентов (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьОтветственныхЗаМестаХраненияДелНаСотрудников(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ЗначениеЗаполнено(МестоХраненияДела.Ответственный) Тогда
				МестоХраненияДела.Ответственный = ЗаменитьПользователяСотрудником(МестоХраненияДела.Ответственный, Ложь);
			КонецЕсли;
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьОтветственныхЗаМестаХраненияДелНаСотрудников не удалось обработать некоторые места хранения (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьОтветственныхЗаПользовательскиеОбработчикиБизнесСобытийНаСотрудников(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ЗначениеЗаполнено(ОбработчикСобытия.Ответственный) Тогда
				ОбработчикСобытия.Ответственный = ЗаменитьПользователяСотрудником(ОбработчикСобытия.Ответственный, Ложь);
			КонецЕсли;
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьОтветственныхЗаПользовательскиеОбработчикиБизнесСобытийНаСотрудников не удалось обработать некоторые обработчики бизнес-событий (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьОтветственныхЗаПравилаАвтозаполненияФайловНаСотрудников(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ЗначениеЗаполнено(ПравилоЗаполнения.Ответственный) Тогда
				ПравилоЗаполнения.Ответственный = ЗаменитьПользователяСотрудником(ПравилоЗаполнения.Ответственный, Ложь);
			КонецЕсли;
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьОтветственныхЗаПравилаАвтозаполненияФайловНаСотрудников не удалось обработать некоторые правила автозаполнения файлов (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьОтветственныхЗаПравилаРазмещенияФайловВТомахНаСотрудников(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ЗначениеЗаполнено(ПравилоРазмещения.Ответственный) Тогда
				ПравилоРазмещения.Ответственный = ЗаменитьПользователяСотрудником(ПравилоРазмещения.Ответственный, Ложь);
			КонецЕсли;
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьОтветственныхЗаПравилаРазмещенияФайловВТомахНаСотрудников не удалось обработать некоторые правила размещения файлов (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьОтветственныхЗаПравилаЭскалацииЗадачНаСотрудников(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ЗначениеЗаполнено(ПравилоЭскалации.Ответственный) Тогда
				ПравилоЭскалации.Ответственный = ЗаменитьПользователяСотрудником(ПравилоЭскалации.Ответственный, Ложь);
			КонецЕсли;
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьОтветственныхЗаПравилаЭскалацииЗадачНаСотрудников не удалось обработать некоторые правила эскалации задач (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьОтветственныхЗаРабочиеГруппыНаСотрудников(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ЗначениеЗаполнено(РабочаяГруппа.Ответственный) Тогда
				РабочаяГруппа.Ответственный = ЗаменитьПользователяСотрудником(РабочаяГруппа.Ответственный, Ложь);
			КонецЕсли;
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьОтветственныхЗаРабочиеГруппыНаСотрудников не удалось обработать некоторые рабочие группы (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьОтветственныхЗаДокументыПередачиДелВАрхивНаСотрудников(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ЗначениеЗаполнено(ДокументПередачи.Ответственный) Тогда
				ДокументПередачи.Ответственный = ЗаменитьПользователяСотрудником(ДокументПередачи.Ответственный, Ложь);
			КонецЕсли;
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьОтветственныхЗаДокументыПередачиДелВАрхивНаСотрудников не удалось обработать некоторые документы передачи дел в арих (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьОтветственныхЗаУничтожениеДелНаСотрудников(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ЗначениеЗаполнено(ДокументУничтожение.Ответственный) Тогда
				ДокументУничтожение.Ответственный = ЗаменитьПользователяСотрудником(ДокументУничтожение.Ответственный,
					Ложь);
			КонецЕсли;
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьОтветственныхЗаУничтожениеДелНаСотрудников не удалось обработать некоторые документы уничтожения дел (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьОтветственныхЗаАлгоритмыПроверкиНаСотрудников(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ЗначениеЗаполнено(АлгоритмПроверки.Ответственный) Тогда
				АлгоритмПроверки.Ответственный = ЗаменитьПользователяСотрудником(АлгоритмПроверки.Ответственный, Ложь);
			КонецЕсли;
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьОтветственныхЗаАлгоритмыПроверкиНаСотрудников не удалось обработать некоторые алгоритмы проверок (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьСоздателейТематикДокументовНаСотрудников(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ЗначениеЗаполнено(ТематикаДокумента.Создал) Тогда
				ТематикаДокумента.Создал = ЗаменитьПользователяСотрудником(ТематикаДокумента.Создал, Ложь);
			КонецЕсли;
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьСоздателейТематикДокументовНаСотрудников не удалось обработать некоторые тематики документов (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьАвторовНаСотрудниковВИсторииСобытийЗадач(Параметры) Экспорт
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не удалось обработать запись истории события задачи %1 по причине:
					 |%2'"), ПолучитьНавигационнуюСсылку(Выборка.Задача), ПодробноеПредставлениеОшибки(
				ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(
				ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка, , ,
				ТекстСообщения);
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьАвторовНаСотрудниковВИсторииСобытийЗадач не удалось обработать записи истории событий задач (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьУстановившихНаСотрудниковВИсторииСостоянийДокументов(Параметры) Экспорт
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не удалось обработать запись истории состояния документа %1 по причине:
					 |%2'"), ПолучитьНавигационнуюСсылку(Выборка.Документ), ПодробноеПредставлениеОшибки(
				ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(
				ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка, , ,
				ТекстСообщения);
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьУстановившихНаСотрудниковВИсторииСостоянийДокументов не удалось обработать записи истории состояний документов (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьУстановившихНаСотрудниковВТекущихСостоянияхДокументов(Параметры) Экспорт
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьУстановившихНаСотрудниковВТекущихСостоянияхДокументов не удалось обработать записи текущих состояний документов (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьИзменившихНаСотрудниковВИсторииСроковДействийДокументов(Параметры) Экспорт
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьИзменившихНаСотрудниковВИсторииСроковДействийДокументов не удалось обработать некоторые записи истории сроков действий документов (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьАвторовНаСотрудниковВНастройкахПовторенияБизнесПроцессов(Параметры) Экспорт
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не удалось обработать настройку повторения процесса %1 по причине:
					 |%2'"), ПолучитьНавигационнуюСсылку(Выборка.БизнесПроцесс), ПодробноеПредставлениеОшибки(
				ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(
				ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка, , ,
				ТекстСообщения);
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьАвторовНаСотрудниковВНастройкахПовторенияБизнесПроцессов не удалось обработать некоторые настройки повторения процессов (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьНаСотрудниковАвторовПереносовСроковВыполненияПроцессовИЗадач(Параметры) Экспорт
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не удалось обработать запись о переносе срока выполнения процесса(задачи) %1 по причине:
					 |%2'"), ПолучитьНавигационнуюСсылку(Выборка.Предмет), ПодробноеПредставлениеОшибки(
				ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(
				ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка, , ,
				ТекстСообщения);
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьНаСотрудниковАвторовПереносовСроковВыполненияПроцессовИЗадач не удалось обработать некоторые записи о переносе сроков выполнения процессов и задач (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьНаСотрудниковАвторовСобытийВХодеОбработок(Параметры) Экспорт
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не удалось обработать запись хода обработки %1 по причине:
					 |%2'"), ПолучитьНавигационнуюСсылку(Выборка.Обработка), ПодробноеПредставлениеОшибки(
				ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(
				ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка, , ,
				ТекстСообщения);
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьНаСотрудниковАвторовСобытийВХодеОбработок не удалось обработать некоторые записи о ходе обработок (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьНаСотрудниковПолучившихОригиналВДанныхДокументов(Параметры) Экспорт
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьНаСотрудниковПолучившихОригиналВДанныхДокументов не удалось обработать некоторые данные документов (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьНаСотрудниковАвторовКаналовОбсуждений(Параметры) Экспорт
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не удалось обработать канал обсуждения %1 по причине:
					 |%2'"), ПолучитьНавигационнуюСсылку(Выборка.Ссылка), ПодробноеПредставлениеОшибки(
				ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(
				ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка, , ,
				ТекстСообщения);
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьНаСотрудниковАвторовКаналовОбсуждений не удалось обработать некоторые каналы обсуждения (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьАвторовЗаписейВПротоколеДоставкеПочты(Параметры) Экспорт
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не удалось обработать запись протокола доставки письма %1 по причине:
					 |%2'"), ПолучитьНавигационнуюСсылку(Выборка.Письмо), ПодробноеПредставлениеОшибки(
				ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(
				ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка, , ,
				ТекстСообщения);
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьАвторовЗаписейВПротоколеДоставкеПочты не удалось обработать некоторые записи протокола доставки писем (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьУстановившихНаСотрудниковВЗаписяхСвязейОбъектов(Параметры) Экспорт
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не удалось обработать запись связи с объектом %1 по причине:
					 |%2'"), ПолучитьНавигационнуюСсылку(Выборка.Объект), ПодробноеПредставлениеОшибки(
				ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(
				ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка, , ,
				ТекстСообщения);
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьУстановившихНаСотрудниковВЗаписяхСвязейОбъектов не удалось обработать некоторые записи связей объектов (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьОтветственныхЛицНаСотрудников(Параметры) Экспорт
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не удалось обработать запись ответственного лица организации %1 по причине:
					 |%2'"), ПолучитьНавигационнуюСсылку(Выборка.ОтветственноеЛицо), ПодробноеПредставлениеОшибки(
				ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(
				ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка, , ,
				ТекстСообщения);
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьОтветственныхЛицНаСотрудников не удалось обработать некоторые записи ответственных организации (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьНаСотрудниковПолучателейУведомленийОПроблемах(Параметры) Экспорт
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не удалось обработать запись получателя уведомлений о проблемах %1 по причине:
					 |%2'"), ПолучитьНавигационнуюСсылку(Выборка.Получатель), ПодробноеПредставлениеОшибки(
				ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(
				ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка, , ,
				ТекстСообщения);
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьНаСотрудниковПолучателейУведомленийОПроблемах не удалось обработать некоторые записи получателях уведомлений (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьПользователейНаСотрудниковВНастройкахДоступностиПоСостоянию(Параметры) Экспорт
		...
		Попытка
			...
			Для Каждого СтрокаТаблицы Из НастройкаДоступности.ИспользоватьДля Цикл
				Если ТипЗнч(СтрокаТаблицы.Участник) = Тип("СправочникСсылка.Пользователи") Тогда
					СтрокаТаблицы.Участник = ЗаменитьПользователяСотрудником(СтрокаТаблицы.Участник, Ложь);
				КонецЕсли;
			КонецЦикла;
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьПользователейНаСотрудниковВНастройкахДоступностиПоСостоянию не удалось обработать некоторые настройки доступности (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьПользователейНаСотрудниковВИндексахНумерации(Параметры) Экспорт
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьПользователейНаСотрудниковВИндексахНумерации не удалось обработать некоторые индексы нумерации (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьОтметившихИсполнениеВЗадачахНаСотрудников(Параметры) Экспорт
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьОтметившихИсполнениеВЗадачахНаСотрудников не удалось обработать некоторые действия задач (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьСоздавшихЗадачиПользователейНаСотрудников(Параметры) Экспорт
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьСоздавшихЗадачиПользователейНаСотрудников не удалось обработать некоторые задачи (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьАвторовНастроекОтложенногоСтартаНаСотрудников(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Запись.АвторДобавленияЗаписи = ЗаменитьПользователяСотрудником(Выборка.АвторДобавленияЗаписи, Ложь);
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьАвторовНастроекОтложенногоСтартаНаСотрудников не удалось обработать некоторые настройки отложенного страта процессов (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьПользователейНаСотрудниковВПравилахЭскалацииЗадач(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ТипЗнч(ПравилоОбъект.НаправлениеЭскалации) = Тип("СправочникСсылка.Пользователи") Тогда
				ПравилоОбъект.НаправлениеЭскалации = ЗаменитьПользователяСотрудником(
					ПравилоОбъект.НаправлениеЭскалации, Ложь);
			КонецЕсли;
			...
			Для Каждого СтрокаТаблицы Из ПравилоОбъект.ДополнительныеУсловия Цикл
				Если ТипЗнч(СтрокаТаблицы.ЗначениеУсловия) = Тип("СправочникСсылка.Пользователи") Тогда
					СтрокаТаблицы.ЗначениеУсловия = ЗаменитьПользователяСотрудником(СтрокаТаблицы.ЗначениеУсловия, Ложь);
				КонецЕсли;
			КонецЦикла;
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не удалось обработать правило эскалации задач %1 по причине:
					 |%2'"), ПолучитьНавигационнуюСсылку(Выборка.Правило), ПодробноеПредставлениеОшибки(
				ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(
				ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка, , ,
				ТекстСообщения);
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьПользователейНаСотрудниковВПравилахЭскалацииЗадач не удалось обработать некоторые правила эскалация задач (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьПользователейНаСотрудниковВИсторииЭскалацииЗадач(Параметры) Экспорт
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьПользователейНаСотрудниковВИсторииЭскалацииЗадач не удалось обработать некоторые записи истории эскалации задач (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьПользователейНаСотрудниковВМоихДокументах(Параметры) Экспорт
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не удалось обработать запись в моих документах для пользователя  %1 по причине:
					 |%2'"), ПолучитьНавигационнуюСсылку(Выборка.Сотрудник), ПодробноеПредставлениеОшибки(
				ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(
				ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка, , ,
				ТекстСообщения);
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьПользователейНаСотрудниковВМоихДокументах не удалось обработать некоторые записи (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьПользователейНаСотрудниковВДокументахРегистрацииСамочувствий(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ТипЗнч(СамочувствиеСотрудника.Создал) = Тип("СправочникСсылка.Пользователи") Тогда
				СамочувствиеСотрудника.Создал = ЗаменитьПользователяСотрудником(СамочувствиеСотрудника.Создал, Ложь);
			КонецЕсли;

			Если ТипЗнч(СамочувствиеСотрудника.Сотрудник) = Тип("СправочникСсылка.Пользователи") Тогда

				СамочувствиеСотрудника.Сотрудник = ЗаменитьПользователяСотрудником(СамочувствиеСотрудника.Сотрудник,
					Ложь);
			КонецЕсли;
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьПользователейНаСотрудниковВДокументахРегистрацииСамочувствий не удалось обработать некоторые документы регистрации самочувствия сотрудников (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьОтветственныхПользователейНаСотрудниковВИнструкциях(Параметры) Экспорт
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьОтветственныхПользователейНаСотрудниковВИнструкциях не удалось обработать некоторые инструкции (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьОтветственныхПользователейНаСотрудниковВДокументахВыгрузкаВССТУ(Параметры) Экспорт
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьОтветственныхПользователейНаСотрудниковВДокументахВыгрузкаВССТУ не удалось обработать некоторые документы (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьПользователейНаСотрудниковВСредиОбъектовДляАвтоподбораАдресатов(Параметры) Экспорт
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не удалось обработать запись настройки автоподбора для писем у пользователя %1 по причине:
					 |%2'"), ПолучитьНавигационнуюСсылку(Выборка.Владелец), ПодробноеПредставлениеОшибки(
				ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(
				ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка, , ,
				ТекстСообщения);
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьПользователейНаСотрудниковВСредиОбъектовДляАвтоподбораАдресатов не удалось обработать некоторые записи  (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьПользователейНаСотрудниковВАдреснойКниге(Параметры) Экспорт
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьПользователейНаСотрудниковВАдреснойКниге не удалось обработать некоторые записи адресной книги (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьПользователейНаСотрудниковВДанныхПоискаПоАдреснойКниге(Параметры) Экспорт
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не удалось обработать запись для поиска в адресной книге объекта %1 по причине:
					 |%2'"), ПолучитьНавигационнуюСсылку(Выборка.ОбъектПоиска), ПодробноеПредставлениеОшибки(
				ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(
				ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка, , ,
				ТекстСообщения);
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьПользователейНаСотрудниковВДанныхПоискаПоАдреснойКниге не удалось обработать некоторые записи поиска в адресной книге (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьПользователейНаСотрудниковВПротоколеРаботы(Параметры) Экспорт
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не удалось обработать запись протокола работы по объекту %1. Причина:
					 |%2'"), СсылкаНаОбъектДанных, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(
				ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка, , ,
				ТекстСообщения);
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьПользователейНаСотрудниковВПротоколеРаботы не удалось обработать некоторые записи протокола работы (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьПользователейНаСотрудниковВЗаписяхХронометража(Параметры) Экспорт
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьПользователейНаСотрудниковВЗаписяхХронометража не удалось обработать некоторые записи о хронометраже (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьПользователейНаСотрудниковВКатегорияхОбъектов(Параметры) Экспорт
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не удалось обработать запись о категории объекта %1. Причина:
					 |%2'"), ПолучитьНавигационнуюСсылку(Выборка.Объект), ОбработкаОшибок.ПодробноеПредставлениеОшибки(
				ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(
				ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка, , ,
				ТекстСообщения);
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьПользователейНаСотрудниковВКатегорияхОбъектов не удалось обработать некоторые записи (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьПользователейНаСотрудниковВПравилахКатегоризации(Параметры) Экспорт
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не удалось обработать запись правила категоризации %1 по причине:
					 |%2'"), ПолучитьНавигационнуюСсылку(Выборка.Ссылка), ОбработкаОшибок.ПодробноеПредставлениеОшибки(
				ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(
				ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка, , ,
				ТекстСообщения);
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьПользователейНаСотрудниковВПравилахКатегоризации не удалось обработать некоторые записи (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаменитьПользователейНаСотрудниковВКатегорияхДанных(Параметры) Экспорт
	...
	Если ОбработаноОбъектов = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр(
			"ru = 'Процедуре ЗаменитьПользователейНаСотрудниковВКатегорияхДанных не удалось обработать некоторые записи (пропущены): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура СоздатьСотрудникаДляАдминистратора() Экспорт
	...
	Если ОбщегоНазначенияДокументооборот.ЭтоДокументооборотХолдинга() И ПолучитьФункциональнуюОпцию("ИспользоватьКОД") Тогда
...
//  Строка
//
Функция ПредставлениеСотрудника(ФизЛицо, ПредставлениеДолжностиПодразделения, Должность = Неопределено,
	...
	Если Не ЗначениеЗаполнено(ПредставлениеДолжностиПодразделения) И ЗначениеЗаполнено(Должность) И ЗначениеЗаполнено(
		Подразделение) Тогда
	...
	Возврат СтрШаблон("%1%2", ФИО, ПредставлениеДолжностиПодразделения);
...
Процедура ПодобратьИУстановитьОсновногоСотрудника(ФизическоеЛицо) Экспорт
	...
	Если ЗначениеЗаполнено(ТекОсновнойСотрудник) И ТаблСотрудников.Найти(ТекОсновнойСотрудник, "Ссылка")
		<> Неопределено Тогда
...
Процедура ОбновлениеДействияСотрудников() Экспорт
	...
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(
		Метаданные.РегламентныеЗадания.ОбновлениеДействияСотрудников);
...
Функция ПроверитьСотрудниковНаДействительностьПодготовить(МассивСотрудников, ПредлагатьЗамену, ТолькоНедействительные)
	...
	РезультатПроверки.ЕстьНедействительныеСотрудники = СведенияТаблицей.Найти(Ложь, "Действует") <> Неопределено;
	РезультатПроверки.ЕстьНедействительныеПользователи = СведенияТаблицей.Найти(Ложь, "ПользовательДействует")
		<> Неопределено;
