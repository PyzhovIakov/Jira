...
Процедура ДобавитьНаблюдателяОтМенеджера(ЭтаФорма) Экспорт
	...
	Если ТипЗнч(ЭтаФорма.Постановщик) = Тип("СправочникСсылка.Партнеры") Тогда
		Подразделение = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЭтаФорма.Объект.ГлавныйМенеджер, "Подразделение");
		Если ЗначениеЗаполнено(Подразделение) Тогда
			Наблюдатель = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Подразделение, "ТекущийРуководитель");
		КонецЕсли;
	КонецЕсли;

	Если ТипЗнч(ЭтаФорма.Постановщик) = Тип("СправочникСсылка.Пользователи") Тогда
		Подразделение = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЭтаФорма.Постановщик, "Подразделение");
		Если ЗначениеЗаполнено(Подразделение) Тогда
			Наблюдатель = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Подразделение, "ТекущийРуководитель");
		КонецЕсли;
	КонецЕсли;
