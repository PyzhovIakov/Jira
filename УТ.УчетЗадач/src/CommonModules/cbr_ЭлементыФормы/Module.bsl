...
Функция СздДинамическийСписок(Форма,
    ...
    ТаблицаФормы = СздТаблицаФормы(Форма,ИмяСписка,РодительСписка,ИмяСписка,СтруктураКолонок,СтруктураСвойств,СтруктураСобытий,СвояКоманднаяПанель,ЭлементПеред,КанБанЭлемент);
...
Процедура СоздатьКолонкиДляТаблицыФормы(Форма,ТаблицаФормы,СтруктураКолонок,ЭтоДС = ЛОЖЬ,КанБанЭлемент)
	...
	Если КанБанЭлемент Тогда   
		...
		НоваяКолонкаТаблицы.Шрифт = Шрифт;
	  	...
	  	НоваяГруппаВложенных = Форма.Элементы.Добавить("ГруппаВложенныхКолонок"+ИмяСписка, Тип("ГруппаФормы"), НоваяГруппа);
    ...
    Для Каждого тКолонка Из СтруктураКолонок Цикл
		...
		Если ИмяКолонки = "ЦветПриоритета" Тогда  
			Продолжить;
		КонецЕсли;
        НоваяКолонкаТаблицы = СоздатьЭлементФормы(Форма,ИмяСписка + ИмяКолонки,Тип("ПолеФормы"),?(КанБанЭлемент , ?(тКолонка.Ключ <> "Пустая" ,НоваяГруппаВложенных,НоваяГруппаЭлемента),ТаблицаФормы  ));
        НоваяКолонкаТаблицы.Вид = ?(ЭтоДС,ВидПоляФормы.ПолеНадписи,ВидПоляФормы.ПолеВвода);  
		НоваяКолонкаТаблицы.АвтоМаксимальнаяВысота = Ложь;
		НоваяКолонкаТаблицы.АвтоМаксимальнаяШирина = Ложь;
		НоваяКолонкаТаблицы.ОтображатьВШапке = Ложь;  
