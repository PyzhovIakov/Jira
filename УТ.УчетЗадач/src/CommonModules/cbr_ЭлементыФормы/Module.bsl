...
Функция СздТаблицаФормы(Форма,
	...
	//ТаблицаФормы.ЦветРамки = ЦветаСтиля.ЦветСтрокиДоски;
...
Процедура СоздатьКолонкиДляТаблицыФормы(Форма,ТаблицаФормы,СтруктураКолонок,ЭтоДС = ЛОЖЬ)
	...
	НоваяКолонкаТаблицы = СоздатьЭлементФормы(Форма,ИмяСписка + "ЦветПриоритета",Тип("ПолеФормы"), ТаблицаФормы );
	...
	// Внешний вид 
	НоваяГруппа = Форма.Элементы.Добавить("ГруппаКолонок"+ИмяСписка, Тип("ГруппаФормы"), ТаблицаФормы);
    ...
    Для Каждого тКолонка Из СтруктураКолонок Цикл
		...
		Если ИмяКолонки = "ЦветПриоритета" Тогда  
			Продолжить;
		КонецЕсли;
        НоваяКолонкаТаблицы = СоздатьЭлементФормы(Форма,ИмяСписка + ИмяКолонки,Тип("ПолеФормы"),НоваяГруппа);
        НоваяКолонкаТаблицы.Вид = ?(ЭтоДС,ВидПоляФормы.ПолеНадписи,ВидПоляФормы.ПолеВвода);  
		НоваяКолонкаТаблицы.АвтоМаксимальнаяВысота = Ложь;
		НоваяКолонкаТаблицы.АвтоМаксимальнаяШирина = Ложь;
		НоваяКолонкаТаблицы.ОтображатьВШапке = Ложь;  
