...
Процедура СоздатьКомментарийКЗадачеИзВопроса(ЗадачаСсылка) Экспорт
	...
	Если Выборка.Следующий() И ЗначениеЗаполнено(Выборка.АйДиЗадачиБитрикс) Тогда
		...
		Ответ = OPI_Bitrix24.ДобавитьКомментарийЗадачи(ДанныеАвторизации.URL, Выборка.АйДиЗадачиБитрикс, 
...
Функция ПолучитьДанныеЗадачи(ЗадачаСсылка, ТипИнтеграции)
	...
	Запрос.Текст =
		...
		|	ЕСТЬNULL(cbr_Задача.Приоритет.ПриоритетВБитриксе, &ПриоритетВБитриксеСтандартный) КАК Приоритет,
...
Процедура ПереработатьДанныеСтруктуры(ДанныеЗадачи)
	...
	ПереработатьСтруктуруОписаниеЗадачи(ДанныеЗадачи);
...
Процедура ПереработатьСтруктуруОписаниеЗадачи(ДанныеЗадачи)
	...
	Если ДанныеЗадачи.Свойство("ОписаниеЗадачи") Тогда
		...
		Для Каждого ПараграфДокумента Из ФорматированныйДокумент.Элементы Цикл
			...
			ДобавитьПараграфКТексту(ОписаниеЗадачи, ПараграфДокумента, ИндексСписка);
...
Процедура ДобавитьПараграфКТексту(ИтоговыйТекст, Знач ПараграфДокумента, ИндексСписка)
	...
	Для Каждого ЭлементПараграфа Из ПараграфДокумента.Элементы Цикл
		...
		Если ТипЗнч(ЭлементПараграфа) = Тип("ТекстФорматированногоДокумента") Тогда
			ИтоговыйТекст = ИтоговыйТекст + ЭлементПараграфа.Текст + Символы.ПС;
				
		ИначеЕсли ТипЗнч(ЭлементПараграфа) = Тип("КартинкаФорматированногоДокумента") Тогда
			ИтоговыйТекст = ИтоговыйТекст + ЭлементПараграфа.Картинка + Символы.ПС;
