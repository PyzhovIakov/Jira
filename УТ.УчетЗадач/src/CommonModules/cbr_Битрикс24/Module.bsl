...
Процедура ОтправкаДанныхВБитрикс24() Экспорт
	// TODO: Титов - получение записей из регистра в нужном порядке
	// TODO: Титов - отправка с помощи ОбработатьДействиеДляБитрикса
	// TODO: Титов - обработка результата удалить или оставить запись
	// TODO: Титов - записать ошибку и дату попытки + счетчик попыток
КонецПроцедуры
...
Процедура ДобавитьДействиеВОчередь(Знач ЗадачаСсылка, Знач Триггер) Экспорт
	// TODO: Титов - обновить запись в регистре если она есть, если нет то создать
	// TODO: Титов - как то выставлять приоритет на отправку действия (создание задачи в первую очередь)
КонецПроцедуры

// Обработать входящий триггер. И отправить задачу в нужную процедуру
// 
// Параметры:
//  ЗадачаСсылка 	- ДокументСсылка.cbr_Задача 			- Задача ссылка
//  Триггер 		- ПеречислениеСсылка.cbr_ПодпискаДляТриггера
Процедура ОбработатьДействиеДляБитрикса(Знач ЗадачаСсылка, Знач Триггер) Экспорт
	...
	ДействиеИнтеграции = ДанныеНастройки.Значение;
	
	Если ДействиеИнтеграции = Перечисления.cbr_Б24_Действия.СменитьСтатусЗадачи Тогда
		СменитьСтатусЗадачи(ЗадачаСсылка, ДанныеАвторизации);
		
	ИначеЕсли ДействиеИнтеграции = Перечисления.cbr_Б24_Действия.ОтправитьЗадачу Тогда
		ОтправитьЗадачуВБитрикс(ЗадачаСсылка, ДанныеАвторизации);
		
	ИначеЕсли ДействиеИнтеграции = Перечисления.cbr_Б24_Действия.СменитьИсполнителяЗадачи Тогда
		СменитьИсполнителяЗадачи(ЗадачаСсылка, ДанныеАвторизации);
		
	ИначеЕсли ДействиеИнтеграции = Перечисления.cbr_Б24_Действия.СменитьДедлайнЗадачи Тогда
		СменитьДедлайнЗадачи(ЗадачаСсылка, ДанныеАвторизации);
		
	ИначеЕсли ДействиеИнтеграции = Перечисления.cbr_Б24_Действия.СменитьОценкуЗадачи Тогда
		СменитьОценкуЗадачи(ЗадачаСсылка, ДанныеАвторизации);
	
	ИначеЕсли ДействиеИнтеграции = Перечисления.cbr_Б24_Действия.ВернутьВышестоящуюЗадачуВРаботу Тогда
		ВернутьВышестоящуюЗадачуВРаботу(ЗадачаСсылка, ДанныеАвторизации);
	
	ИначеЕсли ДействиеИнтеграции = Перечисления.cbr_Б24_Действия.ОтправитьКомментарий Тогда
		ДобавитьКомментарийКЗадачеБитрикс(ЗадачаСсылка, ДанныеАвторизации);
		
	ИначеЕсли ДействиеИнтеграции = Перечисления.cbr_Б24_Действия.ОтправитьКомментарийИзВопроса Тогда
		СоздатьКомментарийКЗадачеИзВопроса(ЗадачаСсылка, ДанныеАвторизации);
		
	ИначеЕсли ДействиеИнтеграции = Перечисления.cbr_Б24_Действия.УдалитьЗадачу Тогда
		УдалитьЗадачуВБитриксе(ЗадачаСсылка, ДанныеАвторизации);
...
Процедура ОтправитьЗадачуВБитрикс(Знач ЗадачаСсылка, Знач ДанныеАвторизации) Экспорт
	...
	ОтправитьЧекЛистВБитрикс(СтруктураЗадачи, ДанныеЗадачи, ДанныеАвторизации);
...
Процедура УдалитьЗадачуВБитриксе(Знач ЗадачаСсылка, Знач ДанныеАвторизации) Экспорт
...
Процедура СменитьСтатусЗадачи(Знач ЗадачаСсылка, Знач ДанныеАвторизации) Экспорт
...
Процедура СменитьДедлайнЗадачи(Знач ЗадачаСсылка, Знач ДанныеАвторизации) Экспорт
...
Процедура СменитьОценкуЗадачи(Знач ЗадачаСсылка, Знач ДанныеАвторизации) Экспорт
...
Процедура ВернутьВышестоящуюЗадачуВРаботу(Знач ЗадачаСсылка, Знач ДанныеАвторизации) Экспорт

	ГлавнаяЗадача = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗадачаСсылка, "ГлавнаяЗадача");
	
	СменитьСтатусЗадачи(ЗадачаСсылка, ДанныеАвторизации);
	СменитьСтатусЗадачи(ГлавнаяЗадача, ДанныеАвторизации);
	
КонецПроцедуры

Процедура СменитьИсполнителяЗадачи(Знач ЗадачаСсылка, Знач ДанныеАвторизации) Экспорт
	...
	Запрос.Текст =
		...
		|	ЕСТЬNULL(cbr_Б24_Настройки.Значение, &СтатусВБитриксеСтандартный) КАК STATUS,
		|	cbr_Б24_СоответствиеЗадач.АйДиЗадачиБитрикс КАК АйДиЗадачи,
		|	cbr_СоответствиеПользователейДляИнтеграций.АйДиПользователя КАК RESPONSIBLE_ID
		|ИЗ
		|	РегистрСведений.cbr_СтатусыЗадач.СрезПоследних(, Задача = &ЗадачаСсылка) КАК cbr_СтатусыЗадачСрезПоследних
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.cbr_Б24_Настройки КАК cbr_Б24_Настройки
		|		ПО cbr_СтатусыЗадачСрезПоследних.Статус = cbr_Б24_Настройки.Настройка
		...
		|			ПО ВТ_ЗадачаИсполнитель.Исполнитель = cbr_СоответствиеПользователейДляИнтеграций.Пользователь
...
Процедура СоздатьКомментарийКЗадачеИзВопроса(Знач ЗадачаСсылка, Знач ДанныеАвторизации) Экспорт
...
Процедура ДобавитьКомментарийКЗадачеБитрикс(Знач ЗадачаСсылка, Знач ДанныеАвторизации) Экспорт
	...
	Если СтруктураКомментария.Свойство("КартинкиВОписании") Тогда
		...
		АйДиКаталога = ПолучитьАйДиПапкиПользователя(СтруктураКомментария.АйДиПользователя, ДанныеАвторизации, "Загруженные файлы");
...
Процедура ПереработатьДанныеСтруктуры(ДанныеЗадачи)
	...
	Для Каждого Элемент Из ДанныеЗадачи Цикл
		...
		Если Не ЗначениеЗаполнено(Элемент.Значение) И ТипЗнч(Элемент.Значение) <> Тип("Соответствие") И ТипЗнч(
			Элемент.Значение) <> Тип("Массив") Тогда
...
Функция ПолучитьАйДиХранилищаПользователя(Знач АйДиПользователя, Знач ДанныеАвторизации) Экспорт
...
Функция ПолучитьАйДиПапкиПользователя(Знач АйДиПользователя, Знач ДанныеАвторизации, Знач ИмяПапки = "") Экспорт
	
	Если ДанныеАвторизации.Свойство("АйДиХранилища") 
			И ЗначениеЗаполнено(ДанныеАвторизации.АйДиХранилища)
			И АйДиПользователя = ДанныеАвторизации.АйДиАдмин Тогда
				
		АйДиХранилища = ДанныеАвторизации.АйДиХранилища;
		
	Иначе
		АйДиХранилища = ПолучитьАйДиХранилищаПользователя(АйДиПользователя, ДанныеАвторизации);
	КонецЕсли;
...
Процедура ОбработатьОшибку(Знач Ответ)
