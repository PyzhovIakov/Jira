...
Процедура ОтправитьЗадачуВБитрикс(ЗадачаСсылка) Экспорт
	...
	Если ЗначениеЗаполнено(АйДиЗадачиБитрикс) Тогда
		...
		Ответ = cbr_OPIBitrix24.ИзменитьЗадачу(ДанныеАвторизации.URL, АйДиЗадачиБитрикс, СтруктураЗадачиДляБитрикса, ДанныеАвторизации.Токен);
		ЭтоНоваяЗадача = Ложь;
		
	Иначе
		
		Ответ = cbr_OPIBitrix24.СоздатьЗадачу(ДанныеАвторизации.URL, СтруктураЗадачиДляБитрикса, ДанныеАвторизации.Токен);
	...
	Если ЭтоНоваяЗадача Тогда
		...
		Ответ = cbr_OPIBitrix24.ДобавитьКомментарийЗадачи(ДанныеАвторизации.URL, АйДиЗадачиБитрикс, ТекстПервогоКомментария, ДанныеАвторизации.Токен);
	...
	ИначеЕсли СтруктураЗадачи.Свойство("ЧекЛист") Тогда
		...
		Пока Индекс <= СтруктураЗадачи.ЧекЛист.Количество() Цикл
			...
			Ответ = cbr_OPIBitrix24.ДобавитьЭлементЧеклистаЗадачи(ДанныеАвторизации.URL, АйДиЗадачиБитрикс, Таск["Текст"], СтатусТаска, ДанныеАвторизации.Токен);
			Индекс = Индекс + 1;
	...
	Если ЗначениеЗаполнено(СтруктураЗадачи.АйДиПапкиФайлов) Тогда
		...
		Ответ = cbr_OPIBitrix24.ПолучитьДочерниеЭлементыКаталога(ДанныеАвторизации.URL, СтруктураЗадачи.АйДиПапкиФайлов, , ДанныеАвторизации.Токен);
	...
	Если СтруктураЗадачи.Свойство("Файл") Тогда
		...
		Для Каждого Файл Из СтруктураЗадачи.Файл Цикл
			...
			Если Файл.Значение["АйДиФайлаБитрикс"] = "0" Тогда
				...
				Ответ = cbr_OPIBitrix24.ДобавитьФайлВЗадачу(ДанныеАвторизации.URL, АйДиЗадачиБитрикс, АйДиФайла, ДанныеАвторизации.Токен);
	...
	Если ЗначениеЗаполнено(ФайлыЗадачиБитрикс) Тогда
		...
		Для Каждого Элемент Из ФайлыЗадачиБитрикс Цикл
			Ответ = cbr_OPIBitrix24.ПоместитьФайлВКорзину(ДанныеАвторизации.URL, Элемент["ID"], ДанныеАвторизации.Токен);
...
Процедура УдалитьЗадачуВБитриксе(ЗадачаСсылка) Экспорт
	...
	Ответ = cbr_OPIBitrix24.УдалитьЗадачу(ДанныеАвторизации.URL, АйДиЗадачиБитрикс, ДанныеАвторизации.Токен);
...
Функция ПолучитьЗадачуИзБитрикса(ЗадачаСсылка) Экспорт
	...
	ДанныеАвторизации = ПолучитьДанныеАвторизации();
	Ответ = cbr_OPIBitrix24.ПолучитьЗадачу(ДанныеАвторизации.URL, Выборка.АйДиЗадачиБитрикс, ДанныеАвторизации.Токен);
...
Функция ПолучитьСписокХранилищ(Знач СтруктураОтбора = "") Экспорт
	...
	Пока НомерЗаписи < МаксимальноЗаписей Цикл
		...
		Ответ = cbr_OPIBitrix24.ПолучитьСписокХранилищ(ДанныеАвторизации.URL, ДанныеАвторизации.Токен, ДопПараметры);
...
Функция ПолучитьСписокПолейХранилища() Экспорт
	...
	ДанныеАвторизации = ПолучитьДанныеАвторизации();
	Ответ = cbr_OPIBitrix24.ПолучитьПоляХранилища(ДанныеАвторизации.URL, ДанныеАвторизации.Токен);
		...
		Для Каждого Элемент Из ЧекЛистЗадачиБитрикс Цикл
			...
			Если Индекс <= ЧекЛистЗадачи.Количество() Тогда
				...
				Если Элемент.Значение["title"] <> Таск["Текст"] Тогда
					Ответ = cbr_OPIBitrix24.ИзменитьЭлементЧеклистаЗадачи(ДанныеАвторизации.URL, АйДиЗадачиБитрикс, АйДиЭлемента, Таск["Текст"], Таск["Статус"], ДанныеАвторизации.Токен);	
				ИначеЕсли Элемент.Значение["isComplete"] <> Таск["Статус"] Тогда
					
					Если Таск["Статус"] = "Y" Тогда
						Ответ = cbr_OPIBitrix24.ВыполнитьЭлементЧеклистаЗадачи(ДанныеАвторизации.URL, АйДиЗадачиБитрикс, АйДиЭлемента, ДанныеАвторизации.Токен);
					Иначе
						Ответ = cbr_OPIBitrix24.ВозобновитьЭлементЧеклистаЗадачи(ДанныеАвторизации.URL, АйДиЗадачиБитрикс, АйДиЭлемента, ДанныеАвторизации.Токен);
					КонецЕсли;
					
				КонецЕсли;
				
			Иначе
				Ответ = cbr_OPIBitrix24.УдалитьЭлементЧеклистаЗадачи(ДанныеАвторизации.URL, АйДиЗадачиБитрикс, АйДиЭлемента, ДанныеАвторизации.Токен);
			КонецЕсли;
...
Функция ОчиститьЧекЛистЗадачи(АйДиЗадачиБитрикс)
	...
	ДанныеАвторизации = ПолучитьДанныеАвторизации();
	Ответ = cbr_OPIBitrix24.ПолучитьЧеклистЗадачи(ДанныеАвторизации.URL, АйДиЗадачиБитрикс, ДанныеАвторизации.Токен);
	...
	Для Каждого Элемент Из ЧекЛистЗадачи Цикл
		...
		Ответ = cbr_OPIBitrix24.УдалитьЭлементЧеклистаЗадачи(ДанныеАвторизации.URL, АйДиЗадачиБитрикс, Элемент["ID"], ДанныеАвторизации.Токен);
...
Функция ЗагрузитьФайлБитрикс(Знач ФайлСсылка, Знач ПапкаФайлыЗадачи)
	...
	Если ДанныеФайла <> Неопределено Тогда
		Ответ = cbr_OPIBitrix24.ЗагрузитьФайлВКаталог(ДанныеАвторизации.URL, ИмяФайла, ДанныеФайла, ПапкаФайлыЗадачи, MIMEТипФайла, ДанныеАвторизации.Токен);;
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Функция ПолучитьАйДиПапкиФайлов(Знач ЗадачаСсылка, Знач АйДиЗадачиБитрикс, Знач АйДиОсновнойПапки)
	...
	Ответ = cbr_OPIBitrix24.ПолучитьДочерниеЭлементыКаталога(ДанныеАвторизации.URL, АйДиОсновнойПапки, СтруктураОтбора, ДанныеАвторизации.Токен);
	...
	Иначе
		Ответ = cbr_OPIBitrix24.СоздатьПодкаталог(ДанныеАвторизации.URL, АйДиОсновнойПапки, АйДиЗадачиБитрикс, ДанныеАвторизации.Токен);	
...
//	СтруктураДляБитрикса = cbr_OPIBitrix24.ПолучитьСтруктуруЗадачи(URL);
//	
//	Текст = "";
//	
