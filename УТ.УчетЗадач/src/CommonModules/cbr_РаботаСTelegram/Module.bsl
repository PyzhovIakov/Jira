...
Процедура ОтправитьСообщениеПользователюПоСсылке(Пользователь, ТекстСообщения, ПредметУведомления = "") Экспорт
	Попытка
		...
		КонецЕсли;
		РезультатОтправки = cbr_OPITelegram.ОтправитьТекстовоеСообщение(Константы.cbr_Токен_БотТелеграм.Получить(),
...
Процедура АктивацияЗапросовНаПодключениеТелеграм() Экспорт
	РезультатGet = cbr_OPITelegram.ПолучитьОбновления(Константы.cbr_Токен_БотТелеграм.Получить(), ,
