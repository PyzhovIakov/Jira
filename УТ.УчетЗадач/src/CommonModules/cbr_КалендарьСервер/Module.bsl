...
Функция ФизЛицаСотрудников(МассивСотрудников) Экспорт
	...
	Возврат cbr_КалендарьСервер.ФизЛицаСотрудников(МассивСотрудников);
...
Функция ПолучитьСтруктурыЭлементовПланировщика(НастройкиОтображения, ЗаписиКалендаря = Неопределено) Экспорт
	...
	Для Каждого ВыделеннаяДата Из ВыделенныеДаты Цикл
		...
		СобытияНаДату = ПолучитьТаблицуСобытий(
			...
			НастройкиОтображения.ФизЛица, НастройкиОтображения.ОтображатьОтклоненные, ЗаписиКалендаряСНапоминаниями,
			 НастройкиОтображения.ОтображатьДействующиеЗадачи, НастройкиОтображения.ЗадачаФильтр, ЗаписиКалендаря);
...
//	
	...
	НастройкиОтображения.Вставить(
		"ОтображатьЗанятость", ПолучитьПерсональнуюНастройку("ОтображатьЗанятость"));
	...
	НастройкиОтображения.Вставить(
		"ОтображатьОтклоненные", ПолучитьПерсональнуюНастройку("ОтображатьОтклоненные"));
...
Процедура ОтобразитьПланДня(ДеревоПланДня, НастройкиОтображения) Экспорт
	...
	Для Каждого ВыделеннаяДата Из ВыделенныеДаты Цикл
		...
		СобытияНаДату = ПолучитьТаблицуСобытий(
			ОтображаемаяДатаНачала, ОтображаемаяДатаОкончания, НастройкиОтображения.ОтображатьПомеченныеНаУдаление,
			НастройкиОтображения.ФизЛица, НастройкиОтображения.ОтображатьОтклоненные, ЗаписиКалендаряСНапоминаниями,
			НастройкиОтображения.ОтображатьДействующиеЗадачи, НастройкиОтображения.ЗадачаФильтр);
		Для Каждого Событие Из СобытияНаДату Цикл
			НоваяСтрока = СобытияПоЧасам.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Событие);
		КонецЦикла;
	...
	Для Каждого ВыделеннаяДата Из ВыделенныеДаты Цикл
		...
		Пока НачалоТекущегоДняНедели < ОтображаемаяДатаОкончания Цикл
			...
			Для Каждого СтрокаСобытие Из СобытияДняПоЧасам Цикл
				...
				//		cbr_КаленарьКлиентСервер.ПолучитьИндексЦвета(СтрокаСобытие.Цвет);
				//КонецЕсли;
...
Функция ПолучитьПерсональнуюНастройку(Настройка) Экспорт
	...
	ИначеЕсли Настройка = "ОтображатьОтклоненные" Тогда
		...
		Значение = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
			"РабочийКалендарь", "ОтображатьОтклоненные", Ложь);

	ИначеЕсли Настройка = "ОтображатьНавигационныйКалендарь" Тогда

		Значение = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
			"РабочийКалендарь", "ОтображатьНавигационныйКалендарь", Ложь);

	ИначеЕсли Настройка = "СвернутьСобытияВесьДеньВКалендаре" Тогда

		Значение = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
			"РабочийКалендарь", "СвернутьСобытияВесьДеньВКалендаре", Ложь);

	ИначеЕсли Настройка = "ВремяНачалаОтображения" Тогда

		Значение = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
			"РабочийКалендарь", "ВремяНачалаОтображения", Дата("00010101080000"));

	ИначеЕсли Настройка = "ВремяОкончанияОтображения" Тогда

		Значение = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
			"РабочийКалендарь", "ВремяОкончанияОтображения", Дата("00010101190000"));

	ИначеЕсли Настройка = "МасштабСтандартный" Тогда

		Значение = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
			"РабочийКалендарь", "МасштабСтандартный", 100);

	ИначеЕсли Настройка = "МасштабСПанельюИнформации" Тогда

		Значение = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
			"РабочийКалендарь", "МасштабСПанельюИнформации", 70);
	...
	ИначеЕсли Настройка = "ОтображатьЗанятость" Тогда
		...
		Значение = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
			"РабочийКалендарь", "ОтображатьЗанятость", Истина);
	...
	ИначеЕсли Настройка = "СкрытьОтсутствияБудуРазбиратьЗадачи" Тогда
		...
		Значение = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
			"РабочийКалендарь", "СкрытьОтсутствияБудуРазбиратьЗадачи", Истина);
...
Процедура УстановитьПерсональнуюНастройку(Настройка, Значение) Экспорт
	...
	ИначеЕсли Настройка = "ОтображатьОтклоненные" Тогда
		...
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(
			"РабочийКалендарь", "ОтображатьОтклоненные", Значение);
	...
	ИначеЕсли Настройка = "ОтображатьНавигационныйКалендарь" Тогда
		...
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(
			"РабочийКалендарь", "ОтображатьНавигационныйКалендарь", Значение);
	...
	ИначеЕсли Настройка = "СвернутьСобытияВесьДеньВКалендаре" Тогда
		...
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(
			"РабочийКалендарь", "СвернутьСобытияВесьДеньВКалендаре", Значение);
	...
	ИначеЕсли Настройка = "ВремяНачалаОтображения" Тогда
		...
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(
			"РабочийКалендарь", "ВремяНачалаОтображения", Значение);
	...
	ИначеЕсли Настройка = "ВремяОкончанияОтображения" Тогда
		...
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(
			"РабочийКалендарь", "ВремяОкончанияОтображения", Значение);
	...
	ИначеЕсли Настройка = "МасштабСтандартный" Тогда
		...
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(
			"РабочийКалендарь", "МасштабСтандартный", Значение);
	...
	ИначеЕсли Настройка = "МасштабСПанельюИнформации" Тогда
		...
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(
			"РабочийКалендарь", "МасштабСПанельюИнформации", Значение);
	...
	ИначеЕсли Настройка = "ОтображатьЗанятость" Тогда
		...
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(
			"РабочийКалендарь", "ОтображатьЗанятость", Значение);
...
//			
//			Если ЗаписьКалендаря.ТипЗаписиКалендаря = Перечисления.cbr_ТипЗаписиКалендаря.ПовторяющеесяСобытие Тогда
//				
...
//		ЗаписьКалендаря,
//		Перечисления.cbr_НастройкиЗаписейРабочегоКалендаря.Отработана);
...
Функция ПолучитьТаблицуЗанятости(Знач ФизЛица, ДатаНачала, ДатаОкончания, ОтобразитьЗанятостьВДне = Истина,
	...
	//Если Не cbr_КаленарьКлиентСервер.ЯвляетсяКорректнымПериодом(ДатаНачала, ДатаОкончания) Тогда
	//	Возврат ТаблицаЗанятости;
	//КонецЕсли;
	//
...
Процедура СортироватьПоСостояниюЗанятости(ТаблицаЗанятости, ДополнительноеНаправлениеСортировки = "",
	ДополнительнаяСортировка = "") Экспорт
//	
...
Процедура ЗагрузитьДанныеПланировщика(Планировщик, НастройкиОтображения)
	...
	Если НастройкиОтображения.ОтображатьСобытия Тогда
		...
		Для Каждого ВыделеннаяДата Из ВыделенныеДаты Цикл
			...
			СобытияНаДату =  ПолучитьТаблицуСобытий(
				ОтображаемаяДатаНачала, ОтображаемаяДатаОкончания, НастройкиОтображения.ОтображатьПомеченныеНаУдаление,
				НастройкиОтображения.ФизЛица, НастройкиОтображения.ОтображатьОтклоненные,
				ЗаписиКалендаряСНапоминаниями, НастройкиОтображения.ОтображатьДействующиеЗадачи,
	...
	//			cbr_КаленарьКлиентСервер.УстановитьЗначениеИзмеренияИнтервалаФона(
	//				ИнтервалФона,
	//				"ФизическоеЛицо",
...
Функция ПолучитьТаблицуСобытий(ОтображаемаяДатаНачала, ОтображаемаяДатаОкончания, ОтображатьПомеченныеНаУдаление,
	ФизЛица, ОтображатьОтклоненные, ЗаписиКалендаряСНапоминаниями,
	ОтображатьДействующиеЗадачи, ЗадачаФильтр, ЗаписиКалендаря = Неопределено)
...
Процедура РассчитатьДоступностьПоПолномуАлгоритму(ТаблицаДоступности, ТаблицаЗанятости, ФизЛицо, ДатаНачала,
	ДатаОкончания)
	...
	Пока НачалоДняИтерации < КонецДняПоследнейИтерации Цикл
		...
		Для Каждого СтрокаДоступностьДняИтерации Из ДоступностьДняИтерации Цикл
			...
			ВнестиЗанятостьВТаблицуЗанятости(ТаблицаЗанятости, ФизЛицо, ДатаНачалаСтроки, ДатаОкончанияСтроки, Занят,
				ДатаНачала, ДатаОкончания);
...
Процедура РассчитатьДоступностьПоКраткомуАлгоритму(ТаблицаДоступности, ФизЛицо, ТаблицаЗанятости, ЗанятостьСотрудника,
	ДатаНачала, ДатаОкончания)
	...
	Пока НачалоДняИтерации < КонецДняПоследнейИтерации Цикл
		...
		Для Каждого СтрокаДоступностьДняИтерации Из ДоступностьДняИтерации Цикл
			...
			ВнестиЗанятостьВТаблицуЗанятости(ТаблицаЗанятостиВременная, ФизЛицо, ДатаНачалаСтроки, ДатаОкончанияСтроки,
				СтрокаДоступностьДняИтерации.Занят, ДатаНачалаТаблицыЗанятости, ДатаОкончанияТаблицыЗанятости);

		КонецЦикла;

		ВнестиЗанятостиВТаблицуЗанятости(ТаблицаЗанятостиВременная, ТаблицаЗанятости, ФизЛицо,
			ДатаНачалаТаблицыЗанятости, ДатаОкончанияТаблицыЗанятости, Перечисления.СостоянияЗанятости.Доступен);
...
Процедура ВнестиЗанятостиВТаблицуЗанятости(ТаблицаЗанятости, ТаблицаЗанятостиИсточник, ФизЛицо, ДатаНачала,
	ДатаОкончания, Занят = Неопределено)
	...
	Для Каждого СтрокаЗанятостиСотрудника Из МассивЗанятостиСотрудника Цикл
		...
		ВнестиЗанятостьВТаблицуЗанятости(ТаблицаЗанятости, ФизЛицо, СтрокаЗанятостиСотрудника.ДатаНачала,
			СтрокаЗанятостиСотрудника.ДатаОкончания, СтрокаЗанятостиСотрудника.Занят, ДатаНачала, ДатаОкончания);
...
Функция ПолучитьТаблицуОбщейЗанятостиПланировщика(ТаблицаЗанятости, НастройкиОтображения)
	...
	ДатаНачалаОбщая = cbr_КаленарьКлиентСервер.ПолучитьДатаНачалаОбщая(НастройкиОтображения);
	ДатаОкончанияОбщая = cbr_КаленарьКлиентСервер.ПолучитьДатаОкончанияОбщая(НастройкиОтображения);
