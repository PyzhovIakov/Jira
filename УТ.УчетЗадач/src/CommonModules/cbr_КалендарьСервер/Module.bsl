...
Функция ПолучитьСтруктурыЭлементовПланировщика(НастройкиОтображения, ЗаписиКалендаря = Неопределено) Экспорт
	...
	Для Каждого ВыделеннаяДата Из ВыделенныеДаты Цикл
		...
		ОтображаемаяДатаНачала =
			cbr_КаленарьКлиентСервер.ПолучитьДатуНачалаОтображаемогоПериода(
				ПериодОтображения, ВыделеннаяДата);
		ОтображаемаяДатаОкончания =
			cbr_КаленарьКлиентСервер.ПолучитьДатуОкончанияОтображаемогоПериода(
				ПериодОтображения, ВыделеннаяДата);
...
Функция СтруктурыЭлементовЗаписи(ЗаписьКалендаря,
	...
	ЭлементПланировщика.ЦветТекста = ЦветаСтиля.cbr_ЦветКалендаряПодпись;
...
//	
	...
	НастройкиОтображения.Вставить(
		...
		ЦветаСтиля.cbr_ЦветКалендаряПодпись);
	
	// Цвет календаря фон шапки
	НастройкиОтображения.Вставить(
		"ЦветКалендаряФонШапки",
		ЦветаСтиля.cbr_ЦветКалендаряФонШапки);
	...
	НастройкиОтображения.Вставить(
		"ЦветФонаТекущегоДня",
		ЦветаСтиля.cbr_ЦветКалендаряФонТекущегоДня);
	
	// Цвет пустого фона.
	НастройкиОтображения.Вставить(
		"ЦветКалендаряДоступноеВремя",
		ЦветаСтиля.cbr_ЦветКалендаряДоступноеВремя);
	
	// Цвет текущей записи.
	НастройкиОтображения.Вставить(
		"ЦветКалендаряТекущаяЗапись",
		ЦветаСтиля.cbr_ЦветКалендаряТекущаяЗапись);
	...
	НастройкиОтображения.Вставить(
		...
		ПолучитьЦветРамкиЭлементаПланировщика(ЦветаСтиля.cbr_ЦветКалендаряТекущаяЗапись));
	...
	НастройкиОтображения.Вставить(
		"ТекущаяЗаписьСсылка",
		Справочники.cbr_Календарь.ПустаяСсылка());
...
Процедура ОтобразитьПланДня(ДеревоПланДня, НастройкиОтображения) Экспорт
	...
	ПериодОтображения =
		cbr_КаленарьКлиентСервер.ПолучитьПериодОтображения(НастройкиОтображения);
	ВыделенныеДаты =
		cbr_КаленарьКлиентСервер.ПолучитьВыделенныеДаты(НастройкиОтображения);
		...
		Пока НачалоТекущегоДняНедели < ОтображаемаяДатаОкончания Цикл
			...
			Для Каждого СтрокаСобытие Из СобытияДняПоЧасам Цикл
				...
				ЭлементЗаписиКалендаря =
					cbr_КаленарьКлиентСервер.СформироватьЭлементЗаписиКалендаря(СтрокаСобытие);
...
Функция ПолучитьЦветТабличногоДокумента(ТипЗаписи) Экспорт  
	...
	Если ЗаписьСправочника = Справочники.cbr_КалендарьЦвета.ПустаяСсылка() Тогда
		Возврат ЦветаСтиля.cbr_ЦветКалендаряПоУмолчанию;
	Иначе 
...
Процедура УстановитьУсловноеОформлениеПланДня(УсловноеОформление) Экспорт
	...
	Элемент.Оформление.УстановитьЗначениеПараметра(
		"ЦветФона", ЦветаСтиля.cbr_ЦветКалендаряФонШапки);
	Элемент.Оформление.УстановитьЗначениеПараметра(
		"ЦветТекста", ЦветаСтиля.cbr_ЦветКалендаряПодпись);
	Элемент.Оформление.УстановитьЗначениеПараметра(
		"Шрифт", Новый Шрифт(ШрифтыСтиля.ОбычныйШрифтТекста, , , Истина));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра(
		"ЦветФона", ЦветаСтиля.cbr_ЦветКалендаряФонТекущегоДня);
	Элемент.Оформление.УстановитьЗначениеПараметра(
		"ЦветТекста", ЦветаСтиля.cbr_ЦветКалендаряПодписьТекущийДень);
...
Функция ПолучитьТаблицуСобытий(
	...
	Запрос.Текст = 
		...
		|	cbr_Задача.Номер + ""; "" + cbr_Задача.Номер КАК Текст,
		|	cbr_Задача.Наименование КАК Наименование2
		|ИЗ
...
Функция ПолучитьЦветЗанятости(Занятость)
	ЦветЗанятости = ЦветаСтиля.cbr_ЦветКалендаряДоступноеВремя;
...
Функция ПолучитьЦветРамкиЭлементаПланировщика(ЦветФона)
	...
	ЦветРамки = ЦветаСтиля.cbr_РамкаКалендаряПоУмолчанию;
	Возврат ЦветРамки;
