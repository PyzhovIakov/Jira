...
//
Процедура ОбработкаПередНачаломРедактированиемЭлемента(Планировщик, НовыйЭлемент, СтандартнаяОбработка,
	ФизЛицоКалендаря) Экспорт
...
Процедура ОбработкаПередСозданием(Планировщик, Начало, Конец, Значения, Текст, СтандартнаяОбработка,
	ИспользоватьБыстроеРедактирование, ФизЛицоКалендаря, НастройкиОтображения, УникальныйИдентификатор) Экспорт
...
Процедура УстановитьПометкиУдаления(Записи, ПометкаУдаления, ОбработанныеВопросы = Неопределено) Экспорт
	...
	УстановленаПометкаУдаления = cbr_КалендарьСервер.УстановитьПометкиУдаления(
...
//			cbr_КаленарьКлиентСервер.СформироватьЭлементЗаписиКалендаря(ДанныеСтроки);
...
//					
//					ЭлементЗаписиКалендаря =
//						cbr_КаленарьКлиентСервер.СформироватьЭлементЗаписиКалендаря(ЭлементВДне);
//					ВыделенныеЭлементы.Добавить(ЭлементЗаписиКалендаря);
//					
//				КонецЦикла;
//				
...
//
Процедура ПеренестиЭлементыПланировщикаНаДату(Планировщик, ДанныеФормыПланировщик, НастройкиОтображения, Элементы,
	ДатаПереноса, УникальныйИдентификатор) Экспорт
	...
	Для Каждого Элемент Из Элементы Цикл
		...
		Если ИзмененнаяЗаписьКалендаря.ТипЗаписиКалендаря = ПредопределенноеЗначение(
			"Перечисление.cbr_ТипЗаписиКалендаря.ПовторяющеесяСобытие") Тогда
	...
	Результат = cbr_КалендарьСервер.ПеренестиЭлементыПланировщикаНаДату(ИзмененныеЗаписиКалендаря, ДатаПереноса,
		НастройкиОтображения);

	ОбработатьРезультатИзмененияЭлементовПланировщика(Результат, ИсключенияПовторения, ДанныеФормыПланировщик,
...
//	
//	Результат = cbr_КалендарьСервер.ПеренестиЭлементыПланировщикаНаДату(ЭлементыЗаписейКалендаря, ДатаПереноса);
//	
...
Процедура СоздатьЗаписьКалендаряПланировщик(НастройкиОтображения, ФизЛицо) Экспорт
	...
	СотрудникФизЛица = cbr_КалендарьКлиент.ОсновнойСотрудникФизЛица(ФизЛицо);
...
//		Если ДанныеСтроки.ТипЗаписиКалендаря = ПредопределенноеЗначение("Перечисление.cbr_ТипЗаписиКалендаря.ПовторяющеесяСобытие") Тогда
...
//  ФизЛицо	 - СправочникСсылка.ФизическиеЛица - физ. лицо, в календаре которого будет создана запись.
//
Процедура ОбработкаПередНачаломДобавленияПланДня(ЭлементПланДня, Отказ, Копирование, Родитель, Группа,
	НастройкиОтображения, ФизЛицо = Неопределено) Экспорт
//	
//	Отказ = Истина;
//	
//	Если НастройкиОтображения.ЗапретСоздания Тогда
...
//  НастройкиОтображения	- Структура					 - Настройки отображения.
//
Процедура ОбработкаНачалаПеретаскиванияПланДня(ЭлементПланДня, ПараметрыПеретаскивания, Выполнение,
	НастройкиОтображения) Экспорт
//	
//	Если НастройкиОтображения.ЗапретИзменения Тогда
//		Выполнение = Ложь;
//		Возврат;
...
//  НастройкиОтображения - Структура					 - Настройки отображения.
//
Процедура ОбработкаПроверкиПеретаскиванияПланДня(ЭлементПланДня, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка,
	Поле, НастройкиОтображения) Экспорт
...
//		Или Не cbr_КаленарьКлиентСервер.ЭтоЭлементЗаписиКалендаря(
...
//  НастройкиОтображения 	- Структура					 - Настройки отображения.
//
Процедура ОбработкаПеретаскиванияПланДня(ЭлементПланДня, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле,
	НастройкиОтображения) Экспорт
...
//	
//	Результат =
//		cbr_КалендарьСервер.ОтработатьВыделенныеЗаписиКалендаря(ЗаписиКалендаря);
//	
...
Процедура ОбработкаОкончанияРедактирования(Планировщик, ДанныеФормыПланировщик, НастройкиОтображения,
	...
	Если ИзмененныеЗаписиКалендаря.Количество() > 0 Тогда
		//Результат = cbr_КалендарьСервер.СохранитьЗаписиКалендаря(
...
Процедура ОбработкаСозданиеЭлемента(Планировщик, ДанныеФормыПланировщик, НастройкиОтображения, ФизЛицоКалендаря)
	...
	СозданнаяЗаписьКалендаря.Вставить("Сотрудник", cbr_КалендарьКлиент.ОсновнойСотрудникФизЛица(ДляФизЛица));

	ЗначениеКопирования = Неопределено;
	Если ВыделенныйЭлемент.Значение <> Неопределено Тогда
		Если ТипЗнч(ВыделенныйЭлемент.Значение.Ссылка) = Тип("СправочникСсылка.cbr_Календарь") Тогда
			ЗначениеКопирования = ВыделенныйЭлемент.Значение.Ссылка;
		КонецЕсли;
	КонецЕсли;

	Результат = cbr_КалендарьСервер.СоздатьЗаписьКалендаря(
	...
	Если Результат.Напоминание <> Неопределено Тогда
		...
		Оповестить("Запись_НапоминанияПользователя_Документооборот", Результат.Напоминание,
			Результат.Напоминание.Источник);
	КонецЕсли;
	...
	Если НастройкиОтображения.ОтображатьЗанятость И НастройкиОтображения.ОтображатьСобытия Тогда
		...
		Для Каждого СтруктураИнтервалаФона Из ДанныеПланировщика.ИнтервалыФона Цикл
			ДобавитьИнтервалФонаВПланировщик(
				Планировщик, СтруктураИнтервалаФона);
		КонецЦикла;
	КонецЕсли;
...
//
Процедура ОбработатьРезультатИзмененияЭлементовПланировщика(Результат, ИсключенияПовторения, ДанныеФормыПланировщик,
	НастройкиОтображения, УникальныйИдентификатор)
//	
...
//					ПредопределенноеЗначение("Перечисление.cbr_ТипЗаписиКалендаря.ЭлементПовторяющегосяСобытия");
//				Прервать;
//				
//			КонецЕсли;
//			
...
//
Процедура ВыбратьСотрудникаИзАдреснойКниги(ЭлементФормы, ВыбранноеЗначение, ЗаголовокФормы = Неопределено,
	Знач ДополнениеТипа = Неопределено, ТипОбъекта = Неопределено) Экспорт
	...
	Если ДополнениеТипа.СодержитТип(Тип("СправочникСсылка.РабочиеГруппы")) Или ДополнениеТипа.СодержитТип(Тип(
		"СправочникСсылка.СтруктураПредприятия")) Или ДополнениеТипа.СодержитТип(Тип(
	...
	Если ДополнениеТипа.СодержитТип(Тип("СправочникСсылка.АвтоподстановкиДляОбъектов")) Тогда
		...
		Иначе
			ВызватьИсключение СтрШаблон(
				НСтр("ru = 'Неизвестный тип объекта %1.'"), ТипОбъекта);
		КонецЕсли;
	КонецЕсли;
