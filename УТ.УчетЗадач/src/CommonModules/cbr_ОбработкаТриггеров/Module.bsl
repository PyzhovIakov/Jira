...
Процедура ВызовТриггера(Подписка, Задача = Неопределено) Экспорт
	...
	Запрос.Текст =
		...
		|ИЗ
		|	Справочник.cbr_Триггеры КАК Центр_Триггеры
		|ГДЕ
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.cbr_Триггеры.Параметры КАК Центр_ТриггерыПараметры
			...
			Если ЗначениеЗаполнено(Выборка.Ссылка) Тогда
				...
				Если СправочникКонтроля.Уведомлять И Выборка.Владелец().Колонки.Найти("Пользователь") <> Неопределено Тогда
					Если ЗначениеЗаполнено(Выборка.Пользователь) Тогда
						...
						cbr_УведомленияПользователей.ОтправитьУведомлениеПользователю(Выборка.Пользователь,
