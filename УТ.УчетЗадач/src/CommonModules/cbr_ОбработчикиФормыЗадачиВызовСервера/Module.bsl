...
//		Ссылка - ДокументСсылка.cbr_Задача
//	Возвращаемое значение:
// 		СправочникСсылка.cbr_СтатусыЗадач - Статус задачи.
Функция СтатусВыбраннойЗадачи(Ссылка) Экспорт
	ПараметрыРегистра = Новый Структура("Задача", Ссылка);
	Статус = РегистрыСведений.cbr_СтатусыЗадач.ПолучитьПоследнее(, ПараметрыРегистра).Статус;
	
	Если ЗначениеЗаполнено(Статус) Тогда
		ЭтоНовая = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Статус, "Новый");
		ЭтоЗакрыта = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Статус, "Закрыт");
		Возврат Новый Структура("Статус,Новая,Закрыта", Статус, ЭтоНовая, ЭтоЗакрыта);
	Иначе
...
Процедура ДобавитьНаблюдателяОтИсполнителя(ЭтаФорма) Экспорт
	...
	Если ТипЗнч(ЭтаФорма.Исполнитель) = Тип("СправочникСсылка.Пользователи") Тогда
		...
		Если ЗначениеЗаполнено(Подразделение) Тогда
			...
			Если Выборка.Следующий() И ЗначениеЗаполнено(НаблюдательФизЛицо) Тогда
...
//	Возвращает руководителя пользователя
//	Параметры:
//		Пользователь - СправочникСсылка.Пользователи
//	Возвращаемое значение:
//		СправочникСсылка.Пользователи - Руководитель
Функция РуководительПользователя(Пользователь) Экспорт
	Руководитель = Справочники.Пользователи.ПустаяСсылка();
	
	Подразделение = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Пользователь, "Подразделение");
	
	Если ЗначениеЗаполнено(Подразделение) Тогда
		РуководительФизЛицо = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Подразделение, "ТекущийРуководитель");
		...
		Запрос.УстановитьПараметр("ФизическоеЛицо", РуководительФизЛицо);
		Выборка = Запрос.Выполнить().Выбрать();
		
		Если Выборка.Следующий() И ЗначениеЗаполнено(РуководительФизЛицо) Тогда
			Руководитель = Выборка.Ссылка;
		КонецЕсли;
	КонецЕсли;
