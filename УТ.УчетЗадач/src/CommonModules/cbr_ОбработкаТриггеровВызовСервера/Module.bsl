...
Процедура ВызовТриггера(Подписка, Задача = Неопределено, Пользователь = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		Попытка
			 ВыполнениеЗапросаИзСправочника(Выборка, Задача, Пользователь);		
		Исключение
	...
	cbr_Битрикс24.ДобавитьДействиеВОчередь(Задача, Подписка);
	
КонецПроцедуры

Процедура ВыполнениеЗапросаИзСправочника(СправочникКонтроля, Задача, Пользователь)
	Запрос = Новый Запрос;
