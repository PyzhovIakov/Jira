...
Процедура ВызовТриггера(Подписка, Задача = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			ТекстОшибки = СтрШаблон("Ошибка выполнения триггера %1", Выборка.Наименование);
			ОбщегоНазначения.СообщитьПользователю(ТекстОшибки);
		КонецПопытки;
	КонецЦикла;
	
	Если Подписка = Перечисления.cbr_ПодпискаДляТриггера.ПриСозданииНаСервереФормыЗадачи Тогда
		Возврат;
		
	ИначеЕсли Подписка = Перечисления.cbr_ПодпискаДляТриггера.СозданиеВопросаПоЗадаче Тогда
		cbr_Битрикс24.СоздатьКомментарийКЗадачеИзВопроса(Задача);
		
	ИначеЕсли Подписка = Перечисления.cbr_ПодпискаДляТриггера.ОтветНаВопросПоЗадаче Тогда 
		cbr_Битрикс24.СоздатьКомментарийКЗадачеИзВопроса(Задача);
		
	ИначеЕсли Подписка = Перечисления.cbr_ПодпискаДляТриггера.ПослеЗаписиНаСервереФормаЗадачи Тогда 
		Возврат;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СправочникКонтроля.Уведомлять И Выборка.Владелец().Колонки.Найти("Пользователь") <> Неопределено Тогда
			Если ЗначениеЗаполнено(Выборка.Пользователь) Тогда
				УведомитьПользователя(Выборка, СправочникКонтроля);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура УведомитьПользователя(Выборка, СправочникКонтроля)
	ТекущаяДата = ТекущаяДатаСеанса();
	...
	СвойстваУведомления.Вставить("Пользователь", Выборка.Пользователь);
	...
	СвойстваУведомления.Вставить("Источник", Выборка.Ссылка);
	СвойстваУведомления.Вставить("Идентификатор", "");
	
	ПринудительныеУведомления = Новый Структура;
	ПринудительныеУведомления.Вставить("База", Ложь);
