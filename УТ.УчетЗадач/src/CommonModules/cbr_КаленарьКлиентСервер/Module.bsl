...
Функция ПривестиКМестномуВремени(ДатаСеанса, ПараметрыПреобразования) Экспорт
	...
	Если ПараметрыПреобразования.РежимОтображенияМестногоВремени = ПредопределенноеЗначение(
		"Перечисление.РежимыОтображенияМестногоВремени.ВремяСеанса") Тогда
		Возврат ДатаСеанса;
	КонецЕсли;
	...
	ИначеЕсли НастройкиОтображения.ОтображатьЗанятость Тогда
		...
		Для Каждого ФизЛицо Из НастройкиОтображения.ФизЛица Цикл
			...
			Если НастройкиОтображения.ПредставленияИзмерений[ФизЛицо] <> Неопределено Тогда
				ЭлементИзмеренияПомещение.Текст = НастройкиОтображения.ПредставленияИзмерений[ФизЛицо];
			КонецЕсли;
...
Функция ПолучитьОтобразитьЗанятостьВДне(НастройкиОтображения) Экспорт
	...
	ПериодОтображения = ПолучитьПериодОтображения(НастройкиОтображения);
	ОтобразитьЗанятостьВДне = (НастройкиОтображения.ОтображатьЗанятость И ((ПериодОтображения
		= ПредопределенноеЗначение("Перечисление.cbr_ПериодОтображенияРабочегоКалендаря.День")) Или (ПериодОтображения
		= ПредопределенноеЗначение("Перечисление.cbr_ПериодОтображенияРабочегоКалендаря.ТриДня"))
		Или (ПериодОтображения = ПредопределенноеЗначение("Перечисление.cbr_ПериодОтображенияРабочегоКалендаря.Неделя"))
		Или (ПериодОтображения = ПредопределенноеЗначение(
